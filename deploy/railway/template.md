...
The probe exits `0` only when `ghcr.io/pdp-connect/pdpp/core:<version-tag>`
...
6. Run the live smoke in its default, read-only mode — this proves the
   hosted MCP endpoint refuses anonymous access and returns whatever records
   already exist, without ever registering a manifest, ingesting, or
   deleting anything. This is the only safe mode against this real project's
   real owner connection; do not pass `--disposable-env` here (it refuses to
   run unless the owner has zero pre-existing connections, and this project
   has one by this step):
...
7. Restart `core`, then rerun the same read-only command:
...
11. Deploy a fresh scratch project from the published template (no owner
    connection exists yet) and rerun the live smoke plus restart smoke, this
    time with `--disposable-env`, which seeds a deterministic record set and
    cleans it up before exiting — safe here specifically because this is a
    disposable scratch project, and the flag itself refuses to run if the
    owner unexpectedly already has a connection. Destroy this scratch project
    once validated; do not leave it running as a second live deploy.
...
## 2026-06-06 scratch proof (legacy historical record)

The following records the pre-rename publication evidence. It is historical
only; current instructions and publication gates above use `core`.
...
with `--disposable-env` (safe because this scratch project has no owner
connection yet), restart `core`, and rerun the smoke in its default read-only
mode to prove the seeded records and owner login survived the restart.
Destroy the scratch project once validated.
