...
- 2026-05-27: Fixed the native-provider read binding regression by keeping provider-native storage on its internal binding instead of resolving through the polyfill `connector_instances` fan-in store. The full `reference-implementation run test` suite now passes locally under a scrubbed owner-auth environment, so the reference CI gate was promoted from the focused subset to typecheck plus the full suite. Postgres-specific tests remain opt-in behind `PDPP_TEST_POSTGRES_URL`.
