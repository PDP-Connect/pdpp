...
**Status:** SUPERSEDED 2026-08-19, same day. The mechanism proposed below was
already built. Read "What was actually true" at the bottom before anything
else — the analysis above it is wrong in its central claim and is kept only
because the design reasoning about proof-vs-exemption still holds.
...
Everything above assumed `snapshot_import_receipt` was an unbuilt placeholder.
It is not. All three pieces exist and work, confirmed by probing the real
projection:

| receipt state | verdict |
|---|---|
| reconciles (299,248 / 299,248) | `complete` |
| claimed 120,042, ingested 90,000 | `partial` |
| no denominator | `unknown` |

The upload route writes `acquisition_batches` with `parsedCount` and
`receipt_json` (`ref-manual-upload-draft-connection.ts:867`, `:1115`). The
coherence rule already lists the strategy
(`reference-contract/src/evidence/coherence.ts`). The projection consumes it
through the one shared funnel (`ref-control.ts:3226`). No parallel path was
needed or wanted.

I concluded "unbuilt" from a placeholder constant without following it to its
consumers. The lesson is narrow and worth keeping: **a declared-but-unreferenced
constant is evidence about naming, not about behavior.** Grep the consumers
before writing a design note that says something does not exist.

### The real blocker

These two sources are red for a reason no receipt can fix. `mapCoverageAxis`
short-circuits on `if (!lastRun) return "unknown"`, and **the manual-upload
route never enqueues a collection run.** Both connectors' `collect()` already
emit proper `considered`/`covered` evidence (`google_maps/index.ts`
`finishPoints`/`finishSegments`; `whatsapp/index.ts:1256-1300`) — that code has
simply never run for these connections. The missing link is upload→run, not
receipt→projection.

### The open question the note got right for the wrong reason

No manual-upload format carries a self-claim. `estimated_points`,
`estimated_messages`, `estimated_records` are all PDPP's own full parse of the
bytes — true counts, but *our* numbers on both sides of the comparison. So
reconciling `parsed_count` against `accepted_count` detects ingest-side loss and
proves nothing about what the export omitted. WhatsApp's `media_coverage` is the
one real exception: it compares referenced-vs-included attachments, but only for
media.

So the open question at the top stands, and is sharper than stated: the
machinery exists, and for these formats there is still nothing external to
reconcile it against. Also worth noting: `duplicate_count` and `skipped_count`
are declared and projected in every receipt, and no code path writes them.
