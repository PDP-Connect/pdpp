...
- 2026-05-26: Implemented the narrow repair path via OpenSpec change `repair-record-version-noop-detection`. Postgres ingest now performs structural `jsonb` no-op detection, and an owner-only guarded repair script can refill registered derived fields from equivalent prior versions. The live `peregrine Codex` current `sessions` rows were repaired where safe, and the `sessions` connector cursor was seeded with per-thread fingerprints so fixed collector runs preserve those values.
- 2026-05-26: A live versions-per-current probe still showed large historical churn in several streams (`slack/workspace`, `gmail/threads`, `ynab` metadata, `codex` metadata). This is expected historical residue from earlier no-op behavior. Future adapter-level no-op suppression is fixed; historical compaction remains deferred because it is a destructive retention decision and needs a separate OpenSpec change with explicit owner acceptance.
