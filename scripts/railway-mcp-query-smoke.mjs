...
// ---------------------------------------------------------------------------
// Owner-session parsing (pure). The owner login flow is a CSRF-protected HTML
// form: GET the form to read the CSRF cookie + hidden field, then POST the
// password. We parse those out of raw headers/HTML exactly like the reference
// owner-auth test helpers do.
// ---------------------------------------------------------------------------

// Extract a single `name=value` cookie pair from a Set-Cookie header list.
export function findSetCookiePair(setCookies, name) {
...
// Read the hidden CSRF field the owner-auth form renders.
export function extractCsrfFieldValue(html) {
  const match = String(html).match(/<input type="hidden" name="_csrf" value="([^"]+)"\s*\/>/);
...
function getSetCookieList(resp) {
...
// Establish an owner session. When the deploy has no owner password (open
// local-dev), GET /owner/login still returns a usable form or a redirect and we
// proceed with no cookie; the device flow then mints a token directly.
async function establishOwnerSession(origin, ownerPassword, log) {
  ...
  const formResp = await fetch(`${origin}/owner/login`, {
    headers: { Accept: 'text/html' },
    redirect: 'manual',
  });
  const csrfCookie = findSetCookiePair(getSetCookieList(formResp), 'pdpp_owner_csrf');
  const csrfField = extractCsrfFieldValue(await formResp.text());
  if (!csrfCookie || !csrfField) {
    throw new SmokeError(
      'owner-login: could not read the CSRF cookie/field from /owner/login. ' +
        'Is owner auth enabled (PDPP_OWNER_PASSWORD set) and the origin correct?',
    );
  }
  const loginResp = await fetch(`${origin}/owner/login`, {
    method: 'POST',
    headers: {
  ...
  if (loginResp.status !== 302) {
    throw new SmokeError(
      `owner-login: expected 302 after POST /owner/login, got ${loginResp.status}. ` +
        'Wrong owner password?',
    );
  }
  const sessionCookie = findSetCookiePair(getSetCookieList(loginResp), 'pdpp_owner_session');
  if (!sessionCookie) {
    throw new SmokeError('owner-login: login did not issue a pdpp_owner_session cookie.');
  }
