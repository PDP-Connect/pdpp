...
function validCoreEnv(overrides: Partial<EnvMap> = {}): EnvMap {
  return {
...
test("non-HTTPS public origin is a violation", () => {
  const violations = evaluateFlyioCoreEnv(validCoreEnv({ PDPP_REFERENCE_ORIGIN: "http://pdpp-core.fly.dev" }));
  assert.equal(
...
test("forbids topology variables owned by the platform-core image", () => {
  ...
  assert.equal(
    violations.some((v) => v.includes("core AS_PORT must not be set")),
    true
  );
  assert.equal(
    violations.some((v) => v.includes("core RS_PORT must not be set")),
    true
  );
  assert.equal(
