...
const modeContexts: Record<CiMode, string[]> = {
...
function usage(): string {
  return `Usage:
  node scripts/ci-mode.ts status
  node scripts/ci-mode.ts hosted
  node scripts/ci-mode.ts local
  node scripts/ci-mode.ts signoff [--sha <sha>] [--description <text>] [--target-url <url>] [--base <ref>]
...
function runJson<T>(command: string, args: string[], options: RunOptions = {}): T | null {
...
function ghJson<T>(args: string[], options: RunOptions = {}): T | null {
  return runJson<T>("gh", args, options);
}
...
interface RequiredStatusCheckRule {
  parameters?: {
...
interface Ruleset {
  ...
  id: number;
  name: string;
  rules?: RequiredStatusCheckRule[];
  target?: unknown;
}

export function getRequiredStatusContexts(ruleset: Ruleset): string[] {
...
export function detectCiMode(contexts: string[]): CiMode | "custom" {
  ...
  for (const [mode, expected] of Object.entries(modeContexts)) {
    if (JSON.stringify(normalized) === JSON.stringify([...expected].sort())) {
      return mode as CiMode;
    }
  }
  return "custom";
}

export function rulesetWithRequiredStatusContexts(ruleset: Ruleset, contexts: string[]): Ruleset {
  let replaced = false;
...
export function workflowUpdatesForMode(
  workflows: Workflow[],
  mode: string,
  managedPaths: string[] = MANAGED_WORKFLOW_PATHS
): WorkflowUpdate[] {
  ...
  return managedPaths.map((path) => {
    ...
    if (workflow) {
      needsChange = mode === "hosted" ? workflow.state !== "active" : workflow.state === "active";
    }
    return {
      action,
...
function repoApiPath(suffix = ""): string {
  return `repos/:owner/:repo${suffix}`;
}

function getManagedWorkflowPaths(mode: CiMode, workflows: Workflow[]): string[] {
...
function loadRuleset(): Ruleset {
  ...
  if (configuredId) {
    const ruleset = ghJson<Ruleset>(["api", repoApiPath(`/rulesets/${configuredId}`)]);
    if (!ruleset) {
      throw new Error(`ruleset not found: ${configuredId}`);
    }
    return ruleset;
  }
  const name = process.env.PDPP_CI_RULESET_NAME || DEFAULT_RULESET_NAME;
  const rulesets = ghJson<Ruleset[]>(["api", repoApiPath("/rulesets")]) ?? [];
  const summary = rulesets.find((candidate) => candidate.name === name);
  if (!summary) {
    throw new Error(`ruleset not found: ${name}`);
  }
  const ruleset = ghJson<Ruleset>(["api", repoApiPath(`/rulesets/${summary.id}`)]);
  if (!ruleset) {
    throw new Error(`ruleset not found: ${name}`);
  }
  return ruleset;
...
function writeRuleset(ruleset: Ruleset, contexts: string[]): Ruleset {
  const body = JSON.stringify(rulesetWithRequiredStatusContexts(ruleset, contexts), null, 2);
  const updated = ghJson<Ruleset>(
    [
      "api",
      "--method",
      "PUT",
  ...
  if (!updated) {
    throw new Error(`ruleset update returned no body: ${ruleset.id}`);
  }
  return updated;
}

function loadWorkflows(): Workflow[] {
  const response = ghJson<{ workflows?: Workflow[] }>(["api", repoApiPath("/actions/workflows?per_page=100")]);
  return response?.workflows ?? [];
}

function applyManagedWorkflowMode(mode: CiMode): void {
...
function setMode(mode: CiMode): void {
...
function isCleanAndPushed(): boolean {
 ...
 * "nothing changed."
 */
function changedFilesAgainstBase(base: string): string[] {
...
function parseSignoffArgs(args: string[]): SignoffArgs {
  const out: SignoffArgs = {
    base: "origin/main",
  ...
  while (i < args.length) {
    ...
    if (arg === "--base") {
      i += 1;
      out.base = args[i] ?? out.base;
    } else if (arg === "--context") {
      i += 1;
      out.context = args[i] ?? out.context;
    } else if (arg === "--description") {
      i += 1;
      out.description = args[i] ?? out.description;
    } else if (arg === "--sha") {
      i += 1;
      out.sha = args[i] ?? null;
    } else if (arg === "--target-url") {
      i += 1;
      out.targetUrl = args[i] ?? null;
    } else {
...
function signoff(args: string[]): void {
  ...
  const body: { context: string; description: string; state: string; target_url?: string } = {
