...
} from "./check-railway-deploy-env.ts";

const repoRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");

// Railway reference-variable syntax (`${{...}}`), not JS template interpolation.
// biome-ignore lint/suspicious/noTemplateCurlyInString: literal Railway `${{...}}` reference-variable syntax, not JS interpolation.
const REFERENCE_AS_URL = ["http://", "${{reference.RAILWAY_PRIVATE_DOMAIN}}", ":", "${{reference.PORT}}"].join("");
// biome-ignore lint/suspicious/noTemplateCurlyInString: literal Railway `${{...}}` reference-variable syntax, not JS interpolation.
const REFERENCE_RS_URL = ["http://", "${{reference.RAILWAY_PRIVATE_DOMAIN}}", ":7663"].join("");
// biome-ignore lint/suspicious/noTemplateCurlyInString: literal Railway `${{...}}` reference-variable syntax, not JS interpolation.
const REFERENCE_CREDENTIAL_KEY = "${{reference.PDPP_CREDENTIAL_ENCRYPTION_KEY}}";
// biome-ignore lint/suspicious/noTemplateCurlyInString: literal Railway `${{...}}` reference-variable syntax, not JS interpolation.
const POSTGRES_DATABASE_URL = "${{Postgres.DATABASE_URL}}";
// biome-ignore lint/suspicious/noTemplateCurlyInString: literal Railway `${{...}}` reference-variable syntax, not JS interpolation.
const GENERATED_SECRET = "${{ secret(64) }}";
// biome-ignore lint/suspicious/noTemplateCurlyInString: literal Railway `${{...}}` reference-variable syntax, not JS interpolation.
const CONSOLE_PUBLIC_ORIGIN = "https://${{console.RAILWAY_PUBLIC_DOMAIN}}";
// biome-ignore lint/suspicious/noTemplateCurlyInString: literal Railway `${{...}}` reference-variable syntax, not JS interpolation.
const CORE_PUBLIC_ORIGIN = "https://${{core.RAILWAY_PUBLIC_DOMAIN}}";

// A fully-configured Postgres deploy that should satisfy the contract.
function validPostgresEnv(overrides: Partial<EnvMap> = {}): EnvMap {
  return {
    ...
    PDPP_AS_URL: REFERENCE_AS_URL,
    PDPP_RS_URL: REFERENCE_RS_URL,
    PDPP_DATABASE_URL: POSTGRES_DATABASE_URL,
    ...overrides,
  };
}

function validSqliteEnv(overrides: Partial<EnvMap> = {}): EnvMap {
  return {
    PDPP_REFERENCE_ORIGIN: "https://pdpp.example.com",
    PDPP_OWNER_PASSWORD: "s3cret-owner-pw",
    PDPP_CREDENTIAL_ENCRYPTION_KEY: "credential-key-provider-secret",
...
function validConsoleServiceEnv(overrides: Partial<EnvMap> = {}): EnvMap {
  return {
    PDPP_REFERENCE_ORIGIN: CONSOLE_PUBLIC_ORIGIN,
    PDPP_OWNER_PASSWORD: "s3cret-owner-pw",
    PDPP_CREDENTIAL_ENCRYPTION_KEY: REFERENCE_CREDENTIAL_KEY,
    PDPP_AS_URL: REFERENCE_AS_URL,
    PDPP_RS_URL: REFERENCE_RS_URL,
    ...overrides,
  };
...
function validReferenceServiceEnv(overrides: Partial<EnvMap> = {}): EnvMap {
  return {
    PDPP_REFERENCE_ORIGIN: CONSOLE_PUBLIC_ORIGIN,
    PDPP_OWNER_PASSWORD: "s3cret-owner-pw",
    PDPP_CREDENTIAL_ENCRYPTION_KEY: GENERATED_SECRET,
    PDPP_DATABASE_URL: POSTGRES_DATABASE_URL,
    ...overrides,
  };
}

function validCoreServiceEnv(overrides: Partial<EnvMap> = {}): EnvMap {
  return {
    PDPP_REFERENCE_ORIGIN: CORE_PUBLIC_ORIGIN,
    PDPP_OWNER_PASSWORD: "s3cret-owner-pw",
    PDPP_CREDENTIAL_ENCRYPTION_KEY: GENERATED_SECRET,
    PDPP_DATABASE_URL: POSTGRES_DATABASE_URL,
    ...overrides,
...
test("isRailwayReference recognizes Railway reference-variable bindings only", () => {
  assert.equal(isRailwayReference(POSTGRES_DATABASE_URL), true);
  assert.equal(isRailwayReference(REFERENCE_AS_URL), true);
...
test("parseEnv ignores comments and blanks and strips quotes", () => {
  ...
  assert.equal(env.PDPP_AS_URL, REFERENCE_AS_URL);
...
test("non-HTTPS public origin is a violation", () => {
  const violations = evaluateRailwayDeployEnv(validPostgresEnv({ PDPP_REFERENCE_ORIGIN: "http://pdpp.example.com" }));
  assert.equal(
    violations.some((v) => v.includes("must be an https:// origin")),
    true
  );
});
...
test("missing credential key provider is a violation", () => {
  ...
  assert.equal(
    violations.some((v) => v.includes("PDPP_CREDENTIAL_ENCRYPTION_KEY")),
    true
  );
});
...
test("missing private AS/RS targets are violations", () => {
  const violations = evaluateRailwayDeployEnv(validPostgresEnv({ PDPP_AS_URL: "", PDPP_RS_URL: "" }));
  assert.equal(
    violations.some((v) => v.includes("PDPP_AS_URL is not set")),
    true
  );
  assert.equal(
    violations.some((v) => v.includes("PDPP_RS_URL is not set")),
    true
  );
});
...
test("postgres storage without a database URL is a violation", () => {
  ...
  assert.equal(
    violations.some((v) => v.includes("requires PDPP_DATABASE_URL")),
    true
  );
});

test("sqlite backend at the unmounted default path is a violation", () => {
  const violations = evaluateRailwayDeployEnv(validSqliteEnv({ PDPP_DB_PATH: UNMOUNTED_SQLITE_DEFAULT }));
  assert.equal(
    violations.some((v) => v.includes("mounted persistent volume")),
    true
  );
});
...
test("missing database URL and storage backend is a violation", () => {
  ...
  assert.equal(
    unset.some((v) => v.includes("or PDPP_DATABASE_URL must be set")),
    true
  );
});
...
test("unknown storage backend is a violation", () => {
  ...
  assert.equal(
    unknown.some((v) => v.includes('must be "postgres" or "sqlite"')),
    true
  );
});
...
test("core service env rejects topology variables owned by the image", () => {
  ...
  assert.equal(
    violations.some((v) => v.includes("core PORT must not be set")),
    true
  );
  assert.equal(
    violations.some((v) => v.includes("core PDPP_AS_URL must not be set")),
    true
  );
  assert.equal(
    violations.some((v) => v.includes("core PDPP_RS_URL must not be set")),
    true
  );
});
...
test("service env preflight rejects setting PORT on the console service", () => {
  ...
  assert.equal(
    violations.some((v) => v.includes("console PORT must not be set")),
    true
  );
});
