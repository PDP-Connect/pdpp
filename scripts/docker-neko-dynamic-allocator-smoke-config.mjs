...
export function deriveDynamicNekoSmokeConfig(env, runId) {
  ...
  return {
    ...
    surfaceA,
    surfaceB,
    cleanupFilters: (surfaceId) => [`label=${DEPLOYMENT_LABEL}=${deploymentId}`, `label=${SURFACE_LABEL}=${surfaceId}`],
  };
}
...
if (import.meta.main) {
  ...
  process.stdout.write(
    [config.projectName, config.profileRoot, config.deploymentId, config.surfaceA, config.surfaceB].join("\n")
  );
}
