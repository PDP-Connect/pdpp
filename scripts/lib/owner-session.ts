...
export interface OwnerAuthHeader {
  cookie?: string;
}
export interface OwnerAuthResult<Mode extends string> {
  error: string | null;
  header: OwnerAuthHeader;
  mode: Mode;
}

export function getSetCookieList(res: Pick<ResponseLike, "headers">): string[] {
...
export function findSetCookiePair(cookies: string[], name: string): string | null {
  const prefix = `${name}=`;
...
export function extractCsrfFieldValue(html: string): string | null {
  return String(html).match(CSRF_FIELD_PATTERN)?.[1] ?? null;
}
 ...
 */
export async function loginWithOwnerPassword({
  base,
  password,
  fetchImpl,
}: {
  ...
  password: string;
  fetchImpl: FetchImpl;
}): Promise<OwnerAuthResult<"password-session">> {
 ...
 */
export function resolveOwnerAuthForLive({
  base,
  env,
  fetchImpl,
}: {
  ...
  fetchImpl: FetchImpl;
}): Promise<OwnerAuthResult<"cookie" | "password-session" | "none">> {
 ...
 */
export async function establishOwnerSessionCookie({
  origin,
  ownerPassword,
  fetchImpl = fetch,
}: {
