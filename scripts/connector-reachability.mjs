 ...
 */
const TARGETS = [
  { connector: "steam", name: "ISteamUser/GetPlayerSummaries", url: "https://api.steampowered.com/ISteamUser/GetPlayerSummaries/v0002/", expect: "refused" },
  { connector: "steam", name: "IPlayerService/GetOwnedGames", url: "https://api.steampowered.com/IPlayerService/GetOwnedGames/v0001/", expect: "refused" },
  ...
  { connector: "oura", name: "v2 personal_info", url: "https://api.ouraring.com/v2/usercollection/personal_info", expect: "refused" },
  ...
  { connector: "reddit", name: "api/v1/me", url: "https://oauth.reddit.com/api/v1/me", expect: "refused" },
  {
    connector: "jellyfin",
...
function classify(target, status) {
  if (status === 0) return { verdict: "UNREACHABLE", note: "network error or timeout — inconclusive, not a failure" };
  if (status === 404) {
  ...
  if (status >= 500) return { verdict: "INFO", note: `${status} — provider-side error, inconclusive` };
