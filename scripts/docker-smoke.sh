...
dashboard_headers="$(curl -sS -D - -o /dev/null "$ORIGIN/dashboard")"
dashboard_status="$(printf '%s\n' "$dashboard_headers" | awk 'NR == 1 { print $2 }')"
dashboard_location="$(printf '%s\n' "$dashboard_headers" | awk 'tolower($1) == "location:" { print $2; exit }' | tr -d '\r')"
case "$dashboard_status" in
  303|307|308) ;;
  *)
    echo "Expected /dashboard to redirect with owner auth enabled; got status: $dashboard_status" >&2
    exit 1
    ;;
esac
case "$dashboard_location" in
  ...
  *)
    echo "Expected /dashboard to redirect to /owner/login; got location: $dashboard_location" >&2
    exit 1
    ;;
esac
