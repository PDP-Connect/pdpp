...
//   node scripts/check-flyio-deploy-env.ts --core core.env --json
...
export function evaluateFlyioCoreEnv(coreEnv: EnvMap): string[] {
...
function loadEnvFile(path: string): EnvMap {
  try {
    ...
    process.stderr.write(`Error reading env file "${path}": ${message}\n`);
    process.exit(1);
  }
}
...
function main(): void {
  ...
  if (!corePath) {
    process.stderr.write("Usage: node scripts/check-flyio-deploy-env.ts --core <env-file> [--json]\n");
    process.exit(1);
  }
