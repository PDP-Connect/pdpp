...
The older single-purpose script under `packages/polyfill-connectors/bin/local-device-exporter.ts` is still present as a compatibility shim. It does not participate in STATE sync and is being retired by `introduce-local-collector-runner`. Prefer the `@pdpp/local-collector` flow above.
...
- The public/source-instance vocabulary remains an open protocol question owned outside the RI.
