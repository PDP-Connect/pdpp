 ...
 *
 * @param {object} manifestStream - Stream metadata from manifest
 ...
 *
 * @param {object} sqliteHandle - SQLite database handle
 * @returns {Map<string, object>}
 */
export function loadConnectorManifests(sqliteHandle) {
  ...
  for (const row of rows) {
    try {
      const parsed = typeof row.manifest === 'string'
        ? JSON.parse(row.manifest)
        : row.manifest;
      manifests.set(row.connector_id, parsed);
    } catch (err) {
      console.warn(`Failed to parse manifest for connector ${row.connector_id}:`, err.message);
    }
  }
 ...
 * @param {string} streamName - Name of the stream to find
 * @returns {object|null} - The stream object, or null if not found
 */
export function getStreamFromManifest(manifest, streamName) {
 ...
 *
 * @param {object} streamMeta - Stream metadata from manifest
 * @param {string|object} recordJson - JSON data (string or parsed object)
 * @param {string} recordKey - The record_key value from the database
 * @returns {string} - Non-empty string value for primary_key_text column
 * @throws {Error} If JSON parsing fails (indicates corrupt record_json in source)
 */
export function derivePrimaryKeyText(streamMeta, recordJson, recordKey) {
  ...
  if (typeof recordJson === 'string') {
    try {
      ...
      throw new Error(`derivePrimaryKeyText: failed to parse record_json: ${err.message}`);
    }
  } else {
    data = recordJson;
  }
 ...
 * @throws {Error} If JSON parsing fails (indicates corrupt record_json in source)
 */
export function deriveCursorValue(streamMeta, recordJson) {
  ...
  if (typeof recordJson === 'string') {
    try {
      ...
      // Re-throw JSON parse errors so they bubble up to the caller
      // This indicates a corrupt record in the source database
      throw new Error(`deriveCursorValue: failed to parse record_json: ${err.message}`);
    }
  } else {
    data = recordJson;
  }
