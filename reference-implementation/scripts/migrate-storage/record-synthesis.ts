...
export function derivePrimaryKeyText(
  ...
  if (typeof recordJson === "string") {
    try {
      ...
      throw new Error(
        `derivePrimaryKeyText: failed to parse record_json: ${err instanceof Error ? err.message : String(err)}`,
        {
          cause: err,
        }
      );
...
export function deriveCursorValue(
  ...
  if (typeof recordJson === "string") {
    try {
      ...
      // This indicates a corrupt record in the source database
      throw new Error(
        `deriveCursorValue: failed to parse record_json: ${err instanceof Error ? err.message : String(err)}`,
        {
          cause: err,
        }
      );
