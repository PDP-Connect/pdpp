...
import { readPinnedBiomeVersion } from "./measure-mass.ts";
import { regenerateMassBaseline } from "./regenerate-mass-baseline.ts";
...
test("regenerateMassBaseline measures the real workspace and writes a fingerprinted baseline", async () => {
  ...
  try {
    ...
    assert.equal(fingerprint.biomeVersion, await readPinnedBiomeVersion());
    ...
    assert.equal(
      written.total,
      Object.values(files).reduce((sum, mass) => sum + mass, 0)
    );
  } finally {
