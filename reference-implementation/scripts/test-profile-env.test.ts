 ...
 *
 * assertUngatedProfileIsExplicit() is the narrow seam `run-tests.ts` calls
 * before selecting a profile when no accounting authority is bound. These
...
test("an absent PDPP_TEST_PROFILE hard-fails even when a PostgreSQL URL is supplied", () => {
  ...
  assert.throws(() => assertUngatedProfileIsExplicit(env), PROFILE_MUST_BE_SET_PATTERN);
});
...
test("explicit memory-default remains valid with no PostgreSQL URL required", () => {
  ...
  assert.doesNotThrow(() => assertUngatedProfileIsExplicit(env));
});

test("explicit postgres without a URL hard-fails", () => {
  const env: ProcessEnvLike = { PDPP_TEST_PROFILE: "postgres" };
  assert.throws(() => assertUngatedProfileIsExplicit(env), POSTGRES_REQUIRES_URL_PATTERN);
});
