...
/** A child-process environment map, matching Node's `process.env` shape. */
export type ProcessEnvLike = Record<string, string | undefined>;

export function buildScrubbedTestEnv(sourceEnv: ProcessEnvLike = process.env): ProcessEnvLike {
  const denied = new Set<string>(TEST_ENV_DENYLIST);
  const scrubbed: ProcessEnvLike = {};
