 ...
 *
 * Usage:
 *   node reference-implementation/scripts/repair/connector-cursor-reset.ts \
 ...
 */
export function backupTableName({ connectorInstanceId, streams, stamp }: BackupTableNameInput): string {
...
export interface ParsedCursorResetArgs {
  apply: boolean;
 ...
 */
export function parseArgs(argv: string[]): ParsedCursorResetArgs {
  const out: ParsedCursorResetArgs = { apply: false, connectorInstanceId: null, streams: [] };
...
export function validateArgs({
  connectorInstanceId,
  streams,
}: Pick<ParsedCursorResetArgs, "connectorInstanceId" | "streams">): string | null {
...
export interface RunCursorResetInput {
  apply: boolean;
  connectorInstanceId: string;
  pool: pg.Pool;
  stamp: string;
...
export interface CursorResetResult {
  absent: string[];
  applied: boolean;
 ...
 *   }
 */
export async function runCursorReset({
  pool,
  connectorInstanceId,
  streams,
  apply,
  stamp,
}: RunCursorResetInput): Promise<CursorResetResult> {
  // Which targeted streams actually have a stored cursor today.
  const presentResult = await pool.query<{ stream: string }>(
  ...
  const result: CursorResetResult = {
    absent,
    applied: !!apply,
    backupTable: null,
    connectorInstanceId,
  ...
  const backupTable = backupTableName({ connectorInstanceId, stamp, streams: present });
...
export function formatSummary(result: CursorResetResult): string {
  const lines: string[] = [];
...
async function runCli(): Promise<void> {
  ...
  const { connectorInstanceId } = args;
  if (!connectorInstanceId) {
