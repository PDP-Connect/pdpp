 ...
 * `record-current-projection-repair.ts`: that tool fixes one stream; this one
 ...
 *
 * Usage:
 *   node reference-implementation/scripts/repair/record-current-projection-scan-all.ts \
 *     [--connector-id=<id>] \
...
export const SCAN_DRIFT_KINDS = Object.freeze({
  ...
  UNVERIFIED_CURRENT_PAYLOAD_DIFFERS_FROM_LATEST_HISTORY: "unverified_current_payload_differs_from_latest_history",
} as const);

export type ScanDriftKind = (typeof SCAN_DRIFT_KINDS)[keyof typeof SCAN_DRIFT_KINDS];
...
export const REMEDIATION_BY_KIND: Record<ScanDriftKind, RemediationDisposition> = Object.freeze({
  [SCAN_DRIFT_KINDS.MISSING_CURRENT]: "repairable_from_latest_retained_history",
  [SCAN_DRIFT_KINDS.STALE_CURRENT]: "repairable_from_latest_retained_history",
  [SCAN_DRIFT_KINDS.LATEST_DELETED]: "owner_gated_delete_reconciliation",
  [SCAN_DRIFT_KINDS.CURRENT_PAYLOAD_MATCHES_LATEST_HISTORY_BUT_VERSION_DIFFERS]: "safe_current_version_correction",
  [SCAN_DRIFT_KINDS.UNVERIFIED_CURRENT_PAYLOAD_DIFFERS_FROM_LATEST_HISTORY]: "source_resync_required",
  [SCAN_DRIFT_KINDS.CURRENT_VERSION_NEWER_THAN_RETAINED_HISTORY]: "source_resync_or_owner_gated_synthetic_anchor",
  [SCAN_DRIFT_KINDS.CURRENT_NO_RETAINED_HISTORY]: "source_resync_or_owner_gated_synthetic_anchor",
});
...
export interface ScanDriftHistory {
  hasRetained: boolean;
  jsonEqualToCurrent: boolean;
  latestDeleted: boolean;
  latestVersion: number | null;
}

export interface ScanDriftCurrent {
  deleted: boolean;
  version: number;
}
 ...
 */
export function classifyScanDrift(history: ScanDriftHistory, current: ScanDriftCurrent | null): ScanDriftKind | null {
...
export function emptyCounts(): Record<ScanDriftKind, number> {
  const out = {} as Record<ScanDriftKind, number>;
...
export interface ScanDriftRow {
  ...
  payloadEqualToLatestHistory: boolean | null;
  recordKey: string;
...
export interface ScanAllResult {
  counts: Record<ScanDriftKind, number>;
  rows: ScanDriftRow[];
  totalDrift: number;
}
...
interface ScanQueryRow {
  ...
  json_equal: boolean;
  latest_history_deleted: boolean | null;
  latest_history_version: string | number | null;
  record_key: string;
  stream: string;
}
 ...
 */
export async function scanAll({ pool, connectorId, limitPerStream }: ScanAllInput): Promise<ScanAllResult> {
  const params: string[] = [];
  let connectorFilter = "";
  if (connectorId) {
  ...
  const result = await pool.query<ScanQueryRow>(sql, params);

  const counts = emptyCounts();
  const perStreamShown = new Map<string, number>();
  const rows: ScanDriftRow[] = [];
  let totalDrift = 0;

  for (const row of result.rows) {
    const classified = classifyScanQueryRow(row);
    if (!classified) {
      continue;
    }
    ...
    if (limitPerStream && shown >= limitPerStream) {
      continue;
    }
    perStreamShown.set(streamKey, shown + 1);
    rows.push(classified.previewRow);
  }

  return { counts, rows, totalDrift };
}

/**
 * Classify one raw query row and build its preview row, or return `null` when
 * the key is consistent (no drift). Extracted from `scanAll`'s loop purely to
 * keep that loop's own cognitive complexity under Biome's budget -- the
 * classification and preview-shape logic is unchanged from the inline version.
 */
function classifyScanQueryRow(row: ScanQueryRow): { kind: ScanDriftKind; previewRow: ScanDriftRow } | null {
  const history: ScanDriftHistory = {
  ...
  const current: ScanDriftCurrent | null =
...
export function formatScanSummary({ counts, totalDrift, rows }: ScanAllResult): string {
  const lines: string[] = [];
...
export function parsePositiveIntOrNull(raw: string | boolean | undefined): number | null | "invalid" {
  if (raw === null || raw === "") {
    return null;
  }
...
async function runCli(): Promise<void> {
  ...
  let exitCode = 0;
  try {
    const result = await scanAll({ connectorId, limitPerStream, pool });
    if (asJson) {
      // JSON is payload-free by construction: previews carry only metadata.
      console.log(JSON.stringify({ counts: result.counts, rows: result.rows, totalDrift: result.totalDrift }, null, 2));
    } else {
