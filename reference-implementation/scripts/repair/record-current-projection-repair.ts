 ...
 *
 * Usage:
 *   node reference-implementation/scripts/repair/record-current-projection-repair.ts \
...
export type MismatchKind = (typeof MISMATCH_KINDS)[keyof typeof MISMATCH_KINDS];
 ...
 */
export function truncateKey(key: unknown): string {
  const s = String(key ?? "");
  if (s.length <= 16) {
    return s;
  }
 ...
 */
export function backupTableName({ connectorInstanceId, stream, stamp }: BackupTableNameInput): string {
...
export interface LatestHistoryRow {
  deleted: boolean;
  jsonEqual: boolean;
  version: number;
}
 ...
 */
export function classifyMismatch(latest: LatestHistoryRow | null, current: CurrentRow | null): MismatchKind | null {
  if (!latest) {
    return null;
  }
  ...
  if (latest.deleted) {
    ...
    if (current && !current.deleted) {
      return MISMATCH_KINDS.LATEST_DELETED;
    }
    return null;
  }
...
async function runCli(): Promise<void> {
  ...
  const recordKeyArg = args["record-key"];
  const recordKey = typeof recordKeyArg === "string" ? recordKeyArg : null;
 ...
 */
export function parseLimit(raw: string | boolean | undefined): number | null | "invalid" {
...
interface LatestHistoryQueryRow {
  current_deleted: boolean | null;
  current_exists: boolean;
  current_json_bytes: string | number;
  current_version: string | number | null;
  json_equal: boolean;
  latest_history_deleted: boolean;
  latest_history_json_bytes: string | number;
  latest_history_version: string | number;
  record_key: string;
}
...
export async function detectMismatches({
  ...
  recordKey,
  limit,
}: DetectMismatchesInput): Promise<MismatchPreview[]> {
  const params: Array<string | number> = [connectorInstanceId, stream];
  let keyFilter = "";
  if (recordKey) {
  ...
  const sql = `
    ...
    SELECT lh.record_key,
     ...
     ${limitClause}
  `;
  const rows = await pool.query<LatestHistoryQueryRow>(sql, params);

  const previews: MismatchPreview[] = [];
  for (const row of rows.rows) {
    const latest: LatestHistoryRow = {
      deleted: row.latest_history_deleted === true,
      jsonEqual: row.json_equal === true,
      version: Number(row.latest_history_version),
    };
    const current: CurrentRow | null = row.current_exists
      ? { deleted: row.current_deleted === true, version: Number(row.current_version) }
      : null;
  ...
  const orphanParams: Array<string | number> = [connectorInstanceId, stream];
  let orphanKeyFilter = "";
  if (recordKey) {
  ...
  const orphans = await pool.query<OrphanQueryRow>(
    `SELECT r.record_key,
            r.version AS current_version,
            r.deleted AS current_deleted,
            COALESCE(octet_length(r.record_json::text), 0)::bigint AS current_json_bytes
...
export interface RepairResult {
  affected: { inserted: number; updated: number; deleted: number };
  applied: boolean;
  applyDeletes: boolean;
  ...
  error?: string;
  failed: boolean;
  previews: MismatchPreview[];
  repairableCount: number;
  retainedSizeDirtyError?: string;
  retainedSizeDirtyMarked: boolean;
  stream: string;
}
...
export async function runRepair({
  ...
  applyDeletes,
  stamp,
}: RunRepairInput): Promise<RepairResult> {
  ...
  const result: RepairResult = {
    affected: { deleted: 0, inserted: 0, updated: 0 },
    applied: !!apply,
  ...
  const backupTable = backupTableName({ connectorInstanceId, stamp, stream });
  ...
  try {
    ...
    for (const p of repairable) {
      if (p.kind === MISMATCH_KINDS.LATEST_DELETED) {
        // biome-ignore lint/performance/noAwaitInLoops: all repairs in this loop share ONE pg.PoolClient inside a single BEGIN/COMMIT transaction -- a pg client serializes queries on one connection regardless, and parallelizing here would not overlap I/O, only reorder statements inside the transaction non-deterministically.
        await applyDeleteRepair({ client, connectorInstanceId, preview: p, stream });
        ...
        if (outcome === "inserted") {
          result.affected.inserted += 1;
        } else {
          result.affected.updated += 1;
        }
      }
    }
  ...
  try {
    await markRetainedSizeStreamDirtyDirect({ connectorInstanceId, pool, stream });
    ...
    // committed) repair.
    result.retainedSizeDirtyError = String(err && typeof err === "object" && "message" in err ? err.message : err);
  }

  return result;
}

export function isRepairable(preview: MismatchPreview, applyDeletes: boolean): boolean {
  if (preview.kind === MISMATCH_KINDS.MISSING_CURRENT) {
    return true;
  }
  if (preview.kind === MISMATCH_KINDS.STALE_CURRENT) {
    return true;
  }
  if (preview.kind === MISMATCH_KINDS.LATEST_DELETED) {
    return !!applyDeletes;
  }
  return false; // unresolved_pruned is never repairable
}

export function countByKind(previews: MismatchPreview[]): Record<MismatchKind, number> {
  const out: Record<MismatchKind, number> = {
...
interface ApplyProjectionRepairInput {
 ...
 */
export async function applyProjectionRepair({
  client,
  connectorInstanceId,
  stream,
  preview,
}: ApplyProjectionRepairInput): Promise<"inserted" | "updated"> {
...
interface ApplyDeleteRepairInput {
  client: pg.PoolClient;
  connectorInstanceId: string;
  preview: MismatchPreview;
  stream: string;
 ...
 */
export async function applyDeleteRepair({
  client,
  connectorInstanceId,
  stream,
  preview,
}: ApplyDeleteRepairInput): Promise<void> {
  const { recordKey, latestHistoryVersion } = preview;
  ...
  if (res.rowCount === 0) {
    throw new Error(`delete repair for version ${latestHistoryVersion} is no longer safe to project at apply time`);
  }
}

interface MarkRetainedSizeStreamDirtyDirectInput {
  connectorInstanceId: string;
  pool: pg.Pool;
  stream: string;
}
 ...
 * error.
 */
async function markRetainedSizeStreamDirtyDirect({
  pool,
  connectorInstanceId,
  stream,
}: MarkRetainedSizeStreamDirtyDirectInput): Promise<void> {
  const exists = await pool.query<RegclassRow>(
  ...
  const reg = exists.rows[0] ?? { c: null, g: null, s: null };
  if (reg.s) {
    await pool.query("UPDATE retained_size_stream SET dirty = 1 WHERE connector_instance_id = $1 AND stream = $2", [
      connectorInstanceId,
      stream,
    ]);
  }
  if (reg.c) {
    await pool.query("UPDATE retained_size_connection SET dirty = 1 WHERE connector_instance_id = $1", [
      connectorInstanceId,
    ]);
  }
  if (reg.g) {
    await pool.query(`UPDATE retained_size_global SET dirty = 1 WHERE projection_key = 'global'`);
  }
}
...
function printSummary(result: RepairResult): void {
  ...
  console.log(
    `  repairable this mode: ${result.repairableCount}` +
      (result.applyDeletes
        ? " (deletes enabled)"
        : " (deletes NOT enabled; pass --apply-deletes to reconcile latest_deleted)")
  );
  ...
  if (result.applied) {
    ...
    if (result.failed) {
      ...
      console.log(
        "    — or call rebuildRetainedSize() from reference-implementation/server/retained-size-read-model.js"
      );
    } else if (result.retainedSizeDirtyError) {
      console.log(
        `  retained-size read model: dirty-mark failed (${result.retainedSizeDirtyError}); rebuild still required`
      );
    }
