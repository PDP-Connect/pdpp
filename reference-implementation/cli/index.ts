...
import { runOwner } from "./commands/owner.ts";
...
export function legacyAliasHint(group: unknown, sub: unknown): string | null {
...
function isKnownCommand(group: string): group is keyof typeof COMMANDS {
  return Object.hasOwn(COMMANDS, group);
}
...
async function main(): Promise<void> {
  ...
  if (PUBLIC_DELEGATED_COMMANDS.has(group)) {
    const exitCode = await runPublicCli(argv, { stderr: process.stderr, stdout: process.stdout });
  ...
  if (canonical) {
    process.stderr.write(`warning: "pdpp ${group} ${rest[0]}" is deprecated; use "${canonical}" instead.\n`);
  }

  if (!isKnownCommand(group)) {
    throw new PdppUsageError(`Unknown command group: ${group}\n\n${HELP}`);
  }

  await COMMANDS[group](rest);
}

export const __test = { isCliEntryPoint, LEGACY_ALIAS_HINTS, legacyAliasHint, PUBLIC_DELEGATED_COMMANDS };

function isCliEntryPoint(invoked: string | undefined = process.argv[1], moduleUrl: string = import.meta.url): boolean {
  try {
    if (!invoked) {
      return false;
    }
