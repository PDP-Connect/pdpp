...
/**
 * Real lifecycle evidence for `retired`. Omit (pass `null`) when the caller
 * has no instance-level lifecycle row in scope (e.g. the connector-keyed
 * detail path, which has no instance row) — `retired` is then unreachable
 * rather than inferred from health shape.
 */
export interface OwnerStateLifecycle {
  readonly status: "active" | "revoked" | string;
}
 ...
 * pairs with `resolver: "not_measured"`/`"healthy"` (never-run), which is
 * already the honest "nothing to freeze" case.
 */
