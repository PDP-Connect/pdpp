 ...
 */
function actionTargetForInteractionKind(kind) {
  switch (kind) {
    case 'manual_action':
...
function ownerActionForInteractionKind(kind) {
  return kind === 'manual_action' ? 'operate_attachment' : 'provide_value';
}

function sensitivityForInteractionKind(kind) {
  return kind === 'credentials' || kind === 'otp' ? 'secret' : 'non_secret';
}

function reasonCodeForInteractionKind(kind) {
  switch (kind) {
    case 'credentials':
 ...
 */
function lifecycleForTerminalStatus(status) {
  switch (status) {
    case 'success':
    case 'resolved':
      return 'resolved';
 ...
 */
function actionTargetForAssistance(msg) {
  const owner = msg.owner_action;
  if (Array.isArray(msg.attachments) && msg.attachments.some((a) => a && a.kind === 'browser_surface')) {
    return 'remote_surface';
  }
  if (owner === 'act_elsewhere') return 'external_app';
  if (owner === 'operate_attachment') return 'remote_surface';
  if (owner === 'provide_value') return 'dashboard';
  return 'dashboard';
}

function sensitivityForAssistance(msg) {
  if (msg.sensitivity === 'secret') return 'secret';
  return 'non_secret';
}

function expiresAtFromTimeout(nowMs, timeoutSeconds) {
  if (!Number.isFinite(timeoutSeconds) || timeoutSeconds <= 0) return null;
  return new Date(nowMs + timeoutSeconds * 1000).toISOString();
}
 ...
 */
export function createAttentionWriter(opts) {
  ...
  const makeAttentionId = typeof opts.makeAttentionId === 'function'
    ? opts.makeAttentionId
    : (requestId) => `att_${runId}_${requestId}`;
  ...
  function untrack(attentionId) {
    const entry = open.get(attentionId);
    open.delete(attentionId);
    if (entry?.requestId) byRequestId.delete(entry.requestId);
  }

  async function safeUpsert(record) {
    try {
  ...
  async function safeTransition(attentionId, to) {
    try {
  ...
  return {
     ...
     */
    async recordInteractionRequest(msg) {
     ...
     */
    async recordAssistanceRequest(msg) {
     ...
     * drained).
     */
    async resolveByRequestId(requestId, status) {
