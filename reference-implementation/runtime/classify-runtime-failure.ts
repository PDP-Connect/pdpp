...
export function classifyRuntimeFailure(err: unknown): FailureReason {
  const error = err as RuntimeError | undefined;
  if (typeof error?.failure_reason === "string" && error.failure_reason.trim()) {
    return error.failure_reason as FailureReason;
  }
