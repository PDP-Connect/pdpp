 ...
 */
export function isClosedPipeWriteError(err) {
  if (!err || typeof err !== 'object') return false;
  const code = err.code;
  if (!code || !CLOSED_PIPE_CODES.has(code)) return false;
  const syscall = err.syscall;
  if (syscall && syscall !== 'write') return false;
  return true;
}

export const CLOSED_PIPE_ERROR_CODES = CLOSED_PIPE_CODES;
