...
test("encode → decode round-trips the snapshot id and offset exactly", () => {
  for (const payload of [
    { off: 0, snap: "snap_abc" },
    { off: 42, snap: "snap_abc" },
    { off: 1_000_000, snap: "snap-with-dashes_and_UPPER" },
  ]) {
...
test("the cursor is base64url (JSON) — url-safe, no + / = padding characters", () => {
  ...
  const token = encodeSearchLexicalCursor({ off: 255, snap: "???>>>???" });
  ...
  assert.deepEqual(JSON.parse(json), { off: 255, snap: "???>>>???" });
});
...
test("decode returns null when the JSON is valid but the shape is wrong", () => {
  ...
  assert.equal(decodeSearchLexicalCursor(enc({ off: 3, snap: 5 })), null, "snap wrong type");
  assert.equal(decodeSearchLexicalCursor(enc({ off: "3", snap: "x" })), null, "off wrong type");
...
test("decode strips extra fields, returning only the canonical snap/off pair", () => {
  const enc = (obj: unknown) => Buffer.from(JSON.stringify(obj), "utf8").toString("base64url");
  const token = enc({ injected: "ignored", nested: { a: 1 }, off: 9, snap: "snap_x" });
  const decoded = decodeSearchLexicalCursor(token);
  assert.deepEqual(decoded, { off: 9, snap: "snap_x" });
...
test("offset 0 is a valid, distinct cursor (not treated as absent)", () => {
  const token = encodeSearchLexicalCursor({ off: 0, snap: "snap_first" });
  const decoded = decodeSearchLexicalCursor(token);
  assert.deepEqual(decoded, { off: 0, snap: "snap_first" });
  assert.ok(decoded, "expected a decoded payload, not null");
