...
import { deriveSourcePressureBacklog as deriveSourcePressureBacklogNullable } from "../runtime/connection-health.ts";

// Every call in this file passes readable evidence with at least one pending
// gap, so the real `| null` return (only reached for unreadable/empty
// evidence, per the function's own guard) never applies here; this wrapper
// asserts that and returns the narrowed non-null result so every test body
// doesn't need to re-prove it.
function deriveSourcePressureBacklog(
  backlogEvidence: Parameters<typeof deriveSourcePressureBacklogNullable>[0]
): NonNullable<ReturnType<typeof deriveSourcePressureBacklogNullable>> {
  const result = deriveSourcePressureBacklogNullable(backlogEvidence);
  assert.ok(result, "expected a backlog projection for readable evidence");
  return result;
}
...
function gap(over = {}) {
  return { attemptCount: 0, nextAttemptAfter: null, reason: "upstream_pressure", ...over };
}
...
test("next_attempt_at: a non-source-pressure gap does not contribute its floor", () => {
  ...
  const r = deriveSourcePressureBacklog(
    evidence({
      pendingGaps: [
        gap({ nextAttemptAfter: "2026-05-19T23:00:00.000Z", reason: "schema_validation_failed" }),
        gap({ nextAttemptAfter: "2026-05-19T09:00:00.000Z", reason: "rate_limited" }),
      ],
    })
  );
