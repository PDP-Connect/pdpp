...
test("isAcceptedCollectorProtocolVersion: the current version is accepted", () => {
  ...
  assert.ok(
    SUPPORTED_COLLECTOR_PROTOCOL_VERSIONS.includes(COLLECTOR_PROTOCOL_VERSION),
...
test("isAcceptedCollectorProtocolVersion: unknown / blank / non-string versions are rejected", () => {
  ...
  // @ts-expect-error version is typed as `string | null | undefined`; this
  // proves the runtime guard rejects a non-string value even though the
  // type system would otherwise disallow constructing this call.
...
test("buildCollectorProtocolMismatchBody: accepted_versions is a COPY, not the shared array", () => {
  ...
  assert.notEqual(
    body.accepted_versions,
    SUPPORTED_COLLECTOR_PROTOCOL_VERSIONS,
    "must be a fresh array, not the module constant"
  );
  // The public type declares accepted_versions readonly (an API promise to
  // callers); mutating it here is deliberate reflection to prove the
  // underlying array is a genuine copy, not a shared reference disguised by
  // the readonly annotation.
  const mutable = body.accepted_versions as string[];
  mutable.push("mutation");
  assert.ok(
    !SUPPORTED_COLLECTOR_PROTOCOL_VERSIONS.includes("mutation"),
    "mutating the body must not leak into the module constant"
  );
});
