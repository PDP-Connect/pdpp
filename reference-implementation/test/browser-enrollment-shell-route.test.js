...
test('browser-enrollment shell: creates draft for browser-bound connector', async () => {
  await withServer(async ({ asUrl }) => {
    ...
    assert.equal(body.connector_id, 'chase');
...
test('browser-enrollment shell: two calls create two distinct shells', async () => {
  await withServer(async ({ asUrl }) => {
    ...
    const cookie = await ownerLogin(asUrl);
    const r1 = await fetch(`${asUrl}/_ref/connectors/chatgpt/browser-enrollment-shell`, {
      method: 'POST',
      headers: { cookie },
    });
    const r2 = await fetch(`${asUrl}/_ref/connectors/chatgpt/browser-enrollment-shell`, {
      method: 'POST',
      headers: { cookie },
    });
...
test('browser-enrollment shell: emits audit spine event on success', async () => {
  await withServer(async ({ asUrl }) => {
    ...
    const cookie = await ownerLogin(asUrl);
    const res = await fetch(`${asUrl}/_ref/connectors/usaa/browser-enrollment-shell`, {
      method: 'POST',
      headers: { cookie },
    });
    assert.equal(res.status, 201);
    ...
    assert.equal(event.data?.connector_id, 'usaa');
  });
});
...
test('browser-enrollment shell: shell is not visible in owner connections list', async () => {
  await withServer(async ({ asUrl }) => {
    await registerConnector(asUrl, 'chase');
    const cookie = await ownerLogin(asUrl);
    await fetch(`${asUrl}/_ref/connectors/chase/browser-enrollment-shell`, {
      method: 'POST',
      headers: { cookie },
    });
    ...
    assert.ok(!connectorIds.includes('chase'), 'chase draft shell not visible in connections list');
  });
});
