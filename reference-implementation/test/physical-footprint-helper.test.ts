...
if (POSTGRES_URL) {
  test("collectPhysicalFootprint reports a positive total and bounded, ordered relations on Postgres", async () => {
    ...
    try {
      ...
      assert.ok(typeof footprint.physical_bytes === "number", "physical_bytes narrows to number");
      assert.ok(footprint.physical_bytes > 0, "physical_bytes is positive");
      const dbSizeRow = await postgresQuery("SELECT pg_database_size(current_database()) AS bytes");
      const [freshDbSizeRow] = dbSizeRow.rows;
      assert.ok(freshDbSizeRow, "pg_database_size query returns one row");
      const freshDbSize = Number(freshDbSizeRow.bytes);
      ...
      for (const relation of footprint.top_relations) {
        ...
        assert.ok(relation.bytes <= footprint.physical_bytes, "no single relation exceeds the whole database");
        prev = relation.bytes;
  ...
  test("collectPhysicalFootprint does not write to the database (read-only)", async () => {
    ...
    try {
      ...
      assert.ok(afterRow, "post-read database statistics query returns one row");
      assert.ok(beforeRow, "pre-read database statistics query returns one row");
      assert.equal(
        Number(afterRow.writes),
        Number(beforeRow.writes),
  ...
  test("physical footprint live behavior (skipped: PDPP_TEST_POSTGRES_URL unset)", {
    ...
    // The test is registered only to document the environment-gated proof.
  });
}
