...
test("reference fixture manifest only advertises streams the seed connector emits", () => {
  ...
  // The seed connector emits via `emitRecord(stream, ...)`.
  ...
  // Match emitRecord("stream_name", ...) with either JavaScript quote style.
  for (const match of seedSource.matchAll(/emitRecord\((["'])([^"']+)\1/g)) {
    const [, , stream] = match;
    if (stream) {
      emittedStreams.add(stream);
    }
  }
  // Match emit({ type: "PROGRESS"|"STATE", stream: "stream_name", ... }).
  for (const match of seedSource.matchAll(/stream:\s*(["'])([^"']+)\1/g)) {
    const [, , stream] = match;
    if (stream) {
      emittedStreams.add(stream);
    }
