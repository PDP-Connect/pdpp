...
function seedInstance(connectorInstanceId: string, connectorId: string, ownerSubjectId = OWNER): void {
...
function evidenceRowExists(connectorInstanceId: string): boolean {
  return Boolean(
    getDb().prepare("SELECT 1 FROM connector_summary_evidence WHERE connector_instance_id = ?").get(connectorInstanceId)
  );
}
...
test("scoped pass repairs the requested connection without discovering or touching any sibling", () =>
  withTempDb(async () => {
    ...
    const scopedRow = getDb()
      ...
      .get<{ total_records: number }>("cin_scoped");
    assert.ok(scopedRow, "scoped evidence row exists");
    assert.equal(scopedRow.total_records, 1, "the scoped connection repaired to its fresh canonical count");
    const otherRow = getDb()
      .prepare("SELECT total_records FROM connector_summary_evidence WHERE connector_instance_id = ?")
      .get<{ total_records: number }>("cin_other");
    assert.ok(otherRow, "unscoped evidence row exists");
...
test("a scoped pass materializing evidence for a distinct (non-owner_local) subject is never deleted by the next complete pass", () =>
  withTempDb(async () => {
    ...
    assert.equal(
      completeResult.discovered,
      2,
      "the complete census discovers both subjects' connections, not just owner_local"
    );
