  ...
  const authorizationDetails = [
    {
      ...
      streams: [{ connection_id: sourceConnectionId, fields: ["id", "subject"], name: "messages" }],
...
test("owner-source-to-mcp-closure", async () => {
  await withCredentialKey(async () => {
    ...
    try {
      ...
      const oauth = await completeScopedMcpFlow(asUrl, session, gmail.connector_id, gmailConnectionId);
