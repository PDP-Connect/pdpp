...
test("deployment_readiness echoes the input; absent readiness defaults to ready", () => {
  ...
  const r = projectRunAutomationPolicy({ deploymentReadiness: supplied, triggerKind: "scheduled" });
...
test("a blocking policy wins over not-ready (returns the policy reason, manual_only)", () => {
  ...
  assert.ok(r.reason);
  assert.match(r.reason, REGEXP_1, "the policy reason wins, not the readiness reason");
