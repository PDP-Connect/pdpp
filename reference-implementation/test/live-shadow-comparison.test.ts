 ...
 *   node --test --import tsx reference-implementation/test/live-shadow-comparison.test.ts
 */

import assert from "node:assert/strict";
import test from "node:test";
import type { ConnectionHealthSnapshot, ConnectionHealthState } from "../runtime/connection-health.ts";
import { initDb } from "../server/db.ts";
import { closePostgresStorage, initPostgresStorage, resolveStorageBackend } from "../server/postgres-storage.ts";
import { type ConnectorSummary, listConnectorSummaries } from "../server/ref-control.ts";

function enabled(): boolean {
  return process.env.PDPP_LIVE_CONNECTOR_HEALTH_GATE === "1";
}

type Classification = "no_change" | "fixed_lie" | "deliberate_silence_correction" | "unexpected_drift";

function oldHeadline(snapshot: ConnectionHealthSnapshot): string {
  const stateMap: Record<ConnectionHealthState, string> = {
...
function classify(summary: ConnectorSummary): Classification {
...
test("live-shadow-comparison: production projection has no unexpected drift", { skip: !enabled() }, async () => {
  ...
  if (storage.backend === "postgres") {
    assert.ok(storage.databaseUrl, "Postgres storage configuration has a database URL");
    await initPostgresStorage({ backend: "postgres", databaseUrl: storage.databaseUrl }, {});
  } else {
    await initPostgresStorage(undefined, {});
  }
  try {
    ...
    const counts = results.reduce<Partial<Record<Classification, number>>>((acc, row) => {
