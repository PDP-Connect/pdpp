...
import { buildFieldCapabilities as buildFieldCapabilitiesUntyped } from "../server/schema-capabilities.ts";
...
interface FieldCapabilities {
  aggregation: Record<string, FieldCapabilityFlag>;
  exact_filter: FieldCapabilityFlag;
  granted: boolean;
  lexical_search: FieldCapabilityFlag;
  range_filter: FieldCapabilityFlag;
  role: string;
  semantic_search: FieldCapabilityFlag;
  type: string;
}

/**
 * `server/schema-capabilities.js` is unchecked JS (allowJs, checkJs:false);
 * its default-parameter `= null` makes bare type inference collapse the
 * second argument to `null` only, rejecting the real `{ fields: string[] }`
 * grant shape this file actually passes. Boundary-cast once here rather
 * than at every call site.
 */
const buildFieldCapabilities = buildFieldCapabilitiesUntyped as unknown as (
  manifestStream: unknown,
  streamGrant: { fields: string[] } | null
) => Record<string, FieldCapabilities>;

const MANIFEST_STREAM = {
  query: {
    aggregations: { group_by: ["amount"], sum: ["amount"] },
    range_filters: { amount: ["gte", "lte"] },
    search: { lexical_fields: ["body"], semantic_fields: ["body"] },
  },
  schema: {
    properties: {
      amount: { type: "number", x_pdpp_role: "metric", x_pdpp_type: "currency" },
      body: { type: "string" },
      secret: { type: "string" },
    },
  },
...
test("buildFieldCapabilities reflects lexical/semantic search declarations per field", () => {
  const caps = buildFieldCapabilities(MANIFEST_STREAM, { fields: ["amount", "body"] });
  assert.ok(caps.body, "body field capabilities must be present");
  assert.ok(caps.amount, "amount field capabilities must be present");
...
test("buildFieldCapabilities: a null grant (owner/unfiltered) marks every field granted", () => {
  ...
  assert.ok(caps.secret, "secret field capabilities must be present");
