...
const STREAM = "items";
type RecordEnvelope = Parameters<typeof ingestRecord>[1];
...
function readVersionCounter(connectorInstanceId: string = DEFAULT_ACCOUNT_CONNECTOR_INSTANCE_ID): number | null {
  const row = getDb()
    .prepare(
      `SELECT max_version FROM version_counter
       WHERE connector_id = ? AND connector_instance_id = ? AND stream = ?`
    )
    .get(CONNECTOR_ID, connectorInstanceId, STREAM) as { max_version: number } | undefined;
  return row ? row.max_version : null;
}

function readChangeVersions(): number[] {
  return (
    getDb()
      .prepare(
      ...
      .all(CONNECTOR_ID, STREAM) as Array<{ version: number }>
  ).map((row) => row.version);
}

function makeUpsert(id: string, payload: Record<string, unknown>): RecordEnvelope {
  return {
    data: { id, ...payload },
...
function makeDelete(id: string): RecordEnvelope {
  return {
    data: { id },
    emitted_at: "2026-04-29T00:00:00.000Z",
    key: id,
...
test("atomic allocator returns 1 on first call and bumps counter in one statement", () => {
  ...
  try {
    ...
    const first = execReturningOne<{ max_version: number }>(referenceQueries.recordsIngestAllocateNextVersion, [
      CONNECTOR_ID,
    ...
    const second = execReturningOne<{ max_version: number }>(referenceQueries.recordsIngestAllocateNextVersion, [
      CONNECTOR_ID,
      CONNECTOR_INSTANCE_ID,
      STREAM,
    ]);
    ...
    const third = execReturningOne<{ max_version: number }>(referenceQueries.recordsIngestAllocateNextVersion, [
      CONNECTOR_ID,
      CONNECTOR_INSTANCE_ID,
      STREAM,
    ]);
...
test("changed writes for the same (connector_id, stream) allocate distinct increasing versions", async () => {
  ...
  try {
    ...
    for (let i = 1; i < versions.length; i++) {
      ...
      assert.ok(current !== undefined && previous !== undefined, "expected both versions to be present");
      assert.ok(current > previous, `version ${current} must exceed ${previous}`);
    }
    assert.equal(readVersionCounter(), versions.at(-1));
  } finally {
    teardown();
  }
});
...
test("repeated direct delete does not call the allocator (counter unchanged)", async () => {
  ...
  try {
    ...
    assert.equal(readVersionCounter(), counterAfterDelete, "counter must not advance for a repeated direct delete");
...
test("changes_since change-log sequence is contiguous across mixed writes", async () => {
  ...
  try {
    ...
    await ingestRecord(CONNECTOR_ID, makeDelete("b")); // v=3 (ingest delete)
