...
test('web push send failures mark subscriptions without blocking successful fallback work', async () => {
  const store = createWebPushSubscriptionStore();
  store.upsert('owner_local', sampleSubscription('https://push.example.invalid/sub/gone'), {});
  store.upsert('owner_local', sampleSubscription('https://push.example.invalid/sub/ok'), {});
  ...
  const active = store.list('owner_local');
...
test('web push fanout is scoped to the interaction owner subject', async () => {
  const store = createWebPushSubscriptionStore();
  store.upsert('owner_local', sampleSubscription('https://push.example.invalid/sub/local'), {});
  store.upsert('owner_other', sampleSubscription('https://push.example.invalid/sub/other'), {});
  ...
  assert.equal(store.list('owner_local')[0].last_success_at !== null, true);
  assert.equal(store.list('owner_other')[0].last_success_at, null);
});
