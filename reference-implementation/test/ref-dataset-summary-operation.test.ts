...
function baselineDeps(overrides = {}) {
  return {
    getCounts: () => ({ connector_count: 2, record_count: 7, stream_count: 4 }),
...
test("ref.dataset.summary returns the full dataset_summary envelope", async () => {
  ...
  assert.deepEqual(summary.top_connectors, [
    { connector_id: "a", object: "dataset_connector_summary", record_count: 3 },
    { connector_id: "b", object: "dataset_connector_summary", record_count: 2 },
  ]);
...
test("ref.dataset.summary uses projection hot path without raw aggregate dependencies", async () => {
  ...
  const summary = await executeRefDatasetSummary({
    ...
    getProjection: () => ({
      counts: { connector_count: 3, record_count: 11, stream_count: 5 },
...
test("ref.dataset.summary wraps each top connector as dataset_connector_summary", async () => {
  const summary = await executeRefDatasetSummary(
    baselineDeps({
      listTopConnectorCandidates: () => [{ connector_id: "only", record_count: 1 }],
    })
  );
  assert.deepEqual(summary.top_connectors, [
    { connector_id: "only", object: "dataset_connector_summary", record_count: 1 },
  ]);
});
...
test("ref.dataset.summary collapses every time-bound to null when record_count === 0", async () => {
  ...
  assert.deepEqual(summary.top_connectors, []);
  assert.equal(recordTimeCalled, false, "getRecordTimeBounds must not be called on an empty corpus");
  assert.equal(ingestedTimeCalled, false, "getIngestedTimeBounds must not be called on an empty corpus");
});
...
test("ref.dataset.summary awaits dependency promises", async () => {
  ...
  const summary = await executeRefDatasetSummary({
    getCounts: () =>
      new Promise((r) =>
        setImmediate(() => {
          countsResolved = true;
          r({ connector_count: 1, record_count: 1, stream_count: 1 });
        })
      ),
    ...
    getRetainedBytes: () =>
      new Promise((r) =>
        setImmediate(() => {
          bytesResolved = true;
          r({ blob_bytes: 0, record_changes_json_bytes: 0, record_json_bytes: 1 });
        })
      ),
    listTopConnectorCandidates: () =>
      new Promise((r) =>
  ...
  assert.deepEqual(summary.top_connectors, [
    { connector_id: "lone", object: "dataset_connector_summary", record_count: 1 },
  ]);
});
