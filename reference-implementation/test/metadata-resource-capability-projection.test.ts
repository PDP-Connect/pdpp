...
test("buildProtectedResourceMetadata: optional discovery/agent/onboarding blocks appear only when supplied", () => {
  ...
  const meta = buildProtectedResourceMetadata(
    baseMetadataInput({ agentDiscovery: agent, discoveryHints: hints, ownerAgentOnboarding: onboarding })
  );
...
test("buildSemanticRetrievalCapability: returns null unless model+dimensions+metric+indexState all present", () => {
  ...
  assert.equal(
    buildSemanticRetrievalCapability({ distanceMetric: "cosine", indexState: "built", model: "m" }),
    null,
    "missing dimensions => null"
  );
  assert.equal(
    buildSemanticRetrievalCapability({ dimensions: 384, indexState: "built", model: "m" }),
    null,
    "missing distanceMetric => null"
  );
  assert.equal(
    buildSemanticRetrievalCapability({ dimensions: 384, distanceMetric: "cosine", model: "m" }),
    null,
...
test("buildSemanticRetrievalCapability: minimal complete input yields a supported capability", () => {
  ...
  assert.ok(cap, "expected a supported capability");
...
test("buildSemanticRetrievalCapability: backend_identity composes fields in fixed order, dropping absent profile/dtype", () => {
  ...
  assert.ok(full, "expected a supported capability");
  assert.ok(full.score, "expected a score capability");
  assert.equal(
  ...
  assert.ok(bare, "expected a supported capability");
  assert.ok(bare.score, "expected a score capability");
  assert.equal(
...
test("buildSemanticRetrievalCapability: language_bias attached only when supplied", () => {
  ...
  assert.ok(withBias, "expected a supported capability");
  ...
  assert.ok(withoutBias, "expected a supported capability");
