...
// server/source-descriptor.ts. These functions translate grant/manifest/query
...
// `buildSourceDescriptor`'s `sourceBinding` parameter carries a `= null`
// default value in its untyped JS declaration (server/source-descriptor.ts),
// which TS infers as the parameter's whole type even though the real runtime
// contract accepts a binding object too, as every call below proves.
...
test("buildSourceDescriptor: provider_native binding with id yields provider_native descriptor", () => {
  assert.deepEqual(buildSourceDescriptor({ id: "gmail", kind: "provider_native" }), {
    id: "gmail",
...
test("buildSourceDescriptor: connector binding with id yields connector descriptor", () => {
  assert.deepEqual(buildSourceDescriptor({ id: "amazon", kind: "connector" }), { id: "amazon", kind: "connector" });
});
...
test("buildSourceDescriptor: missing id yields null even with a valid kind", () => {
  ...
  assert.equal(buildSourceDescriptor({ id: "", kind: "connector" }), null, "empty id is falsy -> null");
});

test("buildSourceDescriptor: unknown kind yields null", () => {
  assert.equal(buildSourceDescriptor({ id: "x", kind: "mystery" }), null);
...
test("buildSourceDescriptor: provider_native takes precedence is irrelevant per-call, but connector path is distinct", () => {
  ...
  const d = buildSourceDescriptor({ id: "spotify", kind: "connector" });
  assert.ok(d, "expected a source descriptor");
...
test("resolveGrantStorageBinding: returns binding only when connector_id present", () => {
  ...
  assert.equal(
    resolveGrantStorageBinding({ grant_storage_binding: { connector_id: "" } }),
    null,
    "blank connector_id -> null"
  );
...
test("buildClientSourceDescriptor: grant.source wins over storage_binding", () => {
  const tokenInfo = {
    grant: { source: { id: "native-provider", kind: "provider_native" } },
    grant_storage_binding: { connector_id: "fallback-connector" },
  };
  assert.deepEqual(
    buildClientSourceDescriptor(tokenInfo),
    { id: "native-provider", kind: "provider_native" },
...
test("buildClientSourceDescriptor: null when neither grant.source nor storage binding present", () => {
  ...
  assert.equal(
    buildClientSourceDescriptor({ grant: { source: { kind: "connector" } } }),
    null,
    "source without id -> null -> fall through -> null"
  );
});
...
test("resolveNativeStorageBinding: extracts connector_id from native manifest storage_binding", () => {
  assert.deepEqual(resolveNativeStorageBinding({ nativeManifest: { storage_binding: { connector_id: "nc" } } }), {
    connector_id: "nc",
  });
  assert.equal(
    resolveNativeStorageBinding({ nativeManifest: { storage_binding: { connector_id: "" } } }),
    null,
    "blank connector_id -> null"
  );
