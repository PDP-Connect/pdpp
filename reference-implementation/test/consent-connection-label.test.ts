...
/** Mirrors `apps/site/src/lib/consent-connection-label.ts`'s `ConnectionIdentity`. */
interface ConnectionIdentity {
  connectionId: string;
  displayName?: string | null;
}

/** Mirrors `apps/site/src/lib/consent-connection-label.ts`'s `ConsentConnectionLabel`. */
interface ConsentConnectionLabel {
  displayName: string;
  id: string;
}

/**
 * `apps/site/**` is out of this cohort's scope (forbidden territory), so the
 * module under test is imported dynamically at a file URL rather than via a
 * static specifier this migration could normalize (and so its types are
 * mirrored locally above rather than imported, to avoid pulling that
 * package's own module resolution into this program). The dynamic import
 * resolves to `unknown`; this interface boundary-casts it once here instead
 * of leaving every call site `any`.
 */
interface ConsentConnectionLabelModule {
  buildConsentCardConnections: (connector: string, connections: ConnectionIdentity[]) => ConsentConnectionLabel[];
  deriveConnectionDisplayName: (args: {
    connector: string;
    displayName?: string | null | undefined;
    ordinal: number;
    groupSize: number;
  }) => string;
  formatConnectorName: (connector: string) => string;
  isPlaceholderConnectionLabel: (connector: string, displayName: string | null | undefined) => boolean;
}

const { buildConsentCardConnections, deriveConnectionDisplayName, formatConnectorName, isPlaceholderConnectionLabel } =
  (await import(
    new URL("../../apps/site/src/lib/consent-connection-label.ts", import.meta.url).href
  )) as unknown as ConsentConnectionLabelModule;
...
test("deriveConnectionDisplayName preserves owner-set names verbatim", () => {
  assert.equal(
    deriveConnectionDisplayName({ connector: "gmail", displayName: "Personal Gmail", groupSize: 2, ordinal: 1 }),
    "Personal Gmail"
  );
});
...
test("deriveConnectionDisplayName mints <Connector> · account N for never-renamed connections in a group", () => {
  assert.equal(
    deriveConnectionDisplayName({ connector: "gmail", displayName: null, groupSize: 2, ordinal: 2 }),
    "Gmail · account 2"
  );
  assert.equal(
    deriveConnectionDisplayName({ connector: "gmail", displayName: "legacy", groupSize: 3, ordinal: 1 }),
    "Gmail · account 1"
  );
});
...
test("deriveConnectionDisplayName omits the disambiguator for a lone connection", () => {
  assert.equal(
    deriveConnectionDisplayName({ connector: "gmail", displayName: null, groupSize: 1, ordinal: 1 }),
    "Gmail"
  );
  // …but a real owner label on a lone connection is still preserved.
  assert.equal(
    deriveConnectionDisplayName({ connector: "gmail", displayName: "Personal Gmail", groupSize: 1, ordinal: 1 }),
    "Personal Gmail"
  );
});
...
test("buildConsentCardConnections derives a label per connection and carries the stable id", () => {
  ...
  assert.deepEqual(connections, [
    { displayName: "Personal Gmail", id: "cin_personal" },
    { displayName: "Gmail · account 2", id: "cin_work" },
  ]);
});
...
test("buildConsentCardConnections never renders a placeholder, URL, or connection_id as the label", () => {
  ...
  for (const [index, connection] of connections.entries()) {
    const rawConnection = raw[index];
    assert.ok(rawConnection, `raw fixture must have an entry at index ${index}`);
    assert.equal(
    ...
    assert.equal(connection.id, rawConnection.connectionId, "stable id is preserved for telemetry/dedupe");
  ...
  assert.ok(lastConnection, "connections must be non-empty");
  assert.equal(lastConnection.displayName, "Personal Gmail");
  assert.ok(
    connections
      .slice(0, -1)
      // biome-ignore lint/performance/useTopLevelRegex: test assertion patterns remain colocated with the assertion they explain.
      .every((connection: ConsentConnectionLabel) => /^Gmail · account \d+$/.test(connection.displayName))
  );
});
