...
const NEEDS_HUMAN_ATTENTION = /needs_human_attention/u;

function writeDummyConnector(tmpDir: string, name = "dummy-connector.mjs"): string {
...
function anchorStore(connectorId: string, lastRunTimeMs: number) {
  return {
    appendRunHistory: () => {
      // no-op
    },
...
function pressureGap({ attemptCount = 6 }: { attemptCount?: number } = {}): PendingPressureGap {
  return {
    attemptCount,
...
test("T1+T2: scheduled managed-connector run calls runManagedConnectorViaController with background priority", async () => {
  ...
  try {
    ...
    const calls: { id: string; opts: ManagedRunOpts }[] = [];
    ...
    const scheduler = createScheduler({
      ...
      runManagedConnectorViaController: (id, opts) => {
        calls.push({ id, opts });
    ...
    try {
      ...
      const [call] = calls;
      assert.ok(call, "a runManagedConnectorViaController call was captured");
...
test("T2d: a definitive session_required failure is NOT retried even with maxRetries>0 (no three-attempt burst)", async () => {
  ...
  try {
    ...
    const sessionGap = {
      ...
      message: "chatgpt_preprogress_failure: session_required: ChatGPT session is not active.",
      reason: "connector_reported_failed",
      severity: "actionable",
      stream: null,
    ...
    try {
      ...
      const [authRecord] = completedRuns;
      assert.ok(authRecord, "a completed run record was captured");
      assert.equal(authRecord.status, "failed");
      assert.equal(authRecord.attempt, 1, "terminal record should be the first and only attempt");
...
test("T3c: run_already_active controller contention maps to skipped RunRecord", async () => {
  ...
  try {
    ...
    const scheduler = createScheduler({
      ...
      rsUrl: "http://localhost.invalid",
      runManagedConnectorViaController: () => {
        const err: Error & { code?: string } = new Error("Connector already has an active run: run_existing");
...
test("T5: connectorInstanceId matches connectorId when not explicitly set (profile_key = connectorId)", async () => {
  ...
  try {
    ...
    const scheduler = createScheduler({
      ...
      onRunComplete: (record) => completedRuns.push(record),
      rsUrl: "http://localhost.invalid",
      runManagedConnectorViaController: (_id, opts) => {
        calls.push(opts);
    ...
    try {
      ...
      const [t5Call] = calls;
      assert.ok(t5Call, "a runManagedConnectorViaController call was captured");
      assert.equal(
        t5Call.connectorInstanceId,
        connectorId,
...
test("T7b: auth-required managed scheduled failure marks needs-human and suppresses the next tick", async () => {
  ...
  try {
    ...
    try {
      ...
      const [firstRun, secondRun] = completedRuns;
      assert.ok(firstRun, "the first completed run record was captured");
      assert.ok(secondRun, "the second completed run record was captured");
      assert.equal(firstRun.status, "failed");
      assert.deepEqual(firstRun.knownGaps, [authGap], "terminal auth gap should be preserved");
      ...
      assert.match(secondRun.error || "", NEEDS_HUMAN_ATTENTION);
