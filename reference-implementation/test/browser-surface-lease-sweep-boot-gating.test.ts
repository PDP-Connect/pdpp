...
import { armBrowserSurfaceLeaseSweepAfterBoot, createBrowserSurfaceLeaseSweepTimerFor } from "../server/index.ts";
...
test("createBrowserSurfaceLeaseSweepTimerFor never starts the timer on its own", () => {
  const timer = createBrowserSurfaceLeaseSweepTimerFor(fakeController(), { browserSurfaceAllocator: {} }, fakeLogger());
