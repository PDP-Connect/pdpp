...
import type { CountRow, JsonPathForField } from "../server/search-index-counts.ts";
...
test("sumCountRows: sums numeric n, coerces strings, treats missing/garbage as 0, null-safe", () => {
  ...
  const rowsWithGaps: (CountRow | null)[] = [{ n: 5 }, {}, null, { n: 0 }];
  assert.equal(sumCountRows(rowsWithGaps), 5);
...
test("sqliteFieldPathCte: one placeholder triple per field + interleaved [ordinal, field, path] binds", () => {
  const jsonPathForField: JsonPathForField = (f) => `$.${f}`;
