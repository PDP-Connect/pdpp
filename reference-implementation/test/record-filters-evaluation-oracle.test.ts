...
test("coerceComparableValue coerces by single-typed schema and passes strings through", () => {
  ...
  assert.equal(
    coerceComparableValue("2026-07-02T00:00:00Z", { format: "date-time", type: "string" }),
...
test("passesRequestFilters applies range operators with correct inclusive/exclusive boundaries", () => {
  const filters = [
    { field: "amount", fieldSchema: { type: "integer" }, kind: "range", operators: { gte: 10, lt: 100 } },
  ];
...
test("passesRequestFilters rejects a record whose range field is uncoercible", () => {
  const filters = [{ field: "amount", fieldSchema: { type: "integer" }, kind: "range", operators: { gte: 10 } }];
...
test("passesTimeRange enforces an inclusive since and an exclusive until", () => {
  ...
  assert.equal(
    passesTimeRange(
      ...
      { since: "2026-07-01T00:00:00Z", until: "2026-07-03T00:00:00Z" },
      field
    ),
    true
  );
