...
import { assertFieldVisibleToGrant, FieldWindowError, fieldWindowError } from "../server/record-field-window.ts";

function isFieldWindowError(code: string, httpStatus: number) {
  return (err: unknown) => {
...
test("assertFieldVisibleToGrant fails closed when effectiveFields is a non-array", () => {
  assert.throws(() => assertFieldVisibleToGrant("x", "not-an-array"), isFieldWindowError("field_not_granted", 403));
});
