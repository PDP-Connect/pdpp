...
test("resolveStorageConnectorId accepts camelCase objects (Postgres shape, load-bearing)", () => {
  // connection-identity.ts emits camelCase records; the Postgres copy accepted
...
test("resolveStorageConnectorInstanceId prefers an explicit instance id (both shapes)", () => {
  assert.equal(resolveStorageConnectorInstanceId({ connector_instance_id: "cin_abc" }, "chase"), "cin_abc");
  assert.equal(resolveStorageConnectorInstanceId({ connectorInstanceId: "cin_xyz" }, "chase"), "cin_xyz");
});
...
test("resolveStorageConnectorInstanceId throws invalid_connector_id when neither is present (stricter SQLite guard preserved)", () => {
  ...
  assert.ok(err && typeof err === "object" && "code" in err, "must throw with a code");
...
test("getChangeHistoryLimit reads the env var, clamped to >= 0", () => {
  ...
  try {
    ...
    if (original === undefined) {
      ...
      process.env.PDPP_CHANGE_HISTORY_LIMIT = original;
    }
  }
});
