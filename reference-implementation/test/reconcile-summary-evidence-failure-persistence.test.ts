...
test("probe 1: a repair-candidate upsert failure durably fails the row instead of leaving it stale-but-current", () =>
  withTempDb(async () => {
    ...
    const before = getDb()
      .prepare(
        "SELECT total_records, record_snapshot_state, dirty, state FROM connector_summary_evidence WHERE connector_instance_id = ?"
      )
      .get(INSTANCE_ID);
    ...
    let result: Awaited<ReturnType<typeof reconcileConnectorSummaryEvidence>>;
    ...
    try {
      ...
      const after = getDb()
        ...
        .get(INSTANCE_ID);
      assert.ok(after, "the failed repair retains its evidence row");
      assert.notEqual(
...
test("probe 1b: a first-ever observation that immediately fails is never left silently 'current'", () =>
  withTempDb(async () => {
    ...
    try {
      ...
      const row = getDb()
        .prepare(
          "SELECT record_snapshot_state, dirty, state FROM connector_summary_evidence WHERE connector_instance_id = ?"
        )
        .get(INSTANCE_ID);
...
test("probe 1c: simultaneous repair failure AND failure-marker-write failure (both INSERT and UPDATE rejected) still fails closed through the real production read", () =>
  withTempDb(async () => {
    ...
    try {
      ...
      assert.equal(
        untouchedRow.total_records,
        0,
        "the durable row is genuinely untouched by the double-rejected writes"
      );
      assert.equal(
        untouchedRow.record_snapshot_state,
        "current",
        "the durable row's own state is unchanged (both writes were rejected)"
      );
      assert.equal(
        untouchedRow.dirty,
        1,
        "dirty stays set from ingestRecord's own prior successful marker write, never cleared by either rejected write"
      );
      assert.equal(
        untouchedRow.state,
        "stale",
        "state stays set from ingestRecord's own prior successful marker write, never cleared by either rejected write"
      );
...
test("probe 2: a fold failure after a previously-clean checkpointed-empty terminal history fails closed", () =>
  withTempDb(async () => {
    ...
    const before = getDb()
      .prepare(
        "SELECT terminal_facts_state, record_snapshot_state, dirty, state FROM connector_summary_evidence WHERE connector_instance_id = ?"
      )
      .get(INSTANCE_ID);
    assert.ok(before, "the initial reconcile creates an evidence row");
    ...
    let afterSummary: ReturnType<typeof summaryFor>;
    try {
      await reconcileDirtyConnectorSummaryEvidence();
      ...
      const after = getDb()
        ...
        .get(INSTANCE_ID);
      assert.ok(after, "the fold failure retains its evidence row");
      assert.notEqual(
        after.terminal_facts_state,
        "current",
        "terminal_facts must NOT still read current after the fold failed"
      );
