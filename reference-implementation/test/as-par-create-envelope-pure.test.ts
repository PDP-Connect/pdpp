...
test("executeAsParCreate: projects the RFC 9126 PAR envelope at status 201", async () => {
  const out = await executeAsParCreate(
    { baseUrl: "https://as", body: { client_id: "cli" }, nativeManifest: null },
    {
      initiateGrant: () => ({
...
test("executeAsParCreate: forwards body + baseUrl + nativeManifest to initiateGrant", async () => {
  const received: { body?: unknown; opts?: { baseUrl: string; nativeManifest: unknown } } = {};
  const manifest = { provider_id: "acme" };
  await executeAsParCreate(
    { baseUrl: "https://as", body: { a: 1 }, nativeManifest: manifest },
    {
      initiateGrant: (body, opts) => {
        received.body = body;
        received.opts = opts;
        return { authorization_url: "a", expires_in: 1, request_uri: "u" };
      },
    }
  );
  assert.ok(received.opts, "expected initiateGrant to have been called");
...
test("executeAsParCreate: an absent trace_context yields a null traceContext", async () => {
  const out = await executeAsParCreate(
    { baseUrl: "https://as", body: {}, nativeManifest: null },
    { initiateGrant: () => ({ authorization_url: "a", expires_in: 60, request_uri: "u" }) }
  );
