...
import { createPostgresConnectorStateSchedulerDriver } from "./helpers/postgres-connector-state-scheduler-driver.ts";
...
if (POSTGRES_URL) {
  runConnectorStateSchedulerConformance({
    ...
    makeDriver: () => createPostgresConnectorStateSchedulerDriver({ connectionString: POSTGRES_URL }),
    test,
  });
} else {
  test("postgres connector-state/scheduler conformance (skipped: PDPP_TEST_POSTGRES_URL unset)", {
    skip: true,
