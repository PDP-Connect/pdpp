...
import {
  type ConnectorsListItem,
  type ConnectorsListSourceDescriptor,
  executeConnectorsList,
} from "../operations/rs-connectors-list/index.ts";

function makeItem(connectorId: string, overrides: Partial<ConnectorsListItem> = {}): ConnectorsListItem {
  return {
...
test("rs.connectors.list propagates the dependency source descriptor", async () => {
  const source: ConnectorsListSourceDescriptor = {
    id: "native_provider",
...
test("rs.connectors.list awaits dependency promises", async () => {
  let resolved = false;
  const result = await executeConnectorsList(
    { actor: { client_id: "c", grant_id: "g", kind: "client", subject_id: "s" } },
    {
      getSourceDescriptor: () => null,
      listConnectorItems: () =>
        new Promise((resolve) =>
...
test("rs.connectors.list yields zeroed totals when dependency returns empty", async () => {
  ...
  assert.deepEqual(result.envelope, { data: [], object: "list" });
