...
test("chooseDisplayTimestamp prefers semantic value in native mode", () => {
  ...
  assert.equal(
    chooseDisplayTimestamp({ emittedAt: "2026-12-31T00:00:00Z", semanticTimestamp: semantic }),
...
test("chooseDisplayTimestamp falls back to emittedAt when emitted mode or no semantic", () => {
  const semantic = { field: "created_at", value: "2026-01-01T00:00:00Z" };
  assert.equal(
    chooseDisplayTimestamp({ emittedAt: "2026-12-31T00:00:00Z", mode: "emitted", semanticTimestamp: semantic }),
    "2026-12-31T00:00:00Z"
  );
  assert.equal(
    chooseDisplayTimestamp({ emittedAt: "2026-12-31T00:00:00Z", semanticTimestamp: null }),
    "2026-12-31T00:00:00Z"
  );
});
