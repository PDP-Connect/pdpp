const TOP_LEVEL_REGEX_1 = /grant field projection/;
const TOP_LEVEL_REGEX_2 = /paginates the full set|cursor token round-trips/;
...
import { createBrokenInMemoryRecordReadDriver } from "./helpers/broken-record-read-driver.ts";
...
test("harness detects at least one read invariant violation in a broken driver", async () => {
  ...
  const scenarios: Array<{ fn: () => Promise<unknown>; name: string }> = [];
  const collect = (name: string, fn: () => Promise<unknown>): void => {
  ...
  const projectionFailed = failures.some((f) => TOP_LEVEL_REGEX_1.test(f.name));
  const paginationFailed = failures.some((f) => TOP_LEVEL_REGEX_2.test(f.name));
  assert.ok(
    projectionFailed,
    `expected the grant-projection scenario to fail. failures=${JSON.stringify(
  ...
  assert.ok(
    paginationFailed,
    `expected at least one pagination scenario to fail. failures=${JSON.stringify(
      failures.map((f) => f.name),
      null,
      2
    )}`
