 ...
 * `server/auth.ts` module, or `server/index.js`. The approvals queue is
...
test("ref.approvals.list operation does not import server/auth.ts or server/index.js", () => {
  const src = read(OP_REL);
  for (const needle of ["/server/auth", "/server/index"]) {
    const fromPattern = new RegExp(`\\bfrom\\s*['"][^'"]*${needle}['"]`);
    assert.equal(fromPattern.test(src), false, `operation must not import ${needle}`);
  }
});
