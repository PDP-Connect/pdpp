...
function withTempQueryDir(fn: (dir: string) => void): void {
  const dir = mkdtempSync(join(tmpdir(), "pdpp-query-registry-"));
  try {
    // biome-ignore lint/correctness/noVoidTypeReturn: the callback contract explicitly models a void test observer.
    return fn(dir);
  } finally {
 ...
 */
function writeAllRequiredArtifacts(dir: string): void {
...
function requireCursorField(value: unknown): string {
  if (typeof value !== "object" || value === null || Array.isArray(value)) {
    throw new TypeError("query artifact must be an object");
  }
  const fields: Record<string, unknown> = {};
  for (const [key, entry] of Object.entries(value)) {
    fields[key] = entry;
  }
  if (typeof fields.cursorField !== "string") {
    throw new TypeError("query artifact cursorField must be a string");
  }
  return fields.cursorField;
}

test("loads query artifacts deterministically with camelCase keys", () => {
  withTempQueryDir((dir: string) => {
    writeAllRequiredArtifacts(dir);
    mkdirSync(join(dir, "nested"));
    writeFileSync(join(dir, "alpha-query.sql"), "-- @terminator: one\nSELECT 1 AS value\n");
    writeFileSync(join(dir, "nested", "beta_query.sql"), "-- @terminator: one\nSELECT 2 AS value;\n");
...
test("rejects artifacts without @terminator frontmatter", () => {
  withTempQueryDir((dir: string) => {
    ...
    // biome-ignore lint/performance/useTopLevelRegex: test assertion patterns remain colocated with the assertion they explain.
    assert.throws(() => loadReferenceQueries(dir), /missing or invalid @terminator/);
  });
});
...
test("rejects terminator=many SQL without LIMIT and without small-enum annotation", () => {
  withTempQueryDir((dir: string) => {
    ...
    // biome-ignore lint/performance/useTopLevelRegex: test assertion patterns remain colocated with the assertion they explain.
    assert.throws(() => loadReferenceQueries(dir), /must contain a LIMIT \? placeholder OR be annotated @bounded_by/);
  });
});
...
test("accepts terminator=many SQL with explicit LIMIT placeholder", () => {
  withTempQueryDir((dir: string) => {
    ...
    assert.equal(requireCursorField(registry.pagedRecords), "rowid");
  });
});
...
test("rejects terminator=iterate without @cursor_field", () => {
  withTempQueryDir((dir: string) => {
    writeAllRequiredArtifacts(dir);
    writeFileSync(join(dir, "no-cursor.sql"), "-- @terminator: iterate\nSELECT 1\n");
...
test("rejects terminator=exec without leading mutation keyword", () => {
  withTempQueryDir((dir: string) => {
    writeAllRequiredArtifacts(dir);
    writeFileSync(join(dir, "wrong-exec.sql"), "-- @terminator: exec\nSELECT 1\n");

    // biome-ignore lint/performance/useTopLevelRegex: test assertion patterns remain colocated with the assertion they explain.
    assert.throws(() => loadReferenceQueries(dir), /SQL does not begin with INSERT\/UPDATE\/DELETE/);
  });
});
...
test("rejects terminator=exec_one without leading mutation keyword", () => {
  withTempQueryDir((dir: string) => {
    writeAllRequiredArtifacts(dir);
    writeFileSync(join(dir, "wrong-exec-one.sql"), "-- @terminator: exec_one\nSELECT 1 RETURNING value\n");
...
test("rejects terminator=exec_one without RETURNING clause", () => {
  withTempQueryDir((dir: string) => {
    ...
    // biome-ignore lint/performance/useTopLevelRegex: test assertion patterns remain colocated with the assertion they explain.
    assert.throws(() => loadReferenceQueries(dir), /requires a RETURNING clause/);
  });
});
