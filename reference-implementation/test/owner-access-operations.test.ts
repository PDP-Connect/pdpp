...
test("as.dcr.update passes a valid client_name to the capability and returns 200", async () => {
  const outcome = await executeAsDcrUpdate(
    { actingSubjectId: "owner", body: { client_name: " Renamed " }, clientId: "cli_1" },
    {
  ...
  assert.equal(outcome.outcome === "success" ? outcome.client.client_name : null, "Renamed");
});
...
test("as.dcr.update rejects empty / non-object / extra-field bodies with 400 without calling the capability", async () => {
  ...
  for (const body of [null, [], {}, { client_name: "   " }, { client_name: "ok", redirect_uris: [] }]) {
    ...
    const outcome = await executeAsDcrUpdate({ actingSubjectId: "owner", body, clientId: "cli_1" }, deps);
    assert.equal(outcome.outcome, "failure");
    assert.equal(outcome.status, 400);
    assert.equal(outcome.outcome === "failure" ? outcome.errorCode : null, "invalid_client_metadata");
  }
...
test("as.dcr.update maps capability error codes to HTTP status", async () => {
  ...
  for (const [code, status] of cases) {
    // biome-ignore lint/performance/noAwaitInLoops: localized test assertion preserves its explicit contract.
    const outcome = await executeAsDcrUpdate(
      { actingSubjectId: "owner", body: { client_name: "x" }, clientId: "cli_1" },
      {
        updateRegisteredClientName: () => {
          const e: ErrorWithCode = new Error("nope");
          e.code = code;
          throw e;
        },
      }
    ...
    assert.equal(outcome.outcome === "failure" ? outcome.errorCode : null, code);
  }
});
  ...
  const rows = [
    {
      created_at: "t",
      expires_at: null,
      object: "owner_client_token" as const,
...
test("ref.client.token.revoke forwards ids and returns a typed revocation envelope", async () => {
  const outcome = await executeRefClientTokenRevoke(
    { actingSubjectId: "owner", clientId: "cli_1", tokenIdPublic: "tok_aaa" },
    {
  ...
  assert.equal(outcome.outcome, "success");
  assert.equal(outcome.status, 200);
  assert.deepEqual(outcome.outcome === "success" ? outcome.body : null, {
...
test("ref.client.token.revoke reports revoked:false for an unknown-but-owned token id (idempotent)", async () => {
  const outcome = await executeRefClientTokenRevoke(
    { actingSubjectId: "owner", clientId: "cli_1", tokenIdPublic: "tok_missing" },
    { revokeOwnerClientTokenByPublicId: () => ({ revoked: false, token_id_public: "tok_missing" }) }
  );
  assert.equal(outcome.outcome, "success");
  assert.equal(outcome.outcome === "success" ? outcome.body.revoked : null, false);
});
    ...
    const outcome = await executeRefClientTokenRevoke(
      { actingSubjectId: "owner", clientId: "cli_1", tokenIdPublic: "tok_aaa" },
      {
        revokeOwnerClientTokenByPublicId: () => {
          const e: ErrorWithCode = new Error("x");
          e.code = code;
          throw e;
        },
      }
