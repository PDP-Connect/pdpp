...
test("coerceSemanticTimeValue: seconds vs ms interpretation are three orders of magnitude apart", () => {
  ...
  const belowCoerced = coerceSemanticTimeValue(999_999_999_999); // seconds -> far future
  const atCoerced = coerceSemanticTimeValue(1_000_000_000_000); // ms -> 2001
  assert.ok(belowCoerced, "expected a valid below-threshold coercion");
  assert.ok(atCoerced, "expected a valid at-threshold coercion");
  const belowYear = new Date(belowCoerced).getUTCFullYear();
  const atYear = new Date(atCoerced).getUTCFullYear();
