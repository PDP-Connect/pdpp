...
test("isReferenceCompatibleCursorSchema: integer/number qualify; date-typed string qualifies", () => {
  ...
  assert.equal(
    isReferenceCompatibleCursorSchema({ type: ["integer", "null"] }),
    true,
    "integer|null still one non-null"
  );
  assert.equal(isReferenceCompatibleCursorSchema({ format: "date", type: "string" }), true, "string date");
  assert.equal(isReferenceCompatibleCursorSchema({ format: "date-time", type: "string" }), true, "string date-time");
});
...
test("isTimeBucketAggregateFieldSchema: a string typed with a date/date-time format", () => {
  assert.equal(isTimeBucketAggregateFieldSchema({ format: "date", type: "string" }), true, "date");
  assert.equal(isTimeBucketAggregateFieldSchema({ format: "date-time", type: "string" }), true, "date-time");
  assert.equal(isTimeBucketAggregateFieldSchema({ type: "string" }), false, "string with no format");
  assert.equal(isTimeBucketAggregateFieldSchema({ format: "email", type: "string" }), false, "non-date format");
  assert.equal(
    isTimeBucketAggregateFieldSchema({ format: "date", type: "number" }),
    false,
    "number is not a time bucket"
  );
});
