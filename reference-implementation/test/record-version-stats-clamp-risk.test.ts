...
import { clampRecordVersionStatsLimit, normalizeRecordVersionStatsRisk } from "../server/record-version-stats.ts";
...
test("clampRecordVersionStatsLimit: clamps above the max to 500", () => {
  assert.equal(clampRecordVersionStatsLimit(99_999), 500);
...
test("clampRecordVersionStatsLimit: <= 0, non-finite, or NaN falls back to the default 100", () => {
  ...
  assert.equal(clampRecordVersionStatsLimit(Number.POSITIVE_INFINITY), 100, "Infinity => default");
});
