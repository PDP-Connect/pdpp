...
function baseAttention(overrides: Partial<CreateAttentionInput> = {}): AttentionRecord {
...
test("transition to in_progress ALSO promotes notification state (not just acknowledged)", () => {
  ...
  const out = transition(rec, { now: later, to: "in_progress" });
...
test("transition to a terminal state other than acknowledged/in_progress leaves notification untouched", () => {
  ...
  const out = transition(rec, { now: "2026-06-01T01:00:00.000Z", to: "resolved" });
...
test("transition matrix rejects self-loops, backward moves, and out-of-terminal moves", () => {
  ...
  const terminals: readonly AttentionLifecycle[] = ["resolved", "expired", "cancelled", "superseded"];
...
test("decideDedupe cooldown boundary: strictly-less suppresses, exactly-at creates", () => {
  const terminal: AttentionRecord = {
    ...baseAttention(),
    lifecycle: "resolved",
    updated_at: "2026-06-01T00:00:00.000Z",
  };
  const proposed: CreateAttentionInput = {
    connection_id: "cin_1",
    dedupe_key: "conn:reason:kind",
    id: "att_2",
  ...
  assert.deepEqual(decideDedupe({ cooldown_seconds: 60, existing: terminal, proposed }), {
    kind: "suppress",
    reason: "cooldown",
  });
  // elapsed (30) is NOT < 30 => create at the exact boundary.
  assert.deepEqual(decideDedupe({ cooldown_seconds: 30, existing: terminal, proposed }), { kind: "create" });
});

test("decideDedupe: no existing record always creates", () => {
  // `proposed` content is irrelevant on this branch (decideDedupe returns
  // `{ kind: 'create' }` as soon as `existing` is null, before ever reading
  // `proposed`) — still a well-formed CreateAttentionInput so the arbitrary
  // content point is made without relying on an incomplete shape.
  const proposed: CreateAttentionInput = {
    connection_id: "cin_1",
    dedupe_key: "conn:reason:kind",
    id: "att_irrelevant",
  ...
  assert.deepEqual(decideDedupe({ cooldown_seconds: 999, existing: null, proposed }), { kind: "create" });
});
...
test("decideDedupe: reversed clock (proposed before existing.updated_at) clamps elapsed to 0 => cooldown suppress", () => {
  ...
  assert.deepEqual(decideDedupe({ cooldown_seconds: 1, existing: terminal, proposed }), {
    kind: "suppress",
    reason: "cooldown",
  });
});
...
test("long non-secret metadata strings are truncated to 256 chars with an ellipsis", () => {
  ...
  const rec = baseAttention({ metadata: { exact: short, note: long } });
  // biome-ignore lint/style/useDestructuring: localized test assertion preserves its explicit contract.
  const note = rec.metadata.note;
  assert.ok(typeof note === "string", "truncated note metadata should be a string");
  assert.equal(note.length, 257, "256 chars + the ellipsis character");
  assert.ok(note.endsWith("…"));
  assert.equal(note.slice(0, 256), "x".repeat(256));
...
test("pushPayload strips a trailing slash from origin and percent-encodes the id", () => {
  ...
  assert.ok(payload, "pushPayload should produce a payload for an actionable open record");
...
test("pushPayload copy table matches owner_action exactly (provide_value / operate_attachment / act_elsewhere)", () => {
  ...
  assert.ok(provide, "provide_value payload should not be null");
  assert.equal(provide.title, "Owner input needed");
  assert.equal(provide.body, "Chase needs a code or value.");

  const operate = pushPayload(baseAttention({ owner_action: "operate_attachment", response_contract: "none" }), opts);
  assert.ok(operate, "operate_attachment payload should not be null");
  assert.equal(operate.title, "Owner action needed");
  assert.equal(operate.body, "Chase needs to complete a step.");

  const elsewhere = pushPayload(baseAttention({ owner_action: "act_elsewhere", response_contract: "none" }), opts);
  assert.ok(elsewhere, "act_elsewhere payload should not be null");
...
test("pushPayload uses the generic label when source is hidden or display is null", () => {
  ...
  assert.ok(hidden, "hidden-source payload should not be null");
  ...
  assert.ok(noName, "null-display payload should not be null");
