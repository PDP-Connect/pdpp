...
import {
  type ExploreRecordBucketQueryInput,
  type ExploreRecordBucketSparseRow,
  type ExploreRecordBucketsDependencies,
...
): ExploreRecordBucketsDependencies & { calls: ExploreRecordBucketQueryInput[] } {
  const calls: ExploreRecordBucketQueryInput[] = [];
  return {
    calls,
...
function row(overrides: Partial<ExploreRecordBucketSparseRow> = {}): ExploreRecordBucketSparseRow {
  return {
    bucketStart: null,
    count: 0,
    extentCount: 0,
...
test("day granularity: dense zero-fills the gap between two populated days", async () => {
  const deps = depsReturning([
    ...
    row({ bucketStart: "2026-01-03T00:00:00.000Z", count: 1, granularity: "day" }),
  ]);
  ...
  assert.deepEqual(out.buckets, [
    { count: 2, end: "2026-01-02T00:00:00.000Z", start: "2026-01-01T00:00:00.000Z" },
    { count: 0, end: "2026-01-03T00:00:00.000Z", start: "2026-01-02T00:00:00.000Z" },
    { count: 1, end: "2026-01-04T00:00:00.000Z", start: "2026-01-03T00:00:00.000Z" },
  ]);
  assert.deepEqual(out.extent, { count: 3, end: "2026-01-03T22:00:00.000Z", start: "2026-01-01T08:00:00.000Z" });
});
...
test("hour granularity: floors to the top of the hour and steps by one hour", async () => {
  ...
  assert.deepEqual(
    out.buckets.map((b) => b.start),
    ["2026-01-01T10:00:00.000Z", "2026-01-01T11:00:00.000Z", "2026-01-01T12:00:00.000Z"]
  );
  assert.ok(out.buckets[0]);
  assert.ok(out.buckets[1]);
...
test("quarter granularity: floors to the quarter start month (Apr→Apr), steps 3 months", async () => {
  ...
  assert.deepEqual(
    out.buckets.map((b) => b.start),
    ["2026-04-01T00:00:00.000Z", "2026-07-01T00:00:00.000Z"]
  );
  assert.ok(out.buckets[1]);
...
test("year granularity: floors to Jan 1, steps by one year", async () => {
  ...
  assert.deepEqual(
    out.buckets.map((b) => b.start),
    ["2024-01-01T00:00:00.000Z", "2025-01-01T00:00:00.000Z", "2026-01-01T00:00:00.000Z"]
  );
});
...
test("empty result set: no rows yields empty buckets and null extent", async () => {
  ...
  assert.deepEqual(out.buckets, []);
  assert.deepEqual(out.extent, { count: 0, end: null, start: null });
...
test("count coercion: a non-numeric row count degrades to a real number, missing to 0", async () => {
  const deps = depsReturning([
    row({
      bucketStart: "2026-01-01T00:00:00.000Z",
      // @ts-expect-error -- deliberately passing a non-numeric count to prove
      // the operation's Number()-coercion at the storage-row type boundary.
...
test("date-only since/until expand to start-of-day / end-of-day UTC in the query", async () => {
  const deps = depsReturning([]);
  await executeExploreRecordBuckets({ granularity: "day", since: "2026-01-01", until: "2026-01-31" }, deps);
...
test("since after until is rejected", async () => {
  await assert.rejects(
    () =>
      executeExploreRecordBuckets({ granularity: "day", since: "2026-02-01", until: "2026-01-01" }, depsReturning([])),
    InvalidExploreRecordBucketsRequestError
  );
});
...
test("unparseable since is rejected as invalid_request", async () => {
  await assert.rejects(
    () => executeExploreRecordBuckets({ granularity: "day", since: "not-a-date" }, depsReturning([])),
    (err) => {
...
test("until falls back to `now` when omitted", async () => {
  const deps = depsReturning([]);
  await executeExploreRecordBuckets({ granularity: "day", now: "2026-03-15T12:00:00.000Z" }, deps);
  // biome-ignore lint/style/useDestructuring: the property access names the fixture value at its point of use.
  const q = deps.calls[0];
  assert.ok(q);
  assert.equal(q.until, "2026-03-15T12:00:00.000Z");
});
