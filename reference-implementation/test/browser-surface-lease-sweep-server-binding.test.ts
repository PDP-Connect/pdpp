...
// type safety. This fake only ever treats that value as an opaque Map key —
// it never calls ref()/unref()/close(), so those are harmless stubs
// satisfying the interface, not behavior these tests depend on.
...
function createFakeSweepTimers() {
  ...
  return {
    ...
    setIntervalFn: (callback: () => void, ms: number): NodeJS.Timeout => {
      const handle = makeFakeTimeoutHandle(nextId);
      nextId += 1;
...
test("repeated programmatic create/bind/start/close-all cycles never accumulate timers or listeners (simulates repeated startServer/closeServer calls)", async () => {
  ...
  for (let cycle = 0; cycle < 5; cycle += 1) {
    ...
    assert.equal(
      asServerLike.listenerCount("close"),
      1,
      `cycle ${cycle}: exactly one close listener on the AS-like server`
    );
    assert.equal(
      rsServerLike.listenerCount("close"),
      1,
      `cycle ${cycle}: exactly one close listener on the RS-like server`
    );
    ...
    assert.equal(
      fake.scheduled.size,
      0,
      `cycle ${cycle}: cleared once the last server closed, before the next cycle starts`
    );
