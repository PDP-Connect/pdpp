...
test("semantic encode → decode round-trips and carries the literal sem1. prefix", () => {
  const payload = { off: 12, snap: "snap_sem" };
...
test("semantic decode rejects a body without the sem1. prefix", () => {
  ...
  const bodyOnly = Buffer.from(JSON.stringify({ off: 0, snap: "x" }), "utf8").toString("base64url");
...
test("semantic decode returns null for a malformed body and non-string input", () => {
  ...
  // Non-string inputs must be tolerated (the decoder guards typeof), even
  // though the declared param type is `string` — these two calls ARE the
  // assertion that the runtime guard catches what the type system alone would
  // reject at compile time.
  // @ts-expect-error — deliberately passing null to prove the runtime tolerates it.
  assert.equal(decodeSearchSemanticCursor(null), null);
  // @ts-expect-error — deliberately passing undefined to prove the runtime tolerates it.
...
test("semantic decode returns null when the JSON is valid but the shape is wrong", () => {
  const enc = (obj: unknown) => `sem1.${Buffer.from(JSON.stringify(obj), "utf8").toString("base64url")}`;
  ...
  assert.equal(decodeSearchSemanticCursor(enc({ off: 3, snap: 5 })), null, "snap wrong type");
  assert.equal(decodeSearchSemanticCursor(enc({ off: "3", snap: "x" })), null, "off wrong type");
});

test("semantic decode strips extra fields to the canonical snap/off pair", () => {
  const token = `sem1.${Buffer.from(JSON.stringify({ extra: 1, off: 4, snap: "s" }), "utf8").toString("base64url")}`;
  const decoded = decodeSearchSemanticCursor(token);
  assert.deepEqual(decoded, { off: 4, snap: "s" });
});
...
test("a lexical cursor does NOT decode as a valid semantic cursor", () => {
  ...
  const lexical = encodeSearchLexicalCursor({ off: 5, snap: "snap_lex" });
  assert.doesNotMatch(lexical, REGEXP_1, "lexical cursors have no sem1. prefix");
...
test("a semantic cursor does NOT decode as a valid lexical cursor", () => {
  ...
  const semantic = encodeSearchSemanticCursor({ off: 9, snap: "snap_sem" });
