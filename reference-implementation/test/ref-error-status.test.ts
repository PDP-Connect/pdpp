...
import { codeToStatus, recoveryAdmissionExtrasForWire, typeFor } from "../server/routes/ref-error-status.ts";

function statusFor(code: string): number {
  const status = codeToStatus[code];
  assert.ok(status !== undefined, `${code} must be present in codeToStatus`);
  return status;
}
...
test("typeFor(codeToStatus[code]) yields the envelope type for each mapped code", () => {
  ...
  assert.equal(typeFor(statusFor("connection_not_found")), "not_found_error");
  assert.equal(typeFor(statusFor("insufficient_scope")), "permission_error");
  assert.equal(typeFor(statusFor("cursor_expired")), "gone_error");
  assert.equal(typeFor(statusFor("invalid_request")), "invalid_request_error");
  // ambiguous_connection is 409 → falls back to api_error type (by design).
  assert.equal(typeFor(statusFor("ambiguous_connection")), "api_error");
});
