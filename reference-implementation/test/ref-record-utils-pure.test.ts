...
test("pickSemanticTimestamp: falls back to cursor_field when consent field value is missing/blank", () => {
  const out = pickSemanticTimestamp({ consent_time_field: "ct", cursor_field: "cf" }, { cf: "2024-02-02", ct: "   " });
...
test("compareTimestampValues: orders parseable dates chronologically", () => {
  ...
  assert.equal(
    Math.sign(compareTimestampValues("2024-01-01T00:00:00Z", "2024-01-01")),
    0,
    "same instant compares equal"
  );
});
...
test("chooseDisplayTimestamp: native mode uses the semantic value when present", () => {
  assert.equal(
    chooseDisplayTimestamp({
      emittedAt: "2024-01-01",
      mode: "native",
      semanticTimestamp: { field: "x", value: "2023-01-01" },
    }),
    "2023-01-01"
  );
});
...
test("chooseDisplayTimestamp: native mode with no semantic value, and emitted mode, use emittedAt", () => {
  assert.equal(
    chooseDisplayTimestamp({ emittedAt: "2024-01-01", mode: "native", semanticTimestamp: null }),
    "2024-01-01"
  );
  assert.equal(
    chooseDisplayTimestamp({
