...
test("SQLite read adapters reject a missing current manifest before storage access", async () => {
	await assert.rejects(
		() => queryRecords(target, "dormant", grant, {}, null),
		assertClosed,
	);
	await assert.rejects(
		() => getRecord(target, "dormant", "record_1", grant, null),
		assertClosed,
	);
	await assert.rejects(
		() => aggregateRecords(target, "dormant", grant, {}, null),
		assertClosed,
	);
	await assert.rejects(
		() =>
...
test("Postgres read adapters reject a missing current manifest before any query", async () => {
	await assert.rejects(
		() => postgresQueryRecords(target, "dormant", grant, {}, null),
		assertClosed,
	);
	await assert.rejects(
		() => postgresGetRecord(target, "dormant", "record_1", grant, null),
		assertClosed,
	);
	await assert.rejects(
		() =>
	...
	await assert.rejects(
		() => postgresListStreams(target, grant, null),
		assertClosed,
	);
});
