...
function assertThrowsWith(
  fn: () => unknown,
  { code, param, messageIncludes }: { code?: string; messageIncludes?: string; param?: string } = {}
): QueryError {
  let thrown: unknown;
  try {
    fn();
  ...
  if (param !== undefined) {
    assert.equal(thrown.param, param, `expected error.param=${param} but got ${JSON.stringify(thrown.param)}`);
  }
...
test("rejectListOnlyParamsForChangesFeed: flags list-only params, singular/plural grammar, empty passes", () => {
  ...
  assert.equal(rejectListOnlyParamsForChangesFeed({ count: null, sort: "" }), undefined);
  ...
  assertThrowsWith(
    () => rejectListOnlyParamsForChangesFeed({ count: "exact", order: "asc", sort: "a", window: "exact" }),
...
test("resolveListOrder: canonical sort wins; order honored only when sort absent; disagreement rejected", () => {
  const asc: ResolvedSort = { direction: "ASC", field: "emitted_at" };
  const desc: ResolvedSort = { direction: "DESC", field: "emitted_at" };
...
test("decorateRecordWithConnectionIdentity: null-guarded, trims, sets alias + display name", () => {
  ...
  const rec2: Record<string, unknown> = {};
...
test("attachRequestWarningsToResponse: guards + appends to existing warnings", () => {
  ...
  assert.deepEqual((resp.meta as Record<string, unknown>).warnings, [{ code: "partial" }]);

  // Existing meta warnings are preserved and appended to (not replaced).
  const resp2: Record<string, unknown> = {
    meta: { count: { kind: "exact", value: 3 }, warnings: [{ code: "a" }] },
  };
  attachRequestWarningsToResponse(resp2, [{ code: "b" }]);
  const meta2 = resp2.meta as Record<string, unknown>;
  assert.deepEqual(meta2.warnings, [{ code: "a" }, { code: "b" }]);
  assert.deepEqual(meta2.count, { kind: "exact", value: 3 }, "other meta members preserved");
});
