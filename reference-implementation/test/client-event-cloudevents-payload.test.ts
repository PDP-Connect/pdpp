...
import type { DerivedEvent } from "../operations/rs-client-event-derive/index.ts";

function sampleEvent(overrides: Partial<DerivedEvent> = {}): DerivedEvent {
  return {
    data: { connection_id: "cin_1", connector_id: "amazon", stream: "orders" },
...
test("buildEventPayload: subscription_id is injected into data even when event.data lacks it", () => {
  const env = JSON.parse(
    buildEventPayload("evt_1", sampleEvent({ data: { stream: "messages" }, subscriptionId: "sub_inject" }))
  );
...
test("buildEventPayload: event data is spread AFTER subscription_id (event data cannot clobber it away)", () => {
  ...
  const env = JSON.parse(
    buildEventPayload("evt_1", sampleEvent({ data: { challenge: "ch_1" }, subscriptionId: "sub_real" }))
  );
