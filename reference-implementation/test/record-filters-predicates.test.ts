...
const DATE = { format: "date-time", type: "string" };
...
test("parseDateValue: parses ISO strings, rejects non-strings/blank/garbage", () => {
  ...
  assert.equal(parseDateValue(1_735_776_000_000), null);
...
test("coerceComparableValue: non-strict returns null on bad input; strict throws typed errors", () => {
  ...
  assert.throws(
    ...
    (e: unknown) => isQueryError(e) && e.code === "invalid_request" && /Invalid integer value/.test(e.message),
  ...
  assert.throws(
    ...
    // biome-ignore lint/performance/useTopLevelRegex: test assertion patterns remain colocated with the assertion they explain.
    (e: unknown) => isQueryError(e) && e.code === "invalid_request" && /Invalid number value/.test(e.message)
  );
  ...
  assert.throws(
    ...
    // biome-ignore lint/performance/useTopLevelRegex: test assertion patterns remain colocated with the assertion they explain.
    (e: unknown) => isQueryError(e) && e.code === "invalid_request" && /Invalid date value/.test(e.message)
  );
...
test("passesRequestFilters: range operator boundaries are gte-inclusive / gt-exclusive / lte-inclusive / lt-exclusive", () => {
  const f = (operators: Record<string, number>) => [{ field: "amount", fieldSchema: NUM, kind: "range", operators }];
...
test("allowedCandidateRecordKeysFromRows: skips unparseable JSON, applies grant + filters", () => {
  ...
  const rows = [
    { record_json: JSON.stringify({ status: "active" }), record_key: "k1" }, // passes
    { record_json: JSON.stringify({ status: "archived" }), record_key: "k2" }, // filtered out
    { record_json: "{ this is not json", record_key: "k3" }, // parse error -> skipped
    { record_json: JSON.stringify({ status: "active" }), record_key: "k9" }, // not in resources
  ];

  const allowed = allowedCandidateRecordKeysFromRows(rows, { compiledFilters, manifestStream: stream, streamGrant });
