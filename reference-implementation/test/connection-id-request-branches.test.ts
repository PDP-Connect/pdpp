...
test("buildLimitClampedWarning carries the canonical code, param, and machine detail", () => {
  ...
  assert.match(warning.message, TOP_LEVEL_REGEX_1);
...
test("validateConnectionAlias is a no-op for non-object / nullish input", () => {
  ...
  // @ts-expect-error runtime guard covers non-object input (typeof check) though the declared type is narrower — the assertion IS that this stays rejected at the type level while remaining runtime-safe.
...
test("enforceConnectionNarrowing throws connection_not_found when the id does not match the binding", () => {
  assert.throws(
    ...
    (err: unknown) => {
      assert.ok(err instanceof Error);
      const { code, param } = err as Error & { code?: string; param?: string };
      assert.equal(code, "connection_not_found");
      assert.equal(param, "connection_id");
      return true;
    }
  );
});
...
test("enforceConnectionNarrowing throws connection_not_found when the grant has no bound storage", () => {
  for (const bound of ["", null, undefined]) {
    assert.throws(
      ...
      (err: unknown) => err instanceof Error && (err as Error & { code?: string }).code === "connection_not_found",
