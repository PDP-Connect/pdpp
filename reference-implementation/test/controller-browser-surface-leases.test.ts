...
import {
  ...
  type BrowserSurfaceLeaseManagerOptions,
...
const TOP_REGEX_0 = /Connector already has an active run/;
...
function createMemoryBrowserSurfaceLeaseStore({
  surfaces = [],
  leases = [],
}: {
  ...
  surfaces: Map<string, BrowserSurfaceWithPersistenceMetadata>;
  leases: Map<string, BrowserSurfaceLease>;
} {
  ...
  const store: BrowserSurfaceLeaseStore & {
    surfaces: Map<string, BrowserSurfaceWithPersistenceMetadata>;
    leases: Map<string, BrowserSurfaceLease>;
  } = {
    clearSurfaceActiveLease: (surfaceId) => {
      const surface = surfaceRows.get(surfaceId);
      if (!surface) {
        return Promise.resolve(null);
      }
      const updated: BrowserSurfaceWithPersistenceMetadata = { ...surface };
    ...
    leases: leaseRows,
    listLeases: async () => [...leaseRows.values()],
    ...
    readForConnectionIdentities: async () => new Map(),
    repairStaleSurfaceActiveLeases: () => {
    ...
    updateBrowserGenerationHash: (surfaceId, browserGenerationHash) => {
      const surface = surfaceRows.get(surfaceId);
      if (surface) {
        surfaceRows.set(surfaceId, { ...surface, browser_generation_hash: browserGenerationHash });
      }
      return Promise.resolve();
    },
    updateLeaseTerminal: (leaseId, status, options = {}) => {
...
interface CreateManagerOptions {
  ...
  now?: () => Date;
  staticProfileKey?: string | undefined;
  surfaceCap?: number;
}
...
function createManager(options: CreateManagerOptions = {}): BrowserSurfaceLeaseManager {
  ...
  return new BrowserSurfaceLeaseManager({
    config: {
      managedConnectors,
      surfaceCap,
      ...(staticProfileKey === undefined ? {} : { staticProfileKey }),
...
function createDynamicManager(options: CreateManagerOptions = {}): BrowserSurfaceLeaseManager {
...
function createFailingAllocator(): BrowserSurfaceAllocator {
  return {
    ensureSurface: () => Promise.reject(new Error("allocator failed")),
...
function createStopFailingAllocator(): BrowserSurfaceAllocator & {
  ensureRequests: EnsureBrowserSurfaceRequest[];
  stopRequests: StopBrowserSurfaceRequest[];
} {
  ...
  return {
    ...
    stopSurface: (request) => {
      ...
      return Promise.reject(new Error("allocator stop failed"));
    },
  };
}
  ...
  const controller = createController({
    ...
    runConnectorImpl: async (opts) => {
      ...
      return runConnectorImpl
        ? await runConnectorImpl(opts)
        : { checkpoint_summary: null, records_emitted: 0, state: null, status: "succeeded" };
    },
    schedulerStore,
...
async function waitFor(predicate: () => boolean, message: string): Promise<void> {
  ...
  while (Date.now() < deadline) {
    ...
    // biome-ignore lint/performance/noAwaitInLoops: polling must be sequential until the predicate flips.
...
test("managed free surface leases and spawns with browser-surface env", async (t) => {
  ...
  const [firstOpts] = calls.runConnectorOpts;
  assert.ok(firstOpts);
  assert.equal(firstOpts.browserSurfaceEnv?.PDPP_BROWSER_SURFACE_REQUIRED, "neko");
...
test("watchdog cleanup keeps a managed lease unavailable until the presentation terminalizer settles", async (t) => {
  ...
  releasePresentation();
  await waitFor(
    () => manager.getLease("lease_1")?.status === "released",
    "lease releases after presentation terminalizer"
  );
});
...
test("ordinary connector completion keeps a managed lease unavailable until the presentation terminalizer settles", async (t) => {
  ...
  await waitFor(
    () => manager.getLease("lease_1")?.status === "released",
    "lease releases after ordinary completion terminalizer"
  );
});
...
test("ordinary connector failure keeps a managed lease unavailable until the presentation terminalizer settles", async (t) => {
  ...
  await waitFor(
    () => manager.getLease("lease_1")?.status === "released",
    "lease releases after ordinary failure terminalizer"
  );
});
...
test("durable active-run row blocks managed manual and recovery admission before browser-surface acquisition", async () => {
  ...
  await assert.rejects(
    ...
    (err) =>
      ...
      (err as { code?: string; runId?: string }).runId === "run_existing_conflict"
  );
...
test("canonical-url configured managed connector leases short-id run", async (t) => {
  ...
  assert.ok(firstOpts);
  assert.equal(firstOpts.browserSurfaceEnv?.PDPP_BROWSER_SURFACE_REQUIRED, "neko");
  assert.equal(firstOpts.browserSurfaceEnv?.PDPP_BROWSER_SURFACE_PROFILE_KEY, "chatgpt");
...
test("dynamic starting surface does not spawn connector or emit run.started before readiness", async (t) => {
  ...
  assert.equal(calls.persistActiveRun, 1);
  assert.equal((await schedulerStore.listActiveRuns()).length, 1);
...
test("dynamic managed runs with distinct profile keys allocate separate ready surfaces", async (t) => {
  ...
  const runConnectorImpl = (opts: RuntimeRunConnectorOptions) => {
    if (opts.runId === "run_distinct_first") {
      return new Promise<RuntimeRunConnectorResult>((resolve) => {
        releaseFirst = () =>
          resolve({ checkpoint_summary: null, records_emitted: 0, state: null, status: "succeeded" });
      });
    }
    return Promise.resolve<RuntimeRunConnectorResult>({
  ...
  const { calls, controller } = setup(t, { browserSurfaceAllocator: allocator, manager, runConnectorImpl });
  ...
  const firstEnv = calls.runConnectorOpts[0]?.browserSurfaceEnv;
  const secondEnv = calls.runConnectorOpts[1]?.browserSurfaceEnv;
  assert.ok(firstEnv);
  assert.ok(secondEnv);
  assert.notEqual(firstEnv.PDPP_BROWSER_SURFACE_ID, secondEnv.PDPP_BROWSER_SURFACE_ID);
  assert.deepEqual(
...
test("dynamic capacity pressure stops incompatible idle surface and starts distinct-profile run", async (t) => {
  ...
  assert.deepEqual(
    allocator.stopRequests.map((request) => request.surfaceId),
    ["surface_1"]
  );
  assert.deepEqual(
    allocator.ensureRequests.map((request) => request.profileKey),
...
test("dynamic capacity-pressure stop failure recovers on retry and promotes the queued run", async (t) => {
  const allocator = createStopFailingAllocator();
  let attempts = 0;
  const originalStopSurface = allocator.stopSurface.bind(allocator);
  allocator.stopSurface = (request: StopBrowserSurfaceRequest) => {
    attempts += 1;
    if (attempts < 2) {
...
test("dynamic cap queues distinct-profile managed run, then promotes after incompatible idle cleanup", async (t) => {
  ...
  const allocator = createReadyAllocator();
  const manager = createDynamicManager({ leaseWaitTimeoutMs: 1_800_000, now: () => now, surfaceCap: 1 });
  const browserSurfaceLeaseStore = createMemoryBrowserSurfaceLeaseStore();
  const runConnectorImpl = (opts: RuntimeRunConnectorOptions) => {
  ...
  assert.equal(manager.getLease("lease_2")?.status, "waiting_for_browser_surface");
  assert.deepEqual(
    manager.listSurfaces().map((surface) => ({
      active: surface.active_lease_id ?? null,
      health: surface.health,
      id: surface.surface_id,
      last: surface.last_used_at,
      profile: surface.profile_key,
    })),
    [
      {
        active: null,
  ...
  assert.deepEqual(
    manager.listSurfaces().map((surface) => surface.surface_id),
    ["surface_2"]
  );
  assert.deepEqual(
    promoted.map((lease) => lease.pending_run_id),
    ["run_dynamic_cap_second"]
  );
  ...
  await waitFor(
    () => allocator.ensureRequests.length === 2,
    "queued dynamic run should allocate after idle cleanup frees cap"
  );
...
test("persisted stopping dynamic surface does not consume cap after rehydration", async () => {
  ...
  const managerOptions: BrowserSurfaceLeaseManagerOptions = {
  ...
  const manager = new BrowserSurfaceLeaseManager(managerOptions);
  ...
  assert.deepEqual(
    manager.listSurfaces().map((surface) => ({ health: surface.health, id: surface.surface_id })),
    [
      { health: "stopping", id: "surface_stopped" },
      { health: "starting", id: "surface_promoted" },
    ]
  );
});
...
test("dynamic allocator failure emits browser-surface failed without spawning connector", async (t) => {
  ...
  assert.equal(
    events.some((event) => event.event_type === "run.started"),
    false
  );
});
...
test("dynamic allocator failure clears the reservation and admits the next run", async (t) => {
  ...
  const recoveryController = createController({
    ...
    runConnectorImpl: (opts) => {
      calls.runConnector += 1;
      calls.runConnectorOpts.push(opts);
      return Promise.resolve({ checkpoint_summary: null, records_emitted: 0, state: null, status: "succeeded" });
    },
    schedulerStore,
    streamingTargetNonceHooks: {
      clearNonce: () => {
...
test("managed run emits browser-surface cancelled when manual action is cancelled", async (t) => {
  const runConnectorImpl = async (opts: RuntimeRunConnectorOptions): Promise<RuntimeRunConnectorResult> => {
...
test("managed cap-full second connector queues and clears its transient reservation", async (t) => {
  let releaseFirst: () => void = () => undefined;
  const manager = createManager({ staticProfileKey: "managed-profile", surfaceCap: 1 });
  const runConnectorImpl = (opts: RuntimeRunConnectorOptions) => {
  ...
  assert.equal(controller.getActiveRun("other-managed"), null);
  assert.deepEqual(
    (await schedulerStore.listActiveRuns()).map((row) => row.run_id),
    ["run_first"]
  );
  assert.deepEqual(
    controller.listBrowserSurfaceRunProjections().find((run) => run.pending_run_id === "run_second"),
    {
...
test("queued browser-surface timeout emits deferred event without connector failure", async (t) => {
  let releaseFirst: () => void = () => undefined;
  const manager = createManager({ leaseWaitTimeoutMs: 0, staticProfileKey: "managed-profile", surfaceCap: 1 });
  const runConnectorImpl = (opts: RuntimeRunConnectorOptions) => {
    if (opts.runId === "run_first") {
      return new Promise<RuntimeRunConnectorResult>((resolve) => {
        releaseFirst = () =>
...
test("promotion precondition failure defers lease instead of recording clean release", async (t) => {
  ...
  assert.ok(events.some((event) => event.event_type === "run.browser_surface_deferred"));
  assert.equal(
    events.some((event) => event.event_type === "run.browser_surface_released"),
    false
  );
});
...
test("managed connector with active run rejects without acquiring a new lease", async (t) => {
  ...
  const { controller } = setup(t, { browserSurfaceAllocator: createReadyAllocator(), manager, runConnectorImpl });
...
test("duplicate queued managed connector request reports existing pending run", async (t) => {
  ...
  await assert.rejects(
    ...
    (err) => {
      assert.equal((err as { code?: string }).code, "run_browser_surface_queued");
      assert.equal((err as { runId?: string }).runId, "run_waiting");
      return true;
    }
  );
...
test("incompatible static profile defers and clears its transient reservation", async (t) => {
  ...
  assert.equal((await schedulerStore.listActiveRuns()).length, 0);
  assert.deepEqual(
    controller.listBrowserSurfaceRunProjections().find((run) => run.pending_run_id === "run_deferred"),
    {
...
test("sweep reconciles a ready surface whose backing container has already exited", async (t) => {
  ...
  // without one.
  await allocator.stopSurface({ reason: "operator", surfaceId: "surface_1" });
...
test("sweep never touches an active leased run", async (t) => {
  ...
  const { controller, manager: managerRef } = setup(t, {
    browserSurfaceAllocator: allocator,
    manager,
    runConnectorImpl,
  });
...
test("overlapping sweep calls: the second is a no-op while the first is in flight", async (t) => {
  ...
  let unblockAllocator: () => void = () => undefined;
  const blockedGate = new Promise<void>((resolve) => {
    unblockAllocator = resolve;
  });
  let getSurfaceStatusCalls = 0;
  const originalGetSurfaceStatus = allocator.getSurfaceStatus.bind(allocator);
  allocator.getSurfaceStatus = async (surfaceId: string) => {
