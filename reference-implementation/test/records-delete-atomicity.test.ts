...
import { __setDeleteFaultHookForTest, deleteRecord, ingestRecord } from "../server/records.ts";
...
function readChangeRows(): ChangeRow[] {
  return getDb()
    .prepare(
    ...
    .all(CONNECTOR_ID, STREAM) as ChangeRow[];
}
...
function readLiveRecord(recordKey: string) {
  return getDb()
    .prepare(
...
function makeUpsert(id: string, payload: Record<string, unknown>) {
  return {
    data: { id, ...payload },
...
function makeIngestDelete(id: string): Parameters<typeof ingestRecord>[1] {
  return {
    data: { id },
    emitted_at: "2026-04-28T12:00:00.000Z",
    key: id,
...
test("successful direct delete appends exactly one delete change and advances version_counter once", async () => {
  ...
  try {
    ...
    const deleteRow = changesAfter.at(-1);
    assert.ok(deleteRow, "expected at least one change row after delete");
    ...
    assert.ok(live, "expected the live record after a successful delete");
...
test("failure between live mutation and record_changes append rolls back the whole durable direct-delete unit", async () => {
  ...
  try {
    ...
    assert.ok(baselineLive, "expected the seeded live record before injecting a failure");
    ...
    __setDeleteFaultHookForTest((point: string) => {
      if (point === "after-records-mutation") {
        throw new Error("injected fault between records and record_changes (delete)");
      }
    });

    await assert.rejects(() => deleteRecord(CONNECTOR_ID, STREAM, "a"), REGEXP_1);
    ...
    const newRow = recoveredChanges.at(-1);
    assert.ok(newRow, "expected at least one recovered change row");
