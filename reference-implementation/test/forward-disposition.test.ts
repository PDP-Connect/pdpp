...
import { deriveForwardDisposition, type ForwardDispositionInput } from "../runtime/connection-health.ts";
...
function input(overrides: Partial<ForwardDispositionInput> = {}): ForwardDispositionInput {
  return {
    attentionOpen: false,
    coverage: "complete",
    freshness: "fresh",
...
test("resumable: retryable detail gap, no attention -> resumable", () => {
  assert.equal(deriveForwardDisposition(input({ coverage: "retryable_gap", gapRetryable: true })), "resumable");
});

test("resumable: ordinary partial boundary -> resumable even without an explicit retry flag", () => {
  assert.equal(deriveForwardDisposition(input({ coverage: "partial", gapRetryable: false })), "resumable");
});
...
test("gap-before-freshness: retryable_gap that is ALSO stale stays resumable, not owner_refresh_due", () => {
  const disposition = deriveForwardDisposition(
    input({ coverage: "retryable_gap", freshness: "stale", gapRetryable: true, refresh: MANUAL_REFRESH })
  );
...
test("gap-before-freshness: partial that is ALSO stale on a manual connection stays resumable", () => {
  assert.equal(
    deriveForwardDisposition(input({ coverage: "partial", freshness: "stale", refresh: MANUAL_REFRESH })),
    "resumable"
  );
});
...
test("awaiting_owner: outstanding gap blocked on open attention -> awaiting_owner", () => {
  assert.equal(
    deriveForwardDisposition(input({ attentionOpen: true, coverage: "retryable_gap", gapRetryable: true })),
    "awaiting_owner"
  );
});

test("awaiting_owner: a partial boundary blocked on attention awaits the owner, not resumable", () => {
  assert.equal(deriveForwardDisposition(input({ attentionOpen: true, coverage: "partial" })), "awaiting_owner");
});
...
test("awaiting_owner: a complete-but-stale stream is NOT awaiting_owner (no outstanding gap)", () => {
  ...
  const disposition = deriveForwardDisposition(
    input({ attentionOpen: true, coverage: "complete", freshness: "stale", refresh: MANUAL_REFRESH })
  );
...
test("terminal: unsupported is terminal whatever the retryability flag claims", () => {
  assert.equal(deriveForwardDisposition(input({ coverage: "unsupported", gapRetryable: true })), "terminal");
});
...
test("terminal: a terminal_gap blocked on attention awaits the owner (attention wins over terminal)", () => {
  ...
  assert.equal(deriveForwardDisposition(input({ attentionOpen: true, coverage: "terminal_gap" })), "awaiting_owner");
});
