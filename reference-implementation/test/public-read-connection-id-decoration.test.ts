...
import { aggregateRecords, getRecord, ingestRecord, queryRecords } from "../server/records.ts";
...
function recordPayload(id: string, subject: string, receivedAt: string): JsonRecord {
  return {
    data: {
      id,
...
async function withDb(testFn: () => Promise<void>): Promise<void> {
  initDb();
  try {
...
test("records list decorates every record with connection_id + deprecated alias", async () => {
  await withDb(async () => {
    const response: any = await queryRecords(target(), STREAM, grant, {}, manifest);
...
test("records list emits meta.warnings deprecated_alias_used when the deprecated alias was sent", async () => {
  await withDb(async () => {
    const response: any = await queryRecords(target(), STREAM, grant, { connector_instance_id: INSTANCE_ID }, manifest);
...
test("records list does not emit meta.warnings when only canonical connection_id is sent", async () => {
  await withDb(async () => {
    const response: any = await queryRecords(target(), STREAM, grant, { connection_id: INSTANCE_ID }, manifest);
    assert.equal(response.meta, undefined);
  });
});
...
test("records list rejects connection_id outside the grant storage binding", async () => {
  await withDb(async () => {
    await assert.rejects(
      () => queryRecords(target(), STREAM, grant, { connection_id: "cin_other_connection" }, manifest),
      (err: any) => {
...
test("records list changes_since branch decorates each emitted record with connection_id", async () => {
  await withDb(async () => {
    const response: any = await queryRecords(target(), STREAM, grant, { changes_since: "beginning" }, manifest);
    assert.equal(response.object, "list");
...
test("records detail decorates the returned record with connection_id + deprecated alias", async () => {
  await withDb(async () => {
    const record: any = await getRecord(target(), STREAM, "rec-1", grant, manifest);
...
test("records detail rejects connection_id outside the grant storage binding", async () => {
  await withDb(async () => {
    await assert.rejects(
      () => getRecord(target(), STREAM, "rec-1", grant, manifest, { connection_id: "cin_other_connection" }),
      (err: any) => {
        assert.equal(err.code, "connection_not_found");
        assert.equal(err.param, "connection_id");
        return true;
...
test("records aggregate emits meta.warnings when the deprecated alias was sent", async () => {
  await withDb(async () => {
    const response: any = await aggregateRecords(
      target(),
      STREAM,
      grant,
      { connector_instance_id: INSTANCE_ID, metric: "count" },
      manifest
    );
...
test("records aggregate omits meta.warnings when no alias is sent", async () => {
  await withDb(async () => {
    const response: any = await aggregateRecords(target(), STREAM, grant, { metric: "count" }, manifest);
    assert.equal(response.meta, undefined);
  });
});
...
test("records aggregate rejects connection_id outside the grant storage binding", async () => {
  await withDb(async () => {
    await assert.rejects(
      () =>
        aggregateRecords(target(), STREAM, grant, { connection_id: "cin_other_connection", metric: "count" }, manifest),
      (err: any) => {
        assert.equal(err.code, "connection_not_found");
        assert.equal(err.param, "connection_id");
        return true;
...
test("records list rejects unsupported query params with invalid_request", async () => {
  await withDb(async () => {
    await assert.rejects(
      queryRecords(target(), STREAM, grant, { not_a_real_param: "foo" }, manifest),
      // biome-ignore lint/performance/useTopLevelRegex: test assertion patterns remain colocated with the assertion they explain.
      (err: any) => err.code === "invalid_request" && /Unsupported query parameter/.test(err.message)
    );
  });
});
...
test("records list rejects unsupported expand relation with invalid_expand", async () => {
  await withDb(async () => {
    await assert.rejects(
      queryRecords(target(), STREAM, grant, { expand: "nonexistent_relation" }, manifest),
      (err: any) => err.code === "invalid_expand"
    );
  });
});
...
test("records list rejects conflicting connection_id / connector_instance_id with invalid_argument", async () => {
  await withDb(async () => {
    await assert.rejects(
      queryRecords(target(), STREAM, grant, { connection_id: "cin_a", connector_instance_id: "cin_b" }, manifest),
      (err: any) => err.code === "invalid_argument" && err.param === "connector_instance_id"
    );
  });
});
...
test("records list emits meta.count = { kind: none } when count=none is requested", async () => {
  await withDb(async () => {
    const response: any = await queryRecords(target(), STREAM, grant, { count: "none" }, manifest);
...
test("records list emits meta.count.kind=exact with the visible-row count when count=exact", async () => {
  await withDb(async () => {
    const response: any = await queryRecords(target(), STREAM, grant, { count: "exact" }, manifest);
    assert.ok(response.meta);
...
test("records list returns exact for count=estimated without a downgrade warning", async () => {
  ...
  await withDb(async () => {
    const response: any = await queryRecords(target(), STREAM, grant, { count: "estimated" }, manifest);
    assert.ok(response.meta);
    assert.equal(response.meta.count.kind, "exact");
    assert.equal(response.meta.count.value, 2);
...
test("records list rejects unknown count grade with invalid_request", async () => {
  await withDb(async () => {
    await assert.rejects(
      ...
      // biome-ignore lint/performance/useTopLevelRegex: test assertion patterns remain colocated with the assertion they explain.
      (err: any) => err.code === "invalid_request" && /count must be one of/.test(err.message)
    );
  });
});
...
test("records list accepts sort over the manifest cursor field", async () => {
  await withDb(async () => {
    const response: any = await queryRecords(target(), STREAM, grant, { sort: "-received_at" }, manifest);
    assert.equal(response.object, "list");
    assert.equal(response.data.length, 2);
  });
});
...
test("records list rejects sort fields that are not advertised as sortable", async () => {
  await withDb(async () => {
    await assert.rejects(
      ...
      (err: any) =>
        // biome-ignore lint/performance/useTopLevelRegex: test assertion patterns remain colocated with the assertion they explain.
        err.code === "invalid_sort" && err.param === "sort" && /not advertised as sortable/.test(err.message)
    );
  });
});
...
test("records list rejects an empty sort entry", async () => {
  await withDb(async () => {
    await assert.rejects(
      ...
      (err: any) => err.code === "invalid_sort" && err.param === "sort"
    );
  });
});
...
test("records list rejects when sort and order disagree", async () => {
  await withDb(async () => {
    await assert.rejects(
      queryRecords(target(), STREAM, grant, { order: "asc", sort: "-received_at" }, manifest),
      // biome-ignore lint/performance/useTopLevelRegex: test assertion patterns remain colocated with the assertion they explain.
      (err: any) => err.code === "invalid_sort" && /disagree/.test(err.message)
    );
  });
});
  ...
  await withDb(async () => {
    for (const params of [
      ...
      await assert.rejects(
        ...
        // biome-ignore lint/performance/useTopLevelRegex: test assertion patterns remain colocated with the assertion they explain.
        (err: any) => err.code === "invalid_request" && /not supported with changes_since/.test(err.message)
      );
    }
  });
});
