...
function req(headers: Record<string, string> = {}, protocol = "https") {
...
test("isTrustedMetadataRequestOrigin: an explicit non-loopback URL (not host-derived) is trusted by construction", () => {
  ...
  assert.equal(isTrustedMetadataRequestOrigin(req({ host: "pub.example.com" }), "https://real.example.com"), true);
});
...
test("isTrustedMetadataRequestOrigin: a request with no resolvable host is not trusted", () => {
  assert.equal(
    isTrustedMetadataRequestOrigin(req({}), "https://real.example.com", null, HOST_DERIVED),
    false,
    "no host header"
  );
});
