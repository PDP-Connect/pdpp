...
import type { WireConnection } from "../server/routes/_route-contract.ts";
...
const idCtx = { canonicalConnectorKey: (v: string | null | undefined) => (v === "gmail-url" ? "gmail" : null) };
...
test("readConnectionTarget: absent path param yields null on the selected side", () => {
  assert.deepEqual(readConnectionTarget(idCtx, { params: {} }, "connection_id"), {
    connectionId: null,
    connectorKey: null,
  });
  assert.deepEqual(readConnectionTarget(idCtx, { params: {} }, "connector_id"), {
    connectionId: null,
    connectorKey: null,
  });
});
...
class FakeAmbiguousConnectionError extends Error {
  ...
  constructor(message: string, availableConnections: WireConnection[]) {
    super(message);
...
function ambiguousCtx() {
  return {
    ...
    listActiveBindingsForGrant: (): Promise<FakeBinding[]> =>
      Promise.resolve([
        { connectorInstanceId: "ci-1", displayName: "Personal" },
        { connectorInstanceId: "ci-2", displayName: "Work" },
      ]),
    projectBindingForWire: (b: FakeBinding): WireConnection | null => ({
      connection_id: b.connectorInstanceId,
      ...(b.displayName === null || b.displayName === undefined ? {} : { display_name: b.displayName }),
    }),
  };
}
...
test("rethrowAsAmbiguousConnection: filters out bindings that project to null", async () => {
  ...
  ctx.projectBindingForWire = (b: FakeBinding) =>
    b.connectorInstanceId === "ci-2" ? null : { connection_id: b.connectorInstanceId };
  await assert.rejects(
    () => rethrowAsAmbiguousConnection(ctx, { code: "ambiguous_connector_instance" }, "owner-1", "gmail"),
    (err) => {
      assert.deepEqual(
        (err as FakeAmbiguousConnectionError).availableConnections,
        [{ connection_id: "ci-1" }],
        "null projections dropped"
      );
      return true;
    }
  );
