...
function bareB64(obj: unknown): string {
...
test("lexical: encode -> decode round-trips {snap, off}", () => {
  const encoded = encodeSearchLexicalCursor({ off: 40, snap: "snap-1" });
  assert.deepEqual(decodeSearchLexicalCursor(encoded), { off: 40, snap: "snap-1" });
});

test("lexical: a bare base64url(JSON) with the right shape decodes (no prefix required)", () => {
  assert.deepEqual(decodeSearchLexicalCursor(bareB64({ off: 0, snap: "s" })), { off: 0, snap: "s" });
});
...
test("lexical: missing/mistyped snap or off -> null", () => {
  ...
  assert.equal(decodeSearchLexicalCursor(bareB64({ off: "x", snap: 5 })), null, "wrong types -> null");
});
...
test("semantic: encode -> decode round-trips {snap, off}", () => {
  const encoded = encodeSearchSemanticCursor({ off: 10, snap: "snap-2" });
  assert.deepEqual(decodeSearchSemanticCursor(encoded), { off: 10, snap: "snap-2" });
});

test("semantic: the encoded cursor carries the sem1. namespace prefix", () => {
  const encoded = encodeSearchSemanticCursor({ off: 0, snap: "s" });
...
test("semantic: a bare (unprefixed) payload is REJECTED even with the right shape", () => {
  assert.equal(
    decodeSearchSemanticCursor(bareB64({ off: 0, snap: "s" })),
    null,
...
test("semantic: missing/mistyped fields and garbage -> null", () => {
  ...
  // @ts-expect-error — deliberately passing a non-string to prove the runtime
  // tolerates it (returns null) even though the declared type is `string`.
...
test("cross: a semantic cursor does not decode as lexical, and vice-versa", () => {
  const semantic = encodeSearchSemanticCursor({ off: 3, snap: "s" });
  ...
  const lexical = encodeSearchLexicalCursor({ off: 3, snap: "s" });
