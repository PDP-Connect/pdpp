...
test('Collection Profile conformance', async (t) => {
  ...
  await t.test('pre-progress browser profile attach race remains runtime-retryable', async () => {
    ...
    try {
      ...
      const { body: runTimeline } = await fetchJson(`${asUrl}/_ref/runs/${encodeURIComponent(result.run_id)}/timeline`);
      const failedEvent = (runTimeline.data || []).find((event) => event.event_type === 'run.failed');
      assert.equal(failedEvent.data.known_gaps[0].recovery_hint?.action, 'retry_by_runtime');
      assert.equal(failedEvent.data.known_gaps[0].recovery_hint?.retryable, true);
    } finally {
      cleanup();
      await closeServer(server);
    }
...
`, 'utf-8');
  ...
  await t.test('runtime persists only truthful closed Chase and USAA browser-surface evidence', async () => {
    for (const { browserSurface, expectedSurface } of [
      ...
      const { connectorPath, cleanup } = createTestConnector([
        { type: 'SKIP_RESULT', stream: 'items', reason: 'export_affordance_missing', message: 'safe constant', diagnostics },
        { type: 'DONE', status: 'succeeded', records_emitted: 0 },
      ]);
      ...
      try {
        ...
        assert.deepEqual(skippedEvent.data.known_gap.diagnostics, expected);
        const completedEvent = (runTimeline.data || []).find((event) => event.event_type === 'run.completed');
        assert.deepEqual(completedEvent.data.known_gaps[0].diagnostics, expected);
        assert.deepEqual(result.known_gaps[0].diagnostics, expected);
        assert.doesNotMatch(JSON.stringify({ skippedEvent, completedEvent }), /private\.example|alice|accountId|123456|secret|raw_dom/i);
      } finally {
        cleanup();
        await closeServer(server);
      }
...
`, 'utf-8');
  ...
  await t.test('runtime persists collection_rate in spine events and terminal event', async () => {
    ...
    try {
      ...
      assert.equal(connectorDetail.connection_resolution, 'unresolved',
        'zero connections resolves to the typed unresolved projection');
      assert.equal(connectorDetail.connection_health, null,
  ...
  await t.test('connection_health.collection_rate reflects the latest rate-change event for a genuinely materialized (resolved) connection', async () => {
    ...
    try {
      ...
      assert.ok(connectorDetail, 'connector detail must be accessible');
      assert.equal(connectorDetail.connection_resolution, 'resolved',
