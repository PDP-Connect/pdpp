...
/**
 * Import-boundary guards for the `rs.connector-state.put` operation.
 *
...
const OP_REL = 'reference-implementation/operations/rs-connector-state-put/index.ts';

test('rs.connector-state.put operation has no host or storage concretes', () => {
  assertOperationBoundary(read(OP_REL), OP_REL);
});

test('rs.connector-state.put operation does not import server/index.js', () => {
...
test('rs.connector-state.put operation does not import server/records.js', () => {
  const src = read(OP_REL);
