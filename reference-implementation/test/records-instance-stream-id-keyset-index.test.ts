...
const INDEX_NAME = "idx_pg_records_instance_stream_id";
...
/**
 * The post-fix contract, asserted identically for both call sites: the index
 * is chosen, ALL FOUR predicates are absorbed into `Index Cond`, and nothing
 ...
 */
function assertKeysetIndexPlan(plan: string, limit: number): void {
  assert.ok(
    plan.includes(`Index Scan using ${INDEX_NAME}`),
    `LIMIT ${limit} must plan an index scan on ${INDEX_NAME}; got:\n${plan}`
  );
  ...
  for (const predicate of ["connector_instance_id =", "stream =", "deleted =", "id >"]) {
    assert.ok(
...
test(
  ...
  withPostgres(async () => {
    ...
    try {
      ...
      assert.ok(!plan.includes(INDEX_NAME), `pre-fix schema must not use ${INDEX_NAME}; got:\n${plan}`);
      assert.ok(
...
test(
  "PASS-AFTER: the keyset index serves the page query with every predicate in Index Cond and zero rows filtered",
  withPostgres(async () => {
    await seedInterleavedRecords(20_000, 2000);
    try {
      ...
      assert.equal(droppedRows.rowCount, 0, "precondition: the index is genuinely absent before re-bootstrap");
      ...
      assert.equal(indexRows.rowCount, 1, `bootstrapPostgresSchema must build ${INDEX_NAME}`);

      // Both call sites that issue this shape: the source-detail page
      // (LIMIT 50) and the semantic backfill's coverage scan (LIMIT 500).
      ...
      assertKeysetIndexPlan(await explainKeyset(50), 50);
      assertKeysetIndexPlan(await explainKeyset(500), 500);
    } finally {
      await cleanup();
    }
  })
