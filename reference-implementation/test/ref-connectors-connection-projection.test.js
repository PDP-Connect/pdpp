...
test('getConnectorSummaryForRoute scopes retained records to the resolved connection', withTmpDb(async () => {
  ...
  assert.deepEqual(
    [...scoped.stream_records].sort((a, b) => a.stream.localeCompare(b.stream)).map((entry) => ({
      stream: entry.stream,
      count_state: entry.count_state,
      record_count: entry.record_count,
    })),
    [
      { stream: 'files', count_state: 'known_zero', record_count: 0 },
      { stream: 'messages', count_state: 'known', record_count: 1 },
    ],
  );
}));

test('getConnectorSummaryForRoute keeps canonical stream counts current even while retained-byte evidence is dirty', withTmpDb(async () => {
  seedConnector();
  await seedInstances({ sourceKind: 'manual' });
  seedRecord({
    connectorInstanceId: WORK_INSTANCE_ID,
...
test('getConnectorSummaryForRoute exposes known_zero for declared streams once the canonical snapshot is current', withTmpDb(async () => {
  seedConnector();
  await seedInstances({ sourceKind: 'manual' });
  ...
  assert.deepEqual(
    ...
    [
      { stream: 'files', declaration_state: 'declared', count_state: 'known_zero', record_count: 0 },
      { stream: 'messages', declaration_state: 'declared', count_state: 'known_zero', record_count: 0 },
    ],
  );
}));
...
function seedBrowserBoundStaticSecretConnector({ browserRequired = true } = {}) {
  const manifest = {
    ...
    runtime_requirements: { bindings: { network: { required: true }, browser: { required: browserRequired } } },
    streams: [{ name: 'messages', primary_key: ['id'] }],
  };
  getDb()
    .prepare('INSERT INTO connectors(connector_id, manifest, created_at) VALUES (?, ?, ?)')
...
async function appendSessionRequiredRun({ connectorInstanceId, runId }) {
  const scheduler = createSqliteSchedulerStore();
  await Promise.resolve(
    scheduler.appendRunHistory({
      connectorId: CHATGPT_SHAPED_CONNECTOR_ID,
      connectorInstanceId,
      source: { kind: 'connector', id: CHATGPT_SHAPED_CONNECTOR_ID },
      status: 'failed',
      recordsEmitted: 0,
  ...
  await Promise.resolve(
    scheduler.upsertLastRunTime(
      connectorInstanceId,
      Date.parse(NOW),
      NOW,
      CHATGPT_SHAPED_CONNECTOR_ID,
    ),
  );
}

test('ChatGPT-shaped static-secret connection with an unrejected credential and no surface rows projects browser_session repair', withTmpDb(async () => {
  ...
  await withCredentialKey(TEST_CREDENTIAL_KEY, async () => {
    ...
    assert.equal(scoped.rendered_verdict.required_actions.find((action) => action.kind === 'reauth')?.surface?.kind, 'browser_session');
  });
}));
...
test('an optional browser binding does not authorize browser-session repair', withTmpDb(async () => {
