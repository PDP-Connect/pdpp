...
const TOP_LEVEL_REGEX_1 = /network create/;
const TOP_LEVEL_REGEX_2 = /^network rm (\S+)$/m;
const TOP_LEVEL_REGEX_3 = /^(pdpp|pdpp_default|pdpp-reference)$/;
const TOP_LEVEL_REGEX_4 = /REGRESSION_TEST_ENSURE_NETWORK_OK/;
const TOP_LEVEL_REGEX_5 = /network inspect/;
const TOP_LEVEL_REGEX_6 = /network create/;
...
const TOP_LEVEL_REGEX_14 = /"\$\{DC\[@\]\}" up -d --force-recreate --no-deps neko-allocator/;
const TOP_LEVEL_REGEX_15 = /assert_surface_continuity "after forced allocator recreation"/;
...
function makeFakeDockerBin(dir: string, behaviorScript: string): string {
...
test("docker-neko-network-durability-smoke.sh synthesizes a fresh, non-live project/network name on every invocation", () => {
  ...
  const extractNetworkRmArg = (calls: string): string => {
    const match = calls.match(TOP_LEVEL_REGEX_2);
    assert.ok(match, `expected a "network rm <name>" call in: ${calls}`);
    const [, name] = match;
    assert.ok(name, "network rm capture group is empty");
    return name;
  };
  const firstNetworkName = extractNetworkRmArg(firstRunCalls);
  const secondNetworkName = extractNetworkRmArg(secondRunCalls);

  const liveOrDefaultRe = TOP_LEVEL_REGEX_3;
  ...
  assert.notEqual(
    firstNetworkName,
    secondNetworkName,
    "two invocations must synthesize distinct throwaway identifiers, not a fixed name"
  );
...
exit 0`
  ...
  const result = spawnSync(
    ...
    [
      "-c",
      `source "${REFERENCE_STACK_SCRIPT}" && ensure_dynamic_surface_network && echo REGRESSION_TEST_ENSURE_NETWORK_OK`,
    ],
    {
      encoding: "utf8",
      env: {
  ...
  assert.match(result.stdout, TOP_LEVEL_REGEX_4);

  const calls = readFileSync(logPath, "utf8");
  assert.match(calls, TOP_LEVEL_REGEX_5);
  assert.match(calls, TOP_LEVEL_REGEX_6);
  rmSync(fakeDockerDir, { force: true, recursive: true });
});
...
test("reference-stack.sh ensure_dynamic_surface_network fails closed when the network can neither be found nor created", () => {
  ...
  makeFakeDockerBin(fakeDockerDir, `if [[ "$1" == "network" ]]; then exit 1; fi\nexit 0`);
  ...
  assert.notEqual(
    result.status,
    0,
    "a genuine (non-race) failure to create or confirm the network must still fail closed"
  );
  assert.doesNotMatch(result.stdout, TOP_LEVEL_REGEX_7);
  rmSync(fakeDockerDir, { force: true, recursive: true });
});
...
exit 0`
  ...
  assert.notEqual(
    result.status,
    0,
    "smoke should fail fast once the fake docker reports unreachable, after cleanup() has already run"
  );
  ...
  assert.match(
    psCall,
    TOP_LEVEL_REGEX_8,
    "cleanup's docker ps filter must reference this run's own synthesized deployment_id"
  );
  assert.doesNotMatch(
    psCall,
    TOP_LEVEL_REGEX_9,
    "cleanup must not filter solely by the fixed, cross-invocation-shared surface_id literal"
  );

  const psIndex = calls.indexOf(psCall);
  const inspectIndex = calls.indexOf("inspect -f");
  assert.ok(
    inspectIndex > psIndex,
    "a docker inspect verification call must happen after docker ps and before any docker rm"
  );
...
test("docker-neko-network-durability-smoke.sh never honors an inherited PDPP_NEKO_PROFILE_STORAGE_ROOT", () => {
  ...
  const poisonedProfileRoot = join(
    mkdtempSync(join(tmpdir(), "pdpp-poisoned-profile-root-")),
...
test("reference-stack.sh ensure_dynamic_surface_network is a no-op when the network already exists", () => {
  ...
  assert.match(result.stdout, TOP_LEVEL_REGEX_21);
  const calls = readFileSync(logPath, "utf8");
  assert.match(calls, TOP_LEVEL_REGEX_22);
  assert.doesNotMatch(calls, TOP_LEVEL_REGEX_1, "must not attempt to create a network that already exists");
  rmSync(fakeDockerDir, { force: true, recursive: true });
});
