...
test("resolves polyfill GitHub connector when active manifest is the polyfill manifest", () => {
  ...
  const resolved = resolveDefaultConnectorPath(polyfillGithub.connector_id, polyfillGithub);
  assert.ok(resolved, "polyfill GitHub run must resolve to a runnable connector path");
  assert.match(resolved, TOP_LEVEL_REGEX_1, `expected polyfill connector, got ${resolved}`);
  assert.doesNotMatch(
    resolved,
    TOP_LEVEL_REGEX_2,
...
test("resolves reference seed when active manifest is the reference GitHub fixture", () => {
  ...
  assert.match(resolved, TOP_LEVEL_REGEX_3, `expected reference seed, got ${resolved}`);
});
...
test("prefers polyfill path when no manifest is provided and a polyfill implementation exists", () => {
  ...
  assert.match(resolved, TOP_LEVEL_REGEX_4, `expected polyfill connector without manifest hint, got ${resolved}`);
});
...
test("resolves canonical connector keys for URL-shaped reference fixture manifests", () => {
  ...
  assert.match(resolved, TOP_LEVEL_REGEX_6, `expected reference seed for canonical spotify key, got ${resolved}`);
});
