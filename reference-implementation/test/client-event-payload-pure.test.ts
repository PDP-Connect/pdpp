...
const derivedEvent: DerivedEvent = {
  data: { connector_id: "r1", stream: "orders" },
...
test("buildEventPayload: id/type/time come from the event id and the derived event", () => {
  ...
  assert.equal(payload.type, "pdpp.records.changed");
...
test("buildEventPayload: data merges subscription_id with the event data", () => {
  ...
  assert.equal(payload.data.connector_id, "r1", "event data preserved");
