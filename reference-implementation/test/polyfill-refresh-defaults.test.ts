...
const KNOWN_POSTURES = new Set(["none", "credentials", "otp_likely", "manual_action_likely"]);
...
test("every first-party manifest declares capabilities.refresh_policy", () => {
  ...
  for (const name of POLYFILL_MANIFEST_NAMES) {
    ...
    assert.ok(typeof policy.recommended_mode === "string", `${name}: recommended_mode is required`);
    assert.ok(
    ...
    assert.equal(typeof policy.rationale, "string", `${name}: rationale must be a non-empty owner-readable string`);
    assert.ok(typeof policy.rationale === "string", `${name}: rationale must be a string`);
    assert.ok(policy.rationale.trim().length > 0, `${name}: rationale must be a non-empty owner-readable string`);
...
test("automatic first-party manifests declare cadence + background_safe + a staleness window", () => {
  for (const name of POLYFILL_MANIFEST_NAMES) {
    const manifest = readManifest(name);
    const policy = manifest.capabilities?.refresh_policy;
    if (policy?.recommended_mode !== "automatic") {
      continue;
    }
    assert.equal(
    ...
    assert.ok(
      typeof policy.recommended_interval_seconds === "number" && typeof policy.minimum_interval_seconds === "number",
      `${name}: recommended_interval_seconds and minimum_interval_seconds must be numbers`
    );
    assert.ok(
    ...
    assert.equal(policy.background_safe, true, `${name}: automatic policy must set background_safe: true`);
    ...
    assert.ok(
      typeof policy.maximum_staleness_seconds === "number",
      `${name}: maximum_staleness_seconds must be a number`
    );
    assert.ok(
...
test("interaction_posture is consistent with human_interaction", () => {
  for (const name of POLYFILL_MANIFEST_NAMES) {
    ...
    assert.ok(Array.isArray(interactions), `${name}: capabilities.human_interaction must be an array when declared`);
