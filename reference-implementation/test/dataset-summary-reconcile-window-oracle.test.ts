...
interface DirtyStreamProjectionRow {
...
test("dirty time-bound reconcile reports deferred and residual rows while preserving dirty guards", async () =>
  withTempDb(async () => {
    ...
    const result = await reconcileDirtyDatasetSummaryRecordTimeBounds({
      ...
      getStreamRecordTimeBounds: async (connectorId: string, stream: string, field: string) => {
        seen.push(stream);
    ...
    const rows = getDb()
      ...
      .all() as DirtyStreamProjectionRow[];
    const rowByStream = new Map(rows.map((row) => [row.stream, row]));

    const deferredRow = rowByStream.get(deferredStream);
    assert.ok(deferredRow, `${deferredStream} should have a projection row`);
    assert.equal(Number(deferredRow.dirty_record_time_bounds), 1);
    assert.equal(deferredRow.computed_at, CONCURRENT_COMPUTED_AT);
    ...
    for (const stream of seen.filter((stream) => stream !== deferredStream)) {
      ...
      assert.ok(row, `${stream} should have a projection row`);
    ...
    const overflowStreams = rows.map((row) => row.stream).filter((stream) => !seen.includes(stream));
