...
test("verifyOwnerCsrfToken: rejects the wrong secret", () => {
  const token = issueOwnerCsrfToken(secretA);
  assert.equal(
    verifyOwnerCsrfToken(token, secretB),
    false,
    "a token minted with secretA must not verify under secretB"
  );
});
...
test("buildOwnerCsrfSetCookie: default attributes (HttpOnly, SameSite=Lax, Path, Max-Age)", () => {
  ...
  assert.ok(REGEXP_1.test(cookie), "a numeric Max-Age");
...
test("buildOwnerCsrfSetCookie: secure + strict + explicit max-age", () => {
  const cookie = buildOwnerCsrfSetCookie("TOK", { maxAgeSeconds: 3600, sameSite: "strict", secure: true });
