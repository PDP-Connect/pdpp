...
import { __resetControllerInteractionStateForTests } from '../runtime/controller.ts';
...
test('viewport wire normalizes before validating so fractional dimensions never become zero downstream', () => {
  assert.equal(normalizeReferenceWireViewportPayload({ width: 0.5, height: 800 }), null);
  assert.equal(normalizeReferenceWireViewportPayload({ width: 800, height: 0.5 }), null);
  assert.deepEqual(
    normalizeReferenceWireViewportPayload({ width: 390.9, height: 844.9 }),
...
function presentationAttachmentCookie(response) {
...
function createInjectedTimers(clock) {
...
async function waitForCondition(condition, message, attempts = 40) {
...
function buildManualActionConnector(tmpDir, { kind = 'manual_action', timeoutSeconds = 60 } = {}) {
...
function makeAbortableNekoSleep() {
  return (_ms, signal) =>
    new Promise((resolve) => {
...
function makePhonePresentationCompanion({ screenSelections, windowAcknowledgements }) {
  const baseline = { width: 1440, height: 900, rate: 30 };
  ...
  return ({ browser_session_id }) => {
    const fetchImpl = async (url, init = {}) => {
      ...
      if (url === 'https://neko.test/api/room/screen' && method === 'GET') return jsonResponse(baseline);
      if (url === 'https://neko.test/api/room/screen/configurations' && method === 'GET') return jsonResponse(configurations);
...
function makeGatedNekoCompanion(stop) {
  return ({ browser_session_id }) => ({
    backend: 'neko',
    browser_session_id,
    async start() {},
    ...
    async dispatch() {},
    async ackFrame() {},
    getNekoProxyTarget() { return { origin: 'http://127.0.0.1:9' }; },
  });
}
...
async function withHarness(options, fn) {
  ...
  const server = await startServer({
    ...
    nekoWindowSettleProbe:
      ...
      (async () => ({ ok: true, json: async () => ({ settled: true, width: 1440, height: 900 }) })),
...
async function withExpiredPresentation(harnessOptions, fn) {
...
test('mint derives the presentation endpoint from live behavior rather than surface metadata', async () => {
  ...
  await withHarness({
    ...
    nekoWindowSettleProbe: async (endpoint) => {
      ...
      return { ok: true, json: async () => ({ settled: true, width: 1440, height: 900 }) };
    },
  }, async ({ asUrl, spotifyManifest, companions }) => {
    const started = await startRun(asUrl, spotifyManifest.connector_id);
    try {
      const acquired = leaseManager.acquire({ connectorId, runId: started.run_id, profileKey: 'profile_dynamic_1' });
      assert.equal(acquired.lease.status, 'leased');
      const pending = await waitForPendingInteraction(asUrl, started.run_id);
      const mint = await fetchJson(`${asUrl}/_ref/runs/${encodeURIComponent(started.run_id)}/run-interaction-stream`, {
        method: 'POST',
...
test('mint fails loudly before viewer creation when the live window-settle behavior is absent', async () => {
  ...
  await withHarness(
    {
      browserSurfaceLeaseManager: leaseManager,
      nekoWindowSettleProbe: async () => ({ ok: false, json: async () => ({}) }),
      makeCompanion() {
    ...
    async ({ asUrl, spotifyManifest }) => {
      ...
      try {
        ...
        assert.equal(mint.body.error.code, 'managed_surface_window_settle_unavailable');
        assert.match(mint.body.error.message, /retry/i);
...
test('mint capability probe observes the current geometry without changing it', async () => {
  const connectorId = 'https://registry.pdpp.org/connectors/spotify';
  const leaseManager = makeLeaseManager({ connectorId });
  const before = { width: 412, height: 915 };
  ...
  await withHarness(
    {
      ...
      nekoWindowSettleProbe: async (endpoint) => {
        ...
        if (url.pathname === '/pdpp/window-settle') {
          ...
          return { ok: true, json: async () => ({ settled: true, ...currentScreen }) };
        }
  ...
  try {
    await withHarness(
      ...
      async ({ asUrl, spotifyManifest }) => {
        ...
        assert.equal(bearerOnlyMutation.body.error.code, 'presentation_attachment_not_controlling');
        assert.equal(
        ...
        const observerStream = await fetch(`${asUrl}${mint.body.viewer_path}`);
        ...
        assert.equal(
          upstreamRequests.find((request) => request.method === 'POST')?.cookie.includes(controllerCookie.split('=', 1)[0]),
          false,
...
test('input POST dispatches to the companion after attach and rejects bad input', async () => {
  await withHarness({}, async ({ asUrl, spotifyManifest, companions }) => {
    ...
    const fractionalViewport = await fetchJson(`${asUrl}${mint.body.viewport_path}`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Cookie: attachmentCookie },
      body: JSON.stringify({ width: 0.5, height: 844 }),
    });
    assert.equal(fractionalViewport.status, 400);
    assert.equal(fractionalViewport.body.error.code, 'invalid_request');
...
test('only the controlling SSE attachment may rotate the presentation viewport', async () => {
  ...
  await withHarness(
    ...
    async ({ asUrl, spotifyManifest }) => {
      ...
      const controllerStream = await fetch(`${asUrl}${mint.body.viewer_path}`, { signal: controllerAbort.signal });
      const controllerCookie = presentationAttachmentCookie(controllerStream);
      const observerAbort = new AbortController();
      const observerStream = await fetch(`${asUrl}${mint.body.viewer_path}`, { signal: observerAbort.signal });
      assert.equal(observerStream.status, 200);

      const observerViewport = await fetchJson(`${asUrl}${mint.body.viewport_path}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ width: 844, height: 390 }),
      });
      assert.equal(observerViewport.status, 409);
      assert.equal(observerViewport.body.error.code, 'presentation_attachment_not_controlling');
      assert.equal(dispatched.length, 0);

      const observerInput = await fetchJson(`${asUrl}${mint.body.input_path}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ type: 'mouse', action: 'click', x: 4, y: 5 }),
      });
      assert.equal(observerInput.status, 409);
      assert.equal(observerInput.body.error.code, 'presentation_attachment_not_controlling');
      assert.equal(dispatched.length, 0);

      const controllingViewport = await fetchJson(`${asUrl}${mint.body.viewport_path}`, {
        method: 'POST',
      ...
      const controllingInput = await fetchJson(`${asUrl}${mint.body.input_path}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Cookie: controllerCookie },
        body: JSON.stringify({ type: 'mouse', action: 'click', x: 4, y: 5 }),
      });
...
test('two stream sessions in one cookie jar retain session-scoped controller authority', async () => {
  ...
  await withHarness(
    ...
    async ({ asUrl, server, spotifyManifest }) => {
      ...
      const [mintA, mintB] = await Promise.all([runA, runB].map(async (run, index) => {
        const pending = index === 0 ? pendingA : pendingB;
        const minted = await fetchJson(`${asUrl}/_ref/runs/${encodeURIComponent(run.run_id)}/run-interaction-stream`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ interaction_id: pending.interaction_id }),
        });
      ...
      try {
        ...
        for (const [path, body] of [
          [mintA.viewport_path, { width: 844, height: 390 }],
          [mintA.input_path, { type: 'mouse', action: 'click', x: 4, y: 5 }],
        ]) {
          ...
          assert.equal(crossSession.body.error.code, 'presentation_attachment_not_controlling');
        }
        assert.equal(dispatched.length, 0, 'another session cookie must not dispatch into session A');

        for (const [path, body] of [
          [mintA.viewport_path, { width: 390, height: 844 }],
          [mintA.input_path, { type: 'mouse', action: 'click', x: 7, y: 8 }],
        ]) {
          ...
          assert.equal(observerMutation.body.error.code, 'presentation_attachment_not_controlling');
        }
        assert.equal(dispatched.length, 0, 'observer/no-cookie attachments stay read-only');

        for (const [path, cookie, body] of [
          [mintA.viewport_path, cookieA, { width: 390, height: 844 }],
          [mintA.input_path, cookieA, { type: 'mouse', action: 'click', x: 9, y: 10 }],
          [mintB.viewport_path, cookieB, { width: 844, height: 390 }],
          [mintB.input_path, cookieB, { type: 'mouse', action: 'click', x: 11, y: 12 }],
        ]) {
...
test('the controlling stream attachment selects phone portrait and landscape screens, acknowledges the window size, and restores the desktop baseline', async () => {
  const screenSelections = [];
  const windowAcknowledgements = [];
  await withHarness(
    ...
    async ({ asUrl, spotifyManifest }) => {
      ...
      const observerViewport = await fetchJson(`${asUrl}${mint.body.viewport_path}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ width: 412, height: 915 }),
      });
      ...
      for (const viewport of [{ width: 412, height: 915 }, { width: 915, height: 412 }]) {
        const response = await fetchJson(`${asUrl}${mint.body.viewport_path}`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', Cookie: controllerCookie },
        ...
        assert.deepEqual(
          { width: response.body.viewport.width, height: response.body.viewport.height },
          viewport,
        );
      }
      ...
      assert.deepEqual(
        windowAcknowledgements.map(({ width, height }) => ({ width, height })),
        [{ width: 412, height: 915 }, { width: 915, height: 412 }],
      );
...
test('controller-cookie reconnect keeps viewport updates ordered while an observer remains read-only', async () => {
  ...
  await withHarness(
    ...
    async ({ asUrl, spotifyManifest }) => {
      ...
      const initialController = await fetch(`${asUrl}${mint.body.viewer_path}`, { signal: initialControllerAbort.signal });
      ...
      let landscape;
      try {
        const controllerReconnect = await fetch(`${asUrl}${mint.body.viewer_path}`, {
        ...
        portrait = fetchJson(`${asUrl}${mint.body.viewport_path}`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', Cookie: controllerCookie },
          body: JSON.stringify({ width: 390, height: 844 }),
        });
        await waitForCondition(() => dispatchStarted.length === 1, 'first controller viewport must reach the companion');

        landscape = fetchJson(`${asUrl}${mint.body.viewport_path}`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', Cookie: controllerCookie },
          body: JSON.stringify({ width: 844, height: 390 }),
        });
        await new Promise((resolve) => setTimeout(resolve, 10));
        assert.equal(dispatchStarted.length, 1, 'later controller viewport must not interleave before the first dispatch');
      } finally {
...
test('interaction timeout waits for presentation restoration before the runtime commits its terminal response', async () => {
  ...
  await withHarness(
    ...
    async ({ asUrl, spotifyManifest }) => {
      ...
      assert.equal(beforeRestore.body.data.some((event) => event.event_type === 'run.interaction_completed'), false);
      releaseStop();
      const terminal = await waitForRunTerminal(asUrl, started.run_id);
      assert.equal(terminal.data.some((event) => event.event_type === 'run.interaction_completed'), true);
      abort.abort();
    },
  );
});
  ...
  await withHarness(
    {
      makeCompanion: ({ browser_session_id }) => ({
        ...
        async start() {},
        async stop() {
          const err = new Error('restore rejected');
    ...
    async ({ asUrl, spotifyManifest }) => {
      ...
      assert.equal(timeline.data.some((event) => event.event_type === 'run.cancelled'), true);
      abort.abort();
    },
  );
});
...
test('boot recycles captured and unrestored presentation surfaces before they can be reused', async () => {
  const state = {
    ...
    async listUnrestored() {
      return [{
        ...
        baseline: { width: 1280, height: 720, rate: 30 },
      }];
    },
    async markRecycled(browserSessionId) {
  ...
  try {
    ...
    assert.equal(manager.getSurface('surface_dynamic_1') == null, true);
  } finally {
    await closeServer(server);
  }
});
  ...
  await withExpiredPresentation(
    ...
    async ({ abort, asUrl, started }) => {
      try {
        ...
        const timeline = await fetchJson(`${asUrl}/_ref/runs/${encodeURIComponent(started.run_id)}/timeline`);
        assert.equal(timeline.body.data.some((event) => event.event_type === 'run.interaction_completed'), false);
      } finally {
        gate.release();
      }
      await waitForRunTerminal(asUrl, started.run_id);
...
test('POST cancel waits for presentation restore after the bearer session expires', async () => {
  ...
  await withExpiredPresentation(
    ...
    async ({ abort, asUrl, started }) => {
      ...
      let settled = false;
      try {
        cancellation = fetchJson(`${asUrl}/_ref/runs/${encodeURIComponent(started.run_id)}/cancel`, { method: 'POST' }).finally(() => {
          settled = true;
        });
