const TOP_LEVEL_REGEX_1 = /injected fault/;
...
function readVersionCounter(): number | null {
  const row = getDb()
    .prepare(
      `SELECT max_version FROM version_counter
       WHERE connector_id = ? AND stream = ?`
    )
    .get(CONNECTOR_ID, STREAM) as { max_version: number } | undefined;
...
function readLiveRecord(recordKey: string): LiveRecord {
  return getDb()
    .prepare(
    ...
    .get(CONNECTOR_ID, STREAM, recordKey) as LiveRecord;
}
...
test("failure between live mutation and record_changes append rolls back the whole durable unit", async () => {
  ...
  try {
    ...
    await assert.rejects(() => ingestRecord(CONNECTOR_ID, makeUpsert("a", { v: 2 })), TOP_LEVEL_REGEX_1);
