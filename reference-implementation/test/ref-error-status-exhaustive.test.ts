...
test("typeFor produces the right envelope type for every status the table emits", () => {
  const statusToType: Readonly<Record<number, string>> = {
  ...
  for (const status of statusesInTable) {
    ...
    const expectedType = statusToType[status];
    assert.ok(expectedType, `statusToType must define an entry for ${status}`);
    assert.equal(typeFor(status), expectedType, `status ${status} → ${expectedType}`);
  }
});
