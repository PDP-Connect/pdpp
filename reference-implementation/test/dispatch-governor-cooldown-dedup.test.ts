...
import type { SourcePressureCooldownDecision } from "../runtime/scheduler-source-pressure-cooldown.ts";

const REGEXP_1 = /source_pressure_cooldown_applied/;

function createRuntime(): DispatchGovernorRuntimeState {
  return {
...
function createGovernor(runtime: DispatchGovernorRuntimeState) {
...
function cooldown(overrides: Partial<SourcePressureCooldownDecision> = {}): SourcePressureCooldownDecision {
  return {
...
test("resolveCooldownSkip dedupes repeated cooldown identity and re-arms when pressure clears", (t) => {
  ...
  assert.match(firstSkip.error ?? "", REGEXP_1);
