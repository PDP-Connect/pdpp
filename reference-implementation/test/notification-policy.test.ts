...
test("rendered verdict push policy only interrupts on attention with owner-satisfiable primary action", () => {
  ...
  assert.equal(shouldFanoutRenderedVerdict({ channel: "advisory", required_actions: [action] }), false);
  assert.equal(shouldFanoutRenderedVerdict({ channel: "calm", required_actions: [action] }), false);
  assert.equal(
    shouldFanoutRenderedVerdict({
      channel: "attention",
...
test("notification delivery keeps dashboard inbox durable while quieting informational push", () => {
  const quietWindow = { enabled: true, end: "07:00", start: "22:00", timeZone: "UTC" };
