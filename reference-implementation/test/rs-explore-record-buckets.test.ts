...
import {
  ...
  type ExploreRecordBucketsOutput,
...
const RECORDS = [
  { emitted_at: "2024-01-15T12:00:00.000Z", key: "a-jan", partition: PARTITION_A, stream: "alpha" },
  { emitted_at: "2024-03-20T12:00:00.000Z", key: "a-mar", partition: PARTITION_A, stream: "alpha" },
  { emitted_at: "2025-07-04T12:00:00.000Z", key: "b-jul", partition: PARTITION_B, stream: "beta" },
  { emitted_at: "2026-05-10T12:00:00.000Z", key: "a-may", partition: PARTITION_A, stream: "gamma" },
  { emitted_at: "2027-01-01T12:00:00.000Z", key: "b-future", partition: PARTITION_B, stream: "beta" },
];
...
function bucketMap(result: ExploreRecordBucketsOutput) {
  return new Map(result.buckets.map((bucket) => [bucket.start, bucket.count]));
}

async function assertBucketBehavior(label: string, deps: ExploreRecordBucketsDependencies) {
  await seedRecords();
  ...
  assert.deepEqual(
    scoped.buckets.map((bucket) => bucket.count),
    [1, 0, 1]
  );
}
...
if (POSTGRES_URL) {
  ...
  test(
    "rs.explore.record_buckets: Postgres returns dense extent-aware exact buckets (skipped: PDPP_TEST_POSTGRES_URL unset)",
    {
