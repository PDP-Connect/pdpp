...
import { automationModeCopy, projectRunAutomationPolicy } from "../runtime/run-automation-policy.ts";
...
test("projectRunAutomationPolicy defaults deployment readiness to ready when null or omitted", () => {
  assert.deepEqual(
    projectRunAutomationPolicy({ deploymentReadiness: null, triggerKind: "scheduled" }).deployment_readiness,
    { ready: true }
  );
  assert.deepEqual(projectRunAutomationPolicy({ triggerKind: "scheduled" }).deployment_readiness, { ready: true });
});
...
test("projectRunAutomationPolicy keeps recommended_mode=manual blocked unless background_safe=true is declared", () => {
  ...
  assert.equal(p.notification_posture, "informational");
  assert.ok(p.reason);
  assert.match(p.reason, REGEXP_1);
});
...
test("projectRunAutomationPolicy lets a manual trigger start a manual_only (paused) policy", () => {
  ...
  assert.equal(p.notification_posture, "none");
  assert.ok(p.reason);
  assert.match(p.reason, REGEXP_2);
});
...
test("automationModeCopy returns the exact owner-facing copy per mode", () => {
  assert.equal(automationModeCopy("unattended"), "Can refresh in the background without expected owner action.");
  assert.equal(automationModeCopy("assisted"), "Can start in the background and may ask for bounded owner assistance.");
  assert.equal(
...
test("automationModeCopy falls back to the manual-gesture copy for an unknown mode", () => {
  // @ts-expect-error 'bogus' is not a RunAutomationMode — proves the fallback
  // branch handles a value outside the typed union, e.g. from an untyped caller.
