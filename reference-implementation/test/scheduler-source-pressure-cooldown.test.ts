const TOP_LEVEL_REGEX_1 = /rate_limited/;
...
function gap(overrides: Partial<PendingPressureGap> = {}): PendingPressureGap {
...
test("rate_limited is also a pressure reason", () => {
  ...
  assert.equal(decision.cooldownApplied, true);
  assert.match(decision.identity || "", TOP_LEVEL_REGEX_1);
});
...
test("run_cap_deferred is not a source-pressure reason and does not engage cooldown", () => {
  ...
  const decision = computeSourcePressureCooldown([gap({ reason: "run_cap_deferred" })], BASE_INTERVAL_MS, T0);
...
test("each additional unrecovered recovery attempt strictly increases the cooldown", () => {
  ...
  for (let i = 1; i < intervals.length; i++) {
    ...
    assert.ok(current !== undefined && previous !== undefined, `expected intervals at ${i - 1} and ${i}`);
    assert.ok(
      current > previous,
      `attempt_count=${i} (${current}ms) must exceed attempt_count=${i - 1} (${previous}ms)`
    );
  }
});
