...
function expectedTypeByStatus(status: number): string {
  switch (status) {
    case 400:
...
test("typeFor(codeToStatus[code]) pins every mapped code round trip", () => {
  ...
  for (const [code, expectedType] of Object.entries(namedCodeTypes)) {
    ...
    assert.ok(status !== undefined, `${code} must map to a status`);
    assert.equal(typeFor(status), expectedType, `${code} must round-trip to ${expectedType}`);
  }
});
