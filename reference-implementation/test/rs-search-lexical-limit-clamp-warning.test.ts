...
function limitWarnings(query: Record<string, unknown>) {
...
test("an over-cap limit clamps the value and emits exactly one structured limit_clamped warning", () => {
  const parsed = parseSearchLexicalParams({ limit: "500", q: "foo" });
  ...
  const w = warns[0];
  assert.ok(w, "expected a limit_clamped warning");
  assert.equal(w.code, "limit_clamped");
  assert.equal(w.param, "limit");
  assert.deepEqual(w.detail, { max_limit: MAX_LIMIT, requested_limit: 500 });
});

test("a limit exactly at the cap is in-range: clamps to the cap value but emits no warning", () => {
  const parsed = parseSearchLexicalParams({ limit: "100", q: "foo" });
  assert.equal(parsed.limit, MAX_LIMIT);
  assert.equal(limitWarnings({ limit: "100", q: "foo" }).length, 0, "exactly MAX_LIMIT is not a clamp");
});

test("a limit one over the cap is the first value that warns", () => {
  const warns = limitWarnings({ limit: "101", q: "foo" });
  assert.equal(warns.length, 1);
  // biome-ignore lint/style/useDestructuring: localized test assertion preserves its explicit contract.
  const warn = warns[0];
  assert.ok(warn, "expected a limit_clamped warning");
  assert.deepEqual(warn.detail, { max_limit: MAX_LIMIT, requested_limit: 101 });
});
...
test("an over-cap fractional limit reports the floored requested value", () => {
  ...
  const warns = limitWarnings({ limit: "150.9", q: "foo" });
  assert.equal(warns.length, 1);
  // biome-ignore lint/style/useDestructuring: localized test assertion preserves its explicit contract.
  const warn = warns[0];
  assert.ok(warn?.detail, "expected a limit_clamped warning with detail");
  assert.equal(warn.detail.requested_limit, 150);
});
...
test("absent, zero, negative, and non-numeric limits fall back to default with NO warning", () => {
  ...
  for (const query of [
    { q: "foo" }, // absent
    { limit: "0", q: "foo" }, // zero
    { limit: "-5", q: "foo" }, // negative
    { limit: "abc", q: "foo" }, // non-numeric
    { limit: "", q: "foo" }, // empty string
  ]) {
...
test("an in-range explicit limit below the cap neither changes nor warns", () => {
  const parsed = parseSearchLexicalParams({ limit: "7", q: "foo" });
