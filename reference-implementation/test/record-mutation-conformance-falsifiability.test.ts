...
import { createBrokenInMemoryRecordMutationDriver } from "./helpers/broken-record-mutation-driver.ts";
import { runRecordMutationConformance } from "./helpers/record-mutation-conformance.ts";

const REGEXP_1 = /rolls back the durable unit/;
...
test("harness detects at least one durable-mutation invariant violation in a broken driver", async () => {
  ...
  const rollbackFailed = failures.some((f) => REGEXP_1.test(f.name));
  assert.ok(
    rollbackFailed,
