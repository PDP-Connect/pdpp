...
test("validateAuthorizePkce: a valid code/S256/43-char request passes", () => {
  assert.doesNotThrow(() =>
    validateAuthorizePkce({ codeChallenge: goodChallenge, codeChallengeMethod: "S256", responseType: "code" })
  );
});

test("validateAuthorizePkce: non-code response_type is unsupported_response_type", () => {
  expectCode(
    () => validateAuthorizePkce({ codeChallenge: goodChallenge, codeChallengeMethod: "S256", responseType: "token" }),
...
test("validateAuthorizePkce: non-S256 challenge method is invalid_request", () => {
  expectCode(
    () => validateAuthorizePkce({ codeChallenge: goodChallenge, codeChallengeMethod: "plain", responseType: "code" }),
    "invalid_request"
  );
});

test("validateAuthorizePkce: code_challenge length bounds are 43..128 inclusive", () => {
  const base = { codeChallengeMethod: "S256", responseType: "code" };
  ...
  // runtime guard rejects a non-string value even though the type system
  // would otherwise disallow constructing this call.
...
interface HostedMcpAuthorizationDetailsEntry {
  access_mode: string;
...
function isHostedMcpAuthorizationDetailsEntry(value: unknown): value is HostedMcpAuthorizationDetailsEntry {
