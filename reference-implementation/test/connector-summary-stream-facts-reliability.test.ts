...
// Shape of one stream's stored latest-attempt entry, mirroring the
// module-private `StoredStreamFactEntry` in
// `server/connector-summary-read-model.ts` (redeclared here since that type
// is not exported). This file's assertions only ever need
// `checkpoint`/`collected` off the raw fact.
...
function factsFor(evidence: { stream_latest_facts?: unknown } | null): StreamFactsByStream | null {
...
test("bounded first pass: maxEvents:1 processes AT MOST one event (never the whole in-flight batch) and stays stale/incomplete", async () => {
  await withTempDb(async () => {
    ...
    seedTerminalEvent({
      connectorInstanceId: "cin_a",
      runId: "run_2",
      streams: [{ checkpoint: "committed", collected: 1, stream: "threads" }],
    });
    seedTerminalEvent({
      connectorInstanceId: "cin_a",
      runId: "run_3",
      streams: [{ checkpoint: "committed", collected: 1, stream: "labels" }],
    });
    ...
    assert.equal(
      ...
      1,
      "the resume cursor is EXACTLY the one event actually processed, not the whole in-flight batch"
    );
    ...
    assert.ok(row.stream_latest_facts_json, "row must have a fact map after the bounded pass");
    const facts: StreamFactsByStream = JSON.parse(row.stream_latest_facts_json);
    assert.equal(
      Object.keys(facts).length,
      1,
      "exactly the ONE event actually processed is folded — maxEvents:1 is a real ceiling, not merely an early-exit hint"
    );
    ...
    assert.ok(evidence, "evidence row for cin_a must exist");
  ...
  await withTempDb(async () => {
    ...
    const OLD_LOGIC_FACTS = {
      messages: {
        event_seq: 2,
        evidence_as_of: null,
        fact: { checkpoint: "not_staged", collected: 999, stream: "messages" },
        run_id: "old_run",
      },
      threads: {
        event_seq: 2,
        evidence_as_of: null,
        fact: { checkpoint: "not_staged", collected: 999, stream: "threads" },
        run_id: "old_run",
      },
    };
    getDb()
    ...
    assert.ok(row.stream_latest_facts_json, "row must have a fact map after the bounded replay");
    const facts: StreamFactsByStream = JSON.parse(row.stream_latest_facts_json);
    assert.equal(
      Object.keys(facts).length,
      1,
      "only the ONE stream the replay actually re-derived this round is present"
    );
    const [entryPair] = Object.entries(facts);
    assert.ok(entryPair, "exactly one surviving stream entry must be present");
    const [, entry] = entryPair;
    assert.equal(
      entry.fact.collected,
      1,
      "the surviving fact is the NEW-logic value, never the old collected:999 leftover"
    );
      ...
      .run(
        JSON.stringify({
          messages: {
            event_seq: 1,
            evidence_as_of: null,
            fact: { checkpoint: "committed", collected: 5, stream: "messages" },
            run_id: "r",
          },
        }),
        "cin_a"
  ...
  await withTempDb(async () => {
    ...
    for (let i = 0; i < 5; i += 1) {
      seedTerminalEvent({
        connectorInstanceId: "cin_a",
        runId: `run_${i}`,
        streams: [{ checkpoint: "committed", collected: 1, stream: `stream_${i}` }],
      });
    }
    ...
    assert.ok(afterRound1.stream_latest_facts_json, "round 1 must have a fact map");
    const factsAfterRound1: StreamFactsByStream = JSON.parse(afterRound1.stream_latest_facts_json);
    ...
    assert.ok(afterRound2.stream_latest_facts_json, "round 2 must have a fact map");
    const factsAfterRound2: StreamFactsByStream = JSON.parse(afterRound2.stream_latest_facts_json);
    assert.equal(
      Object.keys(factsAfterRound2).length,
      4,
      "round 2 ACCUMULATED on top of round 1 (2 + 2 = 4), not restarted from 0"
    );
    assert.ok(
    ...
    assert.equal(
      afterRound3.terminal_facts_state,
      "current",
      "only the genuinely converged final round may read current"
    );
    assert.equal(afterRound3.terminal_facts_reason_code, null);
    assert.ok(afterRound3.stream_latest_facts_json, "round 3 must have a fact map");
    const factsAfterRound3: StreamFactsByStream = JSON.parse(afterRound3.stream_latest_facts_json);
    ...
    const finalFacts = factsFor(evidence);
    assert.ok(finalFacts, "final converged facts map must exist");
    assert.equal(Object.keys(finalFacts).length, 5);
  });
});
...
test("exact-boundary convergence: a maxEvents budget that exactly equals the remaining history still reads current, not falsely incomplete", async () => {
  await withTempDb(async () => {
    ...
    assert.equal(
      result.incomplete,
      false,
      "a pass whose cursor lands exactly on maxSeq is genuinely converged, not budget-exhausted"
    );
    ...
    assert.equal(
      row.terminal_facts_state,
      "current",
      "exact-boundary convergence must read current, not falsely stale"
    );
    assert.equal(row.terminal_facts_reason_code, null);
    assert.equal(Number(row.stream_facts_event_seq), 3);
    assert.ok(row.stream_latest_facts_json, "row must have a fact map after exact-boundary convergence");
    const facts: StreamFactsByStream = JSON.parse(row.stream_latest_facts_json);
  ...
  await withTempDb(async () => {
    ...
    const FUTURE_FACTS = {
      messages: {
        event_seq: 1,
        evidence_as_of: null,
        fact: { checkpoint: "committed", collected: 42, stream: "messages" },
        run_id: "future_run",
      },
    };
    getDb()
    ...
    assert.equal(
      ...
      "stale",
      "this (older) binary reads the future-version row as unreliable for its OWN observation"
    );
    ...
    assert.equal(
      rawRowAfterRead.terminal_facts_state,
      "current",
      "the STORED state is untouched — still current for a compatible reader"
    );
  ...
  await withTempDb(async () => {
    ...
    const FUTURE_FACTS = {
      messages: {
        event_seq: 1,
        evidence_as_of: null,
        fact: { checkpoint: "committed", collected: 1, stream: "messages" },
        run_id: "run_1",
      },
    };
    getDb()
    ...
    assert.equal(
      afterRollback.terminal_facts_state,
      "current",
      "the durable row was never regressed to stale by the rolled-back older binary"
    );
    ...
    assert.equal(
      Number(rawRow.stream_facts_event_seq),
      1,
      "the rollback window left the CAS baseline a real newer-binary write would still match"
    );
