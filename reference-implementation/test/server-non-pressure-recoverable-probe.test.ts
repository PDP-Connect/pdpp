...
interface PendingGapRow {
  connector_instance_id?: string | null;
  reason?: string | null;
}

// The probe-only subset of the store interface, structurally satisfied by
// both the real SQLite store and the deliberately minimal broken-store stub
// in the fail-closed test (which implements only this one method).
interface GapListingStore {
  listPendingGapsForConnector: (connectorId: string, opts?: { limit?: number }) => Promise<PendingGapRow[]>;
}

// The fuller subset seedGap() needs; the real SQLite store satisfies this.
interface GapSeedingStore extends GapListingStore {
  upsertPendingGap: (input: Record<string, unknown>) => Promise<unknown>;
}
...
async function countNonPressureRecoverable(
  store: GapListingStore,
  connectorId: string,
...
async function seedGap(
  store: GapSeedingStore,
  {
    ...
    stream = "messages",
    recordKey,
  }: { connectorId: string; connectorInstanceId: string; reason: string | null; stream?: string; recordKey?: string }
): Promise<void> {
  await store.upsertPendingGap({
    connectorId,
    connectorInstanceId,
    detailLocator: { id: recordKey ?? "k", kind: "test" },
    grantId: "grant_test",
    reason,
...
test("returns 0 (fail-closed) when the store probe throws", async () => {
