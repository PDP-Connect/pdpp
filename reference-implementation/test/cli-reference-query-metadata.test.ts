...
import { appendQuery } from "../cli/lib/common.ts";
import { attachReferenceQueryMetadata, extractReferenceQueryMetadata } from "../cli/lib/fetch.ts";

// Real Fetch Headers built from a plain map.
...
test("extractReferenceQueryMetadata: reads both reference headers", () => {
  assert.deepEqual(
    extractReferenceQueryMetadata(headers({ "PDPP-Reference-Trace-Id": "trace_1", "Request-Id": "req_1" })),
    { reference_trace_id: "trace_1", request_id: "req_1" }
  );
});

test("extractReferenceQueryMetadata: absent headers => null fields", () => {
  assert.deepEqual(extractReferenceQueryMetadata(headers({})), { reference_trace_id: null, request_id: null });
  assert.deepEqual(
    extractReferenceQueryMetadata(undefined),
    { reference_trace_id: null, request_id: null },
    "undefined headers safe"
  );
});
...
test("attachReferenceQueryMetadata: merges present reference metadata into a plain-object body", () => {
  assert.deepEqual(
    attachReferenceQueryMetadata(
      { object: "list" },
      headers({ "PDPP-Reference-Trace-Id": "trace_1", "Request-Id": "req_1" })
    ),
    { object: "list", reference_trace_id: "trace_1", request_id: "req_1" }
  );
});
