...
test("rs.search.lexical rejects count= as an unsupported parameter", () => {
  assert.throws(
    () => parseSearchLexicalParams({ count: "exact", q: "foo" }),
    (err) => err instanceof SearchLexicalRequestError && err.code === "invalid_request" && err.param === "count",
...
test("rs.search.semantic rejects count= as an unsupported parameter", () => {
  assert.throws(
    () => parseSearchSemanticParams({ count: "exact", q: "foo" }),
    (err) => err instanceof SearchSemanticRequestError && err.code === "invalid_request" && err.param === "count"
  );
});

test("rs.search.hybrid rejects count= as an unsupported parameter", () => {
  assert.throws(
    () => parseSearchHybridParams({ count: "exact", q: "foo" }),
    (err) => err instanceof SearchHybridRequestError && err.code === "invalid_request" && err.param === "count"
  );
});
