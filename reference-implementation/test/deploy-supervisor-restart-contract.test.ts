...
// count.
function assertRealRestartPolicy(block: string, label: string): void {
  const hasDirective = block
    .split("\n")
    // biome-ignore lint/performance/useTopLevelRegex: localized test assertion preserves its explicit contract.
    .some((line: string) => /^\s*restart:\s*(unless-stopped|on-failure)\b/.test(line));
  assert.ok(
    hasDirective,
...
test("root Dockerfile plain reference/reference-browser stages do NOT bake the restart-contract flag", async () => {
  ...
  // biome-ignore lint/style/useDestructuring: localized test assertion preserves its explicit contract.
  const referenceStage = referenceStageMatch[1];
  // biome-ignore lint/style/useDestructuring: localized test assertion preserves its explicit contract.
  const referenceBrowserStage = referenceBrowserStageMatch[1];
  assert.ok(referenceStage, "reference stage capture group is empty");
  assert.ok(referenceBrowserStage, "reference-browser stage capture group is empty");

  assert.doesNotMatch(referenceStage, new RegExp(`${RESTART_FLAG}=1`));
