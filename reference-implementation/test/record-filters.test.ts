...
test("invalidQueryError defaults to invalid_request and honors an override", () => {
  const def = invalidQueryError("m");
  assert.ok(isQueryError(def));
  assert.equal(def.code, "invalid_request");
  const explicit = invalidQueryError("m", "invalid_expand");
...
test("coerceComparableValue parses date/date-time strings to epoch millis", () => {
  const schema = { format: "date", type: "string" };
  assert.equal(coerceComparableValue("2026-01-01", schema), Date.parse("2026-01-01"));
...
test("passesRequestFilters evaluates exact filters via String equality", () => {
  const filters = [{ field: "status", kind: "exact", value: "active" }];
...
test("passesRequestFilters returns false when the comparable value is null", () => {
  const filters = [{ field: "age", fieldSchema: { type: "integer" }, kind: "range", operators: { gte: 10 } }];
...
test("passesRequestFilters honors gte (inclusive lower bound)", () => {
  const filters = [{ field: "age", fieldSchema: { type: "integer" }, kind: "range", operators: { gte: 18 } }];
...
test("passesRequestFilters honors gt (exclusive lower bound)", () => {
  const filters = [{ field: "age", fieldSchema: { type: "integer" }, kind: "range", operators: { gt: 18 } }];
...
test("passesRequestFilters honors lte (inclusive upper bound)", () => {
  const filters = [{ field: "age", fieldSchema: { type: "integer" }, kind: "range", operators: { lte: 65 } }];
...
test("passesRequestFilters honors lt (exclusive upper bound)", () => {
  const filters = [{ field: "age", fieldSchema: { type: "integer" }, kind: "range", operators: { lt: 65 } }];
...
test("passesRequestFilters ANDs multiple operators on one range filter", () => {
  const filters = [{ field: "age", fieldSchema: { type: "integer" }, kind: "range", operators: { gte: 18, lte: 65 } }];
