...
const TOP_LEVEL_REGEX_1 = /must not read process\.env/;

function expectViolation(source: string, needle: string): void {
  ...
  assert.ok(thrown, `expected boundary violation for source containing "${needle}", got pass`);
  assert.match(
