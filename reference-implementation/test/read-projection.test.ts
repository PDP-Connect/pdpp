...
import { normalizeProjectionFields, projectRecordEnvelope } from "../operations/read-projection.ts";
...
test("projectRecordEnvelope: null/empty fields is identity and returns the SAME reference", () => {
  const record = { data: { a: 1, b: 2 }, id: "r1" };
...
test("projectRecordEnvelope: projects INSIDE record.data and preserves outer envelope keys", () => {
  const record = {
    data: { amount: 10, currency: "USD", merchant: "acme" },
    id: "r1",
    stream: "orders",
  };
  ...
  assert.deepEqual(
    out,
    { data: { amount: 10, currency: "USD" }, id: "r1", stream: "orders" },
...
test("projectRecordEnvelope: missing requested data field is simply absent (no undefined hole)", () => {
  const record = { data: { amount: 10 }, id: "r1" };
  const out = projectRecordEnvelope(record, ["amount", "missing"]);
  assert.deepEqual(out, { data: { amount: 10 }, id: "r1" }, `got ${JSON.stringify(out)}`);
...
test("projectRecordEnvelope: with no data object, projects the top-level record", () => {
  const record = { amount: 10, id: "r1", stream: "orders" };
  const out = projectRecordEnvelope(record, ["id", "amount"]);
  assert.deepEqual(out, { amount: 10, id: "r1" }, `got ${JSON.stringify(out)}`);
});
...
test("projectRecordEnvelope: null data is treated as top-level (typeof null === object but falsy guard)", () => {
  const record = { data: null, extra: "x", id: "r1" };
  ...
  assert.deepEqual(out, { data: null, id: "r1" }, `got ${JSON.stringify(out)}`);
