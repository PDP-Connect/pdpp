const TOP_LEVEL_REGEX_1 = /export\s+function\s+buildLiveRecordDetail\b/;
const TOP_LEVEL_REGEX_2 = /\bimport\b[^;]*\bbuildLiveRecordDetail\b[^;]*\bfrom\b[^;]*;/;
...
test("sandbox /sandbox/v1/streams/:stream/records/:recordId route does not import buildLiveRecordDetail", () => {
  const src = read("apps/site/src/app/sandbox/v1/streams/[stream]/records/[recordId]/route.ts");
  const importPattern = TOP_LEVEL_REGEX_2;
  assert.equal(
