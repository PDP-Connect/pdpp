...
function lexHit({
  ...
  stream = "messages",
  score = -1,
}: HitFixtureArgs & { score?: number }): SearchHybridSourceResult {
  return {
    ...
    score: { kind: "bm25", order: "lower_is_better", value: score },
...
function semHit({
  ...
  stream = "messages",
  distance = 0.1,
}: HitFixtureArgs & { distance?: number }): SearchHybridSourceResult {
  return {
    connection_id: connectorInstanceId,
    connector_id: connectorId,
    connector_instance_id: connectorInstanceId,
    ...
    record_key: recordKey,
    record_url: `/v1/streams/${stream}/records/${recordKey}`,
    score: { kind: "semantic_distance", order: "lower_is_better", value: distance },
...
test("hybrid dedup: same connection + same record across both sources collapses to one hit", async () => {
  const deps = {
    runLexical: () => ({ envelope: { data: [lexHit({ connectorInstanceId: "cin_gmail_A", recordKey: "r1" })] } }),
    runSemantic: () => ({ envelope: { data: [semHit({ connectorInstanceId: "cin_gmail_A", recordKey: "r1" })] } }),
  };
  const out = await executeSearchHybrid({ actor: ownerActor, query: { q: "foo" } }, deps);
  ...
  assert.ok(item, "expected a merged hybrid hit");
  ...
  assert.ok(item.scores, "expected per-source scores");
...
test("hybrid forwards connection_id from whichever source emitted first", async () => {
  ...
  const deps = {
    runLexical: () => ({ envelope: { data: [lexHit({ connectorInstanceId: "cin_A", recordKey: "r1" })] } }),
    runSemantic: () => ({ envelope: { data: [semHit({ connectorInstanceId: "cin_B", recordKey: "r1" })] } }),
  };
  const out = await executeSearchHybrid({ actor: ownerActor, query: { q: "foo" } }, deps);
  assert.equal(out.envelope.data.length, 2);
...
test("hybrid invokes each retrieval source once and merges only their delegate results", async () => {
  ...
  const out = await executeSearchHybrid(
    {
      actor: { client_id: "cli_1", grant_id: "grant_1", kind: "client", subject_id: "subj" },
      query: { q: "old", streams: "dormant" },
    },
    {
      runLexical: () => {
        lexicalCalls += 1;
          ...
          warnings: [
            {
              ...
              detail: { connection_id: "cin_gmail_B", source: "gmail" },
              message:
                "Connection 'cin_gmail_B' under connector 'gmail' is not applicable to this query and was skipped.",
            },
          ],
        },
      },
...
test("hybrid: legacy single-binding hits (no connection_id) keep using (connector_id, stream, record_key) dedup", async () => {
  ...
  const stripId = (h: SearchHybridSourceResult) => {
