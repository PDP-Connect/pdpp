...
test('legacy connector-keyed stores migrate to one deterministic default account instance per owner and connector without data loss', async () => {
  ...
  try {
    ...
    assert.deepEqual(scheduler.listActiveRuns(), [], 'the legacy deployment was drained before lease migration');
...
test('two Gmail account instances isolate state, records, schedules, leases, and diagnostics', async () => {
  await withDb(async () => {
    ...
    await leases.upsertLease({ lease_id: 'lease_work', connector_id: GMAIL, profile_key: 'cin_gmail_work_acceptance', account_key: 'work@example.test', run_id: 'run_work_acceptance', status: 'waiting_for_browser_surface', priority_class: 'background', requested_at: NOW, expires_at: '2026-05-18T12:05:00.000Z', fencing_token: 1 });
    await leases.upsertLease({ lease_id: 'lease_personal', connector_id: GMAIL, profile_key: 'cin_gmail_personal_acceptance', account_key: 'personal@example.test', run_id: 'run_personal_acceptance', status: 'waiting_for_browser_surface', priority_class: 'background', requested_at: NOW, expires_at: '2026-05-18T12:05:01.000Z', fencing_token: 1 });
