...
test("register rejects a falsy runId or non-function callback with a no-op unsubscribe", () => {
  ...
  // @ts-expect-error -- runtime guard rejects a non-function callback; this proves it at the type boundary too
  const noopB = registerRemoteTelemetrySink("run-x", "not-a-function");
  // @ts-expect-error -- runtime guard rejects a non-string runId; this proves it at the type boundary too
  // biome-ignore lint/suspicious/noEmptyBlockStatements: skipped test callback is intentionally empty
