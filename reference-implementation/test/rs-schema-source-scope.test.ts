...
import { projectSchemaStreamScope, schemaSourceOptions } from "../operations/rs-schema-get/compact-view.ts";
import type { ConnectorSchemaItem } from "../operations/rs-schema-get/index.ts";

// Local structural stand-in for the module-private `SchemaResponse` type
// (`compact-view.ts` does not export it). `object: "schema"` plus a
// `ConnectorSchemaItem[]` `connectors` array is exactly what the exported
// functions require; the frozen `compact-view.ts` boundary rule means the
// shape has to be reconstructed here rather than imported.
interface SampleSchemaResponse {
  bearer: unknown;
...
// checks an exact shape. `object`/`source` on each connector and
// `object` on each stream are required by `ConnectorSchemaItem`'s type but are
// not read by the functions under test here (they read named keys + `[extra]`
// passthrough), so adding them does not change observed behavior.
function sampleResponse(): SampleSchemaResponse {
  return {
    bearer: { scopes: ["read"] },
    connector_count: 2,
    connectors: [
      {
        ...
        stream_count: 2,
        streams: [
          { field_detail: { amount: {} }, name: "orders", object: "stream_metadata", primary_key: "id" },
          { name: "returns", object: "stream_metadata", primary_key: "id" },
        ],
      },
      {
        connector_key: "gmail",
        ...
        stream_count: 1,
        streams: [{ name: "messages", object: "stream_metadata", primary_key: "id" }],
      },
    ],
...
test("projectSchemaStreamScope: stream scope keeps only matching connectors and recomputes counts", () => {
  ...
  const amazon = out.connectors[0];
  assert.ok(amazon, "expected a surviving connector");
  assert.equal(amazon.connector_key, "amazon");
  // Only the "orders" stream survives inside it.
  assert.deepEqual(
    amazon.streams.map((s) => s.name),
    ["orders"],
    `streams: ${JSON.stringify(amazon.streams.map((s) => s.name))}`
  );
  ...
  assert.equal(amazon.stream_count, 1, "per-connector stream_count recomputed");
});
...
test("projectSchemaStreamScope: full per-field detail on the surviving stream is preserved verbatim", () => {
  ...
  assert.ok(amazon, "expected a surviving connector");
  // biome-ignore lint/style/useDestructuring: localized test assertion preserves its explicit contract.
  const orders = amazon.streams[0];
  assert.ok(orders, "expected a surviving stream");
  // Unlike the compact view, scoping must NOT strip per-field detail.
  assert.deepEqual(orders.field_detail, { amount: {} }, "field_detail must survive full-detail scoping");
...
test("projectSchemaStreamScope: connectionId narrows granted_connections on the surviving connector", () => {
  ...
  assert.ok(amazon, "expected a surviving connector");
  assert.deepEqual(
    amazon.granted_connections,
    [{ connection_id: "conn-a1", display_name: "Amazon Personal" }],
    `granted_connections: ${JSON.stringify(amazon.granted_connections)}`
  );
});
...
test("schemaSourceOptions: enumerates one dedup option per (connection, connector, stream) from granted_connections", () => {
  ...
  const ordersA1 = out.find((o) => o.connection_id === "conn-a1" && o.stream === "orders");
  assert.deepEqual(
    ordersA1,
    {
...
test("schemaSourceOptions: falls back to stream+connector identity entries when granted_connections is absent", () => {
  ...
  const response: SampleSchemaResponse = {
    bearer: null,
    connector_count: 1,
    connectors: [
      {
        connector_key: "plaid",
        object: "connector",
        source: { id: "plaid", kind: "connector" },
        stream_count: 1,
        streams: [
          { connection_id: "conn-p1", display_name: "Checking", name: "transactions", object: "stream_metadata" },
        ],
      },
    ],
    object: "schema",
...
test("schemaSourceOptions: omits connection_id when absent; display_name falls back to the stream name", () => {
    ...
    connectors: [
      {
        ...
        object: "connector",
        source: { id: "localfs", kind: "connector" },
        stream_count: 1,
        streams: [{ name: "files", object: "stream_metadata" }],
      },
    ],
    object: "schema",
  };
  ...
  // biome-ignore lint/style/useDestructuring: localized test assertion preserves its explicit contract.
  const first = out[0];
  assert.ok(first, "expected one option");
  assert.deepEqual(
    first,
    { connector_key: "localfs", display_name: "files", stream: "files" },
    `option: ${JSON.stringify(first)}`
  );
  assert.equal("connection_id" in first, false, "no connection_id key when connection identity is absent");
});
...
test("schemaSourceOptions: de-dupes identical (connection, connector, stream) triples across duplicate grants", () => {
  ...
  const first = out[0];
  assert.ok(first, "expected one option");
  assert.equal(first.connection_id, "conn-a1");
});
