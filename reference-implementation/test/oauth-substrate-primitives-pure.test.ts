...
const TOP_LEVEL_REGEX_1 = /^[0-9a-f]{64}$/;
...
test("base64UrlSha256: is base64url (no +, /, or = padding) and deterministic", () => {
  ...
  assert.ok(!TOP_LEVEL_REGEX_3.test(out), "base64url encoding has no +, /, or padding");
...
test("generateToken: 64 lowercase hex chars (32 random bytes) and non-repeating", () => {
  ...
  assert.match(t, TOP_LEVEL_REGEX_1, "32 bytes -> 64 hex chars");
...
test("generateOAuthRefreshToken: rt_ prefix + base64url body", () => {
  ...
  assert.ok(!TOP_LEVEL_REGEX_2.test(body), "body is base64url (no +, /, padding)");
