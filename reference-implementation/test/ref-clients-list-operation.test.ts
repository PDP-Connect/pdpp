...
import { executeRefClientsList, RefClientsListInvalidRequestError } from "../operations/ref-clients-list/index.ts";
...
test("ref.clients.list emits {object: list, data: []} when there are no clients", async () => {
  const envelope = await executeRefClientsList({ owner: "true" }, { listOwnerIssuedClients: () => [] });
  assert.deepEqual(envelope, { data: [], object: "list" });
});

test("ref.clients.list passes dependency entries through unchanged", async () => {
  const inputs = [sampleClient, { ...sampleClient, active_token_count: 0, client_id: "client_xyz", client_name: null }];
  const envelope = await executeRefClientsList({ owner: "true" }, { listOwnerIssuedClients: () => inputs });
...
test("ref.clients.list rejects a missing owner query parameter with invalid_request", async () => {
  await assert.rejects(
    () => executeRefClientsList({ owner: undefined }, { listOwnerIssuedClients: () => [] }),
...
test('ref.clients.list rejects owner values other than the literal string "true"', async () => {
  for (const raw of ["false", "TRUE", "1", true, 0, null, ["true"]]) {
    ...
    await assert.rejects(
      () => executeRefClientsList({ owner: raw }, { listOwnerIssuedClients: () => [] }),
...
test("ref.clients.list awaits dependency promises", async () => {
  ...
  assert.ok(entry);
  assert.equal(entry.client_id, sampleClient.client_id);
});
