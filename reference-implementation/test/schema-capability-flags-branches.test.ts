...
test("formatFieldCapabilityFlags: top-level type wins over schema.type", () => {
  assert.equal(
    formatFieldCapabilityFlags({ schema: { type: "integer" }, type: "string" }),
    "t=string",
...
test('formatFieldCapabilityFlags: range usable without operators emits a bare "r"', () => {
  assert.equal(
    formatFieldCapabilityFlags({ range_filter: { declared: true, usable: true }, type: "number" }),
    "t=number,r"
  );
});
...
test("formatFieldCapabilityFlags: range unusable emits r=unusable with the reason suffix", () => {
  assert.equal(
    formatFieldCapabilityFlags({ range_filter: { declared: true, reason: "no_index", usable: false }, type: "number" }),
...
test("formatFieldCapabilityFlags: eq unusable WITHOUT a reason has no suffix", () => {
  assert.equal(
    formatFieldCapabilityFlags({ exact_filter: { declared: true, usable: false }, type: "string" }),
...
test("formatFieldCapabilityFlags: lexical/semantic unusable flags carry a reason only when present", () => {
  assert.equal(
    formatFieldCapabilityFlags({
      lexical_search: { declared: true, reason: "not_indexed", usable: false },
...
test("formatFieldCapabilityFlags: aggregation lists only USABLE ops, pipe-joined", () => {
  assert.equal(
    formatFieldCapabilityFlags({
      aggregation: {
        avg: { declared: true, reason: "x", usable: false },
...
test("formatFieldCapabilityFlags: a declared-but-not-usable capability contributes no flag", () => {
  ...
  assert.equal(formatFieldCapabilityFlags({ exact_filter: { declared: true }, type: "string" }), "t=string");
