...
function req(headers: Record<string, string> = {}, protocol = "https"): ResolvePublicUrlRequest {
  const lower: Record<string, string> = {};
  for (const [k, v] of Object.entries(headers)) {
    lower[k.toLowerCase()] = v;
  }
  return { get: (name: string) => lower[name.toLowerCase()], protocol };
}
...
test("buildOwnerAgentControlSurface: a supported family resolves method + absolute URL", () => {
  ...
  assert.ok(list, "expected a list_connections action");
...
test("buildOwnerAgentControlSurface: the surface catalog keeps the {connection_id} placeholder literal", () => {
  ...
  assert.ok(run, "expected a run_connection action");
...
test("shouldUseDirectRequestOrigin: false without an explicit URL, or with a forwarded origin, or non-loopback explicit", () => {
  assert.equal(shouldUseDirectRequestOrigin(req({ host: "pub.example.com" }), null), false, "no explicit URL => false");
  assert.equal(
