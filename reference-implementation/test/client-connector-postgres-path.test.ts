 ...
 *
 * The seam-march migration moves these row operations in `server/auth.ts`
...
if (POSTGRES_URL) {
  ...
  test("registered client: register -> read -> owner-list -> delete through real auth.js postgres adapters", async () => {
    ...
    assert.equal(
      (fetched.metadata as { issuer_subject_id?: string }).issuer_subject_id,
      ownerSubject,
  ...
  test("seed pre-registered client: upsert -> read through real auth.js postgres adapters", async () => {
    ...
    assert.equal(seeded.registration_mode, "pre_registered_public", "seeded registration_mode matches");
  });
} else {
  test("auth.js client/cimd/connector postgres-adapter path (skipped: PDPP_TEST_POSTGRES_URL unset)", {
    skip: true,
