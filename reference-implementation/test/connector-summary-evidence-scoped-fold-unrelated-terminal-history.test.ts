...
// stream name to its latest folded terminal fact. The real read model types
// this field as `unknown` (parsed JSON), so this local interface documents
// the known real shape (matching the `TerminalEventStreamFact` seeded below)
// for this test only.
interface StreamLatestFactsMap {
  [stream: string]: {
    fact: { stream: string; resolved: boolean; record_count: number };
    run_id: string;
    event_seq: number;
...
interface ScopedFoldManifest {
  ...
  streams: Array<{ name: string; primary_key: string[] }>;
  version: string;
}

function seedSqliteConnection(connectorInstanceId: string, connectorId: string, manifest: ScopedFoldManifest) {
  getDb()
...
function seedSqliteTerminalEvent(connectorInstanceId: string, streams: TerminalEventStreamFact[]) {
  sqliteEventSeq += 1;
  const data = {
...
async function countRowsReturned<T>(fn: () => Promise<T>): Promise<{ result: T; rowsReturned: number }> {
  let rowsReturned = 0;
  ...
  Database.prototype.prepare = function patchedPrepare(this: InstanceType<typeof Database>, sql: string) {
    const stmt = originalPrepare.call<
      ...
      [string],
      import("better-sqlite3").Statement<unknown[], unknown>
    >(this, sql);
...
test(
  ...
  withTempDb(async () => {
    ...
    for (let i = 0; i < UNRELATED_TERMINAL_EVENT_COUNT; i += 1) {
      seedSqliteTerminalEvent("cin_scoped_unrelated", [{ record_count: i, resolved: true, stream: "messages" }]);
    }
    ...
    assert.ok(evidence, "the target connection must have an evidence row");
    assert.equal(
    ...
    const streamLatestFacts = evidence.stream_latest_facts as StreamLatestFactsMap | null;
    ...
    assert.equal(streamLatestFacts?.messages?.fact?.record_count, 7, "the target's own fact genuinely folded");
    ...
    assert.ok(unrelatedEvidence, "the unrelated connection must have an evidence row (stamped by the earlier rebuild)");
    assert.equal(
...
test(
  ...
  withTempDb(async () => {
    ...
    seedSqliteTerminalEvent("cin_scoped_target", [{ record_count: 1, resolved: true, stream: "messages" }]);
...
function mustPostgresUrl(): string {
  assert.ok(POSTGRES_URL, "PDPP_TEST_POSTGRES_URL is set (test is skipped otherwise)");
  return POSTGRES_URL;
}

async function seedPostgresConnection(connectorInstanceId: string, connectorId: string, manifest: ScopedFoldManifest) {
...
async function seedPostgresTerminalEvent(connectorInstanceId: string, streams: TerminalEventStreamFact[]) {
  postgresEventSeq += 1;
  const data = {
    collection_facts: { reference_only: true, schema_version: 1, streams },
    connection_id: connectorInstanceId,
...
async function countPostgresRowsReturned<T>(fn: () => Promise<T>): Promise<{ result: T; rowsReturned: number }> {
  let rowsReturned = 0;
  const pool = getPostgresPool();
  const original = pool.query.bind(pool);
  const queryWithRowCount = async <R extends QueryResultRow = QueryResultRow, I = unknown[]>(
    queryTextOrConfig: string | QueryConfig<I>,
    values?: QueryConfigValues<I>
  ): Promise<QueryResult<R>> => {
    const sql = typeof queryTextOrConfig === "string" ? queryTextOrConfig : queryTextOrConfig.text;
    const result = await original<R, I>(queryTextOrConfig, values);
    // biome-ignore lint/performance/useTopLevelRegex: test assertion patterns remain colocated with the assertion they explain.
  ...
  try {
    ...
    for (let i = 0; i < UNRELATED_TERMINAL_EVENT_COUNT; i += 1) {
      ...
      await seedPostgresTerminalEvent("cin_scoped_unrelated_pg", [
        { record_count: i, resolved: true, stream: "messages" },
      ]);
    }
    ...
    assert.ok(evidence, "the target connection must have an evidence row on real PostgreSQL");
    assert.equal(
      evidence.stream_facts_event_seq,
      targetEventSeq,
    ...
    const streamLatestFacts = evidence.stream_latest_facts as StreamLatestFactsMap | null;
    // biome-ignore lint/suspicious/noUnnecessaryConditions: the runtime fixture deliberately exercises an absent or nullable boundary value.
    assert.equal(streamLatestFacts?.messages?.fact?.record_count, 7);
  } finally {
    await cleanupPostgres();
...
test("real PostgreSQL: the real mounted route resolves correctly despite thousands of unrelated terminal events", {
  ...
  try {
    ...
    await seedPostgresTerminalEvent("cin_scoped_target_pg", [{ record_count: 1, resolved: true, stream: "messages" }]);
