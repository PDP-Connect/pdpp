...
async function withServer(optionsOrFn, maybeFn) {
...
function deterministicAttemptBackend(onEmbed) {
  return {
    model: () => onEmbed.model(),
    dimensions: () => 3,
...
function setMessagesSemanticFields(fields) {
  const row = getDb().prepare('SELECT manifest FROM connectors WHERE connector_id = ?').get('codex');
  const manifest = JSON.parse(row.manifest);
  const messages = manifest.streams.find((stream) => stream.name === 'messages');
  messages.query.search.semantic_fields = fields;
  getDb().prepare('UPDATE connectors SET manifest = ? WHERE connector_id = ?').run(JSON.stringify(manifest), 'codex');
}
...
test('semantic-required device preflight is retryable, reservation-free, and privacy-safe without a bounded backend', async () => {
  await withServer({ semanticRetrievalSupported: false }, async ({ asUrl }) => {
    ...
    batch.records[0].data.content = 'private-semantic-preflight-sentinel';
    ...
    assert.equal(response.body.error.code, 'device_ingest_retryable');
    assert.equal(response.body.error.message, 'Device ingest is temporarily unavailable; retry the same batch');
    assert.doesNotMatch(JSON.stringify(response.body), /private-semantic-preflight-sentinel/);
    assert.equal(
      getDb().prepare('SELECT COUNT(*) AS count FROM device_ingest_batch_outcomes WHERE batch_id = ?')
        .get('batch-semantic-preflight-unavailable').count,
      0,
...
test('an injected semantic backend without confirmed attempt fencing is refused before reservation', async () => {
  ...
  await withServer({ semanticRetrievalBackend: unboundedBackend }, async ({ asUrl }) => {
    ...
    batch.records[0].data.content = 'private-unbounded-backend-sentinel';
    batch.body_hash = bodyHash(batch.records);
    const response = await postJson(
      `${asUrl}/_ref/device-exporters/${encodeURIComponent(device.device_id)}/ingest-batches`,
      batch,
    ...
    assert.doesNotMatch(JSON.stringify(response.body), /private-unbounded-backend-sentinel/);
    assert.equal(
      getDb().prepare('SELECT COUNT(*) AS count FROM device_ingest_batch_outcomes WHERE batch_id = ?')
        .get('batch-semantic-preflight-unbounded').count,
      0,
    );
  });
});
...
test('declared semantic fields with empty values complete as a zero-row device index plan', async () => {
  ...
  await withServer({ semanticRetrievalBackend: backend }, async ({ asUrl }) => {
    ...
    assert.equal(response.body.accepted_record_count, 1);
    assert.equal(documentCalls, 0, 'empty declared semantic values produce no vector rows or model work');
    const outcome = getDb()
      .prepare('SELECT status, durable_prefix_count, record_count FROM device_ingest_batch_outcomes WHERE batch_id = ?')
      .get('batch-semantic-empty-values');
    assert.deepEqual(outcome, { status: 'accepted', durable_prefix_count: 1, record_count: 1 });
  });
});
...
test('device ingest preflight and immutable attempt facts fence drift while accepted replay remains validation-free', async () => {
  ...
  let acceptedStatusBeforeBackfill = null;
    ...
    document: () => {
      ...
      if (!manifestChanged) {
        ...
        getDb().prepare('UPDATE connectors SET manifest = ? WHERE connector_id = ?')
          .run(JSON.stringify(manifest), 'codex');
  ...
  await withServer({ semanticRetrievalBackend: backend }, async ({ asUrl }) => {
    ...
    const initialManifest = JSON.parse(
      getDb().prepare('SELECT manifest FROM connectors WHERE connector_id = ?').get('codex').manifest,
    );
    const initialMessages = initialManifest.streams.find((stream) => stream.name === 'messages');
    initialMessages.schema.properties.updated_at = { type: 'string', format: 'date-time' };
    initialMessages.cursor_field = 'timestamp';
    initialMessages.consent_time_field = null;
    initialMessages.primary_key = ['id'];
    getDb().prepare('UPDATE connectors SET manifest = ? WHERE connector_id = ?')
      .run(JSON.stringify(initialManifest), 'codex');
    ...
    assert.equal(
      getDb().prepare('SELECT COUNT(*) AS count FROM device_ingest_batch_outcomes WHERE batch_id = ?')
        .get('batch-invalid-preflight').count,
      0,
    );
    ...
    assert.equal(
      getDb().prepare('SELECT semantic_time FROM records WHERE connector_instance_id = ? AND record_key = ?')
        .get(device.connector_instance_id, 'same-key').semantic_time,
    ...
    assert.deepEqual(
      ...
      { status: 'accepted', durable_prefix_count: 1, record_count: 1 },
    );
    ...
    getDb().prepare(
      'UPDATE records SET semantic_time = ? WHERE connector_instance_id = ? AND record_key = ?',
    ...
    assert.deepEqual(
      getDb().prepare(
        'SELECT version, semantic_time FROM records WHERE connector_instance_id = ? AND record_key = ?',
      ).get(device.connector_instance_id, 'same-key'),
      { version: 1, semantic_time: '2026-04-30T13:00:00.000Z' },
      'fresh no-op input repairs attempt-derived state without version churn',
    );
    const repairedVector = getDb().prepare(`
    ...
    semanticDrift.records[0].data.content = 'semantic identity content';
    semanticDrift.records[0].data.role = 'assistant';
    ...
    assert.match(
      getDb().prepare('SELECT semantic_capability_identity FROM device_ingest_batch_outcomes WHERE batch_id = ?')
        .get('batch-semantic-drift').semantic_capability_identity,
      /attempt-model-c/,
    );
    ...
    const registeredMessages = registeredManifest.streams.find((stream) => stream.name === 'messages');
    registeredMessages.query.search.lexical_fields = ['role'];
    registeredMessages.query.search.semantic_fields = ['content'];
    acceptedStatusBeforeBackfill = getDb()
      .prepare('SELECT status FROM device_ingest_batch_outcomes WHERE batch_id = ?')
      .get('batch-manifest-drift').status;
    ...
    const registrationLexical = getDb().prepare(`
    ...
    assert.deepEqual(registrationLexical.map((row) => row.field), ['role']);
...
test('device ingest preserves op and rejects unsupported op or malformed data before reservation', async () => {
  await withServer(async ({ asUrl }) => {
    ...
    const cases = [
      ['unsupported-op', { op: 'remove', data: { id: 'same-key' } }],
      ['array-upsert-data', { data: ['not', 'an', 'object'] }],
      ['null-upsert-data', { data: null }],
      ['missing-upsert-data', {}],
      ['null-delete-data', { op: 'delete', data: null }],
      ['nonempty-delete-data', { op: 'delete', data: { ignored: true } }],
      ['identity-mismatch', { data: { id: 'private-data-sentinel' } }],
    ];
    assert.notEqual(
      bodyHash([{ stream: 'messages', record_key: 'same-key', op: 'upsert', data: { id: 'same-key' } }]),
      bodyHash([{ stream: 'messages', record_key: 'same-key', op: 'delete', data: {} }]),
    ...
    for (const [suffix, override] of cases) {
      ...
      if (suffix === 'missing-upsert-data') delete records[0].data;
      if (suffix === 'identity-mismatch') records[0].record_key = 'different-key';
      const batch = {
      ...
      assert.equal(response.body.error.code, 'invalid_request', suffix);
      assert.doesNotMatch(JSON.stringify(response.body), /private-data-sentinel|key and data disagree/);
      assert.equal(
        getDb().prepare('SELECT COUNT(*) AS count FROM device_ingest_batch_outcomes WHERE batch_id = ?')
          .get(batch.batch_id).count,
        0,
...
test('batch attempt deadline leaves the durable prefix sticky and retryable', async () => {
  ...
  const backend = {
    ...
    supportsDeviceAttemptDeadline: () => true,
    embedDocument: async () => {
      if (delaySemantic) await new Promise((resolve) => setTimeout(resolve, 30));
  ...
  try {
    await withServer({ semanticRetrievalBackend: backend }, async ({ asUrl }) => {
      ...
      batch.records[0].data.content = 'deadline payload';
      batch.body_hash = bodyHash(batch.records);
      ...
      assert.deepEqual(
        getDb().prepare('SELECT status, durable_prefix_count FROM device_ingest_batch_outcomes WHERE batch_id = ?')
          .get(batch.batch_id),
        { status: 'processing', durable_prefix_count: 1 },
      );
      assert.equal(
        getDb().prepare('SELECT COUNT(*) AS count FROM record_changes WHERE connector_instance_id = ?')
          .get(device.connector_instance_id).count,
        1,
      );
      ...
      assert.deepEqual(
        getDb().prepare('SELECT status, durable_prefix_count FROM device_ingest_batch_outcomes WHERE batch_id = ?')
          .get(batch.batch_id),
        { status: 'accepted', durable_prefix_count: 1 },
      );
      assert.equal(
        getDb().prepare('SELECT COUNT(*) AS count FROM record_changes WHERE connector_instance_id = ?')
          .get(device.connector_instance_id).count,
    ...
    if (previousDeadline === undefined) delete process.env.PDPP_INGEST_BATCH_ATTEMPT_DEADLINE_MS;
    else process.env.PDPP_INGEST_BATCH_ATTEMPT_DEADLINE_MS = previousDeadline;
  }
});
...
test('device ingest canonicalizes connector aliases for replay and rejects a different canonical connector', async () => {
  await withServer(async ({ asUrl }) => {
    ...
    assert.equal(
      getDb().prepare('SELECT connector_id FROM device_ingest_batch_outcomes WHERE batch_id = ?')
        .get(batch.batch_id).connector_id,
      'codex',
    );
    ...
    const beforeConflict = getDb().prepare(
      `SELECT
    ...
    assert.equal(sameBatchDifferentConnector.body.error.code, 'device_batch_conflict');
    assert.deepEqual(
      getDb().prepare(
        `SELECT
           (SELECT COUNT(*) FROM records WHERE connector_instance_id = ?) AS records,
    ...
    assert.equal(differentCanonicalConnector.body.error.code, 'invalid_request');
    assert.equal(
      getDb().prepare('SELECT COUNT(*) AS count FROM device_ingest_batch_outcomes WHERE batch_id = ?')
        .get('batch-different-canonical-connector').count,
      0,
    );
  });
});
...
test('device ingest verifies the shipped durable collector envelope hash', async () => {
  await withServer(async ({ asUrl }) => {
    ...
    assert.equal(accepted.status, 201, JSON.stringify(accepted.body));
    assert.equal(accepted.body.accepted_record_count, 1);
    ...
    assert.equal(representationConflict.body.error.code, 'device_batch_conflict');
    assert.equal(
      getDb().prepare('SELECT COUNT(*) AS count FROM record_changes WHERE connector_instance_id = ?')
        .get(device.connector_instance_id).count,
      1,
...
test('unknown reservation-store failures use the fixed device retry envelope', async () => {
  __setDeviceIngestStoreFaultHookForTest((point) => {
  ...
  try {
    await withServer(async ({ asUrl }) => {
      ...
      assert.equal(response.body.error.code, 'device_ingest_retryable');
      assert.equal(response.body.error.message, 'Device ingest is temporarily unavailable; retry the same batch');
      assert.doesNotMatch(JSON.stringify(response.body), /private-reservation-store-sentinel/);
      assert.equal(
        getDb().prepare('SELECT COUNT(*) AS count FROM device_ingest_batch_outcomes WHERE batch_id = ?')
          .get(batch.batch_id).count,
        0,
...
test('duplicate upsert/delete resumes a sticky cursor and leaves only the tombstone on SQLite', async () => {
  ...
  __setIngestFaultHookForTest((point) => {
    if (point === 'after-version-allocation') {
      ...
      if (versionAllocations === 2) throw new Error('private-durable-second-record-sentinel');
    }
  });
  try {
    await withServer(async ({ asUrl }) => {
      ...
      assert.equal(first.status, 503, JSON.stringify(first.body));
      assert.equal(first.body.error.code, 'device_ingest_retryable');
      assert.doesNotMatch(JSON.stringify(first.body), /private-durable-second-record-sentinel/);
      assert.deepEqual(
        getDb().prepare('SELECT status, durable_prefix_count FROM device_ingest_batch_outcomes WHERE batch_id = ?')
          .get(batch.batch_id),
        { status: 'processing', durable_prefix_count: 1 },
...
test('a post-durable index retry repairs from the newer authoritative writer on SQLite', async () => {
  ...
  const backend = {
    ...
    embedDocument: async (text) => {
      ...
      return text.includes('payload-b')
  ...
  await withServer({ semanticRetrievalBackend: backend }, async ({ asUrl }) => {
    ...
    try {
      const records = [{
        ...
        record_key: 'same-key',
        emitted_at: '2026-07-16T00:00:00.000Z',
        data: { id: 'same-key', content: 'payload-a', role: 'user' },
      }];
      const batch = {
      ...
      assert.equal(first.body.error.code, 'device_ingest_retryable');
      assert.doesNotMatch(JSON.stringify(first.body), /private-semantic-backend-sentinel/);
      assert.equal(getDb().prepare(
        'SELECT version FROM records WHERE connector_instance_id = ? AND stream = ? AND record_key = ?',
      ).get(device.connector_instance_id, 'messages', 'same-key').version, 1);
      ...
      await ingestRecord(
        { connector_id: device.connector_id, connector_instance_id: device.connector_instance_id },
        {
          ...
          key: 'same-key',
          emitted_at: '2026-07-16T00:00:01.000Z',
          data: { id: 'same-key', content: 'payload-b', role: 'assistant' },
        },
      );
      ...
      ).get(device.connector_instance_id, 'messages', 'same-key'), { field: 'content', text: 'payload-b' });
      const semanticRow = getDb().prepare(
      ...
      if (semanticRow) {
        ...
        const semanticRowId = getDb().prepare(
          `SELECT rowid FROM semantic_search_rowid
            WHERE connector_instance_id = ? AND scope_key = ? AND record_key = ?`,
        ).get(device.connector_instance_id, JSON.stringify(['messages', 'content']), 'same-key');
        assert.ok(semanticRowId);
        const vectorRow = getDb().prepare('SELECT embedding FROM semantic_search_vec WHERE rowid = ?')
          .get(semanticRowId.rowid);
...
test('device batch and direct ingest serialize on one instance while a sibling instance overlaps', async () => {
  ...
  try {
    ...
    const manifest = JSON.parse(getDb().prepare('SELECT manifest FROM connectors WHERE connector_id = ?').get('codex').manifest);
    const messages = manifest.streams.find((stream) => stream.name === 'messages');
    ...
    const records = [{
      ...
      record_key: 'same-key',
      emitted_at: '2026-07-16T00:00:00.000Z',
      data: { id: 'same-key', content: 'device blocked', role: 'user', value: 'device' },
    }];
    let batch = null;
    let sameInstanceDirect = null;
    try {
      batch = postJson(
        `${asUrl}/_ref/device-exporters/${encodeURIComponent(device.device_id)}/ingest-batches`,
        {
      ...
      sameInstanceDirect = ingestRecord(
        { connector_id: device.connector_id, connector_instance_id: device.connector_instance_id },
        {
          ...
          key: 'same-key',
          emitted_at: '2026-07-16T00:00:01.000Z',
          data: { id: 'same-key', content: 'direct after device', role: 'user', value: 'direct' },
        },
      ...
      await ingestRecord(
        { connector_id: device.connector_id, connector_instance_id: 'cin_device_direct_sibling' },
        {
          ...
          key: 'sibling-key',
          emitted_at: '2026-07-16T00:00:01.000Z',
          data: { id: 'sibling-key', content: 'sibling overlaps', role: 'user', value: 'sibling' },
        },
      );
      assert.equal(sameInstanceFinished, false, 'direct writer waits for the device batch’s complete derived section');

      releaseDeviceEmbedding.resolve();
      const result = await batch;
      assert.equal(result.status, 201);
      await sameInstanceDirect;
      const finalRecord = getDb().prepare(
