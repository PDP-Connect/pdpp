...
/**
 * `server/db.js` is untyped JS (allowJs, checkJs:false) under server/**,
 * forbidden to touch. Same boundary-cast pattern as
 * `current-projection-recurrence-guard.test.ts`: model the real call/return
 * shapes locally from the source and cast the untyped imports once.
 */
type DbHandle = BetterSqlite3.Database;
...
const PRODUCTION_QUERIES_DIR = join(dirname(fileURLToPath(import.meta.url)), "..", "server", "queries");

function setupQueriesDir(builder: (dir: string) => void): string {
...
/**
 * `loadReferenceQueries(dir)` returns a `ReferenceQueryRegistry` — a fixed
 * set of well-known keys plus a `Record<string, RegisteredQuery>` index
 * signature for the rest. These tests write ad-hoc SQL artifacts under
 * their own tmp dir (e.g. `get-test-item-by-id.sql`) that are not among the
 * well-known keys, so `reg.getTestItemById` resolves through the index
 * signature as `RegisteredQuery | undefined` (noUncheckedIndexedAccess).
 * The wrapper primitives (getOne/getMany/exec/...) each require one
 * specific branded subtype. The brand is a private unique-symbol field the
 * loader alone can populate, so narrowing from the `RegisteredQuery` union
 * down to a specific branded member is not structurally checkable at
 * runtime; each helper below asserts presence (proving the test's own
 * `setupQueriesDir` builder actually wrote the expected artifact) and casts
 * to the terminator-specific handle type the loader is documented to
 * produce for that artifact's `@terminator` annotation.
 */
function requireReadOneQuery(query: RegisteredQuery | undefined): ReadOneQuery {
  assert.ok(query, "expected the test's SQL artifact to be registered");
  return query as ReadOneQuery;
}

function requireReadManyQuery(query: RegisteredQuery | undefined): ReadManyQuery {
  assert.ok(query, "expected the test's SQL artifact to be registered");
  return query as ReadManyQuery;
}

function requireIterateQuery(query: RegisteredQuery | undefined): IterateQuery {
  assert.ok(query, "expected the test's SQL artifact to be registered");
  return query as IterateQuery;
}

function requireMutationQuery(query: RegisteredQuery | undefined): MutationQuery {
  assert.ok(query, "expected the test's SQL artifact to be registered");
  return query as MutationQuery;
}

function requireMutationReturningOneQuery(query: RegisteredQuery | undefined): MutationReturningOneQuery {
  assert.ok(query, "expected the test's SQL artifact to be registered");
  return query as MutationReturningOneQuery;
}

function requireSmallEnumerationQuery(query: RegisteredQuery | undefined): SmallEnumerationQuery {
  assert.ok(query, "expected the test's SQL artifact to be registered");
  return query as SmallEnumerationQuery;
}

/**
 * `better-sqlite3`'s `.get()` return type is opaque (`unknown`) to a caller
 * reading specific columns from an ad-hoc SELECT; each call site here reads
 * exactly the column(s) it declared in its own SQL, so this asserts the row
 * exists and narrows to the caller-declared shape. Same idiom as
 * `current-projection-recurrence-guard.test.ts`'s `requireRow`.
 */
function requireRow<T>(row: unknown): T {
  assert.ok(row, "expected a row for the just-inserted/queried fixture");
  return row as T;
}
...
test("getOne returns the row when one matches", () => {
  const dir = setupQueriesDir((d: string) => {
    writeFileSync(
  ...
  try {
    ...
    const row = getOne(requireReadOneQuery(reg.getTestItemById), [1]);
...
test("getOne returns null when no row matches", () => {
  const dir = setupQueriesDir((d: string) => {
    writeFileSync(join(d, "get-test-item-by-id.sql"), "-- @terminator: one\nSELECT id FROM test_items WHERE id = ?\n");
  });
  try {
    setupDb();
    const reg = loadReferenceQueries(dir);
    const row = getOne(requireReadOneQuery(reg.getTestItemById), [999]);
...
test("getMany returns rows up to limit and signals truncated when more exist", () => {
  ...
  try {
    ...
    const reg = loadReferenceQueries(dir);
    const page = getMany(requireReadManyQuery(reg.listTestItems), [], { limit: 3 });
...
test("getMany returns truncated=false when fewer rows exist", () => {
  ...
  try {
    ...
    const reg = loadReferenceQueries(dir);
    const page = getMany(requireReadManyQuery(reg.listTestItems), [], { limit: 10 });
...
test("getMany rejects limit=0", () => {
  ...
  try {
    setupDb();
    const reg = loadReferenceQueries(dir);
    assert.throws(() => getMany(requireReadManyQuery(reg.listTestItems), [], { limit: 0 }), UnboundedReadError);
  } finally {
    teardown(dir);
  }
});
...
test("getMany rejects negative limit", () => {
  ...
  try {
    setupDb();
    const reg = loadReferenceQueries(dir);
    assert.throws(() => getMany(requireReadManyQuery(reg.listTestItems), [], { limit: -1 }), UnboundedReadError);
  } finally {
    teardown(dir);
  }
});
...
test("getMany rejects limit above MAX_PAGE_LIMIT", () => {
  ...
  try {
    ...
    const reg = loadReferenceQueries(dir);
    assert.throws(
      () => getMany(requireReadManyQuery(reg.listTestItems), [], { limit: MAX_PAGE_LIMIT + 1 }),
      UnboundedReadError
    );
  } finally {
    teardown(dir);
...
test("iterate yields rows lazily and the caller can break early", () => {
  ...
  try {
    ...
    for (const row of iterate<{ id: number; name: string }>(requireIterateQuery(reg.iterateTestItems))) {
      seen.push(row);
      if (seen.length >= 3) {
        break;
      }
...
test("exec runs a mutation and returns changes plus lastInsertRowid", () => {
  ...
  try {
    setupDb();
    const reg = loadReferenceQueries(dir);
    const result = exec(requireMutationQuery(reg.insertTestItem), ["alpha", 7]);
...
test("execReturningOne runs a mutation and returns the RETURNING row", () => {
  ...
  try {
    setupDb();
    const reg = loadReferenceQueries(dir);
    const handle = requireMutationReturningOneQuery(reg.insertTestItemReturning);
    assert.equal(handle.terminator, "exec_one");
    const row = execReturningOne<{ id: number; name: string; value: number }>(handle, ["alpha", 7]);
...
test("execReturningOne mutates the underlying table — atomic upsert+RETURNING returns the freshly written value", () => {
  ...
  try {
    ...
    const reg = loadReferenceQueries(dir);
    const upsertTestCounter = requireMutationReturningOneQuery(reg.upsertTestCounter);
    const first = execReturningOne<{ value: number }>(upsertTestCounter, ["counter"]);
    assert.equal(first.value, 1);
    const second = execReturningOne<{ value: number }>(upsertTestCounter, ["counter"]);
    assert.equal(second.value, 2);
    const third = execReturningOne<{ value: number }>(upsertTestCounter, ["counter"]);
    assert.equal(third.value, 3);
    const persisted = requireRow<{ value: number }>(
      db.prepare("SELECT value FROM test_items WHERE name = ?").get("counter")
    );
...
test("execReturningOne throws when the statement returns zero rows", () => {
  ...
  try {
    ...
    const reg = loadReferenceQueries(dir);
    assert.throws(
      () => execReturningOne(requireMutationReturningOneQuery(reg.deleteTestItemReturning), [9999]),
...
test("transaction commits on normal return", () => {
  ...
  try {
    ...
    const count = requireRow<{ n: number }>(db.prepare("SELECT COUNT(*) AS n FROM test_items").get());
...
test("writeTransaction acquires the write lock at transaction start (BEGIN IMMEDIATE)", () => {
  ...
  let sibling: BetterSqlite3.Database | undefined;
  try {
    closeDb();
    ...
    // Re-bind to a const so the closure below (which TS treats as a
    // potential reassignment site for the outer `let`) keeps the
    // non-undefined narrowing from the `new Database(...)` assignment above.
    const siblingDb = sibling;

    let observedBusy = false;
    writeTransaction(() => {
      // No write yet — under BEGIN IMMEDIATE the lock is already held.
      try {
        siblingDb.exec("BEGIN IMMEDIATE");
        siblingDb.exec("ROLLBACK");
      } catch (err) {
        // biome-ignore lint/performance/useTopLevelRegex: localized test assertion preserves its explicit contract.
    ...
    assert.equal(
      observedBusy,
      true,
      "expected sibling BEGIN IMMEDIATE to see SQLITE_BUSY while writeTransaction held the write lock"
    );
  } finally {
    if (sibling) {
      sibling.close();
...
test("allowUnboundedReadAcknowledged returns rows when count <= maxRows", () => {
  ...
  try {
    ...
    const reg = loadReferenceQueries(dir);
    const rows = allowUnboundedReadAcknowledged(requireSmallEnumerationQuery(reg.listAllItems));
...
test("allowUnboundedReadAcknowledged throws when count exceeds @max_rows", () => {
  ...
  try {
    ...
    const reg = loadReferenceQueries(dir);
    assert.throws(
      () => allowUnboundedReadAcknowledged(requireSmallEnumerationQuery(reg.listAllItems)),
      SmallEnumerationOverflowError
    );
  } finally {
    teardown(dir);
...
test("iterateDynamicSqlAcknowledged streams rows from a dynamically-built SQL string", () => {
  ...
  try {
    ...
    const seen: Array<{ id: number; name: string }> = [];
    for (const row of iterateDynamicSqlAcknowledged<{ id: number; name: string }>(dynamicSql, [2, 10])) {
      seen.push(row);
    }
    assert.equal(seen.length, 3);
    assert.ok(seen[0]);
...
test("iterateDynamicSqlAcknowledged supports caller break-out", () => {
  ...
  try {
    ...
    const seen = [];
    for (const row of iterateDynamicSqlAcknowledged("SELECT id FROM test_items ORDER BY id LIMIT ?", [100])) {
      seen.push(row);
      if (seen.length >= 3) {
        break;
      }
...
test("encodeCursor and decodeCursor round-trip", () => {
  const payload = { k: "abc-123", r: 42, v: 1 };
...
test("decodeCursor rejects unsupported version", () => {
  const encoded = Buffer.from(JSON.stringify({ k: "x", r: 1, v: 999 }), "utf8").toString("base64url");
  assert.throws(() => decodeCursor(encoded), InvalidCursorError);
});
...
test("getMany pagination yields a stable, non-overlapping sequence", () => {
  ...
  try {
    ...
    const page1 = getMany<{ id: number; name: string }>(requireReadManyQuery(reg.firstPage), [], { limit: 3 });
    assert.equal(page1.rows.length, 3);
    assert.equal(page1.truncated, true);
    assert.ok(page1.nextCursor);
    const page1Cursor = decodeCursor(page1.nextCursor);

    const page2 = getMany<{ id: number; name: string }>(requireReadManyQuery(reg.nextPage), [page1Cursor.r], {
      limit: 3,
    });
    ...
    for (let i = 1; i < allIds.length; i++) {
      ...
      const previous = allIds[i - 1];
      assert.ok(current !== undefined && previous !== undefined, "both ids must exist within array bounds");
      assert.ok(current > previous, `expected ${current} > ${previous}`);
    }
  } finally {
    teardown(dir);
  }
