...
const LIFECYCLES = [null, { status: 'active' }, { status: 'draft' }, { status: 'revoked' }];
          ...
          for (const lifecycle of LIFECYCLES) {
            for (const active of PROGRESS_ACTIVE) {
              ...
              if (lifecycle?.status === 'draft') {
                assert.equal(state1.resolver, 'setup_in_progress', 'draft lifecycle must always resolve setup_in_progress');
              }
