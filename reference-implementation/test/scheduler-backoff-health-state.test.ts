...
import { computeNextRunWithBackoff, isCounterResetStatus } from "../runtime/scheduler-backoff.ts";
import type { RunRecord } from "../runtime/scheduler-domain-types.ts";

const BASE_INTERVAL_MS = 60_000;
const T0 = Date.parse("2026-05-16T00:00:00.000Z");

function failedRun(): RunRecord {
  return {
    attempt: 1,
...
function streak(n: number): RunRecord[] {
...
test("recommendedHealthState stays cooling_off one below the blocked promotion threshold", () => {
  const decision = computeNextRunWithBackoff(streak(BLOCKED_PROMOTION_THRESHOLD - 1), BASE_INTERVAL_MS, T0);
...
test("recommendedHealthState promotes to blocked exactly at the promotion threshold", () => {
  const decision = computeNextRunWithBackoff(streak(BLOCKED_PROMOTION_THRESHOLD), BASE_INTERVAL_MS, T0);
...
test("recommendedHealthState stays blocked above the promotion threshold", () => {
  const decision = computeNextRunWithBackoff(streak(BLOCKED_PROMOTION_THRESHOLD + 3), BASE_INTERVAL_MS, T0);
  assert.equal(decision.recommendedHealthState, "blocked");
});
...
test("recommendedHealthState is null for a manual bypass even with a deep streak", () => {
  const decision = computeNextRunWithBackoff(streak(BLOCKED_PROMOTION_THRESHOLD + 5), BASE_INTERVAL_MS, T0, {
    manual: true,
  });
  assert.equal(decision.backoffApplied, false);
  assert.equal(decision.recommendedHealthState, null);
...
test("isCounterResetStatus is false for non-success statuses", () => {
  ...
  // @ts-expect-error 'running' is not a RunStatus — proves the check is a strict
  // equality against 'succeeded', not a truthy/loose status check, even for a
  // caller passing a status string from an untyped boundary.
