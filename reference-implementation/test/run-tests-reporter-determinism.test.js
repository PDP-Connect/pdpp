 ...
 *
 * The fix (reference-implementation/scripts/run-tests.js) stops forwarding
...
async function runNodeTestOnce(extraArgs) {
...
describe('run-tests reporter determinism (compact-record-history.test.js)', () => {
  ...
  it('run-tests.js never forwards --test-force-exit to spawned child test processes', async () => {
    const runTestsSource = await import('node:fs/promises').then((fs) =>
      fs.readFile(fileURLToPath(new URL('../scripts/run-tests.js', import.meta.url)), 'utf8'));
    assert.doesNotMatch(
      runTestsSource,
      /effectiveArgs\s*=\s*\[?['"]--test-force-exit['"]/,
      'run-tests.js must not forward --test-force-exit — it truncates the reporter event stream non-deterministically',
    );
  });
});
