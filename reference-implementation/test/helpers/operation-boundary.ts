 ...
 */
export function discoverOperationModules(repoRoot: string): Array<{ name: string; absPath: string; relPath: string }> {
  const operationsDir = path.join(repoRoot, "reference-implementation", "operations");

  let entries: import("node:fs").Dirent<string>[];
  try {
    entries = readdirSync(operationsDir, { encoding: "utf8", withFileTypes: true });
  } catch (err) {
  ...
  const modules: Array<{ name: string; absPath: string; relPath: string }> = [];
 ...
 */
export function assertOperationBoundary(source: string, label: string): void {
  for (const needle of forbiddenOperationImports) {
    ...
    assert.equal(matched, false, `${label}: operation module must not import "${needle}"`);
  }
  ...
  assert.equal(stripped.includes("process.env"), false, `${label}: operation module must not read process.env`);
}
 ...
 * @param {string} label
 */
export function assertOperationBoundaryAtPath(absPath: string, label: string): void {
