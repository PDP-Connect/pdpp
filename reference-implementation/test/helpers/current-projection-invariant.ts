...
interface ProjectionLatest {
  deleted: boolean;
  jsonEqual: boolean;
  version: number;
}

interface ProjectionCurrent {
  deleted: boolean;
  version: number;
}
...
interface ProjectionRow {
  ...
  current_exists?: number;
  current_version?: number;
  deleted?: number;
  json_equal?: number;
  latest_history_deleted?: number;
  latest_history_version?: number;
  record_key: string;
  stream: string;
  version?: number;
}

interface ProjectionPreview {
  connectorInstanceId: string;
 ...
 */
export function truncateProjectionKey(key: string): string {
...
export function classifyProjectionMismatch(
  latest: ProjectionLatest | null,
  current: ProjectionCurrent | null
): string | null {
  if (!latest) {
    return null;
  }
  ...
  if (latest.deleted) {
    if (current && !current.deleted) {
      return PROJECTION_MISMATCH_KINDS.LATEST_DELETED;
    }
    return null;
  }
 ...
 */
export function detectCurrentProjectionDrift(scope: ProjectionScope = {}) {
  const db = getDb();
  const where: string[] = [];
  const params: string[] = [];
  if (scope.connectorInstanceId !== null && scope.connectorInstanceId !== undefined) {
  ...
  const previews: ProjectionPreview[] = [];
  for (const row of historyRows) {
    ...
    const current =
      row.current_exists === 1 ? { deleted: row.current_deleted === 1, version: Number(row.current_version) } : null;
  ...
  // false "consistent".
  const orphanWhere = [
    "NOT EXISTS (SELECT 1 FROM record_changes c WHERE c.connector_instance_id = r.connector_instance_id AND c.stream = r.stream AND c.record_key = r.record_key)",
  ];
  ...
  const orphanRows = db
    ...
    .all(...orphanParams) as ProjectionRow[];
 ...
 */
export function formatDriftPreviews(previews: ProjectionPreview[]): string {
  return previews
    .map(
      (p) =>
 ...
 */
export function assertNoCurrentProjectionDrift(scope: ProjectionScope = {}): void {
