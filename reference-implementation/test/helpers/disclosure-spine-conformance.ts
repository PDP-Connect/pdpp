...
interface SpineDriver {
  append: (input: Record<string, unknown>) => Promise<SpineEvent>;
  listPage: (
    kind: "grant" | "run" | "trace",
    id: string,
  ...
  ) => Promise<{ summaries: readonly SpineSummary[] }>;
  setup: () => Promise<void>;
 ...
 */
export function runDisclosureSpineConformance({
  label,
  test,
  makeDriver,
}: {
  label: string;
  test: TestFn;
  makeDriver: () => Promise<SpineDriver> | SpineDriver;
}): void {
  ...
  t("append/list preserves append order even when occurred_at is non-monotonic", async () => {
    ...
    try {
      ...
      const sequence = [
        { occurred_at: "2026-04-01T00:00:05.000Z", tag: "a" },
        { occurred_at: "2026-04-01T00:00:10.000Z", tag: "b" },
        { occurred_at: "2026-04-01T00:00:00.000Z", tag: "c" },
        { occurred_at: "2026-04-01T00:00:12.000Z", tag: "d" },
        { occurred_at: "2026-04-01T00:00:07.000Z", tag: "e" },
      ];
      ...
      for (const step of sequence) {
        ...
        assert.ok(r?.event_id, "append must return a record with event_id");
      ...
      assert.deepEqual(eventIds, appended, "listPage must return events in append order, not occurred_at order");
  ...
  t("listPage cursor walks the full timeline without overlap or gap", async () => {
    ...
    try {
      ...
      while (pages < 20) {
        // biome-ignore lint/performance/noAwaitInLoops: Sequential test setup and assertion order is intentional.
        const page = await driver.listPage("trace", traceId, { cursor, limit: 3 });
        pages += 1;
      ...
      assert.deepEqual(collected, ids, "paged walk must visit every event exactly once, in append order");
      assert.equal(new Set(collected).size, collected.length, "paged walk must not repeat events across pages");
  ...
  t("listPage pages long trace, grant, and run timelines", async () => {
    ...
    try {
      const cases = [
        { field: "trace_id", id: "trc_long_kind", kind: "trace" },
        { field: "grant_id", id: "grnt_long_kind", kind: "grant" },
        { field: "run_id", id: "run_long_kind", kind: "run" },
      ];
      const total = 13;
      ...
      for (const c of cases) {
        ...
        while (pages < 20) {
          // biome-ignore lint/performance/noAwaitInLoops: Sequential test setup and assertion order is intentional.
          const page = await driver.listPage(c.kind as "grant" | "run" | "trace", c.id, { cursor, limit: 4 });
          pages += 1;
          for (const ev of page.events) {
            collected.push(ev.event_id);
          }
  ...
  t("latest event lookup returns the most recently appended event even with non-monotonic occurred_at", async () => {
    ...
    try {
      ...
      const schedule = [
        { occurred_at: "2026-04-03T00:00:00.000Z", status: "in_progress", type: "start" },
        { occurred_at: "2026-04-03T00:00:05.000Z", status: "in_progress", type: "progress" },
        { occurred_at: "2026-04-03T00:00:30.000Z", status: "in_progress", type: "progress" },
        { occurred_at: "2026-04-03T00:00:10.000Z", status: "succeeded", type: "finish" },
      ];
      // biome-ignore lint/suspicious/noEvolvingTypes: Accumulator evolves through deliberately heterogeneous fixture data.
      let lastId = null;
  ...
  t("correlation summary reports full extent even when timeline is large and non-monotonic", async () => {
    ...
    try {
      ...
      const offsets = [5000, 1000, 9000, 3000, 11_000, 0, 7000, 2000, 10_000, 4000, 8000, 6000];
      ...
      assert.equal(
        row.event_count,
        occurredValues.length,
        "summary event_count must equal the full correlation extent"
      );
      assert.equal(row.first_at, firstIso, "summary first_at must equal the earliest event");
      assert.equal(row.last_at, lastIso, "summary last_at must equal the latest event");
    } finally {
      await driver.teardown();
    }
  });
  ...
  t("paged walk preserves append order when every event shares the same occurred_at", async () => {
    ...
    try {
      ...
      while (pages < 20) {
        // biome-ignore lint/performance/noAwaitInLoops: Sequential test setup and assertion order is intentional.
        const page = await driver.listPage("trace", traceId, { cursor, limit: 2 });
        pages += 1;
        for (const ev of page.events) {
          collected.push(ev.event_id);
        }
  ...
  t("paged walk per correlation is stable when correlations are interleaved", async () => {
    ...
    try {
      ...
      const idsByTrace: Record<string, string[]> = { [traceA]: [], [traceB]: [] };
      for (const step of sequence) {
        // biome-ignore lint/performance/noAwaitInLoops: Sequential test setup and assertion order is intentional.
        const r = await driver.append(
          evt({
        ...
        const ids = idsByTrace[step.trace];
        assert.ok(ids);
        ids.push(r.event_id);
      }
      ...
      for (const [traceId, expected] of Object.entries(idsByTrace)) {
        ...
        assert.equal(new Set(collected).size, collected.length, `paged walk for ${traceId} must not repeat events`);
      }
    } finally {
      await driver.teardown();
    }
