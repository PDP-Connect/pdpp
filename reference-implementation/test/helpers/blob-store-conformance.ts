...
export interface BlobStoreIdentity {
  backend_kind: string;
  binding_kind: string;
  content_address: { algorithm: string; id_prefix: string };
  dedupe: string;
}

export interface PutBlobArgs {
  blobId: string;
  connectorId: string;
  data: Buffer;
  mimeType: string;
...
export interface BlobRow extends BlobMetadata {
...
export interface BlobStoreDriver {
  getBlob: (blobId: string) => Promise<BlobRow | null>;
  identity: () => BlobStoreIdentity;
  listBindingsForBlob: (blobId: string) => Promise<BindingTuple[]>;
  listBindingsForRecord: (scope: BindingScope) => Promise<BindingTuple[]>;
  putBinding: (tuple: BindingTuple) => Promise<void>;
  putBlob: (args: PutBlobArgs) => Promise<BlobMetadata>;
  setup: () => Promise<void>;
...
function blobIdFor(prefix: string, bytes: Buffer): string {
  return `${prefix}${sha256Hex(bytes)}`;
}

function buildPut(
  driver: BlobStoreDriver,
  {
    connectorId,
    stream,
    recordKey,
    mimeType,
    text,
  }: { connectorId: string; stream: string; recordKey: string; mimeType: string; text: string }
): PutBlobArgs {
...
function bindingsHave(bindings: BindingTuple[], expected: BindingTuple): boolean {
...
function toBuffer(maybeBytes: Buffer | Uint8Array): Buffer {
...
export function runBlobStoreConformance({
  ...
  label: string;
  test: TestFn;
  makeDriver: () => Promise<BlobStoreDriver> | BlobStoreDriver;
}): void {
  const t = (name: string, fn: () => Promise<void>) => test(`[blob-store-conformance:${label}] ${name}`, fn);
  ...
  t("advertises required backend identity fields", async () => {
    ...
    try {
      ...
      assert.equal(typeof id.content_address.algorithm, "string", "content_address.algorithm required");
      assert.ok(id.content_address.algorithm.length > 0, "content_address.algorithm must be non-empty");
      assert.equal(typeof id.content_address.id_prefix, "string", "content_address.id_prefix required");
