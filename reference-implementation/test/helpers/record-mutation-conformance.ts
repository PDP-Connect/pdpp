...
interface MutationDriver {
  ...
  readChanges: () => Promise<MutationRow[]>;
  readLive: (key: string) => Promise<MutationRow | null>;
  readVersionCounter: () => Promise<number | null>;
  setDeleteFault: (hook: ((point: string) => void) | null) => Promise<void>;
  setIngestFault: (hook: ((point: string) => void) | null) => Promise<void>;
  setup: () => Promise<void>;
 ...
 */
export function runRecordMutationConformance({
  label,
  test,
  makeDriver,
}: {
  label: string;
  test: TestFn;
  makeDriver: () => Promise<MutationDriver> | MutationDriver;
}): void {
  ...
  t("failure between ingest live mutation and record_changes append rolls back the durable unit", async () => {
    ...
    try {
      ...
      await assert.rejects(() => driver.ingestUpsert("a", { v: 2 }), REGEXP_1);
  ...
  t("failure between direct-delete live mutation and record_changes append rolls back the durable unit", async () => {
    ...
    try {
      ...
      await assert.rejects(() => driver.directDelete("a"), REGEXP_2);
  ...
  t("record_changes is contiguous across mixed ingest/delete/direct-delete writes", async () => {
    ...
    try {
      ...
      assert.equal(await driver.readVersionCounter(), versions.at(-1));
      assert.equal(new Set(versions).size, versions.length);
    } finally {
      await driver.teardown();
    }
