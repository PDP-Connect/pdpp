...
interface LexicalDriver {
  deleteRecord: (args: { connectorId: string; stream: string; recordKey: string }) => Promise<void>;
  deleteStream: (args: { connectorId: string; stream: string }) => Promise<void>;
  ...
  upsert: (args: LexicalUpsert) => Promise<void>;
}
...
function buildSeedRecords(): SeedRecord[] {
  ...
  return [
    {
      fields: { body: "alpha gamma in body", title: "alpha topic" },
      recordKey: "r1",
    },
    {
      fields: { body: "beta only here", title: "beta topic" },
      recordKey: "r2",
    },
    {
      fields: { body: "alpha mention but quiet", title: "unrelated" },
      recordKey: "r3",
    },
    {
      fields: { body: "no other tokens", title: "gamma plus extras" },
      recordKey: "r4",
    },
  ];
}

async function seedDefault(driver: LexicalDriver): Promise<void> {
...
function recordKeysOf(hits: LexicalHit[]): string[] {
 ...
 */
export function runLexicalRetrievalConformance({
  label,
  test,
  makeDriver,
}: {
  label: string;
  test: TestFn;
  makeDriver: () => Promise<LexicalDriver> | LexicalDriver;
}): void {
  const t = (name: string, fn: () => Promise<void>) => test(`[lexical-conformance:${label}] ${name}`, fn);
  ...
  t("advertises required backend identity fields", async () => {
    ...
    try {
      ...
      assert.equal(typeof id.score.value_semantics, "string", "score.value_semantics required");
    } finally {
    ...
    try {
      ...
      for (const hit of hits) {
        for (const f of hit.matchedFields) {
          assert.ok(["title", "body"].includes(f), `matched_fields entry '${f}' is not in declared searchable fields`);
        }
      }
    } finally {
      await driver.teardown();
  ...
  t("searchable-field restriction excludes hits matched only on excluded fields", async () => {
    ...
    try {
      ...
      assert.ok(!keys.has("r3"), "r3 only has alpha in body — must NOT match when searchableFields=[title]");
      for (const hit of hits) {
        assert.deepEqual(hit.matchedFields, ["title"], "matched_fields must be limited to the declared searchable set");
      }
    } finally {
      await driver.teardown();
    }
  ...
  t("upsert replaces existing index rows for the same record", async () => {
    ...
    try {
      ...
      await driver.upsert({
        connectorId: CONNECTOR_A,
        fields: { body: "no token here", title: "topic only" },
        recordKey: "r1",
        stream: STREAM_POSTS,
      });
      const hits = await driver.search({
      ...
      const keys = new Set(recordKeysOf(hits));
      assert.ok(!keys.has("r1"), "r1 was overwritten with non-matching text — must not match alpha");
    ...
    try {
      ...
      if (id.score.order === "lower_is_better") {
        for (let i = 1; i < hits.length; i += 1) {
          ...
          if (!(previous && current)) {
            throw new Error(`missing hit at index ${i}`);
          }
          assert.ok(
            previous.score <= current.score,
            `lower_is_better violated at index ${i}: ${previous.score} > ${current.score}`
          );
        }
        ...
        for (let i = 1; i < hits.length; i += 1) {
          ...
          assert.ok(
            previous.score >= current.score,
            `higher_is_better violated at index ${i}: ${previous.score} < ${current.score}`
          );
        }
      }
    } finally {
  ...
  t("repeated identical queries produce identical ordering (deterministic ties)", async () => {
    ...
    try {
      ...
      const tieRecords = ["t1", "t2", "t3", "t4"].map((rk) => ({
        fields: { body: "omega body", title: "omega omega omega" },
        recordKey: rk,
      }));
  ...
  t("snippet text is extracted from source field, not generated", async () => {
    ...
    try {
      ...
      if (id.supports_snippets) {
        ...
        assert.equal(hit.snippet.field, "body", "snippet field must be the field that actually matched");
        ...
        const stripMarks = (s: string): string => s.replace(/<\/?[a-zA-Z][^>]*>/g, "");
        const core = stripMarks(hit.snippet.text)
