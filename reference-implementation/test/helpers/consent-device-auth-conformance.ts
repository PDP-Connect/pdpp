...
interface ConsentView {
  ...
  grant_id?: string | null | undefined;
...
interface ConsentDriver {
  approveOwnerDeviceAuth: (code: string) => Promise<ApprovalResult>;
  approvePendingConsent: (uri: string) => Promise<ApprovalResult>;
  denyOwnerDeviceAuth: (code: string) => Promise<void>;
  denyPendingConsent: (uri: string) => Promise<boolean>;
  exchangeOwnerDeviceCode: (input: Record<string, string>) => Promise<ApprovalResult>;
  forceExpireOwnerDeviceAuth: (code: string) => Promise<void>;
  forceExpirePendingConsent: (uri: string) => Promise<void>;
  getRegisteredClientId: () => string;
  getRegisteredConnectorId: () => string;
  lookupOwnerDeviceAuthByApprovalId: (id: string) => Promise<ConsentView | null>;
  lookupOwnerDeviceAuthByUserCode: (code: string) => Promise<ConsentView | null>;
  lookupPendingConsentByApprovalId: (id: string) => Promise<ConsentView | null>;
  lookupPendingConsentByRequestUri: (uri: string) => Promise<ConsentView | null>;
  rewindOwnerDevicePollTimer: (code: string) => Promise<void>;
  setup: () => Promise<void>;
  startOwnerDeviceAuth: (input: Record<string, unknown>) => Promise<OwnerStart>;
  startPendingConsent: (input: Record<string, unknown>) => Promise<PendingStart>;
  teardown: () => Promise<void>;
}

type TestFn = (name: string, fn: () => Promise<void>) => void;

function assertNotSupported(err: unknown): boolean {
  return errorCode(err) === "not_supported";
}
 ...
 */
export function runConsentDeviceAuthConformance({
  label,
  test,
  makeDriver,
}: {
  label: string;
  test: TestFn;
  makeDriver: () => Promise<ConsentDriver> | ConsentDriver;
}): void {
  ...
  t("pending consent: denial is terminal — lookup disappears, approve fails, redeny is a no-op", async () => {
    ...
    try {
      ...
      assert.equal(
        ...
        "not_found",
        `approve-after-deny error MUST carry code='not_found'; got '${errorCode(approveErr)}'`
      );
  ...
  t("pending consent: expired requests are unavailable for lookup, approval, or denial", async () => {
    ...
    try {
      ...
      assert.equal(
        errorCode(approveErr),
        "not_found",
        `expired-approve error MUST carry code='not_found'; got '${errorCode(approveErr)}'`
      );
  ...
  t("owner device auth: start returns RFC 8628-shaped fields plus a separate approval_id", async () => {
    ...
    try {
      ...
      assert.ok(
        typeof start.expires_in === "number" && start.expires_in > 0,
        `expires_in must be a positive number, got ${start.expires_in}`
      );
      assert.ok(start.approval_id, "start must return an approval_id");
      assert.notEqual(start.approval_id, start.device_code, "approval_id MUST NOT be the device_code itself");
      assert.notEqual(start.approval_id, start.user_code, "approval_id MUST NOT be the user_code itself");
    } finally {
      await driver.teardown();
    }
  });
  ...
  t("owner device auth: poll before approval throws authorization_pending", async () => {
    ...
    try {
      ...
      assert.equal(
        ...
        `poll-before-approval MUST throw code='authorization_pending'; got '${errorCode(err)}'`
      );
    } finally {
      await driver.teardown();
    }
  ...
  t("owner device auth: polling faster than the interval throws slow_down", async () => {
    ...
    try {
      ...
      try {
        ...
        if (errorCode(e) !== "authorization_pending") {
          throw e;
        }
      }
      ...
      assert.equal(
        errorCode(err),
        "slow_down",
        `rapid second poll MUST throw code='slow_down'; got '${errorCode(err)}'`
      );
    } finally {
      await driver.teardown();
    }
  ...
  t("owner device auth: approve + exchange round-trip yields an owner access token", async () => {
    ...
    try {
      ...
      assert.ok(approveResult.access_token, "approveOwnerDeviceAuth MUST return an owner access_token");

      const exchange = await driver.exchangeOwnerDeviceCode({
        client_id: driver.getRegisteredClientId(),
        device_code: start.device_code,
      });
      assert.ok(exchange.access_token, "exchangeOwnerDeviceCode after approval MUST return an access_token");
      assert.equal(
      ...
      assert.equal(afterApprove, null, "after approval, public user_code lookup MUST NOT return a pending view");
    } finally {
      await driver.teardown();
    }
  });
  ...
  t("owner device auth: denial is terminal — exchange throws access_denied", async () => {
    ...
    try {
      ...
      assert.equal(afterDeny, null, "after denial, public user_code lookup MUST NOT return a pending view");
      ...
      assert.equal(
        errorCode(exchangeErr),
        "access_denied",
        `exchange-after-deny MUST throw code='access_denied'; got '${errorCode(exchangeErr)}'`
      );
    } finally {
      await driver.teardown();
    }
  ...
  t("owner device auth: expired requests reject exchange with expired_token", async () => {
    ...
    try {
      ...
      assert.equal(
        errorCode(exchangeErr),
        "expired_token",
        `expired exchange MUST throw code='expired_token'; got '${errorCode(exchangeErr)}'`
      );

      const view = await driver.lookupOwnerDeviceAuthByUserCode(start.user_code);
      assert.equal(view, null, "expired owner device auth MUST NOT be returned by public lookup");
    } finally {
      await driver.teardown();
    }
  });
