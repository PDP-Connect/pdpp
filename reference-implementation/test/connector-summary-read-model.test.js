...
test('rebuild derives durable identity + count evidence from canonical state', () =>
  withTempDb(async () => {
    ...
    assert.deepEqual(
      [...row.stream_records].sort((a, b) => a.stream.localeCompare(b.stream)),
      [
        { stream: 'attachments', declaration_state: 'unavailable', count_state: 'known', record_count: 1, retained_record_count: 1 },
        { stream: 'messages', declaration_state: 'unavailable', count_state: 'known', record_count: 2, retained_record_count: 2 },
      ],
    );
  ...
  async () => {
    ...
    try {
      ...
      assert.deepEqual(row.stream_records, [
        { stream: 'messages', declaration_state: 'unavailable', count_state: 'known', record_count: 1, retained_record_count: 1 },
      ]);
      assert.deepEqual(row.retained_bytes, {
