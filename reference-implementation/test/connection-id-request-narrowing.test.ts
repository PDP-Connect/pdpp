...
interface AssertThrowsExpectation {
  code?: string;
  messageIncludes?: string;
  param?: string;
}

function assertThrows(fn: () => void, { code, param, messageIncludes }: AssertThrowsExpectation = {}) {
  let thrown: (Error & { code?: string; param?: string }) | undefined;
  try {
    fn();
  } catch (err) {
    assert.ok(err instanceof Error);
    thrown = err as Error & { code?: string; param?: string };
  }
