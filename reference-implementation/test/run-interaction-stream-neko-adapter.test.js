...
function makeFetch(routes) {
  ...
  const fetchImpl = async (url, init = {}) => {
    ...
    if (!route && init.method === 'GET' && url === 'https://neko.test/api/room/screen') {
      return makeResponse({ json: { width: 1280, height: 720, rate: 30 } });
    }
...
test('n.eko adapter logs in, applies configured viewport endpoint, and emits base64 JPEG frames', async () => {
  ...
  const viewport = fetchImpl.calls.find(
    (call) => call.init.method === 'POST' && call.url.endsWith('/api/room/screen'),
  );
...
test('n.eko adapter frame metadata follows the applied desktop screen preset', async () => {
  ...
  const screenPost = fetchImpl.calls.find((call) => call.init.method === 'POST' && call.url === 'https://neko.test/api/room/screen');
...
test('n.eko adapter emits explicit screen-configuration telemetry for mobile viewport updates', async () => {
  ...
  assert.deepEqual(JSON.parse(fetchImpl.calls.find((call) => call.init.method === 'POST' && call.url === 'https://neko.test/api/room/screen').init.body), {
    width: 448,
    height: 820,
    rate: 30,
  });
...
test('n.eko presentation captures one baseline across reconnects and restores it on terminal stop', async () => {
  const lifecycle = { captured: [], restored: [] };
  const fetchImpl = makeFetch([
    {
      method: 'GET',
      url: 'https://neko.test/api/room/screen',
      response: makeResponse({ json: { width: 1280, height: 720, rate: 30 } }),
    },
    {
      method: 'GET',
      url: 'https://neko.test/api/room/screen/configurations',
      response: makeResponse({ json: [{ width: 390, height: 844, rate: 30 }] }),
    },
    {
      method: 'POST',
      url: 'https://neko.test/api/room/screen',
  ...
  const companion = createNekoCompanion({
    ...
    presentationLifecycle: {
      captureBaseline: ({ baseline }) => lifecycle.captured.push(baseline),
      markRestored: ({ baseline }) => lifecycle.restored.push(baseline),
    },
    screenConfigurationsEndpoint: '/api/room/screen/configurations',
    screenEndpoint: '/api/room/screen',
    sleep: makeAbortableSleep(),
  ...
  await companion.start({ width: 390, height: 844, deviceScaleFactor: 1 });
  ...
  await companion.start({ width: 390, height: 844, deviceScaleFactor: 1 });
  await companion.dispatch({ type: 'viewport', width: 390, height: 844, deviceScaleFactor: 1 });
  await companion.stop();
  ...
  assert.deepEqual(JSON.parse(screenCalls.at(-1).init.body), { width: 1280, height: 720, rate: 30 });
});

test('n.eko presentation keeps terminal restore pending until the baseline window has settled', async () => {
  const lifecycle = { restored: [] };
  let releaseBaselineSettle;
  const baselineSettle = new Promise((resolve) => { releaseBaselineSettle = resolve; });
  const fetchImpl = makeFetch([
    { method: 'GET', url: 'https://neko.test/api/room/screen', response: makeResponse({ json: { width: 1280, height: 720, rate: 30 } }) },
    { method: 'GET', url: 'https://neko.test/api/room/screen/configurations', response: makeResponse({ json: [{ width: 390, height: 844, rate: 30 }] }) },
    { method: 'POST', url: 'https://neko.test/api/room/screen', response: ({ init }) => makeResponse({ json: JSON.parse(init.body) }) },
    {
      method: 'GET',
      url: /https:\/\/neko\.test\/pdpp\/window-settle/,
      response: async ({ url }) => {
        const requested = new URL(url);
        if (requested.searchParams.get('width') === '1280') await baselineSettle;
        return makeResponse({
          json: { settled: true, width: Number(requested.searchParams.get('width')), height: Number(requested.searchParams.get('height')) },
        });
      },
    },
  ]);
  const companion = createNekoCompanion({
    ...
    bearerToken: 'test-token',
    fetchImpl,
    presentationLifecycle: { markRestored: ({ baseline }) => lifecycle.restored.push(baseline) },
    screenConfigurationsEndpoint: '/api/room/screen/configurations',
    screenEndpoint: '/api/room/screen',
    sleep: makeAbortableSleep(),
    target: {
...
test('n.eko refuses a managed surface whose settle endpoint is absent', () => {
  assert.throws(
    ...
    (error) => error?.code === 'neko_window_settle_endpoint_required',
  );
});
...
test('n.eko presentation serializes viewport epochs and discards a stale queued rotation before restore', async () => {
  ...
  const fetchImpl = makeFetch([
    ...
    },
    {
      method: 'GET',
      url: 'https://neko.test/api/room/screen/configurations',
      response: makeResponse({ json: [{ width: 390, height: 844, rate: 30 }, { width: 844, height: 390, rate: 30 }] }),
    },
    {
      method: 'POST',
      url: 'https://neko.test/api/room/screen',
      response: async ({ init }) => {
        ...
        posted.push(body);
        if (body.width === 390) await firstApply;
  ...
  const portrait = companion.dispatch({ type: 'viewport', width: 390, height: 844, deviceScaleFactor: 1 });
  await waitFor(() => posted.length === 1);
  const landscape = companion.dispatch({ type: 'viewport', width: 844, height: 390, deviceScaleFactor: 1 });
