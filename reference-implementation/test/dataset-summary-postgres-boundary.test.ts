...
import { closePostgresStorage, initPostgresStorage, isPostgresStorageBackend } from "../server/postgres-storage.ts";

const TOP_LEVEL_REGEX_1 = /Postgres mode/;
const TOP_LEVEL_REGEX_2 = /getDatasetSummaryProjection/;
const TOP_LEVEL_REGEX_3 = /listStreamProjections/;
const TOP_LEVEL_REGEX_4 = /applyDatasetSummaryRecordDelta/;
const TOP_LEVEL_REGEX_5 = /applyDatasetSummaryBlobDelta/;
const TOP_LEVEL_REGEX_6 = /markDatasetSummaryProjectionStale/;
const TOP_LEVEL_REGEX_7 = /rebuildDatasetSummaryProjection/;
const TOP_LEVEL_REGEX_8 = /reconcileDirtyDatasetSummaryRecordTimeBounds/;

/**
 * `server/dataset-summary-read-model.js`, `server/db.js`, and
 * `server/postgres-storage.js` are untyped JS (allowJs, checkJs:false)
 * under server/**, forbidden to touch. The boundary guard
 * (`assertSqliteBackendForDatasetSummary` in dataset-summary-read-model.js)
 * throws a plain `Error` with a bolted-on `.code = 'storage_backend_mismatch'`
 * string field rather than a dedicated error class, so the `assert.throws`/
 * `assert.rejects` predicate callbacks below need a local narrowing guard
 * to check `.code`/`.message` on the otherwise-`unknown` caught value.
 */
function isStorageBackendMismatchError(err: unknown): err is Error & { code: string } {
...
if (POSTGRES_URL) {
  test("SQLite dataset-summary read model throws storage_backend_mismatch in Postgres mode", async () => {
    ...
    db.prepare(
      ...
      JSON.stringify({
        counts: { connector_count: 99, record_count: 99, stream_count: 99 },
        ...
        retained_bytes: { blob_bytes: 0, record_changes_json_bytes: 0, record_json_bytes: 999_999 },
    ...
    assert.equal(isPostgresStorageBackend(), true, "precondition: Postgres backend must be active for this regression");
    ...
    try {
      ...
      assert.throws(
        () => getDatasetSummaryProjection(),
        (err: unknown) =>
          isStorageBackendMismatchError(err) &&
          err.code === "storage_backend_mismatch" &&
          TOP_LEVEL_REGEX_1.test(err.message) &&
          TOP_LEVEL_REGEX_2.test(err.message),
      ...
      assert.throws(
        ...
        (err: unknown) =>
          isStorageBackendMismatchError(err) &&
          err.code === "storage_backend_mismatch" &&
          TOP_LEVEL_REGEX_3.test(err.message),
      ...
      assert.throws(
        ...
        (err: unknown) => isStorageBackendMismatchError(err) && err.code === "storage_backend_mismatch",
      ...
      assert.throws(
        ...
        (err: unknown) =>
          isStorageBackendMismatchError(err) &&
          err.code === "storage_backend_mismatch" &&
          TOP_LEVEL_REGEX_4.test(err.message),
      ...
      assert.throws(
        ...
        (err: unknown) =>
          isStorageBackendMismatchError(err) &&
          err.code === "storage_backend_mismatch" &&
          TOP_LEVEL_REGEX_5.test(err.message),
      ...
      assert.throws(
        ...
        (err: unknown) =>
          isStorageBackendMismatchError(err) &&
          err.code === "storage_backend_mismatch" &&
          TOP_LEVEL_REGEX_6.test(err.message),
      ...
      await assert.rejects(
        () =>
          rebuildDatasetSummaryProjection({
            ...
            getRetainedBytes: () => ({ blob_bytes: 0, record_changes_json_bytes: 0, record_json_bytes: 0 }),
            listTopConnectorCandidates: () => [],
          }),
        (err: unknown) =>
          isStorageBackendMismatchError(err) &&
          err.code === "storage_backend_mismatch" &&
          TOP_LEVEL_REGEX_7.test(err.message),
      ...
      await assert.rejects(
        ...
        (err: unknown) =>
          isStorageBackendMismatchError(err) &&
          err.code === "storage_backend_mismatch" &&
          TOP_LEVEL_REGEX_8.test(err.message),
  ...
  test(
    "dataset-summary SQLite boundary guard in Postgres mode (skipped: PDPP_TEST_POSTGRES_URL unset)",
    {
      skip: true,
    },
    () => undefined
