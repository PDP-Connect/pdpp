...
function errorWithMessage(message: string): RuntimeError {
...
test("classifyRuntimeFailure maps connector protocol violations", () => {
  ...
  for (const message of messages) {
    assert.equal(classifyRuntimeFailure(errorWithMessage(message)), "connector_protocol_violation", message);
  }
});
