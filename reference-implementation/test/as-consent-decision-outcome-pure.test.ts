...
import {
  ...
  type AsConsentDecisionInput,
  executeAsConsentDecision,
} from "../operations/as-consent-decision/index.ts";

function inputFor(
  overrides: Partial<AsConsentDecisionInput> & { action: AsConsentDecisionInput["action"] }
): AsConsentDecisionInput {
  return {
    approvalId: null,
    requestUri: null,
    subjectId: "owner",
...
function resolvingDeps(overrides: Partial<AsConsentDecisionDependencies> = {}): AsConsentDecisionDependencies {
  return {
    approveGrant: async () => ({ grant: { grant_id: "g-1" }, token: "tok-1" }),
    buildPendingConsentRequestUri: (deviceCode: string) => `urn:req:${deviceCode}`,
    denyGrant: async () => true,
    getPendingConsentByApprovalId: async () => ({ device_code: "dc-1", status: "pending" }),
...
test("executeAsConsentDecision: neither request_uri nor approval_id is a 400 invalid_request", async () => {
  const out = await executeAsConsentDecision(inputFor({ action: "approve" }), resolvingDeps());
...
test("executeAsConsentDecision: an approval_id with a non-pending row is a 404 not_found", async () => {
  const out = await executeAsConsentDecision(
    ...
    resolvingDeps({ getPendingConsentByApprovalId: async () => ({ device_code: "dc-1", status: "consumed" }) })
  );
  assert.ok(out.outcome === "failure");
...
test("executeAsConsentDecision: a request_uri that resolves to no device code is a 400", async () => {
  const out = await executeAsConsentDecision(
    inputFor({ action: "approve", requestUri: "urn:req:missing" }),
...
test("executeAsConsentDecision: approve returns a success/approve outcome with grant + token", async () => {
  let approvedDeviceCode: string | null = null;
  const out = await executeAsConsentDecision(
    inputFor({ action: "approve", approveOptions: {}, requestUri: "urn:req:dc-1", subjectId: "owner" }),
    resolvingDeps({
      approveGrant: async (dc) => {
        approvedDeviceCode = dc;
        await Promise.resolve();
        return { grant: { grant_id: "g-9" }, token: "tok-9" };
      },
    })
  );
  assert.equal(approvedDeviceCode, "dc-1", "approve is called with the resolved device code");
  assert.ok(out.outcome === "success" && out.action === "approve");
  assert.deepEqual(out.grant, { grant_id: "g-9" });
...
test("executeAsConsentDecision: approve surfaces package fields when the grant is a package", async () => {
  const out = await executeAsConsentDecision(
    inputFor({ action: "approve", requestUri: "urn:req:dc-1" }),
    resolvingDeps({
      approveGrant: async () => ({ grant: { grant_id: "g" }, package: true, package_id: "pkg-1", token: "t" }),
    })
  );
  assert.ok(out.outcome === "success" && out.action === "approve");
...
test("executeAsConsentDecision: a deny that removes nothing (falsy) is a 404 not_found", async () => {
  const out = await executeAsConsentDecision(
    inputFor({ action: "deny", requestUri: "urn:req:dc-1" }),
...
test("executeAsConsentDecision: an approval_id resolves through to approve (device code from the row)", async () => {
  ...
  const out = await executeAsConsentDecision(
    ...
    resolvingDeps({
      buildPendingConsentRequestUri: (dc) => {
        builtFrom = dc;
        return `urn:req:${dc}`;
      },
      getPendingConsentByApprovalId: async () => ({ device_code: "dc-from-row", status: "pending" }),
    })
  );
