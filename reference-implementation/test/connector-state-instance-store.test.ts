...
test("connector sync state is isolated by connector_instance_id", async () => {
  ...
  await store.putState({ connectorId, connectorInstanceId: "cin_gmail_work" }, { [stream]: { cursor: "work" } });
  await store.putState(
    { connectorId, connectorInstanceId: "cin_gmail_personal" },
    { [stream]: { cursor: "personal" } }
  );

  assert.deepEqual((await store.getState({ connectorId, connectorInstanceId: "cin_gmail_work" })).state[stream], {
    cursor: "work",
  });
  assert.deepEqual((await store.getState({ connectorId, connectorInstanceId: "cin_gmail_personal" })).state[stream], {
    cursor: "personal",
  });
...
test("sqlite migration preserves existing instance ids when only one sync-state table needs rebuild", () => {
  ...
  try {
    ...
    assert.deepEqual(ownerRows, [
      { connector_instance_id: "cin_gmail_personal", state_json: '{"cursor":"personal"}', stream: "messages" },
      { connector_instance_id: "cin_gmail_work", state_json: '{"cursor":"work"}', stream: "messages" },
    ]);
    const grantRow = verify
      .prepare(
      ...
      .get() as { connector_instance_id: string; state_json: string };
    assert.equal(grantRow.connector_instance_id, makeDefaultAccountConnectorInstanceId("owner_local", "gmail"));
...
test("default connector-only records state uses deterministic default account connector instance id", async () => {
  ...
  assert.equal(
    getDb()
      ...
      .get<{ connector_instance_id: string }>(connectorId, stream)?.connector_instance_id,
    expectedInstanceId
  );
});
