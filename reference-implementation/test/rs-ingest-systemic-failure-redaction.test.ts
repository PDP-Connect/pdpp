...
//
// This suite proves the CURRENT fix: RecordsIngestSystemicFailureError's
// `.message` is a fixed, bounded template (stream name + counts only, both
// already public) that never interpolates the underlying failure's own
// text. A distinctive, deliberately secret-shaped marker injected into a
// real systemic failure's message must appear NOWHERE in the HTTP response
// body or the externally-readable persisted rejection event.
//
...
test("a secret-shaped marker in a systemic failure's underlying message never appears in the HTTP response or the persisted mutation.rejected event", async (t) => {
  ...
  try {
    ...
    assert.match(errorBody.error?.message ?? "", FIXED_TEMPLATE_RE);
  } finally {
