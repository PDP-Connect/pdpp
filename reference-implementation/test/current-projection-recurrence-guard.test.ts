...
import {
  deleteAllRecordsForConnector as deleteAllRecordsForConnectorUntyped,
  deleteAllRecords as deleteAllRecordsUntyped,
  deleteConnectionRecordRowsSqlite as deleteConnectionRecordRowsSqliteUntyped,
  deleteRecord as deleteRecordUntyped,
  ingestRecord as ingestRecordUntyped,
} from "../server/records.ts";
import {
  assertNoCurrentProjectionDrift,
  classifyProjectionMismatch,
  detectCurrentProjectionDrift as detectCurrentProjectionDriftUntyped,
  PROJECTION_MISMATCH_KINDS,
} from "./helpers/current-projection-invariant.ts";

/**
 * `server/db.js` and `server/records.js` are untyped JS (allowJs,
 * checkJs:false) under server/**, forbidden to touch. Same boundary-cast
 * pattern as `run-interaction-stream-routes.test.ts`: model the real
...
const deleteRecord = deleteRecordUntyped as unknown as (
  storageTarget: string,
  stream: string,
  recordId: string
) => Promise<unknown>;

const deleteAllRecords = deleteAllRecordsUntyped as unknown as (
  storageTarget: string,
  stream: string
) => Promise<unknown>;

const deleteAllRecordsForConnector = deleteAllRecordsForConnectorUntyped as unknown as (
  connectorId: string
) => Promise<unknown>;

const deleteConnectionRecordRowsSqlite = deleteConnectionRecordRowsSqliteUntyped as unknown as (
  connectorInstanceId: string | null
) => number;

// Mirrors the drift-preview shape documented in
// `./helpers/current-projection-invariant.js`'s `detectCurrentProjectionDrift`
// docstring — that helper file is frozen (test/helpers/**), so the shape is
// modeled here at the import boundary instead of retyping the helper.
interface ProjectionDriftPreview {
  connectorInstanceId: string;
  currentDeleted: boolean | null;
  currentExists: boolean;
  currentVersion: number | null;
  kind: string;
  latestHistoryDeleted: boolean | null;
  latestHistoryVersion: number | null;
  recordKey: string;
  stream: string;
}

const detectCurrentProjectionDrift = detectCurrentProjectionDriftUntyped as unknown as (scope?: {
  connectorInstanceId?: string | null;
  stream?: string;
}) => ProjectionDriftPreview[];

// `better-sqlite3`'s `.get()`/`.all()` return type is opaque (`unknown`) to a
// caller reading specific columns from an ad-hoc SELECT; each call site here
// reads exactly the column(s) it declared in its own SQL, so this asserts the
// row exists and narrows to the caller-declared shape. Same idiom as
// `spine-events-connector-instance-id-backfill.test.ts`'s `requireRow`.
...
function upsert(id: string, payload: Record<string, unknown> = {}) {
...
function ingestDelete(id: string) {
  return ingestRecord(CONNECTOR_ID, {
    data: { id },
    emitted_at: EMITTED,
    key: id,
...
function instanceIdFor(): string | null {
  ...
  const row = (getDb()
    ...
    getDb()
      ...
      .get(CONNECTOR_ID)) as { connector_instance_id: string } | undefined;
...
test("classifier matches the repair tool decision tree", () => {
  ...
  assert.equal(
    classifyProjectionMismatch({ deleted: false, jsonEqual: true, version: 3 }, { deleted: false, version: 3 }),
    null
  );
  // missing_current: live latest, no current
  assert.equal(classifyProjectionMismatch({ deleted: false, jsonEqual: false, version: 3 }, null), K.MISSING_CURRENT);
  // missing_current: live latest, current is deleted
  assert.equal(
    classifyProjectionMismatch({ deleted: false, jsonEqual: false, version: 3 }, { deleted: true, version: 3 }),
    K.MISSING_CURRENT
  );
  // stale_current: version mismatch
  assert.equal(
    classifyProjectionMismatch({ deleted: false, jsonEqual: true, version: 4 }, { deleted: false, version: 3 }),
    K.STALE_CURRENT
  );
  // stale_current: json mismatch at same version
  assert.equal(
    classifyProjectionMismatch({ deleted: false, jsonEqual: false, version: 3 }, { deleted: false, version: 3 }),
    K.STALE_CURRENT
  );
  // latest_deleted: deleted latest, live current
  assert.equal(
    classifyProjectionMismatch({ deleted: true, jsonEqual: false, version: 5 }, { deleted: false, version: 4 }),
    K.LATEST_DELETED
  );
  // deleted latest + deleted current = consistent
  assert.equal(
    classifyProjectionMismatch({ deleted: true, jsonEqual: false, version: 5 }, { deleted: true, version: 5 }),
    null
  );
  // unresolved_pruned: current newer than all retained history
  assert.equal(
    classifyProjectionMismatch({ deleted: false, jsonEqual: false, version: 2 }, { deleted: false, version: 9 }),
    K.UNRESOLVED_PRUNED
  );
});
...
test("pruning preserves a cold-key anchor while a hot key advances the stream past the limit", async () => {
  ...
  try {
    ...
    const counts = getDb()
      ...
      .all(cin, STREAM) as Array<{ record_key: string; n: number }>;
    const byKey: Record<string, number> = Object.fromEntries(counts.map((r) => [r.record_key, Number(r.n)]));
    assert.equal(byKey.cold, 1, "cold key retains exactly its anchor row");
    const hotCount = byKey.hot;
    ...
    assert.ok(hotCount !== undefined);
    assert.ok(hotCount <= 2, `hot key history stays bounded (got ${hotCount})`);
  } finally {
    teardown();
  }
});
...
test("a cold key DELETED after the stream advances keeps its deleted anchor (no resurrection)", async () => {
  ...
  try {
    ...
    const anchor = getDb()
      ...
      .all(cin, STREAM) as Array<{ version: number; deleted: number }>;
...
test("many cold keys all keep their anchors when one hot key advances the stream", async () => {
  ...
  try {
    ...
    const total = requireRow<{ n: number }>(
      getDb()
        .prepare("SELECT COUNT(*) AS n FROM record_changes WHERE connector_instance_id = ? AND stream = ?")
        .get(cin, STREAM)
    );
...
test("a torn bulk delete (record_changes cleared, records left) is caught as unresolved_pruned", async () => {
  ...
  try {
    ...
    getDb().prepare("DELETE FROM record_changes WHERE connector_instance_id = ? AND stream = ?").run(cin, STREAM);
...
function countHistoryRows(cin: string | null, recordKey: string): number {
  const row = requireRow<{ c: number }>(
    getDb()
      .prepare(
...
function readVersionCounter(cin: string | null): number | null {
  const row = getDb()
    .prepare("SELECT max_version FROM version_counter WHERE connector_instance_id = ? AND stream = ?")
    .get(cin, STREAM) as { max_version: number } | undefined;
  return row ? row.max_version : null;
}

function readCurrentVersion(cin: string | null, recordKey: string): number | null {
  const row = getDb()
    .prepare("SELECT version FROM records WHERE connector_instance_id = ? AND stream = ? AND record_key = ?")
    .get(cin, STREAM, recordKey) as { version: number } | undefined;
...
function deleteAnchor(cin: string | null, recordKey: string) {
  getDb()
    .prepare(
...
async function strandColdAnchor(hotWrites: number) {
  await upsert("cold", { v: 1 });
...
test("unchanged reingest of an unanchored current row recreates its anchor (self-heal)", async () => {
  ...
  try {
    ...
    assert.ok(counterBefore !== null, "version_counter must exist before the heal");
