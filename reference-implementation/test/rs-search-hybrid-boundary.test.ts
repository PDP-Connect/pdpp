const TOP_LEVEL_REGEX_1 = /\bfrom\s*['"][^'"]*\/server\/search['"]/;
const TOP_LEVEL_REGEX_2 = /\bfrom\s*['"][^'"]*\/server\/search-semantic['"]/;
const TOP_LEVEL_REGEX_3 = /\bfrom\s*['"][^'"]*\/server\/search-hybrid['"]/;
...
test("rs.search.hybrid operation does not import server/search-hybrid.js", () => {
  ...
  const src = read(OP_REL);
  const fromPattern = TOP_LEVEL_REGEX_3;
  assert.equal(
    fromPattern.test(src),
    false,
