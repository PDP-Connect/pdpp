...
function invalidWith(code: string, messageSubstring?: string) {
  return (err: unknown) => {
    ...
    if (messageSubstring !== undefined) {
      assert.ok(
        ...
        `message should include ${JSON.stringify(messageSubstring)}, got ${JSON.stringify(err.message)}`
      );
    }
    return true;
  };
...
test("resolveListOrder: canonical sort wins; legacy order applies only when sort is absent", () => {
  ...
  assert.equal(resolveListOrder(null, { direction: "ASC", field: "x" }), "ASC");
...
test("resolveListOrder: agreeing sort+order is accepted; disagreement is rejected", () => {
  assert.equal(resolveListOrder("desc", { direction: "DESC", field: "x" }), "DESC");
  assert.throws(
    () => resolveListOrder("asc", { direction: "DESC", field: "x" }),
    (err: unknown) => {
      assert.ok(isQueryError(err), `expected an Error, got ${JSON.stringify(err)}`);
...
test("validateCountKind accepts the canonical vocabulary and rejects others", () => {
  ...
  assert.throws(
    () => validateCountKind("fuzzy"),
    invalidWith("invalid_request", "count must be one of: none, estimated, exact")
  );
});
...
test("validateWindowKind accepts none/exact and rejects others", () => {
  ...
  assert.throws(
    ...
    invalidWith("invalid_request", "window must be one of: none, exact")
  );
});
