...
function freshHealthySnapshot(): ConnectionHealthSnapshot {
  return {
    axes: {
      attention: "none",
      coverage: "complete",
...
function staleManualSnapshot(): ConnectionHealthSnapshot {
  return {
...
function degradedRetryableSnapshot(): ConnectionHealthSnapshot {
  return {
    ...freshHealthySnapshot(),
    axes: {
      attention: "none",
...
function collectionReport(overrides: Partial<CollectionReportEntryLike> = {}): CollectionReportEntryLike[] {
  return [
    {
      collected: 100,
      considered: 100,
...
function manifestStreams(): ManifestStreamLike[] {
...
test("owner-wire: runtime_ok=false caps channel at calm regardless of snapshot state", () => {
  ...
  const needsAttentionSnapshot: ConnectionHealthSnapshot = {
    ...freshHealthySnapshot(),
    axes: {
      attention: "open",
      coverage: "retryable_gap",
...
test("owner-wire: per-stream rollup is present on the verdict streams array", () => {
  const verdict = synthesizeConnectorVerdict({
    ...
    refresh: null,
    report: [
      { collected: 50, considered: 100, coverage_condition: "partial", pending_detail_gaps: 0, stream: "transactions" },
      { collected: 10, considered: 10, coverage_condition: "complete", pending_detail_gaps: 0, stream: "accounts" },
    ],
    snapshot: freshHealthySnapshot(),
  });
...
test("owner-wire: synthesizer clamps collected to considered (impossible 3/2 cannot escape)", () => {
  ...
  const collected = txStream?.collected ?? 0;
  const considered = txStream?.considered ?? Number.POSITIVE_INFINITY;
  assert.ok(collected <= considered, `collected (${collected}) must be <= considered (${considered})`);
});
