...
function spineEventRow(eventId: string, eventSeq: number | null, status: string): SpineEventRow {
  return {
    actor_id: "test-actor",
    actor_type: "runtime",
    client_id: null,
...
test("SQLite boot creates source/run summary index for spine aggregation", () => {
  ...
  try {
    ...
    const row = getDb()
      ...
      .get<{ sql: string }>("idx_spine_events_source_run_summary");

    assert.ok(row, "idx_spine_events_source_run_summary exists");
    // biome-ignore lint/performance/useTopLevelRegex: localized test assertion preserves its explicit contract.
    assert.match(row.sql, /ON spine_events\(source_kind, source_id, run_id, occurred_at DESC\)/);
    // biome-ignore lint/performance/useTopLevelRegex: localized test assertion preserves its explicit contract.
...
test("spine_events source/run summary index DDL is store-parity", () => {
  const sqliteDdl = readFileSync(new URL("../server/db.ts", import.meta.url), "utf8");
  const postgresDdl = readFileSync(new URL("../server/postgres-storage.ts", import.meta.url), "utf8");
...
test("mergeEventRows keeps distinct rows that share a null/duplicate event_seq, deduping by event_id instead", () => {
  ...
  const rows = [
    spineEventRow("evt-a", null, "in_progress"),
    spineEventRow("evt-b", null, "failed"),
    spineEventRow("evt-c", 5, "succeeded"),
  ];
...
test("mergeEventRows deterministically orders rows that share a null/duplicate event_seq by event_id", () => {
  const merged = mergeEventRows([spineEventRow("evt-z", null, "a"), spineEventRow("evt-a", null, "b")]);
  assert.deepEqual(
...
test("mergeEventRows sorts null-event_seq rows after every real numbered row, regardless of event_id", () => {
  ...
  const merged = mergeEventRows([spineEventRow("evt-aaa-null", null, "a"), spineEventRow("evt-real", 1, "b")]);
  assert.deepEqual(
    merged.map((row) => row.event_id),
...
if (POSTGRES_URL) {
  ...
  test("unfiltered Postgres summaries derive source for legacy NULL-source rows", async () => {
    await withTempDb(async (url) => {
      ...
      await insertEvent(pool, {
        ...
        actorType: "system",
        dataJson: { source: { id: "gmail", kind: "connector" } },
  ...
  test("unfiltered Postgres summary first pages return recent distinct correlations", async () => {
    await withTempDb(async (url) => {
      ...
      assert.deepEqual(
        page.summaries.map((summary) => summary.trace_id),
        ["trace_a", "trace_b"]
      );
      assert.equal(page.hasMore, true);
      const [firstSummary] = page.summaries;
      assert.ok(firstSummary, "the page must contain its first trace summary");
      assert.equal(firstSummary.event_count, 120);
    });
  });
  ...
  test("batched summary hydration orders each correlation by event_seq, not insertion/return order", async () => {
    ...
    await withTempDb(async (url) => {
      ...
      async function insertOrderedEvent({
        eventId,
        traceId,
        eventType,
        status,
      ...
      const page = await postgresListSpineCorrelations("trace", { limit: 10, q: "trace_order_" });
  ...
  test("batched head/tail/terminal queries pin an explicit outer ORDER BY in their SQL text", async () => {
    ...
    await withTempDb(async (url) => {
      ...
      try {
        ...
        await postgresListSpineCorrelations("run", { limit: 10, q: "run_order_by_pin" });
      } finally {
  ...
  test("batched tail-window admission excludes null-event_seq rows under the null-last DESC policy, even at scale where every row cannot fit in the head window", async () => {
    ...
    await withTempDb(async (url) => {
      ...
      const page = await postgresListSpineCorrelations("run", { limit: 10, q: runId });
