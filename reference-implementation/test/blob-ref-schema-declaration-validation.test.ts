...
function validBlobRefSchema(overrides: Record<string, unknown> = {}) {
  return { properties: validBlobRefProperties(), required: ["blob_id"], type: "object", ...overrides };
}

function isManifestError(err: unknown): err is Error & { code: string } {
  return err instanceof Error && "code" in err;
}

function assertRejects(fieldSchema: unknown, messagePart: string) {
  assert.throws(
    () => validateBlobRefSchemaDeclaration(STREAM, fieldSchema, CODE),
    (err: unknown) => {
      assert.ok(isManifestError(err));
...
test("validateBlobRefSchemaDeclaration: rejects a missing or non-object properties", () => {
  ...
  assertRejects({ properties: "x", type: "object" }, "blob_ref must declare object properties");
  assertRejects({ properties: [], type: "object" }, "blob_ref must declare object properties");
});

test("validateBlobRefSchemaDeclaration: rejects a missing required property", () => {
  const { size_bytes: _sizeBytes, ...props } = validBlobRefProperties();
  assertRejects(
    { properties: props, required: ["blob_id"], type: "object" },
    "blob_ref.size_bytes must be type integer"
  );
});
...
test("validateBlobRefSchemaDeclaration: rejects a property with the wrong type", () => {
  ...
  assertRejects({ properties: props2, required: ["blob_id"], type: "object" }, "blob_ref.blob_id must be type string");
});
