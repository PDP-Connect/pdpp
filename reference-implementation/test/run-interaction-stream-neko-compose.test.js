...
test('n.eko compose overlay uses service DNS instead of reference network namespace', async () => {
  ...
  assert.match(overlay, /PDPP_NEKO_WINDOW_SETTLE_URL:\s*\$\{PDPP_NEKO_WINDOW_SETTLE_URL:-http:\/\/neko:9223\/pdpp\/window-settle\}/);
  ...
  assert.match(envExample, /PDPP_NEKO_WINDOW_SETTLE_URL=http:\/\/neko:9223\/pdpp\/window-settle/);
...
test('static and dynamic n.eko paths expose DPR-1 portrait and landscape phone modes', async () => {
  ...
  assert.match(overlay, /neko:\s*[\s\S]*image: \$\{NEKO_IMAGE:-pdpp-neko:local\}/);
  assert.match(overlay, /neko-allocator:\s*[\s\S]*NEKO_IMAGE: \$\{NEKO_IMAGE:-pdpp-neko:local\}/);
  assert.match(dockerfile, /COPY docker\/neko\/xorg\.conf \/etc\/neko\/xorg\.conf/);
...
test('Chromium defaults its launch size to the active n.eko screen', async () => {
  ...
  assert.match(startScript, /SCREEN="\$\{NEKO_DESKTOP_SCREEN:-1440x900@30\}"/);
  assert.match(startScript, /WIDTH="\$\{SCREEN_WIDTH\}"/);
  assert.match(startScript, /HEIGHT="\$\{SCREEN_HEIGHT\}"/);
  assert.match(startScript, /xdotool windowsize --sync/);
  assert.match(startScript, /xwininfo -root -display/);
});
