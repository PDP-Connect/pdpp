...
import { mapSearchFanout, resolveSearchFanoutConcurrency } from "../server/search-fanout.ts";

test("search fanout is unbounded for non-Postgres unless configured", () => {
  assert.equal(resolveSearchFanoutConcurrency({ env: {}, isPostgres: false }), Number.POSITIVE_INFINITY);
});

test("search fanout defaults to a bounded Postgres concurrency", () => {
  assert.equal(resolveSearchFanoutConcurrency({ env: {}, isPostgres: true }), 8);
});
