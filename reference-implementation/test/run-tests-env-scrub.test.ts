...
import { buildScrubbedTestEnv, TEST_ENV_DENYLIST } from "../scripts/test-env.ts";
...
async function runNodeModule(script: string, env: NodeJS.ProcessEnv): Promise<string> {
...
describe("run-tests env scrub", () => {
  it("removes owner-auth vars exported by the parent shell", () => {
    ...
    for (const key of TEST_ENV_DENYLIST) {
      assert.equal(Object.hasOwn(scrubbed, key), false, `${key} must not leak into the test-worker env`);
    }
  ...
  it("scrubbed env lets a child server bind away from the shared AS/RS dev ports", async () => {
    ...
    const serverModuleUrl = new URL("../server/index.ts", import.meta.url).href;
  ...
  it("lists the owner-auth vars that the harness must scrub", () => {
    ...
    assert.deepEqual([...TEST_ENV_DENYLIST].sort(), [
