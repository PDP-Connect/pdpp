...
/**
 * `server/index.js` is unchecked JS (allowJs, checkJs:false); boundary-cast
...
interface FieldCapabilityFlag {
  declared: boolean;
  usable: boolean;
}
interface FieldCapability {
  exact_filter: FieldCapabilityFlag;
  granted: boolean;
  lexical_search: FieldCapabilityFlag;
  schema: Record<string, unknown>;
  type?: string;
}
type FieldCapabilities = Record<string, FieldCapability>;
...
const PILOT: Record<string, PilotSpec> = {
  chase: {
    expectedKind: "money",
  ...
  gmail: {
    ...
    expectedTypes: { date: "timestamp", from_name: "person", snippet: "text", subject: "text" },
    manifest: "gmail.json",
    stream: "messages",
...
function classifyByDeclaredTypes(typesByField: Record<string, string>): string | null {
  let hasMoney = false;
  let hasPerson = false;
  let hasText = false;
...
async function withHttpHarness(
  ...
  fn: (ctx: { asUrl: string; rsUrl: string; connectorId: string }) => Promise<void>
): Promise<void> {
...
async function readFieldCapabilities(
  rsUrl: string,
  token: string,
  connectorId: string,
  stream: string
): Promise<FieldCapabilities> {
  ...
  const streamMetadata = body as StreamMetadataBody;
  assert.equal(streamMetadata.object, "stream_metadata");
  assert.ok(streamMetadata.field_capabilities, "field_capabilities present");
  return streamMetadata.field_capabilities;
}
...
for (const [name, spec] of Object.entries(PILOT)) {
  test(`${name}: real manifest surfaces declared presentation types on ${spec.stream}`, async () => {
    ...
    await withHttpHarness(manifest, async ({ asUrl, rsUrl, connectorId }) => {
      ...
      for (const [field, expected] of Object.entries(spec.expectedTypes)) {
        const cap = fc[field];
        assert.ok(cap, `${name}/${spec.stream}.${field} present in field_capabilities`);
        assert.equal(cap.type, expected, `${name}/${spec.stream}.${field} declared type should be '${expected}'`);
      }
    });
  });
  ...
  test(`${name}: a non-pilot field omits the type key (honest absence)`, async () => {
    ...
    await withHttpHarness(manifest, async ({ asUrl, rsUrl, connectorId }) => {
      const ownerToken = await issueOwnerToken(asUrl);
      const fc = await readFieldCapabilities(rsUrl, ownerToken, connectorId, spec.stream);
      const untypedCap = fc[spec.untypedField];
      assert.ok(untypedCap, `${spec.untypedField} present`);
      assert.equal(
        Object.hasOwn(untypedCap, "type"),
        false,
  ...
  test(`${name}: declared type rides alongside unchanged capability flags`, async () => {
    ...
    await withHttpHarness(manifest, async ({ asUrl, rsUrl, connectorId }) => {
      ...
      for (const field of Object.keys(spec.expectedTypes)) {
        const cap = fc[field];
        assert.ok(cap, `${field} present in field_capabilities`);
        assert.equal(typeof cap.granted, "boolean", `${field}.granted is a boolean`);
        assert.ok(
          cap.exact_filter && typeof cap.exact_filter.declared === "boolean",
          `${field}.exact_filter is well-formed`
        );
        assert.ok(
          cap.lexical_search && typeof cap.lexical_search.declared === "boolean",
