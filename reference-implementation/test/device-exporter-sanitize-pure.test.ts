...
test("sanitizeDeviceExporterDiagnostic: sensitive keys are redacted (recursively)", () => {
  const out = asRecord(
    sanitizeDeviceExporterDiagnostic({
      ...
      cookie: "c",
      nested: { harmless: 42, token: "nested-tok" },
      otp: "000000",
      password: "p",
      safe_field: "keep-me",
      secret: "s",
  ...
  assert.equal(nested.token, "[REDACTED]", "nested sensitive key redacted");
  assert.equal(nested.harmless, 42, "nested non-sensitive scalar preserved");
});
...
test("sanitizeDeviceExporterDiagnostic: arrays are truncated to 20 items", () => {
  const out = asArray(sanitizeDeviceExporterDiagnostic(Array.from({ length: 25 }, (_, i) => i)));
...
test("sanitizeDeviceExporterDiagnostic: depth is capped at 4 with [REDACTED_DEPTH]", () => {
  const out = asRecord(sanitizeDeviceExporterDiagnostic({ a: { b: { c: { d: { e: "too deep" } } } } }));
  // a(1) -> b(2) -> c(3) -> d(4) hits the cap and is replaced.
  const a = asRecord(out.a);
  const b = asRecord(a.b);
  const c = asRecord(b.c);
  assert.equal(c.d, "[REDACTED_DEPTH]", "depth-4 nesting is redacted");
});

test("sanitizeDeviceExporterDiagnostic: unix home/user/root paths are redacted in strings", () => {
  const out = asString(sanitizeDeviceExporterDiagnostic("open failed at /home/tim/.ssh/id_rsa please check"));
...
test("sanitizeDeviceExporterDiagnostic: a redacted key wins even when its value is an object", () => {
  const out = asRecord(sanitizeDeviceExporterDiagnostic({ token: { nested: "still-secret" } }));
...
test("sanitizeLocalCollectorGapDetails: short text passes through, whitespace collapsed", () => {
  ...
  assert.equal(
    sanitizeLocalCollectorGapDetails("multi   space\n\ttext"),
    "multi space text",
...
test("sanitizeLocalCollectorGapDetails: long text is capped at 300 chars with an ellipsis", () => {
  ...
  const out = mustExist(sanitizeLocalCollectorGapDetails(long), "non-blank input yields a string");
  assert.equal(out.length, 300, "output capped at 300 chars");
  assert.equal(out.charCodeAt(out.length - 1), 0x20_26, "terminated with a horizontal-ellipsis (…)");
});
...
test("sanitizeLocalCollectorGapDetails: text at/under 300 chars is NOT ellipsized", () => {
  ...
  assert.notEqual(out.charCodeAt(out.length - 1), 0x20_26, "no ellipsis when under the cap");
});
