...
test("round-trips a URL-shaped connector id without delimiter collapse", () => {
  ...
  assert.deepEqual(parsed, { connectionId: null, connectorId: urlShaped });
});
...
test("round-trips a (connector, connection) tuple with URL-shaped connector id", () => {
  ...
  const parsed = parseHostedMcpSelection(encoded);
  assert.deepEqual(parsed, { connectionId, connectorId: urlShaped });
});
...
test("accepts both string and array form values, dedupes by content", () => {
  const a = encodeHostedMcpSelection({ connectionId: null, connectorId: "slack" });
  const b = encodeHostedMcpSelection({ connectionId: "conn_a", connectorId: "gmail" });

  assert.deepEqual(parseHostedMcpSelections(a), [{ connectionId: null, connectorId: "slack" }]);

  assert.deepEqual(parseHostedMcpSelections([a, b, a]), [
    { connectionId: null, connectorId: "slack" },
    { connectionId: "conn_a", connectorId: "gmail" },
  ]);
});
...
test("dedupes two distinct encodings of the same tuple", () => {
  ...
  const canonical = encodeHostedMcpSelection({ connectionId: "conn_a", connectorId: "gmail" });
  const reordered = Buffer.from('{"connector_id":"gmail","connection_id":"conn_a"}', "utf8").toString("base64url");
  ...
  assert.deepEqual(parsed, [{ connectionId: "conn_a", connectorId: "gmail" }]);
});

test("drops empty, whitespace, non-string, and structurally invalid entries", () => {
  const valid = encodeHostedMcpSelection({ connectionId: null, connectorId: "gmail" });
  ...
  assert.deepEqual(parsed, [{ connectionId: null, connectorId: "gmail" }]);
});

test("blank connection_id is normalized to null", () => {
  const payload = Buffer.from(JSON.stringify({ connection_id: "   ", connector_id: "gmail" }), "utf8").toString(
    "base64url"
  );
...
test("encoder rejects inputs without a usable connectorId", () => {
  // @ts-expect-error testing runtime rejection of a null input
  assert.throws(() => encodeHostedMcpSelection(null), TypeError);
  // @ts-expect-error testing runtime rejection of a missing connectorId
  assert.throws(() => encodeHostedMcpSelection({}), TypeError);
  assert.throws(() => encodeHostedMcpSelection({ connectorId: "" }), TypeError);
  assert.throws(() => encodeHostedMcpSelection({ connectorId: "   " }), TypeError);
  // @ts-expect-error testing runtime rejection of a non-string connectorId
...
test("stream selection encoder rejects inputs missing a stream or connector", () => {
  ...
  // @ts-expect-error testing runtime rejection of a missing connectorId/streamName
  assert.throws(() => encodeHostedMcpStreamSelection({}), TypeError);
  assert.throws(
    // @ts-expect-error testing runtime rejection of a missing streamName
    () => encodeHostedMcpStreamSelection({ connectorId: "gmail" }),
    TypeError
  );
  assert.throws(() => encodeHostedMcpStreamSelection({ connectorId: "gmail", streamName: "   " }), TypeError);
  assert.throws(() => encodeHostedMcpStreamSelection({ connectorId: "", streamName: "messages" }), TypeError);
});
...
test("stream selections parse into entries and a per-source set keyed by hostedMcpSourceKey", () => {
  ...
  const gmailKey = hostedMcpSourceKey({ connectionId: "conn_a", connectorId: "gmail" });
  const slackKey = hostedMcpSourceKey({ connectionId: null, connectorId: "slack" });
  assert.deepEqual([...mustExist(bySource.get(gmailKey), "gmail source entry must exist")].sort(), [
    "messages",
    "threads",
  ]);
  assert.deepEqual([...mustExist(bySource.get(slackKey), "slack source entry must exist")], ["channels"]);
});
...
test("hostedMcpSourceKey matches the dedupe key parseHostedMcpStreamSelections uses internally", () => {
  ...
  const lookup = hostedMcpSourceKey({ connectionId: "conn_a", connectorId: "gmail" });
  assert.ok(bySource.has(lookup), "source key matches the parsed grouping key");
  assert.notDeepEqual(
    bySource.get(lookup),
    bySource.get(hostedMcpSourceKey({ connectionId: null, connectorId: "gmail" })),
...
test("buildHostedMcpAuthorizationDetailForConnector treats blank/whitespace connectionId as unpinned", () => {
  assert.deepEqual(buildHostedMcpAuthorizationDetailForConnector("gmail", ["messages"], "continuous", "   ").streams, [
    { name: "messages" },
  ]);
  assert.deepEqual(buildHostedMcpAuthorizationDetailForConnector("gmail", ["messages"], "continuous", "").streams, [
    { name: "messages" },
