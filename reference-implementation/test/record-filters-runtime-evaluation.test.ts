...
function rangeFilter(operators: Record<string, number>) {
  return [{ field: "n", fieldSchema: intSchema, kind: "range", operators }];
}
...
test("allowedCandidateRecordKeysFromRows: returns only rows passing grant AND filters", () => {
  ...
  const manifestStream = { consent_time_field: "ts" };
  const compiledFilters = [{ field: "n", fieldSchema: intSchema, kind: "range", operators: { gte: 5 } }];
  const rows = [
    { record_json: JSON.stringify({ n: 5, ts: "2021-01-01T00:00:00Z" }), record_key: "keep-1" },
    { record_json: JSON.stringify({ n: 100, ts: "2021-01-01T00:00:00Z" }), record_key: "keep-2" },
    { record_json: JSON.stringify({ n: 4, ts: "2021-01-01T00:00:00Z" }), record_key: "filtered" }, // fails gte:5
    { record_json: JSON.stringify({ n: 9, ts: "2021-01-01T00:00:00Z" }), record_key: "not-granted" }, // not in resources
    { record_json: "{not valid json", record_key: "bad-json" }, // parse failure -> skipped
    { record_json: null, record_key: "null-json" }, // null data -> passesGrant on null still ok, but filter n undefined -> skipped
  ];
  const allowed = allowedCandidateRecordKeysFromRows(rows, { compiledFilters, manifestStream, streamGrant });
  assert.deepEqual(allowed, ["keep-1", "keep-2"]);
});

test("allowedCandidateRecordKeysFromRows: malformed JSON is skipped, not thrown", () => {
  const rows = [{ record_json: "definitely {{ not json", record_key: "x" }];
