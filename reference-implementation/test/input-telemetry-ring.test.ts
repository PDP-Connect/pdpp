...
test("push: assigns a monotonic seq starting at 1 and stamps serverAtMs", () => {
  ...
  assert.ok(a, "push of a valid record returns the stamped record");
  assert.ok(b, "push of a valid record returns the stamped record");
...
test("push: ignores falsy session id or non-object record (never throws)", () => {
  ...
  // @ts-expect-error deliberately non-object record — proves the runtime guard
  // (isStorableRecord) rejects it even though the declared param type is narrower.
  assert.equal(t.push("sess", null), undefined, "null record ignored");
  // @ts-expect-error deliberately non-object record — same guard, string case.
  ...
  assert.deepEqual(t.readSince("sess", 0), { records: [], seq: 0 });
});
...
test("push: seq is per-session (independent counters)", () => {
  ...
  assert.ok(b1, "push of a valid record returns the stamped record");
...
test("push: ring evicts oldest when the buffer exceeds bufferSize; seq keeps climbing", () => {
  ...
  assert.deepEqual(
    records.map((r) => r.i),
    [3, 4, 5],
    "oldest (1,2) evicted, newest kept in order"
  );
  assert.deepEqual(
    records.map((r) => r.seq),
    [3, 4, 5],
    "retained records keep their original seq"
  );
});
...
test("readSince: returns only records with seq STRICTLY greater than `since`", () => {
  ...
  assert.deepEqual(
    afterTwo.records.map((r) => r.seq),
    [3, 4],
    "since=2 excludes seq 2 (strict >)"
  );
...
test("readSince: unknown session yields an empty, zero-seq window", () => {
  const t = createInputTelemetry();
  assert.deepEqual(t.readSince("never", 0), { records: [], seq: 0 });
});
...
test("drop: forgets a session buffer (subsequent read is empty)", () => {
  ...
  t.drop("sess");
  assert.deepEqual(t.readSince("sess", 0), { records: [], seq: 0 }, "dropped session reads empty");
});
