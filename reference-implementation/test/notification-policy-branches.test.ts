 ...
 *
 * The existing `notification-policy.test.ts` proves the headline paths
...
const DAY = { enabled: true, end: "17:00", start: "09:00", timeZone: "UTC" };
...
test("isWithinQuietWindow is false when the window is null or disabled", () => {
  ...
  // Destructuring default (quietWindow = null) applies identically whether
  // the property is omitted or explicitly undefined, so omitting it here
  // exercises the same runtime branch as an explicit `quietWindow: undefined`
  // would, without fighting exactOptionalPropertyTypes for no behavioral gain.
  assert.equal(isWithinQuietWindow({ now: at("2026-05-16T05:00:00Z") }), false);
  assert.equal(
    isWithinQuietWindow({
      now: at("2026-05-16T05:00:00Z"),
      quietWindow: { enabled: false, end: "07:00", start: "22:00" },
    }),
    false
  );
});
...
test("isWithinQuietWindow returns false when start === end (empty window)", () => {
  assert.equal(
    isWithinQuietWindow({
      now: at("2026-05-16T08:00:00Z"),
      quietWindow: { enabled: true, end: "08:00", start: "08:00" },
    }),
    false
  );
});
...
test("isWithinQuietWindow rejects malformed clock strings (parseClockMinutes)", () => {
  for (const bad of ["24:00", "12:60", "7:00", "0700", "", "noon"]) {
    assert.equal(
      isWithinQuietWindow({
        now: at("2026-05-16T05:00:00Z"),
        quietWindow: { enabled: true, end: "07:00", start: bad },
      }),
      false,
...
test("isWithinQuietWindow evaluates the clock in the window timeZone", () => {
  ...
  assert.equal(
    isWithinQuietWindow({
      ...
      quietWindow: { enabled: true, end: "07:00", start: "22:00", timeZone: "America/New_York" },
    }),
    false
  );
});
...
test("classifyAssistanceNotification drops to informational for each missing condition", () => {
  ...
  assert.equal(
    classifyAssistanceNotification({
      ...
      response_contract: "acknowledged",
    }),
    NOTIFICATION_TIERS.INFORMATIONAL
  );
});
...
test("classifyRunEventNotification classifies both assistance event types", () => {
  ...
  assert.equal(
    classifyRunEventNotification({ data, event_type: "run.interaction_required" }),
    NOTIFICATION_TIERS.ACTION_REQUIRED
  );
  assert.equal(
    classifyRunEventNotification({ data, event_type: "run.assistance_requested" }),
    NOTIFICATION_TIERS.ACTION_REQUIRED
  );
});
...
test("classifyRunEventNotification is informational for non-assistance events", () => {
  assert.equal(classifyRunEventNotification({ event_type: "run.completed" }), NOTIFICATION_TIERS.INFORMATIONAL);
...
test("shouldFanoutRenderedVerdict rejects non-object and nullish verdicts", () => {
  ...
  // @ts-expect-error deliberately non-object verdict — proves the runtime
  // `typeof verdict !== "object"` guard rejects it even though the declared
  // param type is narrower.
  assert.equal(shouldFanoutRenderedVerdict("attention"), false);
  // @ts-expect-error deliberately non-object verdict, same guard, number case.
  assert.equal(shouldFanoutRenderedVerdict(42), false);
});

test("shouldFanoutRenderedVerdict rejects a non-array required_actions", () => {
  // @ts-expect-error deliberately non-array required_actions — proves the
  // runtime Array.isArray(...) guard rejects it even though the declared
  // param type is narrower.
...
test("shouldFanoutRenderedVerdict rejects a primary action missing satisfied_when", () => {
  assert.equal(shouldFanoutRenderedVerdict({ channel: "attention", required_actions: [{ audience: "owner" }] }), false);
});
