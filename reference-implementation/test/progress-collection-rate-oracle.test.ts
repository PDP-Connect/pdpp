...
const validCollectionRate: Record<string, unknown> = {
...
function expectInvalidCollectionRate(value: unknown, fieldName: string) {
  assert.throws(
...
test("validateProgressCollectionRate: last_backoff must be null or a bounded supported reason", () => {
  ...
  expectInvalidCollectionRate(
    { ...validCollectionRate, last_backoff: { at_interval_ms: -1, reason: "retry_after" } },
    "at_interval_ms"
  );
  expectInvalidCollectionRate(
    { ...validCollectionRate, last_backoff: { at_interval_ms: 1000, reason: "backpressure" } },
    "reason"
  );
});
