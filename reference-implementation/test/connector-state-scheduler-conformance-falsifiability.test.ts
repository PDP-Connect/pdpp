...
import { createBrokenInMemoryConnectorStateSchedulerDriver } from "./helpers/broken-connector-state-scheduler-driver.ts";
import { runConnectorStateSchedulerConformance } from "./helpers/connector-state-scheduler-conformance.ts";

interface Scenario {
  fn: () => Promise<void>;
  name: string;
}
const STATE_FAILURE = /grant-scoped state is isolated|owner-scoped state for connector A is isolated/;
...
test("harness detects at least one invariant violation in each broken-driver area", async () => {
  ...
  // Sequential execution is intentional: scenarios share the deliberately broken driver state.
  for (const scenario of scenarios) {
    try {
      // biome-ignore lint/performance/noAwaitInLoops: preserve the conformance harness execution order.
      await scenario.fn();
...
// biome-ignore format: localized test assertion preserves its explicit contract.
