...
import { recordIndexWorkStatsForTests, withRecordIndexWorkForTests } from "../server/records.ts";
...
test("record index admission times out only its waiter and transfers one FIFO permit", async () => {
  ...
  try {
    ...
    if (previous.limit === undefined) {
      delete process.env.PDPP_INGEST_INDEX_WORK_LIMIT;
    } else {
      process.env.PDPP_INGEST_INDEX_WORK_LIMIT = previous.limit;
    }
    if (previous.queue === undefined) {
      delete process.env.PDPP_INGEST_INDEX_WORK_QUEUE_LIMIT;
    } else {
      process.env.PDPP_INGEST_INDEX_WORK_QUEUE_LIMIT = previous.queue;
    }
    if (previous.deadline === undefined) {
      delete process.env.PDPP_INGEST_INDEX_WORK_ACQUIRE_DEADLINE_MS;
    } else {
      process.env.PDPP_INGEST_INDEX_WORK_ACQUIRE_DEADLINE_MS = previous.deadline;
    }
  }
});
