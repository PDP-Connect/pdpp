...
function compose(inventoryRows, summaries, overrides = {}) {
...
function ownerAction(kind, satisfiedWhen, overrides = {}) {
  return {
    affects: [],
    audience: "owner",
    cta: "Take action",
...
test("ChatGPT owner action, USAA recovery gap, Chase code fix, and Slack policy stay distinct from coverage pass", () => {
  const chatgptA = summary("chatgpt-a", {
    rendered_verdict: { channel: "attention", required_actions: [{ audience: "owner", satisfied_when: { kind: "attention_resolved" } }] },
  });
  const chatgptB = summary("chatgpt-b", {
    rendered_verdict: { channel: "attention", required_actions: [{ audience: "owner", satisfied_when: { kind: "attention_resolved" } }] },
  });
  const usaa = summary("usaa-a", {
    connection_health: { ...summary("x").connection_health, axes: { attention: "none", coverage: "terminal_gap", freshness: "fresh", outbox: "idle", remote_surface: "none" }, forward_disposition: "terminal", state: "degraded" },
  });
  ...
  const slack = summary("slack-a", {
    ...
    refresh_policy: { recommended_mode: "paused", background_safe: false },
    schedule: { enabled: false },
  });
  const result = compose([chatgptA, chatgptB, usaa, chase, slack].map((item) => inventory(item.connection_id)), [chatgptA, chatgptB, usaa, chase, slack]);
  ...
  assert.deepEqual(result.dimensions.attention.needs_owner.map((item) => item.connection_id), ["chatgpt-a", "chatgpt-b"]);
  assert.deepEqual(result.dimensions.recovery.terminal.map((item) => item.connection_id), ["usaa-a"]);
  assert.deepEqual(result.dimensions.system.degraded_or_broken.map((item) => item.connection_id), ["usaa-a", "chase-a"]);
  assert.deepEqual(result.dimensions.intentional_policy.paused.map((item) => item.connection_id), ["slack-a"]);
});
...
test("the existing coverage audit can pass while an owner-action fleet is unhealthy", () => {
  const chatgpt = summary("chatgpt-a", {
    ...
    streams: ["messages"],
    stream_records: [{ stream: "messages", record_count: 1, last_updated: null }],
    collection_report: [
      {
        stream: "messages",
        required: true,
...
test("runtime outage and stream-audit failure independently make a fleet unhealthy", () => {
  ...
  const unavailableBinding = summary("binding-a", {
    connection_health: {
      ...
      conditions: [{ current: true, type: "RuntimeAvailable", status: "false" }],
    },
  });
...
test("draft and revoked inventory remain explicit while an unassessed connection prevents a health claim", () => {
  ...
  const result = compose(
    [inventory("active-a"), inventory("draft-a", { status: "draft" }), inventory("revoked-a", { status: "revoked", revokedAt: "2026-07-22T00:00:00.000Z" }), inventory("missing-a")],
    [active]
  );
  assert.equal(result.state, "indeterminate");
  assert.deepEqual(result.scope.setup_pending.map((item) => item.connection_id), ["draft-a"]);
  assert.deepEqual(result.scope.intentional_exclusions.map((item) => item.connection_id), ["revoked-a"]);
  assert.deepEqual(result.scope.unassessed.map((item) => item.connection_id), ["missing-a"]);
});
...
test("active and unknown work are indeterminate, while fresh manual and paused policy are healthy", () => {
  const active = summary("active-a", { owner_state: { resolver: "collecting" }, connection_health: { ...summary("x").connection_health, badges: { syncing: true } } });
  const unknown = summary("unknown-a", { connection_health: { ...summary("x").connection_health, axes: { attention: "none", coverage: "unknown", freshness: "unknown", outbox: "unknown", remote_surface: "unknown" }, state: "unknown", unknown_reasons: ["summary_missing"] } });
...
test("fresh paused schedule action is healthy, while stale manual and paused actions are advisory", () => {
  ...
  const staleManual = summary("manual-stale-a", {
    ...
    refresh_policy: { recommended_mode: "manual", background_safe: false },
    schedule: null,
  });
  ...
  assert.deepEqual(staleResult.dimensions.freshness_advisories.map((item) => item.connection_id), [
    "manual-stale-a",
    "paused-stale-a",
  ]);
  assert.deepEqual(staleResult.dimensions.intentional_policy.manual.map((item) => item.connection_id), ["manual-stale-a"]);
  assert.deepEqual(staleResult.dimensions.intentional_policy.paused.map((item) => item.connection_id), ["paused-stale-a"]);
});
...
test("real owner-required repair remains unhealthy even with the same typed satisfaction contract", () => {
  ...
  assert.equal(result.state, "unhealthy");
  assert.equal(result.fully_healthy, false);
  assert.deepEqual(result.dimensions.attention.needs_owner.map((item) => item.connection_id), ["reauth-a"]);
});
...
test("maintainer code-fix evidence and its owner-state resolver independently prevent a green fleet claim", () => {
  ...
  assert.deepEqual(codeFixResult.dimensions.system.degraded_or_broken.map((item) => item.connection_id), ["code-fix-a"]);

  const resolverResult = compose([inventory("blocked-maintainer-a")], [blockedMaintainer]);
  assert.equal(resolverResult.state, "unhealthy");
  assert.deepEqual(resolverResult.dimensions.system.degraded_or_broken.map((item) => item.connection_id), ["blocked-maintainer-a"]);
});
...
test("every closed non-green health member fails the strict fully-healthy claim", () => {
  ...
  const variants = [
    ...
    ...["blocked_maintainer", "collecting", "needs_owner", "not_measured", "refresh_due", "retired", "setup_in_progress", "system_degraded"].map(
      (resolver) => [`owner_resolver:${resolver}`, { owner_state: { resolver } }]
    ),
  ];
...
test("inventory-minus-summary and summary-minus-inventory disagreements are both unassessed", () => {
  ...
  assert.deepEqual(missingSummary.scope.unassessed.map((item) => item.connection_id), ["missing-summary-a"]);

  const extraSummary = compose([], [summary("extra-summary-a")]);
  assert.equal(extraSummary.state, "indeterminate");
  assert.deepEqual(extraSummary.scope.unassessed.map((item) => item.connection_id), ["extra-summary-a"]);
});

test("retryable recovery and stalled work each prevent fleet health", () => {
  const retryable = summary("retry-a", {
    connection_health: { ...summary("x").connection_health, axes: { attention: "none", coverage: "retryable_gap", freshness: "fresh", outbox: "idle", remote_surface: "none" }, forward_disposition: "resumable", state: "degraded" },
  });
  const stalled = summary("stalled-a", {
    connection_health: { ...summary("x").connection_health, axes: { attention: "none", coverage: "complete", freshness: "fresh", outbox: "stalled", remote_surface: "none" }, state: "degraded" },
  });
  const retried = compose([inventory("retry-a")], [retryable]);
  assert.equal(retried.state, "unhealthy");
  assert.deepEqual(retried.dimensions.recovery.retryable.map((item) => item.connection_id), ["retry-a"]);
  const stalledResult = compose([inventory("stalled-a")], [stalled]);
  assert.equal(stalledResult.state, "unhealthy");
  assert.deepEqual(stalledResult.dimensions.stalled_work.map((item) => item.connection_id), ["stalled-a"]);
});
...
test("stale manual or paused policy is advisory rather than a fleet failure", () => {
  const stale = summary("slack-a", {
    refresh_policy: { recommended_mode: "manual", background_safe: false },
    schedule: { enabled: false },
    connection_health: { ...summary("x").connection_health, axes: { attention: "none", coverage: "complete", freshness: "stale", outbox: "idle", remote_surface: "none" }, state: "idle", forward_disposition: "owner_refresh_due" },
  });
  const result = compose([inventory("slack-a")], [stale]);
  assert.equal(result.state, "healthy_with_advisories");
  assert.equal(result.fully_healthy, false);
  assert.deepEqual(result.dimensions.freshness_advisories.map((item) => item.connection_id), ["slack-a"]);
});
