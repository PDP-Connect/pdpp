...
const UNSUPPORTED_VERSION_PATTERN = /1999-01-01/;
...
async function closeStartedServer(server: StartedServer | undefined): Promise<void> {
  if (!server) {
    return;
  }
...
if (POSTGRES_URL) {
  test("PDPP-Version negotiation and the unsupported_version 400 envelope", async (t) => {
    ...
    try {
      await t.test("a mismatched version is rejected with the typed 400 envelope", async () => {
        ...
        // can self-correct.
        assert.match(body.error.message, UNSUPPORTED_VERSION_PATTERN);
  ...
  test("unsupported_version typed error (skipped: PDPP_TEST_POSTGRES_URL unset)", { skip: true }, () => undefined);
}
