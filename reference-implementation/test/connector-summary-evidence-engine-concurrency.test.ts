...
function mustRow<T>(value: T, description: string): NonNullable<T> {
  ...
  withTempDb(async () => {
    ...
    const after = mustRow(
      getDb()
        .prepare("SELECT total_records FROM connector_summary_evidence WHERE connector_instance_id = ?")
        .get("cin_concurrent"),
      "post-reconcile evidence row exists"
    );
...
test("two back-to-back reconcile passes converge to the same fresh canonical state without regressing each other", () =>
  withTempDb(async () => {
    ...
    const finalRow = mustRow(
      getDb()
        .prepare("SELECT total_records FROM connector_summary_evidence WHERE connector_instance_id = ?")
        .get("cin_two_pass"),
      "final evidence row exists"
    );
