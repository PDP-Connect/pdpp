...
test('unknown-denominator: unknown coverage is never projected complete (fresh)', () => {
  ...
  // not UPGRADE it to a `complete` disposition on the strength of collected count.
  const disposition = deriveForwardDisposition(input({ coverage: 'unknown', freshness: 'fresh' }));
  assert.notEqual(disposition, 'complete');
  assert.equal(disposition, 'resumable');
});

test('unknown-denominator: unknown coverage is never complete even with no gap and no attention', () => {
  assert.equal(deriveForwardDisposition(input({ coverage: 'unknown' })), 'resumable');
});

test('unknown-denominator: unknown coverage that is also manual-refresh stale stays resumable, not owner_refresh_due', () => {
  ...
  // to a refresh-due signal — a later run is still expected to establish coverage.
  const disposition = deriveForwardDisposition(
