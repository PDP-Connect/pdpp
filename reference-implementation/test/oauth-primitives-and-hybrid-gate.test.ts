...
const TOP_LEVEL_REGEX_3 = /^[0-9a-f]{64}$/;
const TOP_LEVEL_REGEX_4 = /^rt_[A-Za-z0-9_-]+$/;
...
test("base64UrlSha256: deterministic SHA-256 base64url, url-safe and unpadded", () => {
  ...
  assert.equal(
    base64UrlSha256("pkce-verifier-xyz"),
    createHash("sha256").update("pkce-verifier-xyz").digest("base64url")
  );
  // base64url alphabet only (no +, /, or = padding).
  assert.equal(TOP_LEVEL_REGEX_2.test(base64UrlSha256("hello")), false, "must be base64url, not standard base64");
});
...
test("hashOAuthRefreshToken: coerces non-string input via String() rather than throwing", () => {
  ...
  // @ts-expect-error deliberately non-string input (12345) — proves the
  // runtime String() coercion handles it even though the declared param
  // type is narrower (string).
  assert.equal(hashOAuthRefreshToken(12_345), createHash("sha256").update("12345").digest("base64url"));
});
...
test("generateToken: 64-char lowercase hex (32 random bytes), distinct across calls", () => {
  ...
  assert.match(token, TOP_LEVEL_REGEX_3, `token: ${token}`);
...
test("generateOAuthRefreshToken: rt_ prefix + base64url body, distinct across calls", () => {
  ...
  assert.match(rt, TOP_LEVEL_REGEX_4, `body must be base64url: ${rt}`);
  assert.equal(TOP_LEVEL_REGEX_1.test(rt.slice(3)), false, "body must not contain +, /, or = padding");
...
test("buildHybridRetrievalCapability: composes only when BOTH lexical and semantic are available", () => {
  ...
  assert.ok(both, "expected a hybrid retrieval capability");
  assert.equal(both.supported, true, "defaults have both available => supported");
  assert.ok("sources" in both, "expected the composed capability shape, not { supported: false }");
