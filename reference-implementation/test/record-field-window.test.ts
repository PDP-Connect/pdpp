...
const codeIs = (code: string) => (err: unknown) => isFieldWindowError(err) && err.code === code;
...
test("clampWindowBounds defaults offset to 0 and limit to the default", () => {
  assert.deepEqual(clampWindowBounds(), { limit: FIELD_WINDOW_DEFAULT_LIMIT, limitClamped: false, offset: 0 });
  assert.deepEqual(clampWindowBounds({}), { limit: FIELD_WINDOW_DEFAULT_LIMIT, limitClamped: false, offset: 0 });
});
...
test("normalizeWindowSelector rejects q combined with offset_chars", () => {
  assert.throws(() => normalizeWindowSelector({ offset_chars: 3, q: "x" }), codeIs("invalid_window"));
});
...
test("normalizeWindowSelector query mode derives limit from context when explicit", () => {
  const result = normalizeWindowSelector({ after_chars: 20, before_chars: 10, q: "needle" });
...
test("buildWindowEnvelope reports a complete window from offset 0 covering the whole field", () => {
  const env = buildWindowEnvelope({ limit: 4096, offset: 0, text: "hello", totalChars: 5 });
...
test("buildWindowEnvelope reports has_more and next cursor for a partial window", () => {
  ...
  const env = buildWindowEnvelope({ limit: 10, offset: 0, text: "x".repeat(10), totalChars: 100 });
...
test("buildWindowEnvelope computes previous cursor for a mid-field window", () => {
  ...
  const env = buildWindowEnvelope({ limit: 10, offset: 20, text: "x".repeat(10), totalChars: 100 });
...
test("buildWindowEnvelope clamps start/end to totalChars", () => {
  ...
  const env = buildWindowEnvelope({ limit: 10, offset: 50, text: "", totalChars: 5 });
...
test("assertReadableStringField maps error classes to the right HTTP status", () => {
  assert.throws(
    () => assertReadableStringField("body", "absent"),
    (err: unknown) => isFieldWindowError(err) && err.httpStatus === 404
  );
  assert.throws(
    () => assertReadableStringField("body", "array"),
    (err: unknown) => isFieldWindowError(err) && err.httpStatus === 422
  );
});
