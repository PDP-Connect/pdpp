...
test("buildResponseContractErrorBody: emits internal_contract_error with api_error type", () => {
  ...
  assert.equal(typeof body.error.message, "string", "message is a string");
  assert.ok(String(body.error.message).includes("getRsDiscoveryIndex"), "operation id embedded in message");
});
...
test("buildResponseContractErrorBody: message names the specific operation", () => {
  ...
  assert.ok(String(a.error.message).includes("opA"));
  assert.ok(String(b.error.message).includes("opB"));
});
...
test("applyResponseValidation: even a clearly-malformed payload passes when the op is non-canary", () => {
  const out = applyResponseValidation({ operationId: "notACanary", payload: null, status: 500 });
