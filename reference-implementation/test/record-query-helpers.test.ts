...
test("rejectListOnlyParamsForChangesFeed uses singular grammar for one param", () => {
  ...
  assert.throws(
    ...
    (e: unknown) =>
      // biome-ignore lint/performance/useTopLevelRegex: test assertion patterns remain colocated with the assertion they explain.
      isQueryError(e) && /sort is not supported with changes_since/.test(e.message) && e.code === "invalid_request"
  );
});

test("rejectListOnlyParamsForChangesFeed uses plural grammar for multiple params", () => {
  assert.throws(
    () => rejectListOnlyParamsForChangesFeed({ count: "exact", sort: "-x", window: "exact" }),
    // biome-ignore lint/performance/useTopLevelRegex: test assertion patterns remain colocated with the assertion they explain.
...
test("validateCanonicalSort maps sign prefix to direction on the cursor field", () => {
  ...
  assert.deepEqual(validateCanonicalSort("emitted_at", ms), { direction: "ASC", field: "emitted_at" });
  assert.deepEqual(validateCanonicalSort("-emitted_at", ms), { direction: "DESC", field: "emitted_at" });
});
...
test("validateCanonicalSort rejects non-cursor fields as invalid_sort", () => {
  ...
  assert.throws(
    ...
    (e: unknown) => isQueryError(e) && e.code === "invalid_sort" && e.param === "sort"
  );
  // No cursor field advertised -> nothing is sortable.
  assert.throws(
    () => validateCanonicalSort("emitted_at", {}),
    (e: unknown) => isQueryError(e) && e.code === "invalid_sort"
  );
});
...
test("validateCanonicalSort rejects empty field and conflicting directions", () => {
  ...
  // biome-ignore lint/performance/useTopLevelRegex: test assertion patterns remain colocated with the assertion they explain.
  assert.throws(() => validateCanonicalSort("ts,-ts", ms), /Conflicting sort directions/);
  // Same direction repeated is fine.
  assert.deepEqual(validateCanonicalSort("ts,ts", ms), { direction: "ASC", field: "ts" });
});
...
test("resolveListOrder prefers canonical sort direction", () => {
  assert.equal(resolveListOrder(null, { direction: "DESC", field: "ts" }), "DESC");
  assert.equal(resolveListOrder("desc", { direction: "DESC", field: "ts" }), "DESC");
});

test("resolveListOrder rejects sort/order disagreement", () => {
  assert.throws(
    () => resolveListOrder("asc", { direction: "DESC", field: "ts" }),
    (e: unknown) => isQueryError(e) && e.code === "invalid_sort"
  );
});
