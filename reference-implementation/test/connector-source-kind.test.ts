...
function manifest(bindings: Record<string, unknown> | null): SourceKindManifestLike {
...
test("browser binding (no filesystem) resolves to browser_collector", () => {
  assert.equal(
    resolveEnrolledSourceKind({
      connectorId: "amazon",
      manifest: manifest({ browser: { required: true }, network: { required: true } }),
    }),
    "browser_collector"
  );
});
...
test("filesystem wins over browser when a manifest declares both", () => {
  ...
  assert.equal(
    resolveEnrolledSourceKind({
      connectorId: "hybrid",
      manifest: manifest({ browser: { required: true }, filesystem: { required: true } }),
    }),
    "local_device"
  );
});
...
test("a null/absent manifest is rejected, never defaulted", () => {
  assert.throws(() => resolveEnrolledSourceKind({ connectorId: "unknown", manifest: null }), SourceKindResolutionError);
