...
const TOP_LEVEL_REGEX_1 = /^ab_[0-9a-f]{24}$/;
...
test("makeAcquisitionBatchId carries the ab_ prefix and a 24-hex-char body", () => {
  ...
  assert.match(id, TOP_LEVEL_REGEX_1);
});

test("makeAcquisitionBatchId is deterministic for identical inputs", () => {
  assert.equal(makeAcquisitionBatchId("owner", "gmail", "sha"), makeAcquisitionBatchId("owner", "gmail", "sha"));
});
...
test("makeAcquisitionBatchId uses newline-delimited fields (no cross-field collision)", () => {
  ...
  assert.notEqual(makeAcquisitionBatchId("ab", "c", "x"), makeAcquisitionBatchId("a", "bc", "x"));
});
