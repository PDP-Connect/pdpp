...
import type { ComputeConnectionHealthInput, ConnectionRunEvidence } from "../runtime/connection-health.ts";
...
function succeededRun(overrides: Partial<ConnectionRunEvidence> = {}): ConnectionRunEvidence {
  return {
...
test("healthy + complete coverage + fresh -> forward_disposition complete", () => {
  const snap = computeConnectionHealth(
    input({ coverage: { axis: "complete" }, freshness: { axis: "fresh" }, run: succeededRun() })
  );
...
test("open attention + outstanding gap -> needs_attention headline AND awaiting_owner disposition", () => {
  ...
  const snap = computeConnectionHealth(
    input({
      ...
      coverage: { axis: "retryable_gap" },
      freshness: { axis: "fresh" },
      run: succeededRun({ hasDegradingGaps: true, latestStatus: "failed" }),
    })
  );
...
test("unsupported coverage -> terminal disposition", () => {
  const snap = computeConnectionHealth(
    input({ coverage: { axis: "unsupported" }, freshness: { axis: "fresh" }, run: succeededRun() })
  );
  assert.equal(snap.forward_disposition, "terminal");
});
...
test("absent coverage evidence -> unknown axis -> unmeasured, never checking, complete, or retryable", () => {
  const snap = computeConnectionHealth(input({ coverage: null, freshness: { axis: "fresh" }, run: succeededRun() }));
