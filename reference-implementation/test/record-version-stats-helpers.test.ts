...
test("isVersionChurnCandidate rejects a dirty projection outright", () => {
  assert.equal(isVersionChurnCandidate({ currentRecordCount: 0, dirty: true, recordHistoryCount: 1000 }), false);
});

test("isVersionChurnCandidate rejects a stream with no history", () => {
  assert.equal(isVersionChurnCandidate({ currentRecordCount: 5, dirty: false, recordHistoryCount: 0 }), false);
});

test("isVersionChurnCandidate accepts history with zero current records", () => {
  assert.equal(isVersionChurnCandidate({ currentRecordCount: 0, dirty: false, recordHistoryCount: 1 }), true);
});

test("isVersionChurnCandidate accepts at the vpr >= 5 upper-bound and rejects just below", () => {
  assert.equal(isVersionChurnCandidate({ currentRecordCount: 2, dirty: false, recordHistoryCount: 10 }), true);
  assert.equal(isVersionChurnCandidate({ currentRecordCount: 10, dirty: false, recordHistoryCount: 49 }), false);
});
