 ...
 *      server/stores/connector-instance-store.js) so the race is
...
try {
  const goPayload = await waitForGoLine();
  ...
  if (goPayload.op === 'delete') {
    ...
    outcome = { op: 'delete', ok: true, summary };
    ...
    outcome = { op: 'upsert', ok: true, row };
  } else {
    throw new Error(`unknown op ${goPayload.op}`);
  }
  const finishedAt = Date.now();
  process.stdout.write(`${JSON.stringify({ pid: process.pid, startedAt, finishedAt, outcome })}\n`);
