...
if (!(dbPath && connectorInstanceId)) {
  throw new Error(
    "two-process repair fixture requires PDPP_TWO_PROCESS_FIXTURE_DB_PATH and PDPP_TWO_PROCESS_FIXTURE_CONNECTOR_INSTANCE_ID"
  );
}
...
async function waitForGoLine(): Promise<string> {
...
try {
  ...
  process.stdout.write(`${JSON.stringify({ finishedAt, pid: process.pid, result, startedAt })}\n`);
  closeDb();
  process.exit(0);
} catch (error) {
  process.stdout.write(
    `${JSON.stringify({ error: error instanceof Error ? error.message : String(error), pid: process.pid })}\n`
  );
  closeDb();
  process.exit(1);
}
