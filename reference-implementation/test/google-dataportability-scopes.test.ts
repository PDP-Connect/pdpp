...
test("googleDataPortabilityScopesForConfiguredEnv: a comma list maps valid groups to their scopes in order", () => {
  assert.deepEqual(scopesFor("maps.reviews,maps.photos_videos"), [
    `${SCOPE_PREFIX}maps.reviews`,
    `${SCOPE_PREFIX}maps.photos_videos`,
  ]);
});
  ...
  assert.throws(
    ...
    (err: unknown) => {
      assert.ok(
        ...
        "err is a GoogleDataPortabilityProviderAuthError"
      );
      assert.equal(err.code, "google_dataportability_resource_group_unsupported", `code: ${err.code}`);
