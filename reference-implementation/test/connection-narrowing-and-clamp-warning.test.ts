...
function expectConnectionNotFound(fn: () => void) {
  assert.throws(fn, (err: unknown) => {
    assert.ok(err instanceof Error);
    const { code, param } = err as Error & { code?: string; param?: string };
    assert.equal(code, "connection_not_found", `expected connection_not_found, got ${code}`);
...
test("enforceConnectionNarrowing: conflicting connection_id/alias is rejected before narrowing (invalid_argument)", () => {
  ...
  assert.throws(
    ...
    (err: unknown) => {
      assert.ok(err instanceof Error);
      assert.equal((err as Error & { code?: string }).code, "invalid_argument");
      return true;
    }
  );
});
