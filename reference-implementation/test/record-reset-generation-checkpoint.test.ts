...
import { deleteAllRecords, deleteAllRecordsForConnector, ingestRecord } from "../server/records.ts";
...
function readGenerationSqlite(connectorInstanceId: string): number {
  const row = getDb()
    .prepare("SELECT record_reset_generation FROM connector_instances WHERE connector_instance_id = ?")
    .get(connectorInstanceId) as { record_reset_generation: number };
...
test("connector-wide reset discovers and clears a counter-only namespace (Sol third-verdict P2.2): a stream with ONLY a version_counter row and no live records is not silently skipped", async () => {
  initDb();
  try {
...
test("reinsertion after reset can never reproduce the earlier composite checkpoint (ABA)", async () => {
  ...
  try {
    ...
    assert.equal(
      mustRow(counterBefore, "counterBefore row exists").max_version,
      mustRow(counterAfter, "counterAfter row exists").max_version,
      "fixture: bare version vector DOES collide"
    );
    assert.notEqual(
      generationBefore,
      generationAfter,
