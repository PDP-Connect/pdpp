...
test("scheduler skips due runs while durable attention is unresolved and emits one suppression per identity", async () => {
    ...
    connectors: [
      {
        ...
        maxRetries: 0,
        ownerSubjectId: "owner-attention",
  ...
  try {
    ...
    assert.ok(skip, "expected exactly one completed run");
...
test("scheduler emits a fresh suppression skip when the attention identity changes", async () => {
  ...
  try {
    ...
    assert.ok(firstRun && secondRun, "expected exactly two completed runs");
    // biome-ignore lint/performance/useTopLevelRegex: localized test assertion preserves its explicit contract.
    assert.match(firstRun.error ?? "", /att_first/);
    // biome-ignore lint/performance/useTopLevelRegex: localized test assertion preserves its explicit contract.
    assert.match(secondRun.error ?? "", /att_second/);
  } finally {
    scheduler.stop();
    rmSync(tmpDir, { force: true, recursive: true });
  }
...
test("attention suppression does not bleed across connections", async () => {
  ...
  try {
    ...
    assert.equal(
      readAttempts(peerConnector.attemptsPath).length,
      0,
      "peer connector must not be spawned either (policy-blocked)"
    );
...
test("resolved attention does not replay missed ticks — latest-only catch-up", async () => {
  ...
  try {
    ...
    assert.equal(
      ticksWhileBlocked,
      1,
      "attention suppression must dedupe — exactly one record while attention persists"
    );
    const [firstBlockedTick] = completedRuns;
    assert.ok(firstBlockedTick, "expected exactly one completed run while blocked");
    // biome-ignore lint/performance/useTopLevelRegex: localized test assertion preserves its explicit contract.
    assert.match(firstBlockedTick.error || "", /attention_unresolved/);
    ...
    // biome-ignore lint/performance/useTopLevelRegex: localized test assertion preserves its explicit contract.
    await waitFor(() => completedRuns.some((r) => /automation_policy_blocked/.test(r.error || "")), 5000);
