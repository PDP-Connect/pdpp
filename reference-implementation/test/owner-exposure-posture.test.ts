...
test("test context ignores ALL inferred hosting signals (NODE_ENV/origin/asPublicUrl/bindHost)", () => {
  ...
  const inferred = posture({
    bindHost: "0.0.0.0",
    env: { AS_PUBLIC_URL: "https://app.fly.dev", NODE_ENV: "production", PDPP_REFERENCE_ORIGIN: "https://app.fly.dev" },
    isTestContext: true,
...
test("test context still honors the explicit PDPP_HOSTED override", () => {
  const forced = posture({ env: { PDPP_HOSTED: "1" }, isTestContext: true });
