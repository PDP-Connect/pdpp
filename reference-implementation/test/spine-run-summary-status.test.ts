const TOP_LEVEL_REGEX_1 = /WHERE\s+rn\s*<=/i;
const TOP_LEVEL_REGEX_2 = /event_id/i;
const TOP_LEVEL_REGEX_3 = /event_seq IS NULL/i;
const TOP_LEVEL_REGEX_4 = /event_seq IS NULL/i;
const TOP_LEVEL_REGEX_5 = /WHERE\s+rn\s*<=/i;
const TOP_LEVEL_REGEX_6 = /ORDER BY/i;
...
test("runtime event connection identity gives run summary exact connection identity", async () => {
  await withSpine(async () => {
    ...
    assert.deepEqual(summary.source, { id: "github", kind: "connector" });
...
test("batched head query pins an explicit outer ORDER BY in its SQL text", async () => {
  ...
  await withSpine(async () => {
    ...
    try {
      await listSpineCorrelations("run", { limit: 10, q: "run_order_by_pin" });
    } finally {
    ...
    for (const sql of capturedSql) {
      ...
      const partitionOrderBy = sql.slice(sql.indexOf("PARTITION BY"), sql.search(TOP_LEVEL_REGEX_1));
      assert.match(
        partitionOrderBy,
        TOP_LEVEL_REGEX_4,
        `expected the window function's partition ORDER BY to pin an explicit null-last policy in:\n${sql}`
      );
      const afterFilter = sql.slice(sql.search(TOP_LEVEL_REGEX_5));
      assert.match(afterFilter, TOP_LEVEL_REGEX_6, `expected an outer ORDER BY after the rn filter in:\n${sql}`);
      assert.match(
        afterFilter,
        TOP_LEVEL_REGEX_2,
...
test("batched summary hydration places null-event_seq rows last in the hydrated array, driving a status with no run-terminal override and no independent aggregate involved", async () => {
  ...
  await withSpine(async () => {
    ...
    const page = await listSpineCorrelations("trace", { limit: 50, q: traceId });
