...
interface FakeRes {
  ...
  body: { error: Record<string, unknown> } | null;
  getHeader: (name: string) => unknown;
  json: (b: { error: Record<string, unknown> }) => FakeRes;
...
test("pdppError: builds a typed envelope with status, code, message and a request_id", () => {
  ...
  assert.ok(res.body, "json body set");
...
test("pdppError: attaches resource_metadata + next_step ONLY on 401 when a metadata URL is present", () => {
  ...
  assert.ok(
    typeof res.body.error.next_step === "string" && res.body.error.next_step.length > 0,
    "next_step guidance attached"
  );
});
