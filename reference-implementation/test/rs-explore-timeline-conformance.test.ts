...
const RECORDS_A = [
  { data: { id: "a1", source: "A" }, emitted_at: ts(1), key: "a1" },
  { data: { id: "a3", source: "A" }, emitted_at: ts(3), key: "a3" },
  { data: { id: "a5", source: "A" }, emitted_at: ts(5), key: "a5" },
  { data: { id: "a7", source: "A" }, emitted_at: ts(7), key: "a7" },
  { data: { id: "a9", source: "A" }, emitted_at: ts(9), key: "a9" },
  { data: { id: "a11", source: "A" }, emitted_at: ts(11), key: "a11" },
];
// Partition B records: days 2, 4, 6, 8, 10, 12
const RECORDS_B = [
  { data: { id: "b2", source: "B" }, emitted_at: ts(2), key: "b2" },
  { data: { id: "b4", source: "B" }, emitted_at: ts(4), key: "b4" },
  { data: { id: "b6", source: "B" }, emitted_at: ts(6), key: "b6" },
  { data: { id: "b8", source: "B" }, emitted_at: ts(8), key: "b8" },
  { data: { id: "b10", source: "B" }, emitted_at: ts(10), key: "b10" },
  { data: { id: "b12", source: "B" }, emitted_at: ts(12), key: "b12" },
];
...
async function seedAll() {
  ...
  for (const { partition, r } of items) {
    ...
    await ingestRecord(
      ...
      { data: r.data, emitted_at: r.emitted_at, key: r.key, stream: partition.stream }
    );
  }
}
...
async function seedNewRecord() {
  await ingestRecord(
    { connectorId: NEW_RECORD.partition.connectorId, connectorInstanceId: NEW_RECORD.partition.connectorInstanceId },
    {
      ...
      emitted_at: NEW_RECORD.emitted_at,
      key: NEW_RECORD.key,
      stream: NEW_RECORD.partition.stream,
    }
  );
}
 ...
 */
async function pageToEnd(deps: ExploreTimelineDependencies, pageSize = 3) {
  ...
  while (true) {
    // biome-ignore lint/performance/noAwaitInLoops: Sequential test setup and assertion order is intentional.
    const page = await executeExploreTimeline({ cursor, limit: pageSize }, deps);
    ...
    if (pageCount > 100) {
      throw new Error("pageToEnd: too many pages — possible infinite loop");
    }
  }
...
async function assertConformance(deps: ExploreTimelineDependencies, label: string) {
  await seedAll();
  ...
  for (let i = 1; i < allRecords.length; i += 1) {
    ...
    const currRecord = allRecords[i];
    assert.ok(prevRecord, `${label}: expected allRecords[${i - 1}] to be defined`);
    assert.ok(currRecord, `${label}: expected allRecords[${i}] to be defined`);
    const prev = prevRecord.emitted_at;
    const curr = currRecord.emitted_at;
    assert.ok(prev >= curr, `${label}: order violation at index ${i}: ${prev} < ${curr}`);
  }
  ...
  // --- Snapshot stability: inserting a new record after page 1 does not shift prior pages ---
  {
    ...
    const page1 = await executeExploreTimeline({ cursor: null, limit: 3 }, deps);
    ...
    const page2 = await executeExploreTimeline({ cursor: page1.next_cursor, limit: 3 }, deps);
    assert.equal(page2.snapshot_at, snapshotAt, `${label}: page 2 snapshot_at must match page 1's anchor`);
    ...
    while (nextCursor && (safetyLimit -= 1) > 0) {
      // biome-ignore lint/performance/noAwaitInLoops: Sequential test setup and assertion order is intentional.
      const nextPage = await executeExploreTimeline({ cursor: nextCursor, limit: 3 }, deps);
      assert.equal(nextPage.snapshot_at, snapshotAt, `${label}: all pages must share the same snapshot_at anchor`);
    ...
    const page2After = await executeExploreTimeline({ cursor: page1.next_cursor, limit: 3 }, deps);
    assert.ok(
...
async function assertScopedConformance(deps: ExploreTimelineDependencies, label: string) {
  await seedAll();
  ...
  while (true) {
    ...
    cursor = page.next_cursor;
    if (pageCount > 20) {
      throw new Error(`${label}: scoped traversal did not terminate`);
    }
  }
  ...
  assert.equal(
    streamScoped.data.length,
    RECORDS_B.length,
    `${label}: stream scope must include all partition B records`
  );
...
async function cleanupPostgres() {
  ...
  for (const cid of cids) {
    // biome-ignore lint/performance/noAwaitInLoops: Sequential test setup and assertion order is intentional.
    await postgresQuery("DELETE FROM records WHERE connector_instance_id = $1", [cid]).catch(() => {
      /* intentionally empty */
    });
    await postgresQuery("DELETE FROM record_changes WHERE connector_instance_id = $1", [cid]).catch(() => {
      /* intentionally empty */
    });
    await postgresQuery("DELETE FROM version_counter WHERE connector_instance_id = $1", [cid]).catch(() => {
      /* intentionally empty */
    });
  }
}
  ...
  try {
    ...
    for (const at of [...past, ...future]) {
      ...
      await ingestRecord(
        ...
        { data: { id: key }, emitted_at: at, key, stream: part.stream }
      );
    }
...
test("rs.explore.timeline: the past/future boundary is PINNED in the cursor across pages (SQLite)", async () => {
  ...
  try {
    ...
    let calls = 0;
    const clock = () => {
      const result = calls === 0 ? "2026-06-21T00:00:00.000Z" : "2026-06-30T00:00:00.000Z";
      calls += 1;
      return result;
    };
    ...
    do {
      // biome-ignore lint/performance/noAwaitInLoops: Sequential test setup and assertion order is intentional.
      const page = await executeExploreTimeline({ cursor, limit: 2 }, deps);
      collected.push(...page.data.map((r) => r.record_key));
      cursor = page.has_more ? page.next_cursor : null;
      // biome-ignore lint/suspicious/noAssignInExpressions: Loop guard mutation is deliberately coupled to cursor progression.
    } while (cursor && (guard -= 1) > 0);
...
test("rs.explore.timeline: each record carries semantic_time and the feed is ordered by it (SQLite)", async () => {
  ...
  try {
    ...
    for (const day of [3, 6, 9, 12]) {
      ...
      await ingestRecord(
        { connectorId: part.connectorId, connectorInstanceId: part.connectorInstanceId },
        { data: { id: `m_${day}` }, emitted_at: ts(day), key: `m_${day}`, stream: part.stream }
      );
    }
    ...
    for (let i = 1; i < page.data.length; i += 1) {
      const prevRecord = page.data[i - 1];
      const currRecord = page.data[i];
      assert.ok(prevRecord, `expected page.data[${i - 1}] to be defined`);
      assert.ok(currRecord, `expected page.data[${i}] to be defined`);
      assert.ok(
        prevRecord.semantic_time >= currRecord.semantic_time,
        `feed must be ordered by semantic_time: ${prevRecord.semantic_time} >= ${currRecord.semantic_time}`
      );
    }
    // Newest first: m_12 leads, m_3 trails — and semantic_time == the emitted_at here.
    assert.deepEqual(
      page.data.map((r) => r.record_key),
      ["m_12", "m_9", "m_6", "m_3"]
    );
    // biome-ignore lint/style/useDestructuring: Indexed access expresses the protocol field position under test.
    const firstRecord = page.data[0];
    assert.ok(firstRecord, "expected page.data[0] to be defined");
    assert.equal(firstRecord.semantic_time, ts(12));
  } finally {
    closeDb();
  }
});
  ...
  try {
    ...
    for (let i = 0; i < PARTITION_COUNT; i += 1) {
      ...
      await ingestRecord(
        ...
        { data: { id: key }, emitted_at: ts((i % 27) + 1), key, stream: "records" }
      );
    }
    ...
    const page1 = await executeExploreTimeline({ cursor: null, limit: 5 }, deps);
    ...
    const collect = (page: ExploreTimelineOutput) => {
      for (const r of page.data) {
    ...
    while (cursor) {
      // biome-ignore lint/performance/noAwaitInLoops: Sequential test setup and assertion order is intentional.
      const page = await executeExploreTimeline({ cursor, limit: 5 }, deps);
      collect(page);
      cursor = page.next_cursor;
      guard += 1;
    ...
    await assert.rejects(
      () => executeExploreTimeline({ cursor: "ecr1_deadbeefdeadbeefdeadbeefdeadbeef", limit: 5 }, deps),
      (err) => err instanceof Error && "code" in err && err.code === "invalid_cursor",
  ...
  streams: [
    {
      ...
      schema: {
        properties: {
          create_time: { format: "date-time", type: ["string", "null"] },
          id: { type: "string" },
...
const SEM_RECORDS = [
  { create_time: ts(28), emitted_at: ts(1), key: "r1" },
  { create_time: ts(20), emitted_at: ts(2), key: "r2" },
  { create_time: ts(12), emitted_at: ts(3), key: "r3" },
  { create_time: ts(5), emitted_at: ts(4), key: "r4" },
];
...
async function seedSemanticRecords() {
  for (const r of SEM_RECORDS) {
    ...
    await ingestRecord(
      ...
      {
        data: { create_time: r.create_time, id: r.key, title: `conv ${r.key}` },
...
async function assertSemanticOrder(deps: ExploreTimelineDependencies, label: string) {
