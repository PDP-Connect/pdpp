...
import type * as ScheduleHistoryProjectionModule from "../runtime/schedule-history-projection.ts";
import type { MutableScheduleHistoryFacts } from "../runtime/schedule-history-projection.ts";
...
// Loaded via a dynamic `import()` (not a static import) so the guard above —
// which inspects the module's raw source text for a seeded mutation — runs
// against the same file this test exercises. The result is typed against the
// real module's exports so the rest of the file gets full type-checking.
const {
  applyHistoryRowToScheduleFacts,
  deriveLatestScheduleFacts,
  ensureScheduleHistoryFacts,
  hydrateScheduleHistoryFromLastRunTimes,
}: typeof ScheduleHistoryProjectionModule = await import(projectionModuleUrl.href);

function row(overrides: Partial<SchedulerRunHistoryRecord> = {}): SchedulerRunHistoryRecord {
  return {
    attempt: 1,
...
function createFactsHarness() {
  ...
  return {
    ensure: (connectorKey: string) => ensureScheduleHistoryFacts(facts, connectorKey),
    facts,
  };
}
