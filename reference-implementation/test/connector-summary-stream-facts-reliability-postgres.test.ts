...
interface EvidenceRow {
  ...
  stream_latest_facts_json: Record<string, unknown>;
...
async function evidenceRow(): Promise<EvidenceRow> {
  const result = await postgresQuery<EvidenceRow>(
  ...
  assert.ok(row, "expected connector summary evidence row");
  return row;
}
...
test("real PostgreSQL: a bounded pass (maxEvents:1) processes AT MOST one event and stays stale/incomplete, never current", {
  ...
  try {
    ...
    const seq1 = await seedTerminalEvent({
      runId: "run_1",
    ...
    await seedTerminalEvent({ runId: "run_3", streams: [{ checkpoint: "committed", collected: 1, stream: "labels" }] });
    ...
    assert.ok(evidence, "the connection must still have an evidence row");
...
test("real PostgreSQL: a future-version row is never folded/replayed/mutated durably — this binary fails it closed at read time only", {
  ...
  try {
    ...
    assert.deepEqual(
      afterRow,
      beforeRow,
      "the durable row is byte-for-byte unchanged by this older binary's fold pass"
    );
    ...
    assert.equal(
      ...
      "stale",
      "this binary fails the future-version row closed for its OWN observation"
    );
    ...
    assert.deepEqual(
      rawRowAfterRead,
      beforeRow,
      "reading through this older binary left zero durable trace on real PostgreSQL"
    );
  } finally {
    await cleanup();
    await closePostgresStorage();
