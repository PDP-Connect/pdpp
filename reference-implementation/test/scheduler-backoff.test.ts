...
/**
 * `reasonClassOf`'s `readConnectorReason` reads `connectorError.reason`
 * defensively via its own internal cast (the declared `ConnectorError` type
 * only names `message`/`retryable`), so a connector-authored `reason` string
 * is a real, load-bearing runtime shape this suite needs to construct.
 */
type TestConnectorError = RunRecord["connectorError"] & { reason?: string };

function record(
  overrides: Partial<Omit<RunRecord, "connectorError">> & { connectorError?: TestConnectorError } = {}
): RunRecord {
  const {
    status = "failed",
    terminalReason = null,
...
function failedRun(
  reasonOverrides: Partial<Omit<RunRecord, "connectorError">> & { connectorError?: TestConnectorError } = {}
): RunRecord {
  return record({ status: "failed", ...reasonOverrides });
}

function successRun() {
  return record({ recordsEmitted: 1, status: "succeeded" });
}
...
test("4 consecutive same-class failures yield 2x base interval (2^1)", () => {
  const history = Array.from({ length: 4 }, () => failedRun({ terminalReason: "authentication_error" }));
  const decision = computeNextRunWithBackoff(history, BASE_INTERVAL_MS, T0);
  assert.equal(decision.backoffApplied, true);
...
test("5 consecutive same-class failures yield 4x base interval (2^2)", () => {
  const history = Array.from({ length: 5 }, () => failedRun({ terminalReason: "authentication_error" }));
  const decision = computeNextRunWithBackoff(history, BASE_INTERVAL_MS, T0);
  assert.equal(decision.backoffApplied, true);
...
test("effective interval is capped by maxBackoffMs", () => {
  ...
  const history = Array.from({ length: 12 }, () => failedRun({ terminalReason: "authentication_error" }));
  const decision = computeNextRunWithBackoff(history, BASE_INTERVAL_MS, T0);
  assert.equal(decision.backoffApplied, true);
...
test("explicit small maxBackoffMs caps the effective interval", () => {
  const history = Array.from({ length: 6 }, () => failedRun({ terminalReason: "authentication_error" }));
...
test("custom threshold of 2 engages back-off earlier", () => {
  const history = Array.from({ length: 2 }, () => failedRun({ terminalReason: "authentication_error" }));
...
test("malformed timing inputs do not throw or emit invalid timestamps", () => {
  const history = Array.from({ length: 3 }, () => failedRun({ terminalReason: "authentication_error" }));
  const cases = [
...
test("lastSuccessAtMs null/undefined/non-finite is a no-op (legacy behaviour)", () => {
  const history = Array.from({ length: 5 }, () => streakFailure());
  const candidates: (number | null | undefined)[] = [null, undefined, Number.NaN, Number.POSITIVE_INFINITY, -1];
  for (const lastSuccessAtMs of candidates) {
    const options = lastSuccessAtMs === undefined ? {} : { lastSuccessAtMs };
    const decision = computeNextRunWithBackoff(history, BASE_INTERVAL_MS, T0, options);
