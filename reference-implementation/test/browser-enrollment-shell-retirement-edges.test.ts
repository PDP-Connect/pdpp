...
function shell(connectorInstanceId: string, bindingOverrides: Record<string, unknown> | null, status = "draft") {
  return {
    connectorInstanceId,
    sourceBinding: bindingOverrides === null ? null : { kind: "browser_enrollment_shell", ...bindingOverrides },
    status,
  };
}

test("expiredEnrollmentShellIds retires a shell whose TTL is exactly now (inclusive boundary)", () => {
  assert.deepEqual(expiredEnrollmentShellIds([shell("cin_at", { enrollment_expires_at: NOW })], NOW), ["cin_at"]);
});
...
test("expiredEnrollmentShellIds treats an unparseable TTL as not-yet-expired", () => {
  assert.deepEqual(expiredEnrollmentShellIds([shell("cin_bad", { enrollment_expires_at: "not-a-date" })], NOW), []);
});

test("expiredEnrollmentShellIds ignores a non-string TTL", () => {
  assert.deepEqual(expiredEnrollmentShellIds([shell("cin_num", { enrollment_expires_at: 1_234_567_890 })], NOW), []);
});
...
test("expiredEnrollmentShellIds skips a shell with a null or absent sourceBinding", () => {
  ...
  assert.deepEqual(expiredEnrollmentShellIds([{ connectorInstanceId: "cin_absent", status: "draft" }], NOW), []);
});
