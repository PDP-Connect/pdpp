...
test("isTopLevelSearchableStringField: mixed or non-string types fail", () => {
  assert.equal(
    isTopLevelSearchableStringField({ type: ["string", "integer"] }),
    false,
    "string+integer is not purely string"
  );
...
test("isReferenceCompatibleCursorSchema: string is compatible ONLY with date/date-time format", () => {
  ...
  assert.equal(isReferenceCompatibleCursorSchema({ format: "date-time", type: "string" }), true);
  assert.equal(isReferenceCompatibleCursorSchema({ type: "string" }), false, "plain string not cursorable");
  assert.equal(
    isReferenceCompatibleCursorSchema({ format: "email", type: "string" }),
    false,
    "non-date format not cursorable"
  );
});
...
test("isTimeBucketAggregateFieldSchema: string with date/date-time format only", () => {
  ...
  assert.equal(isTimeBucketAggregateFieldSchema({ format: "date-time", type: ["string", "null"] }), true);
...
function validBlobRef(): BlobRefSchema {
  return {
    properties: {
      blob_id: { type: "string" },
      mime_type: { type: "string" },
...
function expectManifestReject(fn: () => void): void {
  assert.throws(fn, (err: unknown) => {
    assert.ok(typeof err === "object" && err !== null);
    const typed = err as { code?: unknown };
    assert.equal(typed.code, "invalid_request", `expected invalid_request, got ${String(typed.code)}`);
    return true;
  });
}
...
test("validateBlobRefSchemaDeclaration: a non-object blob_ref is rejected", () => {
  expectManifestReject(() =>
    validateBlobRefSchemaDeclaration({ name: "files" }, { type: "string" }, "invalid_request")
  );
});
...
test("validateBlobRefSchemaDeclaration: a missing required property declaration is rejected", () => {
  const { sha256: _sha256, ...properties } = validBlobRef().properties;
  const bad = { ...validBlobRef(), properties };
  expectManifestReject(() => validateBlobRefSchemaDeclaration({ name: "files" }, bad, "invalid_request"));
});
