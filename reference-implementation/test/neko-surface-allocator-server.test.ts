...
const REGEXP_5 = /^\/containers\/([^/]+)$/;
const REGEXP_6 = /^\/containers\/([^/]+)\/json$/;
const REGEXP_7 = /^\/networks\/([^/]+)\/connect$/;
const REGEXP_8 = /^http:\/\/127\.0\.0\.1:59000\/cdp\//;
const REGEXP_9 = /^pdpp-neko-chatgpt-[a-f0-9]{16}$/;
const REGEXP_10 = /^chatgpt-[a-f0-9]{16}$/;
...
interface DockerCreateContainerBody {
  readonly Env: string[];
  ...
  readonly HostConfig: {
    ...
    readonly PortBindings: Record<string, { readonly HostPort: string }[]>;
  };
  readonly Image: string;
  readonly Labels: Record<string, string>;
}

interface DockerNetworkCall {
  readonly Container?: string;
  readonly Name?: string;
}

interface DockerCall<Body = DockerCreateContainerBody | DockerNetworkCall> {
  readonly init: DockerEngineRequestInit & {
    readonly body?: Body;
...
type ServedSurface = BrowserSurface & { readonly window_settle_endpoint: string };
...
function assertSurface(surface: BrowserSurface | null): asserts surface is ServedSurface {
  assert.ok(surface, "expected a surface, got null");
}

function allocatorMetadata(surface: BrowserSurface): Record<string, string> {
  assert.ok(surface.allocator_metadata, "surface must carry allocator_metadata");
  return surface.allocator_metadata;
}

function findCreateCall(
  calls: readonly DockerCall[],
  path: "/containers/create"
): DockerCall<DockerCreateContainerBody> {
  const call = calls.find((c) => c.path === path);
  assert.ok(call, `expected a Docker call to ${path}`);
  return call as DockerCall<DockerCreateContainerBody>;
}

function findNetworkCall(calls: readonly DockerCall[], path: string): DockerCall<DockerNetworkCall> {
  const call = calls.find((c) => c.path === path);
  assert.ok(call, `expected a Docker call to ${path}`);
  return call as DockerCall<DockerNetworkCall>;
}

function findContainer(docker: FakeDocker, id: string): FakeContainer {
  const container = docker.containers.get(id);
  assert.ok(container, `expected FakeDocker to have container ${id}`);
  return container;
}

function callReferencesContainer(call: DockerCall, containerId: string): boolean {
  // biome-ignore lint/style/useDestructuring: Indexed access expresses the protocol field position under test.
  const body = call.init.body;
  return body !== undefined && "Container" in body && body.Container === containerId;
}

function onlyContainerId(docker: FakeDocker): string {
  // biome-ignore lint/style/useDestructuring: Indexed access expresses the protocol field position under test.
  const id = [...docker.containers.keys()][0];
  assert.ok(id, "expected FakeDocker to have at least one container");
  return id;
}
...
const BASE_OPTIONS = Object.freeze({
  ...
  webrtcHostPortEnd: 59_002,
  webrtcHostPortStart: 59_000,
});
...
test("creates and starts an owned n.eko container with sanitized profile storage and readiness metadata", async () => {
  ...
  assert.equal(allocatorMetadata(surface).readiness, "ready");
  assert.equal(allocatorMetadata(surface).host_port, "59000");
  assert.equal(surface.stream_base_url, "http://127.0.0.1:59000/neko");
  assert.match(surface.cdp_url, REGEXP_8);
  assertSurface(surface);
  assert.equal(surface.window_settle_endpoint, "http://127.0.0.1:59000/pdpp/window-settle");
  assert.equal(allocatorMetadata(surface).window_settle_endpoint, "http://127.0.0.1:59000/pdpp/window-settle");

  const create = findCreateCall(docker.calls, "/containers/create");
  assert.equal(create.init.method, "POST");
  assert.ok(create.init.body, "create call must carry a body");
  ...
  assert.ok(queryName, "create call query must carry a name");
  assert.match(queryName, REGEXP_9);
  const profileSlug = create.init.body.Labels[`${LABEL}.profile_slug`];
  assert.ok(profileSlug, "create call must label the profile slug");
  assert.match(profileSlug, REGEXP_10);
  const profilePath = create.init.body.Labels[`${LABEL}.profile_path`];
  assert.ok(profilePath, "create call must label the profile path");
  assert.match(profilePath, REGEXP_11);
  assert.deepEqual(profileFilesystem.calls, [
    { method: "mkdir", options: { mode: 0o700, recursive: true }, path: profilePath },
    { gid: 1000, method: "chown", path: profilePath, uid: 1000 },
    { method: "chmod", mode: 0o700, path: profilePath },
  ]);
  assert.doesNotMatch(queryName, REGEXP_12);
  assert.doesNotMatch(profileSlug, REGEXP_13);
  ...
  assert.equal(
    docker.calls.some((call) => call.path === "/containers/container_1/start"),
    true
  );
});
...
test("parses env-driven HTTP listen config and allocator defaults", () => {
  ...
  assert.ok(options.extraEnv, "options must carry extraEnv");
...
test("ensureNetworkExists creates the network only when it does not already exist", async () => {
  ...
  // biome-ignore lint/style/useDestructuring: Indexed access expresses the protocol field position under test.
  const networkCreateCall = createCalls[0];
  assert.ok(networkCreateCall, "expected a /networks/create call");
  assert.ok(networkCreateCall.init.body, "network create call must carry a body");
  assert.equal((networkCreateCall.init.body as DockerNetworkCall).Name, BASE_OPTIONS.network);
...
test("ensureSurface migrates a legacy-network-only owned container to the expected network and detaches the legacy one", async () => {
  ...
  docker.addLegacyNetworkOwnedContainer("container_1", { legacyNetwork: "pdpp_default", surfaceId: "surface_1" });
  const service = new NekoSurfaceAllocatorService({
    ...BASE_OPTIONS,
    docker,
    fetchImpl: readyFetch(),
  ...
  assert.equal(surface.health, "ready");
  const migrated = findContainer(docker, "container_1");
  assert.ok(migrated.networks.has(BASE_OPTIONS.network), "expected network must be attached");
  assert.equal(
    migrated.networks.has("pdpp_default"),
    false,
    "legacy network must be detached after successful migration"
  );
  const connectCall = findNetworkCall(docker.calls, `/networks/${BASE_OPTIONS.network}/connect`);
  assert.ok(connectCall.init.body, "connect call must carry a body");
  assert.equal(connectCall.init.body.Container, "container_1");
  const disconnectCall = findNetworkCall(docker.calls, "/networks/pdpp_default/disconnect");
  assert.ok(disconnectCall.init.body, "disconnect call must carry a body");
...
test("ensureSurface does NOT detach the legacy network until the expected network is proven reachable by direct IP probe, not just attached", async () => {
  ...
  const migrated = findContainer(docker, "container_1");
  assert.ok(
    migrated.networks.has(BASE_OPTIONS.network),
    "the expected network attach step itself must still have succeeded"
  );
  assert.ok(
    migrated.networks.has("pdpp_default"),
    "legacy network must remain attached because reachability on the expected network was never proven"
  );
  assert.equal(
  ...
  assert.ok(
    requestedHosts.includes(expectedNetworkIp),
    "the allocator must probe the container's IP address on the expected network directly"
  );
  ...
  assert.equal(
    allocatorMetadata(surface).readiness,
  ...
  assertSurface(retried);
  assert.equal(retried.health, "starting");
  assert.equal(allocatorMetadata(retried).readiness, "legacy_network_migration_pending");
  assert.ok(
    findContainer(docker, "container_1").networks.has("pdpp_default"),
    "legacy network still must not be detached on a retry that keeps failing reachability"
  );
  ...
  assertSurface(recovered);
  assert.equal(recovered.health, "ready");
  assert.equal(allocatorMetadata(recovered).readiness, "ready");
  const recoveredContainer = findContainer(docker, "container_1");
  assert.ok(recoveredContainer.networks.has(BASE_OPTIONS.network));
  assert.equal(
    recoveredContainer.networks.has("pdpp_default"),
    false,
    "legacy network must be detached once reachability is finally proven"
  );
});
...
test("ensureSurface migration is idempotent when the container is already attached to the expected network", async () => {
  ...
  await service.ensureSurface({ connectorId: "chatgpt", profileKey: "profile_1", surfaceId: "surface_1" });
  docker.calls.length = 0;

  await service.ensureSurface({ connectorId: "chatgpt", profileKey: "profile_1", surfaceId: "surface_1" });

  assert.equal(
    docker.calls.some((call) => call.path.includes("/connect")),
    false
  );
  assert.equal(
    docker.calls.some((call) => call.path.includes("/disconnect")),
    false
  );
});
...
test("ensureSurface never detaches a network other than the explicitly configured legacy network", async () => {
  ...
  docker.addLegacyNetworkOwnedContainer(id, { legacyNetwork: "pdpp_default", surfaceId: "surface_1" });
  ...
  findContainer(docker, id).networks.add("some-other-user-network");
  const service = new NekoSurfaceAllocatorService({
    ...BASE_OPTIONS,
    docker,
    fetchImpl: readyFetch(),
  ...
  const migrated = findContainer(docker, id);
...
test("ensureSurface does not detach the legacy network when legacyNetwork is not configured", async () => {
  ...
  assert.ok(
    migrated.networks.has(BASE_OPTIONS.network),
    "network attach must still happen without legacyNetwork config"
  );
...
test("ensureSurface preserves the container and reports a bounded pending state when the network attach fails", async () => {
  ...
  assert.equal(allocatorMetadata(surface).readiness, "legacy_network_migration_pending");
  const migrated = findContainer(docker, "container_1");
...
test("ensureSurface keeps the legacy network attached when detach fails, but still reports the surface as reachable", async () => {
  ...
  assert.equal(
    surface.health,
    "ready",
    "the container is already reachable on the new network, so detach failure is not a health problem"
  );
  const migrated = findContainer(docker, "container_1");
  assert.ok(migrated.networks.has(BASE_OPTIONS.network));
  assert.ok(
    migrated.networks.has("pdpp_default"),
    "legacy network stays attached until a later detach attempt succeeds"
  );
});
...
test("ensureSurface retries a previously failed legacy detach on the next access", async () => {
  ...
  assert.equal(findContainer(docker, "container_1").networks.has("pdpp_default"), false);
});
...
test("listSurfaces migrates every legacy-network-only owned container it discovers", async () => {
  const docker = new FakeDocker();
  docker.networks.add(BASE_OPTIONS.network);
  docker.addLegacyNetworkOwnedContainer("container_1", {
    hostPort: 59_000,
    legacyNetwork: "pdpp_default",
    surfaceId: "surface_1",
  });
  docker.addLegacyNetworkOwnedContainer("container_2", {
    hostPort: 59_001,
    legacyNetwork: "pdpp_default",
    surfaceId: "surface_2",
  });
  ...
  assert.equal(findContainer(docker, "container_1").networks.has("pdpp_default"), false);
  assert.equal(findContainer(docker, "container_2").networks.has("pdpp_default"), false);
});
...
test("ensureSurface migrates a container being restarted from an exited-but-not-replaceable state", async () => {
  ...
  docker.networks.add(BASE_OPTIONS.network);
  docker.addLegacyNetworkOwnedContainer("container_1", { legacyNetwork: "pdpp_default", surfaceId: "surface_1" });
  findContainer(docker, "container_1").running = false;
  findContainer(docker, "container_1").state = "created";
  const service = new NekoSurfaceAllocatorService({
    ...BASE_OPTIONS,
    docker,
    fetchImpl: readyFetch(),
  ...
  assert.equal(surface.container_id, "container_1");
  assert.equal(findContainer(docker, "container_1").running, true);
  assert.equal(findContainer(docker, "container_1").networks.has(BASE_OPTIONS.network), true);
  assert.equal(findContainer(docker, "container_1").networks.has("pdpp_default"), false);
});
...
test("an unnamespaced legacy container from a DIFFERENT Compose project is invisible and untouched", async () => {
  ...
  assert.notEqual(
    ...
    "preexisting_legacy_container",
    "a legacy container from a different Compose project must never be reused"
  );
  assert.equal(
    findContainer(docker, "preexisting_legacy_container").networks.has("pdpp_default"),
    true,
    "the foreign-project container itself must be left completely untouched"
  );
  assert.equal(
    docker.calls.some((call) => callReferencesContainer(call, "preexisting_legacy_container")),
    false,
...
test("a container belonging to a DIFFERENT deployment id is never enumerated, migrated, or otherwise touched", async () => {
  ...
  findContainer(docker, "foreign_deployment_container").labels[`${LABEL}.deployment_id`] = "some-other-deployment";
  const service = new NekoSurfaceAllocatorService({
    ...BASE_OPTIONS,
    docker,
    fetchImpl: readyFetch(),
  ...
  assert.equal(
    status?.container_id,
    surface.container_id,
    "getSurfaceStatus for this surface_id must resolve to this deployment's own new container, never the foreign one"
  );
  assert.equal(
    findContainer(docker, "foreign_deployment_container").networks.has("pdpp_default"),
    true,
    "the foreign-deployment container's network attachment must be completely unchanged"
  );
  assert.equal(
    docker.calls.some((call) => callReferencesContainer(call, "foreign_deployment_container")),
    false,
...
test("does not create a Docker container when profile directory preparation fails", async () => {
  ...
  await assert.rejects(
    () => service.ensureSurface({ connectorId: "chatgpt", profileKey: "profile_1", surfaceId: "surface_1" }),
    REGEXP_24
  );
  assert.equal(
    docker.calls.some((call) => call.path === "/containers/create"),
    false
  );
});
...
test("gets, lists, and stops only PDPP-owned surfaces", async () => {
  ...
  const stopped = await service.stopSurface({ reason: "idle_ttl", surfaceId: "surface_1" });
  assert.equal(stopped?.health, "stopping");
  assert.equal(findContainer(docker, "container_1").running, false);
});
...
test("stopSurface tolerates post-stop inspect without Docker host port bindings", async () => {
  ...
  assertSurface(stopped);
  assert.equal(stopped.surface_id, "surface_1");
  assert.equal(stopped.health, "stopping");
  assert.equal(allocatorMetadata(stopped).host_port, "59000");
});
...
test("getSurfaceStatus and listSurfaces tolerate stopped containers without Docker host port bindings", async () => {
  ...
  const service = new NekoSurfaceAllocatorService({ ...BASE_OPTIONS, docker, fetchImpl: readyFetch() });
  await service.ensureSurface({ connectorId: "chatgpt", profileKey: "profile_1", surfaceId: "surface_1" });
  await service.stopSurface({ reason: "idle_ttl", surfaceId: "surface_1" });

  const status = await service.getSurfaceStatus("surface_1");
  const listed = await service.listSurfaces();

  assertSurface(status);
  assert.equal(status.surface_id, "surface_1");
  assert.equal(status.health, "stopping");
  assert.equal(allocatorMetadata(status).readiness, "container_not_running");
  assert.equal(allocatorMetadata(status).host_port, "59000");
  assert.deepEqual(
    listed.map((surface) => [surface.surface_id, surface.health, allocatorMetadata(surface).host_port]),
...
test("getSurfaceStatus keeps rejecting running containers without Docker host port bindings", async () => {
  ...
  const service = new NekoSurfaceAllocatorService({ ...BASE_OPTIONS, docker, fetchImpl: readyFetch() });
  await service.ensureSurface({ connectorId: "chatgpt", profileKey: "profile_1", surfaceId: "surface_1" });
  findContainer(docker, "container_1").hostPort = undefined;
...
test("HTTP DELETE returns a stopped surface when Docker removes the owned container before post-stop inspect", async () => {
  ...
  try {
    ...
    await client.ensureSurface({ connectorId: "chatgpt", profileKey: "profile_1", surfaceId: "surface_1" });

    const stopped = await client.stopSurface({ reason: "operator", surfaceId: "surface_1" });

    assertSurface(stopped);
    assert.equal(stopped.surface_id, "surface_1");
    assert.equal(stopped.health, "stopping");
    assert.equal(allocatorMetadata(stopped).readiness, "container_removed");
  } finally {
    await server.close();
  }
});
...
test("HTTP GET and list return a stopped surface when Docker omits post-stop host port bindings", async () => {
  ...
  try {
    const client = new NekoSurfaceAllocatorClient({ baseUrl: server.url });
    await client.ensureSurface({ connectorId: "chatgpt", profileKey: "profile_1", surfaceId: "surface_1" });
    await client.stopSurface({ reason: "operator", surfaceId: "surface_1" });
...
test("does not create beyond the configured host port range", async () => {
  ...
  const service = new NekoSurfaceAllocatorService({
    ...
    docker,
    fetchImpl: readyFetch(),
    webrtcHostPortEnd: 59_000,
    webrtcHostPortStart: 59_000,
  });
  await service.ensureSurface({ connectorId: "chatgpt", profileKey: "profile_1", surfaceId: "surface_1" });

  await assert.rejects(
    () => service.ensureSurface({ connectorId: "chatgpt", profileKey: "profile_2", surfaceId: "surface_2" }),
...
test("reclaims stopped containers when the dynamic host port range is otherwise full", async () => {
  ...
  const service = new NekoSurfaceAllocatorService({
    ...
    docker,
    fetchImpl: readyFetch(),
    webrtcHostPortEnd: 59_001,
    webrtcHostPortStart: 59_000,
  });
  await service.ensureSurface({ connectorId: "chatgpt", profileKey: "profile_1", surfaceId: "surface_1" });
  await service.ensureSurface({ connectorId: "reddit", profileKey: "profile_2", surfaceId: "surface_2" });
  await service.stopSurface({ reason: "idle_ttl", surfaceId: "surface_1" });
  await service.stopSurface({ reason: "idle_ttl", surfaceId: "surface_2" });
  const stoppedContainerIds = [...docker.containers.keys()];
  const [firstStoppedId, secondStoppedId] = stoppedContainerIds;
  assert.ok(firstStoppedId, "expected at least one stopped container id");
  assert.ok(secondStoppedId, "expected a second stopped container id");
  docker.calls.length = 0;
  ...
  assert.equal(surface.health, "ready");
  assert.equal(allocatorMetadata(surface).host_port, "59000");
  assert.equal(docker.containers.has(firstStoppedId), false);
  assert.equal(docker.containers.has(secondStoppedId), true);
  assert.ok(
    docker.calls.some((call) => call.path === `/containers/${firstStoppedId}` && call.init.method === "DELETE"),
...
test("retries the next host port when Docker reports a start-time port collision", async () => {
  const docker = new FakeDocker();
  docker.failStartForHostPort(59_000);
  const service = new NekoSurfaceAllocatorService({
    ...BASE_OPTIONS,
    docker,
    fetchImpl: readyFetch(),
  ...
  assert.equal(surface.health, "ready");
  assert.equal(allocatorMetadata(surface).host_port, "59001");
  assert.equal(docker.containers.size, 1);
  // biome-ignore lint/style/useDestructuring: Indexed access expresses the protocol field position under test.
  const onlyContainer = [...docker.containers.values()][0];
  assert.ok(onlyContainer, "expected exactly one container");
  assert.equal(onlyContainer.hostPort, 59_001);
  assert.ok(
  ...
  assert.deepEqual(
    docker.calls
      .filter((call): call is DockerCall<DockerCreateContainerBody> => call.path === "/containers/create")
      .map((call) => {
        assert.ok(call.init.body, "create call must carry a body");
        return call.init.body.Labels[`${LABEL}.webrtc_host_port`];
      }),
    ["59000", "59001"]
  );
});
...
test("reports starting until n.eko, CDP, and Chromium probes pass", async () => {
  ...
  assert.equal(surface.health, "starting");
  assert.equal(allocatorMetadata(surface).readiness, "cdp_unready");
});
...
test("HTTP handler matches NekoSurfaceAllocatorClient contract", async () => {
  ...
  try {
    const client = new NekoSurfaceAllocatorClient({ baseUrl: server.url });
    const created = await client.ensureSurface({
      connectorId: "chatgpt",
      profileKey: "profile_1",
      surfaceId: "surface_1",
    });
    ...
    assert.equal((await client.stopSurface({ reason: "operator", surfaceId: "surface_1" }))?.health, "stopping");
...
test("ensureSurface removes a stale exited container and creates a fresh one (no silent restart of a dead carcass)", async () => {
  ...
  await service.ensureSurface({ connectorId: "chatgpt", profileKey: "profile_1", surfaceId: "surface_1" });
  const firstContainerId = onlyContainerId(docker);
  // Simulate the container exiting after the previous run (the bug scenario).
  findContainer(docker, firstContainerId).running = false;
  docker.calls.length = 0;
...
test("stopSurface(surface_failed) removes the container so the next ensureSurface gets a clean slate", async () => {
  ...
  const service = new NekoSurfaceAllocatorService({ ...BASE_OPTIONS, docker, fetchImpl: readyFetch() });
  await service.ensureSurface({ connectorId: "chatgpt", profileKey: "profile_1", surfaceId: "surface_1" });
  const containerId = onlyContainerId(docker);
  docker.calls.length = 0;

  const stopped = await service.stopSurface({ reason: "surface_failed", surfaceId: "surface_1" });
...
test("ensureSurface replaces a RUNNING container that Docker marks unhealthy (wedged CDP carcass, never exits)", async () => {
  ...
  findContainer(docker, firstContainerId).health = "unhealthy";
  docker.calls.length = 0;
...
test("getSurfaceStatus advertises the exact RI pre-claim replacement intent", async () => {
  ...
  const service = new NekoSurfaceAllocatorService({ ...BASE_OPTIONS, docker, fetchImpl: readyFetch() });
  await service.ensureSurface({ connectorId: "chatgpt", profileKey: "profile_1", surfaceId: "surface_1" });
  const container = findContainer(docker, "container_1");

  container.health = "unhealthy";
  const unhealthy = await service.getSurfaceStatus("surface_1");
  assertSurface(unhealthy);
  assert.equal(allocatorMetadata(unhealthy).ensure_disposition, "replace");

  container.health = undefined;
  container.running = false;
  const stopped = await service.getSurfaceStatus("surface_1");
  assertSurface(stopped);
  assert.equal(allocatorMetadata(stopped).ensure_disposition, "replace");
});
...
test("ensureSurface does NOT replace a running container that is cold-starting (no boot loop)", async () => {
  ...
  findContainer(docker, firstContainerId).health = "starting";
  docker.calls.length = 0;
...
class FakeDocker implements DockerEngineTransport {
  calls: DockerCall[] = [];
  containers = new Map<string, FakeContainer>();
  networks = new Set<string>();
  nextId = 0;
  foreignInspectIds = new Set<string>();
  startFailurePorts = new Set<number | undefined>();
  ...
  async requestJson(path: string, init: DockerEngineRequestInit = {}): Promise<unknown> {
    ...
    const networkInspectMatch = path.match(REGEXP_25);
    if (networkInspectMatch?.[1] !== undefined && (call.init.method ?? "GET") === "GET") {
    ...
    if (path === "/networks/create") {
      ...
      if (this.networkCreateConflict) {
        ...
        return { message: `network with name ${String(body.Name)} already exists` };
      }
      this.networks.add(String(body.Name));
      return { Id: body.Name };
    }
    const connectMatch = path.match(REGEXP_7);
    if (connectMatch) {
      const network = decodeURIComponent(connectMatch[1] ?? "");
      const containerId = String(body.Container);
    ...
    const disconnectMatch = path.match(REGEXP_2);
    if (disconnectMatch) {
    ...
    if (path === "/containers/create") {
      ...
      const id = `container_${(this.nextId += 1)}`;
      const createBody = body as {
        Labels: Record<string, string>;
        HostConfig: { NetworkMode: string };
      };
      const hostPort = Number(createBody.Labels[`${LABEL}.webrtc_host_port`]);
      const query = call.init.query ?? {};
      this.containers.set(id, {
        hostPort,
        id,
        labels: createBody.Labels,
        listAsOwned: true,
        name: query.name ?? id,
        networks: new Set([createBody.HostConfig.NetworkMode]),
        running: false,
      });
      return { Id: id };
    }
    const startMatch = path.match(REGEXP_3);
    if (startMatch?.[1] !== undefined) {
      const container = findContainer(this, startMatch[1]);
    ...
    const stopMatch = path.match(REGEXP_4);
    if (stopMatch?.[1] !== undefined) {
      const container = findContainer(this, stopMatch[1]);
      // biome-ignore lint/suspicious/noUnnecessaryConditions: Runtime guard protects an untyped external/test boundary.
    ...
    const removeMatch = path.match(REGEXP_5);
    if (removeMatch?.[1] !== undefined && call.init.method === "DELETE") {
      this.containers.delete(removeMatch[1]);
      return null;
    }
    const inspectMatch = path.match(REGEXP_6);
    if (inspectMatch?.[1] !== undefined) {
      // biome-ignore lint/style/useDestructuring: Indexed access expresses the protocol field position under test.
      ...
      return {
        ...
        State: {
          ...
          ...(container.health === undefined
            ? {}
            : { Health: { FailingStreak: container.health === "unhealthy" ? 12 : 0, Status: container.health } }),
        },
      };
    }
  ...
  addForeignContainer(id: string, labels: Record<string, string>): void {
    this.containers.set(id, {
      hostPort: 59_001,
      id,
      labels,
  ...
  addOwnedContainerWithoutPublishedPorts(id: string, labels: Record<string, string>): void {
    this.containers.set(id, {
      hostPort: undefined,
      id,
  ...
  addStoppedOwnedContainerWithoutPublishedPorts(id: string, labels: Record<string, string>): void {
    this.containers.set(id, {
      hostPort: undefined,
      id,
      labels: { [`${LABEL}.deployment_id`]: DEPLOYMENT_ID, ...labels },
  ...
  failStartForHostPort(port: number): void {
    this.startFailurePorts.add(port);
  }

  ipAddressFor(containerId: string, network: string): string {
  ...
  addOwnedSummaryForForeignInspect(id: string, surfaceId: string): void {
    this.containers.set(id, {
      hostPort: 59_000,
      id,
      labels: {
  ...
  addLegacyNetworkOwnedContainer(
    id: string,
    {
      surfaceId,
      legacyNetwork,
      hostPort = 59_000,
      composeProject = DEPLOYMENT_ID,
    }: {
      surfaceId: string;
...
function selectiveFetch(predicate: (url: URL) => boolean): FetchLike {
  // biome-ignore lint/suspicious/useAwait: Async callback preserves the dependency contract and rejection timing.
  return async (input) => {
    const url = new URL(String(input));
    if (!predicate(url)) {
...
function noopProfileFilesystem(): ProfileFilesystem {
  return {
    async chmod() {
      /* intentionally empty */
    },
...
interface ProfileFilesystemCall {
  readonly gid?: number;
  readonly method: "chmod" | "chown" | "mkdir";
  readonly mode?: number;
  readonly options?: { mode: number; recursive: true };
  readonly path: string;
  readonly uid?: number;
}

function recordingProfileFilesystem(): ProfileFilesystem & { calls: ProfileFilesystemCall[] } {
  const calls: ProfileFilesystemCall[] = [];
  return {
    calls,
    // biome-ignore lint/suspicious/useAwait: Async callback preserves the dependency contract and rejection timing.
    async chmod(path, mode) {
      calls.push({ method: "chmod", mode, path });
    },
    // biome-ignore lint/suspicious/useAwait: Async callback preserves the dependency contract and rejection timing.
    async chown(path, uid, gid) {
      calls.push({ gid, method: "chown", path, uid });
    },
    // biome-ignore lint/suspicious/useAwait: Async callback preserves the dependency contract and rejection timing.
    async mkdir(path, options) {
      calls.push({ method: "mkdir", options, path });
    },
  };
}
