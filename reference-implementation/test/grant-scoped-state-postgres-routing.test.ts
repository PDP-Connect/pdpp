...
import { resolveGrantScopedStateGrant } from "../server/index.ts";
  ...
  test("resolveGrantScopedStateGrant reads from postgres when backend is postgres", async () => {
    ...
    try {
      const grantJson = {
        ...
        grant_id: grantId,
        retention: { mode: "none" },
        source: { connector_id: connectorId, kind: "connector" },
        streams: [{ name: "items" }],
      ...
      let missingErr: (Error & { code?: string }) | undefined;
      try {
        await resolveGrantScopedStateGrant(connectorId, `${grantId}_missing`);
      } catch (e) {
        assert.ok(e instanceof Error);
        missingErr = e as Error & { code?: string };
      }
