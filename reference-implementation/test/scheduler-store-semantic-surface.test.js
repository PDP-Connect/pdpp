...
test('active-run upsert preserves the incumbent row on duplicate admission', async () => {
  await withFreshStore((store) => {
    const first = store.upsertActiveRun({
      connector_instance_id: SEMANTIC_CONNECTOR,
      connector_id: SEMANTIC_CONNECTOR,
    ...
    const second = store.upsertActiveRun({
      connector_instance_id: SEMANTIC_CONNECTOR,
      connector_id: SEMANTIC_CONNECTOR,
    ...
    assert.equal(store.getActiveRun(SEMANTIC_CONNECTOR)?.run_id, 'run_semantic_first');
    assert.deepEqual(
      store.listActiveRuns().map((row) => [row.connector_instance_id, row.run_id]),
