...
import { deleteAllRecords, ingestRecord } from "../server/records.ts";
...
function seedInstanceSqlite({ connectorInstanceId }: { connectorInstanceId: string }): void {
  getDb()
...
function storageTargetFor(connectorInstanceId: string): { connector_id: string; connector_instance_id: string } {
...
test("normalization strips leading zeros without numeric coercion", () => {
  const checkpoint = normalizeRecordSourceCheckpoint({
    resetGeneration: "007",
    streams: [{ maxVersion: "00042", stream: "messages" }],
  });
  assert.equal(checkpoint.reset_generation, "7");
  const [firstStream] = checkpoint.streams;
  assert.ok(firstStream, "expected exactly one normalized stream");
  assert.equal(firstStream.max_version, "42");
});
...
test("recordSourceCheckpointsEqual compares by exact string value in sorted position", () => {
  const a = normalizeRecordSourceCheckpoint({
    resetGeneration: "1",
    streams: [{ maxVersion: "5", stream: "messages" }],
  });
  const bSame = normalizeRecordSourceCheckpoint({
    resetGeneration: "01",
    streams: [{ maxVersion: "05", stream: "messages" }],
  });
...
test("checkpoint comparison rejects null, malformed, and sparse persisted shapes symmetrically", () => {
  const dense = normalizeRecordSourceCheckpoint({
    resetGeneration: "1",
    streams: [{ maxVersion: "5", stream: "messages" }],
  });
  const sparseStreams: unknown[] = [];
  sparseStreams.length = 1;
  const sparse = { reset_generation: "1", streams: sparseStreams };
  const malformedShapes: readonly unknown[] = [
    null,
    { reset_generation: "1", streams: null },
    { reset_generation: "1", streams: "messages" },
    { reset_generation: "1", streams: [null] },
    { reset_generation: "1", streams: [{ max_version: 5, stream: "messages" }] },
    { reset_generation: "1", streams: [{ max_version: "5" }] },
    sparse,
  ];

  for (const malformed of malformedShapes) {
    assert.doesNotThrow(() => recordSourceCheckpointsEqual(malformed, dense));
    assert.equal(recordSourceCheckpointsEqual(malformed, dense), false);
    assert.equal(recordSourceCheckpointsEqual(dense, malformed), false);
  }
  assert.equal(recordSourceCheckpointsEqual(sparse, sparse), false, "matching holes are not checkpoint entries");
});

test("reconciliation repairs malformed persisted checkpoint JSON instead of throwing", async () => {
  initDb();
  try {
    const connectorInstanceId = "cin_checkpoint_malformed_repair";
    seedInstanceSqlite({ connectorInstanceId });
    const warm = await reconcileConnectorSummaryEvidence([connectorInstanceId]);
    assert.equal(warm.repaired, 1, "fixture: the missing evidence row is created");

    getDb()
      .prepare("UPDATE connector_summary_evidence SET record_checkpoint_json = ? WHERE connector_instance_id = ?")
      .run('{"reset_generation":"0","streams":[null]}', connectorInstanceId);

    const repaired = await reconcileConnectorSummaryEvidence([connectorInstanceId]);
    assert.equal(repaired.failed, 0);
    assert.equal(repaired.repaired, 1, "malformed durable checkpoint is classified as a mismatch");
    const row = getDb()
      .prepare("SELECT record_checkpoint_json FROM connector_summary_evidence WHERE connector_instance_id = ?")
      .get<{ record_checkpoint_json: string }>(connectorInstanceId);
    assert.ok(row);
    assert.deepEqual(JSON.parse(row.record_checkpoint_json), {
      reset_generation: "0",
      streams: [],
    });
  } finally {
...
test("a reset advances the read checkpoint even though the bare version vector can collide", async () => {
  ...
  try {
    ...
    assert.equal(
      recordSourceCheckpointsEqual(before, after),
      false,
      "checkpoint differs across the reset despite a colliding version vector"
    );
