...
if (POSTGRES_URL) {
  test("postgres byte-identical re-ingest does not allocate a new version", async () => {
    ...
    try {
      ...
      assert.equal(second.changed, false, "second byte-identical ingest must be suppressed as a no-op");
      ...
      assert.equal(Number(changes.rows[0]?.count || 0), 1, "record_changes must not gain a row for a no-op re-ingest");
  ...
  test("postgres unchanged reingest of an UNANCHORED current row self-heals (re-anchors at a new version)", async () => {
    ...
    const data = { amount: 100, id: recordKey };
    ...
    try {
      ...
      assert.equal(Number(preHist.rows[0]?.c ?? 0), 0, "cold is orphaned before resync");
      ...
      assert.equal(Number(counter.rows[0]?.max_version ?? 0), 10, "exactly one new version allocated");
      const cur = await postgresQuery(
        "SELECT version FROM records WHERE connector_instance_id = $1 AND stream = $2 AND record_key = $3",
        [connectorInstanceId, stream, recordKey]
      );
      assert.equal(Number(cur.rows[0]?.version ?? 0), 10, "current row re-anchored at the new version");
      ...
      assert.equal(Number(counterAfter.rows[0]?.max_version ?? 0), 10, "no version churn once anchored");
    } finally {
      try {
        await postgresQuery("DELETE FROM record_changes WHERE connector_instance_id = $1", [connectorInstanceId]);
        await postgresQuery("DELETE FROM records WHERE connector_instance_id = $1", [connectorInstanceId]);
  ...
  test("postgres re-ingest with whitespace-differing payload still suppressed (jsonb structural equality)", async () => {
    ...
    try {
      const storageTarget = { connectorId, connectorInstanceId };
      const data = { a: 1, b: 2, c: null, id: recordKey };
      ...
      const reordered = { a: 1, b: 2, c: null, id: recordKey };
      const third = await postgresIngestRecord(storageTarget, {
        data: reordered,
        emitted_at: "2026-05-26T12:00:00.000Z",
        key: recordKey,
      ...
      assert.equal(third.changed, false, "reordered-key payload that is structurally equal must be a no-op");
