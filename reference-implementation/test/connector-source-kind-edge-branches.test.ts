...
import { resolveEnrolledSourceKind, sourceKindFromManifestBindings } from "../server/routes/connector-source-kind.ts";

test("sourceKindFromManifestBindings returns null for a non-object bindings value", () => {
  // @ts-expect-error deliberately non-object bindings — proves the runtime
  // `typeof bindings !== "object"` guard, not a type-fixable input.
  assert.equal(sourceKindFromManifestBindings({ runtime_requirements: { bindings: "filesystem" } }), null);
  // @ts-expect-error deliberately non-object bindings — same guard, numeric case.
...
test("sourceKindFromManifestBindings returns null for array bindings without an own filesystem/browser key", () => {
  ...
  // @ts-expect-error deliberately array-shaped bindings — proves the
  // own-property-name guard rejects it despite `typeof [] === "object"`.
...
test("resolveEnrolledSourceKind accepts an explicitly null/undefined requested source kind", () => {
  ...
  assert.equal(resolveEnrolledSourceKind({ connectorId: "codex", manifest: fsManifest }), "local_device");
});
