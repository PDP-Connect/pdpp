...
function assertThrowsCode(fn: () => unknown, code: string, messagePart?: string) {
  assert.throws(fn, (err) => {
    assert.ok(isOAuthError(err), "thrown value is an Error");
...
test("requireAuthorizeString: throws invalid_request for missing / blank / non-string", () => {
  ...
  assertThrowsCode(
    () => requireAuthorizeString({ client_id: "   " }, "client_id"),
    "invalid_request",
    "client_id is required"
  );
  assertThrowsCode(
    () => requireAuthorizeString({ client_id: 42 }, "client_id"),
    "invalid_request",
    "client_id is required"
  );
...
test("validateAuthorizePkce: accepts response_type=code + S256 + a 43..128 char challenge", () => {
  assert.equal(
    validateAuthorizePkce({ codeChallenge: "a".repeat(43), codeChallengeMethod: "S256", responseType: "code" }),
    undefined
  );
  assert.equal(
    validateAuthorizePkce({ codeChallenge: "a".repeat(128), codeChallengeMethod: "S256", responseType: "code" }),
    undefined,
...
test("validateAuthorizePkce: a non-code response_type throws unsupported_response_type", () => {
  assertThrowsCode(
    () => validateAuthorizePkce({ codeChallenge: "a".repeat(43), codeChallengeMethod: "S256", responseType: "token" }),
...
test("validateAuthorizePkce: a non-S256 method throws invalid_request", () => {
  assertThrowsCode(
    () => validateAuthorizePkce({ codeChallenge: "a".repeat(43), codeChallengeMethod: "plain", responseType: "code" }),
    "invalid_request",
...
test("validateAuthorizePkce: an out-of-range or non-string code_challenge throws invalid_request", () => {
  assertThrowsCode(
    () => validateAuthorizePkce({ codeChallenge: "a".repeat(42), codeChallengeMethod: "S256", responseType: "code" }),
    "invalid_request",
    "code_challenge must be 43-128 characters"
  );
  assertThrowsCode(
    () => validateAuthorizePkce({ codeChallenge: "a".repeat(129), codeChallengeMethod: "S256", responseType: "code" }),
    "invalid_request",
    "code_challenge must be 43-128 characters"
  );
  assertThrowsCode(
    () =>
      validateAuthorizePkce({
        // @ts-expect-error codeChallenge is typed as `string`; this proves the
        // runtime guard rejects a non-string value even though the type
        // system would otherwise disallow constructing this call.
...
test("parseAuthorizeAuthorizationDetails: passes an array or object through", () => {
  ...
  assert.equal(
    parseAuthorizeAuthorizationDetails({ authorization_details: obj }),
    obj,
    "object passthrough (same ref)"
  );
});

test("parseAuthorizeAuthorizationDetails: parses a JSON-array string", () => {
  assert.deepEqual(parseAuthorizeAuthorizationDetails({ authorization_details: '[{"type":"pdpp_read"}]' }), [
    { type: "pdpp_read" },
  ]);
});
