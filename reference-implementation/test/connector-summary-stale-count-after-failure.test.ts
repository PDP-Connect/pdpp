const TOP_LEVEL_REGEX_1 = /"count_state":"known_zero"/;
...
test("a repair-candidate upsert failure with a checkpoint that already moved forward reads the stream as stale, never a fabricated known_zero", () =>
  withTempDb(async () => {
    ...
    let summary: ConnectorSummary | undefined;
    try {
      ...
      const rawRow = getDb()
        .prepare(
          "SELECT record_snapshot_state, stream_records_json FROM connector_summary_evidence WHERE connector_instance_id = ?"
        )
        .get(INSTANCE_ID);
      assert.ok(rawRow, "fixture evidence row exists before injected persistence failure");
      assert.ok(typeof rawRow.stream_records_json === "string", "fixture evidence JSON is stored as text");
      assert.notEqual(
        rawRow.record_snapshot_state,
        "current",
        "the durable row is genuinely marked non-current by the failure"
      );
      assert.match(
        rawRow.stream_records_json,
        TOP_LEVEL_REGEX_1,
        "fixture premise: the RAW stored JSON is still the untouched pre-failure known_zero — proving the bug exists at the storage layer before the read-side fix is applied"
      );
    ...
    assert.equal(
      projection?.status,
      "false",
      "ProjectionReliable is false (Sol P1.1, already closed) — the health gate closes correctly"
    );
    ...
    assert.equal(
      afterStream.record_count,
      0,
      "the prior known count is retained as a non-authoritative hint under count_state: stale, per spec.md"
    );
  }));
