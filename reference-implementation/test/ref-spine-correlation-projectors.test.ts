...
test("summaryToTrace: source derived from connector_id when no explicit source/source_kind", () => {
  const out = summaryToTrace(
    baseSummary({ connector_id: "connector-9", source: null, source_id: null, source_kind: null })
  );
  assert.deepEqual(out.source, { id: "connector-9", kind: "connector" }, `source: ${JSON.stringify(out.source)}`);
});
...
test("summaryToGrant: explicit source object wins over connector_id fallback", () => {
  const out = summaryToGrant(
    baseSummary({ connector_id: "connector-1", source: { id: "prov-1", kind: "provider_native" } })
  );
  assert.deepEqual(out.source, { id: "prov-1", kind: "provider_native" }, `source: ${JSON.stringify(out.source)}`);
});

test("summaryToGrant: source built from source_kind+source_id when no explicit source", () => {
  const out = summaryToGrant(
    baseSummary({ connector_id: "connector-1", source: null, source_id: "src-42", source_kind: "connector" })
  );
  assert.deepEqual(out.source, { id: "src-42", kind: "connector" }, `source: ${JSON.stringify(out.source)}`);
});

test("summaryToGrant: source is null when no source, no kind/id pair, and no connector_id", () => {
  const out = summaryToGrant(baseSummary({ connector_id: null, source: null, source_id: null, source_kind: null }));
...
test("summaryToRun: surface_failed with no wait reason falls back to surface status then default", () => {
  const fromStatus = summaryToRun(
    baseSummary({
      browser_surface_status: "evicted",
