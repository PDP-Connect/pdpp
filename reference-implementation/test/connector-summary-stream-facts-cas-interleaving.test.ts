...
test("fold-contract upgrade: a version-2 writer cannot overwrite the current fold-version's terminal map even at the same checkpoint", async () => {
  await withTempDb(async () => {
    ...
    assert.equal(oldBinaryWriteAccepted, false, "the version-2 CAS baseline cannot match the current-fold-version row");
    assert.deepEqual(
