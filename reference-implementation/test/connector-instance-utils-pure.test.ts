...
test("stableJson: object serialization is key-order-independent", () => {
  const a = stableJson({ a: 2, b: 1, c: { y: 2, z: 1 } });
  const b = stableJson({ a: 2, b: 1, c: { y: 2, z: 1 } });
...
test("makeConnectorInstanceSourceBindingKey: equal-but-reordered bindings hash identically", () => {
  const k1 = makeConnectorInstanceSourceBindingKey({ id: "gmail", kind: "connector" });
  ...
  const k3 = makeConnectorInstanceSourceBindingKey({ id: "amazon", kind: "connector" });
...
test("makeConnectorInstanceId: cin_ prefix + 24 hex chars, deterministic", () => {
  ...
  assert.match(id, REGEXP_1, "cin_ prefix + 24-hex slice");
...
test("parseSpineSourceShape: canonical {kind,id} wins over legacy/bare fields", () => {
  ...
  assert.deepEqual(out, { id: "canonical-id", kind: "connector" });
});

test("parseSpineSourceShape: legacy binding_kind used when no canonical kind/id", () => {
  assert.deepEqual(parseSpineSourceShape({ binding_kind: "connector", connector_id: "c1" }), {
    id: "c1",
    kind: "connector",
  });
  assert.deepEqual(parseSpineSourceShape({ binding_kind: "provider_native", provider_id: "p1" }), {
    id: "p1",
...
test("parseSpineSourceShape: bare connector_id/provider_id are mutually exclusive", () => {
  assert.deepEqual(parseSpineSourceShape({ connector_id: "c1" }), { id: "c1", kind: "connector" });
  assert.deepEqual(parseSpineSourceShape({ provider_id: "p1" }), { id: "p1", kind: "provider_native" });
...
test("parseSpineSourceShape: invalid canonical kind falls through, non-object rejects", () => {
  ...
  assert.equal(parseSpineSourceShape({ id: "x", kind: "weird" }), null);
...
test("deriveSpineSource: payload.source is consulted first", () => {
  const out = deriveSpineSource(
    { source: { id: "from-source", kind: "connector" }, source_binding: { connector_id: "from-binding" } },
    {}
  );
  assert.deepEqual(out, { id: "from-source", kind: "connector" });
});
...
test("deriveSpineSource: payload.source_binding used when source absent/unparseable", () => {
  ...
  assert.deepEqual(out, { id: "pb", kind: "provider_native" });
});

test("deriveSpineSource: bare payload connector_id used when no source/source_binding", () => {
  assert.deepEqual(deriveSpineSource({ connector_id: "bare-c" }, {}), { id: "bare-c", kind: "connector" });
});

test("deriveSpineSource: falls back to the row when payload yields nothing", () => {
  // Pure fallback: row source_kind + source_id.
  assert.deepEqual(deriveSpineSource({}, { source_id: "row-c", source_kind: "connector" }), {
    id: "row-c",
    kind: "connector",
  });
  // runtime actor fallback -> connector
  assert.deepEqual(deriveSpineSource(null, { actor_id: "act-1", actor_type: "runtime" }), {
    id: "act-1",
    kind: "connector",
  });
