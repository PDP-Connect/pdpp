...
import type { CollectionReportEntryLike, ManifestStreamLike } from "../runtime/connector-verdict-input.ts";
import { synthesizeConnectorVerdict } from "../runtime/connector-verdict-input.ts";
import type { ProgressEvidence, RenderedVerdict } from "../runtime/rendered-verdict.ts";
...
function deriveOldHeadline(snapshot: ConnectionHealthSnapshot): string {
...
function deriveNewHeadline(verdict: RenderedVerdict): string {
...
interface Fixture {
  assertions: (verdict: RenderedVerdict) => void;
  description: string;
  expectedClassification: "fixed_lie" | "deliberate_silence_correction";
  id: string;
 ...
 */
function classifyChange(fixture: Fixture, verdict: RenderedVerdict) {
  ...
  if (oldHeadline === newHeadline) {
    ...
    return { channel, classification, newHeadline, oldHeadline, reason: fixture.reason ?? "unchanged" };
  }

  // Headline changed — must be either a fixed lie or unexpected drift.
  if (fixture.expectedClassification === "fixed_lie") {
    return { channel, classification: "fixed_lie", newHeadline, oldHeadline, reason: fixture.reason };
  }
  if (fixture.expectedClassification === "deliberate_silence_correction") {
    return {
      channel,
      classification: "deliberate_silence_correction",
      newHeadline,
      oldHeadline,
...
const FIXTURES: Fixture[] = [
  ...
  // the 2532 recovered gaps; new routes them to detail only.
  {
    ...
    refresh: { backgroundSafe: false, interactionPosture: "credentials", recommendedMode: "automatic" },
    report: [
      {
        collected: 500,
        considered: 500,
  ...
  // action behind a green Healthy state.
  {
    ...
    report: [
      { collected: 200, considered: 200, coverage_condition: "complete", pending_detail_gaps: 0, stream: "orders" },
    ],
    runtimeOk: true,
    snapshot: {
  ...
  // new correctly routes to advisory (maintainer status, never owner attention).
  {
    ...
    refresh: null,
    report: [
      { collected: 50, considered: 100, coverage_condition: "terminal_gap", pending_detail_gaps: 0, stream: "records" },
    ],
    runtimeOk: true,
    snapshot: {
  ...
  // new caps channel at calm so a single runtime fault doesn't cascade as N alarms.
  {
    ...
    progress: null,
    reason:
      "Silence correction: runtime fault caps per-connection channel at calm (S4); one global indicator instead of N alarms",
    refresh: null,
    report: [
      { collected: 0, considered: 100, coverage_condition: "retryable_gap", pending_detail_gaps: 0, stream: "records" },
    ],
    runtimeOk: false,
    snapshot: {
      axes: {
...
test("shadow-comparison: no unexpected_drift across all fixtures", () => {
  ...
  for (const fixture of FIXTURES) {
    ...
    results.push({ description: fixture.description, id: fixture.id, ...result });
  ...
  for (const r of results) {
    const icon =
      r.classification === "unexpected_drift"
        ? "✖ DRIFT"
        : // biome-ignore lint/style/noNestedTernary: the compact fixture mapping makes the tested precedence explicit.
          r.classification === "fixed_lie"
          ? "✔ FIXED_LIE"
          : // biome-ignore lint/style/noNestedTernary: the compact fixture mapping makes the tested precedence explicit.
            r.classification === "deliberate_silence_correction"
            ? "✔ SILENCE"
            : "✔ NO_CHANGE";
