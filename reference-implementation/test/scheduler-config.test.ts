...
// `resolveNonNegativeMsOrInfinity` backs both `resolveMaxRunWallClockMs`'s
// shape and `resolveDispatchLivenessCeilingMs` in runtime/scheduler.ts.
// A pure function is the right place to prove the "resolved to 0 (disabled)"
// vs "resolved to the real default" distinction: an end-to-end scheduler
// test would need to wait out the real default (30 minutes for the
// dispatch-liveness ceiling) to draw that line with real timers, which this
// avoids entirely by asserting the resolved value directly.
