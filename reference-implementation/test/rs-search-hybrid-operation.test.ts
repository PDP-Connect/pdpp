...
import {
  ...
  type SearchHybridSubRequestParams,
} from "../operations/rs-search-hybrid/index.ts";
import { ManifestReadAuthorityError } from "../server/manifest-read-authority.ts";

const ownerActor: SearchHybridActor = { kind: "owner", subject_id: "subj_owner" };
const clientActor: SearchHybridActor = {
  client_id: "client_x",
  grant_id: "grant_y",
  kind: "client",
...
function lexicalHit(overrides: Partial<SearchHybridSourceResult> = {}): SearchHybridSourceResult {
  return {
    connector_id: "acme",
...
function semanticHit(overrides: Partial<SearchHybridSourceResult> = {}): SearchHybridSourceResult {
  return {
    connector_id: "acme",
    emitted_at: "2026-04-01T00:00:00Z",
...
function makeDeps(overrides: Partial<SearchHybridDependencies> = {}): SearchHybridDependencies & {
  _lexicalCalls: SearchHybridSubRequestParams[];
  _semanticCalls: SearchHybridSubRequestParams[];
} {
  const lexicalCalls: SearchHybridSubRequestParams[] = [];
  const semanticCalls: SearchHybridSubRequestParams[] = [];
  const base: SearchHybridDependencies = {
    runLexical: (params) => {
...
test("parseSearchHybridParams rejects cursor explicitly (v1 hybrid has no cursor support)", () => {
  ...
  assert.throws(
    () => parseSearchHybridParams({ cursor: "anything", q: "foo" }),
    (err) => {
      assert.ok(err instanceof SearchHybridRequestError);
      assert.equal(err.code, "invalid_request");
...
test("parseSearchHybridParams rejects every explicit forbidden parameter with param set", () => {
  ...
  for (const key of forbidden) {
    assert.throws(
      ...
      (err) => {
        assert.ok(err instanceof SearchHybridRequestError, `${key} should throw a typed error`);
...
test("parseSearchHybridParams clamps and defaults limit", () => {
  ...
  assert.equal(parseSearchHybridParams({ limit: "500", q: "foo" }).limit, 100);
  assert.equal(parseSearchHybridParams({ limit: "0", q: "foo" }).limit, 25);
  assert.equal(parseSearchHybridParams({ limit: "7", q: "foo" }).limit, 7);
});

test("parseSearchHybridParams normalizes streams (string and array)", () => {
  assert.deepEqual(parseSearchHybridParams({ q: "foo", streams: "posts" }).streams, ["posts"]);
  assert.deepEqual(parseSearchHybridParams({ q: "foo", streams: ["posts", "comments"] }).streams, [
    "posts",
    "comments",
  ]);
  assert.deepEqual(parseSearchHybridParams({ q: "foo", "streams[]": "posts" }).streams, ["posts"]);
...
test("parseSearchHybridParams requires filter[...] to bind to exactly one streams[]", () => {
  assert.throws(
    () => parseSearchHybridParams({ filter: { x: 1 }, q: "foo" }),
    (err) => {
      assert.ok(err instanceof SearchHybridRequestError);
      assert.equal(err.code, "invalid_request");
  ...
  const ok = parseSearchHybridParams({ filter: { x: 1 }, q: "foo", streams: "posts" });
...
test("operation invokes BOTH runners with parsed sub-request params verbatim", async () => {
  ...
  await executeSearchHybrid(
    {
      actor: ownerActor,
      query: { limit: "7", q: "overdraft", "streams[]": "posts" },
    },
    deps
  );
  ...
  for (const call of [deps._lexicalCalls[0], deps._semanticCalls[0]]) {
    assert.ok(call, "expected a captured sub-request call");
...
test("errors from either runner propagate unchanged (e.g. grant_stream_not_allowed from semantic)", async () => {
  ...
  await assert.rejects(
    () => executeSearchHybrid({ actor: clientActor, query: { q: "overdraft", streams: "forbidden" } }, deps),
    (err: unknown) => {
...
test("overlapping records are deduped; matched_fields and scores are merged across sources", async () => {
  ...
  const deps = makeDeps({
    runLexical: () => ({
      envelope: {
        data: [
          lexicalHit({ matched_fields: ["title"], record_key: "rec_1" }),
          lexicalHit({ matched_fields: ["title"], record_key: "rec_2" }),
        ],
      },
    }),
    runSemantic: () => ({
      envelope: {
        data: [
          semanticHit({ matched_fields: ["selftext"], record_key: "rec_1" }),
          semanticHit({ matched_fields: ["selftext"], record_key: "rec_3" }),
        ],
      },
    }),
  });
  const out = await executeSearchHybrid({ actor: ownerActor, query: { q: "overdraft" } }, deps);
  ...
  const [rec1] = out.envelope.data;
  assert.ok(rec1, "expected the first merged hit");
  assert.deepEqual(rec1.retrieval_sources, ["lexical", "semantic"]);
  assert.deepEqual(rec1.matched_fields, ["title", "selftext"]);
  assert.ok(rec1.scores, "expected per-source scores");
  assert.ok(rec1.scores.lexical, "expected a lexical score");
  assert.equal(rec1.scores.lexical.kind, "bm25");
  assert.ok(rec1.scores.semantic, "expected a semantic score");
  ...
  assert.ok(rec2, "expected the lexical-only merged hit");
  assert.deepEqual(rec2.retrieval_sources, ["lexical"]);
  assert.ok(rec2.scores, "expected per-source scores");
  assert.ok(rec2.scores.lexical, "expected a lexical score");
  ...
  assert.ok(rec3, "expected the semantic-only merged hit");
  assert.deepEqual(rec3.retrieval_sources, ["semantic"]);
  assert.ok(rec3.scores, "expected per-source scores");
  assert.ok(rec3.scores.semantic, "expected a semantic score");
...
test("matched_fields union deduplicates repeated field names (lexical-first discovery order)", async () => {
  ...
  assert.ok(item, "expected the merged hit");
  assert.deepEqual(item.matched_fields, ["title", "selftext", "body"]);
});
...
test("first non-empty snippet is preserved on dedup", async () => {
  ...
  const out = await executeSearchHybrid({ actor: ownerActor, query: { q: "x" } }, deps);
  // Lexical surfaced the record first → its snippet wins.
  const [item] = out.envelope.data;
  assert.ok(item, "expected the merged hit");
  assert.deepEqual(item.snippet, {
    field: "title",
    text: "<mark>x</mark>",
  });
});
...
test("semantic-only hit retains semantic snippet when lexical did not surface the record", async () => {
  ...
  const [item] = out.envelope.data;
  assert.ok(item, "expected the merged hit");
  assert.deepEqual(item.snippet, { field: "body", text: "verbatim" });
  assert.deepEqual(item.retrieval_sources, ["semantic"]);
});
...
test('every emitted hit carries retrieval_mode:"hybrid"', async () => {
  const deps = makeDeps();
  const out = await executeSearchHybrid({ actor: ownerActor, query: { q: "foo" } }, deps);
...
test("limit is applied after dedup; merged list trims and reports has_more", async () => {
  ...
  const out = await executeSearchHybrid({ actor: ownerActor, query: { limit: "2", q: "foo" } }, deps);
...
test("envelope never carries next_cursor in v1", async () => {
  const deps = makeDeps({
    runLexical: () => ({
      envelope: {
        data: Array.from({ length: 10 }, (_, i) => lexicalHit({ record_key: `lex_${i}` })),
      },
    }),
  });
  const out = await executeSearchHybrid({ actor: ownerActor, query: { limit: "2", q: "foo" } }, deps);
...
test("per-source scores are forwarded under `scores[source]` and individual hits do NOT emit a top-level `score` field", async () => {
  ...
  for (const hit of out.envelope.data) {
    ...
    for (const [source, scoreObj] of Object.entries(hit.scores)) {
      assert.ok(source === "lexical" || source === "semantic", "scores map keys must be lexical or semantic");
...
test("disclosure data block carries query_shape:search_hybrid plus per-source counts", async () => {
  const deps = makeDeps({
    runLexical: () => ({
      envelope: {
        data: [lexicalHit({ record_key: "lex_1" }), lexicalHit({ record_key: "lex_2" })],
      },
    }),
    runSemantic: () => ({
      envelope: {
...
test("disclosure mode tracks the actor kind (client)", async () => {
  const deps = makeDeps();
  const out = await executeSearchHybrid({ actor: clientActor, query: { q: "foo" } }, deps);
...
test("filter[...] bound to exactly one streams[] is forwarded into both sub-requests", async () => {
  ...
  await executeSearchHybrid(
    {
      actor: ownerActor,
      query: { filter: { source_created_at: { gte: "x" } }, q: "foo", streams: "posts" },
    },
    deps
  );
  for (const call of [deps._lexicalCalls[0], deps._semanticCalls[0]]) {
