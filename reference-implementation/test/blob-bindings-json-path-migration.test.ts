...
const TOP_LEVEL_REGEX_1 = /CHECK constraint failed/;
const TOP_LEVEL_REGEX_2 = /UNIQUE constraint failed: blobs\.sha256/;
const TOP_LEVEL_REGEX_3 = /UNIQUE constraint failed/;
...
const Database = require(["better", "sqlite3"].join("-")) as SqliteDatabaseConstructor;

function defaultAccountInstanceId(connectorId: string) {
...
function indexes(raw: SqliteDatabase, table: string): { name: string }[] {
  return raw.prepare(`PRAGMA index_list(${table})`).all() as { name: string }[];
}
...
test("fresh DB has json_path in blob_bindings PK + CHECK constraint", () => {
  ...
  try {
    ...
    const row = raw.prepare("SELECT json_path FROM blob_bindings").get() as { json_path: string };
    ...
    raw
      ...
      .run("blob_sha256_aa", "c1", defaultAccountInstanceId("c1"), "s1", "r1", "/output_preview");
    const all = (
      raw.prepare("SELECT json_path FROM blob_bindings ORDER BY json_path").all() as { json_path: string }[]
    ).map((r) => r.json_path);
...
test("legacy DB is rebuilt: existing rows backfilled with @record, count preserved", () => {
  ...
  `);
  assert.equal((raw.prepare("SELECT COUNT(*) AS n FROM blob_bindings").get() as { n: number }).n, 3);
  assert.ok(!tableColumns(raw, "blob_bindings").includes("json_path"));
  raw.close();
  ...
  try {
    ...
    const all = verifyRaw.prepare("SELECT blob_id, json_path FROM blob_bindings ORDER BY blob_id").all() as {
      blob_id: string;
      json_path: string;
    }[];
    ...
    // Row count preserved.
    assert.equal((verifyRaw.prepare("SELECT COUNT(*) AS n FROM blob_bindings").get() as { n: number }).n, 3);
  } finally {
    verifyRaw.close();
  }
});
...
test("migration is idempotent: second initDb() does not error or duplicate", () => {
  ...
  try {
    assert.equal((v.prepare("SELECT COUNT(*) AS n FROM blob_bindings").get() as { n: number }).n, 1);
...
test("uniq_blobs_sha256 index exists on fresh DB and after migration", () => {
  ...
  try {
    ...
    assert.ok(names.includes("uniq_blobs_sha256"), `uniq_blobs_sha256 missing from blobs indexes: ${names.join(", ")}`);
  } finally {
    raw.close();
  }
});
