...
// Minimal structural shapes for the fastify-backed req/res the untyped
// `server/transport.js` passes into route handlers. Handlers below only
// touch these members.
interface TestHandlerReq {
  body?: unknown;
  headers?: { authorization?: string };
}

interface TestHandlerRes {
  json: (body: unknown) => void;
  redirect: (status: number, url: string) => void;
  send: (body?: unknown) => void;
  status: (code: number) => TestHandlerRes;
}
...
test("applyRequestValidation produces an OAuth-shaped envelope for OAuth manifests", () => {
  ...
  const captured: { status: number; body: Record<string, unknown> | null; headers: Record<string, unknown> } = {
    body: null,
    headers: {},
    status: 0,
  };
...
test("applyRequestValidation produces a PDPP-shaped envelope for PDPP manifests", async () => {
  ...
  assert.ok(pdppManifest, "expected at least one reference-contract manifest declaring pdpp/common/PdppError at 400");
  ...
  const error = captured.body.error as Record<string, unknown>;
  ...
  assert.throws(
    () =>
      app.get(
        ...
        (_req: TestHandlerReq, res: TestHandlerRes) => {
          res.json({});
        }
      ),
...
test("allowlisted route runs route-level auth middleware BEFORE request validation", async () => {
  ...
  const requireAuth = (req: TestHandlerReq, res: TestHandlerRes, next: () => void) => {
...
test("allowlisted route accepts a well-formed body and reaches the handler", async () => {
  ...
  app.patch(
    ...
    { contract: "refSetConnectionDisplayName" },
    (req: TestHandlerReq, res: TestHandlerRes) => {
...
test("non-canary JSON response is not transformed even when it would violate the manifest", async () => {
  ...
  app.get("/non-canary-listconnectors", { contract: "listConnectors" }, (_req: TestHandlerReq, res: TestHandlerRes) => {
    res.json(drift);
  });
  await app.fastify.ready();
...
test("redirects, 204 responses, and string payloads are not run through canary validation", async () => {
  ...
  app.get("/canary-redirect", { contract: "getRsDiscoveryIndex" }, (_req: TestHandlerReq, res: TestHandlerRes) => {
    res.redirect(302, "https://example.test/elsewhere");
  });

  app.get("/canary-empty", { contract: "getRsDiscoveryIndex" }, (_req: TestHandlerReq, res: TestHandlerRes) => {
    res.status(204).send();
  });

  app.get("/canary-text", { contract: "getRsDiscoveryIndex" }, (_req: TestHandlerReq, res: TestHandlerRes) => {
...
test("operation modules do not import the reference-contract runtime", async () => {
  const { discoverOperationModules, assertOperationBoundary } = await import("./helpers/operation-boundary.ts");
