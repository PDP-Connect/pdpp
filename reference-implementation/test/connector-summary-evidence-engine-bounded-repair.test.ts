...
function seedConnections(n: number): void {
...
test("a candidate a bounded pass skips is genuinely deferred, not lost — the next unbounded pass repairs it", () =>
  withTempDb(async () => {
    ...
    assert.ok(rowCount, "count query returns its aggregate row");
    ...
    assert.equal(
      unbounded.repaired,
      6,
      "the second, unbounded pass repairs exactly the 6 previously-skipped candidates"
    );
    ...
    assert.ok(finalRowCount, "count query returns its aggregate row");
...
test("an unbounded call (no options, the default) never skips — every existing consumer is unaffected", () =>
  withTempDb(async () => {
    ...
    assert.equal(
      result.skipped,
      0,
      "the default (unbounded) behavior is preserved for every caller that does not opt into a bound"
    );
  }));
