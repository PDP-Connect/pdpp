...
test("runConnector withholds an unrelated DETAIL_COVERAGE shortfall during a certified stream failure", async () => {
  ...
  const rsUrl = `http://localhost:${server.rsPort}`;
  const manifest = {
    connector_id: "runtime-stream-isolation-failure-and-coverage-shortfall-test",
    streams: [
...
test("runConnector withholds a manifest-declared PARENT state_stream shared with a failed CHILD data stream", async () => {
  ...
  const rsUrl = `http://localhost:${server.rsPort}`;
  const manifest = {
    connector_id: "runtime-stream-isolation-parent-state-stream-test",
    streams: [
      {
...
test("runConnector withholds a state_stream the connector's own DETAIL_COVERAGE associated with the failed data stream", async () => {
  ...
  const rsUrl = `http://localhost:${server.rsPort}`;
  const manifest = {
    connector_id: "runtime-stream-isolation-detail-coverage-state-stream-test",
    streams: [
      {
        name: "child_a",
        primary_key: ["id"],
