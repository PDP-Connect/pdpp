...
async function eventually<T>(assertion: () => T | Promise<T>, label: string, timeoutMs = 3000): Promise<T> {
...
function writeReleasableConnector(tmpDir: string) {
...
function writeImmediateConnector(tmpDir: string) {
  const connectorPath = join(tmpDir, "connector.mjs");
  const attemptsPath = join(tmpDir, "attempts.log");
...
test("two independent scheduler processes sharing one durable store admit exactly one run, incumbent row is never overwritten", async (t) => {
  ...
  let schedulerA: Scheduler | null = null;
  let schedulerB: Scheduler | null = null;
  t.after(() => {
    schedulerA?.stop();
    schedulerB?.stop();
  ...
  const [survivingRow] = rowsAfterConflict;
  assert.ok(survivingRow, "expected exactly one surviving durable row");
  assert.equal(
    survivingRow.run_id,
    incumbentRow.run_id,
    "the incumbent row must survive process B's conflicting attempt"
  );
  ...
  assert.equal(
    readAttempts(connector.attemptsPath).length,
    1,
    "exactly one connector process attempt across both schedulers"
  );
...
test("scheduler restart while an incumbent run is genuinely still live does not launch a second child", async (t) => {
  ...
  let originalScheduler: Scheduler | null = null;
  let restartedScheduler: Scheduler | null = null;
  t.after(() => {
  ...
  assert.equal(
    readAttempts(connector.attemptsPath).length,
    1,
    "restart must not spawn a second connector process while the incumbent is genuinely live"
  );
...
test("manual controller.runNow racing a scheduled tick for the same connector instance admits exactly one", async (t) => {
  ...
  const manualHang = new Promise<RuntimeRunConnectorResult>((resolve) => {
    releaseManual = () => resolve({ records_emitted: 0, status: "succeeded" });
  });
  ...
  assert.equal(
    ...
    false,
    "scheduled tick must not spawn a connector child while the manual run is live"
  );
  ...
  assert.equal(
    survivingRow.run_id,
    durableRowAfterManual.run_id,
    "the manual incumbent row must survive the scheduled tick's conflicting attempt"
  );

  assert.ok(releaseManual, "expected releaseManual to be assigned by the Promise executor");
  releaseManual();
  await manualPromise;
...
test("next scheduled dispatch is anchored to a run's actual completion time, observed via a real second child start", async (t) => {
  ...
  await eventually(
    ...
    "did not observe two real scheduled completions",
    5000
  );
  ...
  assert.ok(first && second, "expected at least two completed runs");
  ...
  assert.ok(firstAttempt !== undefined && secondAttempt !== undefined, "expected at least two attempt timestamps");
  const gapBetweenSpawns = secondAttempt - firstAttempt;
  assert.ok(
