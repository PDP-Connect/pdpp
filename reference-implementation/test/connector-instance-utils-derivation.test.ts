...
test("stableJson: null->{}, scalars via JSON, arrays recurse, object keys are SORTED", () => {
  ...
  const a = stableJson({ a: 2, b: 1, c: 3 });
  const b = stableJson({ a: 2, b: 1, c: 3 });
  ...
  assert.equal(stableJson({ z: { x: 2, y: 1 } }), '{"z":{"x":2,"y":1}}');
});
...
test("makeConnectorInstanceId: cin_ prefix, 24-hex body, deterministic, input-sensitive", () => {
  ...
  assert.match(id, REGEXP_1, `unexpected id shape: ${id}`);
...
test("makeConnectorInstanceSourceBindingKey: order-independent (rides stableJson)", () => {
  ...
  const k2 = makeConnectorInstanceSourceBindingKey({ a: 1, b: 2 });
...
test("parseSpineSourceShape: canonical kind+id wins; non-object -> null", () => {
  ...
  // Canonical shape.
  assert.deepEqual(parseSpineSourceShape({ id: "github", kind: "connector" }), {
    id: "github",
    kind: "connector",
  });
  // A canonical kind that is not in the allowlist falls through to null (no
  // legacy/single-id fields present).
  assert.equal(parseSpineSourceShape({ id: "x", kind: "bogus" }), null);
});
...
test("parseSpineSourceShape: single-id inference requires EXACTLY one of connector_id / provider_id", () => {
  assert.deepEqual(parseSpineSourceShape({ connector_id: "gh" }), { id: "gh", kind: "connector" });
  assert.deepEqual(parseSpineSourceShape({ provider_id: "plaid" }), { id: "plaid", kind: "provider_native" });
...
test("deriveSpineSource: payload.source has precedence over row columns", () => {
  ...
  assert.deepEqual(
    deriveSpineSource(
      { source: { id: "gh", kind: "connector" } },
      { source_id: "plaid", source_kind: "provider_native" }
    ),
    { id: "gh", kind: "connector" }
  );

  // payload.source_binding is the second payload avenue.
  assert.deepEqual(deriveSpineSource({ source_binding: { connector_id: "gh" } }, {}), { id: "gh", kind: "connector" });

  // payload single-id inference (no source/source_binding keys present).
  assert.deepEqual(deriveSpineSource({ provider_id: "plaid" }, {}), { id: "plaid", kind: "provider_native" });
});
...
test("deriveSpineSource: falls back to row columns and the runtime-actor special case", () => {
  ...
  assert.deepEqual(deriveSpineSource(null, { source_id: "gh", source_kind: "connector" }), {
    id: "gh",
    kind: "connector",
  });
  ...
  assert.deepEqual(deriveSpineSource({}, { actor_id: "gh", actor_type: "runtime" }), { id: "gh", kind: "connector" });
  assert.equal(deriveSpineSource({}, { actor_id: "gh", actor_type: "user" }), null);
