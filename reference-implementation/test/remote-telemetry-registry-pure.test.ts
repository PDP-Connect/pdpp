...
test("register: falsy runId or non-function callback returns a no-op unsubscribe and registers nothing", () => {
  const off1 = registerRemoteTelemetrySink("", () => undefined);
  const off2 = Reflect.apply(registerRemoteTelemetrySink, undefined, [uniqueRunId("badcb"), "not-a-function"]);
...
test("unsubscribe after drop is safe (registry already pruned)", () => {
  ...
  const off = registerRemoteTelemetrySink(runId, () => undefined);
  dropRemoteTelemetry(runId);
