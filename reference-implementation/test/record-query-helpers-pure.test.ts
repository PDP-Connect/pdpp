...
function expectInvalidSort(fn: () => unknown): void {
...
test("validateCanonicalSort: sign prefix controls direction on the cursor field", () => {
  assert.deepEqual(validateCanonicalSort("emitted_at", streamCursor), { direction: "ASC", field: "emitted_at" });
  assert.deepEqual(validateCanonicalSort("-emitted_at", streamCursor), { direction: "DESC", field: "emitted_at" });
});
...
test("resolveListOrder: canonical sort direction wins over absent order", () => {
  assert.equal(resolveListOrder(null, { direction: "ASC", field: "emitted_at" }), "ASC");
  assert.equal(resolveListOrder("", { direction: "DESC", field: "emitted_at" }), "DESC");
});

test("resolveListOrder: sort and matching legacy order is fine", () => {
  assert.equal(resolveListOrder("asc", { direction: "ASC", field: "emitted_at" }), "ASC");
});

test("resolveListOrder: sort and DISAGREEING legacy order is invalid_sort", () => {
  expectInvalidSort(() => resolveListOrder("desc", { direction: "ASC", field: "emitted_at" }));
});
...
test("validateWindowKind: accepts none/exact only, rejects estimated (not a window grade)", () => {
  ...
  assert.throws(
    () => validateWindowKind("estimated"),
    // biome-ignore lint/performance/useTopLevelRegex: test assertion patterns remain colocated with the assertion they explain.
    /window must be one of/,
    "estimated is valid for count but NOT window"
  );
  // biome-ignore lint/performance/useTopLevelRegex: test assertion patterns remain colocated with the assertion they explain.
...
test("rejectListOnlyParamsForChangesFeed: no list-only params is a no-op", () => {
  ...
  assert.doesNotThrow(() => rejectListOnlyParamsForChangesFeed({ count: "", sort: "" }));
...
test("rejectListOnlyParamsForChangesFeed: singular vs plural verb agreement", () => {
  ...
  // biome-ignore lint/performance/useTopLevelRegex: test assertion patterns remain colocated with the assertion they explain.
  assert.throws(() => rejectListOnlyParamsForChangesFeed({ count: "b", sort: "a" }), /sort, count are not supported/);
});
...
test("attachRequestWarningsToResponse: appends to existing meta.warnings, preserves other meta", () => {
  const response: Record<string, unknown> = {
    data: [],
    meta: { count: { kind: "exact", value: 3 }, warnings: [{ code: "a" }] },
  };
  attachRequestWarningsToResponse(response, [{ code: "b" }]);
  const meta = response.meta as Record<string, unknown>;
  assert.deepEqual(meta.warnings, [{ code: "a" }, { code: "b" }], "appends, preserving existing");
  assert.deepEqual(meta.count, { kind: "exact", value: 3 }, "preserves sibling meta.count");
});
...
test("mergeMetaCount: sets count while preserving warnings and window", () => {
  const merged = mergeMetaCount(
    { warnings: [{ code: "x" }], window: { kind: "exact" } },
    { kind: "estimated", value: 9 }
  );
