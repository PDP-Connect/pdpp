...
import { createPostgresConnectorInstanceStore, makeConnectorInstanceSourceBindingKey } from '../server/stores/connector-instance-store.js';

const FIXTURE_PATH = fileURLToPath(new URL('./fixtures/connector-instance-two-process-race-fixture.mjs', import.meta.url));
const DELAY_MS = 250;
const ATTEMPTS = 4;
...
function spawnFixture() {
  const child = spawn(process.execPath, [FIXTURE_PATH], {
    env: {
      ...
      // server/stores/connector-instance-store.js.
...
async function seedConnector(connectorId) {
  await postgresQuery(
...
async function cleanupIdentity(ownerSubjectId, connectorId) {
...
test(
  'two genuine OS processes racing deleteConnection and upsert for the SAME identity never resurrect a deleted connection (Postgres advisory-lock discriminator)',
  ...
  async () => {
    ...
    try {
      for (let attempt = 1; attempt <= ATTEMPTS; attempt += 1) {
        ...
        const bindingKey = makeConnectorInstanceSourceBindingKey({ kind: 'local_device', local_binding_name: localBindingName });
        ...
        const original = await store.upsert({
          ...
          sourceBinding: { kind: 'local_device', device_id: 'dexp_race_a', local_binding_name: localBindingName, source_instance_id: 'dsrc_race_a' },
          createdAt: NOW,
          updatedAt: NOW,
        });
        ...
        try {
          ...
          fixture.child.stdin.write(
            `${JSON.stringify({
              ...
              connectorId,
              sourceBindingKey: bindingKey,
              sourceBinding: { kind: 'local_device', device_id: 'dexp_race_b', local_binding_name: localBindingName, source_instance_id: 'dsrc_race_b' },
              now: LATER,
            })}\n`,
          );
          ...
          assert.equal(deleteError, null, `attempt ${attempt}: delete must not throw when racing a concurrent upsert for the same identity — ${deleteError?.message}`);
          ...
          assert.equal(tombstone.rows.length, 1, `attempt ${attempt}: the tombstone must be present after the delete completes`);
        } finally {
