...
if (POSTGRES_URL) {
  async function cleanup(connectorInstanceId: string): Promise<void> {
    try {
  ...
  async function readCounter(connectorInstanceId: string, stream: string): Promise<number | null> {
  ...
  async function readMaxChangeVersion(connectorInstanceId: string, stream: string): Promise<number | null> {
    const r = await postgresQuery(
      "SELECT MAX(version)::bigint AS v FROM record_changes WHERE connector_instance_id = $1 AND stream = $2",
      [connectorInstanceId, stream]
    );
    return r.rows[0]?.v === null || r.rows[0]?.v === undefined ? null : Number(r.rows[0].v);
  }
  ...
  test("lagging version_counter beneath existing history/current allocates above the floor", async () => {
    ...
    try {
      ...
      const data = { id: recordKey, state: "open", title: "Original" };
      ...
      assert.equal(
        await readMaxChangeVersion(connectorInstanceId, stream),
        terminalVersion,
        "history seeded at terminal"
      );
      ...
      const versions = changeRows.rows.map((r) => Number((r as { version: unknown }).version));
      assert.deepEqual(
        versions,
  ...
  test("self-heal of an unanchored row clears a lagging counter without re-using a version", async () => {
    ...
    try {
      ...
      assert.equal(Number(anchor.rows[0]?.version ?? 0), hotVersion + 1);
      ...
      assert.equal(Number(hotAnchor.rows[0]?.version ?? 0), hotVersion);
  ...
  test("no-regression: in-sync counter still allocates monotonically by one", async () => {
    ...
    try {
      ...
      const second = await postgresIngestRecord(storageTarget, {
        data: { ...base, n: 1 },
        emitted_at: "2026-05-26T12:01:00.000Z",
        key: recordKey,
        op: "upsert",
        stream,
      });
      ...
      const third = await postgresIngestRecord(storageTarget, {
        data: { ...base, n: 2 },
        emitted_at: "2026-05-26T12:02:00.000Z",
        key: recordKey,
        op: "upsert",
        stream,
      });
