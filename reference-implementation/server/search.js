...
import {
  ...
  assertOwnerSearchFilterAuthority,
...
let lexicalBackfillPhaseHook = null;

/** Test-only phase seam for deterministic registration/backfill ordering. */
export function __setLexicalBackfillPhaseHookForTest(hook) {
  lexicalBackfillPhaseHook = typeof hook === 'function' ? hook : null;
}

async function maybeLexicalBackfillPhaseForTest(point, context) {
 ...
 * not the JSON string.
 */
export async function lexicalIndexUpsert({ connectorId, connectorInstanceId, stream, recordKey, data, declaredFields }) {
...
export async function lexicalIndexBackfillForManifest({ manifest, log = () => {}, signal = null } = {}) {
  ...
  try {
    ...
    for (const connectorInstanceId of connectorInstanceIds) {
      ...
      progressJob = await withConnectorInstanceWrite(
        connectorInstanceId,
        () => backfillLexicalConnectorInstance(
          connectorId,
          connectorInstanceId,
          manifest.streams,
          progressJob,
