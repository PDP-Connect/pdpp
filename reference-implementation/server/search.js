...
export async function lexicalIndexUpsert({ connectorId, stream, recordKey, data }) {
  ...
  for (const field of declared) {
    ...
    await db.query(sql`
      ...
      VALUES(${connectorId}, ${stream}, ${recordKey}, ${field}, ${value})
    `);
  }
}
    ...
    for (const field of entry.searchableFields) {
      ...
      const rows = await db.query(sql`
        SELECT
          ...
          ${allowsSnippets
            ? sql`snippet(lexical_search_index, 4, '', '', '…', 16)`
            : sql`NULL`}                          AS snippet_text,
        ...
        WHERE lsi.connector_id = ${connectorId}
          AND lsi.stream       = ${entry.streamName}
          AND lsi.field        = ${field}
          AND lsi.text MATCH   ${q}
          AND r.deleted = 0
        ORDER BY score ASC
        LIMIT 200
      `);
...
async function persistSnapshot(snapshot) {
  ...
  await db.query(sql`
    ...
    VALUES(${snapshot.snapshot_id}, ${snapshot.query}, ${snapshot.plan_hash}, ${JSON.stringify(snapshot.results)})
  `);
}
