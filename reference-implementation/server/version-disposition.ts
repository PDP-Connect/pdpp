...
/** A reference-maintained `{ connector, stream }` list entry (with optional metadata). */
...
function isPointInTimeRealField(connector: string, stream: string | undefined): boolean {
  return POINT_IN_TIME_REAL_FIELD_STREAMS.some((entry) => entry.connector === connector && entry.stream === stream);
}

function isRecurringSnapshot(connector: string, stream: string | undefined): boolean {
