...
export function createSqliteDeviceExporterStore() {
  return {
    ...
    // Atomically consume the enrollment code AND — only if this attempt wins
    // the code-consume race, and only for a genuinely new device
    // (`params.isNewDevice`, i.e. `resolveOrCreateEnrollmentDevice`'s
    // `adopted: false` branch) — supersede (revoke) every OTHER active
    // already-completed device still durably bound to the same (owner,
    // connector, source_kind, local_binding_id). A completed binding is
    // never adopted, so a new device here is genuinely new; any other active
    // device sharing the binding is now stale: its enrollment code is
    // already spent, so it can never again be adopted or heartbeat, yet its
    // `device_source_instances` row would otherwise sit "trusted but
    // never-heartbeated" forever, poisoning the connector_instance's
    // outbox-axis aggregation to `unknown` (see
    // fix-fleet-local-collector-evidence-completeness).
    //
    // Completion (consume) and supersession commit as ONE transaction, never
    // two separate writes: consume + supersede wrapped in writeTransaction
    // means a throw anywhere inside rolls back BOTH. If `consumeEnrollmentCode`
    // does not win the race (a concurrent attempt already consumed it), the
    // method returns `false` immediately WITHOUT touching any prior device —
    // a losing or failed replacement enrollment can never revoke the device
    // it was trying to replace. Superseding is folded into the SAME
    // transaction as consume (not a separate call after it returns) so a
    // throw during the revoke cascade also rolls back the consume, instead of
    // leaving the code consumed with a half-applied supersession.
    //
    // Each superseded device is revoked through the SAME cascade an
    // owner-initiated revoke uses — never a raw status flip — which safely
    // spares the connector_instance because the fresh device's own
    // non-revoked source-instance still references it. SQLite: single-writer
    // connection, synchronous — writeTransaction still wraps this so a throw
    // mid-sequence rolls back cleanly rather than leaving a partial write.
    consumeEnrollmentCodeAndSupersedePriorDevices(params: {
      ...
      return writeTransaction(() => {
        ...
        const superseded = allowUnboundedReadAcknowledged<{ device_id: string; source_instance_id: string }>(
          ...
          [params.ownerSubjectId, params.connectorId, params.sourceKind, params.localBindingId, params.deviceId]
        );
        for (const row of superseded) {
          exec(referenceQueries.deviceExportersRevokeDevice, [params.now, params.now, row.device_id]);
          exec(referenceQueries.deviceExportersRevokeCredentialsForDevice, [params.now, row.device_id]);
          exec(referenceQueries.deviceExportersRevokeSourceInstancesForDevice, [params.now, params.now, row.device_id]);
...
export function createPostgresDeviceExporterStore() {
  return {
    ...
    // fix-fleet-local-collector-evidence-completeness).
    //
    // Completion (consume) and supersession commit as ONE Postgres
    // transaction, never two separate writes: the consume UPDATE runs first
    // inside `withPostgresTransaction`; if its `rowCount !== 1` (a
    // concurrent attempt already won the race), the method returns `false`
    // immediately WITHOUT touching any prior device, and the transaction
    // (which made no other writes) commits a no-op. A losing or failed
    // replacement enrollment can therefore never revoke the device it was
    // trying to replace. If the consume wins, the supersede scan+revoke runs
    // in the SAME transaction — a throw anywhere in the revoke cascade rolls
    // back the consume too, so the code never ends up consumed with a
    // half-applied supersession.
    //
    // Each superseded device is revoked through the SAME cascade an
    // owner-initiated revoke uses — never a raw status flip — which safely
    // spares the connector_instance because the fresh device's own
    // non-revoked source-instance still references it.
    async consumeEnrollmentCodeAndSupersedePriorDevices(params: {
      connectorId: string;
      deviceId: string;
      enrollmentCodeId: string;
      isNewDevice: boolean;
      ...
      return await withPostgresTransaction(async (client: PostgresTransactionClient) => {
        const consumeResult = await client.query(
          `UPDATE device_enrollment_codes SET status = 'consumed', device_id = $1, consumed_at = $2
           WHERE enrollment_code_id = $3 AND status = 'pending'`,
          [params.deviceId, params.now, params.enrollmentCodeId]
        );
        ...
        for (const row of superseded.rows) {
          const supersededDeviceId = row.device_id as string;
