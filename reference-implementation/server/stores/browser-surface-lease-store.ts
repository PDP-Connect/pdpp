...
interface BrowserSurfaceRow {
  ...
  browser_generation_hash: string | null;
  cdp_url: string;
  connector_id: string;
  ...
  window_settle_endpoint: string | null;
}
...
export interface BrowserSurfacePersistenceMetadata {
  ...
  readonly window_settle_endpoint?: string;
}

export type BrowserSurfaceWithPersistenceMetadata = BrowserSurface & BrowserSurfacePersistenceMetadata;
...
function mapSurface(row: BrowserSurfaceRow | null | undefined): BrowserSurfaceWithPersistenceMetadata | null {
  ...
  assignOptionalSurfaceField(surface, "active_lease_id", row.active_lease_id);
  assignOptionalSurfaceField(surface, "container_id", row.container_id);
  assignOptionalSurfaceField(surface, "browser_generation_hash", row.browser_generation_hash);
  assignOptionalSurfaceField(surface, "surface_mode", row.surface_mode);
  assignOptionalSurfaceField(surface, "surface_source", row.surface_source);
  assignOptionalSurfaceField(surface, "container_name", row.container_name);
  assignOptionalSurfaceField(surface, "profile_dir", row.profile_dir);
  assignOptionalSurfaceField(surface, "profile_volume", row.profile_volume);
  assignOptionalSurfaceField(surface, "stream_origin", row.stream_origin);
  assignOptionalSurfaceField(surface, "window_settle_endpoint", row.window_settle_endpoint);
  return surface;
}

function assignOptionalSurfaceField<T extends object>(target: T, key: string, value: string | null | undefined): void {
  if (value) {
...
function sqliteSurfaceParams(surface: BrowserSurfaceWithPersistenceMetadata): BindValue[] {
  ...
  return [
    ...
    optionalString(metadata.window_settle_endpoint) ?? null,
    surface.health,
    ...
    metadata.browser_generation_hash ?? null,
...
class SqliteBrowserSurfaceLeaseStore implements BrowserSurfaceLeaseStore {
  upsertSurface(surface: BrowserSurfaceWithPersistenceMetadata): Promise<BrowserSurfaceWithPersistenceMetadata> {
    ...
    execDynamicSqlAcknowledged(
      `INSERT INTO browser_surfaces(
        ...
        cdp_url, stream_base_url, stream_origin, window_settle_endpoint, health, container_id, container_name,
        profile_dir, profile_volume, browser_generation_hash, active_lease_id, created_at, last_used_at
      )
      VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      ON CONFLICT(surface_id) DO UPDATE SET
        ...
        window_settle_endpoint = excluded.window_settle_endpoint,
        ...
        browser_generation_hash = CASE
          ...
          ELSE excluded.browser_generation_hash
        END,
  ...
  updateBrowserGenerationHash(surfaceId: string, browserGenerationHash: string): Promise<void> {
    execDynamicSqlAcknowledged("UPDATE browser_surfaces SET browser_generation_hash = ? WHERE surface_id = ?", [
  ...
  listLeases(): Promise<BrowserSurfaceLease[]> {
    // REVIEWED-DYNAMIC: browser leases are a small controller-owned runtime table.
    const rows = allDynamicRows<BrowserSurfaceLeaseRow>(
      "SELECT * FROM browser_surface_leases ORDER BY requested_at, lease_id"
    );
...
class PostgresBrowserSurfaceLeaseStore implements BrowserSurfaceLeaseStore {
  ...
  async updateBrowserGenerationHash(surfaceId: string, browserGenerationHash: string): Promise<void> {
    await this.#query("UPDATE browser_surfaces SET browser_generation_hash = $1 WHERE surface_id = $2", [
      browserGenerationHash,
      surfaceId,
    ]);
  }
  ...
  async listLeases(): Promise<BrowserSurfaceLease[]> {
    const result = await this.#query("SELECT * FROM browser_surface_leases ORDER BY requested_at, lease_id");
