...
function normalizeStateChange(value: CredentialStateChange): CredentialStateChange {
  ...
  return {
    ...(optionalStateChangeValue(value.actorId) ? { actorId: optionalStateChangeValue(value.actorId) } : {}),
    ...(optionalStateChangeValue(value.actorType) ? { actorType: optionalStateChangeValue(value.actorType) } : {}),
    cause: value.cause,
    ...(optionalStateChangeValue(value.interactionId)
      ? { interactionId: optionalStateChangeValue(value.interactionId) }
      : {}),
    ...(optionalStateChangeValue(value.requestId) ? { requestId: optionalStateChangeValue(value.requestId) } : {}),
    ...(optionalStateChangeValue(value.runId) ? { runId: optionalStateChangeValue(value.runId) } : {}),
    ...(optionalStateChangeValue(value.traceId) ? { traceId: optionalStateChangeValue(value.traceId) } : {}),
  };
}
...
function parseStateChange(value: CredentialRow["state_change_json"]): CredentialStateChange | null {
  ...
  return normalizeStateChange({
    actorId: optionalStateChangeValue(record.actorId),
    actorType: optionalStateChangeValue(record.actorType),
    cause: record.cause as CredentialStateChangeCause,
    interactionId: optionalStateChangeValue(record.interactionId),
    requestId: optionalStateChangeValue(record.requestId),
    runId: optionalStateChangeValue(record.runId),
    traceId: optionalStateChangeValue(record.traceId),
  });
}
