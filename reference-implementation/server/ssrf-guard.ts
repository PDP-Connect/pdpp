 ...
 * can influence — `cimd.ts` (`fetchCimdDocument`, fetching a client's CIMD
 ...
 *    Registries (`server/iana-special-purpose-registry.ts`) as the policy
...
type UndiciConnectorOptions = Parameters<typeof rawConnect>[0];
type HttpsConnectionOptions = Parameters<https.Agent["createConnection"]>[0];
type HttpsConnectionCallback = NonNullable<Parameters<https.Agent["createConnection"]>[1]>;
type ConnectionDone<TSocket extends Duplex> = (error: Error | null, socket: TSocket | null) => void;
type DialOne<TOptions, TSocket extends Duplex> = (
  address: string,
  options: TOptions,
  done: ConnectionDone<TSocket>
) => void;

function defaultDnsLookup(hostname: string, options: { all: true }): Promise<Array<{ address: string }>> {
 ...
 */
function extractEmbeddedIpv4(ipv6Addr: ipaddr.IPv4 | ipaddr.IPv6, byteOffset: number): string | null {
  const bytes = ipv6Addr.toByteArray();
  if (bytes.length < byteOffset + 4) {
    return null;
  }
 ...
 */
function lookupSpecialPurposeRow(addr: ipaddr.IPv4 | ipaddr.IPv6, table: CidrTable): boolean | null {
 ...
 * snapshot (`server/iana-special-purpose-registry.ts`), and allowed by
 ...
 */
export function isGlobalUnicastAddress(ip: unknown): boolean {
 ...
 */
export function isForbiddenIp(ip: unknown): boolean {
...
export async function resolveAllowedAddresses(
  ...
  try {
    ...
    return { kind: "dns_failed", ok: false };
  }
  if (!addrs || addrs.length === 0) {
    return { kind: "no_addresses", ok: false };
  }
  if (addrs.length > maxAddresses) {
    return { count: addrs.length, kind: "too_many_addresses", max: maxAddresses, ok: false };
  }
  for (const addr of addrs) {
    if (!isGlobalUnicastAddressImpl(addr.address)) {
      return { address: addr.address, kind: "forbidden_address", ok: false };
    }
  }
  return { addresses: addrs.map((a) => a.address), ok: true };
}
 ...
 */
function boundedFallbackConnect<TOptions, TSocket extends Duplex>(
  validatedAddresses: readonly string[],
  dialOne: DialOne<TOptions, TSocket>
): (options: TOptions, callback: ConnectionDone<TSocket>) => void {
  const boundedAddresses = validatedAddresses.slice(0, MAX_VALIDATED_ADDRESSES);
  return (opts: TOptions, callback: ConnectionDone<TSocket>) => {
    let index = 0;
    let settled = false;
    const attemptNext = (lastErr: Error | null) => {
      if (settled) {
        return;
      }
      ...
      dialOne(address, opts, (err: Error | null, socket: TSocket | null) => {
        if (settled) {
 ...
 */
function createUndiciConnector(validatedAddresses: readonly string[]): typeof rawConnect {
  const connect = boundedFallbackConnect<UndiciConnectorOptions, Socket | tls.TLSSocket>(
    validatedAddresses,
  ...
  return (options, callback) => {
    connect(options, (error, socket) => {
      ...
      callback(new Error("no validated address could be connected to"), null);
    });
  };
}
 ...
 */
function dialTlsOnce(address: string, opts: HttpsConnectionOptions, cb: ConnectionDone<tls.TLSSocket>): void {
  const { host: _host, port: requestedPort, servername: requestedServername, ...tlsOptions } = opts;
  const port = requestedPort === undefined ? 443 : Number(requestedPort);
  const servername = requestedServername || _host;
  const connectionOptions: tls.ConnectionOptions = { host: address, port };
  if (servername) {
    connectionOptions.servername = servername;
  }
  if (tlsOptions.rejectUnauthorized !== undefined) {
    connectionOptions.rejectUnauthorized = tlsOptions.rejectUnauthorized;
  }
  if (tlsOptions.ca !== undefined) {
    connectionOptions.ca = tlsOptions.ca;
  }
  if (tlsOptions.cert !== undefined) {
    connectionOptions.cert = tlsOptions.cert;
  }
  if (tlsOptions.key !== undefined) {
    connectionOptions.key = tlsOptions.key;
  }
  if (tlsOptions.pfx !== undefined) {
    connectionOptions.pfx = tlsOptions.pfx;
  }
  if (tlsOptions.passphrase !== undefined) {
    connectionOptions.passphrase = tlsOptions.passphrase;
  }
  if (tlsOptions.ciphers !== undefined) {
    connectionOptions.ciphers = tlsOptions.ciphers;
  }
  if (tlsOptions.minVersion !== undefined) {
    connectionOptions.minVersion = tlsOptions.minVersion;
  }
  if (tlsOptions.maxVersion !== undefined) {
    connectionOptions.maxVersion = tlsOptions.maxVersion;
  }
  if (tlsOptions.lookup !== undefined) {
    connectionOptions.lookup = tlsOptions.lookup;
  }
 ...
 * the next).
 */
function createHttpsConnector(validatedAddresses: readonly string[]) {
  const connect = boundedFallbackConnect<HttpsConnectionOptions, tls.TLSSocket>(validatedAddresses, dialTlsOnce);
  return (options: HttpsConnectionOptions, callback: HttpsConnectionCallback): void => {
    connect(options, (error, socket) => {
      if (error || !socket) {
        // @types/node requires a Duplex on error, but Node invokes
        // createConnection callbacks as (error, null). The existing
        // single-settlement/error-without-socket oracle in ssrf-guard.test.js
        // proves this load-bearing runtime contract.
        // @ts-expect-error The upstream @types/node mismatch must remain visible.
        callback(error ?? new Error("no validated address could be connected to"), null);
        return;
      }
      callback(null, socket);
    });
...
export function createPinnedHttpsAgent(
  ...
  class PinnedHttpsAgent extends https.Agent {
    override createConnection(
      options: HttpsConnectionOptions,
      callback?: HttpsConnectionCallback
    ): Duplex | null | undefined {
      if (!callback) {
        return;
      }
      createHttpsConnector(validatedAddresses)(options, callback);
      // biome-ignore lint/complexity/noUselessReturn: required by TypeScript noImplicitReturns to make the callback result explicit.
      return;
    }
  }
