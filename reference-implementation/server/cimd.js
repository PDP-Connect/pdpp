...
import { createPinnedDispatcher, isForbiddenIp, isGlobalUnicastAddress, resolveAllowedAddresses } from './ssrf-guard.js';

// Re-exported for existing callers/tests that import isForbiddenIp from here;
// the classifier itself now lives in ssrf-guard.js so it can be shared with
...
export async function fetchCimdDocument(
  ...
  try {
    ...
    // in ssrf-guard.js) so an attacker-controlled DNS answer cannot force
    ...
    if (!resolved.ok) {
      ...
      switch (resolved.kind) {
        case 'dns_failed':
          err = new Error(`CIMD fetch failed: DNS resolution failed for ${url.hostname}`);
          break;
        case 'no_addresses':
          err = new Error(`CIMD fetch failed: DNS resolution returned no addresses for ${url.hostname}`);
          break;
        case 'too_many_addresses':
          err = new Error(
            `CIMD fetch blocked: ${url.hostname} resolved to ${resolved.count} addresses, exceeding the bound of ${resolved.max}`,
          );
          break;
        case 'forbidden_address':
          err = new Error(
            `CIMD fetch blocked: ${url.hostname} resolves to private/loopback address ${resolved.address}`,
          );
          break;
      }
    ...
    // hostname and race the check above (DNS rebinding). See ssrf-guard.js.
