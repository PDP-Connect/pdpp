...
function removeIndexWorkWaiter(waiter) {
...
async function acquireIndexWork() {
...
async function withIndexWork(operation) {
...
export function recordIndexWorkStatsForTests() {
  return { active: activeIndexWork, queued: indexWorkWaiters.length };
}

export function withRecordIndexWorkForTests(operation) {
...
let recordIndexFaultHook = null;
let sqliteRecordSortBackfillPhaseHook = null;
...
export function __setRecordIndexFaultHookForTest(hook) {
  recordIndexFaultHook = typeof hook === 'function' ? hook : null;
}

/** Test-only seam for deterministic manifest registration ordering. */
export function __setSqliteRecordSortBackfillPhaseHookForTest(hook) {
  sqliteRecordSortBackfillPhaseHook = typeof hook === 'function' ? hook : null;
}

async function maybeSqliteRecordSortBackfillPhaseForTest(point, context) {
...
function maybeRecordIndexFault(point, ctx) {
  if (recordIndexFaultHook) recordIndexFaultHook(point, ctx);
}
 ...
 */
export async function ingestRecord(storageTarget, record, options = {}) {
  const coordinationConnectorId = resolveStorageConnectorId(storageTarget);
  ...
  return withConnectorInstanceWrite(
    ...
    (coordinatorOwnership) => ingestRecordWithinCoordinator(
      storageTarget,
      record,
...
async function ingestRecordWithinCoordinator(storageTarget, record, options) {
  if (isPostgresStorageBackend()) {
    ...
    const outcome = await postgresIngestRecord(storageTarget, record, options);
    if (outcome.changed) {
      const { stream } = record;
      ...
      if (!options.deferIndexes) {
        await maintainRecordIndexes(storageTarget, record, { attemptContext: options.attemptContext });
      }
  ...
  const outcome = writeTransaction(() => {
    const finishDurableOutcome = (value) => {
    ...
    if (wouldBeUnchangedUpsert) {
      ...
      if (anchor) {
        // A processing retry may have refreshed its frozen manifest facts
        // before reaching a suffix record that is already byte-identical to
        // the current row.  The no-op must still repair manifest-derived
        // durable state in the same transaction as its prefix advance;
        // otherwise the batch could be accepted with stale semantic ordering.
        if (options.attemptContext) {
    ...
    return finishDurableOutcome({ kind: 'changed', op, version: nextVersion, selfHeal });
  });
 ...
 */
export async function maintainRecordIndexes(storageTarget, record, options = {}) {
  return withIndexWork(() => maintainRecordIndexesWithinPermit(storageTarget, record, options));
}
...
function authoritativeFinalRecord(entry, current) {
...
async function prepareDeviceFinalRecordsWithinCoordinator(storageTarget, plan, attemptContext, durablePrefixCount) {
  ...
  if (isPostgresStorageBackend()) {
    const repaired = await postgresPrepareDeviceFinalRecords(storageTarget, skipped, attemptContext);
    const repairedByIndex = new Map(repaired.map((entry) => [entry.inputIndex, entry]));
...
async function maintainRecordIndexesWithinPermit(storageTarget, record, options) {
  const connectorId = resolveStorageConnectorId(storageTarget);
  const connectorInstanceId = resolveStorageConnectorInstanceId(storageTarget, connectorId);
 ...
 *       reference-implementation-architecture/spec.md
 */
export async function deleteRecord(storageTarget, stream, recordId, coordinatorOwnership = null) {
  const coordinationConnectorId = resolveStorageConnectorId(storageTarget);
  const coordinationInstanceId = resolveStorageConnectorInstanceId(storageTarget, coordinationConnectorId);
  return withConnectorInstanceWrite(
    coordinationInstanceId,
...
async function deleteRecordWithinCoordinator(storageTarget, stream, recordId) {
  if (isPostgresStorageBackend()) {
 ...
 */
export async function deleteAllRecords(storageTarget, stream, coordinatorOwnership = null) {
  const coordinationConnectorId = resolveStorageConnectorId(storageTarget);
  const coordinationInstanceId = resolveStorageConnectorInstanceId(storageTarget, coordinationConnectorId);
  return withConnectorInstanceWrite(
    coordinationInstanceId,
 ...
 */
function advanceSqliteRecordResetGenerationForStreams(connectorInstanceId, streams) {
  let touchedCount = 0;
...
async function deleteAllRecordsWithinCoordinator(storageTarget, stream) {
  if (isPostgresStorageBackend()) {
...
export async function deleteAllRecordsForConnector(connectorId) {
  ...
  const countRow = getOne(referenceQueries.recordsDeleteCountRecordsByConnector, [storageConnectorId]);
  ...
  for (const connectorInstanceId of [...streamsByInstance.keys()].sort()) {
...
async function postgresDeleteAllRecordsForConnector(connectorId) {
  ...
  for (const connectorInstanceId of [...streamsByInstance.keys()].sort()) {
    await withConnectorInstanceWrite(connectorInstanceId, async () => {
      ...
      for (const stream of streamsByInstance.get(connectorInstanceId)) {
...
function computeIngestSemanticTime(connectorId, streamName, data, effectiveEmittedAt, attemptStreamFacts = null) {
  if (!data || typeof data !== 'object') return effectiveEmittedAt;
  if (attemptStreamFacts) {
    for (const field of [attemptStreamFacts.consentTimeField, attemptStreamFacts.cursorField]) {
      const coerced = typeof field === 'string' ? coerceSemanticTimeValue(data[field]) : null;
...
export async function backfillSqliteRecordSemanticTimesForManifest(manifest) {
  const connectorId = canonicalConnectorKey(manifest?.connector_key || manifest?.connector_id)
    ?? manifest?.connector_key
    ?? manifest?.connector_id;
  if (typeof connectorId !== 'string' || !Array.isArray(manifest?.streams)) return { updated: 0 };
  const streamFacts = manifest.streams
    .map((stream) => ({
      stream: typeof stream?.name === 'string' ? stream.name : null,
  ...
  let updated = 0;
  for (const { connector_instance_id: connectorInstanceId } of instanceRows) {
      ...
      for (const facts of streamFacts) {
        ...
        writeTransaction(() => {
          ...
          for (const row of rows) {
            ...
            if (semanticTime === row.semantic_time) continue;
            update.run(semanticTime, connectorInstanceId, facts.stream, row.record_key);
            updated += 1;
          }
        });
      }
