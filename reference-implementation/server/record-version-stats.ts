...
/** An `Error` carrying stable machine `code`/`param` fields. */
type CodedError = Error & { code?: string; param?: string };
...
/** Risk classification for a stream's version churn. */
interface ChurnClassification {
  riskLevel: "normal" | "watch" | "high";
  riskReasons: string[];
  versionsPerRecord: number;
}

/** A `(connector_instance_id, stream)` candidate key for the bounded scan. */
interface StreamKey {
  connectorInstanceId: string;
  stream: string;
}

/** Optional connector-instance / stream filter for a ground-truth read. */
...
export function normalizeRecordVersionStatsRisk(value: unknown): string | null {
  ...
  if (!VALID_RISKS.has(value as string)) {
    const err: CodedError = new Error("risk must be one of: normal, watch, high");
...
export function classifyRecordVersionChurn({
  currentRecordCount,
  recordHistoryCount,
  recordKeyCount = null,
}: {
  currentRecordCount?: unknown;
  recordHistoryCount?: unknown;
  recordKeyCount?: unknown;
}): ChurnClassification {
...
function buildGroundTruthWhere(
  { connectorInstanceId, stream }: StreamFilter = {},
  dialect: "sqlite" | "postgres" = "sqlite"
): { params: string[]; where: string } {
...
function shapeGroundTruthRow(row: Row) {
  return {
    ...
    listGroundTruthForKeys(keyList: StreamKey[]) {
      ...
      return db.transaction(() => {
        ...
        for (const k of keyList) {
          insert.run(k.connectorInstanceId, k.stream);
        }
        const rows = db
          .prepare(
            `WITH history AS (
...
export async function listRecordVersionGroundTruthStreams({ connectorInstanceId, stream }: StreamFilter = {}) {
 ...
 */
export async function listRecordVersionGroundTruthForKeys({ keys }: { keys?: StreamKey[] } = {}) {
  const keyList = Array.isArray(keys) ? keys.filter((k) => k?.connectorInstanceId && k.stream) : [];
  if (keyList.length === 0) {
    return [];
  }
 ...
 */
export function isVersionChurnCandidate({
  dirty,
  currentRecordCount,
  recordHistoryCount,
}: {
  ...
  if (current === 0) {
    return true; // history_without_current_records
  }
...
export async function buildRecordVersionStatsEnvelope(
  ...
  } = {},
  {
    ...
    // biome-ignore lint/suspicious/noExplicitAny: injected connector-instance store is a duck-typed external dependency.
    connectorInstanceStore?: any;
    listStreams?: (params?: StreamFilter) => Row[] | Promise<Row[]>;
    listGroundTruthStreams?: (params?: StreamFilter) => Row[] | Promise<Row[]>;
  ...
  const useCandidatePath = isUnfiltered && typeof listGroundTruthForKeys === "function";

  let groundTruthRows: Row[];
  if (useCandidatePath) {
    const candidateKeys = (streamRows as Row[])
      ...
      .map((row: Row) => ({ connectorInstanceId: row.connector_instance_id, stream: row.stream }));
    groundTruthRows = candidateKeys.length ? await listGroundTruthForKeys({ keys: candidateKeys }) : [];
  } else {
  ...
  const groundTruthByKey = new Map<string, Row>((groundTruthRows as Row[]).map((row: Row) => [rowKey(row), row]));
  const projectionByKey = new Map<string, Row>((streamRows as Row[]).map((row: Row) => [rowKey(row), row]));
  ...
  const displayNames = await getDisplayNames(
    Array.from(new Set(mergedRows.map((row: Row) => row.connector_instance_id).filter(Boolean))),
