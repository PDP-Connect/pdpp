...
import { createPinnedHttpsAgent, resolveAllowedAddresses } from './ssrf-guard.js';
 ...
 */
export async function guardWebPushEndpoint(endpoint, { dnsLookupImpl, isGlobalUnicastAddressImpl } = {}) {
  let parsed;
  try {
  ...
  return { ok: true, agent: createPinnedHttpsAgent(resolved.addresses) };
}
...
export async function defaultSendNotification(
  ...
  payload,
  config,
  { guardWebPushEndpointImpl = guardWebPushEndpoint, webPushModuleImpl } = {},
) {
  const guard = await guardWebPushEndpointImpl(subscription.endpoint);
  if (!guard.ok) {
    const err = new Error(`Web Push send blocked: ${guard.reason}`);
    err.code = 'web_push_send_blocked';
    throw err;
  }
  const webPushModule = webPushModuleImpl ?? (await import('web-push'));
