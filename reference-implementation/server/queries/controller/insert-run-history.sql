...
-- run_id's row via the run.started/terminal spine-event hook
-- (server/stores/run-history-writer.ts) before this call runs, so this is
-- normally an UPDATE-by-conflict that fills in the scheduler-only
-- enrichment fields (attempt, checkpoint_summary_json, known_gaps_json,
-- reported_records_emitted) the general writer does not carry — see
-- openspec/changes/generalize-run-history-write-authority. The plain
-- INSERT branch (ON CONFLICT DO UPDATE) also covers the case where no row
