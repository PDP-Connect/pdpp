...
-- time bounds, and the distinct connection / stream totals. Used by
...
SELECT
  ...
  COUNT(DISTINCT connector_instance_id)            AS connector_count,
  COUNT(DISTINCT connector_instance_id || char(10) || stream) AS stream_count
FROM records
WHERE deleted = 0
