...
export function unsupportedReason(modality: ConnectorIntentModality): string {
  ...
  if (modality === "api_network") {
    return 'This connector is API/network-only. The reference has no standalone owner-agent API-connect route and no provider-connect URL to send the owner to: an API connection materializes implicitly on first ingest, after a connector has ingested data — there is no owner-mediated pre-creation step and no dashboard flow that adds one (the console marks API/network sources unsupported for the same reason). The missing primitive is a typed owner-agent API-connect intent that returns an `open_url` next step (OAuth authorization endpoint + state, owner completes consent in a browser — never the agent) or an owner-mediated token-capture step, materializing the connection only after provider authorization and first ingest. Until that primitive ships with committed proof, this stays unsupported; see openspec/changes/add-owner-agent-control-surface/design.md ("Deferred: API/network connection initiation").';
  }
