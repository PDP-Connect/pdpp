...
export function unsupportedReason(modality: ConnectorIntentModality): string {
  ...
  if (modality === "api_network") {
    return 'This connector is API/network-only and authenticates with a static provider secret the owner supplies locally (gmail uses a Google app password over IMAP; github uses a personal access token) — there is no OAuth authorization URL to send the owner to. The reference has no standalone owner-agent API-connect route: an API connection materializes implicitly on first ingest, after a connector has ingested data — there is no owner-mediated pre-creation step, no per-connection credential store, and no dashboard flow that adds one (the console marks API/network sources unsupported for the same reason). The missing primitive is a typed owner-agent API-connect intent whose next step is owner-mediated credential capture (the owner, never the agent, supplies the app password / token in their local environment) — or `open_url` only for a genuinely OAuth-backed connector, which none of the current ones are — materializing the connection only after that owner step and first ingest. Until that primitive ships with committed proof, this stays unsupported; see openspec/changes/add-owner-agent-control-surface/design.md ("Deferred: API/network connection initiation").';
  }
