...
export function unsupportedReason(modality: ConnectorIntentModality): string {
  ...
  if (modality === "api_network") {
    return "This connector is API/network-only and authenticates with a static provider secret the owner supplies locally (gmail uses a Google app password over IMAP; github uses a personal access token) — there is no OAuth authorization URL to send the owner to. The reference now has the per-connection encrypted credential store and connection-scoped subprocess injection for this credential model (add-static-secret-owner-connect-primitive), so a captured secret is sealed at rest, never agent-readable, and injected into exactly one connection run. What is still missing is the owner-mediated capture surface (the owner, never the agent, supplies the app password / token in their local environment) and the committed end-to-end proof — intent to owner capture to first ingest to an addressable connection_id, with two mailboxes proven as two connection_ids. Until that proof lands the route stays `unsupported` and does not advertise a one-click next step (`open_url` would apply only to a genuinely OAuth-backed connector, which none of the current ones are); an API connection still materializes only on first ingest, not from this intent. See openspec/changes/add-static-secret-owner-connect-primitive/design.md (Decision 6, proof-before-flip).";
  }
