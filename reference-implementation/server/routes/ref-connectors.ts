...
export function mountRefConnectorsList(app: AppLike, ctx: MountRefConnectorsContext): void {
  app.get(
    ...
    async (req: RouteRequest, res: RouteResponse) => {
      try {
        // No route-owned barrier call here (Sol P1.2): `listConnectorSummaries`/
        // `getConnectorSummaryForRoute` below always run their own barrier
        // internally via `loadConnectorSummaryProjectionDeps` — scoped to the
        // resolved connection when a selector is present, complete otherwise.
        // A route-owned call here was a genuinely redundant, always-UNSCOPED
        // second barrier pass ahead of a caller that may only need a scoped
        // one, defeating the whole point of scoping.
        //
        ...
        if (connectionSelector) {
          if (req.query.limit !== undefined || req.query.cursor !== undefined) {
            throw new ConnectorSummaryPageRequestError(
              req.query.limit !== undefined ? "limit" : "cursor",
        ...
        if (pageRequest && ownerSubjectId) {
          if (!ctx.listConnectorSummaryPage) {
          ...
          const envelope = await executeRefConnectorsList({
            ...
            listConnectorSummariesPage: () =>
              ctx.listConnectorSummaryPage!(ownerSubjectId, pageRequest) as unknown as RefConnectorsListPage,
          });
          res.json(envelope);
          return;
        }
...
export function mountRefConnectorDetail(app: AppLike, ctx: MountRefConnectorsContext): void {
  app.get(
    ...
    async (req: RouteRequest, res: RouteResponse) => {
      try {
        // No route-owned barrier call here (Sol P1.2) — `getConnectorDetail`
        // below always runs its own barrier internally, scoped to the
        // resolved connection once it resolves one.
