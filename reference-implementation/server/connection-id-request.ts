...
import { canonicalConnectorKey, isRegistryUrlConnectorId } from "./connector-key.ts";
...
interface ClampedLimitResult {
  clamped: boolean;
  limit: number;
  requested: number | null;
}

interface LimitClampedWarning {
  code: string;
  detail: {
    max_limit: number;
...
interface DeprecatedAliasWarning {
  code: string;
  message: string;
  param: string;
}
...
/**
 * Shape read by {@link validateConnectionAlias}, {@link resolveRequestConnectionId},
 * and {@link enforceConnectionNarrowing}. Callers pass request query params or
 * an equivalent plain object; only these two identity fields are read.
 */
export interface ConnectionIdRequestParams {
  connection_id?: unknown;
  connector_instance_id?: unknown;
}

interface ResolvedConnectionId {
  connectionId: string | null;
  warnings: DeprecatedAliasWarning[];
}

interface DisplayNameProjectionContext {
 ...
 */
export function clampRecordsPageLimit(rawLimit: unknown): ClampedLimitResult {
  // Number.parseInt coerces any input to a string at runtime (it always did,
  // even pre-migration -- Number.parseInt(undefined, 10) is NaN just like
  // Number.parseInt(String(undefined), 10)); the explicit String(...) here
  // only satisfies the stricter (string, number) => number lib signature and
  // changes no observable behavior.
  const parsed = Number.parseInt(String(rawLimit), 10);
  if (!Number.isFinite(parsed) || parsed <= 0) {
    return { clamped: false, limit: RECORDS_DEFAULT_PAGE_LIMIT, requested: null };
  }
  if (parsed > RECORDS_MAX_PAGE_LIMIT) {
    return { clamped: true, limit: RECORDS_MAX_PAGE_LIMIT, requested: parsed };
  }
  return { clamped: false, limit: parsed, requested: parsed };
}
 ...
 * human diagnostics.
 */
export function buildLimitClampedWarning(requested: number): LimitClampedWarning {
  return {
 ...
 */
export function validateConnectionAlias(requestParams: ConnectionIdRequestParams | null | undefined): void {
 ...
 */
export function resolveRequestConnectionId(
  requestParams: ConnectionIdRequestParams | null | undefined
  ...
  const canonicalSet = typeof canonical === "string" && canonical.length > 0;
  const aliasSet = typeof alias === "string" && alias.length > 0;
  const warnings: DeprecatedAliasWarning[] = [];
  if (aliasSet) {
    warnings.push({
...
export function projectStorageDisplayName(
  displayName: unknown,
  { connectorId = null, connectorInstanceId = null }: DisplayNameProjectionContext = {}
): string | null {
  ...
  if (PLACEHOLDER_DISPLAY_NAMES.has(trimmed)) {
    return null;
  }
  const displayNameKey = canonicalConnectorKey(trimmed);
  const connectorKey = canonicalConnectorKey(connectorId) ?? connectorId;
  if (connectorKey && displayNameKey && displayNameKey === connectorKey) {
    return null;
  }
  if (isRegistryUrlConnectorId(trimmed)) {
    return null;
  }
  if (connectorId && trimmed === connectorId) {
    return null;
  }
  if (connectorInstanceId && trimmed === connectorInstanceId) {
    return null;
  }
  return trimmed;
}
 ...
 */
export function enforceConnectionNarrowing(
  requestParams: ConnectionIdRequestParams | null | undefined,
  boundConnectorInstanceId: unknown
): void {
  ...
  if (typeof boundConnectorInstanceId !== "string" || !boundConnectorInstanceId) {
    const err: ErrorWithCode = new Error("connection_id is not addressable under this grant.");
    err.code = "connection_not_found";
    err.param = "connection_id";
    throw err;
  }
  if (connectionId !== boundConnectorInstanceId) {
    const err: ErrorWithCode = new Error(`connection_id '${connectionId}' is not addressable under this grant.`);
    err.code = "connection_not_found";
    err.param = "connection_id";
    throw err;
  }
