...
test('register rejects a managed surface settle endpoint from a foreign origin', () => {
  ...
  assert.throws(
    ...
    (err) => err.code === 'run_target_window_settle_origin_mismatch',
  );
  registry.shutdown();
});
...
test('register accepts the managed surface settle endpoint derived from its CDP origin', () => {
  ...
  assert.equal(
    registry.get({ runId: 'run_dynamic_1', interactionId: 'int_a' }).window_settle_endpoint,
    'http://10.88.0.4:9223/pdpp/window-settle',
  );
  registry.shutdown();
});
