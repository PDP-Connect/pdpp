...
const sinks = new Map(); // runId → set<callback>

export function registerRemoteTelemetrySink(runId, callback) {
  if (!runId || typeof callback !== 'function') return () => {};
  ...
  return () => {
    ...
    if (current.size === 0) sinks.delete(runId);
  };
}

export function emitRemoteTelemetry(runId, payload) {
...
export function dropRemoteTelemetry(runId) {
  sinks.delete(runId);
}
