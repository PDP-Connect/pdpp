...
type PlaygroundRunTargetRegistry = Pick<RunTargetRegistry, "register">;
...
interface PlaygroundLogger {
  [level: string]: ((entry: Record<string, unknown>) => void) | undefined;
}
...
function buildTestPageHtml({ debug = false }: { debug?: boolean } = {}) {
  if (debug) {
...
function buildTestPageUrl(options: { debug?: boolean } = {}) {
 ...
 */
const inFlights = new Map<PlaygroundBackend, Promise<PlaygroundSession>>();
...
const cachedSessions = new Map<PlaygroundBackend, PlaygroundSession>();
...
export function createPlayground(options?: PlaygroundFactoryOptions) {
  ...
  function log(level: PlaygroundLoggerLevel, msg: string, data?: Record<string, unknown>) {
  ...
  function normalizeBackend(value: string | undefined): PlaygroundBackend {
    const raw = String(value || env.PDPP_STREAM_PLAYGROUND_BACKEND || DEFAULT_PLAYGROUND_BACKEND)
      .trim()
      .toLowerCase();
    if (raw === "cdp" || raw === "neko" || raw === "neko-remote-cdp") {
      return raw;
    }
    const err = new Error('playground backend must be "cdp", "neko", or "neko-remote-cdp"');
    throw Object.assign(err, { code: "invalid_playground_backend" });
  }
  ...
  function inferNekoCdpHttpUrl(parsed: URL): string | null {
    if (parsed.hostname === "neko") {
      return "http://neko:9223/";
    }
  ...
  function inferredNekoCdpHttpUrl(baseUrl: string): string | null {
    try {
  ...
  function resolveNekoCdpHttpUrl(baseUrl: string): string | null {
  ...
  function makeIds(backend: PlaygroundBackend | "neko_remote_cdp"): Pick<PlaygroundSession, "runId" | "interactionId"> {
    const ts = Date.now();
  ...
  function getCachedSessionForRunId(runId: string): PlaygroundSession | null {
   ...
   * We forward it to the per-runId remote-telemetry registry; routes.ts
  ...
  function registerNekoPlaygroundTarget(session: PlaygroundSession, startUrl: string) {
    ...
    playgroundRunTargetRegistry.register({
      backend: "neko",
      base_url: baseUrl,
  ...
  function isCdpBackedSession(session: PlaygroundSession): boolean {
    return ["cdp", "neko-remote-cdp"].includes(session.backend);
  }

  function registerCdpPlaygroundTarget(session: PlaygroundSession) {
  ...
  async function releasePlaygroundBrowser(isolated: IsolatedBrowser) {
    await isolated.release().catch(ignoreCleanupFailure);
    cleanupBrowser = null;
  }

  async function releaseRemoteCdpPlaygroundBrowser(page: PlaygroundPage, isolated: IsolatedBrowser) {
    await page.close().catch(ignoreCleanupFailure);
    await releasePlaygroundBrowser(isolated);
  }

  async function navigatePlaygroundPage(page: PlaygroundPage, debugCalibration: boolean, cleanup: () => Promise<void>) {
    try {
      await page.goto(buildTestPageUrl({ debug: debugCalibration }), { timeout: 15_000, waitUntil: "load" });
    } catch (err) {
  ...
  function publishedCdpEndpointInputs(): { host: string; portRaw: string } {
    return {
  ...
  function missingCdpEndpointMessage(
    { host, portRaw }: { host: string; portRaw: string },
    messages: { missing: string; invalid: (portRaw: string) => string }
  ): string | null {
    if (!host) {
  ...
  async function failAfterCleanup(cleanup: () => Promise<void>, message: string): Promise<never> {
    await cleanup();
    throw new Error(message);
  }

  // biome-ignore lint/suspicious/useAwait: The async signature is part of this caller-facing contract.
  async function requirePublishedCdpEndpoint(
  ...
  async function createCdpPlaygroundSession({
    ...
    const page = pages[0] ?? (await isolated.context.newPage());
  ...
  async function createNekoPlaygroundSession({
    ...
    const session = registerPlaygroundTarget({ backend: "neko", baseUrl, debugCalibration, interactionId, runId });
    installControllerShim();

    log("info", "playground_ready", { backend: "neko", interactionId, runId });
    return session;
  }
  ...
  async function createNekoRemoteCdpPlaygroundSession({
    ...
    const { host, port } = await requirePublishedCdpEndpoint(() => releaseRemoteCdpPlaygroundBrowser(page, isolated), {
    ...
    log("info", "playground_ready", { backend: "neko-remote-cdp", interactionId, runId, wsUrl });
    return session;
  }

  function cachedSessionForBackend(backend: PlaygroundBackend, debugCalibration: boolean): PlaygroundSession | null {
    if (backend === "neko") {
      return null;
    }
