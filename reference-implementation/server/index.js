...
import {
  ...
  queryRecordsAcrossBindings, getRecordAcrossBindings, getRecordFieldWindowAcrossBindings,
...
import { createPresentationScreenStateStore } from './stores/presentation-screen-state-store.ts';
...
async function persistContentAddressedBlob({ connectorId, connectorInstanceId, stream, recordKey, mimeType, data }) {
  return withConnectorInstanceWrite(connectorInstanceId, (ownership) => persistContentAddressedBlobWithinFence({
    connectorId,
    connectorInstanceId,
    stream,
    recordKey,
...
async function recyclePresentationSurface({
  ...
  let retired = false;
  try {
    if (typeof browserSurfaceLeaseManager?.invalidateSurface === 'function') {
      invalidated = browserSurfaceLeaseManager.invalidateSurface(surfaceId, {
      ...
      if (browserSurfaceLeaseStore && invalidated) {
        await browserSurfaceLeaseStore.withLeaseTransaction(async (store) => {
          if (invalidated.surface) await store.upsertSurface({ ...invalidated.surface, health: 'unhealthy' });
          if (invalidated.lease) await store.upsertLease(invalidated.lease);
        });
      }
      if (!retired && typeof browserSurfaceLeaseManager.getSurface === 'function') {
        ...
        retired = browserSurfaceLeaseManager.getSurface(surfaceId) == null;
      }
    }
    if (typeof browserSurfaceAllocator?.stopSurface === 'function') {
      await browserSurfaceAllocator.stopSurface({ surfaceId, reason: 'surface_failed' });
      retired = true;
    }
  } catch (err) {
    logger?.warn?.({ err: String(err?.message || err), surface_id: surfaceId }, 'presentation surface recycle failed');
...
async function reconcileUnrestoredPresentationScreens({
  ...
  if (!presentationScreenStateStore) return;
...
const defaultNekoWindowSettleProbe = (url) => fetch(url);

function resolveNekoWindowSettleProbe(probe) {
...
function buildAsApp(opts = {}) {
  ...
  const streamingSessions =
    ...
    createStreamingSessionStore({
      ...
      ttlMs: opts.streamingSessionTtlMs,
    });
  ...
  const streamingRoutes = registerStreamingRoutes({
    ...
    nekoWindowSettleProbe: resolveNekoWindowSettleProbe(opts.nekoWindowSettleProbe),
    ...
    onPresentationRestoreFailure: async ({ browser_session_id, lease_id, run_id, surface_id }) => {
      await recyclePresentationSurface({
  ...
  if (controller && typeof controller.respondToInteraction === 'function') {
    ...
    async function restorePresentationBeforeTerminal(runId, interactionId, reason) {
      try {
        ...
        await runTargetRegistry.forceUnregister({ runId, interactionId });
      }
    }
    controller.respondToInteraction = async (runId, input = {}) => {
      await restorePresentationBeforeTerminal(
        runId,
        input.interaction_id,
        `interaction_${input.status || 'resolved'}`,
      );
      return originalRespondToInteraction(runId, input);
    };
    ...
    if (opts.presentationTerminalBarrier && typeof opts.presentationTerminalBarrier === 'object') {
      opts.presentationTerminalBarrier.invoke = async ({ interactionId, reason, runId }) => {
        await restorePresentationBeforeTerminal(runId, interactionId, reason);
      };
    }
    opts.presentationTerminalBarrier.releaseLease = async ({ runId }) => {
      await streamingRoutes.restoreOrRetirePresentationForRun({ run_id: runId, reason: 'run_cleanup' });
    };
  }
...
function buildRsApp(opts = {}) {
  ...
  const rsReadContext = {
    ...
    getRecordFieldWindowAcrossBindings, getRecord,
...
export async function startServer(opts = {}) {
  ...
  const browserSurfaceControllerOptions = {
    ...
    ...(opts.browserSurfaceLeaseManager
      ? { browserSurfaceLeaseManager: opts.browserSurfaceLeaseManager }
      : {}),
  ...
  const presentationScreenStateStore =
    opts.presentationScreenStateStore || createPresentationScreenStateStore();
  ...
  const presentationTerminalBarrier = { invoke: null, releaseLease: null };
  ...
  await reconcileUnrestoredPresentationScreens({
    browserSurfaceAllocator: browserSurfaceControllerOptions.browserSurfaceAllocator,
    browserSurfaceLeaseManager: browserSurfaceControllerOptions.browserSurfaceLeaseManager,
    browserSurfaceLeaseStore: browserSurfaceControllerOptions.browserSurfaceLeaseStore,
    logger,
    presentationScreenStateStore,
  });
  ...
  setImmediate(() => {
    runStartupSummaryEvidenceSweepToCompletion({
      runSweep: runBoundedSummaryEvidenceSweep,
      ...
      onRound: (summary, round) => {
        if (summary.repaired > 0 || summary.skipped > 0 || summary.incomplete) {
      ...
      .then((rounds) => {
        ...
        if (last?.incomplete && last.resumeAfterId) {
          logger.info(
            { rounds: rounds.length, resumeAfterId: last.resumeAfterId },
