...
It distils — and corrects — the earlier read-only map at
...
— points at a **deleted file** and will fail. There is no Docker→host CDP bridge
...
(`add-connector-adaptive-lanes` §5), which uses the local browser path.
...
|---|------|------------------|-----------|------|
| 1 | Amazon browser-collector proof + intent flip | `add-browser-collector-enrollment-primitive` §3 (proof), §3 (flip); `add-owner-agent-control-surface` §5.3/§8.5 | 1 session (Amazon login) | low (read-only browse) |
| 2 | ChatGPT hosted-MCP live flow | `canonicalize-connector-keys` §5.2 | 1 session (ChatGPT MCP client) | low |
| 3 | ~~Host browser bridge Docker proof~~ | `design-host-browser-bridge-for-docker` §1.2/§5 | **none — archive instead** | n/a |
| 4 | Adaptive-lane ChatGPT live pilot + telemetry | `add-connector-adaptive-lanes` §5 (pilot), §5 (compare), §6 (maxConcurrency), §6 (next candidate) | 1 session (ChatGPT login) | low |
| 5 | Chase current-activity native-ID investigation | `add-chase-current-activity-stream` §1.2 | opportunistic rider | low |
| 6 | n.eko real-phone stream matrix | `add-run-interaction-streaming-companion` §12.8, §13.5, §14.7, §15.8, §17.5 | 1 session (Android + iOS) | low |
| 7 | Production-backup canonical-key migration close-out | `canonicalize-connector-keys` §3.4 | 1 operator session | **high (data-loss-capable)** |
...
**Closes:** `add-browser-collector-enrollment-primitive` §3 (Amazon end-to-end
proof test + scrubbed fixture) and, as one reviewable post-proof code unit, §3
(flip `browser_bound` → `enroll_browser_collector`) plus
`add-owner-agent-control-surface` §5.3 / §8.5 Amazon acceptance.
...
**Closes:** `canonicalize-connector-keys` §5.2. The Claude side is already
live-proven (2026-05-31); this is the ChatGPT-side parity run. The task text is
explicit: **do not mark §5.2 complete until ChatGPT is also live-proven** or the
...
- At least **two** connections configured so the multi-connection
  `ambiguous_connection → connection_id` retry path is exercised. Confirm with an
  owner read first:
...
URL-shaped connector ids** (`https://…/connectors/…`) appear in any owner-facing
...
§1.2 and §5 reference deleted code. **Do not attempt the proof.** The owner-grade
action is to archive the change as superseded by
`2026-05-29-introduce-local-collector-runner` per `AGENTS.md` § Archiving
...
**Closes:** `add-connector-adaptive-lanes` §5 (run one ChatGPT live pilot with
fixture capture), §5 (compare live telemetry to the serialized baseline). Two
further boxes are **desk decisions gated on this run's telemetry**, not separate
sessions: §6 (whether ChatGPT may raise `maxConcurrency` above `1`) and §6 (next
connector candidate).
...
- The connector configured at its first-pilot pressure: `initialConcurrency = 1`,
  `maxConcurrency = 1` (already the committed default per §4.2).
...
**Then (desk, same evidence):** record the §6 `maxConcurrency` decision — if the
...
is already unit-tested in §5.x — confirm it held live).
...
**Closes:** `add-chase-current-activity-stream` §1.2. This is an
...
- If stable native IDs exist: open a follow-up to update the connector key
  strategy to prefer them; note the attribute/payload path. Leave §1.2 open until
...
surface):** `add-run-interaction-streaming-companion` §12.8, §13.5, §14.7, §15.8,
...
PDPP_STREAM_SMOKE_OWNER_PASSWORD=…  \
...
address chrome); pointer/touch alignment; local→remote paste via the Clipboard
...
- clipboard host→remote paste and remote→host buffer both work;
...
**Closes:** `canonicalize-connector-keys` §3.4 — the owner sign-off remainder.
...
(this is the §7.3 surface smoke against migrated production-shaped data):
...
`expose-connection-identity-on-public-read` (blocked on `apps/web` React test
infra), `split-public-site-and-operator-console` inventory work. Track them, but
