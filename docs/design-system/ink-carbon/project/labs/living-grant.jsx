...
const LivingGrant = () => {
  ...
  const handlePause = useCallback(() => setPaused((isPaused) => !isPaused), []);
  const handleThermalChange = useCallback(({ target }) => setThermal(Number(target.value)), []);
    ...
    >
      ...
      >
        ...
        <div style={{ marginTop: 32, display: "flex", gap: 10 }}>
          <button className="btn btn-paper" style={{ height: 36, fontSize: 13 }} type="button">
            Revoke grant
          </button>
          <button className="btn btn-ghost" style={{ height: 36, fontSize: 13 }} type="button">
            Adjust scope →
          </button>
        </div>
      </div>
      ...
      >
        ...
        <span className="gutter num" style={{ color: thermalColor, minWidth: 70, textAlign: "right" }}>
          {thermalLabel(thermal)} · {Math.round(thermal * 100)}°
        </span>
      </div>
    </div>
  );
