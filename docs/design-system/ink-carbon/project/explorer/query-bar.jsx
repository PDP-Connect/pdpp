...
function QueryBar({ query, onChange, streams, mode, onModeChange, focused, onFocusedChange }) {
  ...
  function onKeyDown(e) {
    ...
    if (e.key === "Escape") { setText(""); commitText(""); }
  }

  // Debounce text changes into the query
  useEffect(() => {
    const t = setTimeout(() => { if (text !== query.text) commitText(text); }, 180);
        ...
        {query.chips.map((c, i) => {
          ...
          return (
            <button
              className="exp-chip"
              key={i}
              onClick={(e) => { e.stopPropagation(); removeChipAt(i); }}
        ...
        <div className="exp-suggest">
          ...
          {suggestions.map((s, i) => (
            <div
              ...
              key={i}
              onMouseDown={(e) => { e.preventDefault(); addChip(s.chip); }}
          ...
          <div className="exp-suggest__row" onMouseDown={(e) => { e.preventDefault(); commitText(text); }} style={{ paddingTop: "0.5rem", paddingBottom: "0.5rem" }}>
