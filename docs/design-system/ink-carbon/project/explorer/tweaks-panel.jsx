...
function TweakRadio({ label, value, options, onChange }) {
  ...
  if (!fitsAsSegments) {
    ...
    // biome-ignore lint/performance/noJsxPropsBind: this standalone design artifact deliberately uses dynamic preview behavior.
    return <TweakSelect label={label} onChange={(s) => onChange(resolve(s))} options={options} value={value} />;
  }
...
function TweakSelect({ label, value, options, onChange }) {
  return (
    <TweakRow label={label}>
      {/* biome-ignore lint/performance/noJsxPropsBind: this standalone design artifact deliberately uses dynamic preview behavior. */}
      <select className="twk-field" onChange={(e) => onChange(e.target.value)} value={value}>
        {options.map((o) => {
...
function __twkIsLight(hex) {
  ...
  // biome-ignore lint/suspicious/noBitwiseOperators: this standalone design artifact deliberately uses dynamic preview behavior.
  const r = (n >> 16) & 255,
    // biome-ignore lint/suspicious/noBitwiseOperators: this standalone design artifact deliberately uses dynamic preview behavior.
    g = (n >> 8) & 255,
    // biome-ignore lint/suspicious/noBitwiseOperators: this standalone design artifact deliberately uses dynamic preview behavior.
  ...
  if (!options?.length) {
    return (
      <div className="twk-row twk-row-h">
        ...
        {/* biome-ignore lint/performance/noJsxPropsBind: this standalone design artifact deliberately uses dynamic preview behavior. */}
        <input className="twk-swatch" onChange={(e) => onChange(e.target.value)} type="color" value={value} />
      </div>
    );
  }
