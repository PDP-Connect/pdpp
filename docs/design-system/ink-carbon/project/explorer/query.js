 ...
 */
const EMAIL_TAG_PATTERN = /<[^>]+>/g;
const WHITESPACE_PATTERN = /\s/;
const EMAIL_TOKEN_SEPARATOR_PATTERN = /[._-]+/g;
const TOKEN_SEPARATOR_PATTERN = /\s+/;
const AMOUNT_FIELD_PATTERN = /amount/i;
const CATEGORY_FIELD_PATTERN = /category/i;
const IMAGE_FIELD_PATTERN = /thumb|image|photo|picture/i;
const TEXT_FIELD_PATTERN = /title|subject|snippet|body|text|caption|merchant|memo|name/i;
(() => {
    const FIELD_ALIASES = {
    ...
    function firstNameToken(person) {
        ...
        const [firstPerson] = Array.isArray(person) ? person : [person];
        let s = String(firstPerson).replace(EMAIL_TAG_PATTERN, "").trim();
        if (!s) {
            return "";
        }
        // If it's an email-only string, take the local part and split on punctuation
        if (s.includes("@") && !WHITESPACE_PATTERN.test(s)) {
            s = s.split("@")[0].replace(EMAIL_TOKEN_SEPARATOR_PATTERN, " ");
        }
        const [tok] = s.split(TOKEN_SEPARATOR_PATTERN);
    ...
    function recordMatchesChip(stream, r, chip) {
        switch (chip.field) {
            ...
            case "from": {
                ...
                const candidateFields = stream.schema.fields.filter((f) => FIELD_ALIASES.from.some((alias) => f.name.toLowerCase().includes(alias)) ||
                    f.type === "person" ||
                    f.type === "person[]");
            ...
            case "to": {
                ...
                return fs.some((f) => (f.type === "person" || f.type === "person[]" || f.type === "text") &&
            ...
            case "amount": {
                const fs = stream.schema.fields;
                const af = fs.find((f) => f.type === "currency")?.name ?? fs.find((f) => AMOUNT_FIELD_PATTERN.test(f.name))?.name;
                if (!af) {
                    return false;
                }
            ...
            case "category": {
                const fs = stream.schema.fields;
                const cf = fs.find((f) => CATEGORY_FIELD_PATTERN.test(f.name))?.name;
            ...
            case "channel": {
                const target = String(chip.value).toLowerCase();
                return ["channel", "channel_id"].some((k) => String(r[k] ?? "")
                    .toLowerCase()
                    .includes(target));
            }
            case "has": {
                if (chip.value === "image") {
                    return (stream.schema.fields.some((f) => f.type === "blob" && (f.media_type ?? "").startsWith("image/")) ||
                        stream.schema.fields.some((f) => IMAGE_FIELD_PATTERN.test(f.name) && (f.type === "blob" || f.type === "url") && r[f.name]));
                }
                if (chip.value === "geo") {
                    return ["lat", "latitude"].some((n) => r[n] !== null && r[n] !== undefined);
                }
    ...
    function textMatches(stream, r, text) {
        ...
        const fields = stream.schema.fields.filter((f) => ["text", "person"].includes(f.type) || TEXT_FIELD_PATTERN.test(f.name));
    ...
    function runQuery(query, allStreams) {
        // biome-ignore lint/suspicious/noEvolvingTypes: this standalone design artifact deliberately uses dynamic preview behavior.
        const hits = [];
    ...
    function computeFacets(hits) {
        ...
        for (const { stream, record } of hits) {
            ...
            for (const f of stream.schema.fields) {
                ...
                if (CATEGORY_FIELD_PATTERN.test(f.name)) {
                    const v = record[f.name];
                    if (v) {
