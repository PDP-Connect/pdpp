...
const AMOUNT_FIELD_PATTERN = /amount/i;
const MERCHANT_FIELD_PATTERN = /merchant|payee|counterparty|seller/i;
const CATEGORY_FIELD_PATTERN = /category|kind|type/i;
const MEMO_FIELD_PATTERN = /memo|note|description/i;
const IMAGE_FIELD_PATTERN = /thumb|image|photo|picture/i;
const CAPTION_FIELD_PATTERN = /caption|title|subject/i;
const CALENDAR_TITLE_FIELD_PATTERN = /title|subject|name/i;
const LATITUDE_FIELD_PATTERN = /^lat(itude)?$/i;
const LONGITUDE_FIELD_PATTERN = /^l(ng|on|ongitude)$/i;
const START_FIELD_PATTERN = /^start/i;
const NON_MEASURE_FIELD_PATTERN = /^(lat|lng|longitude|latitude|id|.*_id|.*_count)$/i;
...
(() => {
  ...
  function LedgerView({ stream, selectedId, onSelect }) {
    ...
    const amountField = (
      ...
      fields.find((f) => f.type === "number" && AMOUNT_FIELD_PATTERN.test(f.name))
    )?.name;
    const merchantField = fields.find((f) => MERCHANT_FIELD_PATTERN.test(f.name))?.name;
    const catField = fields.find((f) => CATEGORY_FIELD_PATTERN.test(f.name) && f.type === "enum")?.name;
    const timeField = getTimeField(stream);
    const memoField = fields.find((f) => MEMO_FIELD_PATTERN.test(f.name))?.name;
      ...
      <div className="exp-ledger">
        <div>
          <div className="exp-ledger__month-strip">
            {monthBuckets.map((m) => (
              // biome-ignore lint/a11y/useButtonType: this standalone design artifact deliberately uses dynamic preview behavior.
              <button
          ...
          <div className="exp-ledger__rows">
            {sorted.map((r) => (
              // biome-ignore lint/a11y/noStaticElementInteractions: this standalone design artifact deliberately uses dynamic preview behavior.
              // biome-ignore lint/a11y/noNoninteractiveElementInteractions: this standalone design artifact deliberately uses dynamic preview behavior.
  ...
  function useMonths(records, timeField) {
    ...
    for (const r of records) {
      ...
      if (!buckets.has(k)) {
        buckets.set(k, { key: k, net: 0, count: 0 });
      }
  ...
  function GalleryView({ stream, selectedId, onSelect }) {
    ...
    const imgField = (
      ...
      fields.find((f) => IMAGE_FIELD_PATTERN.test(f.name))
    )?.name;
    const capField = fields.find((f) => CAPTION_FIELD_PATTERN.test(f.name))?.name;
    const timeField = getTimeField(stream);
  ...
  function MapView({ stream, selectedId, onSelect }) {
    ...
    const latField = fields.find((f) => LATITUDE_FIELD_PATTERN.test(f.name))?.name;
    const lngField = fields.find((f) => LONGITUDE_FIELD_PATTERN.test(f.name))?.name;
    const labelField = fields.find((f) => CAPTION_FIELD_PATTERN.test(f.name))?.name;
    const timeField = getTimeField(stream);
    ...
    const pts = stream.records.filter(
      (r) => r[latField] !== null && r[latField] !== undefined && r[lngField] !== null && r[lngField] !== undefined
    );
    if (!pts.length) {
  ...
  function CalendarView({ stream, selectedId, onSelect }) {
    ...
    const startField = fields.find((f) => START_FIELD_PATTERN.test(f.name))?.name ?? getTimeField(stream);
    const titleField = fields.find((f) => CALENDAR_TITLE_FIELD_PATTERN.test(f.name))?.name;
  ...
  function ChartView({ stream }) {
    ...
    const timeField = getTimeField(stream);
    const isMeasure = (f) => f.type === "number" && !NON_MEASURE_FIELD_PATTERN.test(f.name);
    ...
    function valuesForMeasure(f) {
      return [...stream.records]
        .filter((r) => r[f.name] !== null && r[f.name] !== undefined && r[timeField])
