...
const { fmtRelative, fmtClock, fmtDate, fmtDay, fmtCurrency, fmtDuration, fmtDistance,
        Heatmap, Sparkline, getTimeField, getRecordTitle, NOW } = window.PDPPPrim;
...
function GalleryView({ stream, selectedId, onSelect }) {
  const fields = stream.schema.fields;
  const imgField = (fields.find((f) => f.type === "blob" && (f.media_type ?? "").startsWith("image/"))
                ?? fields.find((f) => /thumb|image|photo|picture/i.test(f.name)))?.name;
  ...
  return (
    <div className="exp-map">
      ...
      <div className="exp-map__legend">
        {pts.length} record{pts.length === 1 ? "" : "s"} · bbox {minLat.toFixed(2)},{minLng.toFixed(2)} → {maxLat.toFixed(2)},{maxLng.toFixed(2)}
      </div>
    </div>
  );
}
...
function CalendarView({ stream, selectedId, onSelect }) {
  ...
  return (
    <>
      <div style={{ marginBottom: "0.75rem", fontFamily: "var(--font-mono)", fontSize: "0.78rem", color: "var(--muted-foreground)" }}>
        {today.toLocaleDateString("en-US", { month: "long", year: "numeric" })}
      </div>
      <div className="exp-cal">
        {["Sun","Mon","Tue","Wed","Thu","Fri","Sat"].map((d) => (
          <div className="exp-cal__dow" key={d}>{d}</div>
        ))}
        {cells.map((d) => {
    ...
    <div className="exp-chart-grid">
      <div className="exp-chart-card">
        <div className="exp-chart-card__head">
          ...
          <span className="exp-chart-card__sub">{stream.records.length} records · {dayCounts.size} active days</span>
        </div>
