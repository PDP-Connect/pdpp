...
const TITLE_FIELD_PATTERN = /title|subject|merchant|name/i;
const AUTHOR_FIELD_PATTERN = /author|from|sender|user/i;
const BODY_FIELD_PATTERN = /body|text|message|content|snippet/i;
const READER_BODY_FIELD_PATTERN = /body|content/i;
const CHANNEL_FIELD_PATTERN = /channel|thread|conversation/i;
const SUBJECT_FIELD_PATTERN = /subject|title/i;
const ACTOR_FIELD_PATTERN = /actor|author|from|user/i;
...
(() => {
  ...
  function TableView({ stream, selectedId, onSelect, projection }) {
    ...
    const priority = (f) => {
      ...
      if (f.type === "text" && TITLE_FIELD_PATTERN.test(f.name)) {
        return 1;
      }
      if (f.type === "person") {
        return 2;
  ...
  function ConversationView({ stream, selectedId, onSelect }) {
    ...
    const authorField = fields.find((f) => AUTHOR_FIELD_PATTERN.test(f.name))?.name;
    const bodyField = fields.find((f) => BODY_FIELD_PATTERN.test(f.name))?.name;
    const channelField =
      fields.find((f) => CHANNEL_FIELD_PATTERN.test(f.name) && f.type !== "id")?.name ??
      fields.find((f) => CHANNEL_FIELD_PATTERN.test(f.name))?.name;
    const subjectField = fields.find((f) => SUBJECT_FIELD_PATTERN.test(f.name))?.name;
  ...
  function ReaderView({ stream, selectedId, onSelect }) {
    ...
    const titleField = fields.find((f) => SUBJECT_FIELD_PATTERN.test(f.name))?.name;
    const bodyField = fields.find((f) => READER_BODY_FIELD_PATTERN.test(f.name))?.name;
    const timeField = getTimeField(stream);
    const actorField = fields.find((f) => ACTOR_FIELD_PATTERN.test(f.name))?.name;
