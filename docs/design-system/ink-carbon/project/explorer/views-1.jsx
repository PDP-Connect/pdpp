...
const { fmtRelative, fmtClock, fmtDate, fmtDay, fmtCurrency, fmtDuration, fmtDistance,
        Avatar, getTimeField, getRecordTime, getRecordTitle } = window.PDPPPrim;
...
function TableView({ stream, selectedId, onSelect, projection }) {
  ...
  const cols = [...visibleFields]
    .filter((f) => f.type !== "id" && f.type !== "blob" && f.type !== "geo" && f.type !== "json" && f.type !== "person[]")
  ...
  return (
    <div className="exp-tl">
      <div className="exp-tl__list">
        {days.map(({ day, items }) => (
          <div key={day}>
            <div className="exp-tl__day-label">{fmtDay(day + "T12:00:00Z")} · {items.length} record{items.length === 1 ? "" : "s"}</div>
            {items.map((r) => (
              <div
              ...
              >
                ...
                <div className="exp-tl__content">
                  ...
                  <small>{[r.from, r.author, r.actor, r.merchant, r.title, r.channel].filter(Boolean).slice(0, 1).join(" · ")
                    || r.id}</small>
                </div>
              </div>
            ))}
      ...
      <div className="exp-tl__scrubber">
        {months.map(([m, count]) => (
          <span className="exp-tl__scrubber-month" key={m} data-active>
            {new Date(m + "-15").toLocaleDateString("en-US", { month: "short" })} <span style={{ opacity: 0.6 }}>{count}</span>
          </span>
        ))}
      </div>
    </div>
...
function ConversationView({ stream, selectedId, onSelect }) {
  ...
  const channelField = fields.find((f) => /channel|thread|conversation/i.test(f.name) && f.type !== "id")?.name
  ...
  const activeRecords = (groups.get(active) ?? []).slice().sort((a, b) =>
          ...
          >
            ...
            <div>
              <div className="exp-conv__head">
                <span className="exp-conv__head-name">{String(r[authorField] ?? "·").replace(/<[^>]+>/g, "").trim()}</span>
                <span className="exp-conv__head-time">{fmtRelative(r[timeField])}</span>
                {subjectField && r[subjectField] ? <span className="exp-conv__head-channel">· {r[subjectField]}</span> : null}
              </div>
              ...
              {Array.isArray(r.reactions) && r.reactions.length > 0 ? (
                <div className="exp-conv__react">
                  {r.reactions.map((rx, i) => (
                    <span key={i}>{rx.emoji} {rx.count}</span>
                  ))}
                </div>
              ) : null}
            </div>
