...
const IMAGE_FIELD_PATTERN = /thumb|image|photo|picture|avatar/i;
const COUNTERPARTY_FIELD_PATTERN = /merchant|payee|counterparty|recipient|seller/i;
const DURATION_FIELD_PATTERN = /^duration/i;
...
function detect(stream) {
  ...
  const _fieldByName = Object.fromEntries(fields.map((f) => [f.name, f]));
  ...
  const signals = {};
  const declare = (cap, capabilityFields) => {
    activated.add(cap);
    signals[cap] = capabilityFields.filter(Boolean);
  };
  ...
  const namedImg = fields.find((f) => IMAGE_FIELD_PATTERN.test(f.name) && (f.type === "blob" || f.type === "url"));
  if (blobImg || namedImg) {
  ...
  if (amountField) {
    const counterparty = fields.find((f) => COUNTERPARTY_FIELD_PATTERN.test(f.name));
  ...
  const hasDuration = fields.some((f) => DURATION_FIELD_PATTERN.test(f.name));
