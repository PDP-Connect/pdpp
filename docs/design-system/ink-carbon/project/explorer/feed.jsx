    ...
    <article className="card card--message" data-selected={selected} onClick={onClick}>
      ...
      {Array.isArray(record.reactions) && record.reactions.length ? (
        <div className="card__react">
          {record.reactions.map((rx, i) => <span key={i}>{rx.emoji} {rx.count}</span>)}
        </div>
      ) : null}
    </article>
  );
  ...
  return (
    <article className="card card--event" data-selected={selected} onClick={onClick}>
      ...
      {Array.isArray(record[attendField]) && record[attendField].length ? (
        <div className="card__meta-row">
          <span className="card__meta-text">{record[attendField].length} attendee{record[attendField].length === 1 ? "" : "s"}</span>
        </div>
      ) : null}
    </article>
  );
...
function ActivityCard({ stream, record, selected, onClick }) {
  ...
  const time = recordTimeISO(stream, record);
  const stats = [];
  if (distField && record[distField] != null) stats.push({ label: "distance", value: fmtDistance(record[distField]) });
  if (durField  && record[durField]  != null) stats.push({ label: "duration", value: fmtDuration(record[durField]) });
  if (elevField && record[elevField] != null) stats.push({ label: "elevation", value: `${Math.round(record[elevField])}m` });
  if (!stats.length && scoreField && record[scoreField] != null) stats.push({ label: scoreField, value: String(record[scoreField]) });
  return (
...
function ReaderCard({ stream, record, selected, onClick }) {
  ...
  return (
    <article className="card card--reader" data-selected={selected} onClick={onClick}>
      ...
      {record[actorField] ? <div className="card__meta-text" style={{ marginTop: "0.5rem" }}>by {cleanPerson(record[actorField])}</div> : null}
    </article>
  );
}
        ...
        {showDiscover && DayStory ? (
          <>
            ...
            <div className="feed__empty">
              ...
              <div className="feed__empty-sub">Pick a person, a merchant, or a channel above to wander further back.</div>
            </div>
          </>
        ) : (
          <div className="feed__empty">
      ...
      {memories.length ? (
        <section className="memories">
          <header className="memories__head">
            ...
            <span className="memories__sub">{memories.length} memor{memories.length === 1 ? "y" : "ies"}</span>
          </header>
