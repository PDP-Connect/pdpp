...
function Peek({ stream, record, onClose, projection }) {
  ...
  function renderValue(field) {
    ...
    if (field.type === "timestamp")
      return <span style={{ fontFamily: "var(--font-mono)" }}>{new Date(v).toLocaleString("en-US", { dateStyle: "medium", timeStyle: "short" })}</span>;
    if (field.type === "currency") return <span style={{ fontFamily: "var(--font-mono)", color: v > 0 ? "var(--success)" : "var(--foreground)" }}>{fmtCurrency(v)}</span>;
    if (field.type === "number") {
      if (field.unit === "meters") return <span style={{ fontFamily: "var(--font-mono)" }}>{fmtDistance(v)} <small style={{ color: "var(--muted-foreground)" }}>({v.toLocaleString()} m)</small></span>;
      if (field.unit === "seconds") return <span style={{ fontFamily: "var(--font-mono)" }}>{fmtDuration(v)}</span>;
      return <span style={{ fontFamily: "var(--font-mono)" }}>{v.toLocaleString()}{field.unit ? <small style={{ color: "var(--muted-foreground)" }}> {field.unit}</small> : null}</span>;
    }
    if (field.type === "id") return <code style={{ fontSize: "0.72rem", color: "var(--muted-foreground)" }}>{v}</code>;
    if (field.type === "url") return <a href={v} rel="noreferrer" style={{ color: "var(--primary)", textDecoration: "underline" }} target="_blank">{v}</a>;
    if (field.type === "blob") {
    ...
    if (field.type === "person") return <span>{String(v).replace(/<[^>]+>/g, "").trim()}</span>;
    if (Array.isArray(v)) {
      if (v.length === 0) return <span style={{ color: "var(--muted-foreground)" }}>[]</span>;
      if (typeof v[0] === "object") return <pre style={{ fontFamily: "var(--font-mono)", fontSize: "0.7rem", whiteSpace: "pre-wrap" }}>{JSON.stringify(v, null, 2)}</pre>;
  ...
  return (
    <aside className="exp-peek">
      <button className="exp-peek__close" onClick={onClose} title="Close">×</button>
      <div className="exp-peek__head">
        <div className="exp-peek__eyebrow">{stream.connector_id} / {stream.name}</div>
      ...
      <div className="exp-peek__body">
          ...
          return (
            <div className="exp-peek__field" data-redacted={isRedacted} key={f.name}>
              ...
              <div className="exp-peek__field-value">
                {isRedacted ? (
                  <span>redacted — {f.redacted_reason ?? "out of scope"}</span>
                ) : (
                  renderValue(f)
                )}
              </div>
        ...
        {projection && redacted.length ? (
          <div style={{ marginTop: "0.9rem", fontFamily: "var(--font-mono)", fontSize: "0.7rem", color: "var(--muted-foreground)" }}>
