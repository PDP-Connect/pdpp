...
(() => {
  ...
  function recordMatchesChip(stream, r, chip) {
    switch (chip.field) {
      case "stream": {
        ...
        return v.some(
          (x) => stream.connector_id === x || stream.name === x || `${stream.connector_id}/${stream.name}` === x
        );
      }
      case "connection": {
        const v = Array.isArray(chip.value) ? chip.value : [chip.value];
      ...
      case "from": {
        ...
        const candidateFields = stream.schema.fields.filter(
          (f) =>
      ...
      case "amount": {
        const fs = stream.schema.fields;
        const af =
          fs.find((f) => f.type === "currency")?.name ?? fs.find((f) => AMOUNT_FIELD_PATTERN.test(f.name))?.name;
        if (!af) {
          return false;
        }
      ...
      case "channel": {
        const target = String(chip.value).toLowerCase();
        return ["channel", "channel_id"].some((k) =>
          String(r[k] ?? "")
            .toLowerCase()
            .includes(target)
        );
      ...
      case "has": {
        if (chip.value === "image") {
          return (
            stream.schema.fields.some((f) => f.type === "blob" && (f.media_type ?? "").startsWith("image/")) ||
            stream.schema.fields.some(
              (f) => IMAGE_FIELD_PATTERN.test(f.name) && (f.type === "blob" || f.type === "url") && r[f.name]
            )
          );
        }
        if (chip.value === "geo") {
  ...
  function textMatches(stream, r, text) {
    ...
    const fields = stream.schema.fields.filter(
      (f) => ["text", "person"].includes(f.type) || TEXT_FIELD_PATTERN.test(f.name)
    );
