          ...
          <header className="exp-topbar">
            ...
            <div className="exp-topbar__right">
              <ViewSwitcher
                ...
                whyOpen={whyOpen}
              />
              <GrantChip grant={grant} onToggleProjection={() => setProjection((x) => !x)} projection={projection} />
            </div>
          </header>

          {/* ── Facets ── */}
          <FacetLine activeView={activeView} facets={facets} hits={hits} onAddChip={addChip} query={query} />
    ...
    return (
      <div className="exp-facets">
        <span className="exp-facets__count">
          {totalCount === 0 ? "no results" : totalCount === 1 ? "1 record" : `${totalCount.toLocaleString()} records`}
        </span>
        {totalCount > 0 ? (
          <>
