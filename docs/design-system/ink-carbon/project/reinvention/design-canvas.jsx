...
function DesignCanvas({ children, minScale, maxScale, style }) {
  ...
  React.useEffect(() => {
    ...
    fetch(`./${DC_STATE_FILE}`)
      ...
      // biome-ignore lint/suspicious/noEmptyBlockStatements: this standalone design artifact deliberately uses dynamic preview behavior.
      .catch(() => {})
      .finally(() => {
  ...
  const sectionOrder = [];
  // biome-ignore lint/complexity/noForEach: this standalone design artifact deliberately uses dynamic preview behavior.
  ...
  React.useEffect(() => {
    ...
    const onPd = (e) => {
      ...
      if (ae?.isContentEditable && !ae.contains(e.target)) {
        ae.blur();
      }
    };
  ...
  React.useEffect(() => {
    ...
    const zoomAt = (cx, cy, factor) => {
      ...
      if (k !== 1) {
        ...
        marker = hit?.closest ? hit.closest("[data-dc-slot],[data-dc-section]") : null;
        if (marker) {
      ...
      <div style={{ display: "flex", gap, padding: "0 60px", alignItems: "flex-start", width: "max-content" }}>
        {order.map((k) => (
          <DCArtboardFrame
            ...
            // biome-ignore lint/performance/noJsxPropsBind: this standalone design artifact deliberately uses dynamic preview behavior.
            onFocus={() => ctx?.setFocus(`${sid}/${k}`)}
            // biome-ignore lint/performance/noJsxPropsBind: this standalone design artifact deliberately uses dynamic preview behavior.
            onRename={(v) => ctx?.patchSection(sid, (x) => ({ labels: { ...x.labels, [k]: v } }))}
            // biome-ignore lint/performance/noJsxPropsBind: this standalone design artifact deliberately uses dynamic preview behavior.
            onReorder={(next) => ctx?.patchSection(sid, { order: next })}
            order={order}
            sectionId={sid}
          />
        ))}
...
async function dcExport(node, w, h, name, kind) {
  ...
  while (pending.length) {
    // biome-ignore lint/performance/noAwaitInLoops: this standalone design artifact deliberately uses dynamic preview behavior.
    await pending.shift();
  }
  const fontCss = (
    await Promise.all(
      fontRules.map(async (rule) => {
        let out = rule.css,
          m;
        const re = /url\((['"]?)([^'")]+)\1\)/g;
        // biome-ignore lint/suspicious/noAssignInExpressions: this standalone design artifact deliberately uses dynamic preview behavior.
  ...
  const cloneStyled = (src) => {
    ...
    if (src.nodeType === 1) {
      ...
      let txt = "";
      // biome-ignore lint/style/useForOf: this standalone design artifact deliberately uses dynamic preview behavior.
...
function DCFocusOverlay({ entry, sectionMeta, sectionOrder }) {
  ...
  const Arrow = ({ dir, onClick }) => (
    ...
    <button
      ...
      // biome-ignore lint/performance/noJsxPropsBind: this standalone design artifact deliberately uses dynamic preview behavior.
      // biome-ignore lint/suspicious/noReturnAssign: this standalone design artifact deliberately uses dynamic preview behavior.
              ...
              {sectionOrder
                ...
                .map((sid) => (
                  ...
                  <button
                    ...
                    onClick={() => {
                      setDd(false);
                      const [f] = sectionMeta[sid].slotIds;
                      if (f) {
                        ctx.setFocus(`${sid}/${f}`);
                      }
                    }}
