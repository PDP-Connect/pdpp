...
const GrantsList = ({ onOpen }) => {
  const openGrant = (event) => onOpen?.(GRANTS.find((grant) => grant.id === event.currentTarget.dataset.grantId));
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
        ...
        <div style={{ display: "flex", gap: 6 }}>
          <button className="pdpp-btn pdpp-btn-ghost" style={{ height: 30, fontSize: 12 }} type="button">
            Filter
          </button>
          <button className="pdpp-btn pdpp-btn-outline" style={{ height: 30, fontSize: 12 }} type="button">
            Export
          </button>
        </div>
      </div>
