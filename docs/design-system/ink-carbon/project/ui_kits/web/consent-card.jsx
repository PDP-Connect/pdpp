...
const ConsentCard = () => {
  const [scopes, setScopes] = window.React.useState(DEFAULT_SCOPES);
  const toggle = (id) => setScopes((s) => s.map((x) => (x.id === id ? { ...x, on: !x.on } : x)));
  const toggleScope = (event) => toggle(event.currentTarget.dataset.scopeId);
    ...
    <div className="pdpp-surface-human" style={{ overflow: "hidden", maxWidth: 640 }}>
      ...
      <div>
        {scopes.map((s, i) => (
          ...
          >
            ...
            >
              {s.on && (
                <svg aria-hidden="true" style={{ position: "absolute", inset: 0 }} viewBox="0 0 18 18">
                  <path
      ...
      <div style={{ padding: "14px 24px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button className="pdpp-btn pdpp-btn-ghost" style={{ height: 34, fontSize: 13 }} type="button">
            Deny
          </button>
          <button
