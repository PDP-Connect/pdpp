...
const RR_SYNCS_PAIRING_CODE = /code (\S+)/;
...
(() => {
          ...
          return (
            <div className="rr-sync-group" key={g.cin}>
              ...
              <div className="pdpp-table rr-cols-sync">
                ...
                {g.streams.map(
                  // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: each stream row keeps its ordered health, schedule, and disclosure rendering together.
                  (s) => {
    ...
    return (
      <div className="rr-ops">
        {RR2.apps.map((a) => {
          ...
          const code = pending ? (a.detail.match(RR_SYNCS_PAIRING_CODE) || [])[1] : null;
          return (
  ...
  function ExportersView() {
    ...
    return (
      <div className="rr-ops">
        {RR2.exporters.map((e) => {
          const isPaused = paused[e.device] ?? e.status === "paused";
          return (
