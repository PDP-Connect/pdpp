...
const SYNCS = [
  { con: "Northstar HR", cin: "cin_nh_e3391c", health: "ok",
    streams: [
      { stream: "pay_statements", cadence: "with payroll", next: "Jun 12 · 06:00Z", rhythm: ["ok", "ok", "ok", "ok", "ok"], delta: "+2 records", when: "today 06:00Z", dur: "18 s" },
      { stream: "employment", cadence: "daily · 06:00Z", next: "Jun 12 · 06:00Z", rhythm: ["ok", "ok", "ok", "ok", "ok"], delta: "no change", when: "today 06:00Z", dur: "4 s", quiet: true },
      { stream: "tax_docs", cadence: "yearly · Feb 01", next: "2027 · Feb 01", rhythm: ["ok"], delta: "no change", when: "Feb 01", dur: "2 s", quiet: true },
    ] },
  { con: "First Meridian — checking", cin: "cin_fm_206b11", health: "failing",
    fix: {
    ...
    streams: [
      { stream: "transactions", cadence: "daily · 05:00Z", next: "held", rhythm: ["ok", "ok", "ok", "ok", "fail"], delta: "held at Jun 10", when: "Jun 11 05:00Z", dur: "2 s", failed: true },
      { stream: "statements", cadence: "monthly", next: "held", rhythm: ["ok", "ok", "fail"], delta: "held at May", when: "Jun 11 05:00Z", dur: "—", failed: true },
      { stream: "balances", cadence: "daily · 05:00Z", next: "held", rhythm: ["ok", "ok", "ok", "fail"], delta: "held at Jun 10", when: "Jun 11 05:00Z", dur: "—", failed: true },
    ] },
  { con: "Gmail — personal", cin: "cin_gm_410c2b", health: "ok",
    streams: [
      { stream: "messages", cadence: "every 15 min", next: "in 11 min", rhythm: ["ok", "ok", "ok", "ok", "ok"], delta: "+38 records", when: "31 min ago", dur: "6 s" },
      { stream: "threads", cadence: "every 15 min", next: "in 11 min", rhythm: ["ok", "ok", "ok", "ok", "ok"], delta: "+12 records", when: "31 min ago", dur: "5 s" },
      { stream: "attachments", cadence: "every 15 min", next: "in 11 min", rhythm: ["ok", "ok", "ok", "ok", "ok"], delta: "+3 records", when: "2 h ago", dur: "9 s" },
    ] },
  { con: "Tonal", cin: "cin_tn_77f024", health: "ok",
    streams: [
      { stream: "listening_history", cadence: "every 15 min", next: "in 3 min", rhythm: ["ok", "ok", "ok", "ok", "ok"], delta: "+4 records", when: "12 min ago", dur: "7 s" },
    ] },
];
    ...
    <div className="rr-sync">
      <div className="rr-sync-health">
        ...
        <div className="rr-sync-health__stat rr-sync-health__stat--note">
          <span className="rr-sync-health__note">Nothing already saved is ever lost — a held connection only pauses new arrivals.</span>
        </div>
      </div>
      ...
      {fixGroup && !fixed && (
        <div className="rr-fix">
          ...
          <div className="rr-fix__act">
            <button className="pdpp-btn pdpp-btn--sm" onClick={() => setFixed(true)} type="button">{fixGroup.fix.action}</button>
          </div>
        </div>
      )}
        ...
        return (
          <div className="rr-sync-group" key={g.cin}>
            <div className="rr-sync-group__head">
              ...
              <span className="rr-sync-group__count">{g.streams.length} {g.streams.length === 1 ? "stream" : "streams"}</span>
            </div>
                ...
                return (
                  <React.Fragment key={s.stream}>
                    ...
                    >
                      ...
                      <span className={"rr-sync-row__delta" + (s.quiet ? " is-quiet" : "") + (failed ? " is-failed" : "")}>
                    ...
                    {isOpen && (
                      <div className="rr-sync-detail">
                        <div className="rr-sync-detail__kv">
                          ...
                          <span className="rr-sync-detail__v">{s.when} · {s.dur}</span>
                        ...
                        <button className="rr-link" type="button">browse this stream →</button>
                      </div>
                    )}
                  </React.Fragment>
                );
        ...
        return (
          <div className={"rr-op" + (pending ? " is-action" : "")} key={a.name}>
            ...
            <span className="rr-op__say">
              {pending ? "Waiting for the device code to be entered on this console." : "Reads through your grants — never more than the grant behind it."}
            </span>
            <span className="rr-op__side">
              {pending ? (
                <span className="rr-op__pending">
                  ...
                  <button className="pdpp-btn pdpp-btn--sm" type="button">Enter code</button>
                </span>
              ) : (
        ...
        return (
          <div className={"rr-op" + (isPaused ? " is-action" : "")} key={e.device}>
            ...
            <span className="rr-op__side">
              ...
              <button className="pdpp-btn pdpp-btn--ghost pdpp-btn--sm" onClick={() => setPaused((p) => ({ ...p, [e.device]: !isPaused }))} type="button">
      ...
      {RR2.subscriptions.map((s) => (
        <div className="rr-op" key={s.url}>
          ...
          <span className="rr-op__events">
            {s.events.split(" · ").map((ev) => <span className="rr-op__event" key={ev}>{ev}</span>)}
          </span>
          <span className="rr-op__side">
            <span className="rr-op__settled">
              <span className="pdpp-endorse pdpp-endorse--continuous">{s.status}</span>
              <button className="rr-link rr-op__test" type="button">test</button>
            </span>
          </span>
        </div>
      ))}
...
function AttentionList({ onGo }) {
  const items = [
    { sev: "fail", name: "First Meridian can't sync", say: "OFX session expired — transactions held at the Jun 10 cursor.", action: "Reauthorize", go: "syncs" },
    { sev: "warn", name: "TaxPrep Co grant expiring", say: "tax_docs.read · single use · still unused.", meta: "26 h left", go: "grants" },
    { sev: "warn", name: "Backups not configured", say: "Your copy deserves a copy — set a snapshot target.", action: "Set up", go: "deployment" },
  ];
  return (
    <div className="rr-ops">
      {items.map((it) => (
        <div className={"rr-op rr-op--attn is-" + it.sev} key={it.name}>
          ...
          <span className="rr-op__side">
            {it.action
              ? <button className="pdpp-btn pdpp-btn--sm" onClick={() => onGo(it.go)} type="button">{it.action}</button>
              : <button className="rr-link" onClick={() => onGo(it.go)} type="button">{it.meta} →</button>}
          </span>
        </div>
      ))}
    </div>
