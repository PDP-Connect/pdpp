...
(() => {
  const { React, RRX } = window;
  const { useState } = React;

  function SourcesView({ grants, onBrowse }) {
          ...
          {RRX.connections.map((c) => {
            ...
            return (
              <button
                ...
                key={c.id}
                onClick={
                  // biome-ignore lint/performance/noJsxPropsBind: this prototype row handler intentionally closes over its dynamic connection identity.
                  () => {
                    setSel(c.id);
                    setRevoking(false);
                  }
