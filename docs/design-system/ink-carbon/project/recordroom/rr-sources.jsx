        ...
        {RRX.connections.map((c) => {
          ...
          return (
            <button
              ...
              key={c.id}
              onClick={() => { setSel(c.id); setRevoking(false); }}
              type="button"
            >
      ...
      <div className="rr-s-detail">
        <div className="pdpp-sheet">
          <div className="pdpp-sheet__head">
            <h3 className="pdpp-sheet__title rr-x-sheet-title">
              {revoked ? <s>{con.name}</s> : con.name}
            </h3>
            <span className="pdpp-sheet__serial">{con.cin}</span>
          </div>
          <div className="pdpp-sheet__body">
            <div className="pdpp-kv">
              <div className="pdpp-kv__row"><span className="pdpp-kv__k">kind</span><span className="pdpp-kv__v">{con.kind}</span></div>
              <div className="pdpp-kv__row"><span className="pdpp-kv__k">account</span><span className="pdpp-kv__v">{con.account}</span></div>
              <div className="pdpp-kv__row"><span className="pdpp-kv__k">config</span><span className="pdpp-kv__v">{con.config}</span></div>
              <div className="pdpp-kv__row"><span className="pdpp-kv__k">auth</span><span className="pdpp-kv__v">{revoked ? "revoked" : con.auth}</span></div>
              <div className="pdpp-kv__row"><span className="pdpp-kv__k">schedule</span><span className="pdpp-kv__v">{con.schedule}</span></div>
              <div className="pdpp-kv__row"><span className="pdpp-kv__k">last run</span><span className="pdpp-kv__v">{synced.includes(con.id) ? "ok · just now" : con.lastRun}</span></div>
              <div className="pdpp-kv__row"><span className="pdpp-kv__k">added</span><span className="pdpp-kv__v">{con.added}</span></div>
            </div>
            {revoked && (
          ...
          <div className="pdpp-sheet__foot">
            <span className="rr-s-actions">
              {!revoked && (
                <button className="pdpp-btn pdpp-btn--ghost pdpp-btn--sm" disabled={syncing} onClick={syncNow} type="button">
                  {syncing ? "syncing…" : "Sync now"}
                </button>
              )}
              {status === "reauth" && (
                <button className="pdpp-btn pdpp-btn--sm" type="button">Reauthorize</button>
              )}
              {!revoked && !revoking && (
                <button className="pdpp-btn pdpp-btn--destructive pdpp-btn--sm" onClick={() => setRevoking(true)} type="button">Revoke instance</button>
              )}
              {!revoked && revoking && (
                <span style={{ display: "flex", gap: 8 }}>
                  <button className="pdpp-btn pdpp-btn--ghost pdpp-btn--sm" onClick={() => setRevoking(false)} type="button">Keep</button>
                  <button className="pdpp-btn pdpp-btn--destructive pdpp-btn--sm" onClick={() => { setLocalRevoked((cur) => [...cur, con.id]); setRevoking(false); }} type="button">Confirm revoke</button>
                </span>
              )}
            </span>
            <button className="rr-link" onClick={() => onBrowse(con.id, null)} type="button">browse records →</button>
          </div>
        </div>
        ...
        <div className="rr-s-manifest">
          ...
          <p className="rr-s-note">
            {"\u201c"}sealed{"\u201d"} streams hold binary or machine arguments — browsable and linked, not text-searched.
            Click any stream to read its records in Explore.
          </p>
        </div>
      </div>
    </div>
