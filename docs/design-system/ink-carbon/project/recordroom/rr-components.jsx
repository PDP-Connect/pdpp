...
const RR_COMPONENTS_WHITESPACE = /\s+/;
...
(() => {
  ...
  function monogram(name) {
    const words = name.split(RR_COMPONENTS_WHITESPACE).filter(Boolean);
    const [firstWord = "", secondWord = ""] = words;
    return (words.length > 1 ? firstWord[0] + secondWord[0] : name.slice(0, 2)).toUpperCase();
  }
    ...
    return (
      ...
      >
        <div className={cls} style={{ "--cols": "inherit" }}>
          ...
          <span>
            <Endorse hours={grant.hoursLeft} status={grant.status} />
          </span>
  ...
  function Inspector({ grant, streams, log, revoking, striking, onRevokeStart, onRevokeConfirm, onRevokeCancel }) {
    ...
    const granted = grant.scopes.map((s) => {
      const [sid] = s.name.split(".");
      const stream = (streams || []).find((x) => x.id === sid);
      const proj = grant.projections?.[sid] || [];
            ...
            <div className="pdpp-sheet__body">
              ...
              {granted.map(({ sid, s, proj, total }) => (
                <div className="rr-insp-item" key={sid}>
                  ...
                  <span className="rr-insp-item__s">
                    {proj.length}
                    {total ? ` of ${total}` : ""} fields cross · {s.terms}
                  </span>
              ...
              {reads.length > 0 && (
                <div className="rr-insp-pulse">
                  {reads.length} read{reads.length === 1 ? "" : "s"} on record · last {reads[0].t.slice(5)}
                </div>
              )}
            </div>
      ...
      <div className="pdpp-sheet rr-stream">
        <div className="pdpp-sheet__head">
          ...
          <span style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <span className="pdpp-sheet__serial">
              {stream.connector} · {stream.records} records
            </span>
          </span>
        </div>
  ...
  function Ceremony({ request, pressing, onToggle, onApprove, onRefuse, onDismiss }) {
    ...
    return (
      // biome-ignore lint/a11y/noStaticElementInteractions lint/a11y/noNoninteractiveElementInteractions lint/a11y/useKeyWithClickEvents: the backdrop has an equivalent document-level Escape dismissal path.
      <div
        className="rr-overlay"
        onClick={
          // biome-ignore lint/performance/noJsxPropsBind: prototype callback captures this rendered fixture state.
          (event) => {
            if (!(pressing || event.target !== event.currentTarget)) {
              onDismiss();
            }
          }
        }
      >
        <div className={`rr-ceremony rr-paper-scope${pressing ? "is-pressing" : ""}`}>
          <div className="pdpp-carbon">
            <div className="pdpp-sheet">
              ...
              <div className="rr-ceremony__body">
                <h2 className="rr-ceremony__title">
                  {request.client} asks to read {request.scopes.length} streams
                </h2>
                <p className="rr-ceremony__sub">
                  Purpose: {request.purposeHuman}. Decide stream by stream — anything you decline stays on the record as
                  declined.
                </p>
