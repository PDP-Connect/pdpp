...
const RR_EXPLORE_TOKEN_SEPARATOR = /\s+/;
const RR_EXPLORE_TYPED_TOKEN = /^([a-z_]+):(.+)$/i;
const RR_EXPLORE_FORM_CONTROL = /^(INPUT|TEXTAREA|SELECT)$/;

(() => {
  const { React } = window;
  const { useState, useEffect, useMemo } = React;
  const { RRX } = window;
  const { labelFor, displayTitle, RecordBody } = window.RRREC;

  const conById = {};
  for (const c of RRX.connections) {
    conById[c.id] = c;
  }
  const recById = {};
  for (const r of RRX.records) {
    recById[r.id] = r;
  }
  ...
  function CopyMono({ text }) {
    ...
    return (
      <button
        ...
        onClick={
          // biome-ignore lint/performance/noJsxPropsBind: prototype callback captures this rendered fixture state.
          () => {
  ...
  // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: the parser intentionally keeps the ordered token grammar in one locally auditable decision table.
  function parseQuery(q) {
    ...
    for (const tok of q.trim().split(RR_EXPLORE_TOKEN_SEPARATOR).filter(Boolean)) {
      const m = tok.match(RR_EXPLORE_TYPED_TOKEN);
      if (!m) {
  ...
  // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: this prototype view owns an order-coupled query, facet, selection, and rendering transaction.
  function ExploreView({ grants, onGo, onJump, seed }) {
    ...
    // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: these ordered predicates mirror the query grammar and must short-circuit in source order.
    function passes(r, opts) {
      const options = opts || {};
      const con = conById[r.con];
      if (!options.ignoreCon && conSel && r.con !== conSel) {
        return false;
      }
      if (!options.ignoreStream && streamSel && r.stream !== streamSel) {
        return false;
      }
      ...
      if (parsed.hasLink && !r.links?.length) {
        return false;
      }
    ...
    const compiled = useMemo(
      // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: the ordered query serialization mirrors the visible filter grammar and preserves parameter order.
      () => {
        const parts = [];
    ...
    const related = [
      ...(rec.links || []).map(([rel, id]) => [rel, id]),
      ...(backlinks[rec.id] || []).filter(([, id]) => !fwdIds.has(id)),
    ...
    useEffect(() => {
      // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: the keyboard state machine keeps ordered guards and transitions together.
      function onKey(e) {
        if (e.target && RR_EXPLORE_FORM_CONTROL.test(e.target.tagName)) {
          return;
        }
    ...
    const activeChips = [];
    if (conSel) {
      activeChips.push({ id: "con", label: conById[conSel].name, clear: () => setConSel(null) });
    }
    if (streamSel) {
      activeChips.push({ id: "stream", label: `stream: ${streamSel}`, clear: () => setStreamSel(null) });
    }
    if (range !== "all") {
      activeChips.push({ id: "range", label: range, clear: () => setRange("all") });
    }
    for (const [i, tk] of parsed.tokens.entries()) {
      activeChips.push({
        id: `tok${i}`,
        label: tk.label,
        clear: () =>
          setQ(
            q
              .split(RR_EXPLORE_TOKEN_SEPARATOR)
              .filter((x) => x !== tk.raw)
              .join(" ")
          ),
      });
    ...
    return (
      <div className="rr-x">
        ...
        <aside className="rr-x-rail">
          ...
          <div className="rr-x-facets">
            <span className="rr-x-facets__label">{conSel ? `Streams — ${conById[conSel].name}` : "Stream names"}</span>
            {!conSel && (
            ...
            {streamFacets.map(([s, n]) => (
              <button
                ...
                onClick={
                  // biome-ignore lint/performance/noJsxPropsBind: prototype callback captures this rendered fixture state.
                  () => setStreamSel(streamSel === s ? null : s)
                }
                type="button"
              >
        ...
        <div className="rr-x-main">
          <div className="rr-x-controls">
            ...
            <div className="rr-x-ranges">
              ...
              <button className="rr-link rr-x-jump" onClick={onJump} type="button">
                jump to an id →
              </button>
            </div>
          ...
          <p className="rr-x-pulse__note">
            {recordCount} records shown · {RRX.totalOnServer} on your server
          </p>
          ...
          {showPartial && (
            <div className="rr-x-partial">
              <button
                ...
                type="button"
              >
                <span className="rr-x-partial__line">
                  Partial view — {partialCon.name} didn't answer for {RRX.partial.streams.length} streams
                </span>
              ...
              {partialOpen && (
                <div className="rr-x-partial__body">
                  <p className="rr-x-partial__expl">
                    This connection was revoked {RRX.partial.revokedOn}. Its streams ({RRX.partial.streams.join(", ")})
                    refuse new reads — that's the revocation holding, not a fault. Records ingested before revocation
          ...
          {RRX.days.map(([day, label]) => {
            ...
            return (
              <div className="rr-x-day" key={day}>
                ...
                {dayRows.map(
                  // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: this fixture row renderer keeps its kind-dependent presentation branches local.
                  (r) => (
                    <button
                    ...
                    >
                      ...
                      <span className={`rr-x-row__title${r.degraded ? "is-derived" : ""}`}>
                        ...
                        {(() => {
                          ...
                          return dt.kicker ? (
                            <>
                              <span className="rr-x-kicker">{dt.kicker}</span>
                              {dt.primary}
                            </>
                          ) : (
                            dt.primary
        ...
        <div className="rr-inspector">
          <div className="rr-anim-swap" key={rec.id + (lens || "you")}>
            <div className="pdpp-sheet">
              ...
              {watchers.length === 0 && (
                <p className="rr-ex-alone">
                  <b>Only you can read this.</b> No grant covers {rec.stream} on {recCon.name} — nothing here crosses.
                </p>
              )}
                ...
                {kept.length > 0 && (
                  <div className="rr-ex-keep">
                    ...
                    <span className="rr-ex-keep__note">
                      {kept.length} {kept.length === 1 ? "field" : "fields"} never leave your server — never sent, not
                      blacked out.
                    </span>
                  </div>
                )}
              ...
              <div className="pdpp-sheet__foot">
                {proj ? (
                  <span className="pdpp-copyline">
                    {shown.length} of {baseFields.length} fields cross to {lensGrant.client} · enforced on every read
                  </span>
                ) : (
                  <span className="pdpp-typed-sm" style={{ color: "var(--muted-foreground)" }}>
                    {baseFields.length} fields · readable by you
                    {watchers.length > 0
                      ? ` · ${watchers.length} ${watchers.length === 1 ? "grant reads" : "grants read"} a projection`
                      : ""}
                  </span>
                )}
              </div>
