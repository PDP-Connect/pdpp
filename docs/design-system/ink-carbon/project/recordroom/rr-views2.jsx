...
(() => {
  ...
  function MiniHead({ title, action, onAction }) {
    return (
      <div className="rr-mini-head">
        ...
        {action && (
          <button className="rr-link" onClick={onAction} type="button">
            {action} →
          </button>
        )}
      </div>
      ...
      <div className="pdpp-table rr-cols-traces">
        ...
        {rows.map((tr) => (
          ...
          >
            ...
            <span className="rr-trace-row__what">
              {tr.stream} · {tr.op}
            </span>
    ...
    return (
      <div className="pdpp-sheet rr-inspector">
        <div className="pdpp-sheet__head">
          <h3 className="pdpp-sheet__title" style={{ whiteSpace: "nowrap" }}>
            {trace.decision === "deny" ? "Refused" : "Served"} in {trace.dur}
          </h3>
          <CopyId id={trace.id} />
        </div>
    ...
    return (
      <div className="rr-ov">
          ...
          <div className="rr-hero pdpp-carbon">
            <div className="rr-hero__sheet">
              <span className="rr-hero__text">
                ...
                <span className="rr-hero__meta">
                  req_atlas_7f2k · purpose: mortgage_preapproval · nothing crosses until you decide
                </span>
              </span>
              <button className="pdpp-btn" onClick={onReview} type="button">
                Review request
              </button>
            </div>
          </div>
        ...
        <div className="pdpp-band">
          <div className="pdpp-band__cell">
            <span className="pdpp-band__v">10</span>
            <span className="pdpp-band__k">connections</span>
          </div>
          <div className="pdpp-band__cell">
            <span className="pdpp-band__v">34</span>
            <span className="pdpp-band__k">streams</span>
          </div>
          <div className="pdpp-band__cell">
            <span className="pdpp-band__v">48,120</span>
            <span className="pdpp-band__k">records</span>
          </div>
          <div className="pdpp-band__cell">
            <span className="pdpp-band__v">{grantsSummary}</span>
            <span className="pdpp-band__k">grants in effect</span>
          </div>
          <div className="pdpp-band__cell">
            <span className="pdpp-band__v">14</span>
            <span className="pdpp-band__k">reads this week</span>
          </div>
        </div>
      ...
      <div className="rr-attn">
        {RR2.sources.map((s) => (
          <div className="rr-attn__row" key={s.name}>
            ...
            <span className="rr-attn__detail">
              {s.kind} · {s.streams} · last sync {s.last}
            </span>
          ...
          {RR2.runs.map((r) => (
            <div className="pdpp-data-row" key={r.id} style={{ "--cols": "inherit" }}>
              ...
              <span className="pdpp-data-row__detail">
                {r.stream} · {r.upserts} upserts · cursor {r.cursor}
                {r.note ? ` · ${r.note}` : ""}
              </span>
              {r.status === "ok" ? (
          ...
          <div className="rr-attn">
            {RR2.tokens.map((t) => (
              <div className="rr-attn__row" key={t.id}>
                ...
                <span className="rr-attn__detail">
                  {t.id} · created {t.created} · for the operator and trusted local agents only
                </span>
                <span className="rr-attn__side">
      ...
      <div className="rr-attn">
        {RR2.exporters.map((e) => (
          <div className="rr-attn__row" key={e.device}>
            ...
            <span className="rr-attn__detail">
              pushes to your server — nothing transits a third party · {e.records} records
            </span>
            <span className="rr-attn__side">
              {e.status === "ok" ? (
  ...
  function CommandPalette({ open, onClose, items, recents = [], onExec }) {
    ...
    return (
      // biome-ignore lint/a11y/noStaticElementInteractions lint/a11y/noNoninteractiveElementInteractions lint/a11y/useKeyWithClickEvents: the backdrop closes by pointer while the palette's existing keyboard handler provides Escape dismissal.
      <div
      ...
      >
        <div className="rr-palette">
          ...
          <div className="rr-palette__list">
            {filtered.length === 0 && (
              <div className="rr-palette__empty">Nothing matches — the record is honest about that.</div>
            )}
            {filtered.map((it, i) => (
              <button
