...
function scopeHuman(name) { return SCOPE_HUMAN[name] || name.replace(/\.read$/, "").replace(/_/g, " "); }
...
const STREAM_RECORD_NOUN = {
  pay_statements: "pay records", employment: "employment records",
  listening_history: "listening records", tax_docs: "tax records",
  transactions: "transactions",
};
function recordNoun(stream) { return STREAM_RECORD_NOUN[stream] || stream.replace(/_/g, " ") + " records"; }
...
function Overview({ grants, requestState, onReview, onGo, onOpenGrant }) {
  ...
  if (pending) {
    hero = {
      ...
      line: <React.Fragment>Atlas Mortgage wants to read <em>your pay, employment, and spending</em>.</React.Fragment>,
      sub: "Nothing leaves until you say so — approve it one piece at a time.",
      cta: <button className="pdpp-btn pdpp-btn--human" onClick={onReview} type="button">Review the request</button>,
    };
  } else if (hasFailure) {
    hero = {
      tone: "alarm",
      kicker: "One thing needs you",
      line: <React.Fragment>Your bank data <em>stopped arriving</em> on Jun 11.</React.Fragment>,
      sub: "First Meridian's connection expired. Nothing you already have is lost — but nothing new arrives until you reconnect.",
      cta: <button className="pdpp-btn pdpp-btn--sm" onClick={() => { setResolved(true); }} type="button">Reconnect the bank</button>,
    };
  } else {
    hero = {
      tone: "calm",
      kicker: "Where you stand",
      line: <React.Fragment>48,120 records from 10 sources — <em>all yours to read</em>.</React.Fragment>,
  ...
  const lately = RR2.traces.slice(0, 4).map((tr) => {
    if (tr.decision === "deny") {
      const why = tr.reason === "scope not granted" ? "you never allowed it"
        : tr.reason === "grant revoked" ? "you'd revoked it"
        : tr.reason;
      return { id: tr.id, when: relDay(tr.t), deny: true,
        text: <React.Fragment><b>{tr.client}</b> tried to read {tr.stream.replace(/_/g, " ")} — turned away, {why}.</React.Fragment> };
    }
    return { id: tr.id, when: relDay(tr.t), deny: false,
      text: <React.Fragment><b>{tr.client}</b> read {tr.records} {recordNoun(tr.stream)} — {tr.fields} fields each.</React.Fragment> };
  });
    ...
    <div className="rr-stand">
      ...
      <section className="rr-stand-block">
        <div className="rr-stand-block__head">
          ...
          <button className="rr-link" onClick={() => onGo("deployment")} type="button">owner tokens →</button>
        </div>
        <div className="rr-bearer">
          {BEARER.map((b) => (
            <div className="rr-bearer__row" key={b.who}>
              ...
              <span className="rr-bearer__how">{b.how} · {b.last}</span>
              <button className="rr-rel__revoke" onClick={() => onGo("deployment")} type="button">revoke</button>
            </div>
          ))}
          <p className="rr-bearer__note">An owner token reads everything — every source, every field, exactly what you see. Keep the list short; revoke anytime.</p>
        </div>
      </section>
      ...
      <div className="rr-stand-grid">
        ...
        <section className="rr-stand-block">
          <div className="rr-stand-block__head">
            ...
            <button className="rr-link" onClick={() => onGo("grants")} type="button">all grants →</button>
          </div>
          <div className="rr-rel-list">
            {active.map((g) => (
              <div className="rr-rel" key={g.id}>
                ...
                <button className="rr-rel__revoke" onClick={() => onOpenGrant(g.id)} type="button">revoke</button>
              </div>
            ))}
            {active.length === 0 && <p className="rr-stand-empty">No grant is out. Nothing is shared — only you and what you've given a token read this server.</p>}
          </div>
        </section>
        ...
        <section className="rr-stand-block">
          <div className="rr-stand-block__head">
            ...
            <button className="rr-link" onClick={() => onGo("traces")} type="button">every read →</button>
          </div>
      ...
      <section className="rr-stand-block">
        ...
        {hasFailure ? (
          ...
          <div className="rr-allclear">
            <span className="rr-allclear__text">Nothing needs you. Grants are within their limits, backups are on, and everything's syncing.</span>
          </div>
        )}
      </section>
    </div>
