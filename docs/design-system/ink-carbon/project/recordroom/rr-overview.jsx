...
(() => {
  ...
  function scopeHuman(name) {
    const scopeName = name.endsWith(".read") ? name.slice(0, -5) : name;
    return SCOPE_HUMAN[name] || scopeName.replaceAll("_", " ");
  }
  function deniedReason(reason) {
    if (reason === "scope not granted") {
  ...
  function Overview({ grants, requestState, onReview, onGo, onOpenGrant }) {
    ...
    const handleReconnect = useCallback(() => setResolved(true), []);
    const handleGo = useCallback((event) => onGo(event.currentTarget.dataset.view), [onGo]);
    const handleOpenGrant = useCallback((event) => onOpenGrant(event.currentTarget.dataset.grantId), [onOpenGrant]);
    ...
    if (pending) {
      ...
      hero = {
        ...
        cta: (
          <button className="pdpp-btn pdpp-btn--sm" onClick={handleReconnect} type="button">
            Reconnect the bank
          </button>
        ),
      };
    ...
    const lately = RR2.traces.slice(0, 4).map((tr) => {
      if (tr.decision === "deny") {
        const why = deniedReason(tr.reason);
        return {
          id: tr.id,
          when: relDay(tr.t),
          deny: true,
          text: (
            <>
              <b>{tr.client}</b> tried to read {tr.stream.replaceAll("_", " ")} — turned away, {why}.
            </>
          ),
        };
      }
    ...
    let attentionContent = null;
    if (hasFailure && window.RRAttentionList) {
      attentionContent = <window.RRAttentionList onGo={onGo} />;
    } else if (!hasFailure) {
      attentionContent = (
      ...
      <div className="rr-stand">
        ...
        <section className="rr-stand-block">
          <div className="rr-stand-block__head">
            ...
            <button className="rr-link" data-view="deployment" onClick={handleGo} type="button">
              owner tokens →
            </button>
          </div>
          <div className="rr-bearer">
            {BEARER.map((b) => (
              <div className="rr-bearer__row" key={b.who}>
                ...
                <button className="rr-rel__revoke" data-view="deployment" onClick={handleGo} type="button">
                  revoke
                </button>
              </div>
            ))}
        ...
        <div className="rr-stand-grid">
          ...
          <section className="rr-stand-block">
            <div className="rr-stand-block__head">
              ...
              <button className="rr-link" data-view="grants" onClick={handleGo} type="button">
                all grants →
              </button>
            </div>
            <div className="rr-rel-list">
              {active.map((g) => (
                <div className="rr-rel" key={g.id}>
                  ...
                  <button className="rr-rel__revoke" data-grant-id={g.id} onClick={handleOpenGrant} type="button">
                    revoke
                  </button>
                </div>
              ))}
          ...
          <section className="rr-stand-block">
            <div className="rr-stand-block__head">
              ...
              <button className="rr-link" data-view="traces" onClick={handleGo} type="button">
                every read →
              </button>
            </div>
