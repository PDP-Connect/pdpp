...
const RR_APP_FORM_CONTROL_TAG = /^(INPUT|TEXTAREA|SELECT)$/;
...
(() => {
  ...
  // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: this prototype shell intentionally keeps its order-coupled view state and rendering dispatch together.
  function App() {
    ...
    useEffect(() => {
      ...
      function onKey(e) {
        if (e.target && RR_APP_FORM_CONTROL_TAG.test(e.target.tagName)) {
          return;
        }
      ...
      <div className="rr-app" data-density={t.density}>
        ...
        {requestState === "open" && (
          <RRCeremony
            ...
            onToggle={
              // biome-ignore lint/performance/noJsxPropsBind: prototype callback captures this rendered fixture state.
              (i) => setReqScopes((cur) => cur.map((s, j) => (j === i ? { ...s, allowed: !s.allowed } : s)))
            }
            pressing={pressing}
