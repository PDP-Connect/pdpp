...
(() => {
    ...
    function load() {
        ...
        loadP = fetch(STATE_FILE)
            ...
            // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: sidecar hydration must atomically preserve racing drops and clears.
            .then((j) => {
            ...
            .catch(() => {
            // A missing sidecar is an empty store.
        })
            .then(() => {
            loaded = true;
    ...
    function save() {
        ...
        const w = window.omelette?.writeFile;
        if (!w) {
            return;
        }
        saving = true;
        Promise.resolve(w(STATE_FILE, JSON.stringify(slots)))
            .catch(() => {
            // A write failure remains retryable on the next user change.
        })
            .then(() => {
            saving = false;
            if (saveDirty) {
    ...
    // ── Custom element ──────────────────────────────────────────────────────
    const stylesheet = ":host{display:inline-block;position:relative;vertical-align:top;" +
    ...
    class ImageSlot extends HTMLElement {
        ...
        constructor() {
            ...
            this._spill.addEventListener("pointerdown", (e) => {
                ...
                const corner = e.target.getAttribute?.("data-c");
                let move;
                if (corner) {
        ...
        // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: one render transaction synchronizes the custom element's shadow DOM state.
        _render() {
            ...
            const editable = !!window.omelette?.writeFile;
            ...
            if (stored?.u && !DATA_IMAGE_URL.test(stored.u)) {
                stored = null;
            }
            const srcAttr = this.getAttribute("src") || "";
            this._userUrl = stored?.u || null;
