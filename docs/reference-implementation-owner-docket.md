...
| Event-subscription HTTPS callback delivery | `owner-live-gated` | `tmp/workstreams/ri-pdpp-events-callback-route-2026-05-31.md`; `tmp/workstreams/ri-event-callback-proof-report.md`; `pdpp-events.vivid.fish` routes to `192.168.1.180:8765`; receiver supports `--host 0.0.0.0`; public `/health` reached the receiver on 2026-05-31 after `192.168.1.4` restarted. | Mint or use a grant-scoped bearer and verify create -> active -> send_test_event -> pull changed data through the public callback URL. | Delivery receives CloudEvents 1.0 structured body and Standard Webhooks v1 signature at the callback; changed-data hints can be pulled with `changes_since`. |
