...
| `expand[]` / `expand_limit` footgun | `false-positive` | `tmp/workstreams/ri-guidance-ledger-no-human-recheck-2026-05-31.md`; MCP and reference-contract schemas point clients to `expand_capabilities`. | No implementation; keep the honest declared-capability mirror. |
| SQLite-bound naming under Postgres mode | `parked` | `tmp/workstreams/ri-guidance-ledger-no-human-recheck-2026-05-31.md`. | Do not run a broad rename without a scoped cleanup change; current hits are backend-specific factories/helpers, not user-facing Postgres drift. |
| Cursor direction documentation | `false-positive` | `tmp/workstreams/ri-guidance-ledger-no-human-recheck-2026-05-31.md`; reference-contract docs distinguish list `next_cursor` from `changes_since` / `next_changes_since`. | No implementation unless a future docs pass rewrites read-contract examples. |
| Inbound `schedule_run` endpoint naming | `false-positive` | `add-source-webhook-ingress`; `schedule_run` is payload action vocabulary on `POST /_ref/source-webhooks/:sourceId`, not a public route needing rename. | No rename; source-webhook change is complete-awaiting-archive. |
...
| Test fixtures leaking in production UI | `false-positive` | `tmp/workstreams/ri-guidance-ledger-no-human-recheck-2026-05-31.md`; public-listing honesty and sandbox/mock-owner split already prevent known fixtures from production listing. | No implementation unless a concrete leaking connector id is found. |
