...
| Current operational ledger | `operational` | `tmp/workstreams/ri-owner-current-state.md`; refreshed after deployed commit `071c64c8`. | Keep updated after merge, deploy, archive, park, or live-gate decisions. | A resumed owner can identify current branches, deployment revision, evidence, and stop condition without chat memory. |

## Acceptance-target gates

| Item | Status | Authority / evidence | Next action | Acceptance criterion |
|---|---|---|---|---|
| Hosted MCP package grant over real MCP JSON-RPC | `resolved` | Delegate gate report; RI owner live probe; the owner's 2026-05-31 external Claude runs in `tmp/workstreams/ri-external-claude-live-evidence-2026-05-31.md`; deployed fix `071c64c8`; external Claude proved readable records and event-subscription lifecycle after reconnect. | No implementation action unless a new external-client failure appears. Preserve the grant-id reconnect note: `connection_id` is the durable source selector; `grant_id` identifies the current authorization grant and can rotate on reauthorization. | External Claude reconnects through hosted MCP, inspects streams and schema from text, queries authorized record payloads from text, and creates/manages event subscriptions without URL-shaped connector ids. |
| Hosted MCP consent picker UX | `needs-verification` | the owner's 2026-05-31 live UI feedback; `071c64c8` deployed repairs in `reference-implementation/server/routes/as-consent-ui-helpers.ts` and `as-authorize.ts`. | Verify in Claude UI that sources start collapsed, source selection enables but does not auto-select streams, single-stream selection is convenient, bulk controls are clear, copy is owner-facing, and invalid selections re-render HTML validation. | The owner can intentionally select sources/streams without contradictory default state or raw JSON validation failures. |
| Multi-connection package disambiguation | `resolved` | the owner's second 2026-05-31 external Claude run reported full `ambiguous_connection` with `available_connections[]`, successful retry with `connection_id`, and four distinct `claude_code` connections. | No implementation action unless a new external-client failure appears. Preserve this as live acceptance evidence. | Multi-connection package grant forces `connection_id` disambiguation and succeeds after retry. |
| Event-subscription HTTPS callback delivery | `owner-live-gated` | `tmp/workstreams/ri-pdpp-events-callback-route-2026-05-31.md`; `tmp/workstreams/ri-event-callback-proof-report.md`; `pdpp-events.vivid.fish` routes to `192.168.1.180:8765`; receiver supports `--host 0.0.0.0`; public `/health` reached the receiver on 2026-05-31 after `192.168.1.4` restarted; external Claude proved create -> active -> send_test_event -> delete through the public callback; local e2e now proves owner-agent create/list/test/records.changed delivery with Standard Webhooks signature verification and client-delete revocation. | Only the public live signed proof remains: restart the receiver with a secret-file populated from the client-returned `whsec_...`, or use a receiver that validates Standard Webhooks and record the validation. | Delivery receives CloudEvents 1.0 structured body and Standard Webhooks v1 signature at the callback; changed-data hints can be pulled with `changes_since`. |
...
| Daisy trusted owner-agent onboarding | `complete-awaiting-archive` | `add-trusted-owner-agent-onboarding` is complete and validates strictly; local tests prove metadata discovery, browser-mediated approval, non-printing credential handoff, REST-only owner credential boundary, owner-agent event subscriptions, signed delivery, and revocation. | Archive the OpenSpec change after owner acceptance. Actual Daisy app live use remains an owner/live smoke, not open implementation scope. | Daisy can start from an entrypoint URL, learn the owner-agent path, obtain owner-approved local credentials without route guessing or bearer paste, and use REST efficiently for current and future data. |
...
| `design-fast-broad-agent-consent` | 18/37 | `active-openspec` | Re-run external Claude against the owner-visible ceremony, then continue only owner-approved consent-design tranches. |
| `add-trusted-owner-agent-onboarding` | 28/28 | `complete-awaiting-archive` | Archive after owner acceptance; remaining Daisy/live-public-callback checks are owner/live residuals, not implementation tasks. |
