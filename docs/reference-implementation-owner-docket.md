...
| Item | Status | Authority / evidence | Next action | Acceptance criterion |
|---|---|---|---|---|
| Hosted MCP package grant over real MCP JSON-RPC | `owner-live-gated` | Delegate gate report; RI owner live probe; the owner's 2026-05-31 external Claude run in `tmp/workstreams/ri-external-claude-live-evidence-2026-05-31.md`; deployed fix `91ccbe56`. | Reconnect external Claude after `91ccbe56`; authorize a real stream-bearing source; confirm `list_streams` and `schema` are readable as text, then query an actually authorized stream. Do not treat `messages`/`conversations` rejection as a defect on a YNAB-only grant. | External Claude reconnects through hosted MCP, inspects streams and schema from text, queries authorized records, and creates/manages event subscriptions without URL-shaped connector ids. |
| Hosted MCP consent picker UX | `needs-verification` | the owner's 2026-05-31 live UI feedback; `91ccbe56` deployed repairs in `reference-implementation/server/routes/as-consent-ui-helpers.ts` and `as-authorize.ts`. | Re-run the hosted consent screen in Claude after deploy and verify the owner sees collapsed sources, source-gated streams, bulk controls, friendlier copy, no registry URL clutter, and HTML validation for no-source submissions. | The owner can intentionally select sources/streams without contradictory default state or raw JSON validation failures. |
| Multi-connection package disambiguation | `owner-live-gated` | Live probe proved typed `ambiguous_connection` with `connection_id`; the owner's latest external Claude run was YNAB-only and could not exercise ambiguity. | Include at least two relevant connections or duplicated connector types in the next external Claude/ChatGPT acceptance run. | Multi-connection package grant forces `connection_id` disambiguation and succeeds after retry. |
| Event-subscription HTTPS callback delivery | `owner-live-gated` | `tmp/workstreams/ri-pdpp-events-callback-route-2026-05-31.md`; `pdpp-events.vivid.fish` routes to `192.168.1.180:8765`; receiver now supports `--host 0.0.0.0`. | Run the receiver, create a subscription with `https://pdpp-events.vivid.fish/webhook`, start/restart it with the returned `whsec_` secret if needed, then verify create -> active -> test delivery. | Delivery receives CloudEvents 1.0 structured body and Standard Webhooks v1 signature at the callback. |
...
| `design-fast-broad-agent-consent` | 18/37 | `active-openspec` | Re-run external Claude against the deployed picker repairs, then continue only owner-approved consent-design tranches. |
