...
| Current operational ledger | `operational` | `tmp/workstreams/ri-owner-current-state.md`; refreshed after deployed commit `91ccbe56`. | Keep updated after merge, deploy, archive, park, or live-gate decisions. | A resumed owner can identify current branches, deployment revision, evidence, and stop condition without chat memory. |

## Acceptance-target gates

| Item | Status | Authority / evidence | Next action | Acceptance criterion |
|---|---|---|---|---|
| Hosted MCP package grant over real MCP JSON-RPC | `owner-live-gated` | Delegate gate report; RI owner live probe; the owner's 2026-05-31 external Claude runs in `tmp/workstreams/ri-external-claude-live-evidence-2026-05-31.md`; deployed fix `91ccbe56`; current owner patch for `query_records` text previews. | Deploy the `query_records` text-preview repair, then reconnect external Claude and verify authorized record payloads are readable in MCP `content[]`, not only in `structuredContent.data`. | External Claude reconnects through hosted MCP, inspects streams and schema from text, queries authorized record payloads from text, and creates/manages event subscriptions without URL-shaped connector ids. |
| Hosted MCP consent picker UX | `needs-verification` | the owner's 2026-05-31 live UI feedback; `91ccbe56` deployed repairs; current follow-up in `reference-implementation/server/routes/as-consent-ui-helpers.ts` and `as-authorize.ts`. | Deploy the follow-up picker repair, then verify in Claude UI that sources start collapsed, source selection enables but does not auto-select streams, single-stream selection is convenient, bulk controls are clear, copy is owner-facing, and invalid selections re-render HTML validation. | The owner can intentionally select sources/streams without contradictory default state or raw JSON validation failures. |
...
| Event-subscription HTTPS callback delivery | `owner-live-gated` | `tmp/workstreams/ri-pdpp-events-callback-route-2026-05-31.md`; `tmp/workstreams/ri-event-callback-proof-report.md`; `pdpp-events.vivid.fish` routes to `192.168.1.180:8765`; receiver supports `--host 0.0.0.0`. | Bring `192.168.1.4` back online, confirm public `/health`, then mint a grant-scoped bearer and verify create -> active -> send_test_event -> pull changed data. | Delivery receives CloudEvents 1.0 structured body and Standard Webhooks v1 signature at the callback; changed-data hints can be pulled with `changes_since`. |
...
| `design-fast-broad-agent-consent` | 18/37 | `active-openspec` | Deploy the follow-up picker repair, re-run external Claude against the owner-visible ceremony, then continue only owner-approved consent-design tranches. |
