...
const {
  RRSidebarFull, RRGrantRow, RRInspector, RRStreamSheet, RRActivityLog, RRCeremony,
  RRTracesView, RROverviewView, RRExploreView, RRSourcesView, RRSourcesView2, RRRunsView,
  RRSchedulesView, RRConnectView, RRDeploymentView, RRExportersView,
  RRSubscriptionsView, RRCommandPalette,
  RRSyncsView, RRConnectView2, RRExportersView2, RRSubscriptionsView2, RRAttentionList,
  RROverview2,
  useTweaks, TweaksPanel, TweakSection, TweakSlider, TweakRadio, TweakButton,
} = window;

/* ─── Fixture data ─── */

const STREAMS = [
  { id: "pay_statements", connector: "Northstar HR", records: "312",
    fields: ["employer", "period_start", "period_end", "gross_pay", "net_pay", "taxes_withheld", "benefits_detail", "bank_routing"] },
  { id: "employment", connector: "Northstar HR", records: "4",
    fields: ["employer", "title", "start_date", "end_date", "manager_contact"] },
  { id: "transactions", connector: "First Meridian", records: "41,203",
    fields: ["date", "amount", "merchant", "category", "account_ref", "memo"] },
  { id: "listening_history", connector: "Tonal", records: "6,597",
    fields: ["track", "artist", "played_at", "device", "playlist_ref"] },
  { id: "tax_docs", connector: "Northstar HR", records: "12",
    fields: ["doc_type", "tax_year", "employer", "document_blob"] },
];

const BASE_GRANTS = [
  {
    id: "grt_lngvw_01", client: "Longview Planning",
    ...
    status: "active", issued: "2025-10-14 09:22Z",
    expiry: "exp 2026-12-14", expiresFull: "2026-12-14 09:22Z",
    projections: {
  ...
  {
    id: "grt_cncrt_02", client: "Concert Recommendations",
    purpose: "live_event_suggestions",
    scopes: [{ name: "listening_history.read", terms: "rolling 12 mo" }],
    declined: [],
    status: "continuous", issued: "2026-01-30 18:04Z",
    expiry: "continuous", expiresFull: "renews monthly · next 2026-07-01",
    projections: { listening_history: ["track", "artist", "played_at"] },
  },
  {
    id: "grt_taxpr_03", client: "TaxPrep Co",
    purpose: "annual_filing_2025",
    scopes: [{ name: "tax_docs.read", terms: "single use" }],
    declined: [],
    status: "expiring", hoursLeft: 26, issued: "2026-06-09 11:40Z",
    expiry: "exp 2026-06-12", expiresFull: "2026-06-12 11:40Z",
    projections: { tax_docs: ["doc_type", "tax_year", "employer"] },
  },
  {
    id: "grt_xwise_09", client: "Crosswise Ads",
    purpose: "ad_personalization",
    scopes: [{ name: "transactions.read", terms: "90 d window" }],
    declined: [],
    status: "revoked", issued: "2026-02-11 08:15Z",
    revokedOn: "2026-05-02", revokedFull: "2026-05-02 14:40Z · by owner",
    expiry: "—", expiresFull: "—",
...
const BASE_LOG = [
  { t: "2026-06-11 07:58Z", kind: "read", verb: "read", what: "pay_statements · 12 records · 5/8 fields", ref: "grt_lngvw_01" },
  { t: "2026-06-11 06:02Z", kind: "read", verb: "read", what: "listening_history · 214 records · 3/5 fields", ref: "grt_cncrt_02" },
  { t: "2026-06-10 22:17Z", kind: "read", verb: "read", what: "employment · 4 records · 3/5 fields", ref: "grt_lngvw_01" },
  { t: "2026-06-10 22:17Z", kind: "deny", verb: "deny", what: "tax_docs read attempt · scope not granted", ref: "grt_lngvw_01" },
  { t: "2026-06-09 11:40Z", kind: "consent", verb: "grant", what: "tax_docs.read · single use · TaxPrep Co", ref: "grt_taxpr_03" },
  { t: "2026-05-02 14:40Z", kind: "revoke", verb: "revoke", what: "transactions.read · Crosswise Ads · by owner", ref: "grt_xwise_09" },
  { t: "2026-05-02 14:39Z", kind: "deny", verb: "deny", what: "transactions read attempt · grant suspended", ref: "grt_xwise_09" },
];
...
const INCOMING = {
  ...
  scopes: [
    { name: "pay_statements.read", terms: "append only · 90 d", desc: "Employer, pay period, gross and net pay", allowed: true },
    { name: "employment.read", terms: "current + 5 yrs", desc: "Employers and dates — no salary history", allowed: true },
...
function App() {
  ...
  useEffect(() => {
    ...
    function onKey(e) {
      ...
      if (e.key === "Escape") { setRevoking(false); }
    }
    window.addEventListener("keydown", onKey);
    return () => window.removeEventListener("keydown", onKey);
  ...
  const grants = useMemo(() => {
    ...
    return all.map((g) =>
      revokedIds.includes(g.id) && g.status !== "revoked"
        ? { ...g, status: "revoked", revokedOn: g.revokedOn || "2026-06-11", revokedFull: g.revokedFull || nowStamp() + " · by owner" }
        : g
    );
  ...
  function approve() {
    ...
    const newGrant = {
      id: "grt_atlas_05", client: INCOMING.client,
      purpose: "mortgage_preapproval",
      scopes: allowed.map(({ name, terms }) => ({ name, terms })),
      declined,
      status: "active", issued: nowStamp(),
      expiry: "exp 2026-09-09", expiresFull: "2026-09-09 · 90 d term",
      justAdded: true,
      projections: {
        pay_statements: ["employer", "period_start", "period_end", "gross_pay", "net_pay"],
        employment: ["employer", "start_date", "end_date"],
    ...
    setTimeout(() => {
      ...
      addLog({ t: nowStamp(), kind: "consent", verb: "grant", what: `${allowed.length} scopes · ${declined.length} declined · ${INCOMING.client}`, ref: newGrant.id });
  ...
  function refuse() {
    ...
    addLog({ t: nowStamp(), kind: "deny", verb: "refuse", what: `access request refused · ${INCOMING.client}`, ref: INCOMING.id });
  }
  ...
  function confirmRevoke() {
    ...
    addLog({ t: nowStamp(), kind: "revoke", verb: "revoke", what: `${g.scopes.map((s) => s.name).join(" · ")} · ${g.client} · by owner`, ref: g.id });
  ...
  const paletteItems = [
    ...
    ...grants.map((g) => ({ label: g.client + " — " + g.id, kind: "grant", run: () => { setView("grants"); setSelected(g.id); } })),
    ...STREAMS.map((s) => ({ label: s.id, kind: "stream", run: () => setView("sources") })),
    { label: "Reauthorize First Meridian", kind: "action", run: () => setView("syncs") },
    ...(requestState === "pending" ? [{ label: "Review Atlas Mortgage request", kind: "action", run: () => setRequestState("open") }] : []),
    { label: "Toggle theme", kind: "action", run: () => setTweak("theme", t.theme === "dark" ? "light" : "dark") },
  ];

  const heads = { ...HEADS, grants: ["Grants", `${activeCount} in effect · ${grants.length - activeCount} struck · ↑↓ select`] };
    ...
    <div className="rr-app" data-density={t.density}>
      ...
      <main className="rr-main">
        <header className="rr-head">
          <span className="rr-head__brand"><span className="rr-side__mark"></span><span>Recordroom</span></span>
          ...
          <div className="rr-head__actions">
            ...
            <button className="rr-chrome-btn" onClick={() => setTweak("theme", t.theme === "dark" ? "light" : "dark")} title={t.theme === "dark" ? "Switch to light" : "Switch to dark"} type="button">{t.theme === "dark" ? "Dark" : "Light"}</button>
            <button className="rr-chrome-btn rr-menu-btn" onClick={() => setNavOpen(true)} type="button">Menu</button>
          </div>
        </header>
        <div className="rr-content" data-screen-label={view} key={view}>
          ...
          {view === "overview" && (
            <RROverview2
              grants={grants}
              onGo={setView}
              onOpenGrant={(id) => { setView("grants"); setSelected(id); }}
              onReview={() => setRequestState("open")}
              requestState={requestState}
            />
          )}
          {view === "explore" && <RRExploreView grants={grants} onGo={setView} onJump={() => setPaletteOpen(true)} seed={exploreSeed} />}
          ...
          {view === "grants" && (
            <div>
                ...
                <div className="rr-incoming pdpp-carbon">
                  <div className="rr-incoming__sheet">
                    <span className="rr-incoming__text">
                      ...
                      <span className="rr-incoming__meta">staged · req_atlas_7f2k · purpose: mortgage_preapproval</span>
                    </span>
                    <button className="pdpp-btn pdpp-btn--sm" onClick={() => setRequestState("open")} type="button">Review</button>
                  </div>
                </div>
              )}
              <div className="pdpp-table rr-cols-grants">
                ...
                {grants.map((g) => (
                  <RRGrantRow grant={g} key={g.id} onSelect={(id) => { setSelected(id); setRevoking(false); setStriking(false); }} selected={selected === g.id} />
                ))}
              </div>
            </div>
          )}
      ...
      {requestState === "open" && (
        <RRCeremony
          onApprove={approve}
          onDismiss={() => { if (!pressing) setRequestState("pending"); }}
          onRefuse={refuse}
      ...
      <RRCommandPalette items={paletteItems} onClose={() => setPaletteOpen(false)} onExec={recordRecent} open={paletteOpen} recents={recents} />

      <TweaksPanel>
        <TweakSection label="Console" />
        <TweakRadio label="Theme" onChange={(v) => setTweak("theme", v)} options={["dark", "light"]} value={t.theme} />
        <TweakRadio label="Density" onChange={(v) => setTweak("density", v)} options={["comfortable", "compact"]} value={t.density} />
        <TweakSection label="Carbon" />
        <TweakSlider label="Offset" max={14} min={5} onChange={(v) => setTweak("carbonOffset", v)} unit="px" value={t.carbonOffset} />
        <TweakSection label="Demo" />
        <TweakButton label="Reset demo state" onClick={() => { localStorage.setItem(LS_KEY, "{}"); location.reload(); }} />
      </TweaksPanel>
    </div>
  );
}
