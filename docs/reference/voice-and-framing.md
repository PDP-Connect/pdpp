...
| "Our hosted PDPP service…" | We don't operate a hosted service for end users. The reference is forkable and self-hostable. | "A public reference deployment (for example `pdpp.example.com`) runs the open-source reference image for inspection. Operators self-host their own instances." |
...
- **Maturity is a state, not a marketing label.** Use the vocabulary: `proven_working`, `needs_human_auth`, `local_only`, `unproven`, `test_stub`, `broken_in_current_deployment`.
...
PDPP is a Linux Foundation Decentralized Trust (LFDT) Labs project, informed by Vana's work on personal data, but the protocol stands on its own. Default to protocol-first framing.
...
- In positioning docs (e.g. DTI engagement, fundraising context): explain the alignment, but avoid cryptographic-trustlessness framing — DTI has explicitly rejected that pitch.
...
- **Aspirational copy on the site** — if a section describes something the reference does not yet do, label it (`Roadmap`, `Aspirational`, `Not yet implemented`). An April 2026 audit of `/reference` flagged exactly this drift; do not repeat it.
...
- Use `connector_id`, `connection`, `connector_instance_id`, `device`, `run`, `schedule`, `coverage`, `grant` as specified in the specification. These are load-bearing.
...
- [`CONTRIBUTING.md`](../../CONTRIBUTING.md) — repo-wide contribution rules and OpenSpec usage.
...
- [`docs/reference/concept-inventory.md`](concept-inventory.md) — canonical concept names.
