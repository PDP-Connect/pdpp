...
- The current normal MCP tool list is exactly `schema`, `query_records`, `aggregate`, `search`, `fetch`, `fetch_blob`, and `read_record_field`. Event-subscription management tools are intentionally absent from normal MCP setup.
