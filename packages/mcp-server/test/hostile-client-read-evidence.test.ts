...
interface LadderRecord {
  ...
  field_windows?: FieldWindow[];
}

interface SearchStructuredContent {
  content_ladder?: { records?: LadderRecord[] };
  resource?: unknown;
  results?: Array<{ evidence_excerpts?: EvidenceExcerpt[]; id?: string; url?: string }>;
}

function searchStructuredContent(result: {
  [key: string]: unknown;
  structuredContent?: unknown;
}): SearchStructuredContent {
  return (result.structuredContent ?? {}) as SearchStructuredContent;
}
...
function hostileSearchFixture() {
  ...
  const fetch = async (urlInput: string | Request | URL, init: RequestInit = {}) => {
    ...
    if ((init.headers as Record<string, string> | undefined)?.Authorization !== "Bearer scoped-token") {
...
test("metadata-only search hit does not invent a body match", async () => {
  ...
  const ladderRecord = structuredContent.content_ladder?.records?.[0];
  assert.ok(ladderRecord, "content_ladder must include at least one record");
  assert.equal(ladderRecord.evidence_excerpts, undefined);
  assert.equal(
    ladderRecord.field_windows?.some((field) => field.preview_text),
    false
  );
});
...
test("read_record_field is an inline small-text fallback, not a resource-only path", async () => {
  ...
  assert.equal(searchStructuredContent(result).resource, undefined);
...
test("search-visible record_uri is normalized to a self-contained callable tool id", async () => {
  ...
  assert.equal(searchStructured.results?.[0]?.id, "cin_slack/messages:m1");
  assert.equal(
...
function largeDataRecordFixture() {
  ...
  const record = {
    ...
    data: {
      id: "o9",
      text: bigText,
      attachment: { object: "blob_ref", blob_id: "blob_9", mime_type: "image/png", size_bytes: 81_234 },
...
test("large/binary/JSON fields escalate deliberately: bounded previews, no accidental dumps", async () => {
  ...
  const structuredContent = result.structuredContent as
    | { content_ladder?: { binary_fields?: LadderField[]; field_windows?: LadderField[]; json_fields?: LadderField[] } }
    | undefined;
