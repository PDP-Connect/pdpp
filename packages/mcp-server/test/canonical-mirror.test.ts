...
test("5.1 query_records exposes canonical read args (fields, expand, expand_limit, filter, sort, cursor, changes_since, limit, count)", async () => {
  ...
  assert.ok(queryRecords, "query_records tool must be registered");
  for (const name of expected) {
    assert.ok(queryRecords.inputSchema.properties?.[name], `query_records must expose canonical arg "${name}"`);
  }
...
test("5.1 descriptions prefer connection_id and connector_key source identity", async () => {
  ...
  const queryRecordsTool = byName.get("query_records");
  assert.ok(queryRecordsTool, "query_records tool must be registered");
...
test("5.2 successful tool calls carry canonical structuredContent that matches the outputSchema", async () => {
  ...
  const structuredContent = result.structuredContent as { data?: unknown; provider_url?: unknown };
  assert.ok(structuredContent.data);
  assert.equal(typeof structuredContent.provider_url, "string");
...
test("grant-scoped query_records does not add owner-only diagnostics", async () => {
  ...
  assert.equal(result.isError, undefined);
  const structuredContent = result.structuredContent as { data: { data: Array<{ id?: string }> } };
  assert.equal(structuredContent.data.data[0]?.id, "o1");
...
test("5.1 search input schema caps `limit` at 100, matching the published search contract", async () => {
  ...
  const limitSchema = schemaProperty(searchTool.inputSchema.properties, "limit");
  assert.equal(
    limitSchema.maximum,
    100,
...
test("7.4 token-efficiency: prose content[] is far smaller than structuredContent JSON", async () => {
  ...
  const proseLen = firstText(result).length;
