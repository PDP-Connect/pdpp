...
interface CompactViewModule {
  ...
  schemaSourceOptions: (data: unknown, opts?: unknown) => unknown[];
}
...
)) as CompactViewModule;

function mcpCompactSchemaExpected(schema: unknown, opts?: unknown): unknown {
  return stripDeprecatedConnectorInstanceAlias(projectSchemaCompactView(schema, opts));
}

function stripDeprecatedConnectorInstanceAlias(value: unknown): unknown {
  if (Array.isArray(value)) {
    return value.map(stripDeprecatedConnectorInstanceAlias);
  }
...
interface SchemaStream {
  ...
  field_capabilities?: Record<string, { schema?: { description?: string } } & string>;
  granted_connections?: unknown;
  name?: string;
  streams?: SchemaStream[];
}
...
interface SchemaDocument {
  ...
  data?: unknown;
  detail?: string;
  field_capability_legend?: Record<string, string>;
  stream_count?: number;
  streams?: unknown;
}

interface SchemaStructuredContent {
  data: SchemaDocument;
  [key: string]: unknown;
}

interface SchemaErrorStructuredContent {
  error?: {
...
function schemaStructuredContent(result: {
  [key: string]: unknown;
  structuredContent?: unknown;
}): SchemaStructuredContent {
  return result.structuredContent as SchemaStructuredContent;
}

function schemaErrorContent(result: {
  [key: string]: unknown;
  structuredContent?: unknown;
}): SchemaErrorStructuredContent {
  return result.structuredContent as SchemaErrorStructuredContent;
}
...
function schemaProperty(properties: { [x: string]: unknown } | undefined, name: string): Record<string, unknown> {
...
const STREAM_SCHEMA_STRUCTURED_BYTE_BUDGET = 6000; // one stream, compact
...
function schemaBodyForQuery(schemaBody: { data: unknown }, url: URL) {
...
function canonicalFullSchemaErrorForQuery(schemaBody: { data: unknown }, url: URL) {
...
test("default schema response stays under the documented package byte budget", async () => {
  ...
  const contentBytes = Buffer.byteLength(firstText(result), "utf8");
  ...
  assert.ok(contentBytes < 8000, `schema content[] summary must stay bounded (got ${contentBytes})`);
...
test("default schema is an index: drops field capability detail but keeps connection identity", async () => {
  ...
  assert.deepEqual(structuredContent.data.field_capability_legend, {
    t: "declared type",
  ...
  assert.equal(structuredContent.data.detail, "compact");
...
test("MCP compact schema drops duplicate top-level streams when connectors are present", async () => {
  ...
  const { client, server } = await connectClient(async (urlInput: string | Request | URL) => {
    const url = new URL(urlInput.toString());
    if (url.pathname === "/v1/schema") {
      return new Response(JSON.stringify(schemaBodyWithStreams), {
        status: 200,
        headers: { "content-type": "application/json" },
      });
    }
  ...
  assert.equal(
    ...
    undefined,
    "schema MCP data must be the schema document, not a nested REST data envelope"
  );
  assert.equal(
  ...
  assert.ok(Array.isArray(structuredContent.data.connectors[0]?.streams));
...
test("default schema requests the REST compact view when the RS supports it", async () => {
  ...
  const { field_capability_legend: _legend, ...actualWithoutLegend } = structuredContent.data;
  assert.deepEqual(
    actualWithoutLegend,
...
test("legacy full-schema fallback still renders the global MCP index projection", async () => {
  ...
  const expected = mcpCompactSchemaExpected(schemaBody.data) as SchemaDocument;
  const structuredContent = schemaStructuredContent(result);
  assert.equal(structuredContent.data.connectors.length, expected.connectors.length);
  assert.equal(structuredContent.data.connectors[0]?.streams[0]?.field_capabilities, undefined);
  assert.equal(structuredContent.data.detail, "compact");
...
test("schema tool description teaches the compact discovery path", async () => {
  ...
  assert.deepEqual(
    schemaProperty(schemaTool.inputSchema.properties, "detail").enum,
    ["compact", "full"],
...
test("query_records input schema caps `limit` at 100 and rejects an over-max value", async () => {
  ...
  assert.ok(queryTool, "query_records tool must be exposed");
  assert.equal(
    schemaProperty(queryTool.inputSchema.properties, "limit").maximum,
    100,
  ...
  const overMaxContent = (overMax.content ?? []) as Array<{ text?: string }>;
  ...
  const limitSchema = schemaProperty(queryTool.inputSchema.properties, "limit");
  assert.equal(limitSchema.maximum, 100);
  assert.ok(limitSchema.exclusiveMaximum === undefined, "the maximum must be inclusive so limit=100 is accepted");
...
test("resolveSchemaDetail defaults to compact, passes valid grades, and rejects unknown values", async () => {
  const { __internal } = await import("../src/tools.ts");
