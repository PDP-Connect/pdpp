...
const FETCH_TEXT_FALLBACK_BYTE_BUDGET = 1200;
...
function requestedFields(url: URL): Set<string> {
  return new Set(url.searchParams.getAll("fields").filter((field) => field.length > 0));
}

function projectPayload(payload: unknown, fields: Set<string>): unknown {
  if (fields.size === 0 || !payload || typeof payload !== "object" || Array.isArray(payload)) {
    return payload;
  }
...
function projectRecordBody(body: Record<string, unknown>, url: URL): unknown {
  const fields = requestedFields(url);
  if (fields.size === 0) {
    return body;
  }
  if (Array.isArray(body.data)) {
    return { ...body, data: body.data.map((record: unknown) => projectPayload(record, fields)) };
  }
...
test("query_records(fields) returns a strict projected envelope with only operational handles", async () => {
  ...
  const proseBytes = byteLength(firstText(result));
  ...
  assert.equal(firstRecord?.ts, undefined);
  assert.equal(firstRecord?.metadata, undefined);
...
test("search prose stays bounded and result hits are not duplicated in structuredContent.data", async () => {
  ...
  assert.equal(structuredContent.data.result_count, 100);
  assert.equal(structuredContent.data.results, undefined);
  assert.equal(structuredContent.data.hits, undefined);
  assert.equal(structuredContent.data.data, undefined);
  // Disambiguation data the agent needs survives once, in the flattened page.
  assert.equal(structuredContent.results.length, 100);
  for (const hit of structuredContent.results) {
...
test("search hit title does not fall back to the snippet", async () => {
  ...
  const [hit] = structuredContent.results;
...
test("fetch returns a declared text field verbatim (no truncation of real document text)", async () => {
  ...
  const structuredContent = result.structuredContent as {
    [key: string]: unknown;
    content_ladder?: { id?: string; record_uri?: unknown };
    data?: unknown;
    id?: string;
    text: string;
    title?: string;
  };
  assert.equal(structuredContent.text.length, 50_000);
  assert.equal(structuredContent.id, "mail:withtext");
  assert.equal(structuredContent.title, "A real message");
  assert.equal(structuredContent.data, undefined);
  const mirrored = JSON.parse(firstText(result));
  const { content_ladder: contentLadder, ...structuredDocument } = structuredContent;
...
test("fetch(fields) projects before rendering the document and preserves source handles", async () => {
  ...
  const structuredContent = result.structuredContent as {
    data?: unknown;
    metadata?: { connection_id?: string; stream?: string };
    text: string;
  };
  assert.equal(structuredContent.data, undefined);
  assert.equal(structuredContent.metadata?.stream, "mail");
...
test("fetch text JSON-stringify fallback is bounded and points at structured reads", async () => {
  ...
  const structuredContent = result.structuredContent as {
    data?: unknown;
    id?: string;
    metadata?: { line_items?: unknown };
    text: string;
  };
  const textBytes = byteLength(structuredContent.text);
