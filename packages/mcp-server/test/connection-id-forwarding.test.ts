...
function findCall(calls: FetchCall[], predicate: (url: URL) => boolean): URL {
...
test("fetch surfaces ambiguous_connection envelope and accepts retry with connection_id", async () => {
  ...
  const ambiguousError = ambiguous.structuredContent as {
    error?: { available_connections?: unknown[]; code?: string; retry_with?: { field?: string } };
  };
  assert.equal(ambiguous.isError, true);
  assert.equal(ambiguousError.error?.code, "ambiguous_connection");
  assert.equal(ambiguousError.error?.available_connections?.length, 2, "envelope must list candidate connections");
  assert.equal(
    ambiguousError.error?.retry_with?.field,
    "connection_id",
...
test("every normal read tool input schema declares optional connection_id and no deprecated alias", async () => {
  ...
  for (const toolName of expected) {
    ...
    assert.ok(tool.inputSchema.properties?.connection_id, `${toolName} input must declare connection_id`);
    assert.equal(
    ...
    assert.ok(!required.includes("connection_id"), `${toolName} connection_id must remain optional`);
  }
