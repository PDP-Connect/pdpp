...
async function postHostedMcpJson(message: unknown) {
  const { fetch } = makeFakeRs();
...
interface JsonRpcToolsListResult {
  ...
  result?: {
    tools: Array<{
      ...
      description?: string;
      inputSchema?: { properties?: { [x: string]: unknown } };
      name: string;
    }>;
  };
}

async function readJsonRpc(response: Response): Promise<JsonRpcToolsListResult> {
...
test("filter is object-shaped for query_records, aggregate, and search", async () => {
  ...
  for (const toolName of ["query_records", "aggregate", "search"]) {
    ...
    const filterSchema = tool.inputSchema.properties?.filter as { type?: string } | undefined;
...
test("long connection_id recovery paragraph is not repeated across tools", async () => {
  ...
  const toolsWithLongPhrase = result.tools.filter((t) => t.description?.includes(longPhraseFragment));
  assert.equal(
