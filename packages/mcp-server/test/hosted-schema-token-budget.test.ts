...
function makeLargeSchemaFetch({ connectorCount = 4, streamsPerConnector = 6, fieldsPerStream = 30 } = {}) {
  ...
  const fetch = async (urlInput: string | Request | URL, init: RequestInit = {}) => {
    ...
    if (auth !== "Bearer scoped-token") {
      return new Response(JSON.stringify({ error: { type: "authentication", code: "invalid_token" } }), {
        status: 401,
...
async function postMcpJson(message: unknown, fakeFetch: typeof fetch) {
...
interface HostedSchemaToolResult {
...
interface JsonRpcToolCallResponse {
  error?: unknown;
  result?: HostedSchemaToolResult;
}

async function readJsonRpc(response: Response): Promise<JsonRpcToolCallResponse> {
  ...
  if (contentType.includes("text/event-stream")) {
    ...
    assert.ok(lastLine, "SSE response must carry at least one data frame");
...
async function callSchemaTool(args: unknown, fakeFetch: typeof fetch): Promise<HostedSchemaToolResult> {
  ...
  assert.ok(rpc.result, "tools/call must return a result");
  return rpc.result;
}
...
test("hosted Streamable HTTP schema tool/call serves the compact default under the package byte budget", async () => {
  ...
  assert.ok(stream, "compact schema must include at least one stream");
  assert.equal(
...
test("hosted Streamable HTTP per-stream schema scopes the document over the wire", async () => {
  ...
  assert.equal(
    typeof stream.field_capabilities?.field_0,
    "string",
