...
import type { RsClient } from "../src/rs-client.ts";
...
// assertExpandCapabilities only calls rs.getJson; a fake exercising just that
// method is cast to RsClient (the production dependency) rather than widening
...
test("assertExpandCapabilities: throws UnadvertisedExpandError when stream has no expand_capabilities", async () => {
  const rs = makeSchemaRs([{ name: "statements" /* no expand_capabilities field */ }]);
...
test("assertExpandCapabilities: throws UnadvertisedExpandError when expand_capabilities is an empty array", async () => {
  ...
  await assert.rejects(
    ...
    (err: unknown) => {
      assert.ok(err instanceof UnadvertisedExpandError);
...
interface IntegrationRsOptions {
  hasExpandCaps: boolean;
  recordsBody?: unknown;
  streamName: string;
}
...
function makeIntegrationRs({ streamName, hasExpandCaps, recordsBody }: IntegrationRsOptions) {
  ...
  const fetch = async (urlInput: string | Request | URL) => {
    ...
    if (url.pathname === "/v1/schema") {
      const expandCaps = hasExpandCaps ? [{ name: "account", cardinality: "has_one", usable: true }] : undefined;
      const stream: { expand_capabilities?: unknown[]; name: string } = { name: streamName };
      if (expandCaps !== undefined) {
        stream.expand_capabilities = expandCaps;
      }
      return new Response(JSON.stringify({ object: "schema", streams: [stream] }), {
        status: 200,
        headers: { "content-type": "application/json" },
      });
    }
    if (url.pathname.startsWith(`/v1/streams/${encodeURIComponent(streamName)}/records`)) {
      return new Response(JSON.stringify(recordsBody ?? { records: [], has_more: false }), {
        status: 200,
        headers: { "content-type": "application/json" },
      });
    }
...
async function connectClient(rsOptions: ReturnType<typeof makeIntegrationRs>) {
