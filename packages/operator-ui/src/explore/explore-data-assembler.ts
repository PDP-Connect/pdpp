...
function timelineRecordToEntry(
  ...
  // The server's semantic_time is authoritative. When an older server omits it,
  // the local manifest-derived timestamp determines whether the fallback is
  // semantic or merely the ingestion timestamp.
  const localDisplay = pickSearchDisplayTimestamp({
    data,
    emittedAt: rec.emitted_at,
    metadata: lookupSearchTimestampMetadata(timestampMetadata, rec.connector_id, rec.stream),
  });
  const serverSemanticTime =
    typeof rec.semantic_time === "string" && rec.semantic_time.length > 0 ? rec.semantic_time : null;
  const displayValue = serverSemanticTime ?? localDisplay.value;
