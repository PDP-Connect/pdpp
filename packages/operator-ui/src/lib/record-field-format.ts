 ...
 *
 * `record-preview.ts` consumes the same minor-units detection here so the
 * Explorer feed and the console record surfaces agree on how a declared
 * currency field is formatted.
 */
import type { DeclaredFieldTypes } from "./record-kind.ts";
