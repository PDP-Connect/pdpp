...
export interface ExplorerFeedEntry {
  ...
  displayAt: string;
  /**
   * True when `displayAt` is a declared semantic/authored timestamp (e.g. a
   * message's `received_at`); false when the stream declares no semantic time
   * field (e.g. Gmail labels, which have no API-provided creation time) and
   * `displayAt` fell back to ingestion time. Drives the "ingested" qualifier
   * on the feed row so ingestion time is never mistaken for the record's own
   * event/creation time.
   */
  displayIsSemantic: boolean;
  emittedAt: string;
  /**
