...
function tableColumns(db: SqliteDb, name: string): string[] {
  return db
...
// biome-ignore lint/complexity/noExcessiveCognitiveComplexity: merge workflow has essential ordering (validate, plan, dry-run vs commit, summarize, optional delete) — splitting fragments the transactional semantics
function main(): void {
  ...
  const plan: Array<{ sql: string; table: string }> = [];
  for (const t of srcTables) {
    ...
    const srcCols = new Set(
      db
        .prepare(`PRAGMA src.table_info(${t})`)
        .all()
        .map((c) => String(c.name))
    );
  ...
  try {
    ...
    console.error("merge failed, rolled back:", m);
    process.exit(1);
  }
  ...
  if (tableExists(db, "records")) {
    ...
    for (const r of rows) {
      ...
      const n = Number(r.n);
      console.error(`  ${cid.padEnd(52)} ${stream.padEnd(20)} ${n}`);
    }
  }

  db.close();

  if (args.deleteSource && args.source) {
    ...
    for (const ext of ["-journal", "-wal", "-shm"]) {
      ...
      if (existsSync(sibling)) {
        unlinkSync(sibling);
        console.error(`  also deleted ${sibling}`);
      }
    }
  }
}
