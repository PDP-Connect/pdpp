...
interface ProbeSnap {
  ...
  error?: string;
  keyMatchClasses?: Record<string, ClassSample>;
  orderCardCls?: string;
...
const snap: ProbeSnap = await page.evaluate((): ProbeSnap => {
  ...
  const byClass: Record<string, ClassSample> = {};
  for (const e of elements) {
    ...
    const key = e.cls;
    if (!byClass[key]) {
      byClass[key] = { tag: e.tag, samples: [] };
    }
    if (byClass[key].samples.length < 3) {
      byClass[key].samples.push(e.text);
    }
  }
  ...
  const keyMatches = Object.keys(byClass).filter((k) =>
    ...
    /header|summary|info|total|date|status|delivery|ship|address|recipient|item|title|body|left|right|grid|col|column|label|value/i.test(
      k
    )
  );
  ...
  return {
    ...
    outerHtmlSnippet: card.outerHTML
      // biome-ignore lint/correctness/noEmptyCharacterClassInRegex: [^] matches any char including newline; replacement pattern relies on it
