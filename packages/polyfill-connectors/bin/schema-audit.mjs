...
const DB_PATH =
  ...
  join(homedir(), 'code/pdpp/packages/polyfill-connectors/.pdpp-data/polyfill.sqlite');

const CONNECTORS = {
  'https://registry.pdpp.org/connectors/amazon': await import('../connectors/amazon/schemas.js'),
  'https://registry.pdpp.org/connectors/chase': await import('../connectors/chase/schemas.js'),
  'https://registry.pdpp.org/connectors/chatgpt': await import('../connectors/chatgpt/schemas.js'),
  'https://registry.pdpp.org/connectors/usaa': await import('../connectors/usaa/schemas.js'),
};

function fetchRecords(connectorId, stream) {
...
for (const [connectorId, schemasMod] of Object.entries(CONNECTORS)) {
  ...
  const name = connectorId.split('/').pop();
  summary[name] = {};
  ...
  for (const stream of Object.keys(SCHEMAS)) {
    ...
    let fail = 0;
    const failSamples = [];
    const issueCountByPath = {};
    for (const row of rows) {
      let data;
      try { data = JSON.parse(row.json); } catch {
        fail++;
        if (failSamples.length < 3) failSamples.push({ key: row.key, reason: 'invalid JSON' });
        continue;
      }
...
for (const [conn, streams] of Object.entries(summary)) {
  ...
  for (const [stream, result] of Object.entries(streams)) {
    ...
    const status = total === 0 ? '—' : result.fail === 0 ? '✅' : result.pass === 0 ? '❌' : '⚠ ';
