...
export function buildConnectorSpec(
	...
	if (streams.length === 0) {
		throw new Error(
			`run requires --streams <a,b,c> for connector ${options.connector}`,
		);
	}
	return {
...
export function parseArgs(args: string[]): CliOptions {
	...
	if (command !== "enroll" && command !== "run" && command !== "advertise") {
		throw new Error(
			"usage: collector-runner <enroll|run|advertise> --base-url <url> [options]",
		);
	}
...
export function scopedDefaultQueuePath(
	...
	return join(
		dirname(defaultQueuePath),
		`${stem}.${safeQueuePathSegment(connectionId)}${extension}`,
	);
}
