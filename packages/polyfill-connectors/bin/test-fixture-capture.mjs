...
if (!capture) throw new Error('capture session not created (PDPP_CAPTURE_FIXTURES not respected?)');

// Simulate RECORDs with PII
capture.recordRecord({
  type: 'RECORD',
  stream: 'orders',
  data: { id: 'abc', buyer_email: 'alice@example.com', phone: '(555) 123-4567' },
});
capture.recordRecord({
  type: 'RECORD',
  stream: 'orders',
...
const stubPage = {
  async content() {
    return '<html><body>Email: alice@example.com, phone (555) 123-4567</body></html>';
  },
};
...
if (!rawRecords.includes('alice@example.com')) throw new Error('raw records should contain PII');

const rawDom = await readFile(join(capture.baseDir, 'dom', 'orders-list.html'), 'utf8');
if (!rawDom.includes('alice@example.com')) throw new Error('raw dom should contain PII');
...
if (emailsRemainingInRecords) throw new Error(`scrubbed records still contain original email: ${emailsRemainingInRecords[0]}`);
if (!scrubbedRecords.includes('redacted@example.com')) throw new Error('scrubbed records missing redaction marker');

if (scrubbedDom.includes('alice@example.com')) throw new Error('scrubbed dom still contains original email');

const ssnMatch = scrubbedRecords.match(/123-45-6789/);
if (ssnMatch) throw new Error('scrubbed records still contain SSN');
if (!scrubbedRecords.includes('000-00-0000')) throw new Error('scrubbed records missing SSN redaction marker');
