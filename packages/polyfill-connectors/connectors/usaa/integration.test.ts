...
test("emitExportFailure: a date-field fill failure is structural, not export pressure", async () => {
  const { deps, messages } = makeHarness();
  const diag: DiagnosticInfo = {
    diag: null,
    error: "end field detached before fill",
  ...
  assert.match(skip.message, /source_structure_changed/);
  const emittedDiag = skip.diagnostics as DiagnosticInfo & { outcome: string };
  assert.equal(emittedDiag.outcome, "source_structure_changed");
});
