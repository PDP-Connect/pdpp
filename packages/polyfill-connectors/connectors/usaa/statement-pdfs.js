...
function safeAccountSlug(accountId, fallback) {
  if (accountId && /^[A-Za-z0-9_-]+$/.test(accountId)) return accountId;
  if (accountId) return createHash('sha256').update(accountId).digest('hex').slice(0, 16);
...
async function locateDownloadMenuItem(page) {
  const candidates = [
    ...
    page.locator('[role="menu"] a, [role="menu"] button', { hasText: /download/i }).first(),
    page.locator('a, button').filter({ hasText: /^\s*Download( PDF)?\s*$/i }).first(),
  ];
  for (const c of candidates) {
    if (await c.count().catch(() => 0)) return c;
  }
...
async function downloadStatementFromRow({ page, rowIndex, downloadQueue }) {
  ...
  if (!optBtn) {
    ...
    if (await link.count().catch(() => 0)) {
      ...
      try {
        ...
        if (!path) return { ok: false, reason: 'download_no_path' };
        const { readFile } = await import('node:fs/promises');
        const buffer = await readFile(path);
        return { ok: true, buffer, suggestedFilename: dl.suggestedFilename() };
      } catch (err) {
        return { ok: false, reason: 'direct_link_failed', diag: { error: err.message?.slice(0, 160) } };
      }
    }
  ...
  try {
    ...
    return { ok: false, reason: 'options_click_failed', diag: { error: err.message?.slice(0, 160) } };
  }
  await sleep(500);
  ...
  if (!dlItem) {
    ...
    const menuHtml = await page.locator('[role="menu"]').first().innerHTML().catch(() => null);
    // Dismiss whatever popped open so we don't cascade-break the next row.
    await page.keyboard.press('Escape').catch(() => {});
    return {
      ok: false,
      reason: 'no_download_menuitem',
      diag: { menu_html: menuHtml ? menuHtml.replace(/\s+/g, ' ').slice(0, 500) : null },
    };
  }
  ...
  try {
    ...
    await page.keyboard.press('Escape').catch(() => {});
    return { ok: false, reason: 'download_click_failed', diag: { error: err.message?.slice(0, 160) } };
  }
  ...
  try {
    ...
    await page.keyboard.press('Escape').catch(() => {});
    return { ok: false, reason: 'download_timeout', diag: { error: err.message?.slice(0, 160) } };
  }
}
...
export async function hydrateStatementPdfs({
  ...
  if (!/\/my\/documents/.test(page.url())) {
    await page.goto('https://www.usaa.com/my/documents', { waitUntil: 'domcontentloaded', timeout: 30000 }).catch(() => {});
    await sleep(5000);
  }
  ...
  for (const s of statements) {
    ...
    try {
      ...
      if (onSkip) onSkip({ statement: s, reason: 'persist_failed', diag: { error: err.message?.slice(0, 160) } });
    }
 ...
 */
function detectStatementClosing(text) {
  // Credit-card: "Statement Closing Date 02/17/26"
  const cc = text.match(/Statement\s+Closing\s+Date\s+(\d{1,2})\/\d{1,2}\/(\d{2,4})/i);
  if (cc) {
  ...
  const ck = text.match(/Statement\s+Period\s+\d{1,2}\/\d{1,2}\/\d{4}\s*[-–]\s*(\d{1,2})\/\d{1,2}\/(\d{4})/i);
  if (ck) return { closingMonth: Number(ck[1]), closingYear: Number(ck[2]) };
  return null;
}
 ...
 */
function toIso(mm, dd, closingContext) {
  const m = Number(mm);
  const d = Number(dd);
  ...
  if (!closingYear || closingYear < 1990 || closingYear > 2100) return null;
...
function currencyToCentsFromStatement(s) {
  if (!s) return null;
 ...
 */
function parseModernCheckingEra(text, { closing }) {
  ...
  let inTable = false;
  const tupleOrd = new Map();
  for (const raw of lines) {
    const line = raw.trim();
    if (!line) continue;
    if (/^\s*(TRANSACTIONS|ACCOUNT\s+ACTIVITY|DEPOSITS?\s+AND\s+OTHER\s+CREDITS|WITHDRAWALS?\s+AND\s+OTHER\s+DEBITS)\s*$/i.test(line)) {
      inTable = true; continue;
    }
    if (/^\s*(ENDING\s+BALANCE|TOTAL\s+FEES|FEE\s+SUMMARY|DAILY\s+BALANCE\s+SUMMARY)/i.test(line)) {
      inTable = false; continue;
    }
    if (!inTable) continue;
    // "MM/DD <desc> <amount> <balance>" or "MM/DD/YY <desc> <amount>"
    const m = line.match(/^(\d{1,2})\/(\d{1,2})(?:\/(\d{2,4}))?\s+(.+?)\s+(-?\$?[\d,]+\.\d{2})(?:\s+(-?\$?[\d,]+\.\d{2}))?\s*$/);
    if (!m) continue;
    ...
    const ctx = m[3]
      ...
      : closing;
    const iso = toIso(m[1], m[2], ctx);
    if (!iso) continue;
    const description = m[4].replace(/\s{2,}/g, ' ').trim();
    const amount = currencyToCentsFromStatement(m[5]);
    const balance = m[6] ? currencyToCentsFromStatement(m[6]) : null;
 ...
 */
function parseCreditCardEra(text, { closing }) {
  const lines = text.split(/\r?\n/);
  const txns = [];
  let inTable = false;
  const tupleOrd = new Map();
  for (const raw of lines) {
    ...
    if (/^\s*(TRANSACTIONS|PURCHASES|PAYMENTS\s+AND\s+CREDITS)\s*$/i.test(line) &&
        !/^\s*TOTAL/i.test(line)) {
      inTable = true; continue;
    }
    // Section ends: any summary/total line inside a statement.
    if (/^\s*(TOTAL\b|FEES\s+CHARGED|INTEREST\s+CHARGED|YEAR-TO-DATE|IMPORTANT\s+ACCOUNT)/i.test(line)) {
      inTable = false; continue;
    }
    if (!inTable) continue;
    // "MM/DD MM/DD [Ref#] <desc> <amount>". Amount supports trailing minus
    // (USAA prints payments as "$10.00-" not "-$10.00") and optional $.
    const m = line.match(/^(\d{1,2})\/(\d{1,2})\s+(\d{1,2})\/(\d{1,2})\s+(.+?)\s+(-?\$?[\d,]+\.\d{2}-?)\s*$/);
    if (!m) continue;
    const iso = toIso(m[1], m[2], closing);
    if (!iso) continue;
    const description = m[5].replace(/\s{2,}/g, ' ').trim();
    const amount = currencyToCentsFromStatement(m[6]);
    if (amount == null) continue;
    const tupleKey = `${iso}|${amount}|${description}`;
    const ord = tupleOrd.get(tupleKey) || 0;
    tupleOrd.set(tupleKey, ord + 1);
    txns.push({ iso, amount, description, balance: null, tupleKey, ord });
  }
  return txns;
}
...
export async function parsePdfStatement({ buffer, accountId, accountName, period }) {
  ...
  if (!closing && period) {
    ...
    if (y && m) closing = { closingYear: y, closingMonth: m };
  }
  if (!closing) {
  ...
  const records = best.map((t) => ({
    ...
    check_number: (t.description.match(/CHECK\s*#?\s*0*(\d+)/i) || [])[1] || null,
    source: provenance,
    fetched_at: nowIso,
  }));
  return { txns: records, parseMeta: { era: chosen, year: closing.closingYear, closingMonth: closing.closingMonth } };
}
