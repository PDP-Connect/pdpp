...
const AMAZON_UFPO_DETAIL_FIXTURE = new URL("./__fixtures__/order-detail-ufpo-minimal.html", import.meta.url);
...
test("processListOrder: the already-hydrated skip frees per-run detail budget for a genuinely new order (tail convergence)", async () => {
  ...
  assert.deepEqual(coverage.hydrated.sort(), ["ord-new-reaches-budget", "ord-old-1", "ord-old-2"].sort());
...
test("processListOrder: ufpo grocery detail URL hydrates instead of becoming a gap", async () => {
  ...
  await processListOrder(page, deps, makeRunFlags(), makeListOrder({ orderId: "ord-ufpo-1" }));

  assert.deepEqual(coverage.required, ["ord-ufpo-1"]);
  assert.deepEqual(coverage.hydrated, ["ord-ufpo-1"]);
  assert.deepEqual(coverage.gap, []);
  assert.equal(findDetailGaps(protocolMessages).length, 0, "ufpo detail pages must not emit degraded detail gaps");
  assert.ok(
    emitted.some((r) => r.stream === "order_items" && r.data.asin === "B02UFPO002"),
