...
function healthTypeShort(t) {
  if (!t) return null;
  return t.replace(/^HKQuantityTypeIdentifier|^HKCategoryTypeIdentifier|^HKDataType/, '');
}
...
async function streamParse({ path, onRecord, onWorkout, onProgress }) {
  let buf = '';
  ...
  return new Promise((resolve, reject) => {
    stream.on('data', (chunk) => {
      ...
      while ((m = re.exec(buf)) !== null) {
        ...
        if (tag === 'Record') { onRecord(attrs); recordCount++; }
        else if (tag === 'Workout') { onWorkout(attrs); workoutCount++; }
        lastEnd = re.lastIndex;
      }
      buf = buf.slice(lastEnd);
      if (recordCount + workoutCount > 0 && (recordCount + workoutCount) % 10000 === 0) {
        onProgress(recordCount, workoutCount);
      }
    });
    stream.on('end', () => { onProgress(recordCount, workoutCount); resolve(); });
...
async function main() {
  ...
  const path = existsSync(join(dir, 'export.xml'))
    ...
    : existsSync(join(dir, 'apple_health_export', 'export.xml'))
      ...
      : null;
  if (!path) {
    emit({ type: 'SKIP_RESULT', reason: 'export_not_found', message: `no export.xml in ${dir}/ or ${dir}/apple_health_export/` });
  ...
  let latestRec = sinceRec;
  let latestWork = sinceWork;

  emit({ type: 'PROGRESS', message: `Streaming ${path}` });

  await streamParse({
    path,
    onProgress: (rc, wc) => emit({ type: 'PROGRESS', message: `Parsed ${rc} records, ${wc} workouts` }),
    onRecord: (attrs) => {
      if (!requested.has('records')) return;
      const startDate = isoDate(attrs.startDate);
      if (!startDate) return;
      if (sinceRec && startDate <= sinceRec) return;
      const type = healthTypeShort(attrs.type) || attrs.type || 'Unknown';
      const value = attrs.value != null ? Number(attrs.value) : null;
      ...
      emitRecord('records', {
        ...
        value_raw: !isFinite(value) && attrs.value ? attrs.value : null,
        start_date: startDate,
        end_date: isoDate(attrs.endDate),
      });
      if (!latestRec || startDate > latestRec) latestRec = startDate;
    },
    onWorkout: (attrs) => {
      if (!requested.has('workouts')) return;
      const startDate = isoDate(attrs.startDate);
      if (!startDate) return;
      if (sinceWork && startDate <= sinceWork) return;
      const id = hashId(`${attrs.workoutActivityType || ''}|${attrs.sourceName || ''}|${startDate}`);
      emitRecord('workouts', {
        id,
        workout_activity_type: attrs.workoutActivityType ? attrs.workoutActivityType.replace(/^HKWorkoutActivityType/, '') : null,
      ...
      if (!latestWork || startDate > latestWork) latestWork = startDate;
    },
  });

  if (requested.has('records')) emit({ type: 'STATE', stream: 'records', cursor: { last_start_date: latestRec } });
  if (requested.has('workouts')) emit({ type: 'STATE', stream: 'workouts', cursor: { last_start_date: latestWork } });
