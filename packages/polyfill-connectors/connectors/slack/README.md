...
**Transport:** these four calls run through a real headless Chromium page (`acquireSlackApiBrowserTransport` in `index.ts`), not plain Node `fetch`. The page first navigates to Slack's documented [`api.test`](https://docs.slack.dev/reference/methods/api.test/) document at `https://slack.com/api/api.test` and asserts that the final page origin is exactly `https://slack.com` before any credentialed API call: an authenticated root-page redirect to `app.slack.com` is rejected and retried after clean browser release. The launcher's initial `about:blank` page makes a `credentials: "include"` request cross-origin, and Slack's wildcard CORS response cannot authorize that credentialed mode. slackdump's own Slack Web API client wraps every live call in a uTLS transport that emulates a real Chrome TLS ClientHello; a plain Node `fetch()` presents a different TLS fingerprint at the handshake layer and Slack's edge rejects it as `invalid_auth`/401 even for a valid token+cookie pair — confirmed by reading slackdump's own request-construction path, not inferred. `manifests/slack.json` declares `runtime_requirements.bindings.browser` as `required: false`: the connector's core streams (all ten above) stay fully headless, and a runtime with no browser binding still spawns the connector and collects them; only these four optional streams fail closed with a per-stream `SKIP_RESULT` when no browser is available. **Residual risk:** the exact-origin API bootstrap is proven against real local Chrome with seeded fake `d`/`d-s` cookies and a live unauthenticated Slack response; authenticated acceptance by the live Vana Slack workspace remains owner-only UAT; the request shape remains proven byte-identical to slackdump's own reference client.
