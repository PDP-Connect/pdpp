...
**Transport:** these four calls use the same direct HTTPS transport as Slackdump's durable session provider: the existing `xoxc` form token, derived `d`/`d-s` cookies, and user agent are sent by `nodeFetchSlackApiTransport`. No browser binding or browser surface is required for collection. Each stream remains independently optional, so a real Slack auth, permission, rate-limit, or network failure produces an honest per-stream `SKIP_RESULT` without failing the archive streams. **Residual risk:** authenticated acceptance by the live Vana Slack workspace remains owner-only UAT; request construction is covered by focused direct-transport tests and the official Slackdump source comparison.
