...
**Transport:** these four calls run through a real headless Chromium page (`acquireSlackApiBrowserTransport` in `index.ts`), not plain Node `fetch`. slackdump's own Slack Web API client wraps every live call in a uTLS transport that emulates a real Chrome TLS ClientHello; a plain Node `fetch()` presents a different TLS fingerprint at the handshake layer and Slack's edge rejects it as `invalid_auth`/401 even for a valid token+cookie pair — confirmed by reading slackdump's own request-construction path, not inferred. `manifests/slack.json` declares `runtime_requirements.bindings.browser` as `required: false`: the connector's core streams (all ten above) stay fully headless, and a runtime with no browser binding still spawns the connector and collects them; only these four optional streams fail closed with a per-stream `SKIP_RESULT` when no browser is available. **Residual risk:** the browser-transport fix has not been confirmed against a live Slack workspace from this development environment (egress to slack.com is blocked here); the transport mechanism itself is proven end-to-end against a real launched Chromium, and the request shape is proven byte-identical to slackdump's own reference client, but final confirmation that Slack's edge accepts the resulting fingerprint is owner-only UAT.
