...
**Transport:** these four calls run through a real headless Chromium page (`acquireSlackApiBrowserTransport` in `index.ts`), not plain Node `fetch`. The page first navigates to Slack's documented [`api.test`](https://docs.slack.dev/reference/methods/api.test/) document at `https://slack.com/api/api.test` and asserts that the final page origin is exactly `https://slack.com` before any credentialed API call: an authenticated root-page redirect to `app.slack.com` is rejected and retried after clean browser release. The page context receives the `d`/`d-s` cookies through `context.addCookies`, and page fetch removes the browser-forbidden `Cookie`/`User-Agent` headers, leaving the form token while Chromium owns the cookie jar and user agent. This mirrors Slackdump v3.1.13's `chttp.New` cookie jar and `rusq/slack` form request methods. `manifests/slack.json` declares `runtime_requirements.bindings.browser` as `required: false`: the connector's core streams (all ten above) stay fully headless, and a runtime with no browser binding still spawns the connector and collects them; only these four optional streams fail closed with a per-stream `SKIP_RESULT` when no browser is available. **Residual risk:** authenticated acceptance by the live Vana Slack workspace remains owner-only UAT; the request shape is covered by the focused browser-transport regression and the pinned Slackdump source comparison.
