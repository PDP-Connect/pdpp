...
import {
  ...
  type BrowserSurfaceWaitOutcome,
...
const CHASE_CURRENT_ACTIVITY_TABLE_MARKER_RE = /<table\b[^>]*\bclass=["'][^"']*\bmds-activity-table\b[^"']*["']/giu;
...
function countStructuralMarker(html: string, marker: RegExp): number {
  const matches = html.match(marker);
  return matches?.length ?? 0;
}

/** Never retain a final URL: classify the known overview route in memory. */
export function classifyChaseCurrentActivityRoute(value: string): BrowserSurfaceDiagnostic["route"] {
  try {
    const url = new URL(value);
    if (url.hostname !== "secure.chase.com" || url.pathname !== CHASE_DASHBOARD_OVERVIEW_PATH) {
      return "unknown";
    }
    return url.hash === CHASE_DASHBOARD_OVERVIEW_HASH ? "expected" : "unknown";
  } catch {
    return "unknown";
  }
}

function chaseCurrentActivityDiagnostic(
  input: Omit<Parameters<typeof buildBrowserSurfaceDiagnostic>[0], "kind">
): BrowserSurfaceDiagnostic {
  const diagnostic = buildBrowserSurfaceDiagnostic({ ...input, kind: "chase_current_activity" });
  if (!diagnostic) {
    throw new Error("closed Chase browser-surface diagnostic input was invalid");
  }
  return diagnostic;
}
...
export async function snapshotDashboardHtmlForCurrentActivity(
  ...
  managedSurface: BrowserSurfaceManagedState = "unknown"
): Promise<{ diagnostic: BrowserSurfaceDiagnostic; html: string; rowSurfaceReady: boolean }> {
  const immediateHtml = await page.content().catch((): string => "");
  const immediateParserCount = parseCurrentActivityDom(immediateHtml, referenceDateIso).length;
  const immediateDashboardMarkerCount = countStructuralMarker(immediateHtml, CHASE_DASHBOARD_MARKER_RE);
  const immediateTableMarkerCount = countStructuralMarker(immediateHtml, CHASE_CURRENT_ACTIVITY_TABLE_MARKER_RE);
  const route = classifyChaseCurrentActivityRoute(page.url());
  if (immediateParserCount > 0) {
    return {
      diagnostic: chaseCurrentActivityDiagnostic({
        activityTableMarkerCount: immediateTableMarkerCount,
        dashboardMarkerCount: immediateDashboardMarkerCount,
        managedSurface,
        parserCount: immediateParserCount,
        readCount: 1,
        route,
        targetCount: immediateParserCount,
        waitOutcome: "not_needed",
      }),
      html: immediateHtml,
      rowSurfaceReady: true,
    };
  }

  let waitOutcome: BrowserSurfaceWaitOutcome = "resolved";
  await page
...
export async function runCurrentActivity(
  ...
  filteredAccounts: readonly ChaseAccount[],
  surfaceDiagnostic?: BrowserSurfaceDiagnostic
): Promise<void> {
  ...
  if (emitted === 0) {
    await deps.emit({
      ...
      diagnostics: { account_count: 1, ...(surfaceDiagnostic ? { browser_surface: surfaceDiagnostic } : {}) },
    });
  } else {
    await deps.emit({
      type: "PROGRESS",
    ...
    async collect(ctx: BrowserCollectContext): Promise<void> {
      ...
      try {
        ...
        const dashboardHtmlForCurrentActivity = deps.wantsCurrentActivity
          ? await snapshotDashboardHtmlForCurrentActivity(
              page,
              deps.emittedAt.slice(0, 10),
              browserSurfaceManagedState(browserSurface)
            )
          : {
              diagnostic: chaseCurrentActivityDiagnostic({
        ...
        if (deps.wantsCurrentActivity) {
          await runCurrentActivity(
            deps,
            dashboardHtmlForCurrentActivity.html,
            filteredAccounts,
            dashboardHtmlForCurrentActivity.diagnostic
          );
        }
