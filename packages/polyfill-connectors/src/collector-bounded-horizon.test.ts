...
async function seedClaudeCode(): Promise<ConnectorFixture> {
	...
	await writeFile(
		...
		'{"broken PDPP_SENTINEL_MUST_NOT_BE_READ\n',
		"utf8",
	);
	return {
...
async function seedCodex(): Promise<ConnectorFixture> {
	...
	await writeFile(
		join(oldDir, "rollout-2020-01-05T00-00-00-sess2020.jsonl"),
		'{"broken PDPP_SENTINEL\n',
		"utf8",
	);
	await writeFile(
...
async function startRunHarness(
...
async function runFixture(
	...
	const args = [
		"--import",
		"tsx",
		join(CONNECTORS_DIR, fixture.connector, "index.ts"),
	];
	...
	test(`${connectorId}: an interrupted run before completion commits no coverage checkpoint`, async () => {
		...
		try {
			...
			await assert.rejects(() =>
				runFixture(fixture, harness, { abortSignal: controller.signal }),
			);
			assert.equal(
				harness.terminalPosts.length,
				0,
