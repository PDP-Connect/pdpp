...
// ─── post-assist origin establishment ────────────────────────────────────
// Regression for production run_1787918248525 (12:57Z) and run_1787880617136
// (01:30Z), the SAME fault twice: the owner personally cleared a Venmo
// security check, the resume path probed, `ensureVenmoOrigin` issued a full
// `page.goto`, and that fresh navigation re-armed the wall he had just passed.
// The run died `venmo_origin_navigation_failed`.
//
// The landing was a Venmo FAMILY host, not a stranger: the stored message read
// `(landed on [redacted-url])`, `redactTransportDetail` only substitutes on an
// ABSOLUTE_URL match, and `describeLandedOrigin` emits a `https://` string ONLY
// on its family branch. So the guard's own one-shot family retry had already
// fired and been walled too — a second `goto` is not the answer.

test("post-assist: a family-host page routes in-page instead of re-navigating", async () => {
  const gotoUrls: string[] = [];
  const assigned: string[] = [];
  let currentUrl = "https://id.venmo.com/signin";
  const page: Pick<Page, "evaluate" | "goto" | "url" | "waitForLoadState"> = {
    async evaluate(fn: unknown, arg?: unknown): Promise<unknown> {
      // The in-page route: mirror a real SPA `location.assign` by moving the
      // page, exactly as a browser would.
      if (typeof arg === "string" && arg.startsWith("https://")) {
        assigned.push(arg);
        currentUrl = arg;
        return undefined;
      }
      return await Promise.resolve({ kind: "live", ownerId: "1234567890123456789" });
    },
    goto(url: string): ReturnType<Page["goto"]> {
      gotoUrls.push(url);
      currentUrl = url;
  ...
  const result = await probeVenmoAccount(page as Page, "pre_submit", { afterOwnerAssist: true });

  assert.deepEqual(
    gotoUrls,
    [],
    "post-assist must NOT issue a full navigation — that is what re-armed the wall in run_1787918248525"
  );
  assert.deepEqual(assigned, ["https://venmo.com/"], "it must still REACH the venmo.com origin, in-page");
  assert.equal(result.live, true, "and the probe must then actually run, not read a stale page");
});

test("post-assist: falls back to the normal guard when the in-page route does not land", async () => {
  // Honest fallback: no client router, a hard redirect, or an unexpected host.
  // The failure surface must be UNCHANGED — the same guard, the same fault.
  const gotoUrls: string[] = [];
  let currentUrl = "https://account.venmo.com/";
  const page: Pick<Page, "evaluate" | "goto" | "url" | "waitForLoadState"> = {
    async evaluate(_fn: unknown, arg?: unknown): Promise<unknown> {
      if (typeof arg === "string" && arg.startsWith("https://")) {
        return undefined; // route silently does nothing: no SPA router
      }
      return await Promise.resolve({ kind: "live", ownerId: "1234567890123456789" });
    },
    goto(url: string): ReturnType<Page["goto"]> {
  ...
  assert.deepEqual(gotoUrls, ["https://venmo.com/"], "an unlanded in-page route must fall back to the real guard");
  assert.equal(result.live, true);
});

test("PRESERVED: a fresh run still navigates — the post-assist path must not leak", async () => {
  // Negative control. Without this, deleting the `afterOwnerAssist` guard
  // entirely would look correct: the fresh-run path MUST keep navigating,
  // because `about:blank` has nothing to preserve and CORS still demands the
  // venmo.com origin.
  const gotoUrls: string[] = [];
  let currentUrl = "about:blank";
  const page: Pick<Page, "evaluate" | "goto" | "url" | "waitForLoadState"> = {
  ...
  assert.deepEqual(gotoUrls, ["https://venmo.com/"], "a fresh run must still navigate normally");
  assert.equal(result.live, true);
});
