...
test("listed manifests declare a top-level connector_id for auto-registration", () => {
  // `register-all` and live manifest reconciliation both consume
  // `connector_id`. A public listing that is only keyed by `connector_key`
  // is not auto-registerable and will be skipped by the reconciliation path.
  const offenders: string[] = [];
  for (const name of MANIFEST_NAMES) {
    const manifest = readManifest(name);
    if (manifest.capabilities?.public_listing?.listed !== true) {
      continue;
    }
  ...
  assert.deepEqual(
    offenders,
    [],
    `manifests with public_listing.listed=true must declare a top-level connector_id so they can auto-register and reconcile: ${offenders.join(", ")}`
  );
});
