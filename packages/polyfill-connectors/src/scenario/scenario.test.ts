...
test("TRACE_POLICY: every EmittedMessage kind has an explicit disposition, exactly the thirteen kinds the protocol declares", () => {
  const expectedKinds = [
    "RECORD",
    "STATE",
    "PROGRESS",
...
test("TRACE_POLICY: ASSISTANCE and ASSISTANCE_STATUS are the only unsupported_claim_withheld kinds", () => {
  const withheld = Object.entries(TRACE_POLICY)
    .filter(([, disposition]) => disposition === "unsupported_claim_withheld")
    .map(([kind]) => kind)
    .sort();
  assert.deepEqual(withheld, ["ASSISTANCE", "ASSISTANCE_STATUS"]);
});
