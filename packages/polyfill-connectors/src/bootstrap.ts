...
export async function probeBrowser({
  ...
  console.log(
    `Probing logged-in state headlessly against profile at ${PROFILE_DIR}\n`
  );
  ...
  for (const key of platforms) {
    ...
    try {
      ...
      status[key] = (await platform.isLoggedIn(page, context))
        ? "ok"
        : "logged_out";
    } catch (err) {
