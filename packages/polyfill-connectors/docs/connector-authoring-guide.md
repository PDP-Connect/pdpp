...
```ts
import { z } from "zod";
import { pdppSafeText, nullablePdppSafeText } from "@pdpp/connector-protocol/pdpp-safe-text";
...
```ts
import { safeTextPreview, PDPP_PREVIEW_MAX_CHARS } from "@pdpp/connector-protocol/safe-text-preview";
...
- **Full-scan prune at run boundary.** If your stream re-fetches the complete source set each run, call `cursor.dropUnseenIds()` after the loop and before emitting STATE. Without this, deleted records keep their fingerprint forever and a later re-add silently skips. Streams whose run is *not* a full scan (a partial paginated tail) must NOT call `dropUnseenIds`; the carry-forward is the right behavior.
...
**DOM-capture connectors: human review is mandatory, not optional.** The default name rule only fires on labelled/keyed fields (`name: value`, `"customerName": "..."`). A bare name inside an HTML text node — the normal case for a rendered page, e.g. `<span> Jane Doe </span>` with no adjacent label — matches nothing and is **not** redacted. This is a deliberate scope limit (see the comment in `src/scrub-defaults.ts`), not a bug, but it means the deterministic scrubber alone is provably unsafe on DOM/HTML captures. Any connector whose raw fixtures are captured HTML must go through eye review or the `--llm-redactions-dir` structured-redaction path before its scrubbed output is committed — do not treat the deterministic pass as sufficient for HTML.
