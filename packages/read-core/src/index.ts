...
export function stableInlineJson(value: any) {
  return JSON.stringify(value);
}

export function truncateText(value: any, limit: any) {
...
export function extractRecordRows(body: any) {
...
export function summarizeRecordEvidence(body: any, label: any, options: AnyOptions = {}) {
...
export function summarizeFieldWindowEvidence(body: any) {
...
export function formatEnvelopeHandles(body: any) {
  const parts = [];
...
export function buildRecordContentLadder(record: any, options: AnyOptions = {}) {
...
export function buildRecordSetContentLadder(body: any, options: AnyOptions = {}) {
  const records = extractRecordRows(body)
    .map((record: any) => buildRecordContentLadder(record, options))
    .filter(Boolean)
...
export function defaultEncodeResourceUri(kind: any, payload: AnyRecord) {
  return `pdpp://${kind}/${encodeContentHandle(kind, payload)}`;
}

export function encodeContentHandle(kind: any, payload: AnyRecord) {
...
export function decodeContentHandle(handle: any, expectedKind: any) {
...
export function sanitizeRecordForEvidence(record: any) {
...
export function binaryFieldMetadata(fieldPath: any, value: any) {
...
function recordContentIdentity(record: any, fallback: AnyRecord = {}) {
  const payload = (objectValue(record?.data) || objectValue(record?.record) || objectValue(record)) as AnyRecord;
...
function recordContentFields(record: any, identity: AnyRecord, options: AnyOptions) {
  const payload = (objectValue(record?.data) || objectValue(record?.record) || objectValue(record)) as AnyRecord;
...
function recordContentBinaryFields(record: any, options: AnyOptions) {
  const payload = (objectValue(record?.data) || objectValue(record?.record) || objectValue(record)) as AnyRecord;
  return Object.entries(payload)
...
function recordContentJsonFields(record: any, identity: AnyRecord, options: AnyOptions) {
  const payload = (objectValue(record?.data) || objectValue(record?.record) || objectValue(record)) as AnyRecord;
  return Object.entries(payload)
...
function isJsonEvidenceField(fieldPath: any, value: any) {
...
function sanitizeEvidenceValue(value: any): any {
...
function isContentStringField(fieldPath: any, value: any) {
  return (
...
function isLargeBase64Field(fieldPath: any, value: any) {
...
function blobRefMetadata(value: any) {
...
function envelopeField(body: any, key: string) {
...
function envelopeCount(body: any) {
...
function objectValue(value: any): AnyRecord | null {
