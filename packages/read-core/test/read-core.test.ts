...
const summaryHeaderPattern = /^messages: 2 record\(s\)\. has_more=true\./;
const cursorPattern = /next_cursor="cursor_2"/;
const exactCountPattern = /count=exact:9/;
const truncatedRecordPattern = /record_preview_truncated=true|record\[1\]/;
const moreRecordsPattern = /more_records=2/;
const cursorFollowupPattern = /followup=rerun_cursor_or_limit/;
const fieldWindowUriPattern = /^pdpp:\/\/field-window\//;
const recordIdentityPattern = /messages:m1/;
const textSuffixPattern = /text$/;
...
test("summarizeRecordEvidence is bounded and exposes continuation facts", () => {
  ...
  assert.match(text, summaryHeaderPattern);
  assert.match(text, cursorPattern);
  assert.match(text, exactCountPattern);
  assert.match(text, truncatedRecordPattern);
...
test("summarizeRecordEvidence gives content-only clients a follow-up path for omitted rows", () => {
  ...
  assert.match(text, moreRecordsPattern);
  assert.match(text, cursorFollowupPattern);
});
...
test("content ladder handles are state, not authorization-bearing provider paths", () => {
  ...
  assert.match(uri, fieldWindowUriPattern);
  assert.doesNotMatch(uri, recordIdentityPattern);
  assert.doesNotMatch(uri, textSuffixPattern);
});
