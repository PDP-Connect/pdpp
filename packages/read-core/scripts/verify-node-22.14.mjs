...
import { boundRuntimeEnvironment, resolveNpmRuntime } from './npm-runtime.mjs';
import { runOfflineConsumerProbe } from './pack-install-test.mjs';

export const exactNodeVersion = 'v22.14.0';
const packageRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');

export function assertExactRuntime(actualVersion = process.version) {
  assert.equal(
    actualVersion,
    exactNodeVersion,
...
async function main() {
  ...
  const receipt = {
    command: {
      packageScript: 'pnpm build && node scripts/verify-node-22.14.mjs',
