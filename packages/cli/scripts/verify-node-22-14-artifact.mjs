...
try {
  ...
  const output = execFileSync(process.execPath, [resolve(packageRoot, 'scripts/pack-install-run.mjs')], {
    cwd: packageRoot,
  ...
  assert.ok(receipt.subprocesses.some(({ label }) => label === 'pnpm build'), 'receipt missed the build subprocess');
  assert.ok(receipt.subprocesses.some(({ label }) => label.startsWith('npm ')), 'receipt missed npm subprocesses');
  assert.ok(receipt.subprocesses.some(({ label }) => label.startsWith('npx ')), 'receipt missed npx subprocesses');
