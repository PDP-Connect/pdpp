...
test("ref run timeline: outputs JSON when --format json", async () => {
  ...
  const captured = capture();
  await runRefRun(["timeline", "run-abc", "--as-url", "https://ref.test", "--format", "json"], captured.io, fetch);
...
test("ref run timeline: sends Cookie header from --owner-session flag", async () => {
  ...
  const { io } = capture();
  await runRefRun(["timeline", "run-abc", "--as-url", "https://ref.test", "--owner-session", "mysecret"], io, fetch);
...
test("ref run timeline: sends Cookie from PDPP_OWNER_SESSION_COOKIE env var", async () => {
  ...
  try {
    const { io } = capture();
    await runRefRun(["timeline", "run-abc", "--as-url", "https://ref.test"], io, fetch);
...
test("ref grant timeline: sends Cookie header from --owner-session", async () => {
  ...
  const { io } = capture();
  await runRefGrant(["timeline", "grant-xyz", "--as-url", "https://ref.test", "--owner-session", "tok=val"], io, fetch);
...
test("ref trace show: outputs JSON when --format json", async () => {
  ...
  const captured = capture();
  await runRefTrace(["show", "trace-111", "--as-url", "https://ref.test", "--format", "json"], captured.io, fetch);
  ...
  const fetch = mockFetch({
    "https://ref.test/_ref/runs/run-abc/timeline": {
      body: {
        data: [{ event: "started" }],
        meta: {
          warnings: [{ code: "deprecated_alias", message: "connector_instance_id is deprecated; use connection_id" }],
        },
      },
    },
  });
