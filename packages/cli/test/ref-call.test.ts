...
test("ref call sends a bearer GET to /v1/owner/* and never sends a cookie", async () => {
  ...
  try {
    const code = await runRefCall(["GET", "/v1/owner/control", "--as-url", "https://ref.test"], cap.io, fetchImpl);
    assert.equal(code, 0);
    const { url, opts } = fetchImpl.calls[0];
...
test("ref call rejects a mismatched --auth before issuing any request", async () => {
  ...
  const cap = capture();
  await assert.rejects(
    runRefCall(["POST", "/_ref/runs/r1/cancel", "--as-url", "https://ref.test", "--auth", "bearer"], cap.io, fetchImpl),
    PdppUsageError
  );
