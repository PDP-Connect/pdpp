...
import { runRefEventSubscriptions } from "../src/ref/commands/event-subscriptions.ts";
...
test("event-subscriptions list forwards filter flags as query params", async () => {
  ...
  const c = capture();
  const code = await runRefEventSubscriptions(
    ["list", "--as-url", "http://ref.test", "--client-id", "client_alpha", "--status", "disabled", "--format", "json"],
    c.io,
    fetchImpl
  );
  assert.equal(code, 0);
...
test("unknown subcommand throws usage error", async () => {
  await assert.rejects(
    () =>
      runRefEventSubscriptions(["frobnicate"], capture().io, async () => ({
        ok: true,
        status: 200,
        text: async () => "",
        headers: { get: () => null },
