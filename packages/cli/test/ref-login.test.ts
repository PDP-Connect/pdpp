...
test("extractCookieFromSetCookie reads named cookie from various shapes", () => {
  assert.equal(extractCookieFromSetCookie(["pdpp_owner_session=ok; Path=/; HttpOnly"], "pdpp_owner_session"), "ok");
  assert.equal(extractCookieFromSetCookie("pdpp_owner_session=v2; Secure", "pdpp_owner_session"), "v2");
  assert.equal(extractCookieFromSetCookie("other=1; Path=/", "pdpp_owner_session"), null);
});
...
test("ref run timeline: --owner-session flag overrides cached session", async () => {
  await withTmpCache(async (cacheRoot) => {
    ...
    const { io } = capture();
    await runRefRun(
      ["timeline", "run-1", "--as-url", "https://ref.test", "--cache-root", cacheRoot, "--owner-session", "flag-only"],
      io,
      fetch
    );
