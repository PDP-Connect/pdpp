...
async function loadReleasercTypes() {
  ...
  const firstTypesBlockMatch = yaml.match(
  ...
  const [, block] = firstTypesBlockMatch;
  return parseTypesYaml(block);
}

function parseTypesYaml(block: string): Record<string, string | boolean>[] {
...
test("release notes config exists and lists both scoped and generic sections", async () => {
  ...
  const sections = types
    .filter((entry) => entry.scope || entry.type === "feat" || entry.type === "fix")
...
test("feat(local-collector) routes to the local-collector section", async () => {
  ...
  const transformed = transform(commit({ type: "feat", scope: "local-collector", subject: "enroll smoke" }), {
    host: "",
    owner: "",
    repository: "",
  });
...
test("feat(cli) routes to the cli section", async () => {
  const { transform } = await makeTransform();
  const transformed = transform(commit({ type: "feat", scope: "cli", subject: "help text" }), {
    host: "",
    owner: "",
    repository: "",
  });
...
test("fix(local-collector) routes to the local-collector Bug Fixes section", async () => {
  const { transform } = await makeTransform();
  const transformed = transform(commit({ type: "fix", scope: "local-collector", subject: "retry path" }), {
    host: "",
    owner: "",
    repository: "",
  });
...
test("feat with no scope falls back to the generic Features section", async () => {
  const { transform } = await makeTransform();
  const transformed = transform(commit({ type: "feat", subject: "generic change" }), {
    host: "",
    owner: "",
    repository: "",
  });
...
test("feat with an unknown scope still falls back to the generic Features section", async () => {
  const { transform } = await makeTransform();
  const transformed = transform(commit({ type: "feat", scope: "web", subject: "unrelated change" }), {
    host: "",
    owner: "",
    repository: "",
  });
...
test("chore is suppressed for both scoped and unscoped commits", async () => {
  const { transform } = await makeTransform();
  const generic = transform(commit({ type: "chore", subject: "bump dev dep" }), {
    host: "",
    owner: "",
    repository: "",
  });
  assert.equal(generic, undefined);
  const scoped = transform(commit({ type: "chore", scope: "local-collector", subject: "tidy fixtures" }), {
    host: "",
    owner: "",
    repository: "",
  });
