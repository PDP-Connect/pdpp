...
export interface ViewportObservation {
  ...
  layout: ViewportSize;
  mobile?: boolean;
  orientation?: { angle: number; type: string } | null;
  safeArea?: { bottom: number; left: number; right: number; top: number } | null;
  timestampMs?: number;
  virtualKeyboard?: VirtualKeyboardSample | null;
  visual?: VisualViewportSample | null;
}

export type ViewportTransitionKind =
  | "browser-chrome"
  | "keyboard-occlusion"
  | "layout-resize"
  | "orientation-change"
...
function heightDrop(previous: ViewportSize, next: ViewportSize): number {
  return previous.height - next.height;
}

function sameWidth(previous: ViewportSize, next: ViewportSize): boolean {
  return Math.abs(previous.width - next.width) <= VIEWPORT_TOLERANCE_PX;
}

function sameHeight(previous: ViewportSize, next: ViewportSize): boolean {
  return Math.abs(previous.height - next.height) <= VIEWPORT_TOLERANCE_PX;
}

function sameSize(previous: ViewportSize, next: ViewportSize): boolean {
  return sameWidth(previous, next) && sameHeight(previous, next);
}

function sameSizeWithin(previous: ViewportSize, next: ViewportSize, tolerancePx: number): boolean {
  return Math.abs(previous.width - next.width) <= tolerancePx && Math.abs(previous.height - next.height) <= tolerancePx;
}

function isKeyboardSizedDrop(previous: ViewportSize, next: ViewportSize): boolean {
  const drop = heightDrop(previous, next);
  const dropRatio = drop / previous.height;
  return (
    sameWidth(previous, next) &&
...
function orientationChanged(previous: ViewportObservation, next: ViewportObservation): boolean {
...
function explicitKeyboardInset(next: ViewportObservation): number {
...
function visualKeyboardInset(previous: ViewportObservation, next: ViewportObservation): number {
...
function layoutKeyboardInset(previous: ViewportObservation, next: ViewportObservation): number {
...
function visualViewportMoved(previous: ViewportObservation, next: ViewportObservation): boolean {
  if (!(previous.visual && next.visual)) {
    return false;
  }
  return (
    Math.abs(previous.visual.offsetTop - next.visual.offsetTop) > VIEWPORT_TOLERANCE_PX ||
    Math.abs(previous.visual.pageTop - next.visual.pageTop) > VIEWPORT_TOLERANCE_PX ||
    !sameSize(previous.visual, next.visual)
  );
}

function zoomChanged(previous: ViewportObservation, next: ViewportObservation): boolean {
  if (!(previous.visual && next.visual)) {
    return false;
  }
...
function mobileChromeHeightChanged(previous: ViewportObservation, next: ViewportObservation): boolean {
...
export function classifyViewportTransition(
  previous: ViewportObservation | null | undefined,
  ...
  const keyboardInset = Math.max(
    ...
    explicitKeyboardInset(next),
    next.editableFocused ? visualKeyboardInset(previous, next) : 0,
    next.editableFocused ? layoutKeyboardInset(previous, next) : 0
  );
  if (keyboardInset > 0) {
    return {
  ...
  if (
    visualViewportMoved(previous, next) &&
