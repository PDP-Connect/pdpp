...
export class BrowserSurfaceLeaseManager {
  ...
  invalidateSurface(
    surfaceId: string,
    options: { reason?: BrowserSurfaceWaitReason; releaseLease?: boolean } = {},
    ...
    let failedLease: BrowserSurfaceLease | undefined;
   ...
   * persist the changes.
   */
  async reconcileSurfacesWithAllocator(
    allocator: BrowserSurfaceAllocator,
  ): Promise<{ evicted: BrowserSurface[]; downgraded: BrowserSurface[] }> {
    if (this.#config.surfaceMode !== "dynamic") {
      return { evicted: [], downgraded: [] };
    }
    const evicted: BrowserSurface[] = [];
    const downgraded: BrowserSurface[] = [];
    for (const surface of [...this.#surfaces.values()]) {
      if (surface.backend !== "neko") {
        continue;
      }
      let live: BrowserSurface | null;
      try {
  ...
  #detachStaleInitialSurfaceLease(surface: BrowserSurface, nonTerminalLeaseIds: ReadonlySet<string>): BrowserSurface {
  ...
  #retainedSurfaceCount(): number {
    return [...this.#surfaces.values()].filter(
      (surface) => surface.retained === true && this.#surfaceConsumesCapacity(surface),
    ).length;
  }
   ...
   */
  #retainedDemandCount(excludeLeaseId?: string): number {
    const fromQueuedLeases = [...this.#leases.values()].filter(
      (lease) =>
        lease.lease_id !== excludeLeaseId &&
        lease.retained === true &&
        lease.status === "waiting_for_browser_surface",
    ).length;
   ...
   */
  #retainedCreationWouldExhaustReserve(excludeLeaseId?: string): boolean {
