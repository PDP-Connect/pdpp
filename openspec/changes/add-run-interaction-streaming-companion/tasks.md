...
- [ ] 12.8 Rebuild/recreate the n.eko Docker overlay and run the full desktop plus real-phone smoke matrix before marking the tranche complete. No-human checks (OpenSpec, remote-surface + reference streaming units, console types/build, live-CDP smoke) pass via `pnpm stream:no-human-verify`; physical real-phone smoke matrix remains. Re-confirmed green 2026-06-01 for the nine deterministic checks that do not require rebuilding the Docker overlay. Two residual sub-gates: (a) Docker n.eko overlay rebuild via `PDPP_DOCKER_DYNAMIC_NEKO_ALLOCATOR_SMOKE=1 pnpm stream:no-human-verify` is no-human but is currently blocked in this build sandbox — the `neko` image `patchright-chromium` stage hangs after the 170 MiB Chromium download completes; a pre-existing live `pdpp-neko`/`pdpp-neko-allocator` overlay was healthy with the allocator `/surfaces` HTTP surface returning 200, so the runtime path is sound and the block is the image-build environment, not the code; (b) physical real-phone smoke matrix.
