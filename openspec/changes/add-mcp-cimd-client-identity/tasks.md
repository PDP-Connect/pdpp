...
- [ ] 5.1 Add a single "Connect Agents" page or panel to the operator console. Keep the
...
- [ ] 5.2 List operator-created CIMD documents and their stable URLs inside the Connect
       Agents flow rather than as a separate OAuth administration surface.
- [ ] 5.3 Render one client-specific command set at a time for Claude Code and Codex
       (default OAuth and explicit CIMD identity variants) for each client identity.
- [ ] 5.4 Include revocation action (deletes the document; tokens issued to that client_id
...
- [x] 6.2 Unit: redirect_uri same-origin constraint allows localhost exception, rejects
       cross-origin.
- [ ] 6.3 Unit: fetch status/redirect/size-over-5-KB/timeout/malformed-document failures abort authorization and never issue a grant.
...
- [ ] 6.8 Dashboard: Connect Agents page shows one recommended setup path by default,
       does not ask for owner/control-plane bearer tokens, and keeps advanced/headless
       credentials separate from normal MCP setup.
- [ ] 6.9 Run `openspec validate add-mcp-cimd-client-identity --strict` and
