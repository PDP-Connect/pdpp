...
- The owner-facing connection projection (`listConnectorSummaries` / `_ref/connections`) SHALL represent a connector that has no connection as a not-connected catalog entry (no real `connector_instance_id`) rather than as a synthesized active connection. Owner connection actions (sync, pause, resume, revoke, delete) SHALL NOT be offered for a connector that has no connection.
...
- No PDPP protocol contract change. The `_ref` / owner-connections surface shape gains a not-connected catalog representation; clients reading real connections are unaffected because a not-connected catalog entry carries no `connector_instance_id`.
