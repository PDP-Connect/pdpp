...
This conflates two distinct concepts the reference already names elsewhere: a **catalog connector** (a `connector_id` you can add, owned by the `connectors` table and add-connection surface) versus a **connection** (a `connector_instance_id` you configured). The catalog-completeness honesty contract (`Reference connector catalog SHALL be complete for listed first-party manifests`) is about catalog visibility, not connection projection; it does not require — and should not produce — `connector_instances` rows.
...
- Catalog completeness on a fresh database is preserved: every `listed: true` first-party manifest remains visible through the registered connector catalog / add-connection surface, sourced from the `connectors` table or shipped manifests, without a `connector_instances` row.
...
- `reference-implementation-architecture` — clarifies that catalog completeness is satisfied by the registered connector catalog / add-connection surface, independent of `connector_instances`.
