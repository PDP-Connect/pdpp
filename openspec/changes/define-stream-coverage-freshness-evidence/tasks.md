...
- [ ] 7.1 Join declared manifest streams against a stable canonical live-record snapshot and synthesize exact-zero rows only when record-snapshot evidence is current at its exact source checkpoint; retained-size rows own byte/history/blob measures only. (Re-opened 2026-07-16 by `reconcile-active-summary-evidence`: the checked retained-size count authority was disproved by live missing/dirty evidence and is superseded before implementation.)
- [ ] 7.2 Console stream rows render "0 records" only from a current canonical snapshot and render count-unavailable/stale when record evidence is unobserved/stale/failed; retained-byte failure never erases a current count.
- [ ] 7.3 Tests: mixed zero/nonzero streams under current record evidence, unobserved/stale/failed record evidence reads unavailable, dirty retained bytes preserve canonical counts, and a fresh local collector with zero live rows in declared streams projects exact zero on SQLite and real Postgres.
...
- [ ] 9.1 Reproducible machine audit that fails when a required stream rests unmeasured/unknown beneath a settled connection verdict: a seeded local test plus a live mode reusing the owner-journey acceptance harness. The live mode SHALL audit every settled connection, distinguish stored-manifest drift (per-stream `coverage_strategy` missing from the served collection report) from producer gaps (strategy present, checkpoint/denominator absent), treat active bounded work as inconclusive, and keep declared-stream count absence inconclusive when record-snapshot evidence is non-current instead of fabricating zero. (Re-opened 2026-07-15: post-#315-deploy live acceptance found two defects. (1) `isSettledConnection` only excluded `revoked`, so `fix-pending-connection-discovery`'s later `draft`/`setup_in_progress` connections — now served by the same `/_ref/connectors` route this audit reads — were judged as settled and seven HEB draft enrollment shells with no coverage evidence yet were reported as hard failures; fixed by excluding `status === "draft"` and `owner_state.resolver === "setup_in_progress"` from settled scope. (2) A masked stream whose coverage-report check and retained-record check both independently detected the same missing evidence was pushed to `maskedStreams` twice, duplicating GitHub/Gmail/HEB findings within one connection; fixed by deduping pushes on `(stream, evidence class)` while keeping distinct classes on the same or different streams separately visible. Both fixes proven with mutation tests: reverting either one independently fails a new regression test. `reference-implementation/test/stream-health-audit.test.js` gates both. Re-opened again 2026-07-16 because that audit still treats retained-size availability as count authority; `reconcile-active-summary-evidence` must move it to canonical record-snapshot evidence before this item is complete.)
...
- [ ] 9.4 Full-suite acceptance: `openspec validate define-stream-coverage-freshness-evidence --strict`, reference-implementation/polyfill-connectors/console suites, typechecks, and lint. (Raw reference suite is green as of 2026-07-21; leave open because the unrelated declared reference lint baseline remains red.)
...
- [x] 10.2 Make the connection-scoped `coverage_diagnostics` STATE proof fail closed: require a valid `{ fetched_at }` cursor, server `updated_at`, a full known-store diagnostic inventory, sanitized metadata, and worst-wins folding including malformed, dropped, duplicate, and `unaccounted` rows. (Re-closed 2026-07-21 final re-gate: unsupported inventory authority is explicitly unreliable and production-path tests prove it cannot seed complete report rows or an audit pass.)
...
- [x] 10.4 Emit the full `coverage_diagnostics` STATE proof from Claude Code after full collection and before successful terminal completion.
...
- [x] 10.6 Add focused proof and negative-control tests (including local-vs-account precedence, state/read/heartbeat/outbox/gap failures, run/schedule admission, UI modality, and real ChatGPT audit red) plus machine audit coverage. (Raw-green 2026-07-21: committed production projections cover shipped ChatGPT's six required red streams and all 18 cookie/owner-agent local-device controls; unsupported-local proof remains covered.)
