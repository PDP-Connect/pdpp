...
## Canonical-Count Exactness

The retained-size stream projection is structurally sparse and independently
repairable. It cannot own live-record counts: a missing or dirty retained-size
row says nothing trustworthy about whether the canonical record set is empty,
and retained bytes may fail while current counts remain observable. The count
authority is therefore a stable canonical `records WHERE deleted = false`
snapshot stamped with the connection's exact record-source checkpoint. The
connector summary joins the manifest's declared streams against that exhaustive
live-record snapshot and synthesizes exact zero only when its record-snapshot
evidence is current. When record evidence is unobserved, stale, or failed,
absent rows remain unavailable/stale rather than becoming fabricated zero.

Retained-size rows remain authoritative for byte, history, and blob measures
only. Dirty or unavailable retained-size evidence makes those measures
unavailable without erasing a current canonical count. Coverage remains another
independent axis: proven local coverage neither manufactures a count nor turns a
known zero into proof that the provider boundary was completely observed. This
authority correction MUST be implemented and verified by the dependent
`reconcile-active-summary-evidence` change before the two deltas are archived in
that order.
...
6. Make the connection rollup consume required-stream unknown/unmeasured
   ...
   gap (USAA `transactions`), make canonical counts exact under current
   record-snapshot evidence, and replace the manual live acceptance with a
   reproducible machine audit. Apply/archive this change before the dependent
   `reconcile-active-summary-evidence` `MODIFIED` delta.
