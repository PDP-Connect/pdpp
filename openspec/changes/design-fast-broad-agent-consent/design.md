...
- **Hosted MCP `retention.classification` is off-spec.** The hosted MCP picker currently emits `retention: { classification: 'client_policy', description: '…' }` (see `reference-implementation/server/index.js::HOSTED_MCP_PICKER_RETENTION`). `spec-core.md` defines `retention` as `{ max_duration: ISO 8601 duration, on_expiry: 'delete' | 'anonymize' }` and treats it as a structured policy declaration. The AS validator (`reference-implementation/server/auth.js::normalizePendingGrantRequest`) currently passes `retention` through without shape validation, so the drift is silent on the wire but visible to anyone who reads a grant. The picker's access-mode narrowing tranche deliberately does NOT expose owner control over retention because narrowing the wrong shape would deepen the drift. The next slice that touches retention should: (a) reconcile the hosted MCP `retention` shape with `spec-core.md`, then (b) expose a retention preset control in the picker. Until then the picker copy treats retention as a fixed package default rather than an owner-narrowable knob.
