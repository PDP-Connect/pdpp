...
4. **Where do blob bytes live?** (New 2026-04-20.) The Slack connector produced 24 GB of attachment bytes for one workspace, largest single file 810 MB. The reference impl has a `blobs` SQLite table with a `data BLOB` column that's not yet wired. Attempting to store 24 GB of video in a SQLite column would make the file unmanageable (backups, VACUUM, mmap all break at that scale). This suggests topology-for-records and topology-for-blobs may be different questions — records stay in SQLite, blobs go to content-addressed filesystem or object store, both fronted by the same HTTP surface. See `blob-hydration-open-question.md` for the record-side framing of the same problem.
