...
- **WHEN** a connection whose manifest refresh policy declares it manual or background-unsafe has aged past its freshness window
...
- **WHEN** a manual-refresh-only connection (manifest `recommended_mode: "manual"` or `background_safe: false`) has a succeeded last run, complete coverage, and freshness `stale`
