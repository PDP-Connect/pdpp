...
- [ ] Add resume tests for interrupted attachment backfill (crash mid-window, replay from durable cursor). Collector-runner `STATE` persistence/replay is now covered by `design-local-collector-state-sync`; the remaining missing piece is an IMAP test double or similar seam for crashing and resuming `runAllMailPasses` mid-window.
