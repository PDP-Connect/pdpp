...
- **AND** the warning SHALL link to the operator action (`npm i -g @pdpp/local-collector@latest`) without exposing device tokens or local paths

### Requirement: Public operator copy SHALL advertise the npm-installable collector path as the supported public flow

The public dashboard, `pdpp connect` copy, and operator-facing docs SHALL advertise `npx -y @pdpp/local-collector ...` (or a global install of the same package) as the supported public path for filesystem-class connector collection on a fresh host. Monorepo-clone instructions SHALL remain available only as a development path, not as the primary public flow.
