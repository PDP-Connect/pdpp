...
| Rate-control algorithms | GCRA/token-bucket for admission gating; AIMD for adaptive rate adjustment; composite concurrency + rate layers |
| Checkpoint/queue models | Slice = smallest replayable unit; time budget as outer bound; item count as soft inner ceiling; budget check at slice boundaries |
...
### D2. Token bucket for inter-request pacing

Token bucket TB(r, B_max) is the correct pacing primitive for a catch-up
workload. Tokens accrue at fill rate `r` up to burst depth `B_max`; a request is
admitted only when a token is available. This allows accumulated credit to burst
during recovery (catching up fast) while enforcing a sustained rate ceiling.

Leaky bucket is wrong for a catch-up workload: it enforces a constant output
rate, preventing burst acceleration. (Supported by: Stripe docs, Kong analysis,
AWS Builder's Library, Linkerd retry quota design — four independent sources.)
...
AIMD (Additive Increase, Multiplicative Decrease) governs dynamic fill-rate
adjustment: the fill rate grows additively on successful responses and decreases
multiplicatively on throttle signals (429, 503, elevated latency). This finds
the provider's true rate ceiling from below without requiring explicit quota
configuration. (Supported by: AWS SDK adaptive mode, Google SRE client-side
throttling.)
...
### D8. Commit-gated monotonic checkpoint
...
written page, never the last attempted page.
...
- Not a requirement to implement a distributed multi-client concurrency
  ...
  credential, coordination is an open implementation question; this change does
  not address it.
...
- **Multiple connectors sharing a provider credential.** If two concurrent
  ...
  half rate. This change leaves that coordination to the implementation.
