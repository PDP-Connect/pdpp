...
- [x] 1.3 Publish host port from `${PDPP_POSTGRES_BIND_HOST:-127.0.0.1}:${PDPP_POSTGRES_PORT:-55432}` mapped to container `5432`, so default proof runs are loopback-only.
...
- [x] 2.1 Add `PDPP_POSTGRES_IMAGE`, `PDPP_POSTGRES_BIND_HOST`, `PDPP_POSTGRES_PORT`, `PDPP_POSTGRES_USER`, `PDPP_POSTGRES_PASSWORD`, and `PDPP_POSTGRES_DB` to `.env.docker.example`.
