...
- [x] 6.3 Second sub-slice (partial — agent-connect only): `server/routes/as-agent-connect.ts` exporting `createAgentConnectAttemptStore`, `mountAsAgentConnect`, `mountAsAgentConnectToken`. The in-progress attempt state is encapsulated in `AgentConnectAttemptStore` (created once in `buildAsApp`); the store is shared between the two HTTP routes and the consent approve/deny handlers via injected callbacks. `server/index.js` shrank from 5851→5793 LOC (−58 net for this tranche). Verified: `pnpm verify` clean (typecheck + Biome), 25/25 `agent-cli.test.js`, 82/82 `as-operations*.test.js`, 10/10 `owner-auth.test.js`. Remaining routes (DCR, authorize, consent, device UI, MCP-package authorize, polyfill connectors) still inline pending next owner-approved slice.
