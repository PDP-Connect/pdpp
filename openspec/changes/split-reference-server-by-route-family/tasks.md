...
- [x] 6.5 PAR slice: `server/routes/as-par.ts` exporting `mountAsPar`. Covers `POST /oauth/par` (RFC 9126, public endpoint). Base-URL resolution computed once at mount time from `opts.asPublicUrl` / `AS_PUBLIC_URL` env (same logic as inline code); native manifest and `initiateGrant` injected via `MountAsParContext`. `handleError` + `setReferenceTraceId` injected for header propagation. Imports for `executeAsParCreate` removed from `server/index.js`. `pnpm run verify` passes; `openspec validate split-reference-server-by-route-family --strict` passes. Covered by: `test/as-operations.test.js` (40/40), `test/pdpp.test.js` (PAR paths), `test/cli.test.js` (PAR paths).
- [x] 6.6 Device-UI slice: `server/routes/as-device-ui.ts` exporting `mountAsDeviceUi`. Covers `GET /device` (owner-session-gated code-entry / approval form), `POST /device/approve`, and `POST /device/deny` (both owner-session + CSRF gated). All hosted-UI rendering helpers (`escapeHtml`, `renderHostedDocument`, `renderPageIntro`, `renderEmptyState`, `renderKeyValueList`, `renderSurface`, `renderResultState`) injected via `HostedUiRenderer` to avoid a direct `.js` import. Owner-session enable/subject-id state, CSRF helpers, `ownerDeviceAuthStore` store capabilities, and `OWNER_AUTH_DEFAULT_SUBJECT_ID` all injected via `MountAsDeviceUiContext`. Imports for `executeAsDeviceDecision` removed from `server/index.js`. `pnpm run verify` passes; `openspec validate split-reference-server-by-route-family --strict` passes. Covered by: `test/owner-csrf.test.js` (30/30), `test/security-device-code-exposure.test.js` (6/6), `test/as-operations.test.js` (40/40 including device.decision tests).
