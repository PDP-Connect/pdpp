...
- **WHEN** a `gmail/labels`, `usaa/statements`, or `chase/accounts` key's history contains adjacent versions whose only difference is the run-clock field (`fetched_at` for statements/accounts) or is byte-identical under the stored body (labels)
- **THEN** the tool SHALL classify those adjacent versions as removable under the policy's `excludeKeys` (`[]` for `gmail/labels`, `["fetched_at"]` for `usaa/statements` and `chase/accounts`), matching the connector's per-record fingerprint no-op-emit definition
- **AND** a version that changes any retained source field (for example a renamed Gmail label, a re-hydrated USAA statement with a different `pdf_sha256`, or a renamed Chase account) SHALL remain a fingerprint boundary that is never collapsed
