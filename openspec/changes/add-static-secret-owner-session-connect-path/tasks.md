...
- [ ] 1.1 Add `'draft'` to `VALID_STATUSES` in
  `server/stores/connector-instance-store.js` so `normalizeRecord` and
  `updateStatus` admit it on both arms.
- [ ] 1.2 Widen the SQLite `connector_instances.status` CHECK to include
  `'draft'` in the base DDL (`server/db.js`).
- [ ] 1.3 Add a forward CHECK-widening migration
...
- [ ] 1.4 Add a store primitive `activateDraft(connectorInstanceId, { now })`
  on both SQLite and Postgres arms: flip `draft → active`; no-op if the row is
  not `draft` (idempotent, concurrency-safe).
- [ ] 1.5 Test: status admission, migration idempotency, `activateDraft` flip +
  no-op guard.

## 2. Read-surface exclusion for drafts

- [ ] 2.1 Exclude `draft` from `listByOwner` by default on both arms (the single
...
- [ ] 2.2 Confirm `resolveActiveByConnector` / `listActiveByConnector` remain
  active-only (already SQL-filtered).
- [ ] 2.3 Test: a draft does not appear on `/_ref/connections`,
...
- [ ] 3.1 Add `allowStatuses` (default `['active']`) to
  ...
  unchanged.
- [ ] 3.2 Thread `allowStatuses` through the host
  `resolveOwnerConnectorNamespace` (`server/index.js`).
- [ ] 3.3 Test: default rejects draft; `['active','draft']` admits draft;
  connector-only resolution is unaffected.

## 4. Owner-session draft-create route

- [ ] 4.1 Add `POST /_ref/connectors/:connectorId/draft-connection`
...
- [ ] 4.2 Mount the route in `server/index.js` with its context.
- [ ] 4.3 Test: draft created for gmail/github; rejected for non-static-secret;
  two creates → two distinct ids; no secret in response or audit; not visible on
  reads.

## 5. Capture admits a draft target

- [ ] 5.1 Pass `allowStatuses: ['active', 'draft']` in the capture route's
  namespace resolution (`server/routes/ref-static-secret-credentials.ts`); no
  other change.
- [ ] 5.2 Test: owner-session capture seals onto a draft; bearer/agent cannot
  reach the draft as a target.

## 6. First-ingest activation

- [ ] 6.1 In `mountRsRecordsIngest` (`server/routes/rs-mutation.ts`): pass
...
- [ ] 6.2 Test: first ingest with records flips draft → active and the
  connection becomes visible; zero-record/failed ingest leaves it draft.

## 7. Validation

- [ ] 7.1 `openspec validate add-static-secret-owner-session-connect-path
  --strict`.
- [ ] 7.2 Focused reference-implementation tests added in tranches above.
- [ ] 7.3 `pnpm --dir reference-implementation run verify` (tsc + lint) when
  source changes.
- [ ] 7.4 `git diff --check`.
