...
- the reference-only owner-auth placeholder has now landed across the current browser-facing owner/operator surfaces: when `PDPP_OWNER_PASSWORD` is set, the reference gates `/consent*`, `/device*`, and `/dashboard*` with one local-only signed-session cookie, ignores freeform `subject_id` submissions in favor of `PDPP_OWNER_SUBJECT_ID` (defaulting to `owner_local`), and keeps `/owner/login` as the stable owner entry point. The remaining open question is the durable owner-auth story beyond this placeholder. See [`design-notes/owner-auth-placeholder-open-question-2026-04-22.md`](design-notes/owner-auth-placeholder-open-question-2026-04-22.md).
- the hosted-UI alignment tranche for the reference AS has now landed: `GET /consent`, `GET /device`, the approval/deny result pages, and `GET /owner/login` all render through a shared hosted-UI module (`reference-implementation/server/hosted-ui.js`) that reuses the PDPP brand mark, typography, and `data-surface` language. `/owner/login` becomes a stable owner-entry page: sign-in form when placeholder auth is enabled, honest disabled-state landing page when it is not, and signed-in landing page when a session already exists. A single reference-only stylesheet is served at `GET /__pdpp/hosted-ui.css`. This stylesheet is explicitly reference-only implementation support, not a PDPP protocol surface.
- the reference-local third-party connect convenience tranche has now landed: dynamic client registration is enabled by default for the forkable local reference (backed by a shared reference-local default initial access token), `pdpp-web-dashboard` and `pdpp-polyfill-owner-bootstrap` are seeded as default pre-registered public clients, and a small example third-party app (`reference-implementation/examples/third-party-app/`) exercises the current thin provider-connect flow end to end. This is reference-local convenience, not PDPP protocol widening — `PDPP_ENABLE_DYNAMIC_CLIENT_REGISTRATION=0` still disables DCR, operator-supplied `PDPP_DCR_INITIAL_ACCESS_TOKENS` still wins, and the example client does not prove a full generic authorization-code redirect profile. See [`design-notes/reference-local-third-party-connect-defaults-2026-04-22.md`](design-notes/reference-local-third-party-connect-defaults-2026-04-22.md).
- single-origin reference composition has now landed as an explicit reference-hosting mode: the Next app and the AS/RS remain internal processes, but the local reference product can present one browser-facing origin with shared owner session across `/owner/*` and `/dashboard/*`, proxied protocol/reference namespaces, and public metadata/verification URLs that point at the browser-facing origin rather than the internal `:7662/:7663` listen ports. The reference now treats `direct` and `composed` as deliberate local hosting modes, with shared topology helpers (`reference-implementation/server/reference-topology.js`) and `PDPP_REFERENCE_MODE` / `PDPP_REFERENCE_ORIGIN` as the local control knobs. This remains a reference-hosting choice, not a PDPP protocol requirement. See [`design-notes/single-origin-reference-composition-plan-2026-04-22.md`](design-notes/single-origin-reference-composition-plan-2026-04-22.md).
...
3. collapse the current browser-facing operator/reference split into one reference origin by composition, so the control plane, hosted approval UI, and public protocol metadata stop teaching separate-origin local reference usage by default
