...
- the hosted-UI alignment tranche for the reference AS has now landed: `GET /consent`, `GET /device`, the approval/deny result pages, and `/owner/login` (when the placeholder is enabled) all render through a shared hosted-UI module (`reference-implementation/server/hosted-ui.js`) that reuses the PDPP brand mark, typography, and `data-surface` language. A single reference-only stylesheet is served at `GET /__pdpp/hosted-ui.css`. This stylesheet is explicitly reference-only implementation support, not a PDPP protocol surface.
