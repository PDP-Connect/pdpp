...
- the approval-surface half of the owner-auth placeholder has now landed: when `PDPP_OWNER_PASSWORD` is set, the reference gates `/consent*` and `/device*` with a local-only signed-session cookie and ignores freeform `subject_id` submissions in favor of `PDPP_OWNER_SUBJECT_ID` (defaulting to `owner_local`). What remains open is whether `/dashboard` should be protected by the same placeholder session before a more durable owner-auth story is chosen. See [`design-notes/owner-auth-placeholder-open-question-2026-04-22.md`](design-notes/owner-auth-placeholder-open-question-2026-04-22.md).
