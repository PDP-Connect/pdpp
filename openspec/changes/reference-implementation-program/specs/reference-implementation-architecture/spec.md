...
- **WHEN** the reference runtime preserves validated connector-declared `DONE.error` details only for failed or cancelled terminal states and rejects contradictory success terminals
...
This architecture change SHALL reflect settled root-spec decisions in the reference implementation without re-opening them as OpenSpec design questions.
