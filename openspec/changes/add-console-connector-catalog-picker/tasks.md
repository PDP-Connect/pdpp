...
- [x] 1.1 Add `apps/console/src/app/dashboard/lib/connection-catalog.ts`: a pure
...
- [x] 1.2 Add `connection-catalog.test.ts`: assert the catalog covers every
...
- [x] 2.1 Records page (`records/page.tsx`, server component) calls
      `listConnectorManifests()`, builds the catalog, and passes it to
      `RecordsListView`.
- [x] 2.2 `RecordsListView` forwards the catalog to `AddConnectionGuidance`.
- [x] 2.3 `AddConnectionGuidance` renders the catalog grouped by disposition:
...
- [x] 3.1 `node --test --import tsx` focused console suite incl. new
      `connection-catalog.test.ts`, `connection-modality.test.ts`,
      `records-list-view.test.ts` — pass.
- [x] 3.2 `pnpm --dir apps/console types:check` — pass.
- [x] 3.3 `pnpm exec ultracite check` on changed files — pass.
- [x] 3.4 `openspec validate add-console-connector-catalog-picker --strict` — pass.
- [x] 3.5 `git diff --check` — clean.
