...
The hosted MCP package adapter SHALL build its child `RsClient` fetch base from a configured internal resource-server base URL when one is present, and SHALL fall back to the advertised public resource when no internal base is configured. Concretely, `handleHostedMcp` resolves an internal base (new optional env, e.g. `RS_INTERNAL_URL`) and passes it to `createPackageRsClient` as the child fetch base, distinct from the advertised public `resource`.

Default for the configured internal base is the loopback/service-DNS resource-server address — the same internal address the web app already uses (`PDPP_RS_URL: http://reference:7663`). When the env is unset and no default applies, the adapter falls back to the public resource, so current behavior is preserved for deployments that do not set it.
