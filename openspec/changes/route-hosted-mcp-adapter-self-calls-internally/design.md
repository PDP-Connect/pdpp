...
**No new env is introduced.** Reuse the established `PDPP_RS_URL` → `rsInternalUrl` config when it is explicitly supplied by the operator or test harness. The bare topology default (`http://localhost:7663`) is not treated as an adapter internal base. The reference compose sets `PDPP_RS_URL: http://reference:7663` for the web app, and the adapter reads the same explicit value. Inventing a parallel `RS_INTERNAL_URL` was rejected — it would duplicate `PDPP_RS_URL`/`rsInternalUrl`.
...
- **Internal base diverges from the live RS (wrong host/port)** → self-calls fail to connect. Mitigation: use the same explicit `PDPP_RS_URL` value the web app already uses in compose (`http://reference:7663`); treat a connect failure as a typed RS error like any other, and keep the public fallback only for unset config.
