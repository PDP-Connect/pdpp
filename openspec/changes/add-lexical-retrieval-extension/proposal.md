...
- **Lock the discovery carrier** to the existing resource-server metadata document. The advertisement is published as a `capabilities.lexical_retrieval` object inside that document and answers only global facts about the extension: whether it is supported, where the endpoint lives, whether cross-stream is supported, whether snippets are supported, and global limit defaults. It does **not** duplicate per-stream field declarations, and it does **not** become a broader capability document. It is reachable without a grant.
...
### New Capabilities
- `lexical-retrieval`: defines an optional, discoverable, grant-safe, lexical-only retrieval surface at `GET /v1/search` with stream-level searchable-field declarations and an advertisement published inside the existing resource-server metadata document.
...
- `openspec/specs/lexical-retrieval/spec.md` (new, on archival)
