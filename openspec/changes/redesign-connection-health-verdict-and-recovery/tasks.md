...
- [x] 13.0c Make stalled-outbox owner actions cause-specific: `state_read_failed` SHALL tell the owner to re-run the collector only, `dead_letter_backlog` SHALL include retry-dead-letters then re-run, and `stale_pending` SHALL re-run/restart the collector; expose the cause-specific remediation payload on `required_actions[]` for the focused console recovery panel.
