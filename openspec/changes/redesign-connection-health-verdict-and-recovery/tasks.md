...
- [x] 13.0b Correct the upstream synthesizer channel derivation for live false-reassurance cases: `axes.outbox:"stalled"` SHALL create a non-calm `Check the collector` owner action and degraded resumable stale gaps SHALL create advisory `retry_gap` / `Retry now` instead of calm `wait`; gate both at the synthesizer level.
