...
- [ ] 2.11.1 Apply a single `access_mode` to every child grant in a package; do not offer per-source access-mode mixing in this tranche.
  ...
  - Not attempted (left unchecked): the batch path currently preserves each staged entry's own `access_mode` (`approveStagedGrantBatch` issues each child against its staged selection; `auth.js:3074-3083`), so a package CAN today hold children with mixed access modes — there is no single-mode guard and no test pinning it. The consent form offers no per-source access-mode control (consistent with the intent), but the normative "apply one access mode to every child / reject or collapse mixing" guard is unimplemented. Blocker: deciding the guard semantics (reject mixed vs collapse to one) is a design call the spec leaves to implementation; adding it changes approval behavior beyond a reconcile. Left unchecked.
