...
- [x] Add `bin/reconcile-manifests.ts` CLI that walks every manifest, finds the matching connector, reports drift, exits nonzero on any drift.
...
- [x] Author 38 fixture files (62 total records) at `fixtures/<connector>/scrubbed/pilot-real-shape/records/<stream>.jsonl` for the six newly-schemed connectors. Authored by sub-agents from `schemas.ts` + `parsers.ts` + `types.ts`; reviewed and replayed.
- [x] All synthetic content uses `[REDACTED_*]` placeholders. No real owner data, no DB extracts. Records are committable.
