...
### Requirement: Schema-bearing connectors SHALL ship a committed pilot fixture and replay test
Every connector that exports a `validateRecord` from `schemas.ts` SHALL also ship:
...
- **WHEN** a connector ships `schemas.ts` but no committed fixture under `fixtures/<connector>/scrubbed/pilot-real-shape/records/`
