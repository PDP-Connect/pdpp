...
#### Scenario: Live schedule projection exposes the exact active sync

- **WHEN** a connection summary exposes `schedule.active_run_id` for a live run
- **THEN** the source-detail active status/action affordance SHALL link directly to `/syncs/<run_id>`
- **AND** the Syncs overview SHALL surface the same exact link without inferring the run from last-run history
- **AND** both surfaces SHALL preserve the full active run id exactly as provided by the schedule projection

#### Scenario: Unscheduled live run still resolves to its exact sync

- **WHEN** a connection summary has `schedule: null`
- **AND** its last run status is one of the current connector-summary active states (`pending`, `started`, or `in_progress`)
- **THEN** the Syncs overview SHALL link to that exact `/syncs/<run_id>`
- **AND** the source-detail active status/action affordance SHALL do the same

#### Scenario: Terminal and unknown summary statuses never become active links

- **WHEN** a connection summary has `schedule: null`
- **AND** its last run status is terminal, failed, or unknown
- **THEN** neither source detail nor Syncs overview SHALL promote that run to an exact active-sync link
