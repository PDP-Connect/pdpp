...
- [ ] 1.1 Add `packages/polyfill-connectors/src/collector-build-info.ts` exporting
...
- [ ] 1.2 Add `collector-build-info.ts` to `local-collector/tsconfig.build.json`
  `include`, and keep its `.d.ts` in `postbuild.mjs` `declarationKeep`.
- [ ] 1.3 Extend `postbuild.mjs` to overwrite the compiled
  `dist/polyfill-connectors/src/collector-build-info.js` with the real
  `version` (resolved package.json), `revision` (`git rev-parse --short=12 HEAD`
  → `PDPP_BUILD_REVISION` env → `source` fallback), and `builtAt`.

## 2. Send agent_version on heartbeats

- [ ] 2.1 Add optional `agent_version?: string` to `HeartbeatRequest` in
  `local-device-client.ts` (the wire schema already permits it).
- [ ] 2.2 In `collector-runner.ts`, compute `buildAgentVersion()` once and include
...
- [ ] 3.1 Add `agent_version: device.agentVersion ?? null` to the projected
  `device_exporter` object in `projectDeviceExporter`
  (`ref-device-exporters.ts`).
- [ ] 3.2 Render the reported agent version in the console device-exporters page
...
- [ ] 4.1 `collector-build-info` unit test: committed default is `0.0.0+source`;
  `buildAgentVersion()` composes `version+revision`; the string matches
  `^[^+]+\+([0-9a-f]{7,40}|source)$` and carries no path/secret.
- [ ] 4.2 `collector-runner.test.ts`: a run's heartbeats carry a non-empty
  `agent_version` equal to `buildAgentVersion()` (`0.0.0+source` in test).
- [ ] 4.3 Device-exporter route/projection test: the diagnostics projection
  includes `agent_version` — `null` before any versioned heartbeat, the stored
  string after one.

## 5. Validation

- [ ] 5.1 `openspec validate surface-local-collector-build-version --strict`.
- [ ] 5.2 Focused tests: `collector-runner.test.ts`, `collector-build-info` test,
  device-exporter route/store tests, local-collector `runner.test.js`.
- [ ] 5.3 Typechecks: polyfill-connectors / local-collector, reference-implementation,
  console types:check.
- [ ] 5.4 `pnpm --filter @pdpp/reference-contract run check:generated` (expect
  clean — no schema change).
- [ ] 5.5 Biome on changed files; `git diff --check`.
