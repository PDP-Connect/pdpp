...
Credential/session remediation metadata SHALL include a bounded owner-action surface that tells owner surfaces which repair path to open. Stored-credential missing or rejected evidence SHALL project the `stored_credential` surface. The precedence for a credential-shaped failure SHALL be durable or definitive stored-credential rejection, then applicable static-secret credential absence, then browser-session repair. Session-required evidence from a connection with a declared browser-session repair capability SHALL project the `browser_session` surface only after those stronger credential facts. Browser-session repair capability SHALL come from the connection's browser-session binding or an explicit required browser runtime binding; an optional browser binding is placement flexibility, not session-repair capability. Active leases and currently present remote surfaces are runtime state, not repair capability; an idle dynamic runtime SHALL NOT change this routing. A session-shaped reason without supplied browser-session repair capability SHALL NOT by itself be interpreted as browser-session repair. Provider-side prompts SHALL project `provider_interaction`; local collector/device repair SHALL project `local_device`; retry, schedule, maintainer, and no-action states SHALL project non-secret non-navigation surfaces.

A session-shaped reason on a browser-incapable connection with an active, unrejected stored credential is contradictory runtime evidence. The reference SHALL keep credential validity unknown or project a runtime/maintainer condition; it SHALL NOT label the stored credential rejected unless durable rejection evidence or a definitive rejection reason exists.

The same non-rejection rule applies to any non-definitive credential-shaped reason when durable evidence says the stored credential is active and unrejected.
...
**AND** the connection has a declared browser-session repair capability
**AND** there is no durable rejection or applicable static-secret credential absence
...
#### Scenario: Idle runtime preserves browser-session repair capability

**WHEN** a browser-session-repair-capable connection has a present, unrejected stored credential
**AND** its latest failure says `session_required`
**AND** its dynamic managed runtime has no active remote surface
**THEN** the reference implementation SHALL project `browser_session` repair with session-repair copy
**AND** it SHALL NOT claim that the stored credential was rejected.
...
#### Scenario: Static-secret absence wins before session repair

**WHEN** a static-secret-bound connection has no usable stored credential
**AND** a run also says `session_required`
**THEN** the reference implementation SHALL project `stored_credential` capture repair
**AND** it SHALL NOT route that connection to browser-session repair from the session reason alone.
...
#### Scenario: Connector-keyed detail is unresolved or ambiguous

**WHEN** a connector-keyed detail request has zero or multiple visible connections
**THEN** the reference SHALL omit connection health, rendered verdict, and current repair action
**AND** it SHALL NOT manufacture browser-session capability or any connection-scoped repair route from connector-level facts.
