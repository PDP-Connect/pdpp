...
- [x] 5.4 On startup, detect drift (record count vs index row count per stream) and rebuild if mismatched. Log the rebuild via the structured logger. Implemented as `lexicalIndexBackfillForManifest` in `search.js`, called from `startServer` (native mode) and from `registerConnector` (polyfill mode — covers first-time registration on a non-empty DB and the "operator turns on `lexical_fields` for an existing stream" case). Idempotent. Test: "pre-existing records become searchable after lexical_fields are declared (no re-ingest)" in `test/lexical-retrieval.test.js`.
