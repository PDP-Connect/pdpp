...
- Preserve `GET /v1/search/semantic` response shape, error codes (including the explicit forbidden-parameter list), cursor semantics (including the `sem1.` prefix and stale-cursor backend-identity rejection), scoring semantics (`semantic_distance`, lower-is-better), `retrieval_mode: "semantic"` per hit, grant filtering, stream/filter query semantics, backend/profile/model identity disclosure, and disclosure-spine shape exactly.
...
- score-advertisement gate: emit per-result `score` only when capability metadata advertises `score.supported: true`, `score.kind: "semantic_distance"`, and `score.order: "lower_is_better"`;
