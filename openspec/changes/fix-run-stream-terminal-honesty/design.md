...
The stream page SHALL NOT claim recovery unless the run terminal status is completed. When the continuing surface is shown, it revalidates against the run-handle status instead of remaining a static server render, because the run can fail after the owner lands on the stream page.
...
- The continuing surface refreshes until the run-handle status reaches a terminal state.
