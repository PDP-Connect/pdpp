...
- [ ] A genuinely lost retained container process records non-secret two-phase
  replacement evidence and projects continuity-indeterminate, not an immediate
  owner repair. Use exactly `capacity_pressure`, `idle_ttl`, `operator_requested`,
  `restart_reconcile`, `readiness_invalidated`,
  `allocator_internal_ensure_surface`, `same_container_browser_generation_change`,
  or `external_or_host_loss`; losslessly map the public `operator`, `reconcile`,
  and `surface_failed` stop reasons to their named ledger causes and update Luna's
  provisional fixture. Select current evidence by current process generation;
  redact generation identity; and preserve SQLite/Postgres parity and
  two-connection isolation. Only a typed, verified, connection-bound
  `ProviderInvalidationProof` may create at most one existing repair action.
  Portable authenticated-session continuity remains owned by the OPEN / UNSATISFIED handoff in
  `separate-ephemeral-browser-runtime-health`.
...
- [ ] Force a process/container replacement independently for two isolated
  connections and verify they become continuity-indeterminate and non-green with
  no false owner action or retry burst. The separately authorized
  `packages/polyfill-connectors` provider adapter,
  `reference-implementation/runtime` continuity orchestrator, and encrypted
  connection-scoped session store remain OPEN / UNSATISFIED until the exact-provider
  probe gate passes.
