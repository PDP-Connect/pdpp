...
### Requirement: Retained surfaces SHALL be reconstructed across ordinary restart without an intentional process stop, and true process loss SHALL preserve honest continuity uncertainty
...
be surfaced as non-green continuity-indeterminate evidence, not an immediate owner
repair. The RI SHALL emit non-secret two-phase replacement evidence whose exact
cause is one of `capacity_pressure`, `idle_ttl`, `operator_requested`,
`restart_reconcile`, `readiness_invalidated`, `allocator_internal_ensure_surface`,
`same_container_browser_generation_change`, or `external_or_host_loss`. It SHALL
map `operator` to `operator_requested`, `reconcile` to `restart_reconcile`, and
`surface_failed` to `readiness_invalidated` without collapsing idle-TTL or
readiness-invalidated causes into capacity or operator; unknown causality SHALL NOT
be relabeled as policy. Only a typed, verified, provider-originated,
connection-bound `ProviderInvalidationProof` may create at most one existing
connection-scoped repair condition. The portable authenticated-session continuity
handoff remains OPEN / UNSATISFIED until separately authorized and proven.
...
#### Scenario: Lost retained process creates no false owner repair

- **WHEN** a retained surface's container process is genuinely lost across a restart or host event
- **AND** the credential-boundary connector's provider API session cannot be proved
  from the persistent profile alone
- **THEN** the connection SHALL project non-green continuity-indeterminate evidence
  with no owner action
- **AND** scheduled automation SHALL NOT launch a repeated auth-retry burst
- **AND** a browser-session repair action SHALL be created only if a typed verified
  provider invalidation proof is later observed.
