...
3. The surface is later stopped by capacity pressure or idle TTL:
   ...
   - At the container level, `stopSurface` for `idle_ttl`/`capacity_pressure`
...
recreate) cannot be papered over: a fresh container loses ChatGPT API auth. That
is surfaced honestly as one connection-scoped `session_required` browser-session
repair — the same lifecycle owned by `complete-connection-repair-action-surfaces` —
not a silent failure, false-healthy, or retry storm. This change does not attempt to
persist provider session tokens outside the browser process (rejected below).
