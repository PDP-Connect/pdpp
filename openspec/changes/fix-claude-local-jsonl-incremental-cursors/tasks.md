...
- [x] 3.7 Fable cross-stream closure: isolate messages/top-level fallback evidence from sessions evidence; prove A (sessions-only evidence replays messages), B (stale messages replays despite current sessions), and C (messages-only evidence emits sessions).
