...
Pre-v1 checkpoints have only mtime maps, which cannot safely manufacture byte offsets. Migration therefore scans every discovered source from zero and persists the returned physical cursor. A child source is eligible to suppress its records only when the messages stream's own legacy map (or its top-level fallback when `messages.file_mtimes` is absent) has the scan's observed mtime; sessions consult only their own legacy map. A new, missing, malformed, or mismatched entry replays that current source once. This is deliberately per source and per stream: one changed file cannot turn matching contributors into a bulk replay, and a current mtime in one stream cannot claim delivery in another. If the source changed between opening and the decision, the child pass rescans it with emission enabled rather than advancing a fabricated cursor.
