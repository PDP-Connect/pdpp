...
**SUPERSESSION — what the implementation actually did (recorded `ri-split-site-archive-readiness-v1`).** Step 1 (extract `packages/operator-ui` *first*, with both surfaces still in `apps/web`) was **not** performed. The lanes split by **copy-then-trim**: `apps/web` was copied to `apps/console` and to `apps/site`, each app was trimmed to its surface, and `apps/web` was deleted (the live operator runtime stayed in `apps/console`; `apps/site`'s copy of the dashboard clients was converted to types-only). The "keep one working app at every commit" intent was still honored — `apps/web` remained the live surface until late in the order — but the shared-package extraction never landed. Consequence: the shared dashboard feature components and `dashboard/lib/*` helpers are now **duplicated** byte-for-byte in `apps/site` and `apps/console`, and `packages/operator-ui` does not exist. This duplication still conflicts with the normative "shared UI SHALL live in a workspace package … neither deployable SHALL duplicate" requirement in the `reference-implementation-architecture` and `reference-surface-topology` deltas of this change. The migration order above is preserved as the original plan of record; it is superseded as a description of what shipped. The unresolved disposition (extract the package, relax the requirement, or defer de-dup as a tracked follow-up) is an owner decision captured in `tasks.md` §2 and the archive-readiness report — it is the one open item gating archive.
