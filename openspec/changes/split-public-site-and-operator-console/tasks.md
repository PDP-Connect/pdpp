...
- [ ] 6.4 Remove `apps/web/` once tasks 2–5 are stable and verified. (STILL DEFERRED. `apps/site` now exists and is green, but `apps/web` removal additionally requires retargeting the root `Dockerfile` (`--filter pdpp-web build`, `CMD node apps/web/server.js`), `scripts/dev.mjs`, the root `web:*`/`build`/`preview`/`start`/`dev` scripts, and `docker-compose.yml`. That is a separate destabilizing tranche held for owner review; the design's migration order also places this last.)
