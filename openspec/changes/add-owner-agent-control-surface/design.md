...
- A double-revoke is idempotent-by-typed-error: the resolver's active-status gate returns `connector_instance_inactive` for an already-revoked connection, so a second revoke is a clean 4xx, not a silent success or a crash.
