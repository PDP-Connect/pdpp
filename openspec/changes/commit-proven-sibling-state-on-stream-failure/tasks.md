...
- [x] 2.1 Track named stream failures and map them to their covering state streams without connector-specific logic.
- [x] 2.2 Commit only eligible sibling state after a certified stream-scoped failure while preserving failed terminal evidence.

## 3. Verification

- [x] 3.1 Add fail-before/pass-after tests for sibling commit, leaked failed-stream state, parent-state mapping, and uncertified/global failure.
