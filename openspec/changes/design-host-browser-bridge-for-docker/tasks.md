...
- [x] `pnpm --dir packages/polyfill-connectors run test` (2026-04-30 rerun: 785 pass, 0 fail, 3 skipped).
...
- [x] `kind=host_browser_required` interaction emission from connectors. Browser connectors now decorate manual browser interactions as `host_browser_required` when a host bridge is configured; the runtime accepts the kind, dashboard copy distinguishes it, and connector-runtime tests cover the decoration.
- [x] Headless/manual-action mismatch audit. (Runtime now decorates browser `manual_action` messages in headless/no-bridge runs with explicit "rerun headed / configure host bridge" recovery copy. Pure helper coverage in `packages/polyfill-connectors/src/connector-runtime.test.ts` verifies default headless visibility, bridge detection, non-duplication, and no changes for OTP/headed/bridge-capable runs.)
