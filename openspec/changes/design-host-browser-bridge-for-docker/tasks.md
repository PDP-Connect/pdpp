...
- [ ] Headless/manual-action mismatch audit. (Observed 2026-04-28: Reddit launched `--headless`, then emitted `manual_action` copy instructing the operator to use "the browser window." The runtime/connector must either launch visibly for satisfiable manual actions or render an explicit "rerun headed / configure host bridge" recovery path. This applies to native-host headless and Docker headless paths, not only the host bridge.)
