...
2. **Postgres-oriented storage proof (`ConnectorStateStore` + `SchedulerStore`).** *Adapter-swap proof landed at the test level; production interface still not extracted.*
   ...
   Also landed: a non-default Postgres adapter spike at `reference-implementation/test/helpers/postgres-connector-state-scheduler-driver.js` and an env-gated runner at `reference-implementation/test/connector-state-scheduler-conformance-postgres.test.js`. The Postgres driver implements all three concerns directly against Postgres without coupling to the SQLite reference: owner-vs-grant scope enforced through two partial unique indexes (because Postgres treats NULL grant_id as distinct under a non-partial UNIQUE), schedule and active-run upsert through `ON CONFLICT (connector_id) DO UPDATE`, cross-connector `run_id` identity through `UNIQUE(run_id)`, and active-run reconciliation through a transactional drain that records each previously-active run id in a private `terminal_failed_runs` table that the harness's `wasRunMarkedFailed` reads from. Each scenario provisions a uniquely-named schema in `setup()` and drops it in `teardown()`, so concurrent harness runs do not collide.
   The harness gate is `PDPP_TEST_POSTGRES_URL`. When the env var is set the full 16-scenario suite runs against the configured Postgres instance; when unset the runner registers a single skipped test so default `pnpm test` environments do not require Postgres. The Postgres run was self-falsified twice on a real Postgres 16 instance: dropping `UNIQUE(run_id)` causes the cross-connector run_id scenario to fail; collapsing grant_id on read causes both grant-isolation scenarios to fail; restoring each invariant makes all 16 scenarios pass.
   The `pg` package is added as a dev dependency on `reference-implementation` only. There is no production Postgres adapter, no runtime branch on storage backend, and no `ConnectorStateStore` / `SchedulerStore` interface extracted from the controller. Promotion of those shapes into production interfaces remains a separate change.
