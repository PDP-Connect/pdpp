...
- [ ] 3.1 Define storage-only security proof: `ConsentStore` + `OwnerDeviceAuthStore` with SQLite and memory adapters. (Prerequisite harness landed via `add-consent-device-auth-conformance-harness`: SQLite driver + deliberately-broken in-memory falsifiability driver + 12 lifecycle/security scenarios. The broken driver is *not* a conforming memory adapter; no second conforming adapter exists, so the storage-only adapter-swap proof is incomplete.)
