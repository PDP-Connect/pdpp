...
- **Expose a stable JS helper `searchRecordsSemantic(query, opts)` in `apps/web/src/app/dashboard/lib/rs-client.ts`** (the existing module that already wraps the public RS surface for the dashboard — `listStreams`, `getStreamMetadata`, `queryRecords`, `getRecord`, `searchRecordsLexical`). Shape mirrors `searchRecordsLexical`: `(query: string, opts?: { streams?: string[]; limit?: number; cursor?: string }) => Promise<SearchResultPage>`. The helper preserves the full page envelope (`has_more`, `next_cursor`, `data`) rather than flattening to `data[]`, so the caller can paginate. This is NOT the `searchRecordsSemantic(query, scope)` shape the earlier draft proposed; the `scope` argument is removed because dashboard search is moving to an all-stream default.
- **Dashboard search UX cleanup (shared across lexical and semantic)** in `apps/web/src/app/dashboard/search/page.tsx`:
  - Remove `looksLikeMessagesStream()` and `discoverMessagesLikeStreamNames()` entirely. The "messages-like" heuristic is weak (hard-coded name-matching that breaks for every new connector schema) and not worth preserving.
  - Remove the `scope` query parameter, the `scope` form selector, and the `'messages' | 'all'` branch in `searchRecords()`. The dashboard defaults to searching every owner-visible stream that declares the relevant searchable fields (`lexical_fields` for lexical results, `semantic_fields` for semantic results).
  - Remove the hard `DEFAULT_MAX_RESULTS` one-page cap. Paginate using the public `has_more` / `next_cursor` fields already returned by `/v1/search` and `/v1/search/semantic`. Pagination is carried as a `cursor` query parameter on the dashboard page URL.
  - Keep the page a server component; pagination is URL-driven (no client-side cursor state), consistent with the rest of the dashboard.
  - This is a dashboard-scoped UX cleanup, not a protocol change. The public `/v1/search` and `/v1/search/semantic` contracts are untouched.
...
- **No dashboard-only contract beside the public extension**. The dashboard reaches both lexical and semantic retrieval via their public routes (`/v1/search` and `/v1/search/semantic`); the `rs-client.ts` helpers are the only bridge. Dashboard UX cleanup lands here (all-stream default, cursor pagination, removal of the `messages-like` heuristic) because it is cross-cutting between the two retrieval surfaces — not because it is semantic-specific.
...
- `apps/web/src/app/dashboard/lib/rs-client.ts` — add `searchRecordsSemantic(query, opts)` that proxies to `/v1/search/semantic` and preserves the full page envelope (`has_more`, `next_cursor`, `data`); shape mirrors the existing `searchRecordsLexical(query, opts)`
- `apps/web/src/app/dashboard/search/page.tsx` — remove `looksLikeMessagesStream()`, remove `discoverMessagesLikeStreamNames()`, remove the `scope` query parameter and form selector, remove the `DEFAULT_MAX_RESULTS` one-page cap; default to all owner-visible streams; paginate using `has_more` / `next_cursor` via a `cursor` URL parameter on the page
- `apps/web/src/app/dashboard/search/search-filters-form.tsx` — remove the `scope` selector and its associated state/validation (if it still exists after the above page-level edits)
