...
- **Build a pluggable `VectorIndex` interface** in `reference-implementation/server/search-semantic.js`:
  ...
  - Default backend is an **in-memory flat vector store** scoped to the reference's lifetime, suitable for tests and for small local corpora. Backing stores such as `sqlite-vec` are acceptable drop-ins but are NOT normative and NOT added as a dependency in this tranche — keeping the surface area small and avoiding a native-addon mismatch with the just-swapped `better-sqlite3`.
  - The index honors the declared-fields boundary by construction: embeddings are produced and stored only for `(stream, record_key, field)` tuples where `field ∈ stream.manifest.query.search.semantic_fields`.
...
- **No `sqlite-vec` or external vector DB as a hard dependency**. The in-memory flat backend keeps the tranche self-contained; a production deployment can swap in a real backend via the `VectorIndex` interface without changing the public contract.
...
- `reference-implementation/server/search-semantic.js` (new) — `searchRecordsSemantic()` helper, `EmbeddingBackend` interface + default deterministic-stub implementation, `VectorIndex` interface + default in-memory flat implementation, grant-gated plan builder
- `reference-implementation/server/db.js` — add `semantic_search_meta` table (additive; no change to existing tables; no triggers — maintenance in JS)
