...
`delete_connection(connection_id)` is the **connection-scoped purge**: it removes the configured connection row AND erases the collected data, history, search derivatives, blob bindings, and schedule for exactly that one `connection_id`, and clears (does not delete) the device-source-instance soft-reference to it. It does NOT erase a running collection — a connection with an in-flight run is REFUSED (the `controller_active_runs` lease is never deleted while running; the owner stops/awaits the run, then deletes). It preserves the audit spine, all sibling connections, all other devices, and all disclosure grants.
...
- **I2 — explicit erasure, not implied.** The set of erased tables is the cascade table above, enumerated, not "whatever has a matching column." Records, changes, version counters, blobs, blob bindings, search indices, attention records, and schedule are erased; the device-source-instance back-ref is cleared. The `controller_active_runs` lease is NOT in the erased set — an in-flight run is REFUSED (I7), never deleted. Anything not listed is preserved by default. A future table that gains a `connector_instance_id` column is NOT auto-erased until this contract is updated to list it.
...
- **I4 — typed idempotency.** Deleting an already-deleted / unknown `connection_id` returns a typed, non-crashing result. The chosen shape: a second delete of the same id returns `connector_instance_not_found` (404) — the row is gone, so it is indistinguishable from never-existed, and that is the honest answer. This route raises the SAME `connector_instance_not_found` code the sibling owner-agent instance-control routes (run/schedule/revoke/diagnostics) raise from `ConnectorInstanceResolutionError`, rather than a route-local `connection_not_found`, so the owner-agent surface stays consistent; both map to HTTP 404. (Contrast revoke, whose repeat is `connector_instance_inactive` 400 because the row persists.) A delete that finds the row deletes it and returns 200/204 with a deletion summary.
...
- **I8 — transactional all-or-nothing.** The durable source-of-truth cascade — records, record_changes, version_counter, blob_bindings, blobs, connector_attention_records, connector_schedules, the `device_source_instances.connector_instance_id = NULL` clear, and the `connector_instances` row delete — executes in ONE transaction per backend, keyed strictly on the one `connector_instance_id`. The records-family purge and the schedule/device/row cleanup are NOT two independently-committed transactions: the records purge runs inside the same `writeTransaction` (SQLite) / binds against the same `withPostgresTransaction` client (Postgres) as the row/schedule/device deletes. A failure ANYWHERE in that cascade — record purge OR schedule/device/row cleanup — rolls the whole unit back: either the connection and all its data are gone, or nothing changed. No half-deleted connection (row gone, records orphaned, or vice versa). The lexical/semantic search indices are a REBUILDABLE projection of `records` (the SQLite semantic path keeps a vec0 rowid sidecar a flat by-instance DELETE cannot tear down), so they are torn down per-stream AFTER the durable commit through the proven stream-scoped helpers. A search-teardown failure after the commit leaves orphaned-but-unreachable index rows (the records are gone) — a derived-cache inconsistency recoverable by reindex, NOT a data-integrity loss, and the route/audit do NOT report the committed source-of-truth delete as failed because of it.
...
- `connector_instance_not_found` (404) — unknown id, already-deleted id, or foreign-owner id (I4/I5). Existence is not leaked across owners. This is the SAME code the sibling owner-agent instance-control routes raise (consistency over a route-local `connection_not_found`); both map to HTTP 404.
...
- In ONE transaction: erase the records-family tables (records, record_changes, version_counter, blob_bindings, blobs) plus attention and schedule rows for exactly this `connector_instance_id` (cascade table), `SET connector_instance_id = NULL` on `device_source_instances` rows pointing at it, then delete the `connector_instances` row LAST (or tombstone it for the default-account class per Decision 1). The records-family purge runs inside this SAME transaction as the schedule/device/row cleanup (not a separate commit), so the whole durable cascade is atomic (I8). `controller_active_runs` is NOT erased — an in-flight run is refused, not deleted. The rebuildable search-index projection is torn down AFTER this commit.
- Refuse with `connection_run_active` if `controller_active_runs` has a row for the id (I7).
...
- **Idempotency (I4):** delete twice → first 200/summary, second 404 `connector_instance_not_found`.
...
- **Transactional (I8):** inject a failure during the record purge → assert full rollback (connection and all data still present). AND inject a failure during the schedule/device/row cleanup AFTER the record-purge code has run → assert the record purge is rolled back too (source data still present), proving the records purge and the row/schedule/device cleanup share one transaction and cannot leave a half-deleted connection.
