...
- [x] 7.5 Add a gated Docker smoke or focused integration coverage that starts two managed connector runs with distinct profile keys and proves separate dynamic n.eko surfaces are allocated or queued according to cap. (`pnpm docker:neko:dynamic-allocator-smoke` is an allocator-level gated smoke: it creates two managed dynamic surfaces with distinct dummy profile keys and proves distinct containers/ports/profile paths when the two-port range allows. `reference-implementation/test/controller-browser-surface-leases.test.js` now covers the controller-level managed-run path with fake dynamic n.eko allocation: cap 2 allocates separate surfaces for distinct profile keys; cap 1 queues the second run before allocating a second surface, then stops the incompatible ready-idle surface through idle cleanup and proves the queued run promotes into allocator ensure plus connector start after capacity frees.)
