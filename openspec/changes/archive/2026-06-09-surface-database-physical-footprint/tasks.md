...
- [x] Add a read-only physical-footprint helper to
  ...
  (`collectPhysicalFootprint()`; live-verified read-only against `pdpp_proof`.)
- [x] Gate the helper on `isPostgresStorageBackend()`; return `null` (total) and
...
- [x] Bound the relation list to a small top-N (for example 8) so the payload
  stays small and ordered largest-first. (`PHYSICAL_FOOTPRINT_TOP_RELATIONS = 8`.)

## 3. Contract delta (implementation lane)

- [x] Extend the `refDeployment` `database` block shape in the reference contract
  with optional `physical_bytes: number | null` and
  `top_relations: Array<{ name: string; bytes: number }> | null`, leaving
  `database.path` unchanged. (The `/_ref/deployment` shape is hand-maintained in
  `server/deployment-diagnostics.ts` `DeploymentDiagnosticsReport.database`, not
  generated by `@pdpp/reference-contract` — extended there with `PhysicalFootprint`.)
- [x] Build the new fields into the `database` block in
  ...
  (`normalizePhysicalFootprint(input.physicalFootprint)`; `getPhysicalFootprint`
  dep wired to `collectPhysicalFootprint` in `index.js`.)
- [x] Update the operator-ui `DeploymentDiagnostics.database` type in
  `packages/operator-ui/src/lib/ref-client.ts` (currently
  `database: { path: string }`) in lockstep with the server shape. (Also updated
  the console mirror `apps/console/src/app/dashboard/lib/ref-client.ts`.)
- [x] `pnpm --filter reference-implementation check:generated` (or repo
  equivalent) stays clean after the delta; ref-client type mirror matches.
  (No contract codegen covers `/_ref/deployment`; lockstep proven by ref
  `tsc --noEmit` 0, operator-ui `tsc` 0, and console `types:check` 0.)

## 4. Console rendering (implementation lane — depends on §3)

- [x] Extend `DatabaseSection` in
  ...
  (logical): M" comparison drawn from the existing dataset summary. (Pure
  render-model `buildStorageFootprintModel` in `lib/storage-footprint.ts`;
  console page fetches `getDatasetSummary` best-effort and passes `retainedBytes`.)
- [x] Label the relation composition "approximate"; never alias, sum, or replace
  `total_retained_bytes` with `physical_bytes`. ("Largest relations (approximate
  — does not sum to the on-disk total)"; physical and retained render as two
  separate labeled `Field`s.)
- [x] Render the SQLite / unmeasured state cleanly (a `—` with a note) when
  `physical_bytes` is `null`, with no fabricated `0`. (`—` Field + "On-disk size
  unmeasured" `Callout`.)

## 5. Tests (implementation lane)

- [x] Helper unit test against a live Postgres: `physical_bytes` is a positive
  ...
  are pure reads). (`test/physical-footprint-helper.test.js`; live-PG cases gated
  on `PDPP_TEST_POSTGRES_URL`, 3/3 green against `pdpp_proof`; read-only proven
  via `pg_stat_database` write counters.)
- [x] Deployment-diagnostics contract test: the Postgres path carries
  `physical_bytes` + `top_relations`; the SQLite path carries `null`/empty and
  still carries `database.path`. (+5 tests in `test/deployment-diagnostics.test.js`,
  incl. NaN-total → null and no-secret-leak.)
- [x] operator-ui `DatabaseSection` component test: renders the physical footprint
  + logical comparison on the Postgres shape, and the `—`/unmeasured state on the
  SQLite/`null` shape. `biome` 0-new; `tsc` 0. (Pure render-model test
  `src/lib/storage-footprint.test.ts`, 9/9; the `.test.ts` glob excludes `.tsx`,
  so the render-model is the tested unit, matching the repo's component-test pattern.)
- [x] Record any pre-existing baseline failures (via stash) before attributing a
  failure to this change. (All targeted suites green; no baseline failures
  attributed — see report.)
