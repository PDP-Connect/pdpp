...
- [x] 13.0a Eliminate the Runs "What's missing?" card as an ungoverned verdict copy fork: bind card prose to `rendered_verdict.forward_statement`, bind CTA/status to `required_actions[0] + channel`, suppress calm/no-action cards, preserve source-pressure wait behavior, and gate terminal-gap / retryable-gap / stale-manual-refresh / dead-letter-backlog / benign-healthy cases with a condition-matrix test.
- [x] 13.0b Correct the upstream synthesizer channel derivation for live false-reassurance cases: `axes.outbox:"stalled"` SHALL create a non-calm owner action and degraded resumable stale gaps SHALL create advisory `retry_gap` / `Retry now` instead of calm `wait`; gate both at the synthesizer level.
- [x] 13.0c Make stalled-outbox owner actions cause-specific: `state_read_failed` SHALL tell the owner to re-run the collector only, `dead_letter_backlog` SHALL include a recover preview and recover apply, and `stale_pending` SHALL re-run/restart the collector through the same recovery primitive; expose the cause-specific remediation payload on `required_actions[]` for the focused console recovery panel.
- [x] 13.0d Correct unknown freshness tone: an otherwise-healthy source with `freshness:"unknown"` SHALL render `Checking`/grey with a freshness annotation and SHALL NOT render as `Healthy` or `Degraded`.
- [x] 13.0e Correct unknown coverage disposition: `coverage:"unknown"` SHALL render `checking`/grey, SHALL NOT create a `retry_gap` action, and SHALL NOT say the next run fills remaining data.
- [x] 13.0f Make device-local recovery owner-readable: summary surfaces SHALL explain the host-local collector problem in plain language, technical queue terms SHALL NOT be the primary CTA or forward statement, and the focused recovery panel SHALL show the exact copyable command(s), target host, and per-command purpose.
- [x] 13.0g Make device-local recovery source-profile correct: rendered commands SHALL target the device-binding `source_instance_id`, SHALL NOT substitute the public `connection_id` / `connector_instance_id` into local outbox commands, SHALL fail closed when the source binding is ambiguous, and SHALL use `pdpp-local-collector recover --source-instance-id <id>` so the collector CLI resolves the enrolled local profile/queue before touching the outbox.
- [x] 13.1 Owner-only live verification + Codex RI owner review: confirm against live `pdpp.vivid.fish` that the dashboard renders the three journeys correctly (no `2532` on the dashboard, Amazon/Chase advisories, ChatGPT calm), that the self-heal loop lands on the existing connection, and that Risk 1's refresh-evidence wiring holds end-to-end; record any residual as a named risk rather than leaving the change pseudo-active.
  - [x] 13.1a Live verification performed on 2026-06-17: ChatGPT rendered `Healthy` / `calm` / `Fresh today`, owner dashboard/sources/runs pages did not expose `2532`, Amazon stale manual-refresh evidence reached the rendered verdict, and peregrine Claude Code moved out of owner attention after source-profile recovery.
  - [x] 13.1b Named residuals recorded in `design-notes/live-verification-closeout-2026-06-17.md`: current live Amazon/Chase evidence no longer matches the original advisory archetype examples, self-heal remains test-proven rather than live-proven, and local collector recovery still has a background-drain UX residual.
