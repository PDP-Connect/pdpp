...
- Owner-only live verification deferred (was task 5.1). The control's contract is proven deterministically on `main`: the client wrapper classifier (`cancel-run-result.test.ts`, 8 tests), the structural component/page-gating test (`cancel-run-control.test.ts`, 10 tests), and the already-shipped owner-session route `POST /_ref/runs/{run_id}/cancel` (`reference-implementation/server/routes/run-cancel.ts`) that returns `202` / `404 no_active_run` / `409 run_already_terminal`. The one remaining check is operator-visible end-to-end behaviour an owner must run on a live instance with an active run: open the run detail page, confirm the Cancel control appears only while active, click → confirm → observe the run terminal as `run.cancelled` on the timeline and the control disappear (Playwright screenshots before/after). This is an owner-only live action; per `AGENTS.md` it is recorded here as a residual risk rather than holding the change active on a step only the owner can perform. The verification commitment is preserved: the owner should run this once on a deployed console before relying on the control operationally.
