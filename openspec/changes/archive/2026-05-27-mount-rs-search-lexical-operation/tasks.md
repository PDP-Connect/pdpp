...
- [x] 2.3 Add sandbox fixture dependencies in `apps/web/src/app/sandbox/_demo/operations-fixtures.ts` (`createSandboxSearchLexicalDependencies`) backed by deterministic substring matching over `DEMO_RECORDS` and an in-memory snapshot cache. The factory compiles request `filter[...]` payloads against the demo stream's declared `fields` and evaluates them per record: exact filters on top-level scalar demo fields are honored; range filters reject because the sandbox manifest declares no `query.range_filters`; unknown fields reject. Plan-time rejections surface as `SearchLexicalRequestError(invalid_request)`, which the sandbox route maps to the canonical `invalid_request` envelope.
...
- [x] 3.3 Add sandbox route regression tests proving the fixture honors filters: exact filter with a matching value narrows hits to that record; exact filter with a non-matching value returns an empty list; unknown field returns `invalid_request`; range filter (`filter[field][op]=value`) returns `invalid_request` because the sandbox manifest declares no `query.range_filters`; `filter[...]` without exactly one `streams[]` still returns the operation-owned `invalid_request`.
...
- `/sandbox/v1/search` returns the existing `LiveSearchResponse` envelope (`object: 'list'`, `url: '/sandbox/v1/search'`, `has_more`, `data: search_result[]`) for valid populated queries, and emits the same bm25-shaped score field when scoring is advertised. Empty/missing `q` returns the canonical `invalid_request` envelope (a deliberate, owner-approved drift from the prior `routes.test.ts` empty-`q` assertion). `filter[...]` is evaluated by the fixture: matching exact filters narrow the result set, non-matching exact filters yield an empty list, unknown fields and unsupported range filters reject with `invalid_request`.
