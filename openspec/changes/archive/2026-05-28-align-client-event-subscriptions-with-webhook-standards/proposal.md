...
- Set the CloudEvents envelope `specversion` to `"1.0"` (CloudEvents 1.0 conformant). Carry the PDPP profile version in the `pdppversion` CloudEvents extension attribute. Deliver as CloudEvents JSON **structured mode**: the HTTP `content-type` is `application/cloudevents+json; charset=utf-8` and the entire envelope is the JSON body.
- Use only CloudEvents-conformant context attributes at the envelope's top level (lowercase alphanumeric, ≤ 20 chars). The occurrence time SHALL be the standard `time` attribute (not `occurred_at`). The subscription identifier SHALL live in `data.subscription_id` (not top-level `subscription_id`, which would violate CloudEvents §context-attribute-naming with an underscore). Routing-by-subscription remains available through the standard `source` URL.
- Replace `PDPP-Event-Timestamp` / `PDPP-Event-Id` / `PDPP-Subscription-Id` / `PDPP-Event-Signature` with Standard Webhooks headers `webhook-id`, `webhook-timestamp`, `webhook-signature`. Drop the `PDPP-Subscription-Id` header entirely — the subscription id already appears in `data.subscription_id` and is recoverable from the `source` URL.
- Replace the signing construction `{ts}.{body}` with the Standard Webhooks construction `{webhook-id}.{webhook-timestamp}.{body}`. Encode the signature as `v1,<base64>` (Standard Webhooks v1 scheme) instead of `sha256=<hex>`. The signed `{body}` SHALL be the exact bytes of the structured-mode CloudEvents JSON body.
