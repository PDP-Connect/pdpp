...
- **Owner-only live render check (was task 3.2) — satisfied live.** The deployed-console verification that a Chase `transactions` detail page renders the related-account back-link is owner-gated (it needs an instance with Chase data). It is recorded here as a residual risk rather than an open task per `AGENTS.md`, and it is already satisfied by the live records/connections proof (`tmp/workstreams/ri-records-connections-live-proof-v2-report.md`, Warning 4): on deployed revision `9dc62b5868fc`, a Chase transaction carrying `account_id=1212486749` resolves to its parent `accounts/1212486749` detail at HTTP 200. The contract proof remains the unit tests in `relationships.test.ts`; the live check only confirms operator-visible rendering, which is bundled-manifest-derived and deploy-revision-independent.
- **Dead-link tolerance.** A child-declared `has_one` whose `foreign_key` value has no matching parent record (e.g. the parent stream is ungranted or the account was not collected) lands on the parent-detail `notFound()` page. This is the same calm dead-link behavior the companion change accepts for its symmetric links; no crash, no error toast.
- **Single-scalar `foreign_key` assumption.** The rule serves a single scalar `foreign_key` per `has_one`. A connector declaring a composite or array foreign key is a follow-up rule extension, not a regression here.
