...
**IMPLEMENTATION ORDER NOTE.** The lanes ultimately split by **copy-then-trim** before extracting the shared UI package: `apps/web` was copied to `apps/console` and `apps/site`, each app was trimmed to its surface, and `apps/web` was deleted. That temporarily duplicated shared dashboard/sandbox components across the two apps. The duplication was later resolved by `ri-split-operator-ui-extraction-v1`: the shared feature components, explore helpers, pure dashboard libs, and `DashboardDataSource` seam now live in `packages/operator-ui` and are consumed by both `apps/site` and `apps/console`. The final artifact state therefore satisfies the normative shared-package requirements even though the commit order differed from the preferred migration order above.
...
- **Shared components must not regress sandbox vs dashboard.** Mitigation: keep shared feature components in `packages/operator-ui`, keep live and mock data-source implementations app-local, and verify both deployables after any shared UI change. The implementation briefly used copy-then-trim before package extraction; that state is resolved and is not the accepted architecture.
