...
- [x] 12.5 Define and enforce browser-owner modes: n.eko-owned Chromium for the SLVP, browser-owner/Patchright-compatible browser posture for stealth-sensitive runs, and stealth-gated page-level helpers (`strict`, `balanced`, `assistive`).
...
- [x] 13.5 Re-run affected checks, rebuild/recreate the n.eko Docker overlay, and run public desktop plus real-phone smoke with debug telemetry. Affected no-human checks pass via `pnpm stream:no-human-verify` (matrix-transform coordinate/letterbox coverage lives in remote-surface units). Re-confirmed green 2026-06-01. Docker rebuild sub-gate CLOSED (see 12.8 — patchright-chromium build hang fixed; `PDPP_DOCKER_DYNAMIC_NEKO_ALLOCATOR_SMOKE=1 pnpm stream:no-human-verify` rebuilds/recreates the overlay green). Remaining public desktop + real-phone smoke with debug telemetry is recorded as residual owner-only verification (see design.md "Residual Owner-Only Verification") per the OpenSpec closeout rule.
...
- [x] 14.7 Run affected unit checks, OpenSpec validation, Docker n.eko rebuild/recreate, and public desktop plus real-phone smoke with debug telemetry. Affected unit checks + OpenSpec validation pass via `pnpm stream:no-human-verify` (protocol validators, viewport/keyboard classification, media-settle, and replay-harness coverage all green in remote-surface units). Re-confirmed green 2026-06-01. Docker rebuild sub-gate CLOSED (see 12.8 — patchright-chromium build hang fixed; `PDPP_DOCKER_DYNAMIC_NEKO_ALLOCATOR_SMOKE=1 pnpm stream:no-human-verify` rebuilds/recreates the overlay green). Remaining public desktop + real-phone smoke with debug telemetry is recorded as residual owner-only verification (see design.md "Residual Owner-Only Verification") per the OpenSpec closeout rule.
...
- [x] 15.8 Re-run affected checks, OpenSpec validation, Docker n.eko rebuild/recreate, and public desktop plus real-phone smoke focused on mobile clipboard. Affected checks + OpenSpec validation re-run green via `pnpm stream:no-human-verify` (clipboard-policy fixtures for all browser/permission/Unicode cases live in remote-surface units). Public desktop and Playwright mobile-emulated smoke passed previously. Re-confirmed green 2026-06-01. Docker rebuild sub-gate CLOSED (see 12.8 — patchright-chromium build hang fixed; `PDPP_DOCKER_DYNAMIC_NEKO_ALLOCATOR_SMOKE=1 pnpm stream:no-human-verify` rebuilds/recreates the overlay green). Remaining physical real-phone mobile-clipboard smoke is recorded as residual owner-only verification (see design.md "Residual Owner-Only Verification") per the OpenSpec closeout rule; mobile-emulated Playwright is not a substitute.
...
- [x] 17.5 Re-run affected checks, rebuild/recreate the n.eko Docker overlay, and run public desktop plus real-phone smoke focused on keyboard reacquire, rotation settle, and visual sharpness. Affected checks re-run green via `pnpm stream:no-human-verify` (viewport-authority, bounded-capture, keyboard-reacquire, and CSS-vs-capture coverage in remote-surface units; live-CDP smoke proves frame/input/resize against real Chromium). Public raw-CDP desktop and mobile-emulated smokes passed previously with clean rotation/visual-quality/pointer telemetry. Re-confirmed green 2026-06-01. Docker rebuild sub-gate CLOSED (see 12.8 — patchright-chromium build hang fixed; `PDPP_DOCKER_DYNAMIC_NEKO_ALLOCATOR_SMOKE=1 pnpm stream:no-human-verify` rebuilds/recreates the overlay green). Remaining physical real-phone keyboard/rotation/sharpness smoke is recorded as residual owner-only verification (see design.md "Residual Owner-Only Verification") per the OpenSpec closeout rule.
...
## 20. Local Patchright Browser Posture

- [x] 20.1 Make local non-n.eko browser launches prefer Patchright's bundled Chromium by default, matching the managed n.eko binary family.
- [x] 20.2 Keep `PDPP_BROWSER_CHANNEL` as an explicit operator compatibility override instead of silently preferring branded Chrome.
- [x] 20.3 Update browser-posture docs/spec language and add focused launcher tests.
- [x] 20.4 Re-run focused connector checks and strict OpenSpec validation.
