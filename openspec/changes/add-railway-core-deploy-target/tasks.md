...
- [ ] 2.3 Verify restart survival for the chosen backend in the local harness
  (records and owner session persist across a container restart). Not done in
  this tranche — needs a live/Docker harness; the env check enforces the
  configuration precondition (no unmounted-default SQLite), but the actual
  restart-survival run is part of the live go/no-go (step 7) and the next slice.
...
- [x] 3.1 Wire the composed-origin smoke (`scripts/docker-smoke.sh`) as the
  ...
  path.)
- [ ] 3.2 Add a deterministic record-seed (hand-imported fixture, no connector
  ...
  stack. Not done in this tranche — heavier end-to-end script (owner-token mint +
  RS ingest + MCP client); deferred to the next slice. The go/no-go checklist
  records it as live step 6.
...
- SQLite-on-mounted-path smoke variant — boots on SQLite alone and survives a
  container restart (acceptance step 7 durability check).
