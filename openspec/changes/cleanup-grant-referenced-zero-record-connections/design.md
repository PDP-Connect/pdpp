...
Auditability and rollback: dry-run prints, for every row, either the candidate id + the informational member-display note, or the full skip reasons. `--apply` is transactional and supports `--backup-to <path>` (a `VACUUM INTO` snapshot for SQLite; for Postgres, the operator's own snapshot is required and the script refuses `--apply --backup-to` on Postgres with a precise message rather than implying it took a backup it cannot take). The revoke emits no destructive cascade and is reversible by flipping the row back to `active` from the backup.
