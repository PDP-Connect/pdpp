...
- [ ] 3.5 Implement strict validation for unsupported parameters, fields, filter operators, sort fields, and expansion targets. (Partial: records/search reject unsupported top-level params, unsupported expand relations, conflicting aliases, and unsupported filter operators with typed errors. Canonical `sort` is now honored: the sign-prefix (`sort=-field` → DESC, `sort=field` → ASC) controls direction over the advertised cursor field, and a disagreement between `sort` and legacy `order` raises `invalid_sort` instead of silently picking one. Fields other than the advertised cursor field continue to be rejected with `invalid_sort` because the reference runtime only advertises the cursor field as sortable via `/v1/schema`. Remaining work: surface every other advertised capability through `/v1/schema` and prove it is enforced (task 4.2).)
...
- [ ] 3.7 Implement graded count support (`none`, `estimated`, `exact`) backed by existing projections when possible. (Partial: `count=none` and `count=exact` are fully implemented on the SQLite reference path via a visible-row scan that mirrors `aggregateRecords`. `count=estimated` is honored by computing the exact value cheaply on the same scan; the response carries `meta.count.kind = 'exact'` and no warning, since returning a higher-fidelity grade than requested is an upgrade rather than a downgrade per the spec's "Requested count is downgraded" rule. A future `compatibility_fallback` warning is appropriate only when the runtime truly cannot honor the requested grade. Postgres-backed list reads do not yet implement graded counts.)
...
- [ ] 7.3 Add regression tests for no silent filter/sort/expand no-ops. (Partial: unsupported params, filter operators, alias conflicts, unsupported expand targets, and unsortable sort fields are covered. Canonical `sort` direction is now enforced and asserted by `public-read-connection-id-decoration.test.js`: a `sort=-received_at` request that returned ascending order would fail the test. Per-capability conformance from `/v1/schema` still requires the capability document tranche.)
