...
- **[Large single-line requests are still bounded in memory, not streamed]** → The transport keeps the hosted request limit and per-line limit, but byte slicing still operates on the buffered request body. The split-cgroup Docker oracle proves the configured 200 MiB value is only a request/line ceiling in the current Node/Fastify shape, not a 200 MiB supported container floor. Current representative facts on `node:22-bookworm-slim`: 200 MiB OOMs before readiness for a 1 KiB journey; 290 MiB has produced both OOM-before-readiness and successful 1 KiB outcomes and is not a stable bound; 384 MiB completes 1 KiB; 512 MiB completes 10 MiB and 50 MiB but OOMs during 100 MiB; 1 GiB returns exact 413 with `FST_ERR_CTP_BODY_TOO_LARGE` above the request ceiling. Production release must provision for this amplification, lower the public request ceiling, or replace the route with streaming line handling.
