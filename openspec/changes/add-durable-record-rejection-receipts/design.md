...
- **[First/latest occurrence provenance is compact]** → This tranche records `first_run_id`, `latest_run_id`, first/latest input indexes, replay count, and first/latest timestamps rather than a full occurrence table. That bounds storage while preserving the provenance needed for owner inspection and stale-after-acceptance diagnosis.
- **[Retained-size projection includes quarantine rows]** → Quarantine byte/count quota remains authoritative inside `record_rejection_quota`, owner rejection list/detail expose payload bytes, and retained-size global, connection, stream, record-family, and top-row projections now include rejection payload byte/count measures. Exact replay must not increment those measures.
- **[Pending receipts can become stale after later acceptance]** → This tranche marks a pending receipt `stale_after_acceptance` when a later accepted record matches the rejected payload identity. Retry, discard, replacement, and explicit owner resolution mutations remain deferred to the owner-disposition change after shared transaction/outbox integration.
- **[Large single-line requests are still bounded in memory, not streamed]** → The transport keeps the hosted request limit and per-line limit, but byte slicing still operates on the buffered request body. The configured 50 MiB value is a buffered hosted request/line ceiling, not a universal container memory bound. Current representative facts on `node:22-bookworm-slim`: with server memory and swap capped at 512 MiB, the split-cgroup Docker oracle completes an exact 50 MiB invalid-UTF-8 request body and returns exact 413 with `FST_ERR_CTP_BODY_TOO_LARGE` for a 50 MiB + 1 byte body. Larger buffered payload support requires materially higher memory evidence or a streaming follow-up.
- **[#108 backup inventory must stay current after schema changes]** → The byte-native `record_rejections.payload`, `record_rejection_quota.pending_receipt_count`, first/latest provenance, stale-after-acceptance status, retained-size projections, 50 MiB hosted-ingest schema base, and schema-derived durable table set were refreshed on final #123 base `92bc081d9d9550cf25b1d4890e8089eaeccc67f0`. The release evidence now includes source inventory coverage plus real SQLite backup/restore and disposable PostgreSQL 16 dump/restore checks. Future schema changes must rerun the same inventory guard before landing.
