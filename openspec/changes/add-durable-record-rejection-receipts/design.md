...
The canonical ingest operation already owns an ordered array of exact non-empty NDJSON line strings before it parses valid records and passes only objects to the host batch optimization. It must preserve `{inputIndex, rawLine, parsedRecord?}` until every line has a terminal outcome. `inputIndex` is zero-based in that non-empty-line sequence; blank lines do not consume indexes.

For each permanent line failure, the server retains that exact bounded line, its SHA-256 digest, typed reason, admitted binding, and pending metadata. This includes pre-storage JSON parse failure with explicit reason `malformed_ndjson` and the storage classifier's narrow permanent allowlist. A digest, counter, or timeline event alone cannot reconstruct a source item after its cursor moves.
...
`storage owner namespace + connector_instance_id + stream + SHA-256(exact input bytes) + rejection_generation`

`rejection_generation` is a versioned server constant tied to the classifier and retained-input format. It prevents a future classifier migration from accidentally treating an old receipt as proof under incompatible semantics. The receipt also records `connector_id`, `run_id` when supplied, first and latest input indexes, first/last seen timestamps, and a bounded replay count, but those fields do not change replay identity.
...
The table tracks pending payload bytes. A configured per-owner or deployment fallback limit is checked inside the fenced quarantine transaction with the backend serialization described in D4. If a new pending payload cannot be admitted, the ingest request fails retryably and the runtime cannot stage later state. Exact receipt replay does not consume quota again.

Pending payloads do not expire automatically because source replay may be impossible after cursor commit. Connection deletion removes all associated quarantine rows and payloads inside the existing source-of-truth deletion transaction, or through an `ON DELETE CASCADE` proven active on both backends. The migration defines a conservative default quota and exposes deployment configuration without assuming this development host's capacity.
...
- **[Byte-derived replay permits semantically equivalent duplicates]** → Accept this bounded v1 trade-off; exact replay is deterministic and avoids a cross-language canonicalizer. Measure duplicates before adding canonical JSON.
...
1. Add the SQLite and PostgreSQL quarantine table, indexes, byte accounting, and connection-delete cascade. Existing rows require no backfill because past rejected payloads are not reconstructable.
