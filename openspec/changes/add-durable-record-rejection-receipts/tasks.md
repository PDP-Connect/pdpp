...
- [x] 3.1 Preserve `{inputIndex, rawLine, parsedRecord?}` through `executeRecordsIngest` and its sequential per-record ingest path so every terminal outcome maps to the zero-based non-empty NDJSON line sequence without reconstructing bytes from objects.
