...
- [ ] 1.1 Commit the real SQLite regression oracle that proves a balanced `2xx` permanent rejection cannot advance state without a durable receipt, using the current invalid-record-identity case.
...
- [ ] 4.1 Extend the hosted ingest response reader to validate attempted counts and the full rejection vector before the batch can clear.
