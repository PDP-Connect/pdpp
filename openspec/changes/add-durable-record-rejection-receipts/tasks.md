...
- [ ] 7.7 Release gate: add first/latest run provenance or bounded occurrence facts, include quarantine payloads in retained-size accounting, resolve stale-pending semantics after later acceptance, decide whether to provision, lower, or stream the 200 MiB single-line request ceiling, and rerun the #108 schema-derived backup inventory after the final schema shape. The split-cgroup Docker oracle now records representative facts without closing the release gate: with `node:22-bookworm-slim`, 200 MiB OOMs before readiness for a 1 KiB journey; 290 MiB is variable and has also completed 1 KiB; 384 MiB completes 1 KiB; 512 MiB completes 10 MiB and 50 MiB but OOMs during 100 MiB; 1 GiB returns exact 413 with `FST_ERR_CTP_BODY_TOO_LARGE` above the request ceiling.
