...
- [x] 3.3 Update the dashboard deployment-token surface so the smooth owner-agent path is prominent and the bearer-copy/debug path is clearly secondary.
...
- [x] 5.5 Define and pin the SLVP agent-readable onboarding entrypoint for non-CLI local agents. A trusted agent starting from `https://pdpp.vivid.fish` must be able to discover the owner-agent flow from advertised metadata without relying on one universal URL guess: OAuth protected-resource metadata remains canonical, `/llms.txt` and any supported `.well-known` agent-discovery aliases point to the canonical owner-agent/MCP/REST guidance, and unsupported agent-doc paths fail with a concise machine-readable response rather than a large HTML 404. Acceptance evidence should include a Simon/Daisy-equivalent smoke that completes metadata discovery without route guessing.
  - Implemented: `/llms.txt` (served by `apps/web` and `apps/site` via `agentSkillsLLMSIndex()`) now appends a `Trusted owner-agent onboarding` section pointing at the canonical `/.well-known/oauth-protected-resource` metadata block, the fetchable `/.well-known/skills/pdpp-owner-agent/SKILL.md` skill, the grant-scoped `/mcp` boundary, the owner REST `/v1/**` + `pdpp owner-agent onboard` CLI guidance, and an explicit "Do not paste tokens" rule. `/.well-known/llms.txt` rewrites to `/llms.txt` in both apps (matching the existing `.well-known/skills` alias). No broad catch-all was added; unknown agent-doc paths keep the existing per-route 404 behavior (the skills namespace already returns JSON 404s). Covered by the extended `catalog.test.ts` and `.well-known/skills` route tests in both apps. The CLI-driven Daisy-equivalent metadata-discovery smoke remains `packages/cli/test/owner-agent-reference-smoke.test.js` (see Acceptance Closeout in `design.md`).
