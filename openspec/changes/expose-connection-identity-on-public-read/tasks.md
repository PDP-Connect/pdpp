...
- [ ] Reconcile ChatGPT MCP registration parity with the current in-repo server contract. A live ChatGPT registration must expose the same canonical read-tool inputs the backend requires, especially `connection_id` on `query_records` after `ambiguous_connection`, and it must not leave agents blocked by stale callable schemas such as a legacy `filter?: string` shape. Event-subscription tools must either be exposed with their callback requirements or be explicitly documented as unsupported for that host. Acceptance evidence should include a ChatGPT live smoke that discovers tools, triggers `ambiguous_connection`, retries successfully with `connection_id`, and records event-subscription availability.
