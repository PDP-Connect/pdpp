...
- [x] 1.4 Ensure submitted secret values, raw browser bearer URLs, QR secrets, and durable credentials are not written to timeline payloads.
...
- [x] 3.2 Represent unavailable browser-surface registration honestly in assistance state and dashboard copy.
- [x] 3.3 Keep CDP, Playwright, n.eko, WebRTC, and target-registration details inside browser binding or remote-surface layers.
- [x] 3.4 Add tests for non-browser attachments such as URL or QR assistance without requiring a browser surface.
...
- [x] 5.3 Audit USAA, Chase, Amazon, Reddit, and Gmail for owner-assistance cases and map each to the new model or explicitly defer.
...
- [x] 6.4 Record the fresh ChatGPT app-push Docker flow as residual post-deploy evidence rather than blocking archive-readiness. Owner-only step: requires live ChatGPT credentials, Docker deployment, and operator observation. Unit tests (6.2) cover the state-mapping and timeout-escalation paths; `design.md` records the residual risk and the exact live check.
- [x] 6.5 Record the browser-control manual-action Docker flow as residual post-deploy evidence rather than blocking archive-readiness. Owner-only step: requires a live browser-backed connector in Docker and operator-driven run. Unit tests (6.2) cover the attachment to stream-CTA mapping; `design.md` records the residual risk and the exact live check.
