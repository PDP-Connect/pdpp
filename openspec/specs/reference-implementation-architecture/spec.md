...
### Requirement: Remote surface package SHALL remain a standalone published dependency

`@opendatalabs/remote-surface` is an independently published package (on npm) that PDPP consumes as an ordinary versioned dependency; it is not owned, vendored, or published by the PDPP repository. It SHALL retain an architecture and package shape that external consumers can install from the published artifact, typecheck, import, and evaluate without a PDPP monorepo checkout or unpublished workspace dependencies. PDPP SHALL depend on it through a semver range in the consuming app manifests, and SHALL NOT reintroduce an in-monorepo `packages/remote-surface` copy or a `workspace:*` dependency on it.

#### Scenario: A consumer installs the package outside the monorepo

- **WHEN** an external consumer installs the packed `@opendatalabs/remote-surface` artifact in a clean project
...
#### Scenario: The package is published and consumed

- **WHEN** PDPP builds against `@opendatalabs/remote-surface`
- **THEN** it SHALL resolve the dependency from the public registry release, not from an in-repo package copy
- **AND** upgrading PDPP to a newer remote-surface release SHALL be a manifest version-range change, verified by the consuming app's typecheck, rather than a source migration inside PDPP

### Requirement: Remote surface public APIs SHALL be host-neutral

`@opendatalabs/remote-surface` SHALL expose public API names, types, documentation, and examples that describe generic remote-surface host concepts rather than PDPP reference-runtime internals.
...
- **WHEN** a host that does not implement the PDPP reference runtime integrates `@opendatalabs/remote-surface`
...
Server store and lease APIs exposed by `@opendatalabs/remote-surface` SHALL describe host-owned persistence and surface lifecycle contracts instead of binding external consumers to PDPP reference runtime storage or operator-control semantics.
...
### Requirement: PDPP SHALL depend on a compatible published remote-surface release

PDPP SHALL depend on a compatible published release of `@opendatalabs/remote-surface` through a pinned semver range and SHALL verify that its own build resolves and typechecks against that release. Publication, tarball hygiene, and release readiness of the package are owned by the remote-surface repository, not PDPP.

#### Scenario: PDPP pins a published release

- **WHEN** PDPP declares its dependency on `@opendatalabs/remote-surface`
- **THEN** the dependency SHALL be a semver range resolving to a version published on the public registry
- **AND** it SHALL NOT be a `workspace:*`, relative-path, or unpublished-sibling dependency

#### Scenario: Consumer verification runs in CI

- **WHEN** PDPP's build or typecheck runs against the pinned remote-surface release
- **THEN** the consuming apps SHALL resolve every imported public entrypoint and typecheck against the published declarations
- **AND** a failure to resolve or typecheck against the pinned release SHALL block the PDPP build

#### Scenario: PDPP upgrades to a newer remote-surface release

- **WHEN** PDPP moves to a newer `@opendatalabs/remote-surface` version
- **THEN** the change SHALL be a manifest version-range bump verified by consumer typecheck, not a source migration inside PDPP
- **AND** publication concerns (registry metadata, README, license, provenance) SHALL remain owned by the remote-surface repository
...
- **WHEN** the `@opendatalabs/remote-surface` package source is inspected
...
`@opendatalabs/remote-surface` exposes package exports organized around host-neutral remote-surface concepts. Exported APIs SHALL be organized by protocol, server broker, client viewer/controllers, backend adapters, diagnostics, leases, and test utilities rather than by PDPP route names or dashboard file structure. PDPP SHALL consume these exports as an external dependency and SHALL NOT re-add generic streaming code to reference-only modules when a package export destination exists.

#### Scenario: Package exports are introduced

- **WHEN** implementation adds full streaming architecture exports to `@opendatalabs/remote-surface`
...
Backend-neutral remote-surface streaming primitives live in the external `@opendatalabs/remote-surface` package. The package owns generic protocol shapes, session broker interfaces, client viewer interfaces, backend adapter interfaces, input/viewport/clipboard channel shapes, diagnostics schema, and allocator/session seams. The reference implementation SHALL remain the host adapter for PDPP-specific routes, run timelines, auth, persistence, and connector handoff, and SHALL consume the package rather than reimplement these primitives.
...
- **AND** it SHALL NOT be implied by extracting `@opendatalabs/remote-surface`
...
- **WHEN** event, frame, input, viewport, clipboard, target, or diagnostics parsing moves from reference or dashboard code into `@opendatalabs/remote-surface`
...
- **WHEN** viewport classification, geometry, clipboard policy, media-settle, visual-quality, keyboard, IME, or pointer policy moves into `@opendatalabs/remote-surface`
...
CI SHALL build and publish `ghcr.io/pdp-connect/pdpp/reference-browser` from the
...
`PDPP_REFERENCE_IMAGE=ghcr.io/pdp-connect/pdpp/reference-browser:<tag>`.

#### Scenario: Browser-capable image contains Patchright browsers at the standard path

- **WHEN** an operator pulls `ghcr.io/pdp-connect/pdpp/reference-browser:main`
...
- **THEN** `ghcr.io/pdp-connect/pdpp/reference-browser` SHALL receive the same
  `:main`, `:sha-*`, and version tags in the same CI run

#### Scenario: Default reference image remains browser-free

- **WHEN** an operator pulls the default `ghcr.io/pdp-connect/pdpp/reference:main`
