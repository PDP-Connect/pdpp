...
#### Scenario: Verification-code pages are structured OTP, not browser-control handoffs
- **WHEN** a connector can satisfy a verification-code page by entering a submitted code into a field
- **THEN** the connector SHALL emit an existing `INTERACTION` with kind `otp`
- **AND** the reference SHALL treat it as progress posture `blocked`, owner action `provide_value`, response obligation `response_required`, and secret sensitivity
- **AND** the reference SHALL NOT infer `manual_action` from the presence of a verification-code page alone
