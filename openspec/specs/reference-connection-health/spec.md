...
Browser-session repair capability SHALL be passed to the health projection as a connection/runtime capability derived from a browser-session connection binding or an explicit required browser runtime binding; an optional browser binding alone SHALL NOT establish it. Current remote-surface occupancy is runtime telemetry, not that capability: absent or idle surface evidence SHALL NOT convert a browser-session-required failure into stored-credential rejection. For a credential-shaped failure, durable or definitive rejection SHALL take precedence, then applicable static-secret credential absence, then browser-session repair. Conversely, a session-shaped reason without browser-session repair capability and without durable or definitive credential-rejection evidence SHALL NOT be labeled as credential rejection; nor SHALL any other non-definitive credential-shaped reason when durable evidence proves the credential active and unrejected.
