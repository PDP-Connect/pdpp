...
import {
  ...
  type RemoteSurfaceDiagnosticsEvent,
...
import {
  ...
  type MobileKeyboardFocusEvent,
  type MobileKeyboardFocusState,
  type RemoteEditableRect,
...
type KeyboardFocusPayload = KeyboardFocusMessage;
type NekoViewerHandle = ReturnType<typeof createRemoteSurfaceViewer>;
interface NekoViewerRef {
...
interface RemoteKeyboardFocusContext {
  ...
  remoteEditableRect: RemoteEditableRect | null;
  setKeyboardAffordanceVisible: (visible: boolean) => void;
...
function readRemoteEditableRectFromEventData(eventData: string): RemoteEditableRect | null {
  try {
    const payload = JSON.parse(eventData) as { element?: unknown };
    return readRemoteEditableRect(payload.element);
  } catch {
    return null;
  }
}
...
function markRemoteKeyboardFocused(
  ...
  keyboardFocusStateRef.current = focusTransition.state;
  setKeyboardAffordanceVisible(focusTransition.state.affordanceVisible);
  ...
  if (readPointerCoarse()) {
    ...
    if (focusTransition.effect === "show-affordance") {
      ...
      logDebug("neko.keyboard_focus.geometry_unavailable", {
        inputType,
        reason: "remote-focus-event-has-no-editable-geometry",
      });
    }
    return;
  }
  updateAdapterRemoteInputFocus(surfaceAdapterRef.current, viewerRef, true, logDebug);
}
...
function scheduleRemoteKeyboardBlur({
  ...
  const blurTransition = transitionMobileKeyboardFocus(keyboardFocusStateRef.current, { type: "remote-blur" });
  keyboardFocusStateRef.current = blurTransition.state;
  ...
  keyboardBlurTimeoutRef.current = setTimeout(() => {
    keyboardBlurTimeoutRef.current = null;
    updateAdapterRemoteInputFocus(surfaceAdapterRef.current, viewerRef, false, logDebug);
...
function updateAdapterRemoteInputFocus(
  ...
  if (focused) {
    const session = getMountedNekoViewerSession(viewerRef.current, adapter);
    if (!session) {
      logDebug("neko.keyboard_focus.viewer_unavailable", { state: viewerRef.current?.getLifecycleState() ?? null });
      return;
    }
...
function isNekoKeyboardProxyFocused(container: Element | null): boolean {
  const textarea = container?.querySelector<HTMLTextAreaElement>('[data-pdpp-soft-keyboard="neko"]');
  return Boolean(textarea && document.activeElement === textarea);
}

function focusNekoKeyboardProxy(
  viewer: NekoViewerHandle | null,
  adapter: NekoSurfaceAdapter | null,
  container: Element | null
): boolean {
  const session = getMountedNekoViewerSession(viewer, adapter);
  if (!session) {
    return false;
  }
  try {
    session.focusKeyboard();
    return isNekoKeyboardProxyFocused(container);
  } catch {
    return false;
  }
}

function mobileKeyboardFocusEventForPointer({
  atMs,
  pointerId,
  remotePoint,
  type,
  ...
  atMs: number;
  pointerId: number;
  remotePoint: { x: number; y: number } | null;
  type: "pointercancel" | "pointerdown" | "pointermove" | "pointerup";
}): MobileKeyboardFocusEvent {
  if (type === "pointercancel" || !remotePoint) {
    return { atMs, pointerId, type: "pointercancel" };
  }
  return { atMs, pointerId, remotePoint, type };
}

function invalidateMobileKeyboardEditableRectCache(
  keyboardFocusStateRef: { current: MobileKeyboardFocusState },
  setKeyboardAffordanceVisible: (visible: boolean) => void,
  reason: "geometry-epoch" | "navigation" | "remount"
): void {
  const transition = transitionMobileKeyboardFocus(keyboardFocusStateRef.current, {
    reason,
    type: "editable-rect-cache-invalidated",
  });
...
function pointerDebugPayload({
  ...
  const effectiveViewport =
    ...
    mediaIntrinsic.height > 0
      ...
      : viewportInfo;
  const mapped = mapPointerToStreamViewport({ containerBox, event, imageBox, viewport: effectiveViewport });
  return {
...
function readVirtualKeyboardSample(): VirtualKeyboardSample | null {
...
function StreamStage({
  ...
  const [mountedCdpSurface, setMountedCdpSurface] = useState<CdpClientSurface | null>(null);
  ...
  const presentationAttachmentReadyRef = useRef(false);
  // n.eko's client-config path is stable across browser sessions. Keep the
  // server-provided identity separately so a replacement session cannot reuse
  // the previous session's settled presentation geometry.
  ...
  const nekoViewerRef = useRef<NekoViewerHandle | null>(null);
  ...
  // The SOLE call site for a neko backend/session transition's viewport
  // measurement. See stream-surface-measure-gate.ts: a measurement requested
  // right after `backend_ready` can race the surface swap, reading the
  // OUTGOING surface's container instead of the incoming one — the
  // coordinator defers until the incoming surface's own container attaches
  // (keyed by browserSessionId), or measures immediately when the request is
  // a same-session reconnect echo. `attachStreamHandlers`'s `backend_ready`
  // listener and `setStreamSurfaceNode` are the ONLY two call sites that may
  // reach `requestViewportMeasureRef` for the neko path, and both go through
  // this coordinator — neither may call `requestViewportMeasureRef` directly.
  const nekoSurfaceMeasureCoordinatorRef = useRef(
    createStreamSurfaceMeasureCoordinator((source) => requestViewportMeasureRef.current?.(source))
  );
  // The `browserSessionId` (or "cdp" when no neko session is active) of the
  // surface `<NekoSurface key={...}>`/`<BrowserSurface>` is CURRENTLY
  // rendering. Kept in sync at render time (below, alongside `nekoSession`)
  // so the ref callback — which cannot read component state directly — can
  // tag every attach with the key of the surface that produced it.
  const currentSurfaceKeyRef = useRef<string>("cdp");
  const nekoNativeViewportRef = useRef(false);
  const setStreamSurfaceNode = useCallback((node: HTMLDivElement | null) => {
    containerRef.current = node;
    setContainerNode(node);
    nekoSurfaceMeasureCoordinatorRef.current.attachSurface(node, currentSurfaceKeyRef.current);
  }, []);
  const requestNekoSurfaceMeasure = useCallback((browserSessionId: string) => {
    nekoSurfaceMeasureCoordinatorRef.current.requestBackendReady(browserSessionId);
  }, []);
  ...
  const resetPresentationForBrowserSession = useCallback(() => {
    ...
    presentationOrientationHoldUntilRef.current = 0;
  ...
  const attachStreamHandlers = useCallback(
    (source: EventSource, callbacks: { onAttached: () => void; onTransportError: () => void }) => {
      source.addEventListener("attached", (ev) => {
        ...
        const presentationSession = beginPresentationSession(
          {
            browserSessionId: browserSessionIdRef.current,
            localSurfaceViewport: localSurfaceViewportInfoRef.current,
            presentationViewport: presentationViewportInfoRef.current,
            stablePresentationViewport: stablePresentationViewportInfoRef.current,
          },
          payload.browser_session_id
        );
        browserSessionIdRef.current = presentationSession.state.browserSessionId;
        if (presentationSession.reset) {
          resetPresentationForBrowserSession();
          logDebug("stream_session.presentation_reset", {
      ...
      source.addEventListener("url_changed", (ev) => {
        ...
        invalidateMobileKeyboardEditableRectCache(keyboardFocusStateRef, setKeyboardAffordanceVisible, "navigation");
      ...
      source.addEventListener("keyboard_focus", (ev) => {
        const eventData = streamEventData(ev);
    ...
    [
      ...
      logDebug,
      onStatus,
      requestNekoSurfaceMeasure,
      resetPresentationForBrowserSession,
      setCanonicalViewportInfo,
    ]
  );
  ...
  const prepareViewportPost = useCallback(
    ({
      previous,
      viewport,
      context: { force = false, measured = { height: viewport.height, width: viewport.width }, source = "unknown" },
    }: {
      previous: ViewportPayload | null;
      viewport: ViewportPayload;
      context: { force?: boolean; measured?: { height: number; width: number }; source?: string };
    }) => {
      const observedNode = containerRef.current;
      const url = viewportUrlRef.current;
      if (!url) {
        logDebug("viewport.skip.no-url", {
          measured: { height: Math.round(measured.height), width: Math.round(measured.width) },
      ...
      if (!presentationAttachmentReadyRef.current) {
        logDebug("viewport.skip.awaiting-presentation-attachment", {
          measured: { height: Math.round(measured.height), width: Math.round(measured.width) },
          snapshot: readViewportDebugSnapshot(observedNode),
          source,
        });
      ...
      return {
        onEquivalent: () => {
          logViewportDecision("skip", "equivalent-viewport");
    ...
    [logDebug, scheduleViewportHoldFollowUp, setCanonicalViewportInfo]
  );

  const viewportWriters = useMemo(
    () =>
      createViewportWriters({
        lastPostState: lastPostedViewportRef,
        onMissingViewport: (width, height, { source = "unknown" }) => {
          logDebug("viewport.skip.no-viewport", {
            measured: { height: Math.round(height), width: Math.round(width) },
            snapshot: readViewportDebugSnapshot(containerRef.current),
            source,
          });
        },
        prepareTransport: prepareViewportPost,
        readViewport: readStageViewport,
      }),
    [logDebug, prepareViewportPost, readStageViewport]
  );
  ...
  const postViewport = useCallback(
    (
      ...
      viewportWriters.postViewport(width, height, {
        force,
  ...
  const applyRemoteSurfaceViewport = useCallback(
    (viewport: ViewportPayload) => {
      viewportWriters.applyViewport(viewport, { source: "remote-surface.viewport-match" });
    },
    [viewportWriters]
  );
  ...
  }, [clipboardPolicy, logDebug, mountedCdpSurface, nekoSession, readyBackend, remoteClipboard]);
  ...
  const handleKeyboard = useCallback(() => {
    if (readyBackend === "cdp") {
      ...
      const focused = focusMountedCdpTextInputInTrustedEvent(mountedCdpSurface);
      logDebug("cdp.corner.keyboard", {
        adapterMounted: mountedCdpSurface?.getLifecycleState() === "mounted",
        adapterPresent: !!mountedCdpSurface,
        adapterState: mountedCdpSurface?.getLifecycleState() ?? null,
        focused,
    ...
    if (affordanceTap) {
      const activation = activateMobileKeyboardAffordance(keyboardFocusStateRef.current, {
        focusTextInput: () => {
          getMountedNekoViewerSession(viewer, adapter)?.focusKeyboard();
        },
        isTextInputFocused: () => isNekoKeyboardProxyFocused(containerRef.current),
      });
      ...
      logDebug(
        activation.focused ? "neko.keyboard_focus.affordance_tap" : "neko.keyboard_focus.affordance_tap_failed",
        {
          controllerTextareaFocused: activation.focused,
  ...
  }, [logDebug, mountedCdpSurface, nekoSession, readyBackend]);
    ...
    <div className="relative flex h-full w-full flex-col bg-black" data-pdpp-stream-debug={debugEnabled}>
      {nekoSession ? (
        <NekoSurface
          ...
          key={nekoSession.browserSessionId}
          keyboardFocusStateRef={keyboardFocusStateRef}
          ...
          session={nekoSession}
          setKeyboardAffordanceVisible={setKeyboardAffordanceVisible}
          status={status}
        ...
        <BrowserSurface
          ...
          logDebug={logDebug}
          onAdapterMounted={setMountedCdpSurface}
      ...
      <CornerControls
        ...
        onCopy={
          ...
          (readyBackend === "cdp" && mountedCdpSurface && cdpMobileControls.showCopy)
            ? handleMobileCopy
            : undefined
        }
        onKeyboard={
          (readyBackend === "neko" && clipboardPolicy.showKeyboardButton) ||
          (readyBackend === "cdp" && mountedCdpSurface && cdpMobileControls.showKeyboard)
            ? handleKeyboard
            : undefined
        }
        onPaste={
          (readyBackend === "neko" && clipboardPolicy.showMobilePasteButton) ||
          (readyBackend === "cdp" && mountedCdpSurface && cdpMobileControls.showPaste)
            ? handleMobilePaste
            : undefined
        }
        status={status}
      ...
      {readyBackend !== "unknown" && (nekoSession || mountedCdpSurface) && clipboardPolicy.showClipboardSheet ? (
        <ClipboardSheet
          capabilities={clipboardCapabilities}
          connectorName={connectorName}
          getSurface={() => (readyBackend === "neko" ? nekoSurfaceAdapterRef.current : mountedCdpSurface)}
          getViewerSession={() =>
...
function NekoSurface({
  ...
  applyViewport: (viewport: ViewportPayload) => void;
  debugEnabled: boolean;
  keyboardFocusStateRef: { current: MobileKeyboardFocusState };
  ...
  const containerRef = useRef<HTMLDivElement | null>(null);
  const activeViewerPressesRef = useRef(createActiveViewerPresses());
  ...
  const handleViewerDiagnostic = useCallback(
    (event: RemoteSurfaceDiagnosticsEvent) => {
      logDebug("remote_surface_viewer.diagnostic", {
        error: event.payload && "error" in event.payload ? (event.payload.error ?? null) : null,
        type: event.type,
      });
      if (viewerRef.current?.getViewportState() === "error") {
        setError("The secure browser viewport could not be applied.");
      }
    },
    [logDebug, viewerRef]
  );
  ...
  useEffect(() => {
    if (localSurfaceViewportInfo || viewportInfo) {
      invalidateMobileKeyboardEditableRectCache(keyboardFocusStateRef, setKeyboardAffordanceVisible, "geometry-epoch");
    }
  }, [keyboardFocusStateRef, localSurfaceViewportInfo, setKeyboardAffordanceVisible, viewportInfo]);
  ...
  // NekoPointerController (inside the adapter). StreamStage owns that
  // injected adapter; the viewer session owns mounted keyboard focus and
  // browser-selection clipboard mechanism calls.
  const nekoSurfaceAdapterRef = adapterRef;
  // Mobile soft-keyboard hidden textarea. MobileTextInputController
  // (inside NekoSurfaceAdapter) binds to it lazily when the viewer session
  // focuses the keyboard. The textarea is visually-hidden + focusable so the
  // OS soft keyboard opens when the session focuses it; the controller
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    let adapter: NekoSurfaceAdapter | null = null;
    let viewer: ReturnType<typeof createRemoteSurfaceViewer> | null = null;
    invalidateMobileKeyboardEditableRectCache(keyboardFocusStateRef, setKeyboardAffordanceVisible, "remount");
    setClientConfig(null);
    setError(null);
    setMediaReady(false);

    const releaseCancelledViewer = async (
      nextAdapter: NekoSurfaceAdapter,
      nextViewer: ReturnType<typeof createRemoteSurfaceViewer>
    ) => {
      nextViewer.unmount();
      ...
      if (nekoSurfaceAdapterRef.current === nextAdapter) {
      ...
      .then(async (config) => {
        ...
        const client = createNekoClientApi({
          dispatchInput: (intent) => {
            viewerRef.current?.dispatchInput(intent);
          },
          getTextarea: () => softKeyboardTextareaRef.current,
        });
        const nextAdapter = new NekoSurfaceAdapter({
          client,
          config: { kind: "neko", ...(config as unknown as Record<string, unknown>) },
          // Steady-state media/overlay must show the whole remote screen,
          // never crop it (live-UAT regression: n.eko media rendered cover-
          // fit and cropped by 275px against a correctly full-bleed host).
          // Unset would preserve remote-surface's cover default.
          displayFit: "contain",
        ...
        const nextViewer = createRemoteSurfaceViewer({
          ...
          // (the adapter is constructed before this call and injected here,
          // so this option is redundant with it today, not authoritative —
          // set anyway so the session's own displayFit resolution stays
    ...
    return () => {
      ...
      invalidateMobileKeyboardEditableRectCache(keyboardFocusStateRef, setKeyboardAffordanceVisible, "remount");
      setClientConfig(null);
      // The injected adapter remains console-owned: tear down viewer resources
      // first, then release the adapter after its in-flight mount settles.
      ...
      mountedViewer?.unmount();
      if (viewerRef.current === viewer) {
        viewerRef.current = null;
      }
      const mountedAdapter = adapter ?? nekoSurfaceAdapterRef.current;
      if (nekoSurfaceAdapterRef.current === mountedAdapter) {
        nekoSurfaceAdapterRef.current = null;
      }
      mountedAdapter?.unmount().catch(() => {
  ...
  useEffect(() => {
    ...
    const mappedRemotePointFor = (event: PointerEvent) => {
      const containerBox = mountNode.getBoundingClientRect();
      const media = mountNode.querySelector("video, img");
      const imageBox = media?.getBoundingClientRect();
      return mapPointerToStreamViewport({
        containerBox,
        event,
        imageBox,
        viewport: localSurfaceViewportInfo,
      });
    };
    const handleMobileKeyboardPointer = (
      event: PointerEvent,
      type: "pointercancel" | "pointerdown" | "pointermove" | "pointerup",
      pointerType: "mouse" | "touch" | "pen"
    ) => {
      if (!isCoarsePointer() || pointerType !== "touch" || !event.isPrimary) {
        return;
      }
      const remotePoint = type === "pointercancel" ? null : mappedRemotePointFor(event);
      const transition = transitionMobileKeyboardFocus(
        keyboardFocusStateRef.current,
        mobileKeyboardFocusEventForPointer({
          atMs: Date.now(),
      ...
      keyboardFocusStateRef.current = transition.state;
      setKeyboardAffordanceVisible(transition.state.affordanceVisible);
      if (transition.effect === "none") {
        logDebug("neko.keyboard_focus.trusted_touch.awaiting_remote_confirmation", {
          eventType: type,
          snapshot: readSurfaceDebugSnapshot(mountNode),
        });
        return;
      }
      if (transition.effect !== "focus-text-input") {
        return;
      }
      const focused = focusNekoKeyboardProxy(viewerRef.current, nekoSurfaceAdapterRef.current, mountNode);
      logDebug(focused ? "neko.keyboard_focus.trusted_touch" : "neko.keyboard_focus.trusted_touch_failed", {
        controllerTextareaFocused: focused,
        reason: "remote-focus-confirmed-before-touch-release",
        snapshot: readSurfaceDebugSnapshot(mountNode),
        userActivationActive: navigator.userActivation?.isActive ?? null,
      });
    };
    ...
    const dispatchPointerIntent = (
      ...
      const pointerIntent: Extract<RemoteSurfaceInputPayload, { type: "pointer" }> = {
        ...
        pointerType,
        type: "pointer",
        // The viewer router reaches the same adapter mapper, which expects
        // viewport-absolute client coordinates.
        x: event.clientX,
        y: event.clientY,
      };
      viewer.dispatchInput(pointerIntent);
      trackActiveViewerPress(activeViewerPressesRef.current, pointerIntent);
      if (type === "pointerup" && pointerType === "mouse" && event.button === 0) {
        markRemoteInputFocusedAfterMousePointerUp("pointerup");
      }
    };
    const handler = (event: PointerEvent) => {
      const input = readablePointerInput(event);
      if (!input) {
        return;
      }
      handleMobileKeyboardPointer(event, input.type, input.pointerType);
      dispatchPointerIntent(event, input.type, input.pointerType);
    };
    ...
    mountNode.addEventListener("wheel", wheelHandler as EventListener, { capture: true, passive: false });
    ...
    return () => {
      ...
      mountNode.removeEventListener("wheel", wheelHandler as EventListener, { capture: true });
  ...
  return (
    <div className="flex flex-1 items-center justify-center overflow-hidden">
      ...
      >
        ...
        {/* `focusKeyboard()` on the mounted viewer session; its injected    */}
        {/* adapter binds the controller through the n.eko client shim.      */}
...
function BrowserSurface({
  ...
  logDebug: StreamDebugLogger;
  onAdapterMounted: (adapter: CdpClientSurface | null) => void;
  ...
  const sendCdpInput = useCallback(
    // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: This host bridge intentionally keeps transport logging, paste delivery errors, and non-paste retry semantics at one boundary.
    async (payload: Record<string, unknown>) => {
      // A dropped pointer/keyboard/scroll event is non-fatal (the user
      // retries the gesture), but a dropped "paste" (also how typed IME text
      // is delivered — see CdpClientSurface.sendText/pasteText, both route
      // through Input.insertText) must fail closed: pasteText()'s caller
      // (ClipboardSheet's sendSheetTextToBrowser) reads a resolved promise as
      // delivery and clears the local text / shows "Sent to the browser".
      // Swallowing a missing URL, a fetch rejection, or a non-2xx here would
      // make that a false success.
      ...
      if (!url) {
        ...
        if (isPaste) {
          throw new Error("Cannot send paste input: no active stream input URL");
        }
        return;
      }
      ...
      try {
        ...
        if (isPaste && !response.ok) {
          throw new Error(`Paste input rejected by server: ${response.status}`);
        }
      } catch (err) {
        if (logInput) {
        ...
        /* a single dropped non-paste input is non-fatal; the user will retry */
      }
    },
  ...
  useEffect(() => {
      ...
      client: {
        ...
        clipboardSink: {
          writeText(text) {
            ...
            return writeCdpClipboardToDevice({
              onWriteFailure: onRemoteClipboardRef.current,
              policy: clipboardPolicyRef.current,
              text,
              writeText: clipboard?.writeText ? (value) => clipboard.writeText(value) : null,
            });
          },
        },
        mediaSink: {
        ...
        getClipboardPolicy: () => {
          const policy = clipboardPolicyRef.current;
          return {
            canForwardNativePasteEvent: policy.canForwardNativePasteEvent,
            canReadRemoteSelection:
        ...
        getSoftKeyboardElement: () => softKeyboardTextareaRef.current,
    ...
    let detachImeBridge: (() => void) | null = null;
    ...
    // `unmount()` races ahead of a still-pending `mount()` if fired
    // immediately in cleanup: CdpClientSurface.mount() rejects when called
    // outside "idle", and a delayed mount's `.then()` continuation can run
    // AFTER an unmount already reset lifecycleState to "idle", resurrecting
    // "mounted" and re-publishing/re-attaching listeners on a torn-down
    // surface. Chain the unmount onto the mount promise so it only runs once
    // mount has actually settled (mounted -> unmount it; failed -> nothing to
    // unmount).
    const mountPromise = adapter.mount(node);
    mountPromise
      .then(() => {
        if (!disposed && adapterRef.current === adapter && adapter.getLifecycleState() === "mounted") {
          onAdapterMounted(adapter);
          // The ref is the same focus target supplied to CdpClientSurface;
          // the bridge helper fails closed if the host has not committed it.
          detachImeBridge = attachCdpMobileTextInputBridge(softKeyboardTextareaRef.current, adapter, (err) => {
            logDebug("surface.cdp-frame.ime_bridge_dispatch_error", {
              error: err instanceof Error ? err.message : String(err),
            });
          });
        }
    ...
    return () => {
      ...
      if (adapterRef.current === adapter) {
        ...
        onAdapterMounted(null);
      }
      detachImeBridge?.();
      mountPromise
        .catch(() => {
          // The mount itself already failed; nothing mounted to tear down.
        })
        .then(unmountAdapter);
    };
  }, [containerRef, logDebug, onAdapterMounted, sendCdpInput]);
  ...
  return (
    <div className="pdpp-stream-fit-host flex flex-1 items-center justify-center overflow-hidden">
      ...
      >
        ...
        {/* Mobile soft-keyboard + IME bridge. Visually hidden but focusable:
            the OS won't open the soft keyboard without a focused text field,
            so the explicit Keyboard control focuses it on coarse pointers.
            `attachCdpMobileTextInputBridge` (mount effect below) wires
            `MobileTextInputController` onto this exact textarea — package
            version 1.5.1 (`createSoftKeyboardBridge`) uses the identical
            hidden-textarea pattern for its own mobile keyboard path. Not
            display:none / not visibility:hidden — those make focus() a
            silent no-op on iOS. */}
        <textarea
          autoCapitalize="off"
          autoComplete="off"
...
async function sendSheetTextToBrowser({
  ...
  setLocalText: (text: string) => void;
  setPasteState: (state: ClipboardPasteState) => void;
  surface: NekoSurfaceAdapter | CdpClientSurface | null;
}) {
  ...
  if (surface && surfaceState === "mounted") {
    try {
      ...
      // A CDP paste can now reject (missing input URL, fetch failure, non-2xx
      // response) instead of resolving true — see `sendCdpInput`. Treat that
      // exactly like `pasted === false` rather than letting it propagate as
      // an unhandled rejection; n.eko's `pasteText` never throws today, so
      // this branch is CDP-only and does not change n.eko behavior.
      pasteError = err;
    }
  }
  logDebug(
...
async function requestBrowserCopyFromSheet({
  ...
  policy: ClipboardPolicyDecision;
  setCopyState: (state: ClipboardCopyState) => void;
  session: NekoRemoteSurfaceSession | CdpClientSurface | null;
}) {
  const remoteToLocalAllowed =
    policy.directionPolicy === "remote-to-local" || policy.directionPolicy === "bidirectional-text";
  if (!remoteToLocalAllowed) {
  ...
  if (session) {
    surfaceState = "getLifecycleState" in session ? session.getLifecycleState() : "mounted";
  }
...
function ClipboardSheet({
  ...
  capabilities: ClipboardCapabilities;
  connectorName: string;
  getSurface: () => NekoSurfaceAdapter | CdpClientSurface | null;
  getViewerSession: () => NekoRemoteSurfaceSession | CdpClientSurface | null;
  logDebug: StreamDebugLogger;
...
function CornerControls({
  ...
  connectorName: string;
  keyboardAffordanceVisible: boolean;
      ...
      <div className="pdpp-stream-control-row" ref={rowRef}>
        ...
        {expanded && onKeyboard ? (
          <button
            aria-label={
              keyboardAffordanceVisible
                ? `Tap to type in ${connectorName} browser`
                : `Show keyboard for ${connectorName} browser`
            }
            className={
              keyboardAffordanceVisible
                ? "pdpp-stream-keyboard-affordance pdpp-stream-control-button"
          ...
          >
            {keyboardAffordanceVisible ? (
              <span className="pdpp-stream-keyboard-affordance-label">Tap to type</span>
            ) : null}
            <svg
              aria-hidden
              fill="none"
