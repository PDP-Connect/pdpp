...
// ─── Pointer-mapping lifecycle (RS1.5 FINAL review P2) ───────────────────────
//
// `latestNekoPointerMapping` is module-global state consumed by
// `getNekoControlPos`'s fallback-2 path. A stale mapping surviving a layout
// reset, a container/viewport mismatch, or a `stopNeko` teardown is a
// wrong-target click: a reconnect/rotation-window tap can land through a
// prior surface's scale instead of the new one's (72px off in the review's
// repro: session A at 1981x960 scale 1.0667 retained into session B at
// 1400x1005 scale 0.95714). `neko-client.ts`'s `applyViewportLayout`,
// `getNekoControlPos`, and `stopNeko` are not exported for direct DOM-driven
// integration testing (this module is a real-DOM/real-n.eko-instance
// singleton; apps/console's test suite has no jsdom dependency anywhere —
// see neko-media-contain-fit.test.ts and this file's own source-shape
// pattern), so these tests prove the same invariants at the level this
// codebase's test infrastructure supports: the mapping's own validity gate
// as a pure function, and — via source inspection — that every one of the
// three invalidation boundaries the review named actually calls the
// clearing helper, and that the write path is the sole assignment site.

const LATEST_POINTER_MAPPING_FIELD_RE = /let latestNekoPointerMapping: NekoMediaPointerMappingGeometry \| null = null;/;
const SET_POINTER_MAPPING_FN_RE =
  /function setLatestNekoPointerMapping\(mapping: NekoMediaPointerMappingGeometry\): void \{/;
const CLEAR_POINTER_MAPPING_FN_RE = /function clearLatestNekoPointerMapping\(\): void \{/;
const IS_FINITE_SCALE_FN_RE =
  /function isFiniteScale\(value: number\): boolean \{\s*return Number\.isFinite\(value\) && value > 0;/;
const SET_REJECTS_INVALID_SCALE_RE =
  /function setLatestNekoPointerMapping\([\s\S]{0,200}if \(!\(isFiniteScale\(mapping\.scaleX\) && isFiniteScale\(mapping\.scaleY\)\)\) \{\s*latestNekoPointerMapping = null;\s*return;\s*\}/;
const DIRECT_ASSIGNMENT_RE = /(?<!latestNeko)(?<![.\w])latestNekoPointerMapping\s*=\s*(?!null;)/g;
const APPLY_VIEWPORT_LAYOUT_RESET_CLEAR_RE =
  /if \(!viewportLayout\) \{[\s\S]{0,400}clearLatestNekoPointerMapping\(\);[\s\S]{0,200}return;\s*\}/;
const APPLY_VIEWPORT_LAYOUT_PENDING_CLEAR_RE =
  /if \(!viewportLayoutMatchesContainer\(viewportLayout, container\)\) \{[\s\S]{0,400}clearLatestNekoPointerMapping\(\);[\s\S]{0,120}applyPendingViewportLayout\(/;
const STOP_NEKO_CLEAR_RE =
  /export function stopNeko\(container: HTMLElement\): void \{[\s\S]{0,800}clearLatestNekoPointerMapping\(\);/;
const SET_CALL_SITE_RE = /setLatestNekoPointerMapping\(primaryPointerMapping \?\? overlaySelection\.pointerMapping\);/;
const SET_POINTER_MAPPING_FN_BODY_RE = /function setLatestNekoPointerMapping[\s\S]*?\n}\n/;
const CLEAR_POINTER_MAPPING_FN_BODY_RE = /function clearLatestNekoPointerMapping[\s\S]*?\n}\n/;

test("pointer-mapping lifecycle: the module field's only writers are setLatestNekoPointerMapping/clearLatestNekoPointerMapping", async () => {
  const { readFile } = await import("node:fs/promises");
  const { fileURLToPath } = await import("node:url");
  const here = fileURLToPath(new URL(".", import.meta.url));
  const src = await readFile(`${here}neko-client.ts`, "utf8");

  assert.match(src, LATEST_POINTER_MAPPING_FIELD_RE, "the module-global mapping field must exist as documented");
  assert.match(src, SET_POINTER_MAPPING_FN_RE, "setLatestNekoPointerMapping must exist as the sole write path");
  assert.match(
    src,
    CLEAR_POINTER_MAPPING_FN_RE,
    "clearLatestNekoPointerMapping must exist as the sole invalidation path"
  );
  assert.match(
    src,
    SET_CALL_SITE_RE,
    "applyViewportLayout's steady-state branch must assign through setLatestNekoPointerMapping, not a direct assignment"
  );

  // No OTHER assignment to the field is allowed outside the two lifecycle
  // functions themselves (whose own bodies assign `latestNekoPointerMapping
  // = null` or `= mapping`) — a stray `latestNekoPointerMapping = ...`
  // anywhere else in the file would bypass the validity gate and/or the
  // documented invalidation contract.
  const assignmentsOutsideLifecycleFns = src
    .split(SET_POINTER_MAPPING_FN_BODY_RE)
    .join("")
    .split(CLEAR_POINTER_MAPPING_FN_BODY_RE)
    .join("")
    .match(DIRECT_ASSIGNMENT_RE);
  assert.equal(
    assignmentsOutsideLifecycleFns,
    null,
    `found a direct assignment to latestNekoPointerMapping outside its lifecycle functions: ${JSON.stringify(assignmentsOutsideLifecycleFns)} — all writes must go through setLatestNekoPointerMapping/clearLatestNekoPointerMapping`
  );
});

test("pointer-mapping lifecycle: cleared at every authority-invalidation boundary (layout reset, pending/mismatch, stopNeko)", async () => {
  const { readFile } = await import("node:fs/promises");
  const { fileURLToPath } = await import("node:url");
  const here = fileURLToPath(new URL(".", import.meta.url));
  const src = await readFile(`${here}neko-client.ts`, "utf8");
  ...
  assert.match(
    src,
    APPLY_VIEWPORT_LAYOUT_RESET_CLEAR_RE,
    "applyViewportLayout's null-layout reset branch must clear the mapping before returning — no settled layout means no authoritative geometry"
  );
  assert.match(
    src,
    APPLY_VIEWPORT_LAYOUT_PENDING_CLEAR_RE,
    "applyViewportLayout's container/viewport-mismatch branch must clear the mapping before delegating to applyPendingViewportLayout — this is the exact reconnect/rotation window the review's repro used, where neither the native overlay nor screen-state authority is available either"
  );
  assert.match(
    src,
    STOP_NEKO_CLEAR_RE,
    "stopNeko must clear the mapping during teardown — a remount (same or different browser session) must never inherit the outgoing instance's scale/offset"
  );
});

test("pointer-mapping lifecycle: setLatestNekoPointerMapping rejects non-finite/non-positive scale factors before they can reach a fallback division", async () => {
  const { readFile } = await import("node:fs/promises");
  const { fileURLToPath } = await import("node:url");
  const here = fileURLToPath(new URL(".", import.meta.url));
  const src = await readFile(`${here}neko-client.ts`, "utf8");
  ...
  assert.match(
    src,
    IS_FINITE_SCALE_FN_RE,
    "isFiniteScale must require Number.isFinite AND > 0 (rejects NaN, Infinity, 0, and negatives)"
  );
  assert.match(
    src,
    SET_REJECTS_INVALID_SCALE_RE,
    "setLatestNekoPointerMapping must reject the mapping (set null, not store it) when either scale factor fails isFiniteScale — an invalid scale reaching getNekoControlPos's (clientX - rect.left) / scaleX division would produce Infinity/NaN coordinates dispatched at the remote control surface"
  );
});

/**
 * Value-level proof that the validity gate's LOGIC is correct, independent
 * of the DOM/module-singleton wiring `setLatestNekoPointerMapping` itself
 * requires: mirrors `isFiniteScale` exactly, so this is a numeric oracle for
 * what that function computes, not a re-implementation with different
 * semantics — same relationship neko-media-contain-fit.test.ts's coverSize/
 * containSize oracle bears to its source functions.
 */
function isFiniteScaleOracle(value: number): boolean {
  return Number.isFinite(value) && value > 0;
}

test("pointer-mapping lifecycle: scale validity gate accepts real scale factors and rejects every degenerate case", () => {
  // Real scale factors from the review's own repro (session A: 1981x960 at
  // scale 1.0667; session B: 1400x1005 at scale 0.95714).
  assert.equal(isFiniteScaleOracle(1.0667), true);
  assert.equal(isFiniteScaleOracle(0.957_14), true);
  assert.equal(isFiniteScaleOracle(1), true);

  // Degenerate cases a 0-size intrinsic or an upstream division-by-zero
  // could produce.
  assert.equal(isFiniteScaleOracle(0), false);
  assert.equal(isFiniteScaleOracle(-1), false);
  assert.equal(isFiniteScaleOracle(Number.NaN), false);
  assert.equal(isFiniteScaleOracle(Number.POSITIVE_INFINITY), false);
  assert.equal(isFiniteScaleOracle(Number.NEGATIVE_INFINITY), false);
});
