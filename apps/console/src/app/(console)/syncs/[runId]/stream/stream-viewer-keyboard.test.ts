...
const STREAM_DIALOG_LANDSCAPE_SCROLL_RE =
  /@media \(pointer: coarse\) and \(orientation: landscape\)[\s\S]*\.pdpp-stream-dialog[\s\S]*overflow-y:\s*auto/;
const STREAM_DIALOG_FITS_CONTAINER_WIDTH_RE =
  /\.pdpp-stream-dialog[\s\S]*inline-size:\s*100%;[\s\S]*max-inline-size:\s*100%;[\s\S]*width:\s*100%;[\s\S]*max-width:\s*100%/;
const STREAM_DIALOG_BACKDROP_CLIPS_X_RE = /\.pdpp-stream-dialog-backdrop[\s\S]*overflow:\s*clip/;
const GLOBAL_DOCUMENT_CLIP_RE = /html,\s*body[\s\S]*overflow-x:\s*clip/;
...
const CDP_SURFACE_ADAPTER_WIRING_RE =
  /new CdpClientSurface\(\{[\s\S]*cdp: createPdppCdpTransport\(sendCdpInput\)[\s\S]*mediaSink:[\s\S]*getViewportInfo: \(\) => viewportInfoRef\.current[\s\S]*getFrameElement: \(\) => imgRef\.current[\s\S]*getSoftKeyboardElement: \(\) => softKeyboardTextareaRef\.current/;
...
const DEBUG_DRAIN_GATE_RE =
  /Debug-only drain[\s\S]*?if \(!debugEnabled\) \{\s*return;\s*\}[\s\S]+?drainRemotePlaygroundEvents/;
...
// NEKO_NATIVE_VIEWPORT_OPTIONS moved to stream-viewer-geometry.ts (extracted so
// the parity test can import it under plain node --test); the viewer now only
// imports it. Assert against the extracted module for the concrete values and
// against the viewer for the import.
const NEKO_NATIVE_VIEWPORT_OPTIONS_GEOMETRY_FILE = `${HERE}stream-viewer-geometry.ts`;
const NEKO_NATIVE_VIEWPORT_OPTIONS_DEFINITION_RE =
  /export const NEKO_NATIVE_VIEWPORT_OPTIONS[\s\S]*deviceScaleFactor:\s*1[\s\S]*highDprCapture:\s*false/;
const NEKO_NATIVE_VIEWPORT_OPTIONS_RE = /NEKO_NATIVE_VIEWPORT_OPTIONS,[\s\S]*from "\.\/stream-viewer-geometry\.ts"/;
...
const NEKO_BACKEND_READY_REMEASURE_RE =
  /payload\.backend === "neko"[\s\S]*nekoNativeViewportRef\.current = true[\s\S]*requestNekoSurfaceMeasure\(browserSessionId\)/;
...
test("n.eko follow-up viewport posts use native one-to-one coordinates", async () => {
  const [viewerSrc, geometrySrc] = await Promise.all([
    readFile(STREAM_VIEWER_FILE, "utf8"),
    readFile(NEKO_NATIVE_VIEWPORT_OPTIONS_GEOMETRY_FILE, "utf8"),
  ]);

  assert.match(
    geometrySrc,
    NEKO_NATIVE_VIEWPORT_OPTIONS_DEFINITION_RE,
    "n.eko viewport options force a CSS-pixel native coordinate space"
  );
  assert.match(
    viewerSrc,
    NEKO_NATIVE_VIEWPORT_OPTIONS_RE,
    "the viewer imports NEKO_NATIVE_VIEWPORT_OPTIONS from the shared geometry module rather than re-deriving it"
  );
  assert.match(
    viewerSrc,
  ...
  assert.match(
    ...
    "n.eko backend_ready routes its remeasure through the surface-measure coordinator using native viewport options " +
      "(the coordinator's own gating behavior — reconnect measures now, identity change defers — is covered by the " +
      "production-shaped behavioral tests in stream-surface-measure-gate.test.ts, not by source matching here)"
  );
  assert.match(viewerSrc, READ_STAGE_VIEWPORT_POST_RE, "shared viewport writer uses readStageViewport");
