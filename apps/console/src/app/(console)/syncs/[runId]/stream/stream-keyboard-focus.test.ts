...
test("confirmation before pointerup focuses only when the current mapped point is inside the confirmed rect", () => {
  ...
  ({ state } = transition(state, { atMs: 20, rect: editableRect, type: "remote-focus" }));
...
test("late confirmation matches the same completed gesture and exposes a retryable affordance", () => {
  let state = createMobileKeyboardFocusState();
  ({ state } = transition(state, { atMs: 10, pointerId: 3, remotePoint: editablePoint, type: "pointerdown" }));
...
test("a warm confirmed editable rect focuses on one trusted tap", () => {
  let state = createMobileKeyboardFocusState();
  ({ state } = transition(state, { atMs: 10, rect: editableRect, type: "remote-focus" }));
  ({ state } = transition(state, { atMs: 20, pointerId: 3, remotePoint: editablePoint, type: "pointerdown" }));
...
test("a warm cache miss leaves the late-confirmation affordance behavior unchanged", () => {
  let state = createMobileKeyboardFocusState();
  ({ state } = transition(state, { atMs: 10, rect: editableRect, type: "remote-focus" }));
  ({ state } = transition(state, { atMs: 20, pointerId: 3, remotePoint: unrelatedPoint, type: "pointerdown" }));
...
test("an expired warm cache falls back to the existing late-confirmation affordance", () => {
  ...
  ({ state } = transition(state, { atMs, pointerId: 3, remotePoint: editablePoint, type: "pointerdown" }));
...
test("geometry, navigation, and remount each invalidate a warm editable cache", () => {
  for (const reason of ["geometry-epoch", "navigation", "remount"] as const) {
    let state = createMobileKeyboardFocusState();
    ({ state } = transition(state, { atMs: 10, rect: editableRect, type: "remote-focus" }));
    ({ state } = transition(state, { reason, type: "editable-rect-cache-invalidated" }));
    ({ state } = transition(state, { atMs: 20, pointerId: 3, remotePoint: editablePoint, type: "pointerdown" }));
...
test("a tap just outside a warm editable rect never summons the keyboard", () => {
  ...
  ({ state } = transition(state, { atMs: 10, rect: editableRect, type: "remote-focus" }));
  ({ state } = transition(state, { atMs: 20, pointerId: 3, remotePoint: justOutside, type: "pointerdown" }));
...
test("pointer identity, movement, cancel, expiry, and blur fail closed", () => {
  let state = createMobileKeyboardFocusState();
  ({ state } = transition(state, { atMs: 0, pointerId: 9, remotePoint: editablePoint, type: "pointerdown" }));
...
test("failed affordance focus remains retryable and success consumes it", () => {
  let state = createMobileKeyboardFocusState();
  ({ state } = transition(state, { atMs: 10, pointerId: 1, remotePoint: editablePoint, type: "pointerdown" }));
  ({ state } = transition(state, { atMs: 20, pointerId: 1, remotePoint: editablePoint, type: "pointerup" }));
  ({ state } = transition(state, { atMs: 30, rect: editableRect, type: "remote-focus" }));
...
test("remote-focus at mount then first tap after realistic delay focuses directly (warm cache covers mount→tap)", () => {
  ...
  ({ state } = transition(state, { atMs: atRealisticTap, pointerId: 1, remotePoint: editablePoint, type: "pointerdown" }));
  const released = transition(state, { atMs: atRealisticTap + 50, pointerId: 1, remotePoint: editablePoint, type: "pointerup" });
...
test("negative case: tap at expired stale geometry still requires re-confirmation", () => {
  ...
  ({ state } = transition(state, { atMs: atExpiredCache, pointerId: 2, remotePoint: editablePoint, type: "pointerdown" }));
  const released = transition(state, { atMs: atExpiredCache + 50, pointerId: 2, remotePoint: editablePoint, type: "pointerup" });

  // Expired cache falls back to awaiting-confirmation pattern, requiring a second remote-focus
  assert.equal(released.effect, "none");
  assert.equal(released.state.gesture?.phase, "awaiting-confirmation", "expired cache must re-confirm via second remote-focus");
