...
test("schedule.active_run_id wins over a live last_run on the same connection", () => {
  const model = buildSyncsViewModel({
    connectors: [
      connector({
        connection_id: "cin_heb_scheduled",
        connector_id: "heb",
        display_name: "HEB — Scheduled",
        last_run: connectorRun({ event_count: 12, run_id: "run_1784089918637", status: "succeeded" }),
        schedule: schedule({ active_run_id: "run_1784089918638" }),
        streams: ["orders"],
      }),
    ],
    runs: [
  ...
  assert.equal(byConnection.get("cin_heb_scheduled")?.activeRunId, "run_1784089918638");
  assert.equal(byConnection.get("cin_heb_scheduled")?.lastRunDelta, "+12 records");
});

test("a genuinely active last_run backfills the exact sync link when schedule is null", () => {
  const model = buildSyncsViewModel({
    connectors: [
      connector({
        connection_id: "cin_heb_live",
        connector_id: "heb",
        display_name: "HEB — Live",
        last_run: connectorRun({
          event_count: 5,
          run_id: "run_1784089918639",
          status: "pending",
        }),
        schedule: null,
        streams: ["orders"],
    ...
    runs: [
      run({
        connection_id: "cin_heb_live",
        event_count: 5,
        first_at: "2026-06-13T04:01:00Z",
        last_at: "2026-06-13T04:01:06Z",
        run_id: "run_1784089918639",
        status: "pending",
      }),
    ],
  ...
  const byConnection = new Map(model.groups.map((group) => [group.connectionId, group]));
  assert.equal(byConnection.get("cin_heb_live")?.activeRunId, "run_1784089918639");
});

test("a succeeded terminal last_run is never promoted to an active sync link", () => {
  const model = buildSyncsViewModel({
    connectors: [
      connector({
        connection_id: "cin_heb_succeeded",
        connector_id: "heb",
        display_name: "HEB — Succeeded",
        last_run: connectorRun({ event_count: 7, run_id: "run_1784089918640", status: "succeeded" }),
        schedule: null,
        streams: ["orders"],
      }),
    ],
    runs: [
      run({
        connection_id: "cin_heb_succeeded",
        event_count: 7,
        first_at: "2026-06-13T04:02:00Z",
        last_at: "2026-06-13T04:02:06Z",
        run_id: "run_1784089918640",
        status: "succeeded",
      }),
    ],
  });
  ...
  const byConnection = new Map(model.groups.map((group) => [group.connectionId, group]));
  assert.equal(byConnection.get("cin_heb_succeeded")?.activeRunId, null);
});

test("an error terminal last_run is never promoted to an active sync link", () => {
  const model = buildSyncsViewModel({
    connectors: [
      connector({
        connection_id: "cin_heb_error",
        connector_id: "heb",
        display_name: "HEB — Error",
        last_run: connectorRun({ event_count: 8, run_id: "run_1784089918641", status: "error" }),
        schedule: null,
        streams: ["orders"],
      }),
    ],
    runs: [
      run({
        connection_id: "cin_heb_error",
        event_count: 8,
        first_at: "2026-06-13T04:03:00Z",
        last_at: "2026-06-13T04:03:06Z",
        run_id: "run_1784089918641",
        status: "error",
      }),
    ],
  });
  ...
  const byConnection = new Map(model.groups.map((group) => [group.connectionId, group]));
  assert.equal(byConnection.get("cin_heb_error")?.activeRunId, null);
});

test("an unknown last_run status is never promoted to an active sync link", () => {
  const model = buildSyncsViewModel({
    connectors: [
      connector({
        connection_id: "cin_heb_unknown",
        connector_id: "heb",
        display_name: "HEB — Unknown",
        last_run: connectorRun({ event_count: 9, run_id: "run_1784089918642", status: "future_status" }),
        schedule: null,
        streams: ["orders"],
      }),
    ],
    runs: [
      run({
        connection_id: "cin_heb_unknown",
        event_count: 9,
        first_at: "2026-06-13T04:04:00Z",
        last_at: "2026-06-13T04:04:06Z",
        run_id: "run_1784089918642",
  ...
  const byConnection = new Map(model.groups.map((group) => [group.connectionId, group]));
  assert.equal(byConnection.get("cin_heb_unknown")?.activeRunId, null);
});
...
test("a draft connection produces a PendingSetupCard, not a SyncGroup or FailureCard", () => {
  const model = buildSyncsViewModel({ connectors: [draftConnector()], runs: [] });
  ...
  assert.equal(card.name, "Gmail - owner@example.com");
...
test("a draft connection counts toward needYourHand and inflates onSchedule by zero", () => {
  ...
  assert.equal(model.band.needYourHand, 1, "the draft is the only needs-you work item here");
  // The healthy connection's single stream is the only thing on schedule; the
  // draft contributes zero SyncGroup streams (it never forms a group).
  assert.equal(
    model.band.onSchedule,
    model.groups.reduce((sum, g) => sum + g.totalStreamCount, 0)
  );
});
...
test("a draft connection's needs-you count matches sourceWorkFromConnectors' needsOwner group (shared attention headline)", () => {
  ...
  assert.equal(sourceAttentionHeadline(sourceGroups).needsYou, 1, "sanity: the draft alone is the needs-you work here");
  assert.equal(model.band.needYourHand, sourceAttentionHeadline(sourceGroups).needsYou);
});

test("a revoked-and-draft connection is dropped entirely (revoked check runs before the draft branch)", () => {
  const model = buildSyncsViewModel({
    connectors: [draftConnector({ revoked_at: "2026-07-10T00:00:00Z" })],
    runs: [],
  });

  // `buildSyncsViewModel` routes to `toPendingSetupCard` on `isSetupInProgressConnector`
  // BEFORE `projectSyncProjection` (which is what checks `revoked_at`) ever
  // runs — so a revoked+draft summary (which owner-state.ts guarantees cannot
  // arise on real data; see source-actionability.test.ts) surfaces as a
  // pending-setup card rather than being dropped. Pinning the actual behavior
  // here, not an assumption, exactly like the sources-view-model coverage for
  // the same inconsistent-fixture case.
  assert.equal(model.pendingSetupCards.length, 1);
  assert.equal(model.groups.length, 0);
  assert.equal(model.failureCards.length, 0);
});
