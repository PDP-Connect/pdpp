...
export interface SyncGroup {
  /**
   * Exact active run id from durable current evidence:
...
/**
 * A draft connection that has not completed its first credential capture /
 * browser enrollment and has never ingested. It has no run history, schedule,
 * or stream evidence, so it cannot form a {@link SyncGroup} (a table of
 * per-stream sync rows) or a {@link FailureCard} (a rendered-verdict-derived
 * defect) — both assume evidence a draft does not have yet. This card is the
 * Syncs-surface half of connection discoverability: without it, a freshly
 * created connection would be invisible here even after becoming visible on
 * Sources. `continueHref` is the SAME `/connect/status/:id` target every
 * other draft surface (Sources row, source-detail redirect) resolves to. See
...
export interface PendingSetupCard {
  ...
  /** Connector key. */
  connectorId: string;
  /** The single "Continue setup" target — `/connect/status/:id`. */
  continueHref: string;
...
function buildHealthBand(input: {
  failureCards: FailureCard[];
  groups: SyncGroup[];
  pendingSetupWork: readonly SourceWorkItem[];
...
/**
 * Maps a draft connector summary to the {@link PendingSetupCard} shape. Same
 * `/connect/status/:id` target `sources-view-model.ts`'s `sourceDetailHrefFor`
 * resolves to for a draft — duplicated here (one line) rather than imported
 * cross-module, matching this file's existing pattern of per-surface href
...
function projectSyncProjection(input: {
  connector: RefConnectorSummary;
  runs: readonly RunSummary[];
}): SyncProjection | null {
  const { connector, runs } = input;
...
export function buildSyncsViewModel(input: {
  ...
  const pendingSetupCards: PendingSetupCard[] = [];
  const pendingSetupWork: SourceWorkItem[] = [];

  for (const connector of input.connectors) {
    // A draft connection has no run/schedule/coverage evidence — it cannot
    // form a SyncProjection (which reads run history to build rhythm/stream
    // rows). Route it to its own pending-setup card instead of the group/
    // failure-card pipeline below. See `PendingSetupCard`.
    if (isSetupInProgressConnector(connector)) {
      pendingSetupCards.push(toPendingSetupCard(connector));
      pendingSetupWork.push(pendingSetupWorkItem(connector));
      continue;
    }
    const projection = projectSyncProjection({ connector, runs: input.runs });
    if (projection) {
