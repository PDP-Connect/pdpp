...
const REVOKED_ADD_SOURCE_SEARCH = /\/sources\/add\?source_q=/;
...
test("records and runs segments scope their own error boundaries (report 5)", () => {
  // A run-start refresh that fails must hit a contextful sources/runs boundary,
  // not the dashboard-wide "Something went wrong".
  assert.ok(existsSync(`${HERE}error.tsx`), "sources/error.tsx must exist");
  assert.ok(existsSync(`${HERE}../syncs/error.tsx`), "syncs/error.tsx must exist");
});
