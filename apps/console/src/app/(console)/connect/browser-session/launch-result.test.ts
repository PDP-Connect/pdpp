...
const BROWSER_READY_FAILURE_RE = /could not get the secure browser ready/i;
const NO_RUN_ID_RE = /no run id/i;
const SERVICE_HEALTH_RE = /service health/i;
...
test("browser-session launch does not route a surface_failed preflight run to stream", () => {
  ...
  assert.match(result.message, BROWSER_READY_FAILURE_RE);
  assert.doesNotMatch(result.message, SERVICE_HEALTH_RE);
...
test("browser-session launch fails closed when the controller returns no run id", () => {
  ...
  assert.match(result.message, NO_RUN_ID_RE);
});
