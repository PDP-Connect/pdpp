...
/**
 * Read-resilience acceptance invariants for the schedules segment, mirroring
 ...
 * pattern enforces; this file pins the same properties for `/schedules`.
 */
...
const RETIRED_BACK_LINK_RE = /Back to schedules/;
...
const RECOVERING_TESTID_RE = /data-testid="schedules-read-recovering"/;
...
test("the boundary renders the same loading skeleton the route's loading.tsx uses, not a bespoke banner", async () => {
  ...
  // loading.tsx uses ListLoadingSkeleton label="schedules" rows={6}.
  assert.match(src, /ListLoadingSkeleton label="schedules" rows=\{6\}/);
});
