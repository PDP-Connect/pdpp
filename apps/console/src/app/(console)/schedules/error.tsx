...
/**
 * Schedules-segment error boundary (App Router convention) — SLVP bar:
 ...
 * finished flushing (e.g. `schedule-row.tsx`'s poller firing
 * `router.refresh()` while a prior refresh's stream is still in flight). It
 * is a client-transport race below the data layer, not a backend outage —
 * see `sources/error.tsx` for the full original writeup.
 *
 * `/schedules` has no client-cached last-known-read marker, so this boundary
...
export default function SchedulesError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  ...
  return (
    <div data-testid="schedules-read-recovering">
