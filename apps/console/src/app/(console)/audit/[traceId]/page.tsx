...
function traceTimelineHref(traceId: string, cursor: string): string {
  return `/audit/${encodeURIComponent(traceId)}?${new URLSearchParams({ cursor }).toString()}`;
}
    ...
    <Sheet>
      ...
      <SheetBody>
        ...
        <ul className="pdpp-caption mt-3 grid gap-1" style={{ color: "var(--color-muted-foreground)" }}>
          ...
          <li>
            Open{" "}
            <a className="underline underline-offset-2" href="/deployment" style={{ color: "var(--color-foreground)" }}>
          ...
          <KV>
            ...
            {first?.grant_id ? (
              <KVRow k="grant">
                <a
                  ...
                  href={`/grants/${encodeURIComponent(first.grant_id)}`}
            ...
            {first?.run_id ? (
              <KVRow k="run">
                <a
                  className="pdpp-typed"
                  href={`/syncs/${encodeURIComponent(first.run_id)}`}
                  style={{ color: "var(--color-foreground)" }}
                >
                  {first.run_id} →
                </a>
