...
function buildNav(routes: Routes, mode: ShellMode): NavItem[] {
  ...
  if (mode === "live") {
    nav.push({ href: routes.section.connect, label: "Connect apps", match: (a) => a === "connect" });
...
function GrantsSubnav() {
  ...
  // Note: owner-token issuance lives under /deployment/tokens, not
  // here — tokens are an operator/developer concern, not a grants concern.
  const items = [
    { href: "/grants#pending-approvals", label: "Pending approvals" },
    { href: "/grants/packages", label: "Packages" },
    { href: "/grants/request", label: "Grant request" },
  ];
...
function DeploymentSubnav({ routes }: { routes: Routes }) {
  ...
  const items = [
    ...
    { href: routes.section.connect, label: "Connect apps" },
