...
/**
 * A safe `return_to` must begin with a clean owner-route prefix (as the whole
 * path or followed by a `/`, `?`, or `#`), so an attacker cannot smuggle
 * `/owner/login`-style loops or arbitrary same-origin paths through login.
 */
