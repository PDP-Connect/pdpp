...
/**
 * Read-resilience acceptance invariants for the grants segment, mirroring
 ...
 * pattern enforces; this file pins the same properties for `/grants`.
 */
...
const RETIRED_READ_FAILURE_FRAMING_RE = /read failure|unchanged/i;
const RETIRED_BACK_LINK_RE = /Back to grants/;
...
const RECOVERING_TESTID_RE = /data-testid="grants-read-recovering"/;
...
test("the boundary renders the same loading skeleton the route's loading.tsx uses, not a bespoke banner", async () => {
  ...
  // loading.tsx uses ListLoadingSkeleton label="grants" rows={6}.
  assert.match(src, /ListLoadingSkeleton label="grants" rows=\{6\}/);
});
