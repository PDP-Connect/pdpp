 ...
 *   4. Child grant rows link to `/grants/[grantId]` with
...
const CHILD_HREF_RE = /\/grants\/\$\{encodeURIComponent\(child\.grant_id\)\}/;
...
const CUMULATIVE_LINEAGE_LINK_RE = /\/grants\/packages\/\$\{encodeURIComponent\(member\.package_id\)\}/;
const PARENT_LINK_RE = /\/grants\/packages\/\$\{encodeURIComponent\(pkg\.parent_package_id\)\}/;
