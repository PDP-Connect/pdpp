...
/**
 * Read-resilience acceptance invariants for the event-subscriptions segment,
...
const RETIRED_BACK_LINK_RE = /Back to event subscriptions/;
...
const RECOVERING_TESTID_RE = /data-testid="event-subscriptions-read-recovering"/;
...
test("the boundary renders the same loading skeleton the route's loading.tsx uses, not a bespoke banner", async () => {
  ...
  // loading.tsx uses ListLoadingSkeleton label="event subscriptions" rows={6}.
  assert.match(src, /ListLoadingSkeleton label="event subscriptions" rows=\{6\}/);
});
