...
function ReproduceToggle({ flowId, format }: { flowId: string; format: ReproduceFormat }) {
  const base = `/dashboard/deployment/tokens?flow=${encodeURIComponent(flowId)}`;
  return (
    ...
    <div className="rounded-md border border-border" data-surface="human">
      ...
      <ul className="divide-y divide-border/70">
        {tokens.map((token) => {
          ...
          return (
            <li
              className={`flex flex-col gap-2 px-5 py-3 sm:flex-row sm:items-center sm:justify-between ${isHighlight ? "bg-muted/30" : ""}`}
              key={token.client_id}
            >
