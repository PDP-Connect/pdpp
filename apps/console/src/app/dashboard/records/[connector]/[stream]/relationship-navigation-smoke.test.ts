...
// .../records/[connector]/[stream]/ -> repo root is six levels up from console
...
test("DIRECTION 1 transaction -> account: detail-page chain yields the parent account detail href", async () => {
  ...
  assert.equal(
    link?.href,
    `/dashboard/records/${CONNECTION_ID}/accounts/${ACCOUNT_KEY}`,
...
test("DIRECTION 2 account -> transactions: list-page chain yields the filtered child-list href", async () => {
  ...
  assert.equal(
    toTransactions?.href,
    `/dashboard/records/${CONNECTION_ID}/transactions?filter[account_id]=${ACCOUNT_KEY}`,
