...
const RECORD_INSPECTOR_FILE = fileURLToPath(
  new URL("apps/console/src/app/dashboard/components/record-inspector.tsx", REPO_ROOT)
);
...
// <Timestamp precision="time" value={entry.displayAt}> render + its CSS treatment.
const ROW_TIME_RENDER_RE = /<Timestamp className="rr-x-row__time" precision="time" value=\{entry\.displayAt\} \/>/;
...
test("Explore rows expose visible inspect/open selection affordances", async () => {
  ...
  assert.match(
    src,
    ROW_TIME_RENDER_RE,
    "feed rows must render a per-record time-of-day (R2: Tim's 'no timestamp on the row')"
  );
