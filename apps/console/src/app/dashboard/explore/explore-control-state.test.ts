...
test("buildCompleteStreamHref preserves source scope and exact filters for the full-set route", () => {
  ...
  assert.equal(url.pathname, "/dashboard/records/conn_gmail_personal/messages%2Fthreads");
...
test("buildCompleteStreamHref falls back to connector id and does not invent unsupported filters", () => {
  ...
  assert.equal(href, "/dashboard/records/github/issues");
});
...
test("buildCompleteStreamHref preserves server-supported sort order", () => {
  ...
  assert.equal(
    buildCompleteStreamHref("/dashboard/records", subject, { order: "oldest" }),
    "/dashboard/records/conn_gmail_personal/messages?order=asc"
  );
  assert.equal(
    buildCompleteStreamHref("/dashboard/records", subject, { order: "newest" }),
    "/dashboard/records/conn_gmail_personal/messages?order=desc"
  );
});

test("buildRecordDetailHref builds the record-detail route from clean path segments (no sort query)", () => {
  const href = buildRecordDetailHref("/dashboard/records", {
    connectionId: "cin_bc1efca69a1c386d610f0924",
    connectorId: "usaa",
    stream: "transactions",
    recordId: "d495b98f5bfe6ce1ae10c465aeb607b5",
  });
  assert.equal(href, "/dashboard/records/cin_bc1efca69a1c386d610f0924/transactions/d495b98f5bfe6ce1ae10c465aeb607b5");
});
...
test("buildRecordDetailHref reproduce-the-bug: no query string, record key is the FINAL path segment", () => {
  ...
  assert.equal(href, `/dashboard/records/gmail/messages/${encodeURIComponent(recordId)}`);
});
...
test("buildRecordDetailHref falls back to connectorId when connection identity is unknown (search rows)", () => {
  ...
  assert.equal(href, "/dashboard/records/github/issues/42");
});
