...
test("cancel control copy states it stops only the current run and preserves records/schedule/grants/config", async () => {
  const src = await readFile(CONTROL_FILE, "utf8");
...
const ACTION_REVALIDATES_RE = /revalidatePath\(`\/dashboard\/runs\/\$\{trimmed\}`\)/;
