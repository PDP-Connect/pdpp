...
function statusLabelPattern(labels) {
    return new RegExp(`^\\*\\*(?:${labels.join("|")}):\\*\\*.*$`);
}
function stripStatusBlock(body, labels) {
...
function extractProgrammeBody(rootText, docName, statusLabels) {
    const lines = rootText.split("\n");
    if (!lines[0]?.startsWith("# ")) {
...
function requireHeaderMatch(sourceText, sourceName, label) {
    const match = sourceText.match(new RegExp(`^\\*\\*${label}:\\*\\*\\s*(.+?)\\s*$`, "m"));
    const value = match?.[1]?.trim();
    if (!value) {
        throw new Error(`sync-spec-docs: ${sourceName} header ${label} line did not match the expected shape. ` +
