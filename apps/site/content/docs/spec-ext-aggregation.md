...
<Callout type="info" title="Spec status">
  ...
  Scope: Optional per-stream aggregation capability at `GET /v1/streams/{stream}/aggregate`; not required for Core conformance; support declared in stream metadata (Core §11 Extensions).
</Callout>
