...
import { isPlaceholder, siteConfig, siteFlags } from "@/lib/site-config.ts";
...
export default async function Page() {
  const [register, trustRegistries] = await Promise.all([
    readJson<RegisterEntry>("register", "index.json"),
    readJson<TrustRegistry>("register", "trust-registries.json"),
  ]);
  ...
  return (
    <PdppConceptPage>
      <PdppConceptDoc>
        ...
        <VerificationSection />
        <RegisterSection register={register} trustRegistries={trustRegistries} />
        <ChannelsSection />
      </PdppConceptDoc>
    </PdppConceptPage>
  );
...
function RegisterSection({
  ...
  trustRegistries: readonly TrustRegistry[];
}) {
  return (
    <PdppConceptSection id="register" title="The register">
      ...
      <Text as="p" className="mt-3" color="subtle" family="mono" size="stamp">
        ...
        <Link className="hover:text-primary" href="/register/index.json">
          /register/index.json
        </Link>{" "}
      ...
      <Text as="p" className="mt-2 max-w-[68ch]" color="muted" size="small" wrap="pretty">
        A current entry on one of these stands in place of a KYB-style check when an organisation applies for a Verified
        status.
      </Text>
      ...
      <Text as="p" className="mt-3" color="subtle" family="mono" size="stamp">
        <Link className="hover:text-primary" href="/register/trust-registries.json">
    ...
    <PdppConceptSection id="channels" title="Channels and contacts">
      <div className="mt-6 grid grid-cols-1 gap-px md:grid-cols-2">
        ...
        <PdppEmailContactCard
          body="A wrong description, a name that should not be listed, anything about a status. Answered within five working days."
