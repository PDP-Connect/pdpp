...
- **CTAs / specificity:** keep `pdpp-cta*` classes — `.pdpp-editorial a` beats weak TW. Copy rhythm: parent `flex flex-col gap-*` on `Text` stacks (default); per-`Text` `mb-*!` when you need to break the rule — [styling-in-apps.md](../../docs/design-system/styling-in-apps.md#migrating-editorial-type-bem--text).
