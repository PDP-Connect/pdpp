...
function DecidedState({
  ...
  return (
    <div style={{ maxWidth: "440px" }}>
      ...
      <button className="mt-2 px-0.5 font-mono text-xs" style={{ color: "var(--muted-foreground)" }} onClick={onReset}>
        ↺ reset
      </button>
    </div>
  );
    ...
    <div style={{ borderBottom: "1px solid var(--border)" }}>
      <div className="flex items-center gap-3 py-2.5">
        ...
        >
          ...
          <Chevron open={expanded} opacity={inactiveOpacity} />
        </button>
      </div>
      {expanded && (
        <div
          className="mb-2 pl-10 text-xs"
          style={{ color: "var(--muted-foreground)", opacity: enabled ? 1 : 0.4 }}
        >
          {optional.detail}
        </div>
      )}
