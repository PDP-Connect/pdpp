...
function HeaderActions({ ownerLoginUrl }: { ownerLoginUrl: string }) {
  return (
    <>
      <Link href="/dashboard/grants#pending-approvals" className={buttonVariants({ variant: "outline", size: "sm" })}>
        Pending approvals
      </Link>
...
function FlowDetails({ flow, examples }: { examples: FlowExamples | null; flow: FlowState }) {
  return (
    <>
      {flow.lastError ? <InlineError prefix="Last action error" message={flow.lastError} /> : null}
...
export default async function OwnerTokenBootstrapPage({ searchParams }: { searchParams: Promise<Params> }) {
  ...
  return (
    <DashboardShell active="grants">
      ...
      {error ? <InlineError prefix="Device-flow error" message={error} /> : null}

      <StartFlowSection flow={flow} />

      {flow ? <FlowDetails flow={flow} examples={examples} /> : <EmptyFlowInstructions />}
    </DashboardShell>
  );
}
