...
function HeaderMeta({ flow }: { flow: FlowState | null }) {
  return (
    <>
      <MetaPill
        label="flow"
...
function VerificationLink({ flow }: { flow: FlowState }) {
  const href = flow.verificationUriComplete ?? flow.verificationUri;
  if (!href) {
    return <>—</>;
  }
  return (
...
function StartFlowSection({ flow }: { flow: FlowState | null }) {
  return (
    <Section
    ...
    >
      <form action={startOwnerTokenFlowAction} className="flex flex-col gap-3 sm:flex-row sm:items-end">
        <label className="flex min-w-0 flex-1 flex-col gap-1" htmlFor="bootstrap-client-id">
      ...
      <p className="pdpp-caption mt-3 text-muted-foreground">
        ...
        the{" "}
        <Link className="underline-offset-2 hover:underline" href="/dashboard/grants/request">
...
function CurrentFlowSection({ flow }: { flow: FlowState }) {
  return (
    <Section title="Current flow">
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        ...
        <DetailCard title="Device authorization">
          ...
          <DetailRow label="verification" value={<VerificationLink flow={flow} />} />
        </DetailCard>
        <DetailCard title="Approval">
          <form className="space-y-3">
            <input name="flow_id" type="hidden" value={flow.flowId} />
            <label className="flex flex-col gap-1" htmlFor="bootstrap-subject-id">
              <span className="pdpp-eyebrow">Subject id</span>
              <Input
                defaultValue={flow.subjectId ?? "owner_local"}
                id="bootstrap-subject-id"
                name="subject_id"
                type="text"
              />
            </label>
            <div className="flex flex-wrap gap-2">
              <Button formAction={approveOwnerTokenFlowAction} size="sm" type="submit">
                <span className="font-mono">POST /device/approve</span>
              </Button>
              <Button formAction={denyOwnerTokenFlowAction} size="sm" type="submit" variant="destructive">
          ...
          <p className="pdpp-caption mt-2 inline-flex flex-wrap items-baseline gap-1 text-muted-foreground">
            State: <StatusBadge inline status={flow.status} />
...
function ExchangeIntrospectSection({ flow }: { flow: FlowState }) {
  return (
...
function EquivalentsSection({ examples }: { examples: FlowExamples }) {
  return (
    <Section description="Reproduce the full flow outside the dashboard." title="3. Equivalents">
...
function FlowDetails({ flow, examples }: { examples: FlowExamples | null; flow: FlowState }) {
  return (
    <>
      {flow.lastError ? <InlineError message={flow.lastError} prefix="Last action error" /> : null}
      <CurrentFlowSection flow={flow} />
      <ExchangeIntrospectSection flow={flow} />
      {examples ? <EquivalentsSection examples={examples} /> : null}
    </>
  );
}

function EmptyFlowInstructions() {
  return (
  ...
  return (
    <DashboardShell active="grants">
      <PageHeader
        ...
        meta={<HeaderMeta flow={flow} />}
        title="Owner device flow"
      />

      {error ? <InlineError message={error} prefix="Device-flow error" /> : null}

      <StartFlowSection flow={flow} />

      {flow ? <FlowDetails examples={examples} flow={flow} /> : <EmptyFlowInstructions />}
    </DashboardShell>
  );
}
...
function DetailCard({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="rounded-md p-4" data-surface="human">
