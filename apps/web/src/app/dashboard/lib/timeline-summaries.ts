...
function firstString(data: RecordData, skipKeys: string[] = ["id"]): string {
  for (const [k, v] of Object.entries(data)) {
    if (skipKeys.includes(k) || ID_FIELD.test(k)) {
      continue;
    }
    if (typeof v !== "string" || !v.trim()) {
      continue;
    }
    if (UUID_LIKE.test(v.trim())) {
      continue;
    }
    return s(v);
  }
...
const SUMMARIES: Record<string, SummaryFn> = {
  ...
  "ynab::months": (d) => {
    ...
    const parts = [month];
    if (income) {
      parts.push(`income ${income}`);
    }
    if (budgeted) {
      parts.push(`budgeted ${budgeted}`);
    }
    return parts.join(" — ");
  },
...
function formatAmount(milli: number): string {
  ...
  const n = Math.abs(milli) > 10_000 ? milli / 1000 : milli;
  const sign = n < 0 ? "-" : "";
