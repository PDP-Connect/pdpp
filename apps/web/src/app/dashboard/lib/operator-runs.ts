...
function parseDurationInput(value: string, label: string): number {
  ...
  if (!match) {
    throw new Error(
      `Invalid ${label} value '${trimmed}'. Use values like 30m, 60s, 2h, or 1d.`,
    );
  }
...
export async function saveConnectorSchedule(
  ...
  const body = {
    ...
    ...(input.jitter?.trim()
      ? { jitter_seconds: parseDurationInput(input.jitter, 'schedule jitter') }
      : {}),
  };
...
export async function pauseConnectorSchedule(connectorId: string) {
  const response = await fetchAs(
    `/_ref/connectors/${encodeURIComponent(connectorId)}/schedule/pause`,
    {
      method: 'POST',
    },
  );
...
export async function resumeConnectorSchedule(connectorId: string) {
  const response = await fetchAs(
    `/_ref/connectors/${encodeURIComponent(connectorId)}/schedule/resume`,
    {
      method: 'POST',
    },
  );
