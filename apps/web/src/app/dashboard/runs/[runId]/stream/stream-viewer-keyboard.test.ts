...
const VIEWER_REMOTE_INPUT_FOCUS_VIA_ADAPTER_RE =
  /const applyRemoteInputFocus = \(focused: boolean\)[\s\S]*adapter\.setRemoteInputFocused\(focused\)[\s\S]*adapter\.focusTextInput\(\)[\s\S]*adapter\.blurTextInput\(\)/;
...
const NEKO_SETTLING_DISPLAYABLE_BLOCK_RE = /if \(displayable && !mediaDisplayableRef\.current\) \{[\s\S]*?\n {8}\}/;
const SET_MEDIA_READY_RE = /setMediaReady\(true\)/;
const PRESENTATION_VIEWPORT_READY_RE = /onPresentationViewportReady\(/;
...
const PLAYGROUND_LAYOUT_CLAIM_RE =
  /function emitPlaygroundEvents[\s\S]+?claimPlaygroundEvent\(playgroundSeenRef\.current,/;
const DEBUG_DRAIN_ENABLED_RE = /if \(!debugEnabled\) \{\s*return;\s*\}[\s\S]+?fetchNekoStatusBestEffort/;
const DEBUG_DRAIN_DUPLICATE_RE = /claimPlaygroundEvent\(playgroundSeenRef\.current,[\s\S]+?===\s*"duplicate"/;
const DEBUG_DRAIN_POLL_MS_RE = /nekoDebugDrainPollMs:\s*250,/;
const DEBUG_DRAIN_POLL_TIMER_RE = /setTimeout\(drainOnce, STREAM_VIEWER_POLICY\.nekoDebugDrainPollMs\)/;
const STATUS_POLL_TIMER_RE = /setTimeout\(drainOnce, STREAM_VIEWER_POLICY\.nekoStatusPollMs\)/;
const DEBUG_DRAIN_BLOCK_RE = /Debug-only drain[\s\S]+?\}, \[clientConfig\?\.statusPath, debugEnabled, logDebug\]\);/;
const APPLY_SCREEN_RE = /applyScreen\(/;
const APPLY_FALLBACK_RE = /applyFallback\(/;
const FIRST_NEKO_LAYOUT_APPLIED_RE = /firstNekoLayoutAppliedRef/;
...
const NEKO_BACKEND_READY_RE =
  /payload\.backend === "neko"[\s\S]*nekoNativeViewportRef\.current = true[\s\S]*requestViewportMeasureRef\.current\?\.\("neko-backend-ready"\)/;
const VIEWPORT_POST_READ_STAGE_RE = /const viewport = readStageViewport\(width, height\)/;
const RESIZE_READ_STAGE_RE = /const viewport = readStageViewport\(rect\.width, rect\.height\)/;
...
test("n.eko presentation waits for settled media before promoting a resized viewport", async () => {
  ...
  const displayableBlock = viewerSrc.match(NEKO_SETTLING_DISPLAYABLE_BLOCK_RE)?.[0] ?? "";
  assert.ok(displayableBlock.length > 0, "settling displayable block is identifiable");
  assert.doesNotMatch(displayableBlock, SET_MEDIA_READY_RE);
  assert.doesNotMatch(displayableBlock, PRESENTATION_VIEWPORT_READY_RE);
});
...
test("stream viewer drains remote playground.* events when stream_debug=1, with page-scoped dedupe against the layout poll", async () => {
  ...
  assert.match(viewerSrc, PLAYGROUND_LAYOUT_CLAIM_RE, "layout poll claims events through claimPlaygroundEvent");
  ...
  assert.match(
    viewerSrc,
    DEBUG_DRAIN_POLL_MS_RE,
    "debug drain has a dedicated 250ms cadence (not the 50ms layout-poll cadence)"
  );
  assert.match(viewerSrc, DEBUG_DRAIN_POLL_TIMER_RE, "debug drain useEffect uses nekoDebugDrainPollMs");
  assert.doesNotMatch(
    viewerSrc,
    STATUS_POLL_TIMER_RE,
  ...
  assert.doesNotMatch(drainBlock, APPLY_SCREEN_RE, "debug drain must not call applyScreen");
  assert.doesNotMatch(drainBlock, APPLY_FALLBACK_RE, "debug drain must not call applyFallback");
  assert.doesNotMatch(drainBlock, FIRST_NEKO_LAYOUT_APPLIED_RE, "debug drain must not mark first-layout flag");
...
test("n.eko follow-up viewport posts use native one-to-one coordinates", async () => {
  ...
  assert.match(
    viewerSrc,
    NEKO_BACKEND_READY_RE,
    "n.eko backend_ready immediately remeasures/reposts using native viewport options"
  );
  assert.match(viewerSrc, VIEWPORT_POST_READ_STAGE_RE, "viewport POST path uses readStageViewport");
  assert.match(viewerSrc, RESIZE_READ_STAGE_RE, "resize/presentation paths use readStageViewport");
});
