...
const DELAYED_PDPP_TAP_RE = /window\.setTimeout\([^)]*clickNekoAtPoint/;
const NATIVE_TAP_ASSIST_RE = /native_tap_assist/;
const NATIVE_TAP_OBSERVED_RE = /native_tap_observed/;
const CSS_VIEWPORT_DIVISOR_RE = /controlCoordinateSize\.width\s*\/\s*rect\.width/;
const NEKO_GET_MOUSE_POS_RE = /nekoInstance\?\._overlay\?\.getMousePos/;
const CONTAINER_OVERRIDE_MIN_WIDTH_DELTA_RE = /NEKO_CONTAINER_OVERRIDE_MIN_WIDTH_DELTA_PX\s*=\s*\d+/;
const CONTAINER_OVERRIDE_HEIGHT_ONLY_SKIP_RE = /container_rect_override\.skipped[\s\S]{0,200}height-only-delta/;
const READ_TOUCH_TIME_VIEWPORT_RE = /function readTouchTimeViewport\(/;
const TOUCH_VIEWPORT_WINDOW_RE = /window:\s*\{/;
const TOUCH_VIEWPORT_VISUAL_VIEWPORT_RE = /visualViewport:/;
const TOUCH_VIEWPORT_DOCUMENT_ELEMENT_RE = /documentElement:/;
const TOUCH_VIEWPORT_SCROLLING_RE = /scrolling:/;
const TOUCH_VIEWPORT_ORIENTATION_RE = /orientation:/;
const TOUCH_VIEWPORT_SCALE_RE =
  /scale:\s*Number\.isFinite\(visualViewport\.scale\)\s*\?\s*visualViewport\.scale\s*:\s*null/;
const POINTER_MAPPING_VIEWPORT_RE = /viewport:\s*readTouchTimeViewport\(\)/;
...
const NATIVE_TAP_INTERACTION_SEQ_RE =
...
const FOCUS_INTERVAL_SOFT_KEYBOARD_RE = /isPdppSoftKeyboardElement\(active\)/;
const FOCUS_INTERVAL_SKIP_RE =
...
test("n.eko getNekoControlPos prefers n.eko-authoritative coordinate basis (no PDPP CSS-viewport remap)", async () => {
  ...
  assert.match(fn, NEKO_GET_MOUSE_POS_RE, "getNekoControlPos prefers n.eko-authoritative getMousePos as first mapping");
});
...
test("n.eko containerRect override skips height-only deltas (keyboard / visualViewport churn)", async () => {
  ...
  assert.match(
    src,
    CONTAINER_OVERRIDE_MIN_WIDTH_DELTA_RE,
    "container-rect override has an explicit minimum-width-delta gate"
  );
  assert.match(
    src,
    CONTAINER_OVERRIDE_HEIGHT_ONLY_SKIP_RE,
...
test("n.eko touch-time pointer mapping captures viewport basis (window/visualViewport/documentElement/scrolling/orientation)", async () => {
  ...
  assert.match(src, READ_TOUCH_TIME_VIEWPORT_RE, "readTouchTimeViewport helper exists");
  // The helper must emit each of the four basis surfaces.
  const fn = src.split("function readTouchTimeViewport(")[1]?.split("\nfunction ")[0] ?? "";
  assert.match(fn, TOUCH_VIEWPORT_WINDOW_RE, "readTouchTimeViewport reports window dims");
  assert.match(
    fn,
    TOUCH_VIEWPORT_VISUAL_VIEWPORT_RE,
    "readTouchTimeViewport reports visualViewport (incl. offsetTop/scale)"
  );
  assert.match(
    fn,
    TOUCH_VIEWPORT_DOCUMENT_ELEMENT_RE,
    "readTouchTimeViewport reports documentElement clientWidth/clientHeight"
  );
  assert.match(fn, TOUCH_VIEWPORT_SCROLLING_RE, "readTouchTimeViewport reports scrollLeft/scrollTop");
  assert.match(fn, TOUCH_VIEWPORT_ORIENTATION_RE, "readTouchTimeViewport reports orientation type/angle");
  assert.match(fn, TOUCH_VIEWPORT_SCALE_RE, "visualViewport.scale captured (catches pinch zoom mid-touch)");
  ...
  assert.match(mapFn, POINTER_MAPPING_VIEWPORT_RE, "readNekoPointerMapping includes the touch-time viewport snapshot");
});
...
test("n.eko touch.start event captures the full local→remote mapping with an interaction sequence", async () => {
  ...
  assert.match(src, NATIVE_TAP_INTERACTION_SEQ_RE, "native tap observed event carries interactionSeq");
  assert.match(
    src,
