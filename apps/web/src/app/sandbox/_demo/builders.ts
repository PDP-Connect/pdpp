 ...
 * lockstepped.
 *
 * Shape note: shapes are inspired by the live reference's public/RS and
 * `_ref` envelopes (see `apps/web/src/app/dashboard/lib/ref-client.ts`) but
 * are deliberately demo-shaped — every envelope advertises that it is a
 * sandbox-only mock.
 */
...
// ─── Grants / runs / traces summaries + timelines ──────────────────────────
...
export function buildDatasetSummary(): DatasetSummary {
  const recordTimes = DEMO_RECORDS.map((r) => r.record_time).sort();
  let earliest: string | null = null;
  let latest: string | null = null;
  if (recordTimes.length > 0) {
    earliest = recordTimes[0] ?? null;
    latest = recordTimes.at(-1) ?? null;
  }
  ...
  // Approximate retained-bytes — deterministic, fictional.
  const blobBytes = DEMO_RECORDS.reduce((sum, r) => sum + JSON.stringify(r.fields).length, 0);
  return {
    object: "dataset_summary",
    is_demo: true,
    notice: DEMO_NOTICE.notice,
