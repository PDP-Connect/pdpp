...
import { buildPeekReadUrl } from "@/app/dashboard/explore/peek-read-url.ts";
import { attributeSearchHit, shouldIncludeSearchHit } from "@/app/dashboard/explore/search-hit-attribution.ts";
import { sandboxDashboardDataSource } from "../_demo/data-source.ts";

export const dynamic = "force-dynamic";

// Empty-query fan-out caps (mirrored from the live explore page).
        ...
        sandboxDashboardDataSource
          ...
          .then(
            (page): StreamFetchResult => ({
              ok: true,
              entries: page.data
                .map((record): ExplorerFeedEntry | null => {
                  const data = recordData(record.data);
                  ...
                  return {
                    ...
                    connectionDisplayName:
                      summary.display_name || summary.connector_display_name || summary.connector_id,
                    stream: streamName,
                    recordId: record.id,
                    emittedAt: record.emitted_at,
...
async function loadSearchFeed(
  ...
  const warnings: ExplorerWarning[] = [];
  const page = await sandboxDashboardDataSource.searchRecordsLexical(query, { limit: SEARCH_PAGE_LIMIT });
  ...
  return { entries, fromSearch: true, hybridUsed: false, truncated: false, warnings };
}
...
async function dispatchFeed(args: {
  ...
  if (query) {
    const feed = await loadSearchFeed(query, filteredSummaries, filterStreamSet, timestampMetadata, filterConnectionSet);
...
async function buildTimestampMetadata(): Promise<Map<string, SearchTimestampMetadata>> {
  ...
  for (const manifest of await sandboxDashboardDataSource.listConnectorManifests()) {
...
async function buildPeek(
  ...
  let connection: RefConnectorSummary | null = null;
  if (parsed.connectionId) {
    connection = byConnectionId.get(parsed.connectionId) ?? null;
  }
  if (!connection) {
  ...
  try {
    const record = await sandboxDashboardDataSource.getRecord(parsed.connectorId, parsed.stream, parsed.recordId);
    return {
      connectorId: parsed.connectorId,
...
export default async function SandboxExplorePage({
  ...
  const peek = await buildPeek(params.peek, summaryByConnectionId(summaries));
